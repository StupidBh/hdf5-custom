...
namespace H5 {
    ...
    //--------------------------------------------------------------------------
    AtomType::AtomType(const hid_t existing_id) :
    ...
    //--------------------------------------------------------------------------
    AtomType::AtomType(const AtomType& original) :
        DataType(original)
    {
    }
    ...
    size_t AtomType::getPrecision() const
    {
        ...
        if (num_signi_bits == 0) {
            throw DataTypeIException(inMemFunc("getPrecision"), "H5Tget_precision returns invalid number of significant bits");
        }
