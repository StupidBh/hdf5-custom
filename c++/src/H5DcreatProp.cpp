...
namespace H5 {
    ...
    DSetCreatPropList::DSetCreatPropList() :
        ObjCreatPropList(H5P_DATASET_CREATE)
    {
    }
    ...
    H5D_fill_value_t DSetCreatPropList::isFillValueDefined() const
    {
        ...
        if (ret_value < 0) {
            throw PropListIException("DSetCreatPropList::isFillValueDefined", "H5Pfill_value_defined returned H5D_FILL_VALUE_ERROR (-1)");
        }
        else {
            return (status);
        }
    ...
    int DSetCreatPropList::getNfilters() const
    {
        ...
        if (num_filters < 0) {
            throw PropListIException("DSetCreatPropList::getNfilters", "H5Pget_nfilters returned negative number of filters");
        }
        else {
            return (num_filters);
        }
                                              ...
                                              unsigned int& filter_config) const
    {
        ...
        H5Z_filter_t filter_id = H5Pget_filter2(id, static_cast<unsigned>(filter_number), &flags, &cd_nelmts, cd_values, namelen, name, &filter_config);
    ...
    void DSetCreatPropList::getFilterById(H5Z_filter_t filter_id,
                                          ...
                                          unsigned int& filter_config) const
    {
        herr_t ret_value = H5Pget_filter_by_id2(id, filter_id, &flags, &cd_nelmts, cd_values, namelen, name, &filter_config);
        if (ret_value < 0) {
    ...
    //--------------------------------------------------------------------------
    void DSetCreatPropList::setExternal(const char* name, HDoff_t offset, hsize_t size) const
    {
    ...
    int DSetCreatPropList::getExternalCount() const
    {
        ...
        if (num_ext_files < 0) {
            throw PropListIException("DSetCreatPropList::getExternalCount", "H5Pget_external_count returns negative number of external files");
        }
        else {
    ...
    ///\version     2.0.0 \p offset parameter type changed to HDoff_t from off_t.
    //--------------------------------------------------------------------------
    void DSetCreatPropList::getExternal(unsigned idx, size_t name_size, char* name, HDoff_t& offset, hsize_t& size) const
    {
