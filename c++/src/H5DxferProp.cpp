...
namespace H5 {
    ...
    size_t DSetMemXferPropList::getBuffer(void** tconv, void** bkg) const
    {
        ...
        if (buffer_size == 0) {
            throw PropListIException("DSetMemXferPropList::getBuffer", "H5Pget_buffer returned 0 for buffer size - failure");
        }
        return (buffer_size);
    }
    ...
    bool DSetMemXferPropList::getPreserve() const
    {
        ...
        if (ret_value > 0) {
            ...
            throw PropListIException("DSetMemXferPropList::getPreserve", "H5Pget_preserve returned negative value for status");
        }
    }
    ...
    void DSetMemXferPropList::setSmallDataBlockSize(hsize_t size) const
    {
        ...
        if (ret_value < 0) {
            throw PropListIException("DSetMemXferPropList::setSmallDataBlockSize", "H5Pset_small_data_block_size failed");
        }
    }
    ...
    hsize_t DSetMemXferPropList::getSmallDataBlockSize() const
    {
        ...
        if (ret_value < 0) {
            throw PropListIException("DSetMemXferPropList::getSmallDataBlockSize", "H5Pget_small_data_block_size failed");
        }
        return (size);
    }
    ...
    void DSetMemXferPropList::setHyperVectorSize(size_t vector_size) const
    {
        ...
        if (ret_value < 0) {
            throw PropListIException("DSetMemXferPropList::setHyperVectorSize", "H5Pset_hyper_vector_size failed");
        }
    }
    ...
    size_t DSetMemXferPropList::getHyperVectorSize() const
    {
        ...
        if (ret_value < 0) {
            throw PropListIException("DSetMemXferPropList::getHyperVectorSize", "H5Pget_hyper_vector_size failed");
        }
        return (vector_size);
    }
