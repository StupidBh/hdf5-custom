...
namespace H5 {
    ...
    ///\exception   H5::FileIException/H5::GroupIException/H5::LocationException
    //--------------------------------------------------------------------------
    DataSet H5Location::createDataSet(const char* name,
                                      const DataType& data_type,
                                      const DataSpace& data_space,
                                      const DSetCreatPropList& dcpl,
                                      const DSetAccPropList& dapl,
    ...
    ///             \c H5std_string for \a name.
    //--------------------------------------------------------------------------
    DataSet H5Location::createDataSet(const H5std_string& name,
                                      const DataType& data_type,
                                      const DataSpace& data_space,
                                      const DSetCreatPropList& dcpl,
                                      const DSetAccPropList& dapl,
    ...
    //--------------------------------------------------------------------------
    void H5Location::link(const char* target_name, const char* link_name, const LinkCreatPropList& lcpl, const LinkAccPropList& lapl) const
    {
        herr_t ret_value = -1;
        hid_t lcpl_id = lcpl.getId();
    ...
    ///\exception   H5::FileIException or H5::GroupIException
    //--------------------------------------------------------------------------
    void H5Location::link(const H5std_string& target_name, const H5std_string& link_name, const LinkCreatPropList& lcpl, const LinkAccPropList& lapl) const
    {
    ...
    //--------------------------------------------------------------------------
    void H5Location::link(const char* curr_name, const Group& new_loc, const char* new_name, const LinkCreatPropList& lcpl, const LinkAccPropList& lapl) const
    {
        herr_t ret_value = -1;
    ...
    void H5Location::link(H5L_type_t link_type, const char* curr_name, const char* new_name) const
    {
        ...
        switch (link_type) {
        case H5L_TYPE_HARD: ret_value = H5Lcreate_hard(getId(), curr_name, H5L_SAME_LOC, new_name, H5P_DEFAULT, H5P_DEFAULT); break;

        case H5L_TYPE_SOFT: ret_value = H5Lcreate_soft(curr_name, getId(), new_name, H5P_DEFAULT, H5P_DEFAULT); break;

        case H5L_TYPE_ERROR:
        case H5L_TYPE_EXTERNAL:
        case H5L_TYPE_MAX:
        default               : throwException("link", "unknown link type"); break;
        } /* end switch */
    ...
    ///\exception   H5::FileIException or H5::GroupIException
    //--------------------------------------------------------------------------
    void H5Location::copyLink(const char* src_name, const Group& dst, const char* dst_name, const LinkCreatPropList& lcpl, const LinkAccPropList& lapl) const
    {
        herr_t ret_value;
    ...
    ///\exception   H5::FileIException or H5::GroupIException
    //--------------------------------------------------------------------------
    void H5Location::copyLink(const char* src_name, const char* dst_name, const LinkCreatPropList& lcpl, const LinkAccPropList& lapl) const
    {
        herr_t ret_value;
        hid_t lcpl_id = lcpl.getId();
        hid_t lapl_id = lapl.getId();
    ...
    ///\exception   H5::FileIException or H5::GroupIException
    //--------------------------------------------------------------------------
    void H5Location::copyLink(const H5std_string& src_name, const H5std_string& dst_name, const LinkCreatPropList& lcpl, const LinkAccPropList& lapl) const
    {
    ...
    ///             \li \c H5O_INFO_ALL
    //--------------------------------------------------------------------------
    void H5Location::getNativeObjinfo(const char* grp_name,
                                      H5_index_t idx_type,
    ...
    ///             \li \c H5O_INFO_ALL
    //--------------------------------------------------------------------------
    void H5Location::getNativeObjinfo(const H5std_string& grp_name,
                                      H5_index_t idx_type,
                                      H5_iter_order_t order,
                                      hsize_t idx,
                                      H5O_native_info_t& objinfo,
    ...
    ///             \li \c H5O_INFO_ALL
    //--------------------------------------------------------------------------
    void H5Location::getObjinfo(const char* grp_name,
                                H5_index_t idx_type,
                                H5_iter_order_t order,
                                hsize_t idx,
                                H5O_info2_t& objinfo,
    ...
    {
        // Use C API to get information of the object
        herr_t ret_value = H5Oget_info_by_idx3(getId(), grp_name, idx_type, order, idx, &objinfo, fields, lapl.getId());
    ...
    ///\param       lapl     - IN: Link access property list
    //--------------------------------------------------------------------------
    void H5Location::getObjinfo(const H5std_string& grp_name,
                                H5_index_t idx_type,
                                H5_iter_order_t order,
                                hsize_t idx,
                                H5O_info2_t& objinfo,
    ...
    //--------------------------------------------------------------------------
    void H5Location::getObjinfo(const char* name, bool follow_link, H5G_stat_t& statbuf) const
    {
    ...
    ///             \c H5std_string for \a name.
    //--------------------------------------------------------------------------
    void H5Location::getObjinfo(const H5std_string& name, bool follow_link, H5G_stat_t& statbuf) const
    {
    ...
    H5std_string H5Location::getObjnameByIdx(hsize_t idx) const
    {
        ...
        name_len = H5Lget_name_by_idx(getId(), ".", H5_INDEX_NAME, H5_ITER_INC, idx, name_C, actual_name_len, H5P_DEFAULT);
    ...
    H5O_type_t H5Location::childObjType(const char* objname) const
    {
        ...
        // Return a valid type or throw an exception for unknown type
            ...
            case H5O_TYPE_NAMED_DATATYPE: objtype = objinfo.type; break;
            case H5O_TYPE_UNKNOWN       :
            case H5O_TYPE_NTYPES        :
            case H5O_TYPE_MAP           :
            default                     : throwException("childObjType", "Unknown type of object");
            }
        }
        return (objtype);
    }
    ...
    H5G_obj_t H5Location::getObjTypeByIdx(hsize_t idx, H5std_string& type_name) const
    {
        ...
        case H5G_RESERVED_6:
        case H5G_RESERVED_7:
        default            : throwException("getObjTypeByIdx", "H5Gget_objtype_by_idx failed");
        }
        return (obj_type);
    }
