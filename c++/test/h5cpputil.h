...
void verify_val(Type1 x, Type2 value, const char* msg, const char* file_name, int line)
{
    if (x != value) {
        cerr << endl;
        cerr << "*** UNEXPECTED VALUE: " << file_name << ":line " << line << ": " << msg << " different: " << x << ", should be " << value << endl;
        IncTestNumErrs();
...
void verify_val_noteq(Type1 x, Type2 value, const char* where, int line, const char* file_name)
{
    ...
    if (x == value) {
        cerr << endl;
        cerr << "*** UNEXPECTED VALUE from " << where << " should not be " << value << " at line " << line << " in " << file_name << endl;
        IncTestNumErrs();
        throw TestFailedException(where, "");
    }
}
...
void verify_val(Type1 x, Type2 value, float epsilon, const char* msg, int line, const char* file_name)
{
    if (x == value) {
        cerr << endl;
        cerr << "*** UNEXPECTED FLOAT VALUE: " << file_name << ":line " << line << ": " << msg << " different: " << x << ", should be " << value
