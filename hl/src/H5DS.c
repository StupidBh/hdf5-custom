 ...
 *-------------------------------------------------------------------------
 */
herr_t H5DSwith_new_ref(hid_t obj_id, bool* with_new_ref)
{
...
herr_t H5DSattach_scale(hid_t did, hid_t dsid, unsigned int idx)
{
    ...
    ds_list_t* dsbuf_w = NULL;          /* array of "REFERENCE_LIST" attribute data to write when adding new reference to a dataset */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5DSiterate_scales(hid_t did, unsigned int dim, int* ds_idx, H5DS_iterate_t visitor, void* visitor_data)
{
    hid_t scale_id;
    int rank;
    H5R_ref_t nref;              /* reference to the DS */
...
herr_t H5DSset_label(hid_t did, unsigned int idx, const char* label)
{
    ...
    union
    {                           /* union is needed to eliminate compiler warnings about */
...
static herr_t H5DS_is_reserved(hid_t did, bool* is_reserved)
{
    ...
    if (isvl > 0) {
        ...
        *is_reserved = vl_str && (strcmp(vl_str, IMAGE_CLASS) == 0 || strcmp(vl_str, PALETTE_CLASS) == 0 || strcmp(vl_str, TABLE_CLASS) == 0);
        H5Treclaim(tid, asid, H5P_DEFAULT, &vl_str);
        H5Sclose(asid);
    }
    else {
