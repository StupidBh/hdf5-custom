 ...
 *-------------------------------------------------------------------------
 */
hid_t H5PTcreate(hid_t loc_id, const char* dset_name, hid_t dtype_id, hsize_t chunk_size, hid_t plist_id)
{
    htbl_t* table = NULL;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5PT_free_id(void* id, void H5_ATTR_UNUSED** _ctx)
{
    free(id);
    return SUCCEED;
}
...
herr_t H5PTget_next(hid_t table_id, size_t nrecords, void* data)
{
    ...
    if ((H5TB_common_read_records(table->dset_id, table->type_id, table->current_index, nrecords, table->size, data)) < 0) {
        goto error;
    }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5PT_create_index(htbl_t* table)
{
    if (table != NULL) {
...
static herr_t H5PT_set_index(htbl_t* table, hsize_t pt_index)
{
...
static herr_t H5PT_get_index(htbl_t* table, hsize_t* pt_index)
{
    /* Ensure index is valid */
    if (table != NULL) {
        if (pt_index) {
...
herr_t H5PTfree_vlen_buff(hid_t table_id, size_t _bufflen, void* buff)
{
    hid_t space_id = H5I_INVALID_HID;
    htbl_t* table;
