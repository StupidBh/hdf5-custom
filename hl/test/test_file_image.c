 ...
 *-------------------------------------------------------------------------
 */
static int test_file_image(size_t open_images, size_t nflags, const unsigned* flags)
{
    ...
    for (i = 0; i < open_images; i++) {
        ...
        if ((dset_id[i] = H5Dcreate2(file_id[i], DSET_NAME, H5T_NATIVE_INT, file_space_id, H5P_DEFAULT, dcpl_id, H5P_DEFAULT)) < 0) {
    ...
    for (i = 0; i < open_images; i++) {
        ...
        /* open rest of valid file images */
            ...
            if (input_flags[i] & H5LT_FILE_IMAGE_OPEN_RW && !(input_flags[i] & H5LT_FILE_IMAGE_DONT_COPY)) {
                 ...
                 */
                memset((uint8_t*)tmp_ptr + SUPER_STATUS_FLAGS_OFF_V0_V1, (int)0, (size_t)SUPER_STATUS_FLAGS_SIZE_V0_V1);
                /* Does the comparison */
