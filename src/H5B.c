...
/* Local Prototypes */
/********************/
static herr_t H5B_find_helper(H5F_t* f, const H5B_class_t* type, haddr_t addr, int exp_level, bool* found, void* udata);
static H5B_ins_t H5B__insert_helper(H5F_t* f,
                                    H5B_ins_ud_t* bt_ud,
...
static herr_t H5B__insert_child(H5B_t* bt, unsigned* bt_flags, unsigned idx, haddr_t child, H5B_ins_t anchor, const void* md_key);
static herr_t H5B__split(H5F_t* f, H5B_ins_ud_t* bt_ud, unsigned idx, void* udata, H5B_ins_ud_t* split_bt_ud /*out*/);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5B_create(H5F_t* f, const H5B_class_t* type, void* udata, haddr_t* addr_p /*out*/)
{
    H5B_t* bt = NULL;
    ...
    assert(shared);
    if (NULL == (bt->native = H5FL_BLK_MALLOC(native_block, shared->sizeof_keys)) || NULL == (bt->child = H5FL_SEQ_MALLOC(haddr_t, (size_t)shared->two_k))) {
        HGOTO_ERROR(H5E_BTREE, H5E_CANTALLOC, FAIL, "memory allocation failed for B-tree root node");
    }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5B_find(H5F_t* f, const H5B_class_t* type, haddr_t addr, bool* found, void* udata)
{
    herr_t ret_value = SUCCEED;
    ...
    if (cmp) {
        *found = false;
        ...
        if (bt->level > 0) {
            ...
            if ((ret_value = H5B_find_helper(f, type, bt->child[idx], (int)(bt->level - 1), found, udata)) < 0) {
...
static herr_t H5B__split(H5F_t* f, H5B_ins_ud_t* bt_ud, unsigned idx, void* udata, H5B_ins_ud_t* split_bt_ud /*out*/)
{
    ...
    H5MM_memcpy(split_bt_ud->bt->native, bt_ud->bt->native + nleft * shared->type->sizeof_nkey, (nright + 1) * shared->type->sizeof_nkey);
...
herr_t H5B_insert(H5F_t* f, const H5B_class_t* type, haddr_t addr, void* udata)
{
    ...
    /* Insert the object */
    if ((int)(my_ins = H5B__insert_helper(f, &bt_ud, type, lt_key, &lt_key_changed, md_key, udata, rt_key, &rt_key_changed, &split_bt_ud /*out*/)) < 0) {
...
static herr_t H5B__insert_child(H5B_t* bt, unsigned* bt_flags, unsigned idx, haddr_t child, H5B_ins_t anchor, const void* md_key)
{
    ...
    if ((idx + 1) == bt->nchildren) {
        ...
        H5MM_memcpy(base + shared->type->sizeof_nkey, base, shared->type->sizeof_nkey); /* No overlap possible - memcpy() OK */
                                    ...
                                    H5B_ins_ud_t* split_bt_ud /*out*/)
{
    ...
    *rt_key_changed = false;
    ...
    if (0 == bt->nchildren) {
         ...
         */
        assert(0 == bt->level);
        if ((type->new_node)(f, H5B_INS_FIRST, H5B_NKEY(bt, shared, 0), udata, H5B_NKEY(bt, shared, 1), bt->child + 0 /*out*/) < 0) {
        ...
        if (type->follow_min) {
            if ((int)(my_ins = (type->insert)(f,
                                              bt->child[idx],
                                              H5B_NKEY(bt, shared, idx),
                                              lt_key_changed,
                                              md_key,
                                              udata,
        ...
        if (bt->level > 0) {
            ...
            if ((int)(my_ins = H5B__insert_helper(f,
                                                  &child_bt_ud,
                                                  type,
                                                  H5B_NKEY(bt, shared, idx),
                                                  lt_key_changed,
            ...
            if ((type->new_node)(f, H5B_INS_LEFT, H5B_NKEY(bt, shared, idx), udata, md_key, &new_child_bt_ud.addr /*out*/) < 0) {
                HGOTO_ERROR(H5E_BTREE, H5E_CANTINSERT, H5B_INS_ERROR, "can't insert minimum leaf node");
            }
            *lt_key_changed = true;
        } /* end else */
        ...
        if (bt->level > 0) {
            ...
            if ((type->new_node)(f, H5B_INS_RIGHT, md_key, udata, H5B_NKEY(bt, shared, idx + 1), &new_child_bt_ud.addr /*out*/) < 0) {
                HGOTO_ERROR(H5E_BTREE, H5E_CANTINSERT, H5B_INS_ERROR, "can't insert maximum leaf node");
            }
            *rt_key_changed = true;
        } /* end else */
        ...
        HGOTO_ERROR(H5E_BTREE, H5E_CANTINSERT, H5B_INS_ERROR, "internal error: could not determine which branch to follow out of this node");
    }
    else if (bt->level > 0) {
        /*
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5B__iterate_helper(H5F_t* f, const H5B_class_t* type, haddr_t addr, int exp_level, H5B_operator_t op, void* udata)
{
 ...
 *-------------------------------------------------------------------------
 */
static H5B_ins_t H5B__remove_helper(H5F_t* f,
                                    haddr_t addr,
                                    const H5B_class_t* type,
                                    int level,
                                    uint8_t* lt_key /*out*/,
                                    bool* lt_key_changed /*out*/,
                                    void* udata,
                                    uint8_t* rt_key /*out*/,
                                    bool* rt_key_changed /*out*/)
{
    ...
    if (bt->level > 0) {
        ...
        if ((int)(ret_value = H5B__remove_helper(f,
                                                 bt->child[idx],
                                                 type,
                                                 level + 1,
                                                 H5B_NKEY(bt, shared, idx) /*out*/,
                                                 lt_key_changed /*out*/,
                                                 udata,
                                                 H5B_NKEY(bt, shared, idx + 1) /*out*/,
        ...
        if (1 == bt->nchildren) {
            ...
            if (level > 0) {
                ...
                if (H5_addr_defined(bt->left)) {
                    ...
                    if (type->critical_key == H5B_LEFT) {
                        H5MM_memcpy(H5B_NKEY(sibling, shared, sibling->nchildren), H5B_NKEY(bt, shared, 1), type->sizeof_nkey);
                    }
            ...
            if (type->critical_key == H5B_LEFT) {
                ...
                *lt_key_changed = true;
                 ...
                 * overwritten) */
                memmove(H5B_NKEY(bt, shared, 1), H5B_NKEY(bt, shared, 2), (bt->nchildren - 1) * type->sizeof_nkey);
            }
            ...
            if (type->critical_key == H5B_LEFT) {
                ...
                memmove(H5B_NKEY(bt, shared, bt->nchildren - 1), H5B_NKEY(bt, shared, bt->nchildren), type->sizeof_nkey);
            }
            else {
                /* Just update rt_key */
             ...
             */
            if (type->critical_key == H5B_LEFT) {
                memmove(H5B_NKEY(bt, shared, idx), H5B_NKEY(bt, shared, idx + 1), (bt->nchildren - idx) * type->sizeof_nkey);
            }
            else {
                memmove(H5B_NKEY(bt, shared, idx + 1), H5B_NKEY(bt, shared, idx + 2), (bt->nchildren - 1 - idx) * type->sizeof_nkey);
            }
    ...
    if (*lt_key_changed && bt != NULL && H5_addr_defined(bt->left)) {
        ...
        H5MM_memcpy(H5B_NKEY(sibling, shared, sibling->nchildren), H5B_NKEY(bt, shared, 0), type->sizeof_nkey);
    ...
    if (bt->level > 0) {
        ...
        if (type->remove) {
            ...
            for (u = 0; u < bt->nchildren; u++) {
                ...
                if ((type->remove)(f, bt->child[u], H5B_NKEY(bt, shared, u), &lt_key_changed, udata, H5B_NKEY(bt, shared, u + 1), &rt_key_changed) < H5B_INS_NOOP) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5B__get_info_helper(H5F_t* f, const H5B_class_t* type, haddr_t addr, const H5B_info_ud_t* info_udata)
{
    H5B_t* bt = NULL;           /* Pointer to current B-tree node */
    H5UC_t* rc_shared;          /* Ref-counted shared info */
    H5B_shared_t* shared;       /* Pointer to shared B-tree info */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5B_valid(H5F_t* f, const H5B_class_t* type, haddr_t addr)
{
