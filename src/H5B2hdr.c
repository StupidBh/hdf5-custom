...
herr_t H5B2__hdr_init(H5B2_hdr_t* hdr, const H5B2_create_t* cparam, void* ctx_udata, uint16_t depth)
{
    ...
    assert(cparam);
    assert(cparam->cls);
    assert((cparam->cls->crt_context && cparam->cls->dst_context) || (NULL == cparam->cls->crt_context && NULL == cparam->cls->dst_context));
    ...
    hdr->swmr_write = (H5F_INTENT(hdr->f) & H5F_ACC_SWMR_WRITE) > 0 && (hdr->cls->id == H5B2_CDSET_ID || hdr->cls->id == H5B2_CDSET_FILT_ID);
...
haddr_t H5B2__hdr_create(H5F_t* f, const H5B2_create_t* cparam, void* ctx_udata)
{
    ...
    if (!H5_addr_defined(ret_value)) {
        if (hdr) {
            ...
            if (H5_addr_defined(hdr->addr) && H5MF_xfree(f, H5FD_MEM_BTREE, hdr->addr, (hsize_t)hdr->hdr_size) < 0) {
