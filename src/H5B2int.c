                    ...
                    unsigned idx)
{
    ...
    if (depth > 1) {
        H5MM_memcpy(&(right_node_ptrs[0]), &(left_node_ptrs[mid_record + (unsigned)1]), sizeof(H5B2_node_ptr_t) * (size_t)(old_node_nrec - mid_record));
    }
    ...
    *internal_flags_ptr |= H5AC__DIRTIED_FLAG;
    ...
    if (hdr->swmr_write && depth > 1) {
        if (H5B2__update_child_flush_depends(hdr, depth, right_node_ptrs, 0, (unsigned)(*right_nrec + 1), left_child, right_child) < 0) {
...
herr_t H5B2__split_root(H5B2_hdr_t* hdr)
{
    ...
    hdr->node_info[hdr->depth].cum_max_nrec =
        ((hdr->node_info[hdr->depth].max_nrec + 1) * hdr->node_info[hdr->depth - 1].cum_max_nrec) + hdr->node_info[hdr->depth].max_nrec;
...
herr_t H5B2__redistribute2(H5B2_hdr_t* hdr, uint16_t depth, H5B2_internal_t* internal, unsigned idx)
{
    ...
    if (depth > 1) {
        ...
        left_addr = internal->node_ptrs[idx].addr;
        if (NULL ==
            (right_internal = H5B2__protect_internal(hdr, internal, &internal->node_ptrs[idx + 1], (uint16_t)(depth - 1), hdr->swmr_write, H5AC__NO_FLAGS_SET))) {
            HGOTO_ERROR(H5E_BTREE, H5E_CANTPROTECT, FAIL, "unable to protect B-tree leaf node");
        }
        right_addr = internal->node_ptrs[idx + 1].addr;
    ...
    if (*left_nrec < *right_nrec) {
        ...
        if (depth > 1) {
            ...
            memmove(&(right_node_ptrs[0]), &(right_node_ptrs[move_nrec]), sizeof(H5B2_node_ptr_t) * (new_right_nrec + (unsigned)1));
        } /* end if */

        /* Update flush dependencies for grandchildren, if using SWMR */
        if (hdr->swmr_write && depth > 1) {
            if (H5B2__update_child_flush_depends(hdr,
                                                 depth,
                                                 left_node_ptrs,
                                                 (unsigned)(*left_nrec + 1),
                                                 (unsigned)(*left_nrec + move_nrec + 1),
                                                 right_child,
                                                 left_child) < 0) {
                HGOTO_ERROR(H5E_BTREE, H5E_CANTUPDATE, FAIL, "unable to update child nodes to new parent");
            }
        ...
        *right_nrec = new_right_nrec;
        ...
        memmove(H5B2_NAT_NREC(right_native, hdr, move_nrec), H5B2_NAT_NREC(right_native, hdr, 0), hdr->cls->nrec_size * (*right_nrec));
        ...
        if (depth > 1) {
            ...
            memmove(&(right_node_ptrs[move_nrec]), &(right_node_ptrs[0]), sizeof(H5B2_node_ptr_t) * (size_t)(*right_nrec + 1));
        ...
        /* Update flush dependencies for grandchildren, if using SWMR */
        if (hdr->swmr_write && depth > 1) {
            if (H5B2__update_child_flush_depends(hdr, depth, right_node_ptrs, 0, (unsigned)move_nrec, left_child, right_child) < 0) {
                HGOTO_ERROR(H5E_BTREE, H5E_CANTUPDATE, FAIL, "unable to update child nodes to new parent");
            }
        }
...
herr_t H5B2__redistribute3(H5B2_hdr_t* hdr, uint16_t depth, H5B2_internal_t* internal, unsigned* internal_flags_ptr, unsigned idx)
{
    ...
    if (depth > 1) {
        ...
        if (NULL ==
            (left_internal = H5B2__protect_internal(hdr, internal, &internal->node_ptrs[idx - 1], (uint16_t)(depth - 1), hdr->swmr_write, H5AC__NO_FLAGS_SET))) {
            HGOTO_ERROR(H5E_BTREE, H5E_CANTPROTECT, FAIL, "unable to protect B-tree internal node");
        }
        left_addr = internal->node_ptrs[idx - 1].addr;
        if (NULL ==
            (middle_internal = H5B2__protect_internal(hdr, internal, &internal->node_ptrs[idx], (uint16_t)(depth - 1), hdr->swmr_write, H5AC__NO_FLAGS_SET))) {
            HGOTO_ERROR(H5E_BTREE, H5E_CANTPROTECT, FAIL, "unable to protect B-tree internal node");
        }
    ...
    /* Redistribute records */
    {
        ...
        if (new_left_nrec > *left_nrec) {
            ...
            if (depth > 1) {
                ...
                memmove(&(middle_node_ptrs[0]), &(middle_node_ptrs[move_nptrs]), sizeof(H5B2_node_ptr_t) * ((*middle_nrec - move_nptrs) + 1));
            } /* end if */
        ...
        if (new_right_nrec > *right_nrec) {
            ...
            /* Slide records in right node up */
            memmove(H5B2_NAT_NREC(right_native, hdr, right_nrec_move), H5B2_NAT_NREC(right_native, hdr, 0), hdr->cls->nrec_size * (*right_nrec));
            ...
            if (depth > 1) {
                ...
                memmove(&(right_node_ptrs[right_nrec_move]), &(right_node_ptrs[0]), sizeof(H5B2_node_ptr_t) * (size_t)(*right_nrec + 1));
            ...
            /* Update flush dependencies for grandchildren, if using SWMR */
            if (hdr->swmr_write && depth > 1) {
                if (H5B2__update_child_flush_depends(hdr, depth, right_node_ptrs, 0, (unsigned)right_nrec_move, middle_child, right_child) < 0) {
                    HGOTO_ERROR(H5E_BTREE, H5E_CANTUPDATE, FAIL, "unable to update child nodes to new parent");
                }
            }
        ...
        if (new_left_nrec < *left_nrec) {
            ...
            memmove(H5B2_NAT_NREC(middle_native, hdr, left_nrec_move), H5B2_NAT_NREC(middle_native, hdr, 0), hdr->cls->nrec_size * curr_middle_nrec);
            ...
            if (left_nrec_move > 1) {
                memmove(H5B2_NAT_NREC(middle_native, hdr, 0), H5B2_NAT_NREC(left_native, hdr, new_left_nrec + 1), hdr->cls->nrec_size * (left_nrec_move - 1));
            }
            ...
            /* Update flush dependencies for grandchildren, if using SWMR */
            if (hdr->swmr_write && depth > 1) {
                if (H5B2__update_child_flush_depends(hdr, depth, middle_node_ptrs, 0, (unsigned)left_nrec_move, left_child, middle_child) < 0) {
                    HGOTO_ERROR(H5E_BTREE, H5E_CANTUPDATE, FAIL, "unable to update child nodes to new parent");
                }
            }
        ...
        if (new_right_nrec < *right_nrec) {
            ...
            memmove(H5B2_NAT_NREC(middle_native, hdr, (curr_middle_nrec + 1)), H5B2_NAT_NREC(right_native, hdr, 0), hdr->cls->nrec_size * (right_nrec_move - 1));
            ...
            if (depth > 1) {
                ...
                memmove(&(right_node_ptrs[0]), &(right_node_ptrs[right_nrec_move]), sizeof(H5B2_node_ptr_t) * (size_t)(new_right_nrec + 1));
            } /* end if */
            ...
            if (hdr->swmr_write && depth > 1) {
                if (H5B2__update_child_flush_depends(hdr,
                                                     depth,
                                                     middle_node_ptrs,
                                                     (unsigned)(curr_middle_nrec + 1),
                                                     (unsigned)(curr_middle_nrec + right_nrec_move + 1),
                                                     right_child,
                                                     middle_child) < 0) {
                    HGOTO_ERROR(H5E_BTREE, H5E_CANTUPDATE, FAIL, "unable to update child nodes to new parent");
                }
                    ...
                    unsigned idx)
{
    ...
    /* Redistribute records into left node */
    {
        ...
        if (hdr->swmr_write && depth > 1) {
            if (H5B2__update_child_flush_depends(hdr,
                                                 ...
                                                 left_node_ptrs,
                                                 (unsigned)(*left_nrec + 1),
                                                 (unsigned)(*left_nrec + *right_nrec + 2),
                                                 right_child,
                                                 left_child) < 0) {
                HGOTO_ERROR(H5E_BTREE, H5E_CANTUPDATE, FAIL, "unable to update child nodes to new parent");
            }
    ...
    if ((idx + 1) < internal->nrec) {
        memmove(H5B2_INT_NREC(internal, hdr, idx), H5B2_INT_NREC(internal, hdr, idx + 1), hdr->cls->nrec_size * (internal->nrec - (idx + 1)));
        memmove(&(internal->node_ptrs[idx + 1]), &(internal->node_ptrs[idx + 2]), sizeof(H5B2_node_ptr_t) * (internal->nrec - (idx + 1)));
    } /* end if */
                    ...
                    unsigned idx)
{
    ...
    /* Redistribute records into left node */
    {
        ...
        if (depth > 1) {
            ...
            /* Slide the node pointers in middle node down */
            memmove(&(middle_node_ptrs[0]),
                    &(middle_node_ptrs[middle_nrec_move]),
        ...
        if (hdr->swmr_write && depth > 1) {
            if (H5B2__update_child_flush_depends(hdr,
                                                 ...
                                                 left_node_ptrs,
                                                 (unsigned)(*left_nrec + 1),
                                                 (unsigned)(*left_nrec + middle_nrec_move + 1),
                                                 middle_child,
                                                 left_child) < 0) {
                HGOTO_ERROR(H5E_BTREE, H5E_CANTUPDATE, FAIL, "unable to update child nodes to new parent");
            }
    ...
    /* Redistribute records into middle node */
    {
        ...
        if (hdr->swmr_write && depth > 1) {
            if (H5B2__update_child_flush_depends(hdr,
                                                 depth,
                                                 middle_node_ptrs,
                                                 (unsigned)(*middle_nrec + 1),
                                                 (unsigned)(*middle_nrec + *right_nrec + 2),
                                                 right_child,
                                                 middle_child) < 0) {
                HGOTO_ERROR(H5E_BTREE, H5E_CANTUPDATE, FAIL, "unable to update child nodes to new parent");
            }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5B2__iterate_node(H5B2_hdr_t* hdr, uint16_t depth, H5B2_node_ptr_t* curr_node, void* parent, H5B2_operator_t op, void* op_data)
{
    ...
    if (depth > 0) {
        ...
        if (NULL == (internal = H5B2__protect_internal(hdr, parent, curr_node, depth, false, H5AC__READ_ONLY_FLAG))) {
            HGOTO_ERROR(H5E_BTREE, H5E_CANTPROTECT, FAIL, "unable to protect B-tree internal node");
        }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5B2__delete_node(H5B2_hdr_t* hdr, uint16_t depth, H5B2_node_ptr_t* curr_node, void* parent, H5B2_remove_t op, void* op_data)
{
    const H5AC_class_t* curr_node_class = NULL; /* Pointer to current node's class info */
    void* node = NULL;                          /* Pointers to current node */
    ...
    if (depth > 0) {
        ...
        /* Lock the current B-tree node */
        if (NULL == (internal = H5B2__protect_internal(hdr, parent, curr_node, depth, false, H5AC__NO_FLAGS_SET))) {
            HGOTO_ERROR(H5E_BTREE, H5E_CANTPROTECT, FAIL, "unable to protect B-tree internal node");
        }
    ...
    if (node &&
        H5AC_unprotect(hdr->f, curr_node_class, curr_node->addr, node, (unsigned)(H5AC__DELETED_FLAG | (hdr->swmr_write ? 0 : H5AC__FREE_FILE_SPACE_FLAG))) < 0) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5B2__node_size(H5B2_hdr_t* hdr, uint16_t depth, H5B2_node_ptr_t* curr_node, void* parent, hsize_t* btree_size)
{
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5B2__update_flush_depend(H5B2_hdr_t* hdr, unsigned depth, H5B2_node_ptr_t* node_ptr, void* old_parent, void* new_parent)
{
    const H5AC_class_t* child_class; /* Pointer to child node's class info */
    ...
    if (node_status & H5AC_ES__IN_CACHE) {
        ...
        if (depth > 1) {
            ...
            /* Protect child */
            if (NULL == (child_int = H5B2__protect_internal(hdr, new_parent, node_ptr, (uint16_t)(depth - 1), false, H5AC__NO_FLAGS_SET))) {
                HGOTO_ERROR(H5E_BTREE, H5E_CANTPROTECT, FAIL, "unable to protect B-tree internal node");
            }
            child_class = H5AC_BT2_INT;
            child = child_int;
