 ...
 *-------------------------------------------------------------------------
 */
H5B2_leaf_t* H5B2__protect_leaf(H5B2_hdr_t* hdr, void* parent, H5B2_node_ptr_t* node_ptr, bool shadow, unsigned flags)
{
...
herr_t H5B2__insert_leaf(H5B2_hdr_t* hdr, H5B2_node_ptr_t* curr_node_ptr, H5B2_nodepos_t curr_pos, void* parent, void* udata)
{
    ...
    if (leaf->nrec == 0) {
        ...
        if (idx < leaf->nrec) {
            memmove(H5B2_LEAF_NREC(leaf, hdr, idx + 1), H5B2_LEAF_NREC(leaf, hdr, idx), hdr->cls->nrec_size * (leaf->nrec - idx));
        }
    } /* end else */
...
herr_t H5B2__remove_leaf(H5B2_hdr_t* hdr, H5B2_node_ptr_t* curr_node_ptr, H5B2_nodepos_t curr_pos, void* parent, void* udata, H5B2_remove_t op, void* op_data)
{
    ...
    if (leaf->nrec > 0) {
        ...
        if (idx < leaf->nrec) {
            memmove(H5B2_LEAF_NREC(leaf, hdr, idx), H5B2_LEAF_NREC(leaf, hdr, (idx + 1)), hdr->cls->nrec_size * (leaf->nrec - idx));
        }
 ...
 *-------------------------------------------------------------------------
 */
H5_ATTR_PURE herr_t H5B2__assert_leaf2(const H5B2_hdr_t H5_ATTR_NDEBUG_UNUSED* hdr,
