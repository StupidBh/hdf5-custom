...
#ifdef H5_HAVE_PARALLEL
    if ((H5F_INTENT(f) & H5F_ACC_RDWR) && !image_generated && cache_ptr->aux_ptr != NULL && f->shared->fs_persist) {
...
herr_t H5C_set_cache_auto_resize_config(H5C_t* cache_ptr, H5C_auto_size_ctl_t* config_ptr)
{
    ...
    case H5C_incr__off: cache_ptr->size_increase_possible = false; break;

    case H5C_incr__threshold:
        if ((config_ptr->lower_hr_threshold <= 0.0) || (config_ptr->increment <= 1.0) || ((config_ptr->apply_max_increment) && (config_ptr->max_increment <= 0))) {
            cache_ptr->size_increase_possible = false;
        }
        break;

    default: /* should be unreachable */ HGOTO_ERROR(H5E_CACHE, H5E_SYSTEM, FAIL, "Unknown incr_mode?!?!?");
    } /* end switch */
    ...
    case H5C_decr__off: cache_ptr->size_decrease_possible = false; break;

    case H5C_decr__threshold:
        if (config_ptr->upper_hr_threshold >= 1.0 || config_ptr->decrement >= 1.0 || (config_ptr->apply_max_decrement && config_ptr->max_decrement <= 0)) {
            cache_ptr->size_decrease_possible = false;
        }
        break;

    case H5C_decr__age_out:
        if ((config_ptr->apply_empty_reserve && config_ptr->empty_reserve >= 1.0) || (config_ptr->apply_max_decrement && config_ptr->max_decrement <= 0)) {
            cache_ptr->size_decrease_possible = false;
        }
        break;
    ...
    default: /* should be unreachable */ HGOTO_ERROR(H5E_CACHE, H5E_SYSTEM, FAIL, "Unknown decr_mode?!?!?");
    } /* end switch */
    ...
    if ((config_ptr->decr_mode == H5C_decr__age_out_with_threshold) || (config_ptr->decr_mode == H5C_decr__age_out)) {
    ...
    if (cache_ptr->flash_size_increase_possible) {
        ...
        case H5C_flash_incr__off: cache_ptr->flash_size_increase_possible = false; break;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5C_set_evictions_enabled(H5C_t* cache_ptr, bool evictions_enabled)
{
    herr_t ret_value = SUCCEED; /* Return value */
     ...
     */
    if ((evictions_enabled != true) && ((cache_ptr->resize_ctl.incr_mode != H5C_incr__off) || (cache_ptr->resize_ctl.decr_mode != H5C_decr__off))) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5C_set_slist_enabled(H5C_t* cache_ptr, bool slist_enabled, bool populate_slist)
{
...
herr_t H5C_validate_resize_config(H5C_auto_size_ctl_t* config_ptr, unsigned int tests)
{
    ...
    if ((tests & H5C_RESIZE_CFG__VALIDATE_INCREMENT) != 0) {
        ...
        default: HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL, "Invalid flash_incr_mode"); break;
        } /* end switch */
    } /* H5C_RESIZE_CFG__VALIDATE_INCREMENT */

    if ((tests & H5C_RESIZE_CFG__VALIDATE_DECREMENT) != 0) {
        if ((config_ptr->decr_mode != H5C_decr__off) && (config_ptr->decr_mode != H5C_decr__threshold) && (config_ptr->decr_mode != H5C_decr__age_out) &&
        ...
        if ((config_ptr->decr_mode == H5C_decr__age_out) || (config_ptr->decr_mode == H5C_decr__age_out_with_threshold)) {
            ...
            if (config_ptr->apply_empty_reserve && (config_ptr->empty_reserve > 1.0 || config_ptr->empty_reserve < 0.0)) {
    ...
    if ((tests & H5C_RESIZE_CFG__VALIDATE_INTERACTIONS) != 0) {
        if ((config_ptr->incr_mode == H5C_incr__threshold) &&
            ((config_ptr->decr_mode == H5C_decr__threshold) || (config_ptr->decr_mode == H5C_decr__age_out_with_threshold)) &&
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5C_cork(H5C_t* cache_ptr, haddr_t obj_addr, unsigned action, bool* corked)
{
