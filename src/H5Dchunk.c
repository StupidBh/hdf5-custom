    ...
    do {                                                                                                                   \
        if ((layout)->version <= H5O_LAYOUT_VERSION_4) {                                                                   \
            if ((length) > UINT32_MAX)                                                                                     \
                HGOTO_ERROR(H5E_DATASET,                                                                                   \
                            H5E_BADRANGE,                                                                                  \
                            err,                                                                                           \
                            "chunk size is greater than UINT32_MAX and can't be encoded with this file "                   \
                            "format version - see H5Pset_libver_bounds()");                                                \
                                                                                                                           \
            if ((layout)->version == H5O_LAYOUT_VERSION_4 && (layout)->storage.u.chunk.idx_type != H5D_CHUNK_IDX_SINGLE) { \
...
/* Helper routines */
static herr_t H5D__chunk_set_info_real(H5O_layout_chunk_t* layout, unsigned ndims, const hsize_t* curr_dims, const hsize_t* max_dims);
...
static herr_t H5D__piece_file_cb(void* elem, const H5T_t* type, unsigned ndims, const hsize_t* coords, void* _opdata);
...
static void* H5D__chunk_lock(const H5D_io_info_t* io_info, const H5D_dset_io_info_t* dset_info, H5D_chunk_ud_t* udata, bool relax, bool prev_unfilt_chunk);
static herr_t
    H5D__chunk_unlock(const H5D_io_info_t* io_info, const H5D_dset_io_info_t* dset_info, const H5D_chunk_ud_t* udata, bool dirty, void* chunk, hsize_t naccessed);
...
#ifdef H5_HAVE_PARALLEL
static herr_t H5D__chunk_collective_fill(const H5D_t* dset, H5D_chunk_coll_fill_info_t* chunk_fill_info, const void* fill_buf, const void* partial_chunk_fill_buf);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__chunk_direct_write(H5D_t* dset, uint32_t filters, hsize_t* offset, size_t data_size, const void* buf)
{
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__chunk_direct_read(const H5D_t* dset, hsize_t* offset, uint32_t* filters, void* buf, size_t* nalloc)
{
    const H5O_layout_t* layout = &(dset->shared->layout);  /* Dataset layout */
    const H5D_rdcc_t* rdcc = &(dset->shared->cache.chunk); /* raw data chunk cache */
    H5D_chunk_ud_t udata;                                  /* User data for querying chunk info */
    ...
    *filters = 0;
    ...
    if (udata.chunk_block.length > 0 && buf && (!nalloc || *nalloc >= udata.chunk_block.length)) {
        ...
        if (H5F_shared_block_read(H5F_SHARED(dset->oloc.file), H5FD_MEM_DRAW, udata.chunk_block.offset, (size_t)udata.chunk_block.length, buf) < 0) {
...
static herr_t H5D__chunk_set_sizes(H5D_t* dset)
{
    ...
    if (dset->shared->layout.u.chunk.enc_bytes_per_dim) {
        if (dset->shared->layout.u.chunk.enc_bytes_per_dim < max_enc_bytes_per_dim) {
            HGOTO_ERROR(H5E_DATASET, H5E_BADVALUE, FAIL, "stored chunk dimension encoding length is insufficient to encode chunk dimensions");
        }
    }
    else {
...
static herr_t H5D__chunk_construct(H5F_t* f, H5D_t* dset)
{
    ...
    for (u = 0; u < layout->u.chunk.ndims - 1; u++) {
         ...
         */
        if (dset->shared->curr_dims[u] && dset->shared->max_dims[u] != H5S_UNLIMITED && dset->shared->max_dims[u] < layout->u.chunk.dim[u]) {
...
static herr_t H5D__chunk_init(H5F_t* f, H5D_t* dset, hid_t dapl_id, bool open_op)
{
    ...
    if (dset->shared->ndims > 1) {
        ...
        for (u = 0; u < dset->shared->ndims; u++) {
            ...
            /* Round up to the next integer # of chunks, to accommodate partial chunks */
            rdcc->scaled_dims[u] = (dset->shared->curr_dims[u] + dset->shared->layout.u.chunk.dim[u] - 1) / dset->shared->layout.u.chunk.dim[u];
 ...
 *-------------------------------------------------------------------------
 */
bool H5D__chunk_is_space_alloc(const H5O_storage_t* storage)
{
 ...
 *-------------------------------------------------------------------------
 */
bool H5D__chunk_is_data_cached(const H5D_shared_t* shared_dset)
{
    FUNC_ENTER_PACKAGE_NOERR
...
static herr_t H5D__chunk_io_init(H5D_io_info_t* io_info, H5D_dset_io_info_t* dinfo)
{
     ...
     */
    if (H5F_SHARED_HAS_FEATURE(io_info->f_sh, H5FD_FEAT_HAS_MPI) && H5F_shared_get_coll_metadata_reads(io_info->f_sh) &&
...
static herr_t H5D__chunk_io_init_selections(H5D_io_info_t* io_info, H5D_dset_io_info_t* dinfo)
{
    ...
    if (dinfo->nelmts == 1
        ...
        if (sel_hyper_flag) {
            ...
            if (H5S_select_iterate(&bogus, dataset->shared->type, dinfo->file_space, &iter_op, &io_info_wrap) < 0) {
                HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL, "unable to create file chunk selections");
            }
        ...
        else if (sel_hyper_flag && fm->f_ndims == 1 && fm->m_ndims == 1 && H5S_SELECT_IS_REGULAR(dinfo->mem_space) && H5S_SELECT_IS_SINGLE(dinfo->mem_space)) {
 ...
 *-------------------------------------------------------------------------
 */
void* H5D__chunk_mem_alloc(size_t size, void* pline)
{
 ...
 *-------------------------------------------------------------------------
 */
void* H5D__chunk_mem_xfree(void* chk, const void* pline)
{
 ...
 *-------------------------------------------------------------------------
 */
void H5D__chunk_mem_free(void* chk, void* pline)
{
 ...
 *-------------------------------------------------------------------------
 */
void* H5D__chunk_mem_realloc(void* chk, size_t size, const H5O_pline_t* pline)
{
    void* ret_value = NULL; /* Return value */
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5D__free_piece_info(void* item, void H5_ATTR_UNUSED* key, void H5_ATTR_UNUSED* opdata)
{
...
static herr_t H5D__create_piece_map_single(H5D_dset_io_info_t* di, H5D_io_info_t* io_info)
{
    ...
    /* Calculate the index of this chunk */
    piece_info->index = H5VM_array_offset_pre(fm->f_ndims, di->layout->u.chunk.down_chunks, piece_info->scaled);
...
static herr_t H5D__create_piece_file_map_all(H5D_dset_io_info_t* di, H5D_io_info_t* io_info)
{
    ...
    while (sel_points) {
        ...
        if (num_partial_dims > 0) {
            if (H5S_select_hyperslab(new_piece_info->fspace, H5S_SELECT_SET, zeros, NULL, curr_partial_clip, NULL) < 0) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__create_piece_file_map_hyper(H5D_dset_io_info_t* dinfo, H5D_io_info_t* io_info)
{
    H5D_chunk_map_t* fm;                    /* Convenience pointer to chunk map */
    H5S_t* tmp_fchunk = NULL;               /* Temporary file dataspace */
    hsize_t sel_start[H5O_LAYOUT_NDIMS];    /* Offset of low bound of file selection */
    ...
    while (sel_points) {
        ...
        if (true == H5S_SELECT_INTERSECT_BLOCK(dinfo->file_space, coords, end)) {
             ...
             *  the current chunk.
             */
            if (H5S_combine_hyperslab(dinfo->file_space, H5S_SELECT_AND, coords, NULL, fm->chunk_dim, NULL, &tmp_fchunk) < 0) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__create_piece_mem_map_hyper(const H5D_dset_io_info_t* dinfo)
{
    H5D_chunk_map_t* fm;                  /* Convenience pointer to chunk map */
    ...
    if (H5SL_count(fm->dset_sel_pieces) == 1) {
        ...
        while (curr_node) {
            ...
            if (H5S_SEL_ALL == chunk_sel_type) {
                ...
                if (H5S_select_hyperslab(piece_info->mspace, H5S_SELECT_SET, coords, NULL, fm->chunk_dim, NULL) < 0) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__create_piece_mem_map_1d(const H5D_dset_io_info_t* dinfo)
{
    H5D_chunk_map_t* fm;          /* Convenience pointer to chunk map */
    ...
    if (H5SL_count(fm->dset_sel_pieces) == 1) {
        ...
        while (curr_node) {
            ...
            if (H5S_select_hyperslab(piece_info->mspace, H5S_SELECT_SET, mem_sel_start, NULL, &tmp_count, &chunk_points) < 0) {
                HGOTO_ERROR(H5E_DATASET, H5E_CANTSELECT, FAIL, "can't create chunk memory selection");
            }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__piece_file_cb(void H5_ATTR_UNUSED* elem, const H5T_t H5_ATTR_UNUSED* type, unsigned ndims, const hsize_t* coords, void* _opdata)
{
    ...
    /* Calculate the index of this chunk */
    chunk_index = H5VM_chunk_index_scaled(ndims, coords, dinfo->layout->u.chunk.dim, dinfo->layout->u.chunk.down_chunks, scaled);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__piece_mem_cb(void H5_ATTR_UNUSED* elem, const H5T_t H5_ATTR_UNUSED* type, unsigned ndims, const hsize_t* coords, void* _opdata)
{
    H5D_io_info_wrap_t* opdata = (H5D_io_info_wrap_t*)_opdata;
    ...
    /* Calculate the index of this chunk */
    chunk_index = H5VM_chunk_index(ndims, coords, dinfo->layout->u.chunk.dim, dinfo->layout->u.chunk.down_chunks);
 ...
 *-------------------------------------------------------------------------
 */
htri_t H5D__chunk_cacheable(const H5D_io_info_t H5_ATTR_PARALLEL_USED* io_info, H5D_dset_io_info_t* dset_info, haddr_t caddr, bool write_op)
{
    ...
    if (dataset->shared->dcpl_cache.pline.nused > 0) {
        if (dataset->shared->layout.u.chunk.flags & H5O_LAYOUT_CHUNK_DONT_FILTER_PARTIAL_BOUND_CHUNKS) {
            has_filters = !H5D__chunk_is_partial_edge_chunk(dataset->shared->ndims,
    ...
    if (has_filters) {
        ...
        if (io_info->using_mpi_vfd && (H5F_ACC_RDWR & H5F_INTENT(dataset->oloc.file))) {
            ...
            if (!(((hsize_t)((size_t)dataset->shared->layout.u.chunk.size) == dataset->shared->layout.u.chunk.size) &&
                  ((size_t)dataset->shared->layout.u.chunk.size <= dataset->shared->cache.chunk.nbytes_max))) {
                if (write_op && !H5_addr_defined(caddr)) {
                    ...
                    if (fill->fill_time == H5D_FILL_TIME_ALLOC ||
                        (fill->fill_time == H5D_FILL_TIME_IFSET && (fill_status == H5D_FILL_VALUE_USER_DEFINED || fill_status == H5D_FILL_VALUE_DEFAULT))) {
                        /* Check for size_t overflow */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__chunk_read(H5D_io_info_t* io_info, H5D_dset_io_info_t* dset_info)
{
    H5SL_node_t* chunk_node;           /* Current node in chunk skip list */
    ...
    if (io_info->use_select_io == H5D_SELECTION_IO_MODE_ON) {
        ...
        if (H5D_LAYOUT_CB_PERFORM_IO(io_info)) {
            ...
            assert(sizeof(chunk_mem_spaces_local) / sizeof(chunk_mem_spaces_local[0]) == sizeof(chunk_file_spaces_local) / sizeof(chunk_file_spaces_local[0]));
            assert(sizeof(chunk_mem_spaces_local) / sizeof(chunk_mem_spaces_local[0]) == sizeof(chunk_addrs_local) / sizeof(chunk_addrs_local[0]));
            if (num_chunks > (sizeof(chunk_mem_spaces_local) / sizeof(chunk_mem_spaces_local[0]))) {
                ...
                if (NULL == (chunk_addrs = H5MM_malloc(num_chunks * sizeof(haddr_t)))) {
                    HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL, "memory allocation failed for chunk address list");
                }
            } /* end if */
            else {
        ...
        while (chunk_node) {
            ...
            if (H5_addr_defined(udata.chunk_block.offset)) {
                ...
                if (H5D__fill(dset_info->dset->shared->dcpl_cache.fill.buf,
                              dset_info->dset->shared->type,
                              dset_info->buf.vp,
                              dset_info->type_info.mem_type,
                              chunk_info->mspace) < 0) {
        ...
        if (H5D_LAYOUT_CB_PERFORM_IO(io_info)) {
            ...
            if (H5F_shared_select_read(H5F_SHARED(dset_info->dset->oloc.file),
                                       H5FD_MEM_DRAW,
                                       (uint32_t)num_chunks,
                                       chunk_mem_spaces,
                                       chunk_file_spaces,
        ...
        while (chunk_node) {
            ...
            /* Check for non-existent chunk & skip it if appropriate */
            if (H5_addr_defined(udata.chunk_block.offset) || UINT_MAX != udata.idx_hint || !skip_missing_chunks) {
    ...
    if (dset_info->dset->shared->cache.sieve.sieve_buf) {
        dset_info->dset->shared->cache.sieve.sieve_loc = HADDR_UNDEF;
        dset_info->dset->shared->cache.sieve.sieve_size = 0;
        dset_info->dset->shared->cache.sieve.sieve_buf = (unsigned char*)H5FL_BLK_FREE(sieve_buf, dset_info->dset->shared->cache.sieve.sieve_buf);
    }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__chunk_write(H5D_io_info_t* io_info, H5D_dset_io_info_t* dset_info)
{
    H5SL_node_t* chunk_node;           /* Current node in chunk skip list */
    H5D_io_info_t ctg_io_info;         /* Contiguous I/O info object */
    H5D_dset_io_info_t ctg_dset_info;  /* Contiguous I/O dset info object */
    ...
    if (io_info->use_select_io == H5D_SELECTION_IO_MODE_ON) {
        ...
        if (H5D_LAYOUT_CB_PERFORM_IO(io_info)) {
             ...
             * because this is raw data) */
            H5_CHECK_OVERFLOW(num_chunks, size_t, uint32_t);
            if (H5F_shared_select_write(H5F_SHARED(dset_info->dset->oloc.file),
                                        H5FD_MEM_DRAW,
                                        (uint32_t)num_chunks,
                                        chunk_mem_spaces,
                                        chunk_file_spaces,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__chunk_io_term(H5D_io_info_t H5_ATTR_UNUSED* io_info, H5D_dset_io_info_t* di)
{
    H5D_chunk_map_t* fm;        /* Convenience pointer to chunk map */
    herr_t ret_value = SUCCEED; /*return value        */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D_chunk_idx_reset(H5O_storage_chunk_t* storage, bool reset_addr)
{
    herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__chunk_create(const H5D_t* dset /*in,out*/)
{
    H5D_chk_idx_info_t idx_info; /* Chunked index info */
    H5O_storage_chunk_t* sc = &(dset->shared->layout.storage.u.chunk);
    herr_t ret_value = SUCCEED;  /* Return value */
...
herr_t H5D__chunk_lookup(const H5D_t* dset, const hsize_t* scaled, H5D_chunk_ud_t* udata)
{
    ...
    if (found) {
        ...
        if (!H5D__chunk_cinfo_cache_found(&dset->shared->cache.chunk.last, udata)) {
             ...
             */
#ifdef H5_HAVE_PARALLEL
            if (!((H5F_HAS_FEATURE(idx_info.f, H5FD_FEAT_HAS_MPI)) && (H5F_INTENT(dset->oloc.file) & H5F_ACC_RDWR) && dset->shared->dcpl_cache.pline.nused))
#endif
                H5D__chunk_cinfo_cache_update(&dset->shared->cache.chunk.last, udata);
        } /* end if */
    } /* end else */
...
static herr_t H5D__chunk_flush_entry(const H5D_t* dset, H5D_rdcc_ent_t* ent, bool reset)
{
    ...
    if (ent->dirty) {
        ...
        if (dset->shared->dcpl_cache.pline.nused && !(ent->edge_chunk_state & H5D_RDCC_DISABLE_FILTERS)) {
            ...
            if (H5Z_pipeline(&(dset->shared->dcpl_cache.pline), 0, &(udata.filter_mask), err_detect, filter_cb, &nbytes, &alloc, &buf) < 0) {
    ...
    if (reset) {
        ...
        if (ent->chunk != NULL) {
            ent->chunk = (uint8_t*)H5D__chunk_mem_xfree(ent->chunk, ((ent->edge_chunk_state & H5D_RDCC_DISABLE_FILTERS) ? NULL : &(dset->shared->dcpl_cache.pline)));
        }
    } /* end if */
    ...
    while ((p[0] || p[1]) && (rdcc->nbytes_used + size) > total) {
        ...
        for (i = 0; i < nmeth && (rdcc->nbytes_used + size) > total; i++) {
            if (0 == i && p[0] && !p[0]->locked &&
                ((0 == p[0]->rd_count && 0 == p[0]->wr_count) || (0 == p[0]->rd_count && dset->shared->layout.u.chunk.size == p[0]->wr_count) ||
 ...
 *-------------------------------------------------------------------------
 */
static void* H5D__chunk_lock(const H5D_io_info_t H5_ATTR_NDEBUG_UNUSED* io_info,
                             const H5D_dset_io_info_t* dset_info,
                             H5D_chunk_ud_t* udata,
                             bool relax,
                             bool prev_unfilt_chunk)
{
    ...
    if (UINT_MAX != udata->idx_hint) {
        ...
        if (pline->nused) {
            if (udata->new_unfilt_chunk) {
                ...
                if (H5D__chunk_is_partial_edge_chunk(dset->shared->ndims, layout->u.chunk.dim, dset_info->store->chunk.scaled, dset->shared->curr_dims)) {
        ...
        if (relax) {
            ...
            if (H5_addr_defined(chunk_addr)) {
                ...
                if (NULL == (chunk = H5D__chunk_mem_alloc(buf_alloc, (udata->new_unfilt_chunk ? old_pline : pline)))) {
                    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL, "memory allocation failed for raw data chunk");
                }

                /* Read chunk from disk */
                if (H5F_shared_block_read(H5F_SHARED(dset->oloc.file), H5FD_MEM_DRAW, chunk_addr, chunk_disk_size, chunk) < 0) {
                ...
                if (old_pline && old_pline->nused) {
                    ...
                    if (H5Z_pipeline(old_pline, H5Z_FLAG_REVERSE, &(udata->filter_mask), err_detect, filter_cb, &chunk_nbytes, &buf_alloc, &chunk) < 0) {
                ...
                if (fill->fill_time == H5D_FILL_TIME_ALLOC ||
                    ...
                    if (H5D__fill_init(&fb_info, chunk, NULL, NULL, NULL, NULL, &dset->shared->dcpl_cache.fill, dset->shared->type, (size_t)0, chunk_size) < 0) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__chunk_unlock(const H5D_io_info_t H5_ATTR_NDEBUG_UNUSED* io_info,
                                const H5D_dset_io_info_t* dset_info,
    ...
    if (UINT_MAX == udata->idx_hint) {
        ...
        if (udata->new_unfilt_chunk) {
            ...
            is_unfiltered_edge_chunk =
                H5D__chunk_is_partial_edge_chunk(dset->shared->ndims, layout->u.chunk.dim, dset_info->store->chunk.scaled, dset->shared->curr_dims);
        } /* end if */
        ...
        if (dirty) {
            ...
            if (chunk) {
                chunk = H5D__chunk_mem_xfree(chunk, (is_unfiltered_edge_chunk ? NULL : &(dset->shared->dcpl_cache.pline)));
            }
        } /* end else */
    } /* end if */
    else {
 ...
 *-------------------------------------------------------------------------
 */
static int H5D__chunk_allocated_cb(const H5D_chunk_rec_t* chunk_rec, void* _udata)
{
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__chunk_allocate(const H5D_t* dset, bool full_overwrite, const hsize_t old_dim[])
{
    H5D_chk_idx_info_t idx_info;                                           /* Chunked index info */
    ...
    bool using_mpi = false;                     /* Flag to indicate that the file is being accessed with an MPI-capable file driver */
    ...
    for (op_dim = 0; op_dim < space_ndims; op_dim++) {
        ...
        if ((layout->u.chunk.flags & H5O_LAYOUT_CHUNK_DONT_FILTER_PARTIAL_BOUND_CHUNKS) && pline->nused > 0 && space_dim[op_dim] % chunk_dim[op_dim] != 0) {
        ...
        while (!carry) {
            ...
            if (should_fill) {
                ...
                if (using_mpi) {
                    ...
                    if (0 == chunk_fill_info.num_chunks % 1024) {
                        ...
                        if (NULL ==
                            (tmp_realloc = H5MM_realloc(chunk_fill_info.chunk_info, (chunk_fill_info.num_chunks + 1024) * sizeof(struct chunk_coll_fill_info)))) {
                            HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL, "memory allocation failed for chunk fill info");
                        }
...
#endif /* H5_HAVE_PARALLEL */
                {
                    ...
                    if (H5F_shared_block_write(H5F_SHARED(dset->oloc.file), H5FD_MEM_DRAW, udata.chunk_block.offset, (size_t)chunk_size, *fill_buf) < 0) {
            ...
            for (i = ((int)space_ndims - 1); i >= 0; --i) {
                ...
                if (scaled[i] > max_unalloc[i]) {
                    ...
                    if (unfilt_edge_chunk_dim[i] && edge_chunk_scaled[i] == max_unalloc[i] && scaled[i] < edge_chunk_scaled[i]) {
                        nunfilt_edge_chunk_dims--;
                        if (should_fill && nunfilt_edge_chunk_dims == 0 && !fb_info.has_vlen_fill_type) {
                            assert(!H5D__chunk_is_partial_edge_chunk(space_ndims, chunk_dim, scaled, space_dim));
                            assert(orig_chunk_size);
                            fill_buf = &fb_info.fill_buf;
                            H5_CHECKED_ASSIGN(chunk_size, hsize_t, orig_chunk_size, size_t);
                        } /* end if */
...
herr_t H5D__chunk_update_old_edge_chunks(H5D_t* dset, hsize_t old_dim[])
{
    ...
    for (op_dim = 0; op_dim < space_ndims; op_dim++) {
        ...
        max_edge_chunk_sc[op_dim] = MIN((old_dim[op_dim] - 1) / chunk_dim[op_dim], MAX((space_dim[op_dim] / chunk_dim[op_dim]), 1) - 1);
...
static herr_t H5D__chunk_collective_fill(const H5D_t* dset, H5D_chunk_coll_fill_info_t* chunk_fill_info, const void* fill_buf, const void* partial_chunk_fill_buf)
{
     ...
     */
    if (need_sort) {
        qsort(chunk_fill_info->chunk_info, chunk_fill_info->num_chunks, sizeof(struct chunk_coll_fill_info), H5D__chunk_cmp_coll_fill_info);
    }
    ...
    if (H5F_shared_vector_write(H5F_SHARED(dset->oloc.file), (uint32_t)blocks, io_types, io_addrs, all_same_block_len ? io_2sizes : io_sizes, io_wbufs) < 0) {
...
static herr_t H5D__chunk_prune_fill(H5D_chunk_it_ud1_t* udata, bool new_unfilt_chunk)
{
    ...
    if (!udata->fb_info_init) {
        ...
        if (H5D__fill_init(&udata->fb_info,
                           NULL,
                           NULL,
                           NULL,
                           NULL,
...
herr_t H5D__chunk_prune_by_extent(H5D_t* dset, const hsize_t* old_dim)
{
    ...
    disable_edge_filters = (layout->u.chunk.flags & H5O_LAYOUT_CHUNK_DONT_FILTER_PARTIAL_BOUND_CHUNKS) && (idx_info.pline->nused > 0);
    ...
    for (op_dim = 0; op_dim < (unsigned)space_ndims; op_dim++) {
        ...
        if (0 == space_dim[op_dim]) {
            ...
            max_fill_chunk_sc[op_dim] = (hssize_t)(((MIN(space_dim[op_dim], old_dim[op_dim]) - 1) / chunk_dim[op_dim]));
        }
    ...
    for (op_dim = 0; op_dim < (unsigned)space_ndims; op_dim++) {
        ...
        while (!carry) {
            ...
            if (0 == ndims_outside_fill) {
                ...
                assert(!new_unfilt_dim[op_dim] || (!new_unfilt_chunk != !H5D__chunk_is_partial_edge_chunk(space_ndims, layout->u.chunk.dim, scaled, old_dim)));
 ...
 *-------------------------------------------------------------------------
 */
static int H5D__chunk_addrmap_cb(const H5D_chunk_rec_t* chunk_rec, void* _udata)
{
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__chunk_addrmap(const H5D_t* dset, haddr_t chunk_addr[])
{
    H5D_chk_idx_info_t idx_info; /* Chunked index info */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__chunk_delete(H5F_t* f, H5O_t* oh, H5O_layout_t* layout)
{
    H5D_chk_idx_info_t idx_info; /* Chunked index info */
 ...
 *-------------------------------------------------------------------------
 */
static int H5D__chunk_copy_cb(const H5D_chunk_rec_t* chunk_rec, void* _udata)
{
    ...
    if (udata->do_convert) {
        ...
        else if ((H5T_get_class(dt_src, false) == H5T_REFERENCE) && (udata->file_src != udata->idx_info_dst->f)) {
            fix_ref = true;
        }
        else {
    ...
    if (must_filter && (is_vlen || fix_ref) && !udata->chunk_in_cache) {
        ...
        if (H5Z_pipeline(pline, H5Z_FLAG_REVERSE, &filter_mask, H5Z_NO_EDC, filter_cb, &nbytes, &udata->buf_size, &udata->buf) < 0) {
    ...
    if (is_vlen) {
        ...
        if (udata->cpy_info->expand_ref) {
            ...
            if (H5O_copy_expand_ref(udata->file_src, dt_src, udata->buf, nbytes, udata->idx_info_dst->f, bkg, udata->cpy_info) < 0) {
     ...
     * file */
    if (must_filter && (is_vlen || fix_ref || udata->chunk_in_cache)) {
        if (H5Z_pipeline(pline, 0, &(udata_dst.filter_mask), H5Z_NO_EDC, filter_cb, &nbytes, &udata->buf_size, &udata->buf) < 0) {
    ...
    udata_dst.chunk_idx = H5VM_array_offset_pre(udata_dst.common.layout->ndims - 1, udata_dst.common.layout->max_down_chunks, udata_dst.common.scaled);
...
static int H5D__chunk_dump_index_cb(const H5D_chunk_rec_t* chunk_rec, void* _udata)
{
    ...
    if (udata->stream) {
        ...
        fprintf(udata->stream, "        0x%08x %8" PRIuHSIZE " %10" PRIuHADDR " [", chunk_rec->filter_mask, chunk_rec->nbytes, chunk_rec->chunk_addr);
        for (u = 0; u < udata->ndims; u++) {
            fprintf(udata->stream, "%s%" PRIuHSIZE, (u ? ", " : ""), (chunk_rec->scaled[u] * udata->chunk_dim[u]));
        }
        fputs("]\n", udata->stream);
    } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__chunk_stats(const H5D_t* dset, bool headers)
{
    H5D_rdcc_t* rdcc = &(dset->shared->cache.chunk);
    double miss_rate;
    char ascii[32];
 ...
 *-------------------------------------------------------------------------
 */
bool H5D__chunk_is_partial_edge_chunk(unsigned dset_ndims, const hsize_t* chunk_dims, const hsize_t scaled[], const hsize_t* dset_dims)
{
    unsigned u;             /* Local index variable */
    bool ret_value = false; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__chunk_file_alloc(const H5D_chk_idx_info_t* idx_info, const H5F_block_t* old_chunk, H5F_block_t* new_chunk, bool* need_insert, const hsize_t* scaled)
{
...
static int H5D__chunk_format_convert_cb(const H5D_chunk_rec_t* chunk_rec, void* _udata)
{
    ...
    if (new_idx_info->pline->nused && (new_idx_info->layout->u.chunk.flags & H5O_LAYOUT_CHUNK_DONT_FILTER_PARTIAL_BOUND_CHUNKS) &&
        (H5D__chunk_is_partial_edge_chunk(udata->dset_ndims, new_idx_info->layout->u.chunk.dim, chunk_rec->scaled, udata->dset_dims))) {
 ...
 *-------------------------------------------------------------------------
 */
static int H5D__chunk_index_empty_cb(const H5D_chunk_rec_t H5_ATTR_UNUSED* chunk_rec, void* _udata)
{
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__chunk_index_empty(const H5D_t* dset, bool* empty)
{
    H5D_chk_idx_info_t idx_info;   /* Chunked index info */
    H5D_rdcc_ent_t* ent;           /* Cache entry  */
    ...
    *empty = true;
    ...
    if (H5_addr_defined(idx_info.layout->storage.u.chunk.idx_addr)) {
        ...
        if ((dset->shared->layout.storage.u.chunk.ops->iterate)(&idx_info, H5D__chunk_index_empty_cb, empty) < 0) {
            HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL, "unable to retrieve allocated chunk information from index");
        }
    }
...
static int H5D__chunk_iter_cb(const H5D_chunk_rec_t* chunk_rec, void* udata)
{
    ...
    {
        ret_value = (data->op)(offset, (unsigned)chunk_rec->filter_mask, data->base_addr + chunk_rec->chunk_addr, chunk_rec->nbytes, data->op_data);
    }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__chunk_iter(H5D_t* dset, H5D_chunk_iter_op_t op, void* op_data)
{
    const H5D_rdcc_t* rdcc = NULL; /* Raw data chunk cache */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__chunk_get_offset_copy(const H5D_t* dset, const hsize_t* offset, hsize_t* offset_copy)
{
    unsigned u;
    herr_t ret_value = SUCCEED; /* Return value */
