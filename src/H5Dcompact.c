...
herr_t H5D__compact_fill(const H5D_t* dset)
{
    ...
    if (H5D__fill_init(&fb_info,
                       dset->shared->layout.storage.u.compact.buf,
                       NULL,
                       NULL,
                       NULL,
                       NULL,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__compact_init(H5F_t H5_ATTR_UNUSED* f, H5D_t* dset, hid_t H5_ATTR_UNUSED dapl_id, bool H5_ATTR_UNUSED open_op)
{
 ...
 *-------------------------------------------------------------------------
 */
static bool H5D__compact_is_space_alloc(const H5O_storage_t H5_ATTR_UNUSED* storage)
{
                                   ...
                                   hsize_t mem_offset_arr[])
{
    ...
    if (H5F_SHARED_HAS_FEATURE(io_info->f_sh, H5FD_FEAT_MEMMANAGE)) {
        ...
        if ((ret_value = H5VM_opvv(mem_max_nseq,
                                   mem_curr_seq,
                                   mem_size_arr,
                                   mem_offset_arr,
                                   dset_max_nseq,
                                   ...
                                   dset_size_arr,
                                   dset_offset_arr,
                                   H5D__compact_iovv_memmanage_cb,
                                   &udata)) < 0) {
        ...
        if ((ret_value = H5VM_memcpyvv(dset_info->buf.vp,
                                       mem_max_nseq,
                                       mem_curr_seq,
                                       mem_size_arr,
                                       mem_offset_arr,
                                       dset_info->store->compact.buf,
                                       dset_max_nseq,
                                       dset_curr_seq,
                                       dset_size_arr,
                                       dset_offset_arr)) < 0) {
                                    ...
                                    hsize_t mem_offset_arr[])
{
    ...
    if (H5F_SHARED_HAS_FEATURE(io_info->f_sh, H5FD_FEAT_MEMMANAGE)) {
        ...
        /* Use the vectorized memory copy routine to do actual work */
        if ((ret_value = H5VM_memcpyvv(dset_info->store->compact.buf,
                                       dset_max_nseq,
                                       dset_curr_seq,
                                       dset_size_arr,
                                       dset_offset_arr,
    ...
    if (H5T_detect_class(dt_src, H5T_VLEN, false) > 0) {
        ...
        if (f_src != f_dst) {
            ...
            if (cpy_info->expand_ref) {
                 ...
                 */
                if (H5O_copy_expand_ref(f_src, dt_src, storage_src->buf, storage_src->size, f_dst, storage_dst->buf, cpy_info) < 0) {
