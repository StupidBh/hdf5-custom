...
static ssize_t H5D__contig_readvv(const H5D_io_info_t* io_info,
                                  const H5D_dset_io_info_t* dinfo,
...
/* Helper routines */
static herr_t H5D__contig_write_one(H5D_io_info_t* io_info, H5D_dset_io_info_t* dset_info, hsize_t offset, size_t size);
static herr_t H5D__contig_may_use_select_io(H5D_io_info_t* io_info, const H5D_dset_io_info_t* dset_info, H5D_io_op_type_t op_type);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__contig_alloc(H5F_t* f, H5O_storage_contig_t* storage /*out */)
{
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__contig_fill(H5D_t* dset)
{
    ...
    if (H5D__fill_init(&fb_info, NULL, NULL, NULL, NULL, NULL, &dset->shared->dcpl_cache.fill, dset->shared->type, npoints, max_temp_buf) < 0) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__contig_check(const H5F_t* f, const H5O_layout_t* layout, const H5S_extent_t* extent, const H5T_t* dt)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__contig_init(H5F_t* f, H5D_t* dset, hid_t H5_ATTR_UNUSED dapl_id, bool H5_ATTR_UNUSED open_op)
{
    size_t tmp_sieve_buf_size;  /* Temporary holder for sieve buffer size */
    herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
bool H5D__contig_is_space_alloc(const H5O_storage_t* storage)
{
 ...
 *-------------------------------------------------------------------------
 */
bool H5D__contig_is_data_cached(const H5D_shared_t* shared_dset)
{
    FUNC_ENTER_PACKAGE_NOERR
...
static herr_t H5D__contig_may_use_select_io(H5D_io_info_t* io_info, const H5D_dset_io_info_t* dset_info, H5D_io_op_type_t op_type)
{
    ...
    else if ((op_type == H5D_IO_OP_READ && dataset->shared->cache.sieve.sieve_dirty) || (op_type == H5D_IO_OP_WRITE && dataset->shared->cache.sieve.sieve_buf)) {
        io_info->use_select_io = H5D_SELECTION_IO_MODE_OFF;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__contig_read(H5D_io_info_t* io_info, H5D_dset_io_info_t* dinfo)
{
    herr_t ret_value = SUCCEED; /* Return value */
    ...
    if (io_info->use_select_io == H5D_SELECTION_IO_MODE_ON) {
        ...
        if (H5D_LAYOUT_CB_PERFORM_IO(io_info)) {
            ...
            if (H5F_shared_select_read(H5F_SHARED(dinfo->dset->oloc.file),
                                       H5FD_MEM_DRAW,
                                       dinfo->nelmts > 0 ? 1 : 0,
                                       &dinfo->mem_space,
                                       &dinfo->file_space,
                                       &(dinfo->store->contig.dset_addr),
                                       &dst_type_size,
                                       &(dinfo->buf.vp)) < 0) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__contig_write(H5D_io_info_t* io_info, H5D_dset_io_info_t* dinfo)
{
    herr_t ret_value = SUCCEED; /* Return value */
    ...
    if (io_info->use_select_io == H5D_SELECTION_IO_MODE_ON) {
        ...
        if (H5D_LAYOUT_CB_PERFORM_IO(io_info)) {
             ...
             * already verified it won't be used, and the metadata accumulator
             * because this is raw data) */
            if (H5F_shared_select_write(H5F_SHARED(dinfo->dset->oloc.file),
                                        H5FD_MEM_DRAW,
                                        dinfo->nelmts > 0 ? 1 : 0,
                                        &dinfo->mem_space,
                                        &dinfo->file_space,
...
static herr_t H5D__contig_write_one(H5D_io_info_t* io_info, H5D_dset_io_info_t* dset_info, hsize_t offset, size_t size)
{
    ...
    if (H5D__contig_writevv(io_info, dset_info, (size_t)1, &dset_curr_seq, &dset_len, &dset_off, (size_t)1, &mem_curr_seq, &mem_len, &mem_off) < 0) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__contig_readvv_sieve_cb(hsize_t dst_off, hsize_t src_off, size_t len, void* _udata)
{
    ...
    if (NULL == sieve_info->sieve_buf) {
        ...
        if (len > sieve_info->sieve_buf_size) {
            ...
            if (H5F_shared_block_read(f_sh, H5FD_MEM_DRAW, sieve_info->sieve_loc, sieve_info->sieve_size, sieve_info->sieve_buf) < 0) {
                HGOTO_ERROR(H5E_DATASET, H5E_READERROR, FAIL, "block read failed");
            }
        ...
        /* Entire request is not within this data sieve buffer */
            ...
            if (len > sieve_info->sieve_buf_size) {
                ...
                if ((sieve_start >= addr && sieve_start < (contig_end + 1)) || ((sieve_end - 1) >= addr && (sieve_end - 1) < (contig_end + 1))) {
                    /* Flush the sieve buffer, if it's dirty */
                    if (sieve_info->sieve_dirty) {
                        /* Write to file */
                        if (H5F_shared_block_write(f_sh, H5FD_MEM_DRAW, sieve_start, sieve_size, sieve_info->sieve_buf) < 0) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__contig_readvv_cb(hsize_t dst_off, hsize_t src_off, size_t len, void* _udata)
{
    ...
    /* Write data */
    if (H5F_shared_block_read(udata->f_sh, H5FD_MEM_DRAW, (udata->dset_addr + dst_off), len, (udata->rbuf + src_off)) < 0) {
        HGOTO_ERROR(H5E_DATASET, H5E_WRITEERROR, FAIL, "block write failed");
    }
                                  ...
                                  hsize_t mem_off_arr[])
{
    ...
    if (H5F_SHARED_HAS_FEATURE(io_info->f_sh, H5FD_FEAT_DATA_SIEVE)) {
        ...
        if ((ret_value = H5VM_opvv(dset_max_nseq,
                                   dset_curr_seq,
                                   dset_len_arr,
                                   dset_off_arr,
                                   mem_max_nseq,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__contig_writevv_sieve_cb(hsize_t dst_off, hsize_t src_off, size_t len, void* _udata)
{
    ...
    if (NULL == sieve_info->sieve_buf) {
        ...
        /* Entire request is not within this data sieve buffer */
            ...
            /* Element size fits within the buffer size */
                ...
                if (((addr + len) == sieve_start || addr == sieve_end) && (len + sieve_size) <= sieve_info->sieve_buf_size && sieve_info->sieve_dirty) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__contig_writevv_cb(hsize_t dst_off, hsize_t src_off, size_t len, void* _udata)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__contig_io_term(H5D_io_info_t H5_ATTR_UNUSED* io_info, H5D_dset_io_info_t* di)
{
    herr_t ret_value = SUCCEED; /*return value        */
