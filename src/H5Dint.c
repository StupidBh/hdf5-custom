...
static herr_t H5D__vlen_get_buf_size_cb(void* elem, const H5T_t* type, unsigned ndim, const hsize_t* point, void* op_data);
static herr_t H5D__vlen_get_buf_size_gen_cb(void* elem, hid_t type_id, unsigned ndim, const hsize_t* point, void* op_data);
...
herr_t H5D__get_space_status(const H5D_t* dset, H5D_space_status_t* allocation)
{
    ...
    if (dset->shared->layout.type == H5D_CHUNKED) {
        ...
        if (H5D__get_num_chunks(dset, dset->shared->space, &n_chunks_alloc) < 0) {
            HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL, "unable to retrieve number of allocated chunks in dataset");
        }
...
static herr_t H5D__update_oh_info(H5F_t* file, H5D_t* dset, hid_t dapl_id)
{
    ...
    if (!(H5O_has_chksum(oh) || (H5F_RFIC_FLAGS(file) & H5F_RFIC_UNUSUAL_NUM_UNUSED_NUMERIC_BITS)) && H5T_is_numeric_with_unusual_unused_bits(type)) {
...
#ifdef H5O_ENABLE_BOGUS
    {
        ...
        if ((H5P_exist_plist(dc_plist, H5O_BOGUS_MSG_FLAGS_NAME) > 0) && (H5P_exist_plist(dc_plist, H5O_BOGUS_MSG_ID_NAME) > 0)) {
 ...
 *--------------------------------------------------------------------------
 */
static herr_t H5D__build_file_prefix(const H5D_t* dset, H5F_prefix_open_t prefix_type, char** file_prefix /*out*/)
{
...
static herr_t H5D__open_oid(H5D_t* dataset, hid_t dapl_id)
{
    ...
    if (msg_exists) {
        ...
        if (msg_exists) {
            ...
            case H5D_COMPACT: fill_prop->alloc_time = H5D_ALLOC_TIME_EARLY; break;

            case H5D_CONTIGUOUS: fill_prop->alloc_time = H5D_ALLOC_TIME_LATE; break;

            case H5D_CHUNKED: fill_prop->alloc_time = H5D_ALLOC_TIME_INCR; break;

            case H5D_VIRTUAL: fill_prop->alloc_time = H5D_ALLOC_TIME_INCR; break;

            case H5D_LAYOUT_ERROR:
            case H5D_NLAYOUTS:
            default              : HGOTO_ERROR(H5E_DATASET, H5E_UNSUPPORTED, FAIL, "not implemented yet");
            } /* end switch */
        } /* end else */
     ...
     */
    must_init_storage = (H5F_INTENT(dataset->oloc.file) & H5F_ACC_RDWR) && !(*dataset->shared->layout.ops->is_space_alloc)(&dataset->shared->layout.storage);
    must_init_storage = must_init_storage && (H5F_HAS_FEATURE(dataset->oloc.file, H5FD_FEAT_ALLOCATE_EARLY) ||
                                              (H5F_HAS_FEATURE(dataset->oloc.file, H5FD_FEAT_HAS_MPI) && dataset->shared->dcpl_cache.pline.nused == 0));
...
herr_t H5D_close(H5D_t* dataset)
{
    ...
    if (dataset->shared->fo_count == 0) {
        ...
        if (dataset->shared->cache.sieve.sieve_buf) {
            dataset->shared->cache.sieve.sieve_buf = (unsigned char*)H5FL_BLK_FREE(sieve_buf, dataset->shared->cache.sieve.sieve_buf);
        }
        ...
        case H5D_VIRTUAL:
            {
                ...
                assert(dataset->shared->layout.storage.u.virt.list || (dataset->shared->layout.storage.u.virt.list_nused == 0));
                ...
                for (i = 0; i < dataset->shared->layout.storage.u.virt.list_nused; i++) {
                    ...
                    for (j = 0; j < dataset->shared->layout.storage.u.virt.list[i].sub_dset_nused; j++) {
                        if (dataset->shared->layout.storage.u.virt.list[i].sub_dset[j].dset) {
                            assert(dataset->shared->layout.storage.u.virt.list[i].sub_dset[j].dset != dataset);
        ...
        case H5D_LAYOUT_ERROR:
        case H5D_NLAYOUTS:
        default              : assert("not implemented yet" && 0);
#ifdef NDEBUG
        ...
        if (dataset->shared->dcpl_id != H5P_DATASET_CREATE_DEFAULT) {
            free_failed |= (H5O_msg_reset(H5O_PLINE_ID, &dataset->shared->dcpl_cache.pline) < 0) || (H5O_msg_reset(H5O_LAYOUT_ID, &dataset->shared->layout) < 0) ||
                           (H5O_msg_reset(H5O_FILL_ID, &dataset->shared->dcpl_cache.fill) < 0) || (H5O_msg_reset(H5O_EFL_ID, &dataset->shared->dcpl_cache.efl) < 0);
        }
         ...
         */
        free_failed |= (H5I_dec_ref(dataset->shared->type_id) < 0) || (H5S_close(dataset->shared->space) < 0) || (H5I_dec_ref(dataset->shared->dcpl_id) < 0) ||
 ...
 *-------------------------------------------------------------------------
 */
H5O_loc_t* H5D_oloc(H5D_t* dataset) { /* Use FUNC_ENTER_NOAPI_NOINIT_NOERR here to avoid performance issues */
                                      FUNC_ENTER_NOAPI_NOINIT_NOERR FUNC_LEAVE_NOAPI(dataset ? &(dataset -> oloc) : (H5O_loc_t*)NULL)
} /* end H5D_oloc() */
 ...
 *-------------------------------------------------------------------------
 */
H5G_name_t* H5D_nameof(H5D_t* dataset) { /* Use FUNC_ENTER_NOAPI_NOINIT_NOERR here to avoid performance issues */
                                         FUNC_ENTER_NOAPI_NOINIT_NOERR FUNC_LEAVE_NOAPI(dataset ? &(dataset->path) : NULL)
} /* end H5D_nameof() */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__alloc_storage(H5D_t* dset, H5D_time_alloc_t time_alloc, bool full_overwrite, hsize_t old_dim[])
{
...
herr_t H5D__get_storage_size(const H5D_t* dset, hsize_t* storage_size)
{
    ...
    case H5D_COMPACT: *storage_size = dset->shared->layout.storage.u.compact.size; break;
    ...
    case H5D_LAYOUT_ERROR:
    case H5D_NLAYOUTS:
    default              : HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL, "not a dataset type");
    }
...
haddr_t H5D__get_offset(const H5D_t* dset)
{
    ...
    case H5D_CONTIGUOUS:
         ...
         */
        if (dset->shared->dcpl_cache.efl.nused == 0 || H5_addr_defined(dset->shared->layout.storage.u.contig.addr)) {
    ...
    case H5D_LAYOUT_ERROR:
    case H5D_NLAYOUTS:
    default              : HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, HADDR_UNDEF, "unknown dataset layout type");
    }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__vlen_get_buf_size_cb(void H5_ATTR_UNUSED* elem, const H5T_t* type, unsigned H5_ATTR_UNUSED ndim, const hsize_t* point, void* op_data)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__vlen_get_buf_size_gen_cb(void H5_ATTR_UNUSED* elem, hid_t type_id, unsigned H5_ATTR_UNUSED ndim, const hsize_t* point, void* op_data)
{
    ...
    if (H5VL_dataset_read(1,
                          &vol_obj_data,
                          H5VL_OBJ_CONNECTOR(vlen_bufsize->dset_vol_obj),
                          &type_id,
                          &vlen_bufsize->mspace_id,
                          &vlen_bufsize->fspace_id,
                          vlen_bufsize->dxpl_id,
                          &vlen_bufsize->common.fl_tbuf,
                          H5_REQUEST_NULL) < 0) {
...
static herr_t H5D__check_filters(H5D_t* dataset)
{
    ...
    if (!dataset->shared->checked_filters) {
        ...
        if (fill_status == H5D_FILL_VALUE_DEFAULT || fill_status == H5D_FILL_VALUE_USER_DEFINED) {
            if (fill->fill_time == H5D_FILL_TIME_ALLOC || (fill->fill_time == H5D_FILL_TIME_IFSET && fill_status == H5D_FILL_VALUE_USER_DEFINED)) {
...
herr_t H5D__set_extent(H5D_t* dset, const hsize_t* size)
{
    ...
    if (changed) {
        ...
        for (dim_idx = 0; dim_idx < dset->shared->ndims; dim_idx++) {
            ...
            if (H5D_CHUNKED == dset->shared->layout.type && dset->shared->ndims > 1) {
                ...
                if (scaled != dset->shared->cache.chunk.scaled_dims[dim_idx]) {
                    ...
                    if ((scaled > dset->shared->cache.chunk.nslots && dset->shared->cache.chunk.scaled_dims[dim_idx] <= dset->shared->cache.chunk.nslots) ||
                        (scaled <= dset->shared->cache.chunk.nslots && dset->shared->cache.chunk.scaled_dims[dim_idx] > dset->shared->cache.chunk.nslots)) {
                        update_chunks = true;
                    }
        ...
        if (H5D_CHUNKED == dset->shared->layout.type) {
             ...
             * expansion */
            if (expand && (dset->shared->layout.u.chunk.flags & H5O_LAYOUT_CHUNK_DONT_FILTER_PARTIAL_BOUND_CHUNKS) && (dset->shared->dcpl_cache.pline.nused > 0)) {
...
herr_t H5D__flush_sieve_buf(H5D_t* dataset)
{
    ...
    if (dataset->shared->cache.sieve.sieve_buf && dataset->shared->cache.sieve.sieve_dirty) {
        ...
        if (H5F_shared_block_write(H5F_SHARED(dataset->oloc.file),
                                   H5FD_MEM_DRAW,
                                   dataset->shared->cache.sieve.sieve_loc,
                                   dataset->shared->cache.sieve.sieve_size,
                                   dataset->shared->cache.sieve.sieve_buf) < 0) {
...
herr_t H5D__format_convert(H5D_t* dataset)
{
    ...
    case H5D_CHUNKED:
        ...
        if (new_idx_info.layout->storage.u.chunk.ops->init &&
            (new_idx_info.layout->storage.u.chunk.ops->init)(&new_idx_info, dataset->shared->space, dataset->oloc.addr) < 0) {
        ...
        if (idx_info.layout->storage.u.chunk.ops->dest && (idx_info.layout->storage.u.chunk.ops->dest)(&idx_info) < 0) {
    ...
    case H5D_VIRTUAL: HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL, "virtual dataset layout not supported");

    case H5D_LAYOUT_ERROR:
    case H5D_NLAYOUTS    : HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL, "invalid dataset layout type");

    default: HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL, "unknown dataset layout type");
    } /* end switch */
    ...
    if (ret_value < 0 && dataset->shared->layout.type == H5D_CHUNKED) {
        ...
        if (init_new_index) {
            ...
            if (new_idx_info.layout->storage.u.chunk.ops->dest && (new_idx_info.layout->storage.u.chunk.ops->dest)(&new_idx_info) < 0) {
 ...
 *-------------------------------------------------------------------------
 */
static int H5D__flush_all_cb(void* _dataset, hid_t H5_ATTR_UNUSED id, void* _udata)
{
...
hid_t H5D_get_create_plist(const H5D_t* dset)
{
    ...
    if (copied_fill.buf != NULL && copied_fill.type == NULL) {
        ...
        if (!H5T_path_noop(tpath)) {
            ...
            if (H5T_detect_class(dst_type, H5T_VLEN, false) > 0 || H5T_detect_class(dst_type, H5T_REFERENCE, false) > 0) {
            ...
            /* Convert fill value */
            if (H5T_convert(tpath, dset->shared->type, dst_type, (size_t)1, (size_t)0, (size_t)0, copied_fill.buf, bkg_buf) < 0) {
                if (bkg_buf) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__refresh(H5D_t* dset, hid_t dset_id)
{
 ...
 *-------------------------------------------------------------------------
 */
hid_t H5D_get_dcpl_id(const H5D_obj_create_t* d)
{
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D_flush_layout_to_dcpl(const H5D_t* dset)
{
    herr_t ret_value = SUCCEED;
