...
herr_t H5D__read(size_t count, H5D_dset_io_info_t* dset_info)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        if (dset_info[i].nelmts > 0 && true == H5S_SELECT_SHAPE_SAME(dset_info[i].mem_space, dset_info[i].file_space) &&
            ...
            if (H5S_select_construct_projection(orig_mem_space[i],
                                                &dset_info[i].mem_space,
                                                (unsigned)H5S_GET_EXTENT_NDIMS(dset_info[i].file_space),
                                                (hsize_t)dset_info[i].type_info.dst_type_size,
                                                &buf_adj) < 0) {
        ...
        if (dset_info[i].nelmts > 0 && dset_info[i].dset->shared->dcpl_cache.efl.nused == 0 &&
            ...
            !(dset_info[i].dset->shared->layout.ops->is_data_cached && (*dset_info[i].dset->shared->layout.ops->is_data_cached)(dset_info[i].dset->shared))) {
            ...
            if (fill_status == H5D_FILL_VALUE_UNDEFINED && (dset_info[i].dset->shared->dcpl_cache.fill.fill_time == H5D_FILL_TIME_ALLOC ||
            ...
            if (dset_info[i].dset->shared->dcpl_cache.fill.fill_time != H5D_FILL_TIME_NEVER) {
                ...
                if (H5D__fill(dset_info[i].dset->shared->dcpl_cache.fill.buf,
                              dset_info[i].dset->shared->type,
                              dset_info[i].buf.vp,
                              dset_info[i].type_info.mem_type,
        ...
        if (dset_info[i].layout_ops.io_init && (dset_info[i].layout_ops.io_init)(&io_info, &(dset_info[i])) < 0) {
            HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL, "can't initialize I/O info");
        }
        io_op_init++;
    ...
    if (io_info.md_io_ops.multi_read_md) {
        ...
        if (!H5D_LAYOUT_CB_PERFORM_IO(&io_info) && io_info.piece_count > 0) {
            ...
            if (io_info.max_tconv_type_size > 0) {
                if (NULL == (io_info.sel_pieces = H5MM_malloc(io_info.piece_count * sizeof(io_info.sel_pieces[0])))) {
                    HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL, "unable to allocate array of selected pieces");
                }
            }
        }
    ...
    for (i = 0; i < count; i++) {
        ...
        if (dset_info[i].nelmts > 0 && true == H5S_SELECT_SHAPE_SAME(dset_info[i].mem_space, dset_info[i].file_space) &&
            ...
            if (H5S_select_construct_projection(orig_mem_space[i],
                                                &dset_info[i].mem_space,
                                                (unsigned)H5S_GET_EXTENT_NDIMS(dset_info[i].file_space),
                                                dset_info[i].type_info.src_type_size,
                                                &buf_adj) < 0) {
                HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL, "unable to construct projected memory dataspace");
            }
            assert(dset_info[i].mem_space);
...
static herr_t H5D__typeinfo_init(H5D_io_info_t* io_info, H5D_dset_io_info_t* dset_info, const H5T_t* mem_type)
{
    ...
    if (type_info->is_xform_noop && type_info->is_conv_noop) {
        ...
        io_info->max_tconv_type_size = MAX3(io_info->max_tconv_type_size, type_info->src_type_size, type_info->dst_type_size);
        ...
        if (opt == true) {
            ...
            if (io_info->op_type == H5D_IO_OP_WRITE) {
                ...
                if (mpi_file_sync_required) {
                    HGOTO_ERROR(H5E_DATASET, H5E_NO_INDEPENDENT, FAIL, "Can't perform independent write when MPI_File_sync is required by ROMIO driver.");
                }
            }
            ...
            if (io_info->op_type == H5D_IO_OP_WRITE) {
                ...
                if (i < io_info->count) {
                    ...
                    if (comm_size > 1) {
                        ...
                        if (H5D__mpio_get_no_coll_cause_strings(local_no_coll_cause_string, 512, global_no_coll_cause_string, 512) < 0) {
                            HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL, "can't get reasons for breaking collective I/O");
                        }
                        ...
                        HGOTO_ERROR(H5E_IO,
                                    ...
                                    "        Local causes: %s\n"
                                    "        Global causes: %s",
                                    local_no_coll_cause_string,
                                    global_no_coll_cause_string);
                    }
                }
            }
    ...
    if (io_info->max_tconv_type_size) {
        ...
        if (io_info->use_select_io == H5D_SELECTION_IO_MODE_ON) {
            ...
            if (target_size < io_info->max_tconv_type_size) {
                ...
                default_buffer_info = (bool)((H5D_TEMP_BUF_SIZE == max_temp_buf) && (NULL == tconv_buf) && (NULL == bkgr_buf));
