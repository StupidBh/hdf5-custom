...
herr_t H5D__layout_set_io_ops(const H5D_t* dataset)
{
    ...
    case H5D_CHUNKED:
        ...
        case H5D_CHUNK_IDX_BTREE: dataset->shared->layout.storage.u.chunk.ops = H5D_COPS_BTREE; break;

        case H5D_CHUNK_IDX_NONE: dataset->shared->layout.storage.u.chunk.ops = H5D_COPS_NONE; break;

        case H5D_CHUNK_IDX_SINGLE: dataset->shared->layout.storage.u.chunk.ops = H5D_COPS_SINGLE; break;

        case H5D_CHUNK_IDX_FARRAY: dataset->shared->layout.storage.u.chunk.ops = H5D_COPS_FARRAY; break;

        case H5D_CHUNK_IDX_EARRAY: dataset->shared->layout.storage.u.chunk.ops = H5D_COPS_EARRAY; break;

        case H5D_CHUNK_IDX_BT2: dataset->shared->layout.storage.u.chunk.ops = H5D_COPS_BT2; break;

        case H5D_CHUNK_IDX_NTYPES:
        default                  : assert(0 && "Unknown chunk index method!"); HGOTO_ERROR(H5E_DATASET, H5E_UNSUPPORTED, FAIL, "unknown chunk index method");
        } /* end switch */
        break;

    case H5D_COMPACT: dataset->shared->layout.ops = H5D_LOPS_COMPACT; break;

    case H5D_VIRTUAL: dataset->shared->layout.ops = H5D_LOPS_VIRTUAL; break;

    case H5D_LAYOUT_ERROR:
    case H5D_NLAYOUTS:
    default              : HGOTO_ERROR(H5E_DATASET, H5E_UNSUPPORTED, FAIL, "unknown storage method");
    } /* end switch */
 ...
 *-------------------------------------------------------------------------
 */
size_t H5D__layout_meta_size(const H5F_t* f, const H5O_layout_t* layout, bool include_compact_data)
{
    ...
    case H5D_CHUNKED:
        if (layout->version < H5O_LAYOUT_VERSION_4) {
            ...
            case H5D_CHUNK_IDX_BTREE: HGOTO_ERROR(H5E_OHDR, H5E_BADVALUE, 0, "v1 B-tree index type found for layout message >v3");
            ...
            case H5D_CHUNK_IDX_NTYPES:
            default                  : HGOTO_ERROR(H5E_OHDR, H5E_CANTENCODE, 0, "Invalid chunk index type");
            } /* end switch */
    ...
    case H5D_LAYOUT_ERROR:
    case H5D_NLAYOUTS:
    default              : HGOTO_ERROR(H5E_OHDR, H5E_CANTENCODE, 0, "Invalid layout class");
    } /* end switch */
...
herr_t H5D__layout_oh_create(H5F_t* file, H5O_t* oh, H5D_t* dset, hid_t dapl_id)
{
    ...
    if (fill_prop->alloc_time == H5D_ALLOC_TIME_EARLY && H5D_COMPACT != layout->type && !dset->shared->dcpl_cache.pline.nused &&
...
herr_t H5D__layout_oh_read(H5D_t* dataset, hid_t dapl_id, H5P_genplist_t* plist)
{
    ...
    if (dataset->shared->layout.ops->init && (dataset->shared->layout.ops->init)(dataset->oloc.file, dataset, dapl_id, true) < 0) {
        HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL, "unable to initialize layout information");
    }
