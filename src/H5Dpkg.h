 ...
 */
    ...
    #define H5D_LAYOUT_CB_PERFORM_IO(IO_INFO) \
        (((IO_INFO)->use_select_io == H5D_SELECTION_IO_MODE_OFF) || ((IO_INFO)->count == 1 && (IO_INFO)->max_tconv_type_size == 0))
    ...
    #define H5D_INIT_PIECE_TCONV(IO_INFO, DINFO, PIECE_INFO)                                                                                          \
        {                                                                                                                                             \
            ...
            if ((IO_INFO)->may_use_in_place_tconv) {                                                                                                  \
                size_t mem_type_size = ((IO_INFO)->op_type == H5D_IO_OP_READ) ? (DINFO)->type_info.dst_type_size : (DINFO)->type_info.src_type_size;  \
                size_t file_type_size = ((IO_INFO)->op_type == H5D_IO_OP_READ) ? (DINFO)->type_info.src_type_size : (DINFO)->type_info.dst_type_size; \
                                                                                                                                                      \
    ...
    #define H5D_VIRTUAL_FIND_OR_ADD_NAME(NAME_TYPE, LAYOUT, STR, STRLEN, ENT, ERR)                                                                               \
        do {                                                                                                                                                     \
            ...
            if ((ENT) > (LAYOUT)->storage.u.virt.list)                                                                                                           \
                HASH_FIND(hh_source_##NAME_TYPE, (LAYOUT)->storage.u.virt.source_##NAME_TYPE##_hash_table, STR, STRLEN, _tmp_ent);                               \
            if (_tmp_ent) {                                                                                                                                      \
                 ...
                 * line. */                                                                                                                                      \
                HASH_ADD_KEYPTR(hh_source_##NAME_TYPE, (LAYOUT)->storage.u.virt.source_##NAME_TYPE##_hash_table, (ENT)->source_##NAME_TYPE##_name, STRLEN, ENT); \
            }                                                                                                                                                    \
        } while (0)
...
typedef herr_t (*H5D_layout_mdio_init_func_t)(struct H5D_io_info_t* io_info, struct H5D_dset_io_info_t* dinfo);
...
typedef struct H5D_layout_ops_t
{
    ...
    H5D_layout_is_data_cached_func_t is_data_cached; /* Query routine to determine if any raw data is cached.  If routine is not present
...
typedef herr_t (*H5D_io_single_read_func_t)(const struct H5D_io_info_t* io_info, const struct H5D_dset_io_info_t* dset_info);
typedef herr_t (*H5D_io_single_write_func_t)(const struct H5D_io_info_t* io_info, const struct H5D_dset_io_info_t* dset_info);

typedef herr_t (*H5D_io_single_read_md_func_t)(const struct H5D_io_info_t* io_info, hsize_t nelmts, H5S_t* file_space, H5S_t* mem_space);
typedef herr_t (*H5D_io_single_write_md_func_t)(const struct H5D_io_info_t* io_info, hsize_t nelmts, H5S_t* file_space, H5S_t* mem_space);
...
typedef struct H5D_chunk_ops_t
{
    ...
    H5D_chunk_load_metadata_func_t load_metadata;   /* Routine to load additional chunk index metadata, such as fixed array data blocks */
...
typedef struct H5D_rdcc_t
{
    ...
    struct H5D_rdcc_ent_t* tmp_head;     /* Head of temporary doubly linked list.  Chunks on this list are not in the hash table
...
H5_DLL herr_t H5D__alloc_storage(H5D_t* dset, H5D_time_alloc_t time_alloc, bool full_overwrite, hsize_t old_dim[]);
...
H5_DLL herr_t H5D_select_io_mem(void* dst_buf, H5S_t* dst_space, const void* src_buf, H5S_t* src_space, size_t elmt_size, size_t nelmts);
...
H5_DLL size_t H5D__gather_mem(const void* _buf, H5S_sel_iter_t* iter, size_t nelmts, void* _tgath_buf /*out*/);
...
H5_DLL herr_t
    H5D__contig_copy(H5F_t* f_src, const H5O_storage_contig_t* storage_src, H5F_t* f_dst, H5O_storage_contig_t* storage_dst, H5T_t* src_dtype, H5O_copy_t* cpy_info);
H5_DLL herr_t H5D__contig_delete(H5F_t* f, const H5O_storage_t* store);
H5_DLL herr_t H5D__contig_check(const H5F_t* f, const H5O_layout_t* layout, const H5S_extent_t* extent, const H5T_t* dt);

/* Functions that operate on chunked dataset storage */
H5_DLL htri_t H5D__chunk_cacheable(const H5D_io_info_t* io_info, H5D_dset_io_info_t* dset_info, haddr_t caddr, bool write_op);
...
H5_DLL herr_t
    H5D__chunk_file_alloc(const H5D_chk_idx_info_t* idx_info, const H5F_block_t* old_chunk, H5F_block_t* new_chunk, bool* need_insert, const hsize_t* scaled);
...
H5_DLL bool H5D__chunk_is_partial_edge_chunk(unsigned dset_ndims, const hsize_t* chunk_dims, const hsize_t* chunk_scaled, const hsize_t* dset_dims);
...
H5_DLL herr_t H5D__chunk_direct_write(H5D_t* dset, uint32_t filters, hsize_t* offset, size_t data_size, const void* buf);
H5_DLL herr_t H5D__chunk_direct_read(const H5D_t* dset, hsize_t* offset, uint32_t* filters, void* buf, size_t* nalloc);
...
H5_DLL herr_t
    H5D__compact_copy(H5F_t* f_src, H5O_storage_compact_t* storage_src, H5F_t* f_dst, H5O_storage_compact_t* storage_dst, H5T_t* src_dtype, H5O_copy_t* cpy_info);
...
H5_DLL herr_t H5D__fill(const void* fill, const H5T_t* fill_type, void* buf, const H5T_t* buf_type, H5S_t* space);
...
H5_DLL herr_t H5D__mpio_select_read(const H5D_io_info_t* io_info, hsize_t nelmts, H5S_t* file_space, H5S_t* mem_space);

/* MPI-IO function to write multi-dsets (Chunk, Contig), it will select either
 * regular or irregular write */
H5_DLL herr_t H5D__mpio_select_write(const H5D_io_info_t* io_info, hsize_t nelmts, H5S_t* file_space, H5S_t* mem_space);
...
H5_DLL herr_t H5D__mpio_get_no_coll_cause_strings(char* local_cause, size_t local_cause_len, char* global_cause, size_t global_cause_len);
