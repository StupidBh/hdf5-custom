...
#define H5D__SCATGATH_USE_CMPD_OPT_READ(DSET_INFO, IN_PLACE_TCONV) \
    ((DSET_INFO)->type_info.cmpd_subset && H5T_SUBSET_FALSE != (DSET_INFO)->type_info.cmpd_subset->subset && !(IN_PLACE_TCONV))
...
/* Local Prototypes */
/********************/
static herr_t H5D__scatter_file(const H5D_io_info_t* io_info, const H5D_dset_io_info_t* dset_info, H5S_sel_iter_t* file_iter, size_t nelmts, const void* buf);
static size_t H5D__gather_file(const H5D_io_info_t* io_info, const H5D_dset_io_info_t* dset_info, H5S_sel_iter_t* file_iter, size_t nelmts, void* buf);
static herr_t H5D__compound_opt_read(size_t nelmts, H5S_sel_iter_t* iter, const H5D_type_info_t* type_info, uint8_t* tconv_buf, void* user_buf /*out*/);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__scatter_file(const H5D_io_info_t* _io_info, const H5D_dset_io_info_t* _dset_info, H5S_sel_iter_t* iter, size_t nelmts, const void* _buf)
{
    ...
    while (nelmts > 0) {
        ...
        if ((*tmp_dset_info.layout_ops.writevv)(&tmp_io_info, &tmp_dset_info, nseq, &dset_curr_seq, len, off, (size_t)1, &mem_curr_seq, &mem_len, &mem_off) < 0) {
 ...
 *-------------------------------------------------------------------------
 */
static size_t H5D__gather_file(const H5D_io_info_t* _io_info, const H5D_dset_io_info_t* _dset_info, H5S_sel_iter_t* iter, size_t nelmts, void* _buf /*out*/)
{
    H5D_io_info_t tmp_io_info;        /* Temporary I/O info object */
    H5D_dset_io_info_t tmp_dset_info; /* Temporary I/O info object */
    hsize_t* off = NULL;              /* Pointer to sequence offsets */
    ...
    while (nelmts > 0) {
        ...
        if ((*tmp_dset_info.layout_ops.readvv)(&tmp_io_info, &tmp_dset_info, nseq, &dset_curr_seq, len, off, (size_t)1, &mem_curr_seq, &mem_len, &mem_off) < 0) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__scatter_mem(const void* _tscat_buf, H5S_sel_iter_t* iter, size_t nelmts, void* _buf /*out*/)
{
 ...
 *-------------------------------------------------------------------------
 */
size_t H5D__gather_mem(const void* _buf, H5S_sel_iter_t* iter, size_t nelmts, void* _tgath_buf /*out*/)
{
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__scatgath_read(const H5D_io_info_t* io_info, const H5D_dset_io_info_t* dset_info)
{
     ...
     * anyways). */
    if (in_place_tconv && H5D__SCATGATH_USE_CMPD_OPT_READ(dset_info, false) && (io_info->use_select_io != H5D_SELECTION_IO_MODE_ON) &&
    ...
    if (H5S_select_iter_init(file_iter, dset_info->file_space, dset_info->type_info.src_type_size, H5S_SEL_ITER_GET_SEQ_LIST_SORTED) < 0) {
    ...
    for (smine_start = 0; smine_start < dset_info->nelmts; smine_start += smine_nelmts) {
        ...
        if (H5D__SCATGATH_USE_CMPD_OPT_READ(dset_info, in_place_tconv)) {
            if (H5D__compound_opt_read(smine_nelmts, mem_iter, &dset_info->type_info, tmp_buf, buf /*out*/) < 0) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__scatgath_write(const H5D_io_info_t* io_info, const H5D_dset_io_info_t* dset_info)
{
     ...
     * background buffer (and therefore could not do the I/O in one operation with in-place conversion
     * anyways). */
    if (in_place_tconv && H5D__SCATGATH_USE_CMPD_OPT_WRITE(dset_info, false) && (io_info->use_select_io != H5D_SELECTION_IO_MODE_ON) &&
        (dset_info->type_info.need_bkg || (dset_info->nelmts <= dset_info->type_info.request_nelmts))) {
        in_place_tconv = false;
    }
    ...
    /* Figure out the strip mine size. */
    if (H5S_select_iter_init(file_iter, dset_info->file_space, dset_info->type_info.dst_type_size, H5S_SEL_ITER_GET_SEQ_LIST_SORTED) < 0) {
        HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL, "unable to initialize file selection information");
    }
    file_iter_init = true; /*file selection iteration info has been initialized */
    ...
    mem_iter_init = true; /*file selection iteration info has been initialized */
    if (H5S_select_iter_init(bkg_iter, dset_info->file_space, dset_info->type_info.dst_type_size, H5S_SEL_ITER_GET_SEQ_LIST_SORTED) < 0) {
        HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL, "unable to initialize background selection information");
    }
    bkg_iter_init = true; /*file selection iteration info has been initialized */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__scatgath_read_select(H5D_io_info_t* io_info)
{
    ...
    for (i = 0; i < io_info->pieces_added; i++) {
        ...
        if (dset_info->type_info.is_xform_noop && dset_info->type_info.is_conv_noop) {
            ...
            if (NULL == (tmp_mem_spaces[i] = H5S_create_simple(1, &io_info->sel_pieces[i]->piece_points, NULL))) {
            ...
            if (io_info->sel_pieces[i]->in_place_tconv) {
                ...
                tconv_bytes_used += io_info->sel_pieces[i]->piece_points * MAX(dset_info->type_info.src_type_size, dset_info->type_info.dst_type_size);
            ...
            if (!H5D__SCATGATH_USE_CMPD_OPT_READ(dset_info, io_info->sel_pieces[i]->in_place_tconv)) {
                ...
                if (dset_info->type_info.need_bkg) {
                    ...
                    bkg_bytes_used += io_info->sel_pieces[i]->piece_points * dset_info->type_info.dst_type_size;
                    ...
                    if (H5T_BKG_YES == dset_info->type_info.need_bkg) {
                        ...
                        if (H5S_select_iter_init(mem_iter, io_info->mem_spaces[i], dset_info->type_info.dst_type_size, 0) < 0) {
                            HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL, "unable to initialize memory selection information");
                        }
                        mem_iter_init = true; /* Memory selection iteration info has been initialized */

                        if ((size_t)io_info->sel_pieces[i]->piece_points !=
                            H5D__gather_mem(io_info->rbufs[i], mem_iter, (size_t)io_info->sel_pieces[i]->piece_points, tmp_bkg_buf /*out*/)) {
                            HGOTO_ERROR(H5E_IO, H5E_READERROR, FAIL, "mem gather failed");
                        }
    ...
    if (H5F_shared_select_read(io_info->f_sh,
                               H5FD_MEM_DRAW,
                               (uint32_t)io_info->pieces_added,
                               tmp_mem_spaces,
                               io_info->file_spaces,
                               io_info->addrs,
                               io_info->element_sizes,
                               tmp_bufs) < 0) {
    ...
    for (i = 0; i < io_info->pieces_added; i++) {
        ...
        if (tmp_mem_spaces[i] != io_info->mem_spaces[i]) {
             ...
             */
            if (H5D__SCATGATH_USE_CMPD_OPT_READ(dset_info, io_info->sel_pieces[i]->in_place_tconv)) {
                if (H5D__compound_opt_read((size_t)io_info->sel_pieces[i]->piece_points, mem_iter, &dset_info->type_info, tmp_bufs[i], io_info->rbufs[i] /*out*/) <
                    0) {
                    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL, "datatype conversion failed");
                }
            }
                ...
                if (!dset_info->type_info.is_xform_noop) {
                    ...
                    if (H5Z_xform_eval(data_transform, tmp_bufs[i], (size_t)io_info->sel_pieces[i]->piece_points, dset_info->type_info.mem_type) < 0) {
                        HGOTO_ERROR(H5E_DATASET, H5E_BADVALUE, FAIL, "Error performing data transform");
                    }
                }
                ...
                /* Scatter the data into memory if this was not an in-place conversion */
                if (!io_info->sel_pieces[i]->in_place_tconv) {
                    if (H5D__scatter_mem(tmp_bufs[i], mem_iter, (size_t)io_info->sel_pieces[i]->piece_points, io_info->rbufs[i] /*out*/) < 0) {
                        HGOTO_ERROR(H5E_DATASET, H5E_READERROR, FAIL, "scatter failed");
                    }
                }
            }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__scatgath_write_select(H5D_io_info_t* io_info)
{
    ...
    for (i = 0; i < io_info->pieces_added; i++) {
        ...
        if (dset_info->type_info.is_xform_noop && dset_info->type_info.is_conv_noop) {
            ...
            /* Initialize memory iterator */
            assert(!mem_iter_init);
            if (H5S_select_iter_init(mem_iter, io_info->mem_spaces[i], dset_info->type_info.src_type_size, 0) < 0) {
                HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL, "unable to initialize memory selection information");
            }
            mem_iter_init = true; /* Memory selection iteration info has been initialized */

            /* Create block memory space */
            if (NULL == (write_mem_spaces[i] = H5S_create_simple(1, &io_info->sel_pieces[i]->piece_points, NULL))) {
                HGOTO_ERROR(H5E_DATASET, H5E_CANTCREATE, FAIL, "unable to create simple memory dataspace");
            }
            spaces_added++;
            ...
            if (io_info->sel_pieces[i]->in_place_tconv) {
                ...
                if ((size_t)io_info->sel_pieces[i]->piece_points !=
                    H5D__gather_mem(io_info->wbufs[i], mem_iter, (size_t)io_info->sel_pieces[i]->piece_points, tmp_write_buf /*out*/)) {
                    HGOTO_ERROR(H5E_IO, H5E_WRITEERROR, FAIL, "mem gather failed");
                }
            }
            ...
            if (H5D__SCATGATH_USE_CMPD_OPT_WRITE(dset_info, io_info->sel_pieces[i]->in_place_tconv)) {
                if (H5D__compound_opt_write((size_t)io_info->sel_pieces[i]->piece_points, &dset_info->type_info, tmp_write_buf) < 0) {
                    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL, "datatype conversion failed");
                }
                ...
                if (H5T_BKG_YES == dset_info->type_info.need_bkg) {
                    ...
                    if (!dset_info->type_info.is_xform_noop) {
                        ...
                        if (H5Z_xform_eval(data_transform, tmp_write_buf, (size_t)io_info->sel_pieces[i]->piece_points, dset_info->type_info.mem_type) < 0) {
                            HGOTO_ERROR(H5E_DATASET, H5E_BADVALUE, FAIL, "Error performing data transform");
                        }
                    }
    ...
    if (io_info->must_fill_bkg) {
        ...
        if (H5F_shared_select_read(io_info->f_sh, H5FD_MEM_DRAW, (uint32_t)bkg_pieces, bkg_mem_spaces, bkg_file_spaces, bkg_addrs, bkg_element_sizes, bkg_bufs) <
            0) {
        ...
        for (i = 0; i < io_info->pieces_added; i++) {
            ...
            if ((H5T_BKG_YES == dset_info->type_info.need_bkg) && !H5D__SCATGATH_USE_CMPD_OPT_WRITE(dset_info, io_info->sel_pieces[i]->in_place_tconv)) {
                ...
                if (io_info->sel_pieces[i]->in_place_tconv) {
                     ...
                     * buf so this is OK. */
                    tmp_write_buf = (void*)((uint8_t*)io_info->tconv_buf + ((const uint8_t*)write_bufs[i] - (const uint8_t*)io_info->tconv_buf));
                }
    ...
    /* Write data to disk */
    H5_CHECK_OVERFLOW(io_info->pieces_added, size_t, uint32_t);
    if (H5F_shared_select_write(io_info->f_sh,
                                H5FD_MEM_DRAW,
                                (uint32_t)io_info->pieces_added,
                                write_mem_spaces,
                                io_info->file_spaces,
...
static herr_t H5D__compound_opt_read(size_t nelmts, H5S_sel_iter_t* iter, const H5D_type_info_t* type_info, uint8_t* tconv_buf, void* user_buf /*out*/)
{
    ...
    assert(H5T_SUBSET_SRC == type_info->cmpd_subset->subset || H5T_SUBSET_DST == type_info->cmpd_subset->subset);
    assert(user_buf);
