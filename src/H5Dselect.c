...
static herr_t H5D__select_io(const H5D_io_info_t* io_info, const H5D_dset_io_info_t* dset_info, size_t elmt_size)
{
    ...
    if (nelmts == 1) {
        ...
        if (io_info->op_type == H5D_IO_OP_READ) {
            if ((tmp_file_len = (*dset_info->layout_ops.readvv)(io_info,
                                                                dset_info,
                                                                file_nseq,
                                                                &curr_file_seq,
                                                                &single_file_len,
            ...
            if ((tmp_file_len = (*dset_info->layout_ops.writevv)(io_info,
                                                                 dset_info,
                                                                 file_nseq,
                                                                 &curr_file_seq,
                                                                 &single_file_len,
        ...
        while (nelmts > 0) {
            ...
            /* Perform I/O on memory and file sequences */
            if (io_info->op_type == H5D_IO_OP_READ) {
                if ((tmp_file_len = (*dset_info->layout_ops
                                          .readvv)(io_info, dset_info, file_nseq, &curr_file_seq, file_len, file_off, mem_nseq, &curr_mem_seq, mem_len, mem_off)) <
                    0) {
                    HGOTO_ERROR(H5E_DATASPACE, H5E_READERROR, FAIL, "read error");
                }
            } /* end if */
                ...
                assert(io_info->op_type == H5D_IO_OP_WRITE);
                if ((tmp_file_len = (*dset_info->layout_ops
                                          .writevv)(io_info, dset_info, file_nseq, &curr_file_seq, file_len, file_off, mem_nseq, &curr_mem_seq, mem_len, mem_off)) <
                    0) {
                    HGOTO_ERROR(H5E_DATASPACE, H5E_WRITEERROR, FAIL, "write error");
                }
            } /* end else */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D_select_io_mem(void* dst_buf, H5S_t* dst_space, const void* src_buf, H5S_t* src_space, size_t elmt_size, size_t nelmts)
{
    ...
    if (nelmts == 1) {
        ...
        if ((bytes_copied = H5VM_memcpyvv(dst_buf,
                                          dst_nseq,
                                          &curr_dst_seq,
                                          &single_dst_len,
                                          &single_dst_off,
        ...
        while (nelmts > 0) {
            ...
            if (curr_dst_seq >= dst_nseq) {
                ...
                if (H5S_SELECT_ITER_GET_SEQ_LIST(dst_sel_iter, vec_size, nelmts, &dst_nseq, &dst_nelem, dst_off, dst_len) < 0) {
            ...
            if (curr_src_seq >= src_nseq) {
                ...
                if (H5S_SELECT_ITER_GET_SEQ_LIST(src_sel_iter, vec_size, nelmts, &src_nseq, &src_nelem, src_off, src_len) < 0) {
                    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTGET, FAIL, "sequence length generation failed");
                }
            ...
            /* Perform vectorized memcpy from src_buf to dst_buf */
            if ((bytes_copied = H5VM_memcpyvv(dst_buf, dst_nseq, &curr_dst_seq, dst_len, dst_off, src_buf, src_nseq, &curr_src_seq, src_len, src_off)) < 0) {
                HGOTO_ERROR(H5E_IO, H5E_WRITEERROR, FAIL, "vectorized memcpy failed");
            }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__select_read(const H5D_io_info_t* io_info, const H5D_dset_io_info_t* dset_info)
{
    herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__select_write(const H5D_io_info_t* io_info, const H5D_dset_io_info_t* dset_info)
{
    herr_t ret_value = SUCCEED; /* Return value */
