...
#define H5D_RTREE_SHOULD_INSERT(entry) \
    !(((entry)->unlim_dim_virtual >= 0) || ((entry)->source_dset.virtual_select && H5S_GET_EXTENT_NDIMS((entry)->source_dset.virtual_select) < 1))
...
static herr_t H5D__virtual_open_source_dset(const H5D_t* vdset, H5O_storage_virtual_ent_t* virtual_ent, H5O_storage_virtual_srcdset_t* source_dset);
static herr_t H5D__virtual_reset_source_dset(H5O_storage_virtual_ent_t* virtual_ent, H5O_storage_virtual_srcdset_t* source_dset);
static herr_t H5D__virtual_str_append(const char* src, size_t src_len, char** p, char** buf, size_t* buf_size);
static herr_t H5D__virtual_copy_parsed_name(H5O_storage_virtual_name_seg_t** dst, H5O_storage_virtual_name_seg_t* src);
...
static herr_t H5D__virtual_pre_io(H5D_dset_io_info_t* dset_info,
...
static herr_t H5D__virtual_read_one_src(H5D_dset_io_info_t* dset_info, H5O_storage_virtual_srcdset_t* source_dset);
static herr_t H5D__virtual_write_one_mapping(H5D_dset_io_info_t* dset_info, H5O_storage_virtual_ent_t* mapping);
static herr_t H5D__virtual_write_one_src(H5D_dset_io_info_t* dset_info, H5O_storage_virtual_srcdset_t* source_dset);

/* R-tree helper functions */
static herr_t H5D__virtual_build_tree(H5O_storage_virtual_t* virt, int rank);
static herr_t H5D__mappings_to_leaves(H5O_storage_virtual_ent_t* mappings,
                                      size_t num_mappings,
                                      H5RT_leaf_t** leaves_out,
...
static herr_t H5D__virtual_not_in_tree_add(H5O_storage_virtual_ent_t*** list, size_t* nused, size_t* nalloc, H5O_storage_virtual_ent_t* mapping);
/*********************/
/* Package Variables */
/*********************/
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__virtual_load_layout(H5F_t* f, H5O_layout_t* layout)
{
    ...
    if (!layout->storage.u.virt.list && layout->storage.u.virt.serial_list_hobjid.addr != HADDR_UNDEF) {
        ...
        if (heap_vers > (uint8_t)H5O_LAYOUT_VDS_GH_ENC_VERS_1) {
            HGOTO_ERROR(H5E_OHDR,
                        ...
                        (unsigned)H5O_LAYOUT_VDS_GH_ENC_VERS_1,
                        (unsigned)heap_vers);
        }
...
herr_t H5D__virtual_copy_layout(H5O_layout_t* layout)
{
    ...
    if (virt->list_nused > 0) {
        ...
        for (i = 0; i < virt->list_nused; i++) {
            ...
            if (NULL == (ent->source_dset.virtual_select = H5S_copy(orig_list[i].source_dset.virtual_select, false, true))) {
            ...
            if (orig_list[i].source_dset.file_name) {
                ...
                else if (orig_list[i].parsed_source_file_name && (orig_list[i].source_dset.file_name != orig_list[i].parsed_source_file_name->name_segment)) {
                    assert(ent->parsed_source_file_name);
                    assert(ent->parsed_source_file_name->name_segment);
                    ent->source_dset.file_name = ent->parsed_source_file_name->name_segment;
                } /* end if */
                else if (NULL == (ent->source_dset.file_name = H5MM_strdup(orig_list[i].source_dset.file_name))) {
                    HGOTO_ERROR(H5E_DATASET, H5E_RESOURCE, FAIL, "unable to duplicate source file name");
                }
            } /* end if */
            if (orig_list[i].source_dset.dset_name) {
                ...
                else if (orig_list[i].parsed_source_dset_name && (orig_list[i].source_dset.dset_name != orig_list[i].parsed_source_dset_name->name_segment)) {
                    assert(ent->parsed_source_dset_name);
                    assert(ent->parsed_source_dset_name->name_segment);
                    ent->source_dset.dset_name = ent->parsed_source_dset_name->name_segment;
                } /* end if */
                else if (NULL == (ent->source_dset.dset_name = H5MM_strdup(orig_list[i].source_dset.dset_name))) {
                    HGOTO_ERROR(H5E_DATASET, H5E_RESOURCE, FAIL, "unable to duplicate source dataset name");
                }
            } /* end if */
    ...
    if (virt->tree) {
        ...
        if (virt->not_in_tree_nused > 0) {
            ...
            if ((virt->not_in_tree_list = (H5O_storage_virtual_ent_t**)H5MM_calloc(virt->not_in_tree_nused * sizeof(H5O_storage_virtual_ent_t*))) == NULL) {
                HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL, "unable to allocate not_in_tree_list pointer array");
            }
...
static herr_t H5D__virtual_open_source_dset(const H5D_t* vdset, H5O_storage_virtual_ent_t* virtual_ent, H5O_storage_virtual_srcdset_t* source_dset)
{
    ...
    if (strcmp(source_dset->file_name, ".") != 0) {
        ...
        if (H5F_prefix_open_file(true,
                                 &src_file,
                                 vdset->oloc.file,
                                 H5F_PREFIX_VDS,
                                 vdset->shared->vds_prefix,
...
static herr_t H5D__virtual_reset_source_dset(H5O_storage_virtual_ent_t* virtual_ent, H5O_storage_virtual_srcdset_t* source_dset)
{
    ...
    if (virtual_ent->parsed_source_file_name && (source_dset->file_name != virtual_ent->parsed_source_file_name->name_segment)) {
        ...
        assert((source_dset->file_name == virtual_ent->source_file_name) ||
               (virtual_ent->parsed_source_file_name && (source_dset->file_name == virtual_ent->parsed_source_file_name->name_segment)) || !source_dset->file_name);
    }

    /* Free dataset name */
    if (virtual_ent->parsed_source_dset_name && (source_dset->dset_name != virtual_ent->parsed_source_dset_name->name_segment)) {
        source_dset->dset_name = (char*)H5MM_xfree(source_dset->dset_name);
    }
    else {
        assert((source_dset->dset_name == virtual_ent->source_dset_name) ||
               (virtual_ent->parsed_source_dset_name && (source_dset->dset_name == virtual_ent->parsed_source_dset_name->name_segment)) || !source_dset->dset_name);
    }
            ...
            if (storage->list[i].unlim_dim_source >= 0) {
                ...
                if (storage->list[i].source_dset.dset) {
                    ...
                    if (curr_dims[storage->list[i].unlim_dim_source] == storage->list[i].unlim_extent_source) {
                         ...
                         * extent */
                        clip_size = H5S_hyper_get_clip_extent_match(storage->list[i].source_dset.virtual_select,
                        ...
                        if (storage->view == H5D_VDS_LAST_AVAILABLE) {
                            if (clip_size != storage->list[i].clip_size_virtual) {
                                ...
                                if (storage->list[i].source_dset.clipped_virtual_select) {
                                    assert(storage->list[i].source_dset.clipped_virtual_select != storage->list[i].source_dset.virtual_select);
                                ...
                                /* Copy virtual selection */
                                if (NULL ==
                                    (storage->list[i].source_dset.clipped_virtual_select = H5S_copy(storage->list[i].source_dset.virtual_select, false, true))) {
                                    HGOTO_ERROR(H5E_DATASET, H5E_CANTCOPY, FAIL, "unable to copy virtual selection");
                                }
                            ...
                            if (storage->list[i].source_dset.clipped_source_select) {
                                assert(storage->list[i].source_dset.clipped_source_select != storage->list[i].source_select);
                ...
                for (j = 0; j <= (storage->printf_gap + first_missing); j++) {
                    ...
                    if (j >= (hsize_t)storage->list[i].sub_dset_nalloc) {
                        if (storage->list[i].sub_dset_nalloc == 0) {
                            /* Allocate sub_dset */
                            if (NULL == (storage->list[i].sub_dset =
                                             (H5O_storage_virtual_srcdset_t*)H5MM_calloc(H5D_VIRTUAL_DEF_SUB_DSET_SIZE * sizeof(H5O_storage_virtual_srcdset_t)))) {
                            ...
                            /* Extend sub_dset */
                            if (NULL == (tmp_sub_dset = (H5O_storage_virtual_srcdset_t*)H5MM_realloc(storage->list[i].sub_dset,
                                                                                                     2 * storage->list[i].sub_dset_nalloc *
                            ...
                            (void)memset(&storage->list[i].sub_dset[storage->list[i].sub_dset_nalloc],
                                         0,
                                         storage->list[i].sub_dset_nalloc * sizeof(H5O_storage_virtual_srcdset_t));
                ...
                if ((first_missing == (hsize_t)storage->list[i].sub_dset_nused) && (storage->list[i].clip_size_virtual != HSIZE_UNDEF)) {
        ...
        for (i = 0; i < storage->list_nused; i++) {
            ...
            if ((storage->list[i].unlim_dim_virtual >= 0) && (storage->view == H5D_VDS_FIRST_MISSING) &&
                ...
                if (storage->list[i].unlim_dim_source >= 0) {
                     ...
                     * selection */
                    first_inc_block =
                        H5S_hyper_get_first_inc_block(storage->list[i].source_dset.virtual_select, new_dims[storage->list[i].unlim_dim_virtual], &partial_block);
            ...
            if ((storage->list[i].source_dset.clipped_virtual_select != storage->list[i].source_dset.virtual_select) &&
        ...
        if (storage->list[i].unlim_dim_virtual >= 0) {
            ...
            if (storage->list[i].unlim_dim_source >= 0) {
                ...
                if (storage->list[i].source_dset.dset) {
                     ...
                     * match size of virtual selection */
                    clip_size = H5S_hyper_get_clip_extent_match(storage->list[i].source_select,
                ...
                sub_dset_max = (size_t)H5S_hyper_get_first_inc_block(storage->list[i].source_dset.virtual_select,
                                                                     virtual_dims[storage->list[i].unlim_dim_virtual],
                                                                     &partial_block);
                if (partial_block) {
                    sub_dset_max++;
                ...
                if (!storage->list[i].sub_dset) {
                    ...
                    (void)memset(&storage->list[i].sub_dset[storage->list[i].sub_dset_nalloc],
                                 0,
                                 (sub_dset_max - storage->list[i].sub_dset_nalloc) * sizeof(H5O_storage_virtual_srcdset_t));
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__virtual_init(H5F_t* f, H5D_t* dset, hid_t dapl_id, bool H5_ATTR_UNUSED open_op)
{
 ...
 *-------------------------------------------------------------------------
 */
static bool H5D__virtual_is_space_alloc(const H5O_storage_t H5_ATTR_UNUSED* storage)
{
    ...
    for (i = 0; i < storage->list_nused; i++) {
        ...
        if (storage->list[i].psfn_nsubs || storage->list[i].psdn_nsubs) {
            ...
            for (j = storage->list[i].sub_dset_io_start; j < storage->list[i].sub_dset_io_end; j++) {
                ...
                if (storage->list[i].sub_dset[j].dset && storage->list[i].sub_dset[j].dset->shared->layout.ops->is_data_cached &&
                    storage->list[i].sub_dset[j].dset->shared->layout.ops->is_data_cached(storage->list[i].sub_dset[j].dset->shared)) {
                    HGOTO_DONE(true);
                }
            }
        } /* end if */
        else if (storage->list[i].source_dset.dset && storage->list[i].source_dset.dset->shared->layout.ops->is_data_cached &&
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__virtual_io_init(H5D_io_info_t* io_info, H5D_dset_io_info_t H5_ATTR_UNUSED* dinfo)
{
    FUNC_ENTER_PACKAGE_NOERR
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__virtual_pre_io_process_mapping(H5D_dset_io_info_t* dset_info,
                                                  H5S_t* file_space,
                                                  H5S_t* mem_space,
                                                  hsize_t* tot_nelmts,
                                                  H5O_storage_virtual_ent_t* curr_mapping)
{
    ...
    if (curr_mapping->psfn_nsubs || curr_mapping->psdn_nsubs) {
        ...
        curr_mapping->sub_dset_io_start =
            (size_t)H5S_hyper_get_first_inc_block(curr_mapping->source_dset.virtual_select, bounds_start[curr_mapping->unlim_dim_virtual], NULL);

        /* Get index of first block outside of virtual selection */
        curr_mapping->sub_dset_io_end =
            (size_t)H5S_hyper_get_first_inc_block(curr_mapping->source_dset.virtual_select, bounds_end[curr_mapping->unlim_dim_virtual], &partial_block);
        if (partial_block) {
            curr_mapping->sub_dset_io_end++;
        }
        ...
        for (j = curr_mapping->sub_dset_io_start; j < curr_mapping->sub_dset_io_end; j++) {
            ...
            if (!curr_mapping->sub_dset[j].clipped_virtual_select) {
                ...
                if (curr_mapping->source_space_status == H5O_VIRTUAL_STATUS_CORRECT) {
                    ...
                    if (H5S_SELECT_BOUNDS(curr_mapping->sub_dset[j].virtual_select, tmp_dims, vbounds_end) < 0) {
                        HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL, "unable to get selection bounds");
                    }
                    ...
                    /* Copy virtual selection */
                    if (NULL == (curr_mapping->sub_dset[j].clipped_virtual_select = H5S_copy(curr_mapping->sub_dset[j].virtual_select, false, true))) {
                        HGOTO_ERROR(H5E_DATASET, H5E_CANTCOPY, FAIL, "unable to copy virtual selection");
                    }

                    /* Clip virtual selection to real virtual extent */
                    (void)memset(start, 0, sizeof(start));
                    if (H5S_select_hyperslab(curr_mapping->sub_dset[j].clipped_virtual_select, H5S_SELECT_AND, start, NULL, tmp_dims, NULL) < 0) {
                    ...
                    if (H5S_select_project_intersection(curr_mapping->sub_dset[j].virtual_select,
                                                        curr_mapping->source_select,
                                                        curr_mapping->sub_dset[j].clipped_virtual_select,
                                                        &curr_mapping->sub_dset[j].clipped_source_select,
                                                        true) < 0) {
            ...
            if (curr_mapping->sub_dset[j].clipped_virtual_select) {
                 ...
                 * onto memory space */
                if (H5S_select_project_intersection(file_space,
                                                    mem_space,
                                                    curr_mapping->sub_dset[j].clipped_virtual_select,
                                                    &curr_mapping->sub_dset[j].projected_mem_space,
                                                    true) < 0) {
                    HGOTO_ERROR(H5E_DATASET, H5E_CANTCLIP, FAIL, "can't project virtual intersection onto memory space");
                }

                /* Check number of elements selected */
                if ((select_nelmts = (hssize_t)H5S_GET_SELECT_NPOINTS(curr_mapping->sub_dset[j].projected_mem_space)) < 0) {
        ...
        if (curr_mapping->source_dset.clipped_virtual_select) {
            ...
            if (H5S_select_project_intersection(file_space,
                                                ...
                                                &curr_mapping->source_dset.projected_mem_space,
                                                true) < 0) {
                HGOTO_ERROR(H5E_DATASET, H5E_CANTCLIP, FAIL, "can't project virtual intersection onto memory space");
            }

            /* Check number of elements selected, add to tot_nelmts */
            if ((select_nelmts = (hssize_t)H5S_GET_SELECT_NPOINTS(curr_mapping->source_dset.projected_mem_space)) < 0) {
                HGOTO_ERROR(H5E_DATASET, H5E_CANTCOUNT, FAIL, "unable to get number of elements in selection");
            }
    ...
    *tot_nelmts = 0;
    ...
    if (mappings) {
        ...
        for (size_t i = 0; i < mappings->count; i++) {
            ...
            if (H5D__virtual_pre_io_process_mapping(dset_info, file_space, mem_space, tot_nelmts, (H5O_storage_virtual_ent_t*)curr_leaf->record) < 0) {
        ...
        for (size_t i = 0; i < storage->not_in_tree_nused; i++) {
            if (H5D__virtual_pre_io_process_mapping(dset_info, file_space, mem_space, tot_nelmts, storage->not_in_tree_list[i]) < 0) {
                HGOTO_ERROR(H5E_DATASET, H5E_CANTCLIP, FAIL, "can't process mapping for pre I/O");
            }
        }
    }
        ...
        for (size_t i = 0; i < storage->list_nused; i++) {
            if (H5D__virtual_pre_io_process_mapping(dset_info, file_space, mem_space, tot_nelmts, &storage->list[i]) < 0) {
                HGOTO_ERROR(H5E_DATASET, H5E_CANTCLIP, FAIL, "can't process mapping for pre I/O");
            }
        }
    }
...
static herr_t H5D__virtual_read_one_src(H5D_dset_io_info_t* dset_info, H5O_storage_virtual_srcdset_t* source_dset)
{
    ...
    if (source_dset->projected_mem_space) {
        ...
        if (H5S_select_project_intersection(source_dset->clipped_virtual_select,
                                            source_dset->clipped_source_select,
                                            dset_info->file_space,
                                            &projected_src_space,
                                            true) < 0) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__virtual_read(H5D_io_info_t H5_ATTR_NDEBUG_UNUSED* io_info, H5D_dset_io_info_t* dset_info)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__virtual_write(H5D_io_info_t H5_ATTR_NDEBUG_UNUSED* io_info, H5D_dset_io_info_t* dset_info)
{
    H5O_storage_virtual_t* storage;     /* Convenient pointer into layout struct */
    hsize_t tot_nelmts;                 /* Total number of elements mapped to mem_space */
    size_t nelmts;                      /* Number of elements to process */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__virtual_refresh_source_dset(H5D_t** dset)
{
                                      ...
                                      size_t* not_in_tree_nalloc)
{
    ...
    if (NULL == (not_in_tree = (H5O_storage_virtual_ent_t**)H5MM_malloc(not_in_tree_capacity * sizeof(H5O_storage_virtual_ent_t*)))) {
    ...
    for (size_t i = 0; i < num_mappings; i++) {
        ...
        if (!(H5D_RTREE_SHOULD_INSERT(curr_mapping))) {
            ...
            if (H5D__virtual_not_in_tree_add(&not_in_tree, &curr_not_tree_count, &not_in_tree_capacity, curr_mapping) < 0) {
...
static herr_t H5D__virtual_build_tree(H5O_storage_virtual_t* virt, int rank)
{
    ...
    if (H5D__mappings_to_leaves(mappings, num_mappings, &leaves, &not_in_tree_mappings, &num_leaves, &not_in_tree_count, &not_in_tree_nalloc) < 0) {
...
static herr_t H5D__virtual_not_in_tree_grow(H5O_storage_virtual_ent_t*** list, size_t* nalloc)
{
    ...
    if (NULL == (new_list = (H5O_storage_virtual_ent_t**)H5MM_realloc(*list, new_capacity * sizeof(H5O_storage_virtual_ent_t*)))) {
