 ...
 *-------------------------------------------------------------------------
 */
H5EA_hdr_t* H5EA__hdr_alloc(H5F_t* f)
{
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5EA__hdr_init(H5EA_hdr_t* hdr, void* ctx_udata)
{
    ...
    if (hdr->cparam.cls->crt_context) {
        if (NULL == (hdr->cb_ctx = (*hdr->cparam.cls->crt_context)(ctx_udata))) {
            HGOTO_ERROR(H5E_EARRAY, H5E_CANTCREATE, FAIL, "unable to create extensible array client callback context");
        }
    }
 ...
 *-------------------------------------------------------------------------
 */
void* H5EA__hdr_alloc_elmts(H5EA_hdr_t* hdr, size_t nelmts)
{
    ...
    if (idx >= hdr->elmt_fac.nalloc) {
        ...
        if (NULL == (new_fac = H5FL_SEQ_REALLOC(H5FL_fac_head_ptr_t, hdr->elmt_fac.fac, new_nalloc))) {
            HGOTO_ERROR(H5E_EARRAY, H5E_CANTALLOC, NULL, "memory allocation failed for data block data element buffer factory array");
        }

        /* Zero out new elements allocated */
        memset(new_fac + hdr->elmt_fac.nalloc, 0, (new_nalloc - hdr->elmt_fac.nalloc) * sizeof(H5FL_fac_head_ptr_t));
    ...
    if (NULL == (elmts = H5FL_FAC_MALLOC(hdr->elmt_fac.fac[idx]))) {
        HGOTO_ERROR(H5E_EARRAY, H5E_CANTALLOC, NULL, "memory allocation failed for data block data element buffer");
    }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5EA__hdr_free_elmts(H5EA_hdr_t* hdr, size_t nelmts, void* elmts)
{
    unsigned idx; /* Index of element buffer factory in header */
 ...
 *-------------------------------------------------------------------------
 */
haddr_t H5EA__hdr_create(H5F_t* f, const H5EA_create_t* cparam, void* ctx_udata)
{
...
#ifndef NDEBUG
    {
        ...
        if (cparam->max_nelmts_bits == 0) {
            HGOTO_ERROR(H5E_EARRAY, H5E_BADVALUE, HADDR_UNDEF, "max. # of elements bits must be greater than zero");
        }
        if (cparam->max_nelmts_bits > H5EA_MAX_NELMTS_IDX_MAX) {
            HGOTO_ERROR(H5E_EARRAY, H5E_BADVALUE, HADDR_UNDEF, "max. # of elements bits must be <= %u", (unsigned)H5EA_MAX_NELMTS_IDX_MAX);
        }
        if (cparam->sup_blk_min_data_ptrs < 2) {
            HGOTO_ERROR(H5E_EARRAY, H5E_BADVALUE, HADDR_UNDEF, "min # of data block pointers in super block must be >= two");
        }
        if (!POWER_OF_TWO(cparam->sup_blk_min_data_ptrs)) {
            HGOTO_ERROR(H5E_EARRAY, H5E_BADVALUE, HADDR_UNDEF, "min # of data block pointers in super block must be power of two");
        }
        if (!POWER_OF_TWO(cparam->data_blk_min_elmts)) {
            HGOTO_ERROR(H5E_EARRAY, H5E_BADVALUE, HADDR_UNDEF, "min # of elements per data block must be power of two");
        }
        dblk_page_nelmts = (size_t)1 << cparam->max_dblk_page_nelmts_bits;
        if (dblk_page_nelmts < cparam->idx_blk_elmts) {
            HGOTO_ERROR(H5E_EARRAY, H5E_BADVALUE, HADDR_UNDEF, "# of elements per data block page must be greater than # of elements in index block");
        }
        ...
        if (cparam->max_dblk_page_nelmts_bits > cparam->max_nelmts_bits) {
            HGOTO_ERROR(H5E_EARRAY, H5E_BADVALUE, HADDR_UNDEF, "max. # of elements per data block page bits must be <= max. # of elements bits");
        }
    }
#endif /* NDEBUG */
    ...
    if (NULL == (hdr = H5EA__hdr_alloc(f))) {
        HGOTO_ERROR(H5E_EARRAY, H5E_CANTALLOC, HADDR_UNDEF, "memory allocation failed for extensible array shared header");
    }
    ...
    if (H5EA__hdr_init(hdr, ctx_udata) < 0) {
        HGOTO_ERROR(H5E_EARRAY, H5E_CANTINIT, HADDR_UNDEF, "initialization failed for extensible array header");
    }

    /* Allocate space for the header on disk */
    if (HADDR_UNDEF == (hdr->addr = H5MF_alloc(f, H5FD_MEM_EARRAY_HDR, (hsize_t)hdr->size))) {
        HGOTO_ERROR(H5E_EARRAY, H5E_CANTALLOC, HADDR_UNDEF, "file allocation failed for extensible array header");
    }
    ...
    if (hdr->top_proxy) {
        if (H5AC_proxy_entry_add_child(hdr->top_proxy, f, hdr) < 0) {
            HGOTO_ERROR(H5E_EARRAY, H5E_CANTSET, HADDR_UNDEF, "unable to add extensible array entry as child of array proxy");
        }
    }
        ...
        if (hdr) {
            ...
            if (inserted) {
                if (H5AC_remove_entry(hdr) < 0) {
                    HDONE_ERROR(H5E_EARRAY, H5E_CANTREMOVE, HADDR_UNDEF, "unable to remove extensible array header from cache");
                }
            }

            /* Release header's disk space */
            if (H5_addr_defined(hdr->addr) && H5MF_xfree(f, H5FD_MEM_EARRAY_HDR, hdr->addr, (hsize_t)hdr->size) < 0) {
                HDONE_ERROR(H5E_EARRAY, H5E_CANTFREE, HADDR_UNDEF, "unable to free extensible array header");
            }

            /* Destroy header */
            if (H5EA__hdr_dest(hdr) < 0) {
                HDONE_ERROR(H5E_EARRAY, H5E_CANTFREE, HADDR_UNDEF, "unable to destroy extensible array header");
            }
        } /* end if */
    }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5EA__hdr_incr(H5EA_hdr_t* hdr)
{
    herr_t ret_value = SUCCEED;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5EA__hdr_decr(H5EA_hdr_t* hdr)
{
    herr_t ret_value = SUCCEED;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5EA__hdr_fuse_incr(H5EA_hdr_t* hdr)
{
    FUNC_ENTER_PACKAGE_NOERR
 ...
 *-------------------------------------------------------------------------
 */
size_t H5EA__hdr_fuse_decr(H5EA_hdr_t* hdr)
{
    size_t ret_value = 0;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5EA__hdr_modified(H5EA_hdr_t* hdr)
{
    herr_t ret_value = SUCCEED;
 ...
 *-------------------------------------------------------------------------
 */
H5EA_hdr_t* H5EA__hdr_protect(H5F_t* f, haddr_t ea_addr, void* ctx_udata, unsigned flags)
{
    ...
    if (NULL == (hdr = (H5EA_hdr_t*)H5AC_protect(f, H5AC_EARRAY_HDR, ea_addr, &udata, flags))) {
        HGOTO_ERROR(H5E_EARRAY, H5E_CANTPROTECT, NULL, "unable to protect extensible array header, address = %llu", (unsigned long long)ea_addr);
    }
    ...
    if (hdr->swmr_write && NULL == hdr->top_proxy) {
        ...
        /* Add header as child of 'top' proxy */
        if (H5AC_proxy_entry_add_child(hdr->top_proxy, f, hdr) < 0) {
            HGOTO_ERROR(H5E_EARRAY, H5E_CANTSET, NULL, "unable to add extensible array entry as child of array proxy");
        }
    } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5EA__hdr_unprotect(H5EA_hdr_t* hdr, unsigned cache_flags)
{
    herr_t ret_value = SUCCEED;
    ...
    if (H5AC_unprotect(hdr->f, H5AC_EARRAY_HDR, hdr->addr, hdr, cache_flags) < 0) {
        HGOTO_ERROR(H5E_EARRAY, H5E_CANTUNPROTECT, FAIL, "unable to unprotect extensible array hdr, address = %llu", (unsigned long long)hdr->addr);
    }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5EA__hdr_delete(H5EA_hdr_t* hdr)
{
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5EA__hdr_dest(H5EA_hdr_t* hdr)
{
    herr_t ret_value = SUCCEED;
    ...
    if (hdr->cb_ctx) {
        if ((*hdr->cparam.cls->dst_context)(hdr->cb_ctx) < 0) {
            HGOTO_ERROR(H5E_EARRAY, H5E_CANTRELEASE, FAIL, "unable to destroy extensible array client callback context");
        }
    } /* end if */
    hdr->cb_ctx = NULL;
        ...
        for (u = 0; u < hdr->elmt_fac.nalloc; u++) {
            ...
            if (hdr->elmt_fac.fac[u]) {
                if (H5FL_fac_term(hdr->elmt_fac.fac[u]) < 0) {
                    HGOTO_ERROR(H5E_EARRAY, H5E_CANTRELEASE, FAIL, "unable to destroy extensible array header factory");
                }
