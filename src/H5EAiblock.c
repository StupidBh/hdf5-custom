 ...
 *-------------------------------------------------------------------------
 */
H5EA_iblock_t* H5EA__iblock_alloc(H5EA_hdr_t* hdr)
{
    ...
    if (hdr->cparam.idx_blk_elmts > 0) {
        if (NULL == (iblock->elmts = H5FL_BLK_MALLOC(idx_blk_elmt_buf, (size_t)(hdr->cparam.idx_blk_elmts * hdr->cparam.cls->nat_elmt_size)))) {
            HGOTO_ERROR(H5E_EARRAY, H5E_CANTALLOC, NULL, "memory allocation failed for index block data element buffer");
        }
    }
    ...
    if (iblock->ndblk_addrs > 0) {
        if (NULL == (iblock->dblk_addrs = H5FL_SEQ_MALLOC(haddr_t, iblock->ndblk_addrs))) {
            HGOTO_ERROR(H5E_EARRAY, H5E_CANTALLOC, NULL, "memory allocation failed for index block data block addresses");
        }
    }
    ...
    if (iblock->nsblk_addrs > 0) {
        if (NULL == (iblock->sblk_addrs = H5FL_SEQ_MALLOC(haddr_t, iblock->nsblk_addrs))) {
            HGOTO_ERROR(H5E_EARRAY, H5E_CANTALLOC, NULL, "memory allocation failed for index block super block addresses");
        }
    }
 ...
 *-------------------------------------------------------------------------
 */
haddr_t H5EA__iblock_create(H5EA_hdr_t* hdr, bool* stats_changed)
{
    H5EA_iblock_t* iblock = NULL; /* Extensible array index block */
    ...
    if (NULL == (iblock = H5EA__iblock_alloc(hdr))) {
        HGOTO_ERROR(H5E_EARRAY, H5E_CANTALLOC, HADDR_UNDEF, "memory allocation failed for extensible array index block");
    }
    ...
    if (HADDR_UNDEF == (iblock_addr = H5MF_alloc(hdr->f, H5FD_MEM_EARRAY_IBLOCK, (hsize_t)iblock->size))) {
        HGOTO_ERROR(H5E_EARRAY, H5E_CANTALLOC, HADDR_UNDEF, "file allocation failed for extensible array index block");
    }
    ...
    if (hdr->cparam.idx_blk_elmts > 0) {
        ...
        if ((hdr->cparam.cls->fill)(iblock->elmts, (size_t)hdr->cparam.idx_blk_elmts) < 0) {
            HGOTO_ERROR(H5E_EARRAY, H5E_CANTSET, HADDR_UNDEF, "can't set extensible array index block elements to class's fill value");
        }
    } /* end if */
    ...
    if (H5AC_insert_entry(hdr->f, H5AC_EARRAY_IBLOCK, iblock_addr, iblock, H5AC__NO_FLAGS_SET) < 0) {
        HGOTO_ERROR(H5E_EARRAY, H5E_CANTINSERT, HADDR_UNDEF, "can't add extensible array index block to cache");
    }
    inserted = true;
        ...
        if (iblock) {
            ...
            if (inserted) {
                if (H5AC_remove_entry(iblock) < 0) {
                    HDONE_ERROR(H5E_EARRAY, H5E_CANTREMOVE, HADDR_UNDEF, "unable to remove extensible array index block from cache");
                }
            }

            /* Release index block's disk space */
            if (H5_addr_defined(iblock->addr) && H5MF_xfree(hdr->f, H5FD_MEM_EARRAY_IBLOCK, iblock->addr, (hsize_t)iblock->size) < 0) {
                HDONE_ERROR(H5E_EARRAY, H5E_CANTFREE, HADDR_UNDEF, "unable to release file space for extensible array index block");
            }

            /* Destroy index block */
            if (H5EA__iblock_dest(iblock) < 0) {
                HDONE_ERROR(H5E_EARRAY, H5E_CANTFREE, HADDR_UNDEF, "unable to destroy extensible array index block");
            }
        } /* end if */
    }
 ...
 *-------------------------------------------------------------------------
 */
H5EA_iblock_t* H5EA__iblock_protect(H5EA_hdr_t* hdr, unsigned flags)
{
    ...
    if (NULL == (iblock = (H5EA_iblock_t*)H5AC_protect(hdr->f, H5AC_EARRAY_IBLOCK, hdr->idx_blk_addr, hdr, flags))) {
        HGOTO_ERROR(H5E_EARRAY, H5E_CANTPROTECT, NULL, "unable to protect extensible array index block, address = %llu", (unsigned long long)hdr->idx_blk_addr);
    }
    ...
    if (!ret_value) {
        ...
        if (iblock && H5AC_unprotect(hdr->f, H5AC_EARRAY_IBLOCK, iblock->addr, iblock, H5AC__NO_FLAGS_SET) < 0) {
            HDONE_ERROR(H5E_EARRAY, H5E_CANTUNPROTECT, NULL, "unable to unprotect extensible array index block, address = %llu", (unsigned long long)iblock->addr);
        }
    } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5EA__iblock_unprotect(void* _iblock, unsigned cache_flags)
{
    ...
    if (H5AC_unprotect(iblock->hdr->f, H5AC_EARRAY_IBLOCK, iblock->addr, iblock, cache_flags) < 0) {
        HGOTO_ERROR(H5E_EARRAY, H5E_CANTUNPROTECT, FAIL, "unable to unprotect extensible array index block, address = %llu", (unsigned long long)iblock->addr);
    }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5EA__iblock_delete(H5EA_hdr_t* hdr)
{
    H5EA_iblock_t* iblock = NULL; /* Pointer to index block */
    herr_t ret_value = SUCCEED;
        ...
        for (u = 0; u < iblock->nsblk_addrs; u++) {
            ...
            if (H5_addr_defined(iblock->sblk_addrs[u])) {
                ...
                if (H5EA__sblock_delete(hdr, iblock, iblock->sblk_addrs[u], (unsigned)(u + iblock->nsblks)) < 0) {
                    HGOTO_ERROR(H5E_EARRAY, H5E_CANTDELETE, FAIL, "unable to delete extensible array super block");
                }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5EA__iblock_dest(H5EA_iblock_t* iblock)
{
    herr_t ret_value = SUCCEED;
