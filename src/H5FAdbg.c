 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FA__hdr_debug(H5F_t* f, haddr_t addr, FILE* stream, int indent, int fwidth, const H5FA_class_t* cls, haddr_t obj_addr)
{
    /* Local variables */
    ...
    fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth, "Max. # of elements in data block page:", (unsigned)((size_t)1 << hdr->cparam.max_dblk_page_nelmts_bits));

    fprintf(stream, "%*s%-*s %" PRIuHSIZE "\n", indent, "", fwidth, "Number of elements in Fixed Array:", hdr->stats.nelmts);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FA__dblock_debug(H5F_t* f, haddr_t addr, FILE* stream, int indent, int fwidth, const H5FA_class_t* cls, haddr_t hdr_addr, haddr_t obj_addr)
{
    /* Local variables */
    H5FA_hdr_t* hdr = NULL;       /* Shared fixed array header */
    ...
    if (NULL == (dblock = H5FA__dblock_protect(hdr, addr, H5AC__READ_ONLY_FLAG))) {
        HGOTO_ERROR(H5E_FARRAY, H5E_CANTPROTECT, FAIL, "unable to protect fixed array data block, address = %llu", (unsigned long long)addr);
    }
    ...
    fprintf(stream, "%*s%-*s %" PRIuHSIZE "\n", indent, "", fwidth, "Number of elements in Data Block:", hdr->cparam.nelmts);
    fprintf(stream, "%*s%-*s %zu\n", indent, "", fwidth, "Number of pages in Data Block:", dblock->npages);
    fprintf(stream, "%*s%-*s %zu\n", indent, "", fwidth, "Number of elements per Data Block page:", dblock->dblk_page_nelmts);
    ...
    if (dblock->npages) {        /* paging */
        ...
        for (page_idx = 0; page_idx < dblock->npages; page_idx++) {
            if (!H5VM_bit_get(dblock->dblk_page_init, page_idx)) {
                ...
                for (u = 0; u < dblk_page_nelmts; u++) {
                    ...
                    if ((hdr->cparam.cls->debug)(stream,
                                                 (indent + 3),
                                                 MAX(0, (fwidth - 3)),
                                                 (hsize_t)u,
                ...
                if (H5FA__dblk_page_unprotect(dblk_page, H5AC__NO_FLAGS_SET) < 0) {
                    HGOTO_ERROR(H5E_FARRAY, H5E_CANTUNPROTECT, FAIL, "unable to release fixed array data block page");
                }
        ...
        for (u = 0; u < hdr->cparam.nelmts; u++) {
            /* Call the class's 'debug' callback */
            if ((hdr->cparam.cls->debug)(stream, (indent + 3), MAX(0, (fwidth - 3)), (hsize_t)u, ((uint8_t*)dblock->elmts) + (hdr->cparam.cls->nat_elmt_size * u)) <
                0) {
                HGOTO_ERROR(H5E_FARRAY, H5E_CANTGET, FAIL, "can't get element for debugging");
            }
        } /* end for */
