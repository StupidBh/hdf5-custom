 ...
 *-------------------------------------------------------------------------
 */
H5FA_dblock_t* H5FA__dblock_alloc(H5FA_hdr_t* hdr)
{
    ...
    if (hdr->cparam.nelmts > dblock->dblk_page_nelmts) {
        ...
        if (NULL == (dblock->elmts = H5FL_BLK_MALLOC(chunk_elmts, (size_t)dblk_size))) {
            HGOTO_ERROR(H5E_FARRAY, H5E_CANTALLOC, NULL, "memory allocation failed for data block element buffer");
        }
    } /* end else */
 ...
 *-------------------------------------------------------------------------
 */
haddr_t H5FA__dblock_create(H5FA_hdr_t* hdr, bool* hdr_dirty)
{
    H5FA_dblock_t* dblock = NULL; /* Fixed array data block */
    ...
    if (NULL == (dblock = H5FA__dblock_alloc(hdr))) {
        HGOTO_ERROR(H5E_FARRAY, H5E_CANTALLOC, HADDR_UNDEF, "memory allocation failed for fixed array data block");
    }
    ...
    if (HADDR_UNDEF == (dblock_addr = H5MF_alloc(hdr->f, H5FD_MEM_FARRAY_DBLOCK, (hsize_t)dblock->size))) {
        HGOTO_ERROR(H5E_FARRAY, H5E_CANTALLOC, HADDR_UNDEF, "file allocation failed for fixed array data block");
    }
    ...
    if (!dblock->npages) {
        ...
        if ((hdr->cparam.cls->fill)(dblock->elmts, (size_t)hdr->cparam.nelmts) < 0) {
            HGOTO_ERROR(H5E_FARRAY, H5E_CANTSET, HADDR_UNDEF, "can't set fixed array data block elements to class's fill value");
        }
    }
    ...
    if (hdr->top_proxy) {
        if (H5AC_proxy_entry_add_child(hdr->top_proxy, hdr->f, dblock) < 0) {
            HGOTO_ERROR(H5E_FARRAY, H5E_CANTSET, HADDR_UNDEF, "unable to add fixed array entry as child of array proxy");
        }
        ...
        if (dblock) {
            ...
            if (inserted) {
                if (H5AC_remove_entry(dblock) < 0) {
                    HDONE_ERROR(H5E_FARRAY, H5E_CANTREMOVE, HADDR_UNDEF, "unable to remove fixed array data block from cache");
                }
            }

            /* Release data block's disk space */
            if (H5_addr_defined(dblock->addr) && H5MF_xfree(hdr->f, H5FD_MEM_FARRAY_DBLOCK, dblock->addr, (hsize_t)dblock->size) < 0) {
 ...
 *-------------------------------------------------------------------------
 */
H5FA_dblock_t* H5FA__dblock_protect(H5FA_hdr_t* hdr, haddr_t dblk_addr, unsigned flags)
{
    H5FA_dblock_t* dblock = NULL; /* Fixed array data block */
    ...
    if (NULL == (dblock = (H5FA_dblock_t*)H5AC_protect(hdr->f, H5AC_FARRAY_DBLOCK, dblk_addr, &udata, flags))) {
        HGOTO_ERROR(H5E_FARRAY, H5E_CANTPROTECT, NULL, "unable to protect fixed array data block, address = %llu", (unsigned long long)dblk_addr);
    }
    ...
    if (hdr->top_proxy && NULL == dblock->top_proxy) {
        /* Add data block as child of 'top' proxy */
        if (H5AC_proxy_entry_add_child(hdr->top_proxy, hdr->f, dblock) < 0) {
            HGOTO_ERROR(H5E_FARRAY, H5E_CANTSET, NULL, "unable to add fixed array entry as child of array proxy");
        }
        dblock->top_proxy = hdr->top_proxy;
    } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FA__dblock_unprotect(H5FA_dblock_t* dblock, unsigned cache_flags)
{
    ...
    if (H5AC_unprotect(dblock->hdr->f, H5AC_FARRAY_DBLOCK, dblock->addr, dblock, cache_flags) < 0) {
        HGOTO_ERROR(H5E_FARRAY, H5E_CANTUNPROTECT, FAIL, "unable to unprotect fixed array data block, address = %llu", (unsigned long long)dblock->addr);
    }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FA__dblock_delete(H5FA_hdr_t* hdr, haddr_t dblk_addr)
{
    ...
    if (NULL == (dblock = H5FA__dblock_protect(hdr, dblk_addr, H5AC__NO_FLAGS_SET))) {
        HGOTO_ERROR(H5E_FARRAY, H5E_CANTPROTECT, FAIL, "unable to protect fixed array data block, address = %llu", (unsigned long long)dblk_addr);
    }
    ...
    if (dblock->npages) {
        ...
        for (u = 0; u < dblock->npages; u++) {
            ...
            if (H5AC_expunge_entry(hdr->f, H5AC_FARRAY_DBLK_PAGE, dblk_page_addr, H5AC__NO_FLAGS_SET) < 0) {
                HGOTO_ERROR(H5E_FARRAY, H5E_CANTEXPUNGE, FAIL, "unable to remove array data block page from metadata cache");
            }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FA__dblock_dest(H5FA_dblock_t* dblock)
{
    herr_t ret_value = SUCCEED;
