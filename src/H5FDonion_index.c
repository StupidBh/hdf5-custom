 ...
 *-----------------------------------------------------------------------------
 */
herr_t H5FD__onion_ingest_revision_record(H5FD_onion_revision_record_t* r_out, H5FD_t* raw_file, const H5FD_onion_history_t* history, uint64_t revision_num)
{
    ...
    if ((H5FD_get_eoa(raw_file, H5FD_MEM_DRAW) < (addr + size)) && (H5FD_set_eoa(raw_file, H5FD_MEM_DRAW, (addr + size)) < 0)) {
    ...
    if (range == 0) {
        ...
        if (revision_num != r_out->revision_num) {
            HGOTO_ERROR(H5E_ARGS, H5E_BADRANGE, FAIL, "could not find target revision!"); /* TODO: corrupted? */
        }
    ...
    if (r_out->archival_index.n_entries > 0) {
        if (NULL == (r_out->archival_index.list = H5MM_calloc(r_out->archival_index.n_entries * sizeof(H5FD_onion_index_entry_t)))) {
 ...
 *-----------------------------------------------------------------------------
 */
bool H5FD__onion_archival_index_is_valid(const H5FD_onion_archival_index_t* aix)
{
    bool ret_value = true;
 ...
 *-----------------------------------------------------------------------------
 */
int H5FD__onion_archival_index_find(const H5FD_onion_archival_index_t* aix, uint64_t logical_page, const H5FD_onion_index_entry_t** entry_out)
{
    uint64_t low = 0;
    uint64_t high = 0;
    uint64_t n = 0;
    ...
    /* Trivial cases */
    if (aix->n_entries == 0 || logical_page > aix->list[high].logical_page || logical_page < aix->list[0].logical_page) {
        HGOTO_DONE(0);
    }
 ...
 *-----------------------------------------------------------------------------
 */
herr_t H5FD__onion_revision_index_destroy(H5FD_onion_revision_index_t* rix)
{
    herr_t ret_value = SUCCEED;
 ...
 *-----------------------------------------------------------------------------
 */
H5FD_onion_revision_index_t* H5FD__onion_revision_index_init(uint32_t page_size)
{
    ...
    if (NULL == (rix->_hash_table = H5MM_calloc(table_size * sizeof(H5FD_onion_revision_index_hash_chain_node_t*)))) {
    ...
    for (rix->page_size_log2 = 0; (((uint32_t)1 << rix->page_size_log2) & page_size) == 0; rix->page_size_log2++)
        ;
 ...
 *-----------------------------------------------------------------------------
 */
herr_t H5FD__onion_revision_index_insert(H5FD_onion_revision_index_t* rix, const H5FD_onion_index_entry_t* entry)
{
    uint64_t key = 0;
    H5FD_onion_revision_index_hash_chain_node_t* node = NULL;
    ...
    if (rix->n_entries >= (rix->_hash_table_size * 2) || rix->_hash_table_n_keys_populated >= (rix->_hash_table_size / 2)) {
 ...
 *-----------------------------------------------------------------------------
 */
int H5FD__onion_revision_index_find(const H5FD_onion_revision_index_t* rix, uint64_t logical_page, const H5FD_onion_index_entry_t** entry_out)
{
    uint64_t key = 0;
    int ret_value = 0;
 ...
 *-----------------------------------------------------------------------------
 */
size_t H5_ATTR_NO_OPTIMIZE H5FD__onion_revision_record_decode(unsigned char* buf, H5FD_onion_revision_record_t* record)
{
    uint32_t ui32 = 0;
    ...
    for (record->archival_index.page_size_log2 = 0; (((uint32_t)1 << record->archival_index.page_size_log2) & page_size) == 0;
 ...
 *-----------------------------------------------------------------------------
 */
size_t H5FD__onion_revision_record_encode(H5FD_onion_revision_record_t* record, unsigned char* buf, uint32_t* checksum)
{
 ...
 *-----------------------------------------------------------------------------
 */
static int H5FD__onion_archival_index_list_sort_cmp(const void* _a, const void* _b)
{
 ...
 *-----------------------------------------------------------------------------
 */
herr_t H5FD__onion_merge_revision_index_into_archival_index(const H5FD_onion_revision_index_t* rix, H5FD_onion_archival_index_t* aix)
{
    uint64_t n_kept = 0;
    ...
    for (uint64_t i = 0; i < rix->_hash_table_size; i++) {
        ...
        for (node = rix->_hash_table[i]; node != NULL; node = node->next) {
            H5MM_memcpy(&new_aix.list[new_aix.n_entries], &node->entry_data, sizeof(H5FD_onion_index_entry_t));
            new_aix.n_entries++;
        }
    }

    /* Sort the new archival list */
    qsort(new_aix.list, new_aix.n_entries, sizeof(H5FD_onion_index_entry_t), H5FD__onion_archival_index_list_sort_cmp);
    ...
    /* Sort this list */
    qsort(aix->list, aix->n_entries, sizeof(H5FD_onion_index_entry_t), H5FD__onion_archival_index_list_sort_cmp);
