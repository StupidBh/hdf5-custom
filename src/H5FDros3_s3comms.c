...
static void H5FD__s3comms_s3r_req_finish_cb(struct aws_s3_meta_request* meta_request, const struct aws_s3_meta_request_result* meta_request_result, void* user_data);
static bool H5FD__s3comms_s3r_req_finish_pred(void* user_data);

/* Callbacks for operation to read an s3 object */
static int H5FD__s3comms_s3r_read_cb(struct aws_s3_meta_request* meta_request, const struct aws_byte_cursor* body, uint64_t range_start, void* user_data);

/* Callbacks for operation to get the size of an s3 object */
static herr_t H5FD__s3comms_s3r_getsize(s3r_t* handle);
static int
    H5FD__s3comms_s3r_getsize_headers_cb(struct aws_s3_meta_request* meta_request, const struct aws_http_headers* headers, int response_status, void* user_data);
...
static herr_t H5FD__s3comms_get_aws_region(const H5FD__s3comms_aws_params_t* aws_params, const H5FD_ros3_fapl_t* fa, char** aws_region_out);
...
herr_t H5FD__s3comms_init(void)
{
    ...
    H5FD_ros3_aws_event_loop_group_g = aws_event_loop_group_new(H5FD_ros3_aws_allocator_g, &event_loop_group_opts);
    ...
    H5FD_ros3_aws_host_resolver_g = aws_host_resolver_new_default(H5FD_ros3_aws_allocator_g, &host_resolver_opts);
    ...
    if (0 != atexit(H5FD__s3comms_term_func)) {
        HGOTO_ERROR(H5E_VFL, H5E_CANTINIT, FAIL, "couldn't register function for cleaning up aws-c-s3 library");
    }
 ...
 *----------------------------------------------------------------------------
 */
static void H5FD__s3comms_s3r_req_finish_cb(struct aws_s3_meta_request H5_ATTR_UNUSED* meta_request,
                                            ...
                                            void* user_data)
{
    ...
    if (AWS_ERROR_SUCCESS != meta_request_result->error_code) {
        ...
        s3comms_request->err_msg = H5FD__s3comms_httpcode_to_str(meta_request_result->response_status, &handled_http_status);
                ...
                if (meta_request_result->error_response_headers) {
                    ...
                    if (num_headers) {
                        ...
                        for (size_t hdr_idx = 0; hdr_idx < num_headers; hdr_idx++) {
                            ...
                            ret = aws_http_headers_get_index(meta_request_result->error_response_headers, hdr_idx, &header);
 ...
 *----------------------------------------------------------------------------
 */
static void H5FD__s3comms_cred_provider_get_creds_cb(struct aws_credentials* credentials, int error_code, void* user_data)
{
...
static bool H5FD__s3comms_cred_provider_pred(void* user_data)
{
    H5FD__s3comms_request_t* request = (H5FD__s3comms_request_t*)user_data;
 ...
 *----------------------------------------------------------------------------
 */
s3r_t* H5FD__s3comms_s3r_open(const char* url, const H5FD_ros3_fapl_t* fa, const char* fapl_token, const char* alt_endpoint)
{
    ...
    if (AWS_OP_SUCCESS != aws_mutex_init(&aws_params->req_mutex)) {
        HGOTO_ERROR(H5E_VFL, H5E_CANTINIT, NULL, "couldn't initialize mutex: %s", aws_error_str(aws_last_error()));
    }
    mutex_init = true;

    if (AWS_OP_SUCCESS != aws_condition_variable_init(&aws_params->req_cvar)) {
        HGOTO_ERROR(H5E_VFL, H5E_CANTINIT, NULL, "couldn't initialize condition variable: %s", aws_error_str(aws_last_error()));
    }
    cond_var_init = true;
    ...
    if (pathstyle_env && (*pathstyle_env != '\0')) {
        if (0 != HDstrcasecmp(pathstyle_env, "false") && 0 != HDstrcasecmp(pathstyle_env, "off") && 0 != strcmp(pathstyle_env, "0")) {
    ...
    if (!client_bootstrap) {
        HGOTO_ERROR(H5E_VFL, H5E_CANTINIT, NULL, "couldn't bootstrap AWS client: %s", aws_error_str(aws_last_error()));
    }
    ...
    if (H5FD__s3comms_get_credentials_provider(aws_params, fa, fapl_token, &credentials_provider) < 0) {
        HGOTO_ERROR(H5E_VFL, H5E_CANTINIT, NULL, "couldn't create AWS credentials provider for authentication");
    }
    ...
    if (!client) {
        HGOTO_ERROR(H5E_VFL, H5E_CANTINIT, NULL, "couldn't initialize AWS client: %s", aws_error_str(aws_last_error()));
    }
    ...
    if (alt_endpoint && (*alt_endpoint != '\0')) {
        ...
        if (NULL == (handle->alternate_purl = H5FD__s3comms_parse_url(handle, alt_endpoint, &aws_params->alt_parsed_uri))) {
            HGOTO_ERROR(H5E_VFL, H5E_CANTALLOC, NULL, "could not allocate and create parsed alternate endpoint URL");
        }
        if (!handle->alternate_purl->host) {
            HGOTO_ERROR(H5E_VFL, H5E_CANTALLOC, NULL, "invalid alternate endpoint URL specified - could not parse host name");
        }
    }
...
herr_t H5FD__s3comms_s3r_read(s3r_t* handle, haddr_t offset, size_t len, void* dest, size_t dest_size)
{
    ...
    if (H5FD_ros3_debug_g) {
        if (len > 0) {
            ...
            fprintf(stderr, " -- GET: Bytes %" PRIuHADDR " - %" PRIuHADDR ", Request Size: %zu\n", offset, handle->filesize - 1, handle->filesize - offset);
        }
    ...
    if (H5FD__s3comms_format_http_request_message(aws_params, "GET", handle->purl->bucket_name, handle->purl->key, &request_http_message) < 0) {
        HGOTO_ERROR(H5E_VFL, H5E_CANTSET, FAIL, "couldn't create HTTP request message");
    }

    /* Add "Host" header to HTTP message */
    if (H5FD__s3comms_format_host_header(aws_params,
                                         request_http_message,
                                         handle->purl->bucket_name,
    ...
    if (!request) {
        HGOTO_ERROR(H5E_VFL, H5E_READERROR, FAIL, "couldn't create AWS meta-request: %s", aws_error_str(aws_last_error()));
    }
    ...
    if (AWS_OP_SUCCESS !=
        aws_condition_variable_wait_pred(read_params.request.cvar, read_params.request.mutex, H5FD__s3comms_s3r_req_finish_pred, &read_params.request)) {
        aws_mutex_unlock(read_params.request.mutex);
        HGOTO_ERROR(H5E_VFL, H5E_READERROR, FAIL, "couldn't wait on condition variable: %s", aws_error_str(aws_last_error()));
    }

    if (AWS_OP_SUCCESS != aws_mutex_unlock(read_params.request.mutex)) {
        HGOTO_ERROR(H5E_VFL, H5E_READERROR, FAIL, "couldn't unlock mutex: %s", aws_error_str(aws_last_error()));
    }
    ...
    if (read_params.request.status != SUCCEED) {
        ...
        HGOTO_ERROR(H5E_VFL, H5E_READERROR, FAIL, "error occurred while reading s3 object: %s", read_params.request.err_msg);
    }
 ...
 */
static int
    H5FD__s3comms_s3r_read_cb(struct aws_s3_meta_request H5_ATTR_UNUSED* meta_request, const struct aws_byte_cursor* body, uint64_t range_start, void* user_data)
{
    ...
    if (range_start < read_params->read_offset) {
        read_params->request.err_msg = "internal error - data chunk offset was smaller than initial buffer offset";
        ret_value = aws_raise_error(AWS_ERROR_S3_CANCELED);
        goto done;
    }
...
static herr_t H5FD__s3comms_s3r_getsize(s3r_t* handle)
{
    ...
    if (H5FD__s3comms_format_http_request_message(aws_params, "HEAD", handle->purl->bucket_name, handle->purl->key, &request_http_message) < 0) {
        HGOTO_ERROR(H5E_VFL, H5E_CANTSET, FAIL, "couldn't create HTTP request message");
    }
    ...
    if (AWS_OP_SUCCESS !=
        aws_condition_variable_wait_pred(getsize_params.request.cvar, getsize_params.request.mutex, H5FD__s3comms_s3r_req_finish_pred, &getsize_params.request)) {
    ...
    if (getsize_params.request.status != SUCCEED) {
        ...
        HGOTO_ERROR(H5E_VFL, H5E_READERROR, FAIL, "error occurred while getting s3 object size: %s", getsize_params.request.err_msg);
    }
 ...
 *----------------------------------------------------------------------------
 */
static int H5FD__s3comms_s3r_getsize_headers_cb(struct aws_s3_meta_request H5_ATTR_UNUSED* meta_request,
    ...
    if (is_s3_url) {
        ...
        if (NULL != strstr(host_copy, ".")) {
            ...
            if (strcmp(host_component, "s3") == 0 || strncmp(host_component, "s3-", 3) == 0) {
                ...
                } while (host_component && strcmp(host_component, "s3") != 0 && strncmp(host_component, "s3-", 3) != 0);
        ...
        if (is_virtual_style) {
            ...
            if (bucket_name_len < 0) {
                HGOTO_ERROR(H5E_VFL, H5E_INTERNAL, NULL, "internal error occurred when parsing bucket name from url");
            }
...
static herr_t H5FD__s3comms_get_aws_region(const H5FD__s3comms_aws_params_t* aws_params, const H5FD_ros3_fapl_t* fa, char** aws_region_out)
{
    ...
    if (!region_copy) {
        ...
        if (env_region && (*env_region != '\0')) {
            if (NULL == (region_copy = strdup(env_region))) {
                HGOTO_ERROR(H5E_VFL, H5E_CANTALLOC, FAIL, "couldn't copy AWS region string from environment variable");
            }
        }
    }
    ...
    if (!region_copy) {
        ...
        if (env_default_region && (*env_default_region != '\0')) {
            if (NULL == (region_copy = strdup(env_default_region))) {
                HGOTO_ERROR(H5E_VFL, H5E_CANTALLOC, FAIL, "couldn't copy default AWS region string from environment variable");
            }
        }
    }
    ...
    if (!region_copy) {
        ...
        config_coll = aws_profile_collection_new_from_file(aws_params->allocator, config_file_path, AWS_PST_CONFIG);
        if (!config_coll) {
        ...
        if (NULL == (profile_name_str = aws_get_profile_name(aws_params->allocator, NULL))) {
            HGOTO_ERROR(H5E_VFL, H5E_CANTGET, FAIL, "couldn't determine AWS profile to retrieve region information from");
        }
        ...
        if (prop_val) {
            if (NULL == (region_copy = strdup(aws_string_c_str(prop_val)))) {
                HGOTO_ERROR(H5E_VFL, H5E_CANTALLOC, FAIL, "couldn't copy AWS region string from AWS configuration file");
            }
        }
    }
                                                     ...
                                                     struct aws_credentials_provider** credentials_provider_out)
{
    ...
    if (fa && fa->authenticate) {
        ...
        credentials_provider = aws_credentials_provider_new_static(aws_params->allocator, &credentials_provider_opts);
        if (!credentials_provider) {
            HGOTO_ERROR(H5E_VFL, H5E_CANTINIT, FAIL, "couldn't create credentials provider for AWS client: %s", aws_error_str(aws_last_error()));
        }
    }
    ...
    if (!credentials_provider) {
        ...
        credentials_provider = aws_credentials_provider_new_chain_default(aws_params->allocator, &credentials_provider_opts);
        if (!credentials_provider) {
            HGOTO_ERROR(H5E_VFL, H5E_CANTINIT, FAIL, "couldn't create default credentials provider chain for AWS client: %s", aws_error_str(aws_last_error()));
        }
        ...
        if (AWS_OP_SUCCESS !=
            aws_credentials_provider_get_credentials(credentials_provider, H5FD__s3comms_cred_provider_get_creds_cb, &check_cred_provider_params)) {
            HGOTO_ERROR(H5E_VFL,
                        H5E_CANTINIT,
                        FAIL,
        ...
        if (AWS_OP_SUCCESS != aws_mutex_lock(&aws_params->req_mutex)) {
            HGOTO_ERROR(H5E_VFL, H5E_CANTINIT, FAIL, "couldn't lock mutex: %s", aws_error_str(aws_last_error()));
        }

        if (AWS_OP_SUCCESS !=
            aws_condition_variable_wait_pred(&aws_params->req_cvar, &aws_params->req_mutex, H5FD__s3comms_cred_provider_pred, &check_cred_provider_params.request)) {
            aws_mutex_unlock(&aws_params->req_mutex);
            HGOTO_ERROR(H5E_VFL, H5E_CANTINIT, FAIL, "couldn't wait on condition variable: %s", aws_error_str(aws_last_error()));
        }

        if (AWS_OP_SUCCESS != aws_mutex_unlock(&aws_params->req_mutex)) {
            HGOTO_ERROR(H5E_VFL, H5E_CANTINIT, FAIL, "couldn't unlock mutex: %s", aws_error_str(aws_last_error()));
        }
        ...
        if (!check_cred_provider_params.credentials || (access_key_id.len == 0 && secret_access_key.len == 0)) {
    ...
    if (!credentials_provider) {
        ...
        if (!credentials_provider) {
            HGOTO_ERROR(H5E_VFL, H5E_CANTINIT, FAIL, "couldn't create anonymous credentials provider for AWS client: %s", aws_error_str(aws_last_error()));
        }
    }
                                                        ...
                                                        struct aws_http_message** message_out)
{
    ...
    if (!request_http_message) {
        HGOTO_ERROR(H5E_VFL, H5E_CANTALLOC, FAIL, "couldn't allocate memory for HTTP header: %s", aws_error_str(aws_last_error()));
    }

    if (AWS_OP_SUCCESS != aws_http_message_set_request_method(request_http_message, http_method_cursor)) {
        HGOTO_ERROR(H5E_VFL, H5E_CANTSET, FAIL, "couldn't set HTTP request method: %s", aws_error_str(aws_last_error()));
    }

    if (AWS_OP_SUCCESS != aws_byte_buf_init_copy_from_cursor(&path_buf, aws_params->allocator, slash_cursor)) {
        HGOTO_ERROR(H5E_VFL, H5E_CANTALLOC, FAIL, "couldn't allocate memory for path string: %s", aws_error_str(aws_last_error()));
    }
    ...
    if (!use_virtual_style) {
        ...
        if (AWS_OP_SUCCESS != aws_byte_buf_append_dynamic(&path_buf, &bucket_cursor)) {
            HGOTO_ERROR(H5E_VFL, H5E_CANTALLOC, FAIL, "couldn't allocate memory for HTTP request string: %s", aws_error_str(aws_last_error()));
        }
    ...
    if (AWS_OP_SUCCESS != aws_byte_buf_append_encoding_uri_path(&path_buf, &key_cursor)) {
        HGOTO_ERROR(H5E_VFL, H5E_CANTALLOC, FAIL, "couldn't URI-encode object key for HTTP request: %s", aws_error_str(aws_last_error()));
    }
    ...
    if (AWS_OP_SUCCESS != aws_http_message_set_request_path(request_http_message, path_cursor)) {
        HGOTO_ERROR(H5E_VFL, H5E_CANTSET, FAIL, "couldn't set HTTP request path: %s", aws_error_str(aws_last_error()));
    }
                                               ...
                                               const char* port)
{
    ...
    if (use_virtual_style) {
        ...
        if (aws_byte_cursor_starts_with_ignore_case(&host_cursor, &bucket_cursor)) {
            ...
            if (AWS_OP_SUCCESS != aws_byte_buf_init_copy_from_cursor(&host_buf, aws_params->allocator, bucket_cursor)) {
                HGOTO_ERROR(H5E_VFL, H5E_CANTALLOC, FAIL, "couldn't allocate memory for 'Host' header string: %s", aws_error_str(aws_last_error()));
            }
        ...
        if (port) {
            ...
            if (AWS_OP_SUCCESS != aws_byte_buf_init_copy_from_cursor(&host_buf, aws_params->allocator, host_cursor)) {
                HGOTO_ERROR(H5E_VFL, H5E_CANTALLOC, FAIL, "couldn't allocate memory for 'Host' header string: %s", aws_error_str(aws_last_error()));
            }
    ...
    if (AWS_OP_SUCCESS != aws_http_message_add_header(message, host_header)) {
        HGOTO_ERROR(H5E_VFL, H5E_CANTSET, FAIL, "couldn't add 'Host' header to HTTP message: %s", aws_error_str(aws_last_error()));
    }
...
static herr_t H5FD__s3comms_format_range_header(struct aws_http_message* message, haddr_t offset, size_t len)
{
    ...
    if (len > 0) {
        ret = snprintf(byte_range_str, sizeof(byte_range_str), "bytes=%" PRIuHADDR "-%" PRIuHADDR, offset, offset + len - 1);
    }
    else if (offset > 0) {
    ...
    if (AWS_OP_SUCCESS != aws_http_message_add_header(message, range_header)) {
        HGOTO_ERROR(H5E_VFL, H5E_CANTSET, FAIL, "couldn't add 'Range' header to HTTP message: %s", aws_error_str(aws_last_error()));
    }
...
static herr_t H5FD__s3comms_format_user_agent_header(struct aws_http_message* message)
{
    ...
    ret = snprintf(user_agent_str, sizeof(user_agent_str), "libhdf5/%u.%u.%u (vfd:ros3) libaws-c-s3", H5_VERS_MAJOR, H5_VERS_MINOR, H5_VERS_RELEASE);
    ...
    if (AWS_OP_SUCCESS != aws_http_message_add_header(message, user_agent_header)) {
        HGOTO_ERROR(H5E_VFL, H5E_CANTSET, FAIL, "couldn't add 'User-Agent' header to HTTP message: %s", aws_error_str(aws_last_error()));
    }
...
static const char* H5FD__s3comms_httpcode_to_str(long httpcode, bool* handled)
{
    *handled = true;
    ...
    switch (httpcode) {
    case 301: return "resource has been permanently moved"; break;
    case 400:
        return "malformed/Bad request for resource; possible mismatch between specified AWS region and "
               "region in URL (if any)";
        break;
    case 401: return "valid authentication needed to access resource"; break;
    case 403: return "unauthorized access to resource"; break;
    case 404: return "resource not found"; break;
    case 405: return "method not allowed"; break;
    case 408: return "request timed out"; break;
    case 409: return "resource already exists"; break;
    case 410: return "resource has been deleted"; break;
    case 413: return "request for resource was too large"; break;
    case 416: return "requested resource byte range was not satisfiable"; break;
    case 429: return "too many requests"; break;
    case 500: return "internal server error"; break;
    case 501: return "request method not implemented"; break;
