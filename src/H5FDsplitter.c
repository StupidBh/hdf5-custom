...
/* Prototypes */
static herr_t H5FD__splitter_populate_config(H5FD_splitter_vfd_config_t* vfd_config, H5FD_splitter_fapl_t* fapl_out);
static herr_t H5FD__splitter_get_default_wo_path(char* new_path, size_t new_path_len, const char* base_filename);
...
static herr_t H5FD__splitter_ctl(H5FD_t* _file, uint64_t op_code, uint64_t flags, const void* input, void** output);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Pget_fapl_splitter(hid_t fapl_id, H5FD_splitter_vfd_config_t* config /*out*/)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__splitter_flush(H5FD_t* _file, hid_t H5_ATTR_UNUSED dxpl_id, bool closing)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__splitter_read(H5FD_t* _file, H5FD_mem_t H5_ATTR_UNUSED type, hid_t H5_ATTR_UNUSED dxpl_id, haddr_t addr, size_t size, void* buf)
{
    H5FD_splitter_t* file = (H5FD_splitter_t*)_file;
    herr_t ret_value = SUCCEED;
 ...
 *-------------------------------------------------------------------------
 */
static H5FD_t* H5FD__splitter_open(const char* name, unsigned flags, hid_t splitter_fapl_id, haddr_t maxaddr)
{
    ...
    if (!file_ptr->logfp) {
        if (file_ptr->fa.log_file_path[0] != '\0') {
            ...
            if ((log_file_fd = HDopen(file_ptr->fa.log_file_path, O_WRONLY | O_CREAT | O_TRUNC, H5_POSIX_CREATE_MODE_URWGROR)) < 0) {
...
static herr_t H5FD__splitter_close(H5FD_t* _file)
{
    ...
    if (file->wo_file) {
        if (H5FD_close(file->wo_file) == FAIL) {
            H5FD_SPLITTER_WO_ERROR(file, __func__, H5E_VFL, H5E_CANTCLOSEFILE, FAIL, "unable to close W/O file")
        }
    }
    ...
    default:
        if (flags & H5FD_CTL_ROUTE_TO_TERMINAL_VFD_FLAG) {
            ...
            if (flags & H5FD_CTL_FAIL_IF_UNKNOWN_FLAG) {
                HGOTO_ERROR(H5E_VFL, H5E_FCNTL, FAIL, "VFD ctl request failed (unknown op code and fail if unknown flag is set)");
            }
        }
 ...
 *-------------------------------------------------------------------------
 */
static haddr_t H5FD__splitter_alloc(H5FD_t* _file, H5FD_mem_t type, hid_t dxpl_id, hsize_t size)
{
    H5FD_splitter_t* file = (H5FD_splitter_t*)_file; /* VFD file struct */
    haddr_t ret_value = HADDR_UNDEF;                 /* Return value */
