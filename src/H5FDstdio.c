...
#define MY_REGION_OVERFLOW(A, Z) (MY_ADDR_OVERFLOW(A) || MY_SIZE_OVERFLOW(Z) || HADDR_UNDEF == (A) + (Z) || (HDoff_t)((A) + (Z)) < (HDoff_t)(A))
...
static herr_t H5FD_stdio_read(H5FD_t* lf, H5FD_mem_t type, hid_t fapl_id, haddr_t addr, size_t size, void* buf);
static herr_t H5FD_stdio_write(H5FD_t* lf, H5FD_mem_t type, hid_t fapl_id, haddr_t addr, size_t size, const void* buf);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__stdio_init(void)
{
...
static H5FD_t* H5FD_stdio_open(const char* name, unsigned flags, hid_t fapl_id, haddr_t maxaddr)
{
    ...
    if (!f) {
        ...
        if (flags & H5F_ACC_CREAT) {
            ...
            H5Epush_ret(__func__, H5E_ERR_CLS, H5E_IO, H5E_CANTOPENFILE, "file doesn't exist and CREAT wasn't specified", NULL);
        }
    }
        ...
        fclose(f);
        H5Epush_ret(__func__, H5E_ERR_CLS, H5E_IO, H5E_FILEEXISTS, "file exists but CREAT and EXCL were specified", NULL);
    }
    ...
    if (ignore_disabled_file_locks_s != -1) {
        ...
        if (H5Pget_file_locking(fapl_id, &unused, &file->ignore_disabled_file_locks) < 0) {
            fclose(file->fp);
            free(file);
            H5Epush_ret(__func__, H5E_ERR_CLS, H5E_FILE, H5E_CANTGET, "unable to get use disabled file locks property", NULL);
        }
    }
    ...
    if (!GetFileInformationByHandle((HANDLE)file->hFile, &fileinfo)) {
        fclose(file->fp);
        free(file);
        H5Epush_ret(__func__, H5E_ERR_CLS, H5E_FILE, H5E_CANTOPENFILE, "unable to get Windows file descriptor information", NULL);
    } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD_stdio_close(H5FD_t* _file)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD_stdio_query(const H5FD_t* _f, unsigned long /*OUT*/* flags)
{
    /* Quiet the compiler */
    (void)_f;
 ...
 *-------------------------------------------------------------------------
 */
static haddr_t H5FD_stdio_alloc(H5FD_t* _file, H5FD_mem_t /*UNUSED*/ type, hid_t /*UNUSED*/ dxpl_id, hsize_t size)
{
    H5FD_stdio_t* file = (H5FD_stdio_t*)_file;
    haddr_t addr;
 ...
 *-------------------------------------------------------------------------
 */
static haddr_t H5FD_stdio_get_eoa(const H5FD_t* _file, H5FD_mem_t /*UNUSED*/ type)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD_stdio_set_eoa(H5FD_t* _file, H5FD_mem_t /*UNUSED*/ type, haddr_t addr)
{
    H5FD_stdio_t* file = (H5FD_stdio_t*)_file;
 ...
 *-------------------------------------------------------------------------
 */
static haddr_t H5FD_stdio_get_eof(const H5FD_t* _file, H5FD_mem_t /*UNUSED*/ type)
{
    const H5FD_stdio_t* file = (const H5FD_stdio_t*)_file;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD_stdio_get_handle(H5FD_t* _file, hid_t /*UNUSED*/ fapl, void** file_handle)
{
    H5FD_stdio_t* file = (H5FD_stdio_t*)_file;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD_stdio_read(H5FD_t* _file, H5FD_mem_t /*UNUSED*/ type, hid_t /*UNUSED*/ dxpl_id, haddr_t addr, size_t size, void /*OUT*/* buf)
{
    H5FD_stdio_t* file = (H5FD_stdio_t*)_file;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD_stdio_write(H5FD_t* _file, H5FD_mem_t /*UNUSED*/ type, hid_t /*UNUSED*/ dxpl_id, haddr_t addr, size_t size, const void* buf)
{
    H5FD_stdio_t* file = (H5FD_stdio_t*)_file;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD_stdio_flush(H5FD_t* _file, hid_t /*UNUSED*/ dxpl_id, bool closing)
{
    H5FD_stdio_t* file = (H5FD_stdio_t*)_file;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD_stdio_truncate(H5FD_t* _file, hid_t /*UNUSED*/ dxpl_id, bool /*UNUSED*/ closing)
{
    H5FD_stdio_t* file = (H5FD_stdio_t*)_file;
    ...
    if (file->write_access) {
        ...
        if (file->eoa != file->eof) {
            ...
            if (0 == bError) {
                H5Epush_ret(__func__, H5E_ERR_CLS, H5E_IO, H5E_SEEKERROR, "unable to truncate/extend file properly", -1);
            }
#else  /* H5_HAVE_WIN32_API */
            /* Reset seek offset to beginning of file, so that file isn't re-extended later */
            rewind(file->fp);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD_stdio_delete(const char* filename, hid_t /*UNUSED*/ fapl_id)
{
    assert(filename);
