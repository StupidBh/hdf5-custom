...
static herr_t H5FD__subfiling_read(H5FD_t* _file, H5FD_mem_t type, hid_t fapl_id, haddr_t addr, size_t size, void* buf);
static herr_t H5FD__subfiling_write(H5FD_t* _file, H5FD_mem_t type, hid_t dxpl_id, haddr_t addr, size_t size, const void* buf);
static herr_t H5FD__subfiling_read_vector(H5FD_t* file, hid_t dxpl_id, uint32_t count, H5FD_mem_t types[], haddr_t addrs[], size_t sizes[], void* bufs[] /* out */);
static herr_t
    H5FD__subfiling_write_vector(H5FD_t* file, hid_t dxpl_id, uint32_t count, H5FD_mem_t types[], haddr_t addrs[], size_t sizes[], const void* bufs[] /* in */);
...
static herr_t H5FD__subfiling_ctl(H5FD_t* _file, uint64_t op_code, uint64_t flags, const void* input, void** output);
...
static herr_t H5FD__subfiling_io_helper(H5FD_subfiling_t* file,
                                        size_t io_count,
                                        H5FD_mem_t types[],
                                        haddr_t addrs[],
                                        size_t sizes[],
...
static herr_t H5FD__subfiling_mirror_writes_to_stub(H5FD_subfiling_t* file, uint32_t count, H5FD_mem_t types[], haddr_t addrs[], size_t sizes[], const void* bufs[]);
static herr_t H5FD__subfiling_generate_io_vectors(subfiling_context_t* sf_context,
                                                  size_t in_count,
                                                  H5FD_mem_t types[],
                                                  haddr_t file_offsets[],
                                                  size_t io_sizes[],
                                                  H5_flexible_const_ptr_t bufs[],
                                                  H5FD_subfiling_io_type_t io_type,
                                                  size_t* ioreq_count_out,
                                                  uint32_t* iovec_len_out,
                                                  H5FD_mem_t** io_types_out,
                                                  haddr_t** io_addrs_out,
                                                  size_t** io_sizes_out,
                                                  H5_flexible_const_ptr_t** io_bufs_out);
static herr_t H5FD__subfiling_get_iovec_sizes(subfiling_context_t* sf_context,
                                              size_t in_count,
                                              haddr_t file_offsets[],
                                              size_t io_sizes[],
                                              size_t* max_iovec_depth,
                                              size_t* max_num_subfiles);
static herr_t H5FD__subfiling_translate_io_req_to_iovec(subfiling_context_t* sf_context,
                                                        size_t iovec_idx,
                                                        size_t iovec_len,
                                                        size_t iovec_count,
                                                        H5FD_mem_t type,
                                                        haddr_t addr,
                                                        size_t io_size,
                                                        H5_flexible_const_ptr_t io_buf,
                                                        H5FD_subfiling_io_type_t io_type,
                                                        H5FD_mem_t* io_types,
                                                        haddr_t* io_addrs,
                                                        size_t* io_sizes,
                                                        H5_flexible_const_ptr_t* io_bufs);
static herr_t H5FD__subfiling_iovec_fill_first(subfiling_context_t* sf_context,
                                               size_t iovec_len,
                                               int64_t cur_iovec_depth,
                                               int64_t target_datasize,
                                               int64_t start_mem_offset,
                                               int64_t start_file_offset,
                                               int64_t first_io_len,
                                               H5_flexible_const_ptr_t buf,
                                               H5FD_subfiling_io_type_t io_type,
                                               haddr_t* io_addrs_ptr,
                                               size_t* io_sizes_ptr,
                                               H5_flexible_const_ptr_t* io_bufs_ptr);
static herr_t H5FD__subfiling_iovec_fill_last(subfiling_context_t* sf_context,
                                              size_t iovec_len,
                                              int64_t cur_iovec_depth,
                                              int64_t target_datasize,
                                              int64_t start_mem_offset,
                                              ...
                                              size_t* io_sizes_ptr,
                                              H5_flexible_const_ptr_t* io_bufs_ptr);
static herr_t H5FD__subfiling_iovec_fill_first_last(subfiling_context_t* sf_context,
                                                    size_t iovec_len,
                                                    int64_t cur_iovec_depth,
                                                    int64_t target_datasize,
                                                    int64_t start_mem_offset,
                                                    ...
                                                    size_t* io_sizes_ptr,
                                                    H5_flexible_const_ptr_t* io_bufs_ptr);
static herr_t H5FD__subfiling_iovec_fill_uniform(subfiling_context_t* sf_context,
                                                 size_t iovec_len,
                                                 int64_t cur_iovec_depth,
                                                 int64_t target_datasize,
                                                 int64_t start_mem_offset,
...
static void H5_subfiling_dump_iovecs(subfiling_context_t* sf_context,
                                     size_t ioreq_count,
                                     size_t iovec_len,
                                     H5FD_subfiling_io_type_t io_type,
                                     H5FD_mem_t* io_types,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__subfiling_init(void)
{
...
static herr_t H5FD__subfiling_term(void)
{
    ...
    if (!mpi_finalized) {
        ...
        printf(
            "** WARNING **: HDF5 is terminating the Subfiling VFD after MPI_Finalize() was called "
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Pset_fapl_subfiling(hid_t fapl_id, const H5FD_subfiling_config_t* vfd_config)
{
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Pget_fapl_subfiling(hid_t fapl_id, H5FD_subfiling_config_t* config_out)
{
...
herr_t H5FDsubfiling_get_file_mapping(hid_t file_id, char*** filenames, size_t* len)
{
...
static herr_t H5FD__subfiling_sb_decode(H5FD_t* _file, const char* name, const unsigned char* buf)
{
    ...
    if (file->fail_to_encode) {
        HGOTO_ERROR(H5E_VFL, H5E_CANTDECODE, FAIL, "can't decode subfiling driver info message - message wasn't encoded (or encoded improperly)");
    }
    ...
    if (tmpu64 > 0) {
        if (!sf_context->config_file_prefix) {
            if (NULL == (sf_context->config_file_prefix = H5MM_malloc(tmpu64))) {
                HGOTO_ERROR(H5E_VFL, H5E_CANTALLOC, FAIL, "can't allocate space for config file prefix string");
            }
    ...
    if (file->fa.shared_cfg.stripe_size != sf_context->sf_stripe_size) {
        HGOTO_ERROR(H5E_VFL,
                    ...
                    file->fa.shared_cfg.stripe_size);
    }
    ...
    if (file->fa.shared_cfg.stripe_count != sf_context->sf_num_subfiles) {
        HGOTO_ERROR(H5E_VFL,
                    ...
                    file->fa.shared_cfg.stripe_count);
    }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__copy_plist(hid_t fapl_id, hid_t* id)
{
    H5P_genplist_t* plist = NULL;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__subfiling_query(const H5FD_t H5_ATTR_UNUSED* _file, unsigned long* flags /* out */)
{
    FUNC_ENTER_PACKAGE_NOERR
 ...
 *-------------------------------------------------------------------------
 */
static haddr_t H5FD__subfiling_get_eoa(const H5FD_t* _file, H5FD_mem_t H5_ATTR_UNUSED type)
{
 ...
 *-------------------------------------------------------------------------
 */
static haddr_t H5FD__subfiling_get_eof(const H5FD_t* _file, H5FD_mem_t H5_ATTR_UNUSED type)
{
    const H5FD_subfiling_t* file = (const H5FD_subfiling_t*)_file;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__subfiling_get_handle(H5FD_t* _file, hid_t H5_ATTR_UNUSED fapl, void** file_handle)
{
    H5FD_subfiling_t* file = (H5FD_subfiling_t*)_file;
    herr_t ret_value = SUCCEED;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__subfiling_read(H5FD_t* _file, H5FD_mem_t type, hid_t H5_ATTR_UNUSED dxpl_id, haddr_t addr, size_t size, void* buf /*out*/)
{
    H5FD_subfiling_t* file = (H5FD_subfiling_t*)_file;
    herr_t ret_value = SUCCEED;
    ...
    if (H5FD__subfiling_io_helper(file, 1, &type, &addr, &size, (H5_flexible_const_ptr_t*)&buf, IO_TYPE_READ) < 0) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__subfiling_write(H5FD_t* _file, H5FD_mem_t type, hid_t H5_ATTR_UNUSED dxpl_id, haddr_t addr, size_t size, const void* buf /*in*/)
{
    H5FD_subfiling_t* file = (H5FD_subfiling_t*)_file;
    herr_t ret_value = SUCCEED;
    ...
    if (H5FD__subfiling_io_helper(file, 1, &type, &addr, &size, (H5_flexible_const_ptr_t*)&buf, IO_TYPE_WRITE) < 0) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__subfiling_read_vector(H5FD_t* _file, hid_t dxpl_id, uint32_t count, H5FD_mem_t types[], haddr_t addrs[], size_t sizes[], void* bufs[] /* out */)
{
    H5FD_subfiling_t* file = (H5FD_subfiling_t*)_file;
    herr_t ret_value = SUCCEED;
 ...
 */
static herr_t
    H5FD__subfiling_write_vector(H5FD_t* _file, hid_t dxpl_id, uint32_t count, H5FD_mem_t types[], haddr_t addrs[], size_t sizes[], const void* bufs[] /* in */)
{
    H5FD_subfiling_t* file = (H5FD_subfiling_t*)_file;
    herr_t ret_value = SUCCEED;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__subfiling_truncate(H5FD_t* _file, hid_t H5_ATTR_UNUSED dxpl_id, bool H5_ATTR_UNUSED closing)
{
    H5FD_subfiling_t* file = (H5FD_subfiling_t*)_file;
    herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__subfiling_ctl(H5FD_t* _file, uint64_t op_code, uint64_t flags, const void H5_ATTR_UNUSED* input, void** output)
{
    H5FD_subfiling_t* file = (H5FD_subfiling_t*)_file;
    herr_t ret_value = SUCCEED; /* Return value */
                                        ...
                                        H5FD_subfiling_io_type_t io_type)
{
    ...
    for (size_t i = 0; i < io_count; i++) {
        ...
        if (H5FD_REGION_OVERFLOW(addrs[i], io_size)) {
            HGOTO_ERROR(H5E_ARGS, H5E_OVERFLOW, FAIL, "addr[%zu] overflow, addr = %" PRIuHADDR ", size = %zu", i, addrs[i], io_size);
        }
        if ((addrs[i] + io_size) > file_eoa) {
            HGOTO_ERROR(H5E_ARGS,
                        H5E_OVERFLOW,
                        FAIL,
                        "addr overflow, addrs[%zu] = %" PRIuHADDR ", sizes[%zu] = %zu, eoa = %" PRIuHADDR,
                        i,
                        addrs[i],
                        i,
                        io_size,
    ...
    if (num_subfiles == 1) {
        ...
        if (io_type == IO_TYPE_WRITE) {
             ...
             * they are reading
             */
            if (H5FD__subfiling_mirror_writes_to_stub(file, u32_io_count, types, addrs, sizes, (const void**)bufs) < 0) {
         ...
         * this I/O request.
         */
        status = H5FD__subfiling_generate_io_vectors(sf_context,   /* IN:  Subfiling context used to look up config info */
...
#ifdef H5_SUBFILING_DEBUG
        H5_subfiling_dump_iovecs(sf_context, ioreq_count, iovec_len, io_type, io_types, io_addrs, io_sizes, io_bufs);
#endif
        /*
        ...
        for (size_t ioreq_idx = 0; ioreq_idx < ioreq_count; ioreq_idx++) {
            ...
            if (io_type == IO_TYPE_WRITE) {
                /* Make vector write call to VFD controlling subfiles */
                if (H5FD_write_vector(file->sf_file, final_vec_len, io_types_ptr, io_addrs_ptr, io_sizes_ptr, (const void**)io_bufs_ptr) < 0) {
                    HGOTO_ERROR(H5E_VFL, H5E_WRITEERROR, FAIL, "write to subfile failed");
                }
                 ...
                 * they are reading
                 */
                if (H5FD__subfiling_mirror_writes_to_stub(file, final_vec_len, io_types_ptr, io_addrs_ptr, io_sizes_ptr, (const void**)io_bufs_ptr) < 0) {
                    HGOTO_ERROR(H5E_VFL, H5E_WRITEERROR, FAIL, "mirrored write to stub file failed");
                }
            }
            else {
                if (!rank0_bcast || (file->mpi_rank == 0)) {
                    /* Make vector read call to VFD controlling subfiles */
                    if (H5FD_read_vector(file->sf_file, final_vec_len, io_types_ptr, io_addrs_ptr, io_sizes_ptr, (void**)io_bufs_ptr) < 0) {
                        HGOTO_ERROR(H5E_VFL, H5E_READERROR, FAIL, "read from subfile failed");
                    }
                }
            }
...
static herr_t H5FD__subfiling_mirror_writes_to_stub(H5FD_subfiling_t* file, uint32_t count, H5FD_mem_t types[], haddr_t addrs[], size_t sizes[], const void* bufs[])
{
    ...
    if (all_super_or_ohdr_writes) {
        if (H5FD_write_vector(file->stub_file, count, types, addrs, sizes, bufs) < 0) {
            HGOTO_ERROR(H5E_VFL, H5E_WRITEERROR, FAIL, "couldn't write superblock/object header information to stub file");
        }
    }
        ...
        if (H5FD_write_vector(file->stub_file, vec_len, copied_types, copied_addrs, copied_sizes, copied_bufs) < 0) {
            HGOTO_ERROR(H5E_VFL, H5E_WRITEERROR, FAIL, "couldn't write superblock/object header information to stub file");
        }
    }
                                                  ...
                                                  H5_flexible_const_ptr_t** io_bufs_out)
{
    ...
    *iovec_len_out = 0;
     ...
     */
    if (H5FD__subfiling_get_iovec_sizes(sf_context, in_count, file_offsets, io_sizes, &max_iovec_depth, &max_num_subfiles_touched) < 0) {
    ...
    for (size_t io_idx = 0; io_idx < in_count; io_idx++) {
        ...
        if (H5FD__subfiling_translate_io_req_to_iovec(sf_context,
                                                      iovec_idx,
                                                      max_num_subfiles_touched,
                                                      max_iovec_depth,
                                                      mem_type,
                                              ...
                                              size_t* max_num_subfiles)
{
    ...
    for (size_t io_idx = 0; io_idx < in_count; io_idx++) {
        ...
        if (cur_file_offset < 0) {
            HGOTO_ERROR(H5E_VFL, H5E_BADVALUE, FAIL, "file offset of %" PRIuHADDR " at index %zu too large; wrapped around", file_offsets[io_idx], io_idx);
        }
        if (data_size < 0) {
            HGOTO_ERROR(H5E_VFL, H5E_BADVALUE, FAIL, "I/O size of %zu at index %zu too large; wrapped around", io_size, io_idx);
        }
                                                        ...
                                                        H5_flexible_const_ptr_t* io_bufs)
{
...
#ifdef H5_SUBFILING_DEBUG
    H5FD__subfiling_log(sf_context->sf_context_id,
                        "%s: TRANSLATING I/O REQUEST (MEMORY TYPE: %d, ADDR: %" PRIuHADDR
                        ", I/O SIZE: %zu, BUF: %p)\n"
                        "STRIPE SIZE: %" PRId64 ", BLOCK SIZE: %" PRId64
                        ", NUM SUBFILES: %d\n"
                        ...
                        addr,
                        io_size,
                        (io_type == IO_TYPE_WRITE) ? (const void*)io_buf.cvp : (void*)io_buf.vp,
                        stripe_size,
                        block_size,
                        num_subfiles,
                        stripe_idx,
        ...
        if (num_subfiles > 1) {
            ...
            if (is_first) {
                if (is_last) {
                     ...
                     */
                    if (H5FD__subfiling_iovec_fill_first_last(sf_context,
                                                              iovec_len,
                                                              iovec_depth,
                                                              subfile_bytes,
                                                              mem_offset,
                     ...
                     */
                    if (H5FD__subfiling_iovec_fill_first(sf_context,
                                                         iovec_len,
                                                         iovec_depth,
                                                         subfile_bytes,
                                                         mem_offset,
                 ...
                 */
                if (H5FD__subfiling_iovec_fill_last(sf_context,
                                                    iovec_len,
                                                    iovec_depth,
                                                    subfile_bytes,
                                                    mem_offset,
                 ...
                 */
                if (H5FD__subfiling_iovec_fill_uniform(sf_context,
                                                       iovec_len,
                                                       iovec_depth,
                                                       subfile_bytes,
                                                       mem_offset,
    ...
    if (total_bytes != io_size) {
        HGOTO_ERROR(H5E_VFL, H5E_CANTINIT, FAIL, "total bytes (%zu) didn't match data size (%zu)!", total_bytes, io_size);
    }
...
#ifdef H5_SUBFILING_DEBUG
    H5FD__subfiling_log(sf_context->sf_context_id,
                        "%s: start_mem_offset = %" PRId64 ", start_file_offset = %" PRId64 ", first_io_len = %" PRId64,
                        __func__,
                        start_mem_offset,
                        start_file_offset,
                        first_io_len);
    ...
    *io_sizes_ptr = (size_t)first_io_len;
    ...
    if (first_io_len > 0) {
...
#ifdef H5_SUBFILING_DEBUG
            H5FD__subfiling_log(sf_context->sf_context_id,
                                "%s: mem_offset[%zu] = %" PRId64 ", file_offset[%zu] = %" PRId64 ", io_block_len[%zu] = %" PRId64,
                                __func__,
                                i,
                                next_mem_offset,
                                i,
        ...
        if (total_bytes != target_datasize) {
            HGOTO_ERROR(H5E_VFL, H5E_CANTINIT, FAIL, "total bytes (%" PRId64 ") didn't match target data size (%" PRId64 ")!", total_bytes, target_datasize);
        }
    }
...
#ifdef H5_SUBFILING_DEBUG
    H5FD__subfiling_log(sf_context->sf_context_id,
                        "%s: start_mem_offset = %" PRId64 ", start_file_offset = %" PRId64 ", last_io_len = %" PRId64,
                        __func__,
                        start_mem_offset,
                        start_file_offset,
                        last_io_len);
...
#ifdef H5_SUBFILING_DEBUG
    H5FD__subfiling_log(sf_context->sf_context_id,
                        "%s: start_mem_offset = %" PRId64 ", start_file_offset = %" PRId64 ", first_io_len = %" PRId64 ", last_io_len = %" PRId64,
                        __func__,
                        start_mem_offset,
                        start_file_offset,
                        first_io_len,
...
#ifdef H5_SUBFILING_DEBUG
    H5FD__subfiling_log(sf_context->sf_context_id,
                        "%s: start_mem_offset = %" PRId64 ", start_file_offset = %" PRId64 ", segment size = %" PRId64,
                        __func__,
                        start_mem_offset,
                        start_file_offset,
                        stripe_size);
                                     ...
                                     H5_flexible_const_ptr_t* io_bufs)
{
    ...
    H5FD__subfiling_log(sf_context->sf_context_id, "%s: I/O REQUEST VECTORS (mem type, addr, size, buf):", __func__);
    ...
    for (size_t ioreq_idx = 0; ioreq_idx < ioreq_count; ioreq_idx++) {
        ...
        for (size_t i = 0; i < iovec_len; i++) {
            ...
            H5FD__subfiling_log_nonewline(sf_context->sf_context_id,
                                          "(%d, %" PRIuHADDR ", %zu, %p)",
                                          *(io_types + (ioreq_idx * iovec_len) + i),
                                          *(io_addrs + (ioreq_idx * iovec_len) + i),
