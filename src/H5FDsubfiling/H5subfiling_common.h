 ...
 */
static inline void* H5FD__subfiling_cast_to_void(const void* data)
{
    union
    {
...
extern "C"
{
    ...
    H5_DLL herr_t H5FD__subfiling_open_stub_file(const char* name, unsigned flags, MPI_Comm file_comm, H5FD_t** file_ptr, uint64_t* file_id);
    H5_DLL herr_t H5FD__subfiling_open_subfiles(const char* base_filename,
                                                uint64_t file_id,
    ...
    H5_DLL herr_t H5FD__subfiling_set_config_prop(H5P_genplist_t* plist_ptr, const H5FD_subfiling_params_t* vfd_config);
