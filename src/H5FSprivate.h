...
typedef struct H5FS_section_class_t
{
    ...
    H5FS_section_info_t* (
        *deserialize)(const struct H5FS_section_class_t*, const uint8_t*, haddr_t, hsize_t, unsigned*); /* Routine to deserialize a buffer into a "live" section */
    htri_t (*can_merge)(const H5FS_section_info_t*, const H5FS_section_info_t*, void*);                 /* Routine to determine if two nodes are mergeable */
...
H5_DLL H5FS_t* H5FS_create(H5F_t* f,
                           ...
                           hsize_t threshold);
H5_DLL H5FS_t*
    H5FS_open(H5F_t* f, haddr_t fs_addr, uint16_t nclasses, const H5FS_section_class_t* classes[], void* cls_init_udata, hsize_t alignment, hsize_t threshold);
...
H5_DLL herr_t H5FS_sect_add(H5F_t* f, H5FS_t* fspace, H5FS_section_info_t* node, unsigned flags, void* op_data, bool* merged_or_shrunk);
