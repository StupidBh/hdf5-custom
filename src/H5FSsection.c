...
static herr_t H5FS__size_node_decr(H5FS_sinfo_t* sinfo, unsigned bin, H5FS_node_t* fspace_node, const H5FS_section_class_t* cls);
static herr_t H5FS__sect_unlink_size(H5FS_sinfo_t* sinfo, const H5FS_section_class_t* cls, H5FS_section_info_t* sect);
static herr_t H5FS__sect_unlink_rest(H5FS_t* fspace, const H5FS_section_class_t* cls, H5FS_section_info_t* sect);
static herr_t H5FS__sect_remove_real(H5FS_t* fspace, H5FS_section_info_t* sect);
static herr_t H5FS__sect_link_size(H5FS_sinfo_t* sinfo, const H5FS_section_class_t* cls, H5FS_section_info_t* sect);
static herr_t H5FS__sect_link_rest(H5FS_t* fspace, const H5FS_section_class_t* cls, H5FS_section_info_t* sect, unsigned flags);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FS__sinfo_lock(H5F_t* f, H5FS_t* fspace, unsigned accmode)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FS__sinfo_unlock(H5F_t* f, H5FS_t* fspace, bool modified)
{
    herr_t ret_value = SUCCEED; /* Return value */
    ...
    fprintf(stderr,
            ...
            __func__,
            fspace->sinfo_lock_count,
            fspace->sinfo_modified,
            fspace->sinfo_protected);
    fprintf(stderr, "%s: fspace->alloc_sect_size = %" PRIuHSIZE ", fspace->sect_size = %" PRIuHSIZE "\n", __func__, fspace->alloc_sect_size, fspace->sect_size);
#endif /* H5FS_SINFO_DEBUG */
...
static herr_t H5FS__sect_link(H5FS_t* fspace, H5FS_section_info_t* sect, unsigned flags)
{
    ...
    if (ret_value < 0) {
        if (linked_sect && H5FS__sect_unlink_size(fspace->sinfo, cls, sect) < 0) {
            HDONE_ERROR(H5E_FSPACE, H5E_CANTFREE, FAIL, "can't remove section from size tracking data structures");
        }
    }
    ...
    if (fspace->sinfo->merge_list) {
        do {
            ...
            if (greater_sect_node) {
                 ...
                 *  then check for 'can merge' callback
                 */
                if ((!(sect_cls->flags & H5FS_CLS_MERGE_SYM) || ((*sect)->type == tmp_sect->type)) && sect_cls->can_merge) {
                    /* Determine if the sections can merge */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FS_sect_add(H5F_t* f, H5FS_t* fspace, H5FS_section_info_t* sect, unsigned flags, void* op_data, bool* merged_or_shrunk)
{
...
#ifdef H5FS_SINFO_DEBUG
    fprintf(stderr,
            "%s: *sect = {%" PRIuHADDR ", %" PRIuHSIZE ", %u, %s}\n",
            __func__,
            sect->addr,
            sect->size,
            sect->type,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FS__iterate_sect_cb(void* _item, void H5_ATTR_UNUSED* key, void* _udata)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FS__iterate_node_cb(void* _item, void H5_ATTR_UNUSED* key, void* _udata)
{
...
herr_t H5FS_vfd_alloc_hdr_and_section_info_if_needed(H5F_t* f, H5FS_t* fspace, haddr_t* fs_addr_ptr)
{
    ...
    assert((f->shared->fs_strategy == H5F_FSPACE_STRATEGY_FSM_AGGR) || (f->shared->fs_strategy == H5F_FSPACE_STRATEGY_PAGE));
    ...
    if (ret_value < 0) {
        ...
        if (inserted_section) {
            if (H5AC_remove_entry(fspace->sinfo) < 0) {
                HDONE_ERROR(H5E_FSPACE, H5E_CANTEXPUNGE, FAIL, "can't remove file free space section from cache");
            }
        }
        ...
        if (inserted_header) {
            if (H5AC_mark_entry_clean(fspace) < 0) {
                HDONE_ERROR(H5E_FSPACE, H5E_CANTMARKCLEAN, FAIL, "can't mark file free space header as clean");
            }
            ...
            if (H5AC_remove_entry(fspace) < 0) {
                HDONE_ERROR(H5E_FSPACE, H5E_CANTEXPUNGE, FAIL, "can't remove file free space header from cache");
            }
        }
    }
