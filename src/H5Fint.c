...
static herr_t H5F__get_objects(const H5F_t* f, unsigned types, size_t max_index, hid_t* obj_id_list, bool app_ref, size_t* obj_id_count_ptr);
...
static herr_t H5F__check_if_using_file_locks(H5P_genplist_t* fapl, bool* use_file_locking, bool* ignore_disabled_locks);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F__parse_file_lock_env_var(htri_t* use_locks, htri_t* ignore_disabled_locks)
{
 ...
 *-------------------------------------------------------------------------
 */
hid_t H5F_get_access_plist(H5F_t* f, bool app_ref)
{
    ...
    if (H5P_set(new_plist, H5F_ACS_IGNORE_DISABLED_FILE_LOCKS_NAME, &f->shared->ignore_disabled_locks) < 0) {
        HGOTO_ERROR(H5E_FILE, H5E_CANTSET, H5I_INVALID_HID, "can't set 'ignore disabled file locks' property");
    }
    ...
    if (f->shared->page_buf != NULL) {
        ...
        if (H5P_set(new_plist, H5F_ACS_PAGE_BUFFER_MIN_META_PERC_NAME, &(f->shared->page_buf->min_meta_perc)) < 0) {
            HGOTO_ERROR(H5E_FILE, H5E_CANTSET, H5I_INVALID_HID, "can't set minimum metadata fraction of page buffer");
        }
        if (H5P_set(new_plist, H5F_ACS_PAGE_BUFFER_MIN_RAW_PERC_NAME, &(f->shared->page_buf->min_raw_perc)) < 0) {
            HGOTO_ERROR(H5E_FILE, H5E_CANTSET, H5I_INVALID_HID, "can't set minimum raw data fraction of page buffer");
        }
    } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F_get_obj_count(const H5F_t* f, unsigned types, bool app_ref, size_t* obj_id_count_ptr)
{
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F_get_obj_ids(const H5F_t* f, unsigned types, size_t max_objs, hid_t* oid_list, bool app_ref, size_t* obj_id_count_ptr)
{
    herr_t ret_value = SUCCEED; /* Return value */
 ...
 *---------------------------------------------------------------------------
 */
static herr_t H5F__get_objects(const H5F_t* f, unsigned types, size_t max_nobjs, hid_t* obj_id_list, bool app_ref, size_t* obj_id_count_ptr)
{
...
static int H5F__get_objects_cb(void* obj_ptr, hid_t obj_id, void* key)
{
    ...
    if (olist->obj_type == H5I_FILE) {
        if ((olist->file_info.local && (!olist->file_info.ptr.file || (olist->file_info.ptr.file && (H5F_t*)obj_ptr == olist->file_info.ptr.file))) ||
        ...
        case H5I_ATTR: oloc = H5A_oloc((H5A_t*)obj_ptr); break;

        case H5I_GROUP: oloc = H5G_oloc((H5G_t*)obj_ptr); break;

        case H5I_DATASET: oloc = H5D_oloc((H5D_t*)obj_ptr); break;
        ...
        case H5I_MAP: HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, H5_ITER_ERROR, "maps not supported in native VOL connector");
        ...
        case H5I_EVENTSET:
        case H5I_NTYPES:
        default                : HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, H5_ITER_ERROR, "unknown or invalid data object");
        } /* end switch */

        if ((olist->file_info.local && ((!olist->file_info.ptr.file && olist->obj_type == H5I_DATATYPE && H5T_is_immutable((H5T_t*)obj_ptr) == false) ||
                                        (!olist->file_info.ptr.file && olist->obj_type != H5I_DATATYPE) || (oloc && oloc->file == olist->file_info.ptr.file))) ||
            (!olist->file_info.local &&
             ((!olist->file_info.ptr.shared && olist->obj_type == H5I_DATATYPE && H5T_is_immutable((H5T_t*)obj_ptr) == false) ||
              (!olist->file_info.ptr.shared && olist->obj_type != H5I_DATATYPE) || (oloc && oloc->file && oloc->file->shared == olist->file_info.ptr.shared)))) {
            add_obj = true;
        } /* end if */
    } /* end else */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F_prefix_open_file(bool try,
                            H5F_t** _file,
                            H5F_t* primary_file,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F__is_hdf5(const char* name, hid_t fapl_id, bool* is_hdf5)
{
...
static herr_t H5F__dest(H5F_t* f, bool flush, bool free_on_failure)
{
    ...
    if (f->vol_obj) {
        ...
        if (H5T_unregister(H5T_PERS_SOFT, NULL, NULL, NULL, f->vol_obj, NULL) < 0) {
            HDONE_ERROR(H5E_FILE, H5E_CANTRELEASE, FAIL, "unable to free cached type conversion path table entries");
        }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F_open(bool try, H5F_t** _file, const char* name, unsigned flags, hid_t fcpl_id, hid_t fapl_id)
{
    ...
    if (set_flag) {
        if (H5F_INTENT(file) & H5F_ACC_RDWR) { /* Set and check consistency of status_flags */
            ...
            if (file->shared->sblock->super_vers >= HDF5_SUPERBLOCK_VERSION_3) {
                ...
                else if ((file->shared->sblock->status_flags & H5F_SUPER_WRITE_ACCESS) || (file->shared->sblock->status_flags & H5F_SUPER_SWMR_WRITE_ACCESS)) {
                    HGOTO_ERROR(H5E_FILE,
                                H5E_CANTOPENFILE,
                                FAIL,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F__delete(const char* filename, hid_t fapl_id)
{
    herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F_try_close(H5F_t* f, bool* was_closed /*out*/)
{
    unsigned nopen_files = 0;   /* Number of open files in file/mount hierarchy */
    unsigned nopen_objs = 0;    /* Number of open objects in file/mount hierarchy */
    herr_t ret_value = SUCCEED; /* Return value */
    ...
    default               : HGOTO_ERROR(H5E_FILE, H5E_CANTCLOSEFILE, FAIL, "can't close file, unknown file close degree");
    } /* end switch */
    ...
    if (f->shared->fc_degree == H5F_CLOSE_STRONG) {
        ...
        if (f->nopen_objs > 0) {
            ...
            while ((result = H5F_get_obj_ids(f,
                                             H5F_OBJ_LOCAL | H5F_OBJ_DATASET | H5F_OBJ_GROUP | H5F_OBJ_ATTR,
                                             (int)(sizeof(objs) / sizeof(objs[0])),
                                             objs,
                                             false,
                                             &obj_count)) <= 0 &&
                   obj_count != 0) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5F__build_actual_name(const H5F_t* f, const H5P_genplist_t* fapl, const char* name, char** actual_name /*out*/)
{
 ...
 *-------------------------------------------------------------------------
 */
void H5F_addr_encode(const H5F_t* f, uint8_t** pp /*in,out*/, haddr_t addr)
{
    /* Use FUNC_ENTER_NOAPI_NOINIT_NOERR here to avoid performance issues */
    FUNC_ENTER_NOAPI_NOINIT_NOERR
 ...
 *-------------------------------------------------------------------------
 */
void H5F_addr_decode_len(size_t addr_len, const uint8_t** pp /*in,out*/, haddr_t* addr_p /*out*/)
{
 ...
 *-------------------------------------------------------------------------
 */
void H5F_addr_decode(const H5F_t* f, const uint8_t** pp /*in,out*/, haddr_t* addr_p /*out*/)
{
    /* Use FUNC_ENTER_NOAPI_NOINIT_NOERR here to avoid performance issues */
    FUNC_ENTER_NOAPI_NOINIT_NOERR
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F_set_store_msg_crt_idx(H5F_t* f, bool flag)
{
    /* Use FUNC_ENTER_NOAPI_NOINIT_NOERR here to avoid performance issues */
    FUNC_ENTER_NOAPI_NOINIT_NOERR
...
herr_t H5F__set_libver_bounds(H5F_t* f, H5F_libver_t low, H5F_libver_t high)
{
    ...
    if ((f->shared->low_bound != low || f->shared->high_bound != high) && !(H5F_INTENT(f) & H5F_ACC_SWMR_WRITE)) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F__get_file_image(H5F_t* file, void* buf_ptr, size_t buf_len, size_t* image_len)
{
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F__set_paged_aggr(const H5F_t* f, bool paged)
{
    herr_t ret_value = SUCCEED; /* Return value */
...
herr_t H5F__start_swmr_write(H5F_t* f)
{
    ...
    if (grp_dset_count > 0) {
        ...
        for (u = 0; u < grp_dset_count; u++) {
            ...
            case H5I_MAP: HGOTO_ERROR(H5E_FILE, H5E_BADTYPE, FAIL, "maps not supported in native VOL connector");
            ...
            case H5I_EVENTSET:
            case H5I_NTYPES:
            default                : HGOTO_ERROR(H5E_FILE, H5E_BADTYPE, FAIL, "not a valid file object ID (dataset, group, or datatype)"); break;
            } /* end switch */
...
herr_t H5F__format_convert(H5F_t* f)
{
    ...
    if (!(f->shared->fs_strategy == H5F_FILE_SPACE_STRATEGY_DEF && f->shared->fs_persist == H5F_FREE_SPACE_PERSIST_DEF &&
          f->shared->fs_threshold == H5F_FREE_SPACE_THRESHOLD_DEF && f->shared->fs_page_size == H5F_FILE_SPACE_PAGE_SIZE_DEF)) {
 ...
 *-------------------------------------------------------------------------
 */
hid_t H5F_get_file_id(H5VL_object_t* vol_obj, H5I_type_t obj_type, bool app_ref)
{
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F_set_min_dset_ohdr(H5F_t* f, bool minimize)
{
    /* Use FUNC_ENTER_NOAPI_NOINIT_NOERR here to avoid performance issues */
    FUNC_ENTER_NOAPI_NOINIT_NOERR
