...
herr_t H5F__close_mounts(H5F_t* f)
{
    ...
    for (u = f->shared->mtab.nmounts - 1; u < f->shared->mtab.nmounts; u--) {
        ...
        if (f->shared->mtab.child[u].file->parent == f) {
            ...
            memmove(f->shared->mtab.child + u, f->shared->mtab.child + u + 1, (f->shared->mtab.nmounts - u - 1) * sizeof(f->shared->mtab.child[0]));
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F_mount(const H5G_loc_t* loc, const char* name, H5F_t* child, hid_t H5_ATTR_UNUSED plist_id)
{
    ...
    /* Insert into table */
    memmove(parent->shared->mtab.child + md + 1, parent->shared->mtab.child + md, (parent->shared->mtab.nmounts - md) * sizeof(parent->shared->mtab.child[0]));
    ...
    if (H5G_name_replace(NULL, H5G_NAME_MOUNT, mp_loc.oloc->file, mp_loc.path->full_path_r, root_loc.oloc->file, root_loc.path->full_path_r) < 0) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F_unmount(const H5G_loc_t* loc, const char* name)
{
    ...
    if (H5G_name_replace(NULL, H5G_NAME_UNMOUNT, mp_loc.oloc->file, mp_loc.path->full_path_r, root_loc.oloc->file, root_loc.path->full_path_r) < 0) {
 ...
 *-------------------------------------------------------------------------
 */
bool H5F_is_mount(const H5F_t* file)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5F__flush_mounts_recurse(H5F_t* f)
{
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F_traverse_mount(H5O_loc_t* oloc /*in,out*/)
{
