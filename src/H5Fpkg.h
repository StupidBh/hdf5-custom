 ...
 */
    ...
    #define H5F_HAVE_FREE_SPACE_MANAGER(F) ((F)->shared->fs_strategy == H5F_FSPACE_STRATEGY_FSM_AGGR || (F)->shared->fs_strategy == H5F_FSPACE_STRATEGY_PAGE)
    ...
    #define H5F_SUPERBLOCK_VARLEN_SIZE(v, sizeof_addr, sizeof_size)                                                                                        \
        ((v == 0 ? H5F_SUPERBLOCK_VARLEN_SIZE_V0(sizeof_addr, sizeof_size) : 0) + (v == 1 ? H5F_SUPERBLOCK_VARLEN_SIZE_V1(sizeof_addr, sizeof_size) : 0) + \
         (v >= 2 ? H5F_SUPERBLOCK_VARLEN_SIZE_V2(sizeof_addr) : 0))

    /* Total size of superblock, depends on superblock version */
    #define H5F_SUPERBLOCK_SIZE(s) (H5F_SUPERBLOCK_FIXED_SIZE + H5F_SUPERBLOCK_VARLEN_SIZE((s)->super_vers, (s)->sizeof_addr, (s)->sizeof_size))
...
H5_DLL herr_t H5F__super_ext_write_msg(H5F_t* f, unsigned id, void* mesg, bool may_create, unsigned mesg_flags);
...
H5_DLL herr_t H5F__efc_open(bool try, H5F_efc_t* efc, H5F_t** file, const char* name, unsigned flags, hid_t fcpl_id, hid_t fapl_id);
