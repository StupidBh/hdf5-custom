 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F__super_ext_close(H5F_t* f, H5O_loc_t* ext_ptr, bool was_created)
{
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F__super_read(H5F_t* f, H5P_genplist_t* fa_plist, bool initial_read)
{
    ...
    if (!skip_eof_check && initial_read) {
        ...
        if ((eof + sblock->base_addr) < udata.stored_eof) {
            HGOTO_ERROR(H5E_FILE,
                        ...
                        (unsigned long long)sblock->base_addr,
    ...
    if (H5_addr_defined(sblock->ext_addr)) {
         ...
         */
        if (sblock->super_vers < HDF5_SUPERBLOCK_VERSION_2) {
            HGOTO_ERROR(H5E_FILE, H5E_BADVALUE, FAIL, "invalid superblock - extension message should not be defined for version < 2");
        }
     ...
     */
    if (((rw_flags & H5AC__READ_ONLY_FLAG) == 0) && sblock->super_vers >= HDF5_SUPERBLOCK_VERSION_2 && H5_addr_defined(sblock->ext_addr)) {
...
herr_t H5F__super_init(H5F_t* f)
{
    ...
    bool drvinfo_in_cache = false;                     /* Whether the driver info block has been inserted into the metadata cache */
     ...
     */
        ...
        if (sblock->btree_k[H5B_SNODE_ID] != HDF5_BTREE_SNODE_IK_DEF || sblock->btree_k[H5B_CHUNK_ID] != HDF5_BTREE_CHUNK_IK_DEF ||
    ...
    if (need_ext) {
        ...
        if (driver_size > 0) {
            ...
            if (H5AC_insert_entry(f, H5AC_DRVRINFO, sblock->driver_addr, drvinfo, H5AC__PIN_ENTRY_FLAG | H5AC__FLUSH_LAST_FLAG | H5AC__FLUSH_COLLECTIVELY_FLAG) <
                0) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F__super_ext_write_msg(H5F_t* f, unsigned id, void* mesg, bool may_create, unsigned mesg_flags)
{
    H5AC_ring_t orig_ring = H5AC_RING_INV; /* Original ring value */
