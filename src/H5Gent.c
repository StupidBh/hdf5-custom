 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G_ent_decode(const H5F_t* f, const uint8_t** pp, H5G_entry_t* ent, const uint8_t* p_end)
{
    ...
    *pp += 4; /*reserved*/
    ...
    case H5G_NCACHED:
    default              : HGOTO_ERROR(H5E_SYM, H5E_BADVALUE, FAIL, "unknown symbol table entry cache type");
    } /* end switch */
...
herr_t H5G_ent_encode(const H5F_t* f, uint8_t** pp, const H5G_entry_t* ent)
{
    ...
    if (ent) {
        ...
        case H5G_CACHED_SLINK: UINT32ENCODE(*pp, ent->cache.slink.lval_offset); break;
 ...
 *-------------------------------------------------------------------------
 */
void H5G__ent_copy(H5G_entry_t* dst, H5G_entry_t* src, H5_copy_depth_t depth)
{
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__ent_to_link(const H5G_entry_t* ent, const H5HL_t* heap, H5O_link_t* lnk)
{
...
herr_t H5G__ent_debug(const H5G_entry_t* ent, FILE* stream, int indent, int fwidth, const H5HL_t* heap)
{
    ...
    fprintf(stream, "%*s%-*s %lu\n", indent, "", fwidth, "Name offset into private heap:", (unsigned long)(ent->name_off));

    fprintf(stream, "%*s%-*s %" PRIuHADDR "\n", indent, "", fwidth, "Object header address:", ent->header);

    fprintf(stream, "%*s%-*s ", indent, "", fwidth, "Cache info type:");
    switch (ent->type) {
    case H5G_NOTHING_CACHED: fprintf(stream, "Nothing Cached\n"); break;

    case H5G_CACHED_STAB:
        fprintf(stream, "Symbol Table\n");

        fprintf(stream, "%*s%-*s\n", indent, "", fwidth, "Cached entry information:");
        fprintf(stream, "%*s%-*s %" PRIuHADDR "\n", nested_indent, "", nested_fwidth, "B-tree address:", ent->cache.stab.btree_addr);

        fprintf(stream, "%*s%-*s %" PRIuHADDR "\n", nested_indent, "", nested_fwidth, "Heap address:", ent->cache.stab.heap_addr);
        break;
    ...
    case H5G_CACHED_SLINK:
        ...
        if (heap) {
            ...
            fprintf(stream, "%*s%-*s %s\n", nested_indent, "", nested_fwidth, "Link value:", (lval == NULL) ? "" : lval);
        } /* end if */
        else {
    ...
    case H5G_CACHED_ERROR:
    case H5G_NCACHED:
    default              : fprintf(stream, "*** Unknown symbol type %d\n", ent->type); break;
    } /* end switch */
