 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G_mkroot(H5F_t* f, bool create_root)
{
    ...
    if ((H5F_INTENT(f) & H5F_ACC_RDWR) && stab_exists != false && f->shared->sblock->root_ent && f->shared->sblock->root_ent->type != H5G_CACHED_STAB) {
