...
herr_t H5G__user_path_test(hid_t obj_id, char* user_path, size_t* user_path_len, unsigned* obj_hidden)
{
    ...
    case H5I_GROUP: obj_path = H5G_nameof((H5G_t*)obj_ptr); break;

    case H5I_DATASET: obj_path = H5D_nameof((H5D_t*)obj_ptr); break;
    ...
    case H5I_EVENTSET:
    case H5I_NTYPES:
    default                : HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL, "unknown data object type");
    } /* end switch */
    assert(obj_path);
 ...
 */
static int
    H5G__verify_cached_stabs_test_cb(H5F_t* f, const void H5_ATTR_UNUSED* _lt_key, haddr_t addr, const void H5_ATTR_UNUSED* _rt_key, void H5_ATTR_UNUSED* udata)
{
    H5G_node_t* sn = NULL;
    H5O_loc_t targ_oloc;
    H5O_t* targ_oh = NULL;
    ...
    for (i = 0; i < sn->nsyms; i++) {
        ...
        if (stab_exists) {
            ...
            if ((sn->entry[i].cache.stab.btree_addr != stab.btree_addr) || (sn->entry[i].cache.stab.heap_addr != stab.heap_addr)) {
...
herr_t H5G__verify_cached_stabs_test(hid_t gid)
{
    ...
    if ((ret_value = H5B_iterate(grp->oloc.file, H5B_SNODE, stab.btree_addr, H5G__verify_cached_stabs_test_cb, &udata)) < 0) {
