 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__man_dblock_destroy(H5HF_hdr_t* hdr, H5HF_direct_t* dblock, haddr_t dblock_addr, bool* parent_removed)
{
...
herr_t H5HF__man_dblock_new(H5HF_hdr_t* hdr, size_t request, H5HF_free_section_t** ret_sec_node)
{
    ...
    if (!H5_addr_defined(hdr->man_dtable.table_addr) && min_dblock_size == hdr->man_dtable.cparam.start_block_size) {
    ...
    /* Root entry already exists, allocate direct block from root indirect block */
        ...
        if (min_dblock_size > next_size) {
            fprintf(stderr, "%s: Skipping direct block sizes not supported, min_dblock_size = %zu, next_size = %zu\n", __func__, min_dblock_size, next_size);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__man_dblock_locate(H5HF_hdr_t* hdr, hsize_t obj_off, H5HF_indirect_t** ret_iblock, unsigned* ret_entry, bool* ret_did_protect, unsigned flags)
{
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__man_dblock_dest(H5HF_direct_t* dblock)
{
    herr_t ret_value = SUCCEED; /* Return value */
