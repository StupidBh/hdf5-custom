 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__iblock_incr(H5HF_indirect_t* iblock)
{
    herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__iblock_dirty(H5HF_indirect_t* iblock)
{
    herr_t ret_value = SUCCEED; /* Return value */
...
herr_t H5HF__man_iblock_root_create(H5HF_hdr_t* hdr, size_t min_dblock_size)
{
    ...
    if (min_dblock_size > hdr->man_dtable.cparam.start_block_size) {
        ...
        if (H5HF__hdr_skip_blocks(hdr, iblock, have_direct_block, ((nrows - 1) * hdr->man_dtable.cparam.width) - have_direct_block) < 0) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5HF__man_iblock_root_halve(H5HF_indirect_t* iblock)
{
    ...
    if (old_nrows > hdr->man_dtable.max_direct_rows) {
        ...
        if (iblock->nrows > hdr->man_dtable.max_direct_rows) {
            ...
            /* Re-allocate child indirect block array */
            if (NULL ==
                (iblock->child_iblocks = H5FL_SEQ_REALLOC(H5HF_indirect_ptr_t, iblock->child_iblocks, (size_t)(indir_rows * hdr->man_dtable.cparam.width)))) {
                HGOTO_ERROR(H5E_HEAP, H5E_NOSPACE, FAIL, "memory allocation failed for filtered direct entries");
            }
        } /* end if */
        else {
...
herr_t H5HF__man_iblock_alloc_row(H5HF_hdr_t* hdr, H5HF_free_section_t** sec_node)
{
     ...
     * section under it. */
    if (old_sec_node->sect_info.state == H5FS_SECT_SERIALIZED || (H5FS_SECT_SERIALIZED == old_sec_node->u.row.under->sect_info.state) ||
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__man_iblock_unprotect(H5HF_indirect_t* iblock, unsigned cache_flags, bool did_protect)
{
    herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__man_iblock_attach(H5HF_indirect_t* iblock, unsigned entry, haddr_t child_addr)
{
    herr_t ret_value = SUCCEED; /* Return value */
...
herr_t H5HF__man_iblock_detach(H5HF_indirect_t* iblock, unsigned entry)
{
    ...
    if (iblock->block_off == 0) {
        ...
        if (!iblock->removed_from_cache) {
            ...
            if (iblock->nchildren > 0 && hdr->man_dtable.cparam.start_root_rows != 0 && entry > iblock->max_child) {
                unsigned max_child_row; /* Row for max. child entry */
    ...
    if (!iblock->removed_from_cache) {
        ...
        if (iblock->nchildren == 0) {
            ...
            if (NULL ==
                (del_iblock =
                     H5HF__man_iblock_protect(hdr, iblock->addr, iblock->nrows, iblock->parent, iblock->par_entry, true, H5AC__NO_FLAGS_SET, &did_protect))) {
                HGOTO_ERROR(H5E_HEAP, H5E_CANTPROTECT, FAIL, "unable to protect fractal heap indirect block");
            }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__man_iblock_entry_addr(H5HF_indirect_t* iblock, unsigned entry, haddr_t* child_addr)
{
...
herr_t H5HF__man_iblock_size(H5F_t* f, H5HF_hdr_t* hdr, haddr_t iblock_addr, unsigned nrows, H5HF_indirect_t* par_iblock, unsigned par_entry, hsize_t* heap_size)
{
    ...
    *heap_size += iblock->size;
    ...
    if (iblock->nrows > hdr->man_dtable.max_direct_rows) {
        ...
        num_indirect_rows = (H5VM_log2_gen(hdr->man_dtable.row_block_size[hdr->man_dtable.max_direct_rows]) - first_row_bits) + 1;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__man_iblock_dest(H5HF_indirect_t* iblock)
{
    herr_t ret_value = SUCCEED; /* Return value */
