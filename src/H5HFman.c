...
/* Local Prototypes */
/********************/
static herr_t H5HF__man_op_real(H5HF_hdr_t* hdr, const uint8_t* id, H5HF_operator_t op, void* op_data, unsigned op_flags);
...
herr_t H5HF__man_insert(H5HF_hdr_t* hdr, size_t obj_size, const void* obj, void* _id)
{
    ...
    if (sec_node->sect_info.type == H5HF_FSPACE_SECT_FIRST_ROW || sec_node->sect_info.type == H5HF_FSPACE_SECT_NORMAL_ROW) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__man_get_obj_len(H5HF_hdr_t* hdr, const uint8_t* id, size_t* obj_len_p)
{
