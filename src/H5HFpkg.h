 ...
 */
    ...
    #define H5HF_HEADER_SIZE(h)                                                                                                            \
        (                                                                                 /* General metadata fields */                    \
         H5HF_METADATA_PREFIX_SIZE(true) /* Fractal Heap Header specific fields */        /* General heap information */                   \
         + (unsigned)2                                                                    /* Heap ID len */                                \
         + (unsigned)2                                                                    /* I/O filters' encoded len */                   \
         + (unsigned)1 /* Status flags */                                                 /* "Huge" object fields */                       \
         + (unsigned)4                                                                    /* Max. size of "managed" object */              \
         + (unsigned)(h)->sizeof_size                                                     /* Next ID for "huge" object */                  \
         + (unsigned)(h)->sizeof_addr /* File address of "huge" object tracker B-tree  */ /* "Managed" object free space fields */         \
         + (unsigned)(h)->sizeof_size                                                     /* Total man. free space */                      \
         + (unsigned)(h)->sizeof_addr /* File address of free section header */           /* Statistics fields */                          \
         ...
         + (unsigned)(h)->sizeof_size /* Number of tiny objects in heap */                /* "Managed" object doubling table info */       \
    ...
    #define H5HF_MAN_ABS_DIRECT_OVERHEAD(h)                                                                                        \
        (                                               /* General metadata fields */                                              \
         H5HF_METADATA_PREFIX_SIZE(h->checksum_dblocks) /* Fractal heap managed, absolutely mapped direct block specific fields */ \
    ...
    #define H5HF_MAN_INDIRECT_CHILD_DIR_ENTRY_SIZE(h)                                                                                                         \
        (((h)->filter_len > 0 ? ((unsigned)(h)->sizeof_addr + (unsigned)(h)->sizeof_size + (unsigned)4) : /* Size of entries for filtered direct blocks */    \
    ...
    #define H5HF_MAN_INDIRECT_SIZE(h, r)                                                                                            \
        (                                              /* General metadata fields */                                                \
         H5HF_METADATA_PREFIX_SIZE(true)               /* Fractal heap managed, absolutely mapped indirect block specific fields */ \
         + (unsigned)(h)->sizeof_addr                  /* File address of heap owning the block */                                  \
         + (unsigned)(h)->heap_off_size                /* Offset of the block in the heap */                                        \
         ...
         + (((r > (h)->man_dtable.max_direct_rows) ? (r - (h)->man_dtable.max_direct_rows) : 0) * (h)->man_dtable.cparam.width *    \
            (h)->sizeof_addr)                          /* Size of entries for indirect blocks */                                    \
        )
...
H5_DLL hsize_t H5HF__dtable_span_size(const H5HF_dtable_t* dtable, unsigned start_row, unsigned start_col, unsigned num_entries);
...
H5_DLL herr_t H5HF__man_dblock_destroy(H5HF_hdr_t* hdr, H5HF_direct_t* dblock, haddr_t dblock_addr, bool* parent_removed);
H5_DLL H5HF_direct_t*
    H5HF__man_dblock_protect(H5HF_hdr_t* hdr, haddr_t dblock_addr, size_t dblock_size, H5HF_indirect_t* par_iblock, unsigned par_entry, unsigned flags);
H5_DLL herr_t H5HF__man_dblock_locate(H5HF_hdr_t* hdr, hsize_t obj_off, H5HF_indirect_t** par_iblock, unsigned* par_entry, bool* par_did_protect, unsigned flags);
...
H5_DLL herr_t H5HF__man_iter_start_entry(H5HF_hdr_t* hdr, H5HF_block_iter_t* biter, H5HF_indirect_t* iblock, unsigned start_entry);
...
H5_DLL herr_t H5HF__man_iter_curr(H5HF_block_iter_t* biter, unsigned* row, unsigned* col, unsigned* entry, H5HF_indirect_t** block);
...
H5_DLL H5HF_free_section_t* H5HF__sect_single_new(hsize_t sect_off, size_t sect_size, H5HF_indirect_t* parent, unsigned par_entry);
H5_DLL herr_t H5HF__sect_single_revive(H5HF_hdr_t* hdr, H5HF_free_section_t* sect);
H5_DLL herr_t H5HF__sect_single_dblock_info(H5HF_hdr_t* hdr, const H5HF_free_section_t* sect, haddr_t* dblock_addr, size_t* dblock_size);
