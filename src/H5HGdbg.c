...
herr_t H5HG_debug(H5F_t* f, haddr_t addr, FILE* stream, int indent, int fwidth)
{
    ...
    fprintf(stream, "%*s%-*s %lu\n", indent, "", fwidth, "Total collection size in file:", (unsigned long)(h->size));
    ...
    fprintf(stream, "%*s%-*s %u/%lu/", indent, "", fwidth, "Objects defined/allocated/max:", nused, (unsigned long)h->nalloc);
    if (nused) {
        fprintf(stream, "%u\n", maxobj);
    }
    else {
    ...
    for (u = 1; u < h->nused; u++) {
        if (h->obj[u].begin) {
            ...
            fprintf(stream, "%*s%s\n", indent, "", buf);
            fprintf(stream, "%*s%-*s %lu\n", indent + 3, "", MIN(fwidth - 3, 0), "Obffset in block:", (unsigned long)(h->obj[u].begin - h->chunk));
            fprintf(stream, "%*s%-*s %d\n", indent + 3, "", MIN(fwidth - 3, 0), "Reference count:", h->obj[u].nrefs);
            fprintf(stream,
                    "%*s%-*s %lu/%lu\n",
                    indent + 3,
                    "",
