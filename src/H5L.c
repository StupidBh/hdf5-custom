...
static herr_t H5L__create_soft_api_common(const char* link_target,
                                          hid_t link_loc_id,
...
static herr_t H5L__create_hard_api_common(hid_t cur_loc_id,
                                          const char* cur_name,
                                          hid_t new_loc_id,
                                          const char* new_name,
                                          hid_t lcpl_id,
                                          hid_t lapl_id,
                                          void** token_ptr,
                                          H5VL_connector_t** conn);
static herr_t H5L__delete_api_common(hid_t loc_id, const char* name, hid_t lapl_id, void** token_ptr, H5VL_object_t** _vol_obj_ptr);
static herr_t H5L__delete_by_idx_api_common(hid_t loc_id,
                                            ...
                                            void** token_ptr,
                                            H5VL_object_t** _vol_obj_ptr);
static herr_t H5L__exists_api_common(hid_t loc_id, const char* name, bool* exists, hid_t lapl_id, void** token_ptr, H5VL_object_t** _vol_obj_ptr);
static herr_t H5L__iterate_api_common(hid_t group_id,
                                      H5_index_t idx_type,
                                      H5_iter_order_t order,
                                      hsize_t* idx_p,
                                      H5L_iterate2_t op,
...
herr_t H5Lcopy(hid_t src_loc_id, const char* src_name, hid_t dst_loc_id, const char* dst_name, hid_t lcpl_id, hid_t lapl_id)
{
    ...
    /* Copy the link */
    if (H5VL_link_copy(vol_obj1, &loc_params1, vol_obj2, &loc_params2, lcpl_id, lapl_id, H5P_DATASET_XFER_DEFAULT, H5_REQUEST_NULL) < 0) {
                                          ...
                                          H5VL_object_t** _vol_obj_ptr)
{
    ...
    /* Create the link */
    if (H5VL_link_create(&vol_cb_args, *vol_obj_ptr, &loc_params, lcpl_id, lapl_id, H5P_DATASET_XFER_DEFAULT, token_ptr) < 0) {
                            ...
                            hid_t es_id)
{
    ...
    if (H5L__create_soft_api_common(link_target, link_loc_id, link_name, lcpl_id, lapl_id, token_ptr, &vol_obj) < 0) {
                                          ...
                                          H5VL_connector_t** connector)
{
    ...
    vol_cb_args.args.hard.curr_loc_params.obj_type = (H5L_SAME_LOC != cur_loc_id ? H5I_get_type(cur_loc_id) : H5I_BADID);
    ...
    /* Create the link */
    if (H5VL_link_create(&vol_cb_args, (link_vol_obj ? link_vol_obj : curr_vol_obj), &link_loc_params, lcpl_id, lapl_id, H5P_DATASET_XFER_DEFAULT, token_ptr) < 0) {
...
herr_t H5Lcreate_hard(hid_t cur_loc_id, const char* cur_name, hid_t new_loc_id, const char* new_name, hid_t lcpl_id, hid_t lapl_id)
{
    ...
    if (H5L__create_hard_api_common(cur_loc_id, cur_name, new_loc_id, new_name, lcpl_id, lapl_id, NULL, NULL) < 0) {
                            ...
                            hid_t es_id)
{
    ...
    if (H5L__create_hard_api_common(cur_loc_id, cur_name, new_loc_id, new_name, lcpl_id, lapl_id, token_ptr, &connector) < 0) {
...
herr_t H5Lcreate_external(const char* file_name, const char* obj_name, hid_t link_loc_id, const char* link_name, hid_t lcpl_id, hid_t lapl_id)
{
    ...
    *p++ = (H5L_EXT_VERSION << 4) | H5L_EXT_FLAGS_ALL;                /* External link version & flags */
    ...
    if (H5VL_link_create(&vol_cb_args, vol_obj, &loc_params, lcpl_id, lapl_id, H5P_DATASET_XFER_DEFAULT, H5_REQUEST_NULL) < 0) {
                              ...
                              hid_t es_id)
{
    ...
    /* Delete a link asynchronously */
    if (H5L__delete_by_idx_api_common(loc_id, group_name, idx_type, order, n, lapl_id, token_ptr, &vol_obj) < 0) {
        HGOTO_ERROR(H5E_LINK, H5E_CANTDELETE, FAIL, "unable to asynchronously delete link");
    }
 ...
 *
 *--------------------------------------------------------------------------*/
herr_t H5Lexists_async(const char* app_file, const char* app_func, unsigned app_line, hid_t loc_id, const char* name, bool* exists, hid_t lapl_id, hid_t es_id)
{
    H5VL_object_t* vol_obj = NULL;      /* Object for loc_id */
    void* token = NULL;                 /* Request token for async operation        */
    void** token_ptr = H5_REQUEST_NULL; /* Pointer to request token for async operation        */
    ...
    /* If a token was created, add the token to the event set */
    if (NULL != token) {
        if (H5ES_insert(es_id,
                        H5VL_OBJ_CONNECTOR(vol_obj),
                        token,
                        /* clang-format off */
 ...
 */
ssize_t
    H5Lget_name_by_idx(hid_t loc_id, const char* group_name, H5_index_t idx_type, H5_iter_order_t order, hsize_t n, char* name /*out*/, size_t size, hid_t lapl_id)
{
    H5VL_object_t* vol_obj = NULL;    /* object of loc_id */
    H5VL_link_get_args_t vol_cb_args; /* Arguments to VOL callback */
    H5VL_loc_params_t loc_params;     /* Location parameters for object access */
                        ...
                        hid_t es_id)
{
    ...
    if ((ret_value = H5L__iterate_api_common(group_id, idx_type, order, idx_p, op, op_data, token_ptr, &vol_obj)) < 0) {
