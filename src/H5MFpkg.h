 ...
 */
    ...
    #define H5MF_SECT_CLASS_TYPE(F, S) \
        ((H5F_PAGED_AGGR(F)) ? ((S >= (F)->shared->fs_page_size) ? H5MF_FSPACE_SECT_LARGE : H5MF_FSPACE_SECT_SMALL) : H5MF_FSPACE_SECT_SIMPLE)

    /* Get section class cls */
    #define H5MF_SECT_CLS_TYPE(F, S) \
        ((H5F_PAGED_AGGR(F)) ? ((S >= (F)->shared->fs_page_size) ? H5MF_FSPACE_SECT_CLS_LARGE : H5MF_FSPACE_SECT_CLS_SMALL) : H5MF_FSPACE_SECT_CLS_SIMPLE)
...
H5_DLL herr_t H5MF__add_sect(H5F_t* f, H5FD_mem_t alloc_type, H5FS_t* fspace, H5MF_free_section_t* node, bool* merged_or_shrunk);
...
H5_DLL herr_t H5MF__aggr_absorb(const H5F_t* f, H5F_blk_aggr_t* aggr, H5MF_free_section_t* sect, bool allow_sect_absorb);
