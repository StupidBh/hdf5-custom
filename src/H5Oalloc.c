...
static herr_t H5O__add_gap(H5F_t* f, H5O_t* oh, unsigned chunkno, bool* chk_dirtied, size_t idx, uint8_t* new_gap_loc, size_t new_gap_size);
static herr_t H5O__eliminate_gap(H5O_t* oh, bool* chk_dirtied, H5O_mesg_t* mesg, uint8_t* new_gap_loc, size_t new_gap_size);
static herr_t H5O__alloc_null(H5F_t* f, H5O_t* oh, size_t null_idx, const H5O_msg_class_t* new_type, void* new_native, size_t new_size);
static htri_t H5O__alloc_extend_chunk(H5F_t* f, H5O_t* oh, unsigned chunkno, size_t size, size_t* msg_idx);
static herr_t H5O__alloc_find_best_nonnull(const H5F_t* f, const H5O_t* oh, size_t* size, H5O_msg_alloc_info_t* found_msg);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__add_gap(H5F_t H5_ATTR_NDEBUG_UNUSED* f, H5O_t* oh, unsigned chunkno, bool* chk_dirtied, size_t idx, uint8_t* new_gap_loc, size_t new_gap_size)
{
    ...
    if (!merged_with_null) {
        ...
        if (new_gap_size >= (size_t)H5O_SIZEOF_MSGHDR_OH(oh)) {
            ...
            null_msg->raw = (oh->chunk[chunkno].image + oh->chunk[chunkno].size) - (H5O_SIZEOF_CHKSUM_OH(oh) + null_msg->raw_size);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__eliminate_gap(H5O_t* oh, bool* chk_dirtied, H5O_mesg_t* mesg, uint8_t* gap_loc, size_t gap_size)
{
...
static htri_t H5O__alloc_extend_chunk(H5F_t* f, H5O_t* oh, unsigned chunkno, size_t size, size_t* msg_idx)
{
    ...
    for (u = 0; u < oh->nmesgs; u++) {
        ...
        if (oh->mesg[u].chunkno == chunkno && H5O_NULL_ID == oh->mesg[u].type->id &&
            ((oh->mesg[u].raw + oh->mesg[u].raw_size) ==
             ((oh->chunk[chunkno].image + oh->chunk[chunkno].size) - (oh->chunk[chunkno].gap + H5O_SIZEOF_CHKSUM_OH(oh))))) {
            extend_msg = u;
            extended_msg = true;
            break;
        } /* end if */
    ...
    if (oh->version > H5O_VERSION_1 && chunkno == 0) {
        ...
        if (orig_prfx_size < 8 && (chunk0_size + delta) > 4'294'967'295) {
    ...
    /* Create new null message for end of chunk */
        ...
        oh->mesg[extend_msg].raw =
            ((oh->chunk[chunkno].image + oh->chunk[chunkno].size) - (H5O_SIZEOF_CHKSUM_OH(oh) + oh->chunk[chunkno].gap)) + H5O_SIZEOF_MSGHDR_OH(oh);
    ...
    for (u = 0; u < oh->nmesgs; u++) {
        ...
        if (chunkno > 0 && (H5O_CONT_ID == oh->mesg[u].type->id) && (((H5O_cont_t*)(oh->mesg[u].native))->chunkno == chunkno)) {
...
static herr_t H5O__alloc_find_best_nonnull(const H5F_t* f, const H5O_t* oh, size_t* size, H5O_msg_alloc_info_t* found_msg)
{
    ...
    for (u = 0, curr_msg = &oh->mesg[0]; u < oh->nmesgs; u++, curr_msg++) {
        ...
        if (H5O_CONT_ID != curr_msg->type->id) {
            ...
            uint8_t* end_chunk_data = (oh->chunk[msg_chunkno].image + oh->chunk[msg_chunkno].size) -
...
herr_t H5O__alloc_chunk(H5F_t* f, H5O_t* oh, size_t size, size_t found_null, const H5O_msg_alloc_info_t* found_msg, size_t* new_idx)
{
    ...
    if (found_null >= oh->nmesgs) {
        if (found_msg->msgno < 0) {
            ...
            for (u = 0, curr_msg = &oh->mesg[0]; u < oh->nmesgs; u++, curr_msg++) {
                if (curr_msg->chunkno == chunkno - 1) {
                    if (curr_msg->type->id == H5O_NULL_ID) {
                        ...
                        /* Copy the raw data */
                        H5MM_memcpy(p, curr_msg->raw - (size_t)H5O_SIZEOF_MSGHDR_OH(oh), curr_msg->raw_size + (size_t)H5O_SIZEOF_MSGHDR_OH(oh));
            ...
            null_msg->dirty = true;
            null_msg->native = NULL;
            null_msg->raw = oh->chunk[chunkno - 1].image + ((chunkno == 1) ? H5O_SIZEOF_HDR(oh) : H5O_SIZEOF_CHKHDR_OH(oh)) - H5O_SIZEOF_CHKSUM_OH(oh) +
                            H5O_SIZEOF_MSGHDR_OH(oh);
            null_msg->raw_size =
                oh->chunk[chunkno - 1].size - ((chunkno == 1) ? (size_t)H5O_SIZEOF_HDR(oh) : (size_t)H5O_SIZEOF_CHKHDR_OH(oh)) - (size_t)H5O_SIZEOF_MSGHDR_OH(oh);
            ...
            H5MM_memcpy(p, oh->mesg[found_msg->msgno].raw - H5O_SIZEOF_MSGHDR_OH(oh), oh->mesg[found_msg->msgno].raw_size + (size_t)H5O_SIZEOF_MSGHDR_OH(oh));
            ...
            if (found_msg->gap_size > 0) {
                ...
                assert((null_msg->raw + null_msg->raw_size) == (old_null_msg->raw - H5O_SIZEOF_MSGHDR_OH(oh)));
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O__release_mesg(H5F_t* f, H5O_t* oh, H5O_mesg_t* mesg, bool adj_link)
{
    H5O_chunk_proxy_t* chk_proxy = NULL; /* Chunk that message is in */
    bool chk_dirtied = false;            /* Flag for unprotecting chunk */
    herr_t ret_value = SUCCEED;          /* Return value */
        ...
        if (nonnull_size && nonnull_size <= (gap_size + cont_msg->raw_size + (size_t)H5O_SIZEOF_MSGHDR_OH(oh))) {
            ...
            if (gap_size >= (size_t)H5O_SIZEOF_MSGHDR_OH(oh)) {
                ...
                if (cont_u < (oh->nmesgs - 1)) {
                    memmove(&oh->mesg[cont_u], &oh->mesg[cont_u + 1], ((oh->nmesgs - 1) - cont_u) * sizeof(H5O_mesg_t));
                }
                oh->nmesgs--;
            } /* end else */
            ...
            for (v = oh->nmesgs - 1, curr_msg = &oh->mesg[v]; v < oh->nmesgs; v--, curr_msg--) {
                ...
                if (curr_msg->chunkno == deleted_chunkno) {
                    ...
                    if (curr_msg->type->id == H5O_NULL_ID) {
                        ...
                        if (v < (oh->nmesgs - 1)) {
                            memmove(&oh->mesg[v], &oh->mesg[v + 1], ((oh->nmesgs - 1) - v) * sizeof(H5O_mesg_t));
                        }
                        oh->nmesgs--;
                    } /* end if */
                } /* end if */
    ...
    do {
        ...
        for (u = 0, curr_msg = &oh->mesg[0]; u < oh->nmesgs; u++, curr_msg++) {
            if (H5O_NULL_ID == curr_msg->type->id) {
                ...
                if ((curr_msg->raw + curr_msg->raw_size) != ((chunk->image + chunk->size) - (H5O_SIZEOF_CHKSUM_OH(oh) + chunk->gap))) {
                    ...
                    for (v = 0, nonnull_msg = &oh->mesg[0]; v < oh->nmesgs; v++, nonnull_msg++) {
                        ...
                        if ((curr_msg->chunkno == nonnull_msg->chunkno) && ((curr_msg->raw + curr_msg->raw_size) == (nonnull_msg->raw - H5O_SIZEOF_MSGHDR_OH(oh)))) {
                            ...
                            if (H5O_NULL_ID != nonnull_msg->type->id) {
                                ...
                                curr_msg->raw = nonnull_msg->raw + nonnull_msg->raw_size + H5O_SIZEOF_MSGHDR_OH(oh);
                ...
                for (v = 0, null_msg = &oh->mesg[0]; v < oh->nmesgs; v++, null_msg++) {
                    if (H5O_NULL_ID == null_msg->type->id && curr_msg->chunkno > null_msg->chunkno && curr_msg->raw_size <= null_msg->raw_size) {
    ...
    do {
        ...
        for (u = 0, null_msg = &oh->mesg[0]; u < oh->nmesgs; u++, null_msg++) {
            ...
            if (H5O_NULL_ID == null_msg->type->id && null_msg->chunkno > 0 &&
                ((size_t)H5O_SIZEOF_MSGHDR_OH(oh) + null_msg->raw_size) == (oh->chunk[null_msg->chunkno].size - H5O_SIZEOF_CHKHDR_OH(oh))) {
                H5O_mesg_t* curr_msg;     /* Pointer to current message to operate on */
                ...
                if (null_msg->chunkno < (oh->nchunks - 1)) {
                    memmove(&oh->chunk[null_msg->chunkno], &oh->chunk[null_msg->chunkno + 1], ((oh->nchunks - 1) - null_msg->chunkno) * sizeof(H5O_chunk_t));
                ...
                if (null_msg_no < (oh->nmesgs - 1)) {
                    memmove(&oh->mesg[null_msg_no], &oh->mesg[null_msg_no + 1], ((oh->nmesgs - 1) - null_msg_no) * sizeof(H5O_mesg_t));
                }
...
static herr_t H5O__alloc_shrink_chunk(H5F_t* f, H5O_t* oh, unsigned chunkno)
{
    ...
    for (u = oh->nmesgs - 1, curr_msg = &oh->mesg[u]; u < oh->nmesgs; u--, curr_msg--) {
        if ((H5O_NULL_ID == curr_msg->type->id) && (chunkno == curr_msg->chunkno)) {
            ...
            if (curr_msg->raw + curr_msg->raw_size < old_image + new_size - sizeof_chksum) {
                ...
                memmove(curr_msg->raw - sizeof_msghdr, src, (size_t)(old_image + new_size - sizeof_chksum - src));
    ...
    if (total_msg_size < min_chunk_size) {
        ...
        curr_msg->raw_size = MAX(H5O_ALIGN_OH(oh, min_chunk_size - total_msg_size), sizeof_msghdr) - sizeof_msghdr;
        curr_msg->chunkno = chunkno;
    ...
    if (oh->version > H5O_VERSION_1 && chunkno == 0) {
        ...
        /* Check for moving to a 4-byte size encoding */
        else if (orig_prfx_size > 4 && chunk0_newsize <= 4'294'967'295) {
    ...
    for (u = 0, curr_msg = &oh->mesg[0]; u < oh->nmesgs; u++, curr_msg++) {
         ...
         * its size is directly encoded in the object header) */
        if (chunkno > 0 && (H5O_CONT_ID == curr_msg->type->id) && (((H5O_cont_t*)(curr_msg->native))->chunkno == chunkno)) {
