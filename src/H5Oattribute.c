...
/* Local Prototypes */
/********************/
static herr_t H5O__attr_to_dense_cb(H5O_t* oh, H5O_mesg_t* mesg, unsigned H5_ATTR_UNUSED sequence, void* _udata);
...
static herr_t H5O__attr_rename_chk_cb(H5O_t H5_ATTR_UNUSED* oh, H5O_mesg_t* mesg, unsigned H5_ATTR_UNUSED sequence, void* _udata);
static herr_t H5O__attr_rename_mod_cb(H5O_t* oh, H5O_mesg_t* mesg, unsigned H5_ATTR_UNUSED sequence, void* _udata);
static herr_t H5O__attr_remove_update(const H5O_loc_t* loc, H5O_t* oh, H5O_ainfo_t* ainfo);
static herr_t H5O__attr_remove_cb(H5O_t* oh, H5O_mesg_t* mesg, unsigned H5_ATTR_UNUSED sequence, void* _udata);
static herr_t H5O__attr_exists_cb(H5O_t H5_ATTR_UNUSED* oh, H5O_mesg_t* mesg, unsigned H5_ATTR_UNUSED sequence, void* _udata);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__attr_to_dense_cb(H5O_t* oh, H5O_mesg_t* mesg /*in,out*/, unsigned H5_ATTR_UNUSED sequence, void* _udata /*in,out*/)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__attr_open_cb(H5O_t* oh, H5O_mesg_t* mesg /*in,out*/, unsigned sequence, void* _udata /*in,out*/)
{
...
static htri_t H5O__attr_find_opened_attr(const H5O_loc_t* loc, H5A_t** attr, const char* name_to_open)
{
    ...
    if (num_open_attr) {
        ...
        for (u = 0; u < num_open_attr; u++) {
             ...
             */
            if (!strcmp(name_to_open, (*attr)->shared->name) && loc->addr == (*attr)->oloc.addr && loc_fnum == attr_fnum) {
                ret_value = true;
                break;
            } /* end if */
        } /* end for */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__attr_write_cb(H5O_t* oh, H5O_mesg_t* mesg /*in,out*/, unsigned H5_ATTR_UNUSED sequence, void* _udata /*in,out*/)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__attr_rename_chk_cb(H5O_t H5_ATTR_UNUSED* oh, H5O_mesg_t* mesg /*in,out*/, unsigned H5_ATTR_UNUSED sequence, void* _udata /*in,out*/)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__attr_rename_mod_cb(H5O_t* oh, H5O_mesg_t* mesg /*in,out*/, unsigned H5_ATTR_UNUSED sequence, void* _udata /*in,out*/)
{
    H5O_iter_ren_t* udata = (H5O_iter_ren_t*)_udata; /* Operator user data */
    H5O_chunk_proxy_t* chk_proxy = NULL;             /* Chunk that message is in */
    bool chk_dirtied = false;                        /* Flag for unprotecting chunk */
    ...
    if (strcmp(((H5A_t*)mesg->native)->shared->name, udata->old_name) == 0) {
        ...
        if (mesg->flags & H5O_MSG_FLAG_SHARED) {
            ...
            if (strlen(udata->new_name) != strlen(udata->old_name) || old_version != ((H5A_t*)mesg->native)->shared->version) {
 ...
 */
herr_t
    H5O__attr_iterate(hid_t loc_id, H5_index_t idx_type, H5_iter_order_t order, hsize_t skip, hsize_t* last_attr, const H5A_attr_iter_op_t* attr_op, void* op_data)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__attr_remove_cb(H5O_t* oh, H5O_mesg_t* mesg /*in,out*/, unsigned H5_ATTR_UNUSED sequence, void* _udata /*in,out*/)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__attr_exists_cb(H5O_t H5_ATTR_UNUSED* oh, H5O_mesg_t* mesg /*in,out*/, unsigned H5_ATTR_UNUSED sequence, void* _udata /*in,out*/)
{
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O__attr_exists(const H5O_loc_t* loc, const char* name, bool* attr_exists)
{
    H5O_t* oh = NULL;           /* Pointer to actual object header */
    H5O_ainfo_t ainfo;          /* Attribute information for object */
    herr_t ret_value = SUCCEED; /* Return value */
