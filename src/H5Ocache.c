...
static herr_t H5O__cache_get_final_load_size(const void* image_ptr, size_t image_len, void* udata, size_t* actual_len);
...
/* Chunk routines */
static herr_t H5O__chunk_deserialize(H5O_t* oh, haddr_t addr, size_t chunk_size, const uint8_t* image, size_t len, H5O_common_cache_ud_t* udata, bool* dirty);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__cache_get_initial_load_size(void H5_ATTR_UNUSED* _udata, size_t* image_len)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__cache_get_final_load_size(const void* image, size_t image_len, void* _udata, size_t* actual_len)
{
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5O__cache_verify_chksum(const void* _image, size_t len, void* _udata)
{
 ...
 *-------------------------------------------------------------------------
 */
static void* H5O__cache_deserialize(const void* image, size_t len, void* _udata, bool* dirty)
{
    ...
    if (H5O__chunk_deserialize(oh, udata->common.addr, udata->chunk0_size, (const uint8_t*)image, len, &(udata->common), dirty) < 0) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__cache_image_len(const void* _thing, size_t* image_len)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__cache_serialize(const H5F_t* f, void* image, size_t len, void* _thing)
{
    ...
    if (oh->version > H5O_VERSION_1) {
        ...
        *chunk_image++ = oh->flags;
        ...
        case 3: /* 8 byte size */ UINT64ENCODE(chunk_image, chunk0_size); break;

        default: HGOTO_ERROR(H5E_OHDR, H5E_BADVALUE, FAIL, "bad size for chunk 0");
        }
    }
    else {
        /* Version */
    ...
    assert((size_t)(chunk_image - oh->chunk[0].image) == (size_t)(H5O_SIZEOF_HDR(oh) - H5O_SIZEOF_CHKSUM_OH(oh)));
...
static herr_t H5O__cache_notify(H5AC_notify_action_t action, void* _thing)
{
    ...
    default: HGOTO_ERROR(H5E_OHDR, H5E_BADVALUE, FAIL, "unknown action from metadata cache");
    }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__cache_free_icr(void* _thing)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__cache_chk_get_initial_load_size(void* _udata, size_t* image_len)
{
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5O__cache_chk_verify_chksum(const void* _image, size_t len, void* _udata)
{
    const uint8_t* image = (const uint8_t*)_image;           /* Pointer into raw data buffer */
 ...
 *-------------------------------------------------------------------------
 */
static void* H5O__cache_chk_deserialize(const void* image, size_t len, void* _udata, bool* dirty)
{
    ...
    if (udata->decoding) {
        ...
        /* Parse the chunk */
        if (H5O__chunk_deserialize(udata->oh, udata->common.addr, udata->size, (const uint8_t*)image, len, &(udata->common), dirty) < 0) {
         ...
         */
        assert(0 == memcmp(image, udata->oh->chunk[chk_proxy->chunkno].image, udata->oh->chunk[chk_proxy->chunkno].size));
    }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__cache_chk_image_len(const void* _thing, size_t* image_len)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__cache_chk_serialize(const H5F_t* f, void* image, size_t len, void* _thing)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__cache_chk_free_icr(void* _thing)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__prefix_deserialize(const uint8_t* _image, size_t len, H5O_cache_ud_t* udata)
{
    const uint8_t* image = (const uint8_t*)_image; /* Pointer into raw data buffer */
    ...
    if (!memcmp(image, H5O_HDR_MAGIC, (size_t)H5_SIZEOF_MAGIC)) {
        ...
        UINT32DECODE(image, udata->chunk0_size);
        if ((udata->v1_pfx_nmesgs > 0 && udata->chunk0_size < H5O_SIZEOF_MSGHDR_OH(oh)) || (udata->v1_pfx_nmesgs == 0 && udata->chunk0_size > 0)) {
            HGOTO_ERROR(H5E_OHDR, H5E_BADVALUE, FAIL, "bad object header chunk size");
        }
...
static herr_t H5O__chunk_deserialize(H5O_t* oh, haddr_t addr, size_t chunk_size, const uint8_t* image, size_t len, H5O_common_cache_ud_t* udata, bool* dirty)
{
    ...
    bool mesgs_modified = false;       /* Whether any messages were modified when the object header was deserialized */
    herr_t ret_value = SUCCEED;
    ...
    while (chunk_image < eom_ptr) {
        ...
        if ((udata->file_intent & H5F_ACC_RDWR) && H5O_NULL_ID == id && oh->nmesgs > 0 && H5O_NULL_ID == oh->mesg[oh->nmesgs - 1].type->id &&
            ...
            if (id >= H5O_UNKNOWN_ID ||
                ...
                *unknown = id;
                ...
                if (((udata->file_intent & H5F_ACC_RDWR) && (flags & H5O_MSG_FLAG_FAIL_IF_UNKNOWN_AND_OPEN_FOR_WRITE)) ||
                ...
                else if ((flags & H5O_MSG_FLAG_MARK_IF_UNKNOWN) && !(flags & H5O_MSG_FLAG_WAS_UNKNOWN) && (udata->file_intent & H5F_ACC_RDWR)) {
                 ...
                 */
                if (((flags & H5O_MSG_FLAG_SHARED) || (flags & H5O_MSG_FLAG_SHAREABLE)) && H5O_msg_class_g[id] &&
            ...
            if (H5O_CONT_ID == id) {
                ...
                if (NULL == (cont = (H5O_cont_t*)(H5O_MSG_CONT->decode)(udata->f, NULL, 0, &ioflags, mesg->raw_size, mesg->raw))) {
            ...
            /* Check if message is an old mtime message */
                ...
                mtime = (time_t*)(H5O_MSG_MTIME->decode)(udata->f, NULL, 0, &ioflags, mesg->raw_size, mesg->raw);
            ...
            /* Check if message is an new mtime message */
                ...
                /* Decode mtime message */
                mtime = (time_t*)(H5O_MSG_MTIME_NEW->decode)(udata->f, NULL, 0, &ioflags, mesg->raw_size, mesg->raw);
...
static herr_t H5O__chunk_serialize(const H5F_t* f, H5O_t* oh, unsigned chunkno)
{
    ...
    if (oh->version > H5O_VERSION_1) {
        ...
        assert(!memcmp(oh->chunk[chunkno].image, (chunkno == 0 ? H5O_HDR_MAGIC : H5O_CHK_MAGIC), H5_SIZEOF_MAGIC));
    }
    else {
    ...
    if (oh->version > H5O_VERSION_1) {
        ...
        if (oh->chunk[chunkno].gap) {
            memset((oh->chunk[chunkno].image + oh->chunk[chunkno].size) - (H5O_SIZEOF_CHKSUM + oh->chunk[chunkno].gap), 0, oh->chunk[chunkno].gap);
        }
