...
static herr_t H5O__copy_header(const H5O_loc_t* oloc_src, H5O_loc_t* oloc_dst /*out*/, hid_t ocpypl_id, hid_t lcpl_id);
static herr_t H5O__copy_obj(H5G_loc_t* src_loc, H5G_loc_t* dst_loc, const char* dst_name, hid_t ocpypl_id, hid_t lcpl_id);
...
static herr_t H5O__copy_search_comm_dt_cb(hid_t group, const char* name, const H5O_loc_t* obj_oloc, void* udata);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O__copy(const H5G_loc_t* loc, const char* src_name, H5G_loc_t* dst_loc, const char* dst_name, hid_t ocpypl_id, hid_t lcpl_id)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__copy_header_real(const H5O_loc_t* oloc_src, H5O_loc_t* oloc_dst /*out*/, H5O_copy_t* cpy_info, H5O_type_t* obj_type, void** udata /*out*/)
{
    ...
    for (mesgno = 0; mesgno < oh_dst->nmesgs; mesgno++) {
        ...
        if (copy_type->copy_file) {
            ...
            if (NULL == (mesg_dst->native =
                             H5O__msg_copy_file(copy_type, oloc_src->file, mesg_src->native, oloc_dst->file, &recompute_size, &mesg_flags, cpy_info, cpy_udata))) {
             ...
             */
            if (recompute_size) {
                mesg_dst->raw_size = H5O_ALIGN_OH(oh_dst, H5O_msg_raw_size(oloc_dst->file, mesg_dst->type->id, false, mesg_dst->native));
            }
    ...
    assert(current_pos + dst_oh_gap + dst_oh_null + H5O_SIZEOF_CHKSUM_OH(oh_dst) == (size_t)dst_oh_size + oh_dst->chunk[0].image);
    ...
    for (mesgno = 0; mesgno < orig_dst_msgs; mesgno++) {
        ...
        if (copy_type->post_copy_file && mesg_src->native) {
            ...
            if ((copy_type->post_copy_file)(oloc_src, mesg_src->native, oloc_dst, mesg_dst->native, &mesg_flags, cpy_info) < 0) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O_copy_header_map(const H5O_loc_t* oloc_src, H5O_loc_t* oloc_dst /*out*/, H5O_copy_t* cpy_info, bool inc_depth, H5O_type_t* obj_type, void** udata /*out*/)
{
    H5O_addr_map_t* addr_map = NULL; /* Address mapping of object copied */
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
static herr_t H5O__copy_free_addrmap_cb(void* _item, void H5_ATTR_UNUSED* key, void H5_ATTR_UNUSED* op_data)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__copy_free_comm_dt_cb(void* item, void* _key, void H5_ATTR_UNUSED* _op_data)
{
 ...
 *-------------------------------------------------------------------------
 */
static int H5O__copy_comm_dt_cmp(const void* _key1, const void* _key2)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__copy_search_comm_dt_attr_cb(const H5A_t* attr, void* _udata)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__copy_search_comm_dt_check(const H5O_loc_t* obj_oloc, H5O_copy_search_comm_dt_ud_t* udata)
{
    H5O_copy_search_comm_dt_key_t* key = NULL; /* Skiplist key */
    haddr_t* addr = NULL;                      /* Destination address */
    bool obj_inserted = false;                 /* Object inserted into skip list */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__copy_search_comm_dt_cb(hid_t H5_ATTR_UNUSED group, const char H5_ATTR_UNUSED* name, const H5O_loc_t* obj_oloc, void* _udata)
{
...
static htri_t H5O__copy_search_comm_dt(H5F_t* file_src, H5O_t* oh_src, H5O_loc_t* oloc_dst /*in, out*/, H5O_copy_t* cpy_info)
{
    ...
    if (!cpy_info->dst_dt_list_complete) {
        ...
        if (cpy_info->dst_dt_suggestion_list && NULL != (dst_addr = (haddr_t*)H5SL_search(cpy_info->dst_dt_list, key))) {
            ...
            if (search_cb_ret == H5O_MCDT_SEARCH_CONT) {
                 ...
                 * list */
                if (H5G_visit(&dst_root_loc, "/", H5_INDEX_NAME, H5_ITER_NATIVE, NULL, H5O__copy_search_comm_dt_cb, &udata) < 0) {
