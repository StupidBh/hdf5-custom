...
static herr_t H5O__copy_obj_by_ref(H5O_loc_t* src_oloc, H5O_loc_t* dst_oloc, H5G_loc_t* dst_root_loc, H5O_copy_t* cpy_info);
...
static herr_t H5O__copy_expand_ref_region1(H5O_loc_t* src_oloc,
                                           ...
                                           size_t ref_count,
                                           H5O_copy_t* cpy_info);
static herr_t H5O__copy_expand_ref_object2(H5O_loc_t* src_oloc,
                                           const H5T_t* dt_src,
                                           const void* buf_src,
                                           size_t nbytes_src,
                                           H5O_loc_t* dst_oloc,
...
static herr_t H5O__copy_obj_by_ref(H5O_loc_t* src_oloc, H5O_loc_t* dst_oloc, H5G_loc_t* dst_root_loc, H5O_copy_t* cpy_info)
{
    ...
    if (H5_addr_defined(dst_oloc->addr) && (ret_value > SUCCEED)) {
        ...
        snprintf(tmp_obj_name, sizeof(tmp_obj_name), "~obj_pointed_by_%llu", (unsigned long long)dst_oloc->addr);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O_copy_expand_ref(H5F_t* file_src, const H5T_t* dt_src, void* buf_src, size_t nbytes_src, H5F_t* file_dst, void* buf_dst, H5O_copy_t* cpy_info)
{
    ...
    case H5R_ATTR:
    case H5R_OBJECT2:
        if (H5O__copy_expand_ref_object2(&src_oloc, dt_src, buf_src, nbytes_src, &dst_oloc, &dst_root_loc, buf_dst, ref_count, cpy_info) < 0) {
            HGOTO_ERROR(H5E_OHDR, H5E_BADVALUE, FAIL, "unable to expand reference");
        }
        break;
    case H5R_BADTYPE:
    case H5R_MAXTYPE:
    default         : HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL, "invalid reference type"); break;
    } /* end switch */
