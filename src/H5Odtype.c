...
static void* H5O__dtype_decode(H5F_t* f, H5O_t* open_oh, unsigned mesg_flags, unsigned* ioflags, size_t p_size, const uint8_t* p);
...
static herr_t H5O__dtype_pre_copy_file(H5F_t* file_src, const void* mesg_src, bool* deleted, const H5O_copy_t* cpy_info, void* _udata);
static void* H5O__dtype_copy_file(H5F_t* file_src,
                                  ...
                                  H5O_copy_t* cpy_info,
                                  void* udata);
static herr_t H5O__dtype_shared_post_copy_upd(const H5O_loc_t* src_oloc, const void* mesg_src, H5O_loc_t* dst_oloc, void* mesg_dst, H5O_copy_t* cpy_info);
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5O__dtype_decode_helper(unsigned* ioflags /*in,out*/, const uint8_t** pp, H5T_t* dt, bool skip, const uint8_t* p_end)
{
    unsigned flags;
    unsigned version;
    ...
    case H5T_FLOAT:
        ...
        switch ((flags >> 4) & 0x03) {
        case 0: dt->shared->u.atomic.u.f.norm = H5T_NORM_NONE; break;

        case 1: dt->shared->u.atomic.u.f.norm = H5T_NORM_MSBSET; break;

        case 2: dt->shared->u.atomic.u.f.norm = H5T_NORM_IMPLIED; break;

        default: HGOTO_ERROR(H5E_DATATYPE, H5E_UNSUPPORTED, FAIL, "unknown floating-point normalization");
        }
        ...
        if (H5_RANGE_OVERLAP(dt->shared->u.atomic.u.f.sign,
                             dt->shared->u.atomic.u.f.sign,
            ...
            for (dt->shared->u.compnd.nmembs = 0; dt->shared->u.compnd.nmembs < nmembs; dt->shared->u.compnd.nmembs++) {
                ...
                if (dt->shared->u.compnd.nmembs > 0 && dt->shared->u.compnd.memb[dt->shared->u.compnd.nmembs].offset < max_memb_pos) {
                    for (unsigned u = 0; u < dt->shared->u.compnd.nmembs; u++) {
                        if ((dt->shared->u.compnd.memb[dt->shared->u.compnd.nmembs].offset >= dt->shared->u.compnd.memb[u].offset &&
                             dt->shared->u.compnd.memb[dt->shared->u.compnd.nmembs].offset <
                                 (dt->shared->u.compnd.memb[u].offset + dt->shared->u.compnd.memb[u].size)) ||
                            (dt->shared->u.compnd.memb[dt->shared->u.compnd.nmembs].offset < dt->shared->u.compnd.memb[u].offset &&
                ...
                max_memb_pos =
                    MAX(max_memb_pos, (dt->shared->u.compnd.memb[dt->shared->u.compnd.nmembs].offset + dt->shared->u.compnd.memb[dt->shared->u.compnd.nmembs].size));
            }
    ...
    case H5T_REFERENCE:
        ...
        if (dt->shared->u.atomic.u.r.rtype <= H5R_BADTYPE || dt->shared->u.atomic.u.r.rtype >= H5R_MAXTYPE) {
            HGOTO_ERROR(H5E_DATATYPE, H5E_CANTDECODE, FAIL, "invalid reference type");
        }

        /* Set generic flag */
        if (dt->shared->u.atomic.u.r.rtype == H5R_OBJECT2 || dt->shared->u.atomic.u.r.rtype == H5R_DATASET_REGION2 || dt->shared->u.atomic.u.r.rtype == H5R_ATTR) {
    ...
    case H5T_ENUM:
        {
            ...
            if (NULL == (dt->shared->u.enumer.name = (char**)H5MM_calloc(nmembs * sizeof(char*))) ||
                NULL == (dt->shared->u.enumer.value = (uint8_t*)H5MM_calloc(nmembs * dt->shared->parent->shared->size))) {
    ...
    case H5T_ARRAY:
        {
            ...
            for (unsigned u = 0; u < dt->shared->u.array.ndims; u++) {
                ...
                if (dt->shared->u.array.dim[u] != 0 && dt->shared->u.array.nelem > SIZE_MAX / dt->shared->u.array.dim[u]) {
            ...
            if (dt->shared->parent->shared->size > 0 && dt->shared->u.array.nelem > SIZE_MAX / dt->shared->parent->shared->size) {
                HGOTO_ERROR(H5E_DATATYPE, H5E_BADVALUE, FAIL, "array datatype size calculation would overflow");
            }
    ...
    case H5T_COMPLEX:
        {
            ...
            if (!homogeneous) {
                HGOTO_ERROR(H5E_DATATYPE, H5E_UNSUPPORTED, FAIL, "heterogeneous complex number datatypes are currently unsupported");
            }
            ...
            if (dt->shared->u.cplx.form != H5T_COMPLEX_RECTANGULAR) {
                HGOTO_ERROR(H5E_DATATYPE, H5E_UNSUPPORTED, FAIL, "only complex number datatypes in rectangular form are currently supported");
            }

            /* Other bits of the flags beyond bits 0,1,2 should not be set */
            if ((flags >> 3) != 0) {
                HGOTO_ERROR(H5E_DATATYPE, H5E_BADVALUE, FAIL, "invalid flag bits set for complex number datatype");
            }

            /* Decode the base datatype of the complex number */
            if (NULL == (dt->shared->parent = H5T__alloc())) {
                HGOTO_ERROR(H5E_DATATYPE, H5E_NOSPACE, FAIL, "unable to allocate complex number base datatype");
            }
            if (H5O__dtype_decode_helper(ioflags, pp, dt->shared->parent, skip, p_end) < 0) {
                HGOTO_ERROR(H5E_DATATYPE, H5E_CANTDECODE, FAIL, "unable to decode complex number base datatype");
            }

            /* Check if the parent of this complex number type has a version greater
             * than the type itself.
             */
            H5O_DTYPE_CHECK_VERSION(dt, version, dt->shared->parent->shared->version, ioflags, "complex", FAIL)
    ...
    case H5T_NO_CLASS:
    case H5T_NCLASSES:
    default          : HGOTO_ERROR(H5E_DATATYPE, H5E_UNSUPPORTED, FAIL, "unknown datatype class found");
    }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__dtype_encode_helper(uint8_t** pp, const H5T_t* dt)
{
    unsigned flags = 0;
    ...
    *pp += 4;
    ...
    case H5T_INTEGER:
        ...
        case H5T_ORDER_LE: break; /*nothing */

        case H5T_ORDER_BE: flags |= 0x01; break;

        case H5T_ORDER_ERROR:
        case H5T_ORDER_VAX:
        case H5T_ORDER_MIXED:
        case H5T_ORDER_NONE:
        default             : HGOTO_ERROR(H5E_DATATYPE, H5E_UNSUPPORTED, FAIL, "byte order is not supported in file format yet");
        } /* end switch */

        switch (dt->shared->u.atomic.lsb_pad) {
        case H5T_PAD_ZERO: break; /*nothing */

        case H5T_PAD_ONE: flags |= 0x02; break;

        case H5T_PAD_ERROR:
        case H5T_PAD_BACKGROUND:
        case H5T_NPAD:
        default                : HGOTO_ERROR(H5E_DATATYPE, H5E_UNSUPPORTED, FAIL, "bit padding is not supported in file format yet");
        } /* end switch */
        ...
        case H5T_PAD_BACKGROUND:
        case H5T_NPAD:
        case H5T_PAD_ONE       : flags |= 0x04; break;

        default: HGOTO_ERROR(H5E_DATATYPE, H5E_UNSUPPORTED, FAIL, "bit padding is not supported in file format yet");
        } /* end switch */

        switch (dt->shared->u.atomic.u.i.sign) {
        case H5T_SGN_NONE: break; /*nothing */

        case H5T_SGN_2: flags |= 0x08; break;

        case H5T_SGN_ERROR:
        case H5T_NSGN:
        default           : HGOTO_ERROR(H5E_DATATYPE, H5E_UNSUPPORTED, FAIL, "sign scheme is not supported in file format yet");
        } /* end switch */
    ...
    case H5T_FLOAT:
        ...
        case H5T_PAD_ONE: flags |= 0x08; break;
        ...
        case H5T_NORM_NONE: break; /*nothing */

        case H5T_NORM_MSBSET: flags |= 0x10; break;

        case H5T_NORM_IMPLIED: flags |= 0x20; break;

        case H5T_NORM_ERROR:
        default            : HGOTO_ERROR(H5E_DATATYPE, H5E_UNSUPPORTED, FAIL, "normalization scheme is not supported in file format yet");
        } /* end switch */
    ...
    case H5T_COMPOUND:
        {
            ...
            for (i = 0; i < dt->shared->u.compnd.nmembs; i++) {
                ...
                assert(dt->shared->u.compnd.memb[i].type->shared->type != H5T_ARRAY || dt->shared->version >= H5O_DTYPE_VERSION_2);
    ...
    case H5T_COMPLEX:
        ...
        if (dt->shared->u.cplx.form != H5T_COMPLEX_RECTANGULAR) {
            HGOTO_ERROR(H5E_DATATYPE, H5E_CANTENCODE, FAIL, "complex number datatypes not in rectangular form are currently unsupported");
        }
        ...
        if (H5O__dtype_encode_helper(pp, dt->shared->parent) < 0) {
            HGOTO_ERROR(H5E_DATATYPE, H5E_CANTENCODE, FAIL, "unable to encode complex number base datatype");
        }
...
--------------------------------------------------------------------------*/
static void* H5O__dtype_decode(H5F_t* f, H5O_t* open_oh, unsigned H5_ATTR_UNUSED mesg_flags, unsigned* ioflags /*in,out*/, size_t p_size, const uint8_t* p)
{
    bool skip;
    H5T_t* dt = NULL;
    ...
    if (open_oh) {
        if (H5O_SIZEOF_CHKSUM_OH(open_oh) > 0 || (f && (H5F_RFIC_FLAGS(f) & H5F_RFIC_UNUSUAL_NUM_UNUSED_NUMERIC_BITS))) {
...
--------------------------------------------------------------------------*/
static herr_t H5O__dtype_encode(H5F_t H5_ATTR_UNUSED* f, uint8_t* p, const void* mesg)
{
...
--------------------------------------------------------------------------*/
static void* H5O__dtype_copy(const void* _src, void* _dst)
{
...
static size_t H5O__dtype_size(const H5F_t* f, const void* _mesg)
{
    ...
    switch (dt->shared->type) {
    case H5T_INTEGER: ret_value += 4; break;

    case H5T_FLOAT: ret_value += 12; break;

    case H5T_TIME: ret_value += 2; break;

    case H5T_BITFIELD: ret_value += 4; break;

    case H5T_OPAQUE: ret_value += (strlen(dt->shared->u.opaque.tag) + 7) & (H5T_OPAQUE_TAG_MAX - 8); break;
    ...
    case H5T_VLEN: ret_value += H5O__dtype_size(f, dt->shared->parent); break;
    ...
    case H5T_COMPLEX: ret_value += H5O__dtype_size(f, dt->shared->parent); break;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__dtype_free(void* mesg)
{
    herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__dtype_set_share(void* _mesg /*in,out*/, const H5O_shared_t* sh)
{
    H5T_t* dt = (H5T_t*)_mesg;
    ...
    assert(sh->type == H5O_SHARE_TYPE_COMMITTED || (dt->shared->state != H5T_STATE_OPEN && dt->shared->state != H5T_STATE_NAMED));
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__dtype_pre_copy_file(H5F_t* file_src, const void* mesg_src, bool H5_ATTR_UNUSED* deleted, const H5O_copy_t* cpy_info, void* _udata)
{
 ...
 *-------------------------------------------------------------------------
 */
static void* H5O__dtype_copy_file(H5F_t H5_ATTR_UNUSED* file_src,
                                  const H5O_msg_class_t* mesg_type,
                                  void* native_src,
                                  H5F_t* file_dst,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__dtype_shared_post_copy_upd(const H5O_loc_t H5_ATTR_UNUSED* src_oloc,
                                              const void H5_ATTR_UNUSED* mesg_src,
                                              H5O_loc_t H5_ATTR_UNUSED* dst_oloc,
                                              void* mesg_dst,
                                              H5O_copy_t H5_ATTR_UNUSED* cpy_info)
{
...
/*--------------------------------------------------------------------------
 ...
 DESCRIPTION
    ...
    parameter.
--------------------------------------------------------------------------*/
static herr_t H5O__dtype_debug(H5F_t* f, const void* mesg, FILE* stream, int indent, int fwidth)
{
    const H5T_t* dt = (const H5T_t*)mesg;
    const char* s;
    char buf[256];
    ...
    switch (dt->shared->type) {
    case H5T_INTEGER: s = "integer"; break;

    case H5T_FLOAT: s = "floating-point"; break;

    case H5T_TIME: s = "date and time"; break;

    case H5T_STRING: s = "text string"; break;

    case H5T_BITFIELD: s = "bit field"; break;
    ...
    case H5T_COMPLEX: s = "complex number"; break;
    ...
    fprintf(stream, "%*s%-*s %lu byte%s\n", indent, "", fwidth, "Size:", (unsigned long)(dt->shared->size), 1 == dt->shared->size ? "" : "s");

    fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth, "Version:", dt->shared->version);

    if (H5T_COMPOUND == dt->shared->type) {
        fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth, "Number of members:", dt->shared->u.compnd.nmembs);
        for (i = 0; i < dt->shared->u.compnd.nmembs; i++) {
            snprintf(buf, sizeof(buf), "Member %u:", i);
            fprintf(stream, "%*s%-*s %s\n", indent, "", fwidth, buf, dt->shared->u.compnd.memb[i].name);
            fprintf(stream, "%*s%-*s %lu\n", indent + 3, "", MAX(0, fwidth - 3), "Byte offset:", (unsigned long)(dt->shared->u.compnd.memb[i].offset));
        ...
        fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth, "Number of members:", dt->shared->u.enumer.nmembs);
        for (i = 0; i < dt->shared->u.enumer.nmembs; i++) {
            snprintf(buf, sizeof(buf), "Member %u:", i);
            ...
            for (k = 0; k < dt->shared->parent->shared->size; k++) {
                fprintf(stream, "%02x", (unsigned)*((uint8_t*)dt->shared->u.enumer.value + (i * dt->shared->parent->shared->size) + k));
            }
            fprintf(stream, "\n");
        } /* end for */
    } /* end else if */
        ...
        case H5T_CSET_ASCII: s = "ASCII"; break;

        case H5T_CSET_UTF8: s = "UTF-8"; break;
        ...
        case H5T_STR_NULLTERM: s = "NULL Terminated"; break;

        case H5T_STR_NULLPAD: s = "NULL Padded"; break;

        case H5T_STR_SPACEPAD: s = "Space Padded"; break;
        ...
        case H5T_VLEN_SEQUENCE: s = "sequence"; break;

        case H5T_VLEN_STRING: s = "string"; break;
        ...
        case H5T_LOC_MEMORY: s = "memory"; break;

        case H5T_LOC_DISK: s = "disk"; break;
        ...
        case H5T_COMPLEX_RECTANGULAR: fprintf(stream, "%*s%-*s %s\n", indent, "", fwidth, "Form:", "rectangular"); break;
        case H5T_COMPLEX_POLAR      : fprintf(stream, "%*s%-*s %s\n", indent, "", fwidth, "Form:", "polar"); break;
        case H5T_COMPLEX_EXPONENTIAL: fprintf(stream, "%*s%-*s %s\n", indent, "", fwidth, "Form:", "exponential"); break;
        default                     : fprintf(stream, "%*s%-*s %s\n", indent, "", fwidth, "Form:", "invalid"); break;
        }
        ...
        switch (dt->shared->u.atomic.order) {
        case H5T_ORDER_LE: s = "little endian"; break;

        case H5T_ORDER_BE: s = "big endian"; break;

        case H5T_ORDER_VAX: s = "VAX"; break;

        case H5T_ORDER_NONE: s = "none"; break;

        case H5T_ORDER_MIXED: s = "mixed"; break;
        ...
        switch (dt->shared->u.atomic.lsb_pad) {
        case H5T_PAD_ZERO: s = "zero"; break;

        case H5T_PAD_ONE: s = "one"; break;

        case H5T_PAD_BACKGROUND: s = "background"; break;
        ...
        if (H5T_FLOAT == dt->shared->type) {
            ...
            switch (dt->shared->u.atomic.u.f.norm) {
            case H5T_NORM_IMPLIED: s = "implied"; break;

            case H5T_NORM_MSBSET: s = "msb set"; break;

            case H5T_NORM_NONE: s = "none"; break;

            case H5T_NORM_ERROR:
            default            : snprintf(buf, sizeof(buf), "H5T_NORM_%d", (int)(dt->shared->u.atomic.u.f.norm)); s = buf;
            } /* end switch */
            fprintf(stream, "%*s%-*s %s\n", indent, "", fwidth, "Normalization:", s);

            fprintf(stream, "%*s%-*s %lu\n", indent, "", fwidth, "Sign bit location:", (unsigned long)(dt->shared->u.atomic.u.f.sign));

            fprintf(stream, "%*s%-*s %lu\n", indent, "", fwidth, "Exponent location:", (unsigned long)(dt->shared->u.atomic.u.f.epos));

            fprintf(stream, "%*s%-*s 0x%08lx\n", indent, "", fwidth, "Exponent bias:", (unsigned long)(dt->shared->u.atomic.u.f.ebias));

            fprintf(stream, "%*s%-*s %lu\n", indent, "", fwidth, "Exponent size:", (unsigned long)(dt->shared->u.atomic.u.f.esize));

            fprintf(stream, "%*s%-*s %lu\n", indent, "", fwidth, "Mantissa location:", (unsigned long)(dt->shared->u.atomic.u.f.mpos));

            fprintf(stream, "%*s%-*s %lu\n", indent, "", fwidth, "Mantissa size:", (unsigned long)(dt->shared->u.atomic.u.f.msize));

        } /* end if */
        else if (H5T_INTEGER == dt->shared->type) {
            switch (dt->shared->u.atomic.u.i.sign) {
            case H5T_SGN_NONE: s = "none"; break;

            case H5T_SGN_2: s = "2's comp"; break;
