...
/* PRIVATE PROTOTYPES */
static void* H5O__efl_decode(H5F_t* f, H5O_t* open_oh, unsigned mesg_flags, unsigned* ioflags, size_t p_size, const uint8_t* p);
static herr_t H5O__efl_encode(H5F_t* f, bool disable_shared, size_t H5_ATTR_UNUSED p_size, uint8_t* p, const void* _mesg);
...
static void* H5O__efl_copy_file(H5F_t* file_src, void* mesg_src, H5F_t* file_dst, bool* recompute_size, unsigned* mesg_flags, H5O_copy_t* cpy_info, void* udata);
 ...
 */
static void*
    H5O__efl_decode(H5F_t* f, H5O_t H5_ATTR_UNUSED* open_oh, unsigned H5_ATTR_UNUSED mesg_flags, unsigned H5_ATTR_UNUSED* ioflags, size_t p_size, const uint8_t* p)
{
    H5O_efl_t* mesg = NULL;
    int version;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__efl_encode(H5F_t* f, bool H5_ATTR_UNUSED disable_shared, size_t H5_ATTR_UNUSED p_size, uint8_t* p, const void* _mesg)
{
 ...
 *-------------------------------------------------------------------------
 */
static size_t H5O__efl_size(const H5F_t* f, bool H5_ATTR_UNUSED disable_shared, const void* _mesg)
{
    const H5O_efl_t* mesg = (const H5O_efl_t*)_mesg;
    size_t ret_value = 0;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O_efl_total_size(const H5O_efl_t* efl, hsize_t* size)
{
 ...
 *-------------------------------------------------------------------------
 */
static void* H5O__efl_copy_file(H5F_t H5_ATTR_UNUSED* file_src,
                                void* mesg_src,
                                H5F_t* file_dst,
                                ...
                                void H5_ATTR_UNUSED* _udata)
{
    ...
    for (idx = 0; idx < efl_src->nused; idx++) {
        ...
        if (H5HL_insert(file_dst, heap, strlen(efl_dst->slot[idx].name) + 1, efl_dst->slot[idx].name, &(efl_dst->slot[idx].name_offset)) < 0) {
            HGOTO_ERROR(H5E_EFL, H5E_CANTINSERT, NULL, "can't insert file name into heap");
        }
    }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__efl_debug(H5F_t H5_ATTR_UNUSED* f, const void* _mesg, FILE* stream, int indent, int fwidth)
{
    const H5O_efl_t* mesg = (const H5O_efl_t*)_mesg;
    size_t u;
    ...
    fprintf(stream, "%*s%-*s %zu/%zu\n", indent, "", fwidth, "Slots used/allocated:", mesg->nused, mesg->nalloc);
    ...
    for (u = 0; u < mesg->nused; u++) {
        ...
        fprintf(stream, "%*s%-*s %zu\n", indent + 3, "", MAX(fwidth - 3, 0), "Name offset:", mesg->slot[u].name_offset);

        fprintf(stream, "%*s%-*s %" PRIdMAX "\n", indent + 3, "", MAX(fwidth - 3, 0), "Offset of data in file:", (intmax_t)(mesg->slot[u].offset));

        fprintf(stream, "%*s%-*s %" PRIuHSIZE "\n", indent + 3, "", MAX(fwidth - 3, 0), "Bytes reserved for data:", (mesg->slot[u].size));
    } /* end for */
