...
/* PRIVATE PROTOTYPES */
static void* H5O__link_decode(H5F_t* f, H5O_t* open_oh, unsigned mesg_flags, unsigned* ioflags, size_t p_size, const uint8_t* p);
static herr_t H5O__link_encode(H5F_t* f, bool disable_shared, size_t H5_ATTR_UNUSED p_size, uint8_t* p, const void* _mesg);
...
static herr_t H5O__link_pre_copy_file(H5F_t* file_src, const void* mesg_src, bool* deleted, const H5O_copy_t* cpy_info, void* udata);
static void* H5O__link_copy_file(H5F_t* file_src, void* native_src, H5F_t* file_dst, bool* recompute_size, unsigned* mesg_flags, H5O_copy_t* cpy_info, void* udata);
static herr_t
    H5O__link_post_copy_file(const H5O_loc_t* src_oloc, const void* mesg_src, H5O_loc_t* dst_oloc, void* mesg_dst, unsigned* mesg_flags, H5O_copy_t* cpy_info);
 ...
 */
static void*
    H5O__link_decode(H5F_t* f, H5O_t H5_ATTR_UNUSED* open_oh, unsigned H5_ATTR_UNUSED mesg_flags, unsigned H5_ATTR_UNUSED* ioflags, size_t p_size, const uint8_t* p)
{
    ...
    default: HGOTO_ERROR(H5E_OHDR, H5E_BADVALUE, NULL, "no appropriate size for name length");
    }
    if (len == 0) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__link_encode(H5F_t* f, bool H5_ATTR_UNUSED disable_shared, size_t H5_ATTR_UNUSED p_size, uint8_t* p, const void* _mesg)
{
    ...
    *p++ = link_flags;
    ...
    switch (link_flags & H5O_LINK_NAME_SIZE) {
    case 0: /* 1 byte size */ *p++ = (uint8_t)len; break;

    case 1: /* 2 byte size */ UINT16ENCODE(p, len); break;

    case 2: /* 4 byte size */ UINT32ENCODE(p, len); break;

    case 3: /* 8 byte size */ UINT64ENCODE(p, len); break;

    default: assert(0 && "bad size for name");
    } /* end switch */
 ...
 *-------------------------------------------------------------------------
 */
static size_t H5O__link_size(const H5F_t* f, bool H5_ATTR_UNUSED disable_shared, const void* _mesg)
{
    const H5O_link_t* lnk = (const H5O_link_t*)_mesg;
    ...
    switch (lnk->type) {
    case H5L_TYPE_HARD: ret_value += H5F_SIZEOF_ADDR(f); break;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__link_pre_copy_file(H5F_t H5_ATTR_UNUSED* file_src,
 ...
 *-------------------------------------------------------------------------
 */
static void* H5O__link_copy_file(H5F_t H5_ATTR_UNUSED* file_src,
                                 void* native_src,
                                 H5F_t H5_ATTR_UNUSED* file_dst,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__link_debug(H5F_t H5_ATTR_UNUSED* f, const void* _mesg, FILE* stream, int indent, int fwidth)
{
    const H5O_link_t* lnk = (const H5O_link_t*)_mesg;
    herr_t ret_value = SUCCEED; /* Return value */
    ...
    fprintf(
        ...
        (lnk->type == H5L_TYPE_HARD
             ? "Hard"
             : (lnk->type == H5L_TYPE_SOFT ? "Soft" : (lnk->type == H5L_TYPE_EXTERNAL ? "External" : (lnk->type >= H5L_TYPE_UD_MIN ? "User-defined" : "Unknown")))));
    ...
    switch (lnk->type) {
    case H5L_TYPE_HARD: fprintf(stream, "%*s%-*s %" PRIuHADDR "\n", indent, "", fwidth, "Object address:", lnk->u.hard.addr); break;

    case H5L_TYPE_SOFT: fprintf(stream, "%*s%-*s '%s'\n", indent, "", fwidth, "Link Value:", lnk->u.soft.name); break;
    ...
    default:
        if (lnk->type >= H5L_TYPE_UD_MIN) {
            if (lnk->type == H5L_TYPE_EXTERNAL) {
                const char* objname = (const char*)lnk->u.ud.udata + (strlen((const char*)lnk->u.ud.udata) + 1);

                fprintf(stream, "%*s%-*s %s\n", indent, "", fwidth, "External File Name:", (const char*)lnk->u.ud.udata);
                fprintf(stream, "%*s%-*s %s\n", indent, "", fwidth, "External Object Name:", objname);
            } /* end if */
            else {
                fprintf(stream, "%*s%-*s %zu\n", indent, "", fwidth, "User-Defined Link Size:", lnk->u.ud.size);
            } /* end else */
        } /* end if */
        else {
