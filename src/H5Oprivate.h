...
#define H5O_MSG_FLAG_BITS                                                                                                                                    \
    (H5O_MSG_FLAG_CONSTANT | H5O_MSG_FLAG_SHARED | H5O_MSG_FLAG_DONTSHARE | H5O_MSG_FLAG_FAIL_IF_UNKNOWN_AND_OPEN_FOR_WRITE | H5O_MSG_FLAG_MARK_IF_UNKNOWN | \
...
#define H5O_LAYOUT_ALL_CHUNK_FLAGS                        (H5O_LAYOUT_CHUNK_DONT_FILTER_PARTIAL_BOUND_CHUNKS | H5O_LAYOUT_CHUNK_SINGLE_INDEX_WITH_FILTER)
...
typedef struct H5O_storage_virtual_t
{
    ...
    H5O_storage_virtual_ent_t* source_file_hash_table; /* Hash table of virtual entries sorted by source file name. Only the first
                                                          occurrence of each source file name is stored. */
    H5O_storage_virtual_ent_t* source_dset_hash_table; /* Hash table of virtual entries sorted by source dataset name. Only the
    ...
    H5O_storage_virtual_ent_t** not_in_tree_list;      /* Array of POINTERS to mappings NOT in tree for quick access
...
typedef herr_t (*H5O_lib_operator_t)(H5O_t* oh, H5O_mesg_t* mesg /*in,out*/, unsigned sequence, void* operator_data /*in,out*/);
...
H5_DLL herr_t H5O_create(H5F_t* f, size_t size_hint, size_t initial_rc, hid_t ocpl_id, H5O_loc_t* loc /*out*/);
H5_DLL H5O_t* H5O_create_ohdr(H5F_t* f, hid_t ocpl_id);
H5_DLL herr_t H5O_apply_ohdr(H5F_t* f, H5O_t* oh, hid_t ocpl_id, size_t size_hint, size_t initial_rc, H5O_loc_t* loc_out);
...
H5_DLL herr_t H5O_msg_encode(H5F_t* f, unsigned type_id, bool disable_shared, unsigned char* buf, const void* obj);
...
H5_DLL herr_t H5O_refresh_metadata_reopen(hid_t oid, hid_t apl_id, H5G_loc_t* obj_loc, H5VL_connector_t* connector, bool start_swmr);

/* Object copying routines */
H5_DLL herr_t H5O_copy_header_map(const H5O_loc_t* oloc_src, H5O_loc_t* oloc_dst /*out */, H5O_copy_t* cpy_info, bool inc_depth, H5O_type_t* obj_type, void** udata);
H5_DLL herr_t H5O_copy_expand_ref(H5F_t* file_src, const H5T_t* dt_src, void* buf_src, size_t nbytes_src, H5F_t* file_dst, void* buf_dst, H5O_copy_t* cpy_info);
