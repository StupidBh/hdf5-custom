...
#define H5PB__INSERT_LRU(page_buf, page_ptr)                                                                    \
    {                                                                                                           \
        ...
        H5PB__PREPEND((page_ptr), (page_buf)->LRU_head_ptr, (page_buf)->LRU_tail_ptr, (page_buf)->LRU_list_len) \
    }
...
#define H5PB__REMOVE_LRU(page_buf, page_ptr)                                                                   \
    {                                                                                                          \
        ...
        H5PB__REMOVE((page_ptr), (page_buf)->LRU_head_ptr, (page_buf)->LRU_tail_ptr, (page_buf)->LRU_list_len) \
    }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5PB__flush_cb(void* item, void H5_ATTR_UNUSED* key, void* _op_data)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5PB__dest_cb(void* item, void H5_ATTR_UNUSED* key, void* _op_data)
{
    H5PB_entry_t* page_entry = (H5PB_entry_t*)item; /* Pointer to page entry node */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5PB_read(H5F_shared_t* f_sh, H5FD_mem_t type, haddr_t addr, size_t size, void* buf /*out*/)
{
    ...
    if (NULL == page_buf || size >= page_buf->page_size || (bypass_pb && H5FD_MEM_DRAW == type)) {
         ...
         */
        if (NULL == page_buf || (size >= page_buf->page_size && H5FD_MEM_DRAW != type) || (bypass_pb && H5FD_MEM_DRAW == type)) {
            HGOTO_DONE(SUCCEED);
        }
    } /* end if */
    ...
    if (H5FD_MEM_DRAW == type) {
        ...
        num_touched_pages = (last_page_addr / page_buf->page_size + 1) - (first_page_addr / page_buf->page_size);
            ...
            if (node) {
                ...
                if (page_entry->is_dirty) {
                    ...
                    if (i == 0 && first_page_addr != addr) {
                        ...
                        H5MM_memcpy(buf, (uint8_t*)page_entry->page_buf_ptr + offset, page_buf->page_size - (size_t)offset);
                    ...
                    /* special handling for the last page if it is not a full page access */
                        ...
                        offset = (num_touched_pages - 2) * page_buf->page_size + (page_buf->page_size - (addr - first_page_addr));

                        H5MM_memcpy((uint8_t*)buf + offset, page_entry->page_buf_ptr, (size_t)((addr + size) - last_page_addr));
                    ...
                    /* copy the entire fully accessed pages */
                        ...
                        H5MM_memcpy((uint8_t*)buf + (i * page_buf->page_size), page_entry->page_buf_ptr, page_buf->page_size);
                    } /* end else */
                } /* end if */
                node = H5SL_next(node);
            } /* end if */
...
herr_t H5PB_write(H5F_shared_t* f_sh, H5FD_mem_t type, haddr_t addr, size_t size, const void* buf)
{
    ...
    if (H5FD_MEM_DRAW == type && size >= page_buf->page_size) {
        ...
        for (i = 0; i < num_touched_pages; i++) {
            ...
            /* Special handling for the last page if it is not a full page update */
            else if (num_touched_pages > 1 && i == (num_touched_pages - 1) && (search_addr + page_buf->page_size) != (addr + size)) {
                ...
                if (page_entry) {
                    ...
                    /* Update page's data */
                    H5MM_memcpy(page_entry->page_buf_ptr, (const uint8_t*)buf + offset, (size_t)((addr + size) - last_page_addr));
        ...
        for (i = 0; i < num_touched_pages; i++) {
            ...
            /* If not found */
                ...
                if (page_entry) {
                    ...
                    memset((uint8_t*)new_page_buf + offset + access_size, 0, page_size - ((size_t)offset + access_size));
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5PB_enabled(H5F_shared_t* f_sh, H5FD_mem_t type, bool* enabled)
{
    H5PB_t* page_buf;           /* Page buffering info for this file */
    bool bypass_pb = false;     /* Whether to bypass page buffering */
    herr_t ret_value = SUCCEED; /* Return value */
...
static htri_t H5PB__make_space(H5F_shared_t* f_sh, H5PB_t* page_buf, H5FD_mem_t inserted_type)
{
    ...
    if (H5FD_MEM_DRAW == inserted_type) {
        ...
        while (1) {
            if (page_entry->prev && H5F_MEM_PAGE_META == page_entry->type && page_buf->min_meta_count >= page_buf->meta_count) {
