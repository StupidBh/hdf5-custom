...
static herr_t H5PL__path_table_iterate_process_path(const char* plugin_path, H5PL_iterate_type_t iter_type, H5PL_iterate_t iter_op, void* op_data);
static herr_t H5PL__find_plugin_in_path(const H5PL_search_params_t* search_params, bool* found, const char* dir, const void** plugin_info);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5PL__path_table_iterate(H5PL_iterate_type_t iter_type, H5PL_iterate_t iter_op, void* op_data)
{
    unsigned int u;
    ...
    for (u = 0; (u < H5PL_num_paths_g) && (ret_value == H5_ITER_CONT); u++) {
        if ((ret_value = H5PL__path_table_iterate_process_path(H5PL_paths_g[u], iter_type, iter_op, op_data)) < 0) {
            HGOTO_ERROR(H5E_PLUGIN, H5E_BADITER, H5_ITER_ERROR, "can't iterate over plugins in plugin path '%s'", H5PL_paths_g[u]);
        }
    }
...
static herr_t H5PL__path_table_iterate_process_path(const char* plugin_path, H5PL_iterate_type_t iter_type, H5PL_iterate_t iter_op, void* op_data)
{
    ...
    while (NULL != (dp = HDreaddir(dirp))) {
        ...
        if (!strncmp(dp->d_name, "cyg", (size_t)3) && strstr(dp->d_name, ".dll")) {
            ...
            len = strlen(plugin_path) + strlen(H5PL_PATH_SEPARATOR) + strlen(dp->d_name) + 1 /*\0*/ + 4; /* Extra "+4" to quiet GCC warning - 2019/07/05, QAK */
            ...
            if (HDstat(path, &my_stat) == -1) {
                HGOTO_ERROR(H5E_FILE, H5E_CANTGET, H5_ITER_ERROR, "can't stat file %s -- error was: %s", path, strerror(errno));
            }
            ...
            plugin_matches = (iter_type == H5PL_ITER_TYPE_ALL) || ((iter_type == H5PL_ITER_TYPE_FILTER) && (plugin_type == H5PL_TYPE_FILTER)) ||
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5PL__find_plugin_in_path_table(const H5PL_search_params_t* search_params, bool* found, const void** plugin_info)
{
...
static herr_t H5PL__find_plugin_in_path(const H5PL_search_params_t* search_params, bool* found, const char* dir, const void** plugin_info)
{
    ...
    *found = false;
    ...
    if (!(dirp = HDopendir(dir))) {
        HGOTO_ERROR(H5E_PLUGIN, H5E_OPENERROR, FAIL, "can't open directory (%s). Please verify its existence", dir);
    }
    ...
    while (NULL != (dp = HDreaddir(dirp))) {
        ...
        if (!strncmp(dp->d_name, "cyg", (size_t)3) && strstr(dp->d_name, ".dll")) {
            ...
            /* Allocate & initialize the path name */
            len = strlen(dir) + strlen(H5PL_PATH_SEPARATOR) + strlen(dp->d_name) + 1 /*\0*/ + 4; /* Extra "+4" to quiet GCC warning - 2019/07/05, QAK */
            ...
            memset(&my_stat, 0, sizeof(h5_stat_t));
            if (HDstat(path, &my_stat) == -1) {
                HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL, "can't stat file %s -- error was: %s", path, strerror(errno));
            }
