 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5PL__read_file_data(int fd, HDoff_t offset, void* buf, size_t size, const char* filename)
{
    ...
    if (HDlseek(fd, offset, SEEK_SET) < 0) {
        HGOTO_ERROR(H5E_PLUGIN, H5E_SEEKERROR, FAIL, "unable to seek to offset %llu in plugin file '%s'", (unsigned long long)offset, filename);
    }
    ...
    while (left_to_read > 0) {
        ...
        if (0 == bytes_read) {
            HGOTO_ERROR(H5E_PLUGIN, H5E_READERROR, FAIL, "unexpected end of file while reading plugin '%s' at offset %llu", filename, (unsigned long long)offset);
        }
 ...
 *-------------------------------------------------------------------------
 */
static const EVP_MD* H5PL__get_hash_algorithm(H5PL_sig_algo_t algorithm_id)
{
    ...
    case H5PL_SIG_ALGO_SHA256_PSS: ret_value = EVP_sha256(); break;

    case H5PL_SIG_ALGO_SHA384:
    case H5PL_SIG_ALGO_SHA384_PSS: ret_value = EVP_sha384(); break;

    case H5PL_SIG_ALGO_SHA512:
    case H5PL_SIG_ALGO_SHA512_PSS: ret_value = EVP_sha512(); break;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5PL__add_key_to_keystore(EVP_PKEY* key, const char* source)
{
    herr_t ret_value = SUCCEED;
    ...
    if (H5PL_keystore_count_g >= H5PL_keystore_capacity_g) {
        size_t new_capacity = H5PL_keystore_capacity_g == 0 ? H5PL_KEYSTORE_INITIAL_CAPACITY : H5PL_keystore_capacity_g * 2;
        H5PL_keystore_entry_t* new_keystore = (H5PL_keystore_entry_t*)H5MM_realloc(H5PL_keystore_g, new_capacity * sizeof(H5PL_keystore_entry_t));
 ...
 *-------------------------------------------------------------------------
 */
static EVP_PKEY* H5PL__create_public_RSA_from_file(const char* file_path)
{
    BIO* bio = NULL;
    EVP_PKEY* pkey = NULL;
...
static herr_t H5PL__process_key_file(const char* file_path)
{
     ...
     */
    #ifndef H5_HAVE_WIN32_API
static herr_t H5PL__load_keys_from_directory(const char* dir_path)
{
    DIR* dir = NULL;
    ...
    while (NULL != (entry = readdir(dir))) {
        ...
        /* Canonicalize and verify path stays within keystore directory (path traversal protection) */
        {
            ...
            /* Verify canonical file path starts with canonical directory path */
            {
                ...
                if (strncmp(canonical_file, canonical_dir, dir_len) != 0 || (canonical_file[dir_len] != '/' && canonical_file[dir_len] != '\0')) {
                    H5PL_SIG_DEBUG_PRINT("WARNING: Path traversal detected - %s resolves outside keystore directory\n", entry->d_name);
                    free(canonical_dir);
                    free(canonical_file);
                    H5MM_xfree(file_path);
                    continue;
...
static herr_t H5PL__load_keys_from_directory(const char* dir_path)
{
...

    {
        ...
        do {
            ...
            if (GetFullPathNameA(dir_path, MAX_PATH, canonical_dir, NULL) == 0 || GetFullPathNameA(file_path, MAX_PATH, canonical_file, NULL) == 0) {
                H5PL_SIG_DEBUG_PRINT("WARNING: Cannot resolve path for %s\n", find_data.cFileName);
                continue;
            }
            {
                size_t dir_len = strlen(canonical_dir);
                if (_strnicmp(canonical_file, canonical_dir, dir_len) != 0 || (canonical_file[dir_len] != '\\' && canonical_file[dir_len] != '\0')) {
                    H5PL_SIG_DEBUG_PRINT("WARNING: Path traversal detected - %s resolves outside keystore directory\n", find_data.cFileName);
                    continue;
                }
            }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5PL__init_keystore(void)
{
    ...
    if (NULL != (env_keystore = getenv("HDF5_PLUGIN_KEYSTORE"))) {
        if (H5PL__load_keys_from_directory(env_keystore) < 0) {
            HGOTO_ERROR(H5E_PLUGIN, H5E_CANTLOAD, FAIL, "failed to load keys from HDF5_PLUGIN_KEYSTORE: %s", env_keystore);
        }
        keys_loaded = true;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5PL__parse_hex_hash(const char* hex_string, unsigned char* hash)
{
    herr_t ret_value = SUCCEED;
...
static herr_t H5PL__load_revoked_signatures(const char* keystore_dir)
{
    ...
    while (fgets(line, sizeof(line), fp) != NULL) {
        ...
        while (line_len > 0 && (trimmed[line_len - 1] == '\n' || trimmed[line_len - 1] == '\r' || trimmed[line_len - 1] == ' ' || trimmed[line_len - 1] == '\t')) {
        ...
        if (line_len != H5PL_SIGNATURE_HASH_HEX_LEN) {
            H5PL_SIG_DEBUG_PRINT("WARNING: Ignoring invalid revoked signature hash (expected %d hex chars): %s\n", H5PL_SIGNATURE_HASH_HEX_LEN, trimmed);
            continue;
        }
        ...
        if (H5PL_revoked_sigs_count_g >= H5PL_revoked_sigs_capacity_g) {
            ...
            H5PL_revoked_signature_t* new_array = (H5PL_revoked_signature_t*)H5MM_realloc(H5PL_revoked_sigs_g, new_capacity * sizeof(H5PL_revoked_signature_t));
    ...
    if (H5PL_revoked_sigs_count_g > 1) {
        qsort(H5PL_revoked_sigs_g, H5PL_revoked_sigs_count_g, sizeof(H5PL_revoked_signature_t), H5PL__compare_signature_hashes);
    }
...
static bool H5PL__is_signature_revoked(const unsigned char* signature, size_t signature_len)
{
     ...
     */
    if (H5PL_revoked_sigs_count_g > 0) {
        if (NULL != bsearch(hash, H5PL_revoked_sigs_g, H5PL_revoked_sigs_count_g, sizeof(H5PL_revoked_signature_t), H5PL__compare_signature_hashes)) {
            HGOTO_DONE(true);
        }
    }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5PL__hash_file_binary(int fd,
                                     HDoff_t binary_size,
                                     ...
                                     const char* plugin_path)
{
    ...
    while (bytes_read < binary_size) {
        size_t chunk_size = (size_t)((binary_size - bytes_read) > (HDoff_t)H5PL_VERIFY_CHUNK_SIZE ? H5PL_VERIFY_CHUNK_SIZE : (size_t)(binary_size - bytes_read));
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5PL__read_and_validate_footer(int fd, HDoff_t file_size, const char* plugin_path, H5PL_sig_footer_t* footer_out, size_t* binary_size_out)
{
    ...
    if (H5PL__read_file_data(fd, file_size - (HDoff_t)H5PL_SIG_FOOTER_SIZE, footer_buf, H5PL_SIG_FOOTER_SIZE, plugin_path) < 0) {
    ...
    if (!H5PL_sig_decode_footer(footer_buf, sizeof(footer_buf), footer_out)) {
        HGOTO_ERROR(H5E_PLUGIN, H5E_BADVALUE, FAIL, "not a signed HDF5 plugin (bad magic or unsupported format version)");
    }

    /* Validate algorithm ID */
    if (NULL == H5PL__get_hash_algorithm(footer_out->algorithm_id)) {
        HGOTO_ERROR(H5E_PLUGIN, H5E_BADVALUE, FAIL, "unsupported or unknown hash algorithm ID 0x%02X in plugin signature", (unsigned)footer_out->algorithm_id);
    }
    ...
    /* Calculate binary data size with overflow protection */
    {
        ...
        uint64_t sig_and_footer_size = (uint64_t)footer_out->signature_length + (uint64_t)H5PL_SIG_FOOTER_SIZE;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5PL__verify_with_all_keys(int fd, size_t binary_size, const unsigned char* signature, const H5PL_sig_footer_t* footer, const char* plugin_path)
{
    ...
    if (NULL == hash_algorithm) {
        HGOTO_ERROR(H5E_PLUGIN, H5E_BADVALUE, FAIL, "cannot get hash algorithm for ID 0x%02X", (unsigned)footer->algorithm_id);
    }

    /* Hash the binary exactly once - shared across all key verification attempts */
    if (H5PL__hash_file_binary(fd, (HDoff_t)binary_size, hash_algorithm, digest, &digest_len, plugin_path) < 0) {
    ...
    for (size_t key_idx = 0; key_idx < H5PL_keystore_count_g; key_idx++) {
        ...
        if (H5PL_SIG_ALGO_IS_PSS(footer->algorithm_id)) {
            if (1 != EVP_PKEY_CTX_set_rsa_padding(pkey_ctx, RSA_PKCS1_PSS_PADDING) || 1 != EVP_PKEY_CTX_set_rsa_pss_saltlen(pkey_ctx, RSA_PSS_SALTLEN_DIGEST)) {
                EVP_PKEY_CTX_free(pkey_ctx);
                ERR_clear_error();
                continue;
            }
        ...
        verify_result = EVP_PKEY_verify(pkey_ctx, signature, footer->signature_length, digest, (size_t)digest_len);
        EVP_PKEY_CTX_free(pkey_ctx);
        ERR_clear_error();
        ...
        if (verify_result == 1) {
            ...
            verified = true;
            H5PL_SIG_DEBUG_PRINT("Plugin '%s' verified with key from: %s\n", plugin_path, H5PL_keystore_g[key_idx].source);
            break;
        }
    }

    if (!verified) {
        HGOTO_ERROR(H5E_PLUGIN, H5E_BADVALUE, FAIL, "plugin signature verification failed: no key in keystore matched");
    }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5PL__verify_signature_appended(const char* plugin_path)
{
    int fd = -1;
    h5_stat_t st;
    HDoff_t file_size = 0;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5PL__cleanup_signature_resources(void)
{
    FUNC_ENTER_PACKAGE_NOERR
