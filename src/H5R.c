...
herr_t H5Rcreate_object(hid_t loc_id, const char* name, hid_t oapl_id, H5R_ref_t* ref_ptr)
{
    ...
    if (H5VL_object_specific(vol_obj, &loc_params, &obj_spec_vol_cb_args, H5P_DATASET_XFER_DEFAULT, H5_REQUEST_NULL) < 0) {
...
static hid_t H5R__open_object_api_common(H5R_ref_t* ref_ptr, hid_t rapl_id, hid_t oapl_id, void** token_ptr, H5VL_object_t** _vol_obj_ptr)
{
    ...
    if (H5R__get_type((const H5R_ref_priv_t*)ref_ptr) <= H5R_BADTYPE || H5R__get_type((const H5R_ref_priv_t*)ref_ptr) >= H5R_MAXTYPE) {
...
static hid_t H5R__open_region_api_common(H5R_ref_t* ref_ptr, hid_t rapl_id, hid_t oapl_id, void** token_ptr, H5VL_object_t** _vol_obj_ptr)
{
    ...
    if ((H5R__get_type((const H5R_ref_priv_t*)ref_ptr) != H5R_DATASET_REGION1) && (H5R__get_type((const H5R_ref_priv_t*)ref_ptr) != H5R_DATASET_REGION2)) {
        HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, H5I_INVALID_HID, "invalid reference type");
    }
    if (rapl_id < 0) {
        HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, H5I_INVALID_HID, "not a property list");
...
static hid_t H5R__open_attr_api_common(H5R_ref_t* ref_ptr, hid_t rapl_id, hid_t aapl_id, void** token_ptr, H5VL_object_t** _vol_obj_ptr)
{
    ...
    if (NULL ==
        (opened_attr =
             H5VL_attr_open(opened_obj, &loc_params, H5R_REF_ATTRNAME((const H5R_ref_priv_t*)ref_ptr), aapl_id, H5P_DATASET_XFER_DEFAULT, H5_REQUEST_NULL))) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Rget_obj_type3(H5R_ref_t* ref_ptr, hid_t rapl_id, H5O_type_t* obj_type /*out*/)
{
    hid_t loc_id;                       /* Reference location ID */
    H5VL_object_t* vol_obj = NULL;      /* Object of loc_id */
 ...
 *-------------------------------------------------------------------------
 */
ssize_t H5Rget_file_name(const H5R_ref_t* ref_ptr, char* buf /*out*/, size_t size)
{
    hid_t loc_id;      /* Reference location ID */
 ...
 *-------------------------------------------------------------------------
 */
ssize_t H5Rget_obj_name(H5R_ref_t* ref_ptr, hid_t rapl_id, char* buf /*out*/, size_t size)
{
    hid_t loc_id;                       /* Reference location ID */
    H5VL_object_t* vol_obj = NULL;      /* Object of loc_id */
    H5VL_object_get_args_t vol_cb_args; /* Arguments to VOL callback */
 ...
 *-------------------------------------------------------------------------
 */
ssize_t H5Rget_attr_name(const H5R_ref_t* ref_ptr, char* buf /*out*/, size_t size)
{
    ssize_t ret_value; /* Return value */
