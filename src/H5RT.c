...
static herr_t H5RT__search_recurse(H5RT_node_t* node, int rank, hsize_t min[], hsize_t max[], H5RT_result_set_t* result_set);
static herr_t H5RT__node_copy(H5RT_node_t* dest_node, const H5RT_node_t* src_node, const H5RT_leaf_t* old_leaves_base, H5RT_leaf_t* new_leaves_base);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5RT_leaf_init(H5RT_leaf_t* leaf, int rank, void* record)
{
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5RT_leaf_cleanup(H5RT_leaf_t* leaf)
{
    herr_t ret_value = SUCCEED;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5RT__compute_slabs(size_t node_capacity, size_t leaf_count, size_t* slab_count_out, size_t* slab_size_out)
{
...
static herr_t H5RT__bulk_load(H5RT_node_t* node, int rank, H5RT_leaf_t* leaves, size_t count, int prev_sort_dim)
{
    ...
    if (count <= H5RT_MAX_NODE_SIZE) {
        ...
        if (prev_sort_dim != rank - 1) {
            ...
            if (H5_UNLIKELY(HDqsort_r((void*)leaves, count, sizeof(H5RT_leaf_t), H5RT__leaf_compare, (void*)&sort_dim) < 0)) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5RT_search(H5RT_t* rtree, hsize_t min[], hsize_t max[], H5RT_result_set_t** results_out)
{
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5RT_free_results(H5RT_result_set_t* result_set)
{
    herr_t ret_value = SUCCEED;
...
static herr_t H5RT__node_copy(H5RT_node_t* dest_node, const H5RT_node_t* src_node, const H5RT_leaf_t* old_leaves_base, H5RT_leaf_t* new_leaves_base)
{
    ...
    if (src_node->children_are_leaves) {
        ...
        for (int i = 0; i < src_node->nchildren; i++) {
            ...
            if (H5RT__node_copy(dest_node->children.nodes[i], src_node->children.nodes[i], old_leaves_base, new_leaves_base) < 0) {
