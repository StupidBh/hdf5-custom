 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Rget_obj_type2(hid_t id, H5R_type_t ref_type, const void* ref, H5O_type_t* obj_type /*out*/)
{
    H5VL_object_t* vol_obj = NULL;                        /* Object of loc_id */
    H5I_type_t vol_obj_type = H5I_BADID;                  /* Object type of loc_id */
    H5VL_object_get_args_t vol_cb_args;                   /* Arguments to VOL callback */
    ...
    /* Check if using native VOL connector */
    if (H5VL_object_is_native(vol_obj, &is_native_vol_obj) < 0) {
        HGOTO_ERROR(H5E_REFERENCE, H5E_CANTGET, FAIL, "can't determine if VOL object is native connector object");
    }

    /* Must use native VOL connector for this operation */
    if (!is_native_vol_obj) {
        HGOTO_ERROR(H5E_REFERENCE, H5E_VOL, FAIL, "H5Rget_obj_type2 is only meant to be used with the native VOL connector");
    }
...
hid_t H5Rget_region(hid_t id, H5R_type_t ref_type, const void* ref)
{
    ...
    if (!is_native_vol_obj) {
        HGOTO_ERROR(H5E_REFERENCE, H5E_VOL, H5I_INVALID_HID, "H5Rget_region is only meant to be used with the native VOL connector");
    }
 ...
 *-------------------------------------------------------------------------
 */
ssize_t H5Rget_name(hid_t id, H5R_type_t ref_type, const void* ref, char* name /*out*/, size_t size)
{
    H5VL_object_t* vol_obj = NULL;                        /* Object of loc_id */
    H5I_type_t vol_obj_type = H5I_BADID;                  /* Object type of loc_id */
    H5VL_object_get_args_t vol_cb_args;                   /* Arguments to VOL callback */
