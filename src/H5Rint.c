...
static const char* H5R__print_token(const H5O_token_t token)
{
...
herr_t H5R__create_object(const H5O_token_t* obj_token, size_t token_size, H5R_ref_priv_t* ref)
{
    ...
    H5R_LOG_DEBUG("Created object reference, %d, filename=%s, obj_addr=%s, encode size=%u",
                  (int)sizeof(H5R_ref_priv_t),
                  ref->info.obj.filename,
                  H5R__print_token(ref->info.obj.token),
                  ref->encode_size);
...
herr_t H5R__create_attr(const H5O_token_t* obj_token, size_t token_size, const char* attr_name, H5R_ref_priv_t* ref)
{
    ...
    if (strlen(attr_name) > H5R_MAX_STRING_LEN) {
        HGOTO_ERROR(H5E_REFERENCE, H5E_ARGS, FAIL, "attribute name too long (%d > %d)", (int)strlen(attr_name), H5R_MAX_STRING_LEN);
    }
...
herr_t H5R__destroy(H5R_ref_priv_t* ref)
{
    ...
    H5R_LOG_DEBUG("Destroying reference, filename=%s, obj_addr=%s, encode size=%u", ref->info.obj.filename, H5R__print_token(ref->info.obj.token), ref->encode_size);

    H5MM_xfree(ref->info.obj.filename);
    ref->info.obj.filename = NULL;

    switch (ref->type) {
    case H5R_OBJECT2: break;
    ...
    case H5R_OBJECT1:
    case H5R_DATASET_REGION1: break;
    case H5R_BADTYPE        :
    case H5R_MAXTYPE        : assert("invalid reference type" && 0); HGOTO_ERROR(H5E_REFERENCE, H5E_UNSUPPORTED, FAIL, "internal error (invalid reference type)");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5R__set_loc_id(H5R_ref_priv_t* ref, hid_t id, bool inc_ref, bool app_ref)
{
    herr_t ret_value = SUCCEED; /* Return value */
...
hid_t H5R__reopen_file(H5R_ref_priv_t* ref, hid_t fapl_id)
{
    ...
    if (NULL == (new_file = H5VL_file_open(connector_prop.connector, H5R_REF_FILENAME(ref), H5F_ACC_RDWR, fapl_id, H5P_DATASET_XFER_DEFAULT, H5_REQUEST_NULL))) {
...
htri_t H5R__equal(const H5R_ref_priv_t* ref1, const H5R_ref_priv_t* ref2)
{
    ...
    /* Compare filenames */
    if ((ref1->info.obj.filename && (NULL == ref2->info.obj.filename)) || ((NULL == ref1->info.obj.filename) && ref2->info.obj.filename)) {
        HGOTO_DONE(false);
    }
    if (ref1->info.obj.filename && ref1->info.obj.filename && (0 != strcmp(ref1->info.obj.filename, ref2->info.obj.filename))) {
        HGOTO_DONE(false);
    }
...
herr_t H5R__copy(const H5R_ref_priv_t* src_ref, H5R_ref_priv_t* dst_ref)
{
    ...
    case H5R_ATTR:
        ...
        break;
    case H5R_OBJECT1:
    case H5R_DATASET_REGION1: assert("invalid reference type" && 0); HGOTO_ERROR(H5E_REFERENCE, H5E_UNSUPPORTED, FAIL, "internal error (invalid reference type)");
    case H5R_BADTYPE        :
    case H5R_MAXTYPE        :
    default                 : assert("unknown reference type" && 0); HGOTO_ERROR(H5E_REFERENCE, H5E_UNSUPPORTED, FAIL, "internal error (unknown reference type)");
    } /* end switch */
...
herr_t H5R__encode(const char* filename, const H5R_ref_priv_t* ref, unsigned char* buf, size_t* nalloc, unsigned flags)
{
    ...
    /* Encode object token */
    H5R_ENCODE_VAR(H5R__encode_obj_token, &ref->info.obj.token, ref->token_size, p, buf_size, encode_size, "Cannot encode object address");
    ...
    case H5R_ATTR:
        ...
        H5R_ENCODE(H5R__encode_string, ref->info.attr.name, p, buf_size, encode_size, "Cannot encode attribute name");
        break;
    ...
    H5R_LOG_DEBUG("Encoded reference, filename=%s, obj_addr=%s, encode size=%u", ref->info.obj.filename, H5R__print_token(ref->info.obj.token), encode_size);
...
herr_t H5R__decode(const unsigned char* buf, size_t* nbytes, H5R_ref_priv_t* ref)
{
    ...
    /* Decode object token */
    H5R_DECODE_VAR(H5R__decode_obj_token, &ref->info.obj.token, &ref->token_size, p, buf_size, decode_size, "Cannot decode object address");

    /* We do not need to store the filename if the reference is internal */
    if (flags & H5R_IS_EXTERNAL) {
        /* Decode file name */
        H5R_DECODE(H5R__decode_string, &ref->info.obj.filename, p, buf_size, decode_size, "Cannot decode filename");
    ...
    case H5R_DATASET_REGION2:
        /* Decode dataspace */
        H5R_DECODE(H5R__decode_region, &ref->info.reg.space, p, buf_size, decode_size, "Cannot decode region");
        decoded_dataspace = true;
        break;

    case H5R_ATTR:
        /* Decode attribute name */
        H5R_DECODE(H5R__decode_string, &ref->info.attr.name, p, buf_size, decode_size, "Cannot decode attribute name");
    ...
    H5R_LOG_DEBUG("Decoded reference, filename=%s, obj_addr=%s, encode size=%u", ref->info.obj.filename, H5R__print_token(ref->info.obj.token), ref->encode_size);
