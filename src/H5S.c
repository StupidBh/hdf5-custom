 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5S__close_cb(void* _space, void H5_ATTR_UNUSED** request)
{
...
H5S_t* H5S_create(H5S_class_t type)
{
    ...
    switch (type) {
    case H5S_SCALAR: new_ds->extent.nelem = 1; break;

    case H5S_SIMPLE:
    case H5S_NULL  : new_ds->extent.nelem = 0; break;

    case H5S_NO_CLASS:
    default          : assert("unknown dataspace (extent) type" && 0); break;
    } /* end switch */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5S__extent_copy_real(H5S_extent_t* dst, const H5S_extent_t* src, bool copy_max)
{
    unsigned u;
    herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
H5S_t* H5S_copy(const H5S_t* src, bool share_selection, bool copy_max)
{
    H5S_t* dst = NULL;
    H5S_t* ret_value = NULL; /* Return value */
...
hsize_t H5S_get_npoints_max(const H5S_t* ds)
{
    ...
    case H5S_NULL: ret_value = 0; break;

    case H5S_SCALAR: ret_value = 1; break;
    ...
    case H5S_NO_CLASS:
    default          : assert("unknown dataspace class" && 0); HGOTO_ERROR(H5E_DATASPACE, H5E_UNSUPPORTED, 0, "internal error (unknown dataspace class)");
    }
...
int H5S_get_simple_extent_ndims(const H5S_t* ds)
{
    ...
    case H5S_NULL:
    case H5S_SCALAR:
    case H5S_SIMPLE: ret_value = (int)ds->extent.rank; break;

    case H5S_NO_CLASS:
    default          : assert("unknown dataspace class" && 0); HGOTO_ERROR(H5E_DATASPACE, H5E_UNSUPPORTED, FAIL, "internal error (unknown dataspace class)");
    } /* end switch */
 ...
 *-------------------------------------------------------------------------
 */
int H5Sget_simple_extent_dims(hid_t space_id, hsize_t dims[] /*out*/, hsize_t maxdims[] /*out*/)
{
    H5S_t* ds;
    int ret_value = -1;
...
int H5S_extent_get_dims(const H5S_extent_t* ext, hsize_t dims[], hsize_t max_dims[])
{
    ...
    switch (ext->type) {
    case H5S_NULL:
    case H5S_SCALAR: ret_value = 0; break;
...
static htri_t H5S__is_simple(const H5S_t* sdim)
{
    ...
    ret_value = (H5S_GET_EXTENT_TYPE(sdim) == H5S_SIMPLE || H5S_GET_EXTENT_TYPE(sdim) == H5S_SCALAR) ? true : false;
...
--------------------------------------------------------------------------*/
herr_t H5Sset_extent_simple(hid_t space_id, int rank, const hsize_t dims[/*rank*/], const hsize_t max[/*rank*/])
{
 ...
 *-------------------------------------------------------------------------
 */
hid_t H5Screate_simple(int rank, const hsize_t dims[/*rank*/], const hsize_t maxdims[/*rank*/])
{
    H5S_t* space = NULL;
    int i;
    hid_t ret_value = H5I_INVALID_HID;
 ...
 *-------------------------------------------------------------------------
 */
H5S_t* H5S_create_simple(unsigned rank, const hsize_t dims[/*rank*/], const hsize_t maxdims[/*rank*/])
{
    H5S_t* ret_value = NULL; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
const H5S_extent_t* H5S_get_simple_extent(const H5S_t* space)
{
    FUNC_ENTER_NOAPI_NOINIT_NOERR
        ...
        if (space->extent.size[u] != size[u]) {
            ...
            if (space->extent.max && H5S_UNLIMITED != space->extent.max[u] && space->extent.max[u] < size[u]) {
                HGOTO_ERROR(H5E_DATASPACE,
                            ...
                            (unsigned long long)space->extent.max[u]);
            }
 ...
 *-------------------------------------------------------------------------
 */
H5_ATTR_PURE bool H5S_has_extent(const H5S_t* ds)
{
...
htri_t H5Sextent_equal(hid_t space1_id, hid_t space2_id)
{
    ...
    /* check args */
    if (NULL == (ds1 = (const H5S_t*)H5I_object_verify(space1_id, H5I_DATASPACE)) || NULL == (ds2 = (const H5S_t*)H5I_object_verify(space2_id, H5I_DATASPACE))) {
        HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL, "not a dataspace");
    }
...
H5_ATTR_PURE htri_t H5S_extent_equal(const H5S_t* ds1, const H5S_t* ds2)
{
    ...
    if (ds1->extent.rank > 0) {
        ...
        else if ((ds1->extent.max == NULL && ds2->extent.max != NULL) || (ds1->extent.max != NULL && ds2->extent.max == NULL)) {
            HGOTO_DONE(false);
        }
    } /* end if */
