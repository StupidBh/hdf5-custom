...
static herr_t H5SM__find_in_list(const H5SM_list_t* list, const H5SM_mesg_key_t* key, size_t* empty_pos, size_t* list_pos);
static herr_t H5SM__convert_list_to_btree(H5F_t* f, H5SM_index_header_t* header, H5SM_list_t** _list, H5HF_t* fheap, H5O_t* open_oh);
...
static herr_t H5SM__write_mesg(H5F_t* f, H5O_t* open_oh, H5SM_index_header_t* header, bool defer, unsigned type_id, void* mesg, unsigned* cache_flags_ptr);
static herr_t H5SM__decr_ref(void* record, void* op_data, bool* changed);
static herr_t H5SM__delete_from_index(H5F_t* f,
                                      H5O_t* open_oh,
                                      H5SM_index_header_t* header,
                                      const H5O_shared_t* mesg,
                                      unsigned* cache_flags,
                                      size_t* /*out*/ mesg_size,
...
static herr_t H5SM__read_mesg(H5F_t* f, const H5SM_sohm_t* mesg, H5HF_t* fheap, H5O_t* open_oh, size_t* encoding_size /*out*/, void** encoded_mesg /*out*/);
...
static herr_t H5SM__type_to_flag(unsigned type_id, unsigned* type_flag)
{
    ...
    case H5O_PLINE_ID:
    case H5O_ATTR_ID    : *type_flag = (unsigned)1 << type_id; break;

    default: HGOTO_ERROR(H5E_SOHM, H5E_BADTYPE, FAIL, "unknown message type ID");
    } /* end switch */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5SM__get_index(const H5SM_master_table_t* table, unsigned type_id, ssize_t* idx)
{
    unsigned type_flag;
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5SM__can_share_common(const H5F_t* f, unsigned type_id, const void* mesg)
{
...
htri_t H5SM_try_share(H5F_t* f, H5O_t* open_oh, unsigned defer_flags, unsigned type_id, void* mesg, unsigned* mesg_flags)
{
...
#else  /* NDEBUG */
        if ((((H5O_shared_t*)mesg)->type != H5O_SHARE_TYPE_HERE) && (((H5O_shared_t*)mesg)->type != H5O_SHARE_TYPE_SOHM)) {
            HGOTO_DONE(false);
        }
#endif /* NDEBUG */
     ...
     */
    if (H5SM__write_mesg(f, open_oh, &(table->indexes[index_num]), (defer_flags & H5SM_DEFER) != 0, type_id, mesg, &cache_flags) < 0) {
    ...
    assert((ret_value != true) || ((H5O_shared_t*)mesg)->type == H5O_SHARE_TYPE_HERE || ((H5O_shared_t*)mesg)->type == H5O_SHARE_TYPE_SOHM);
#ifndef NDEBUG
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5SM__find_in_list(const H5SM_list_t* list, const H5SM_mesg_key_t* key, size_t* empty_pos, size_t* pos)
{
    size_t x;
    herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5SM__get_refcount_bt2_cb(const void* _record, void* _op_data)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5SM__read_iter_op(H5O_t* oh, H5O_mesg_t* mesg /*in,out*/, unsigned sequence, void* _udata /*in,out*/)
{
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5SM__table_free(H5SM_master_table_t* table)
{
    FUNC_ENTER_PACKAGE_NOERR
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5SM__list_free(H5SM_list_t* list)
{
    FUNC_ENTER_PACKAGE_NOERR
...
herr_t H5SM_table_debug(H5F_t* f, haddr_t table_addr, FILE* stream, int indent, int fwidth, unsigned table_vers, unsigned num_indexes)
{
    ...
    for (x = 0; x < num_indexes; ++x) {
        ...
        fprintf(stream,
                ...
                fwidth,
                "SOHM Index Type:",
                (table->indexes[x].index_type == H5SM_LIST ? "List" : (table->indexes[x].index_type == H5SM_BTREE ? "B-Tree" : "Unknown")));

        fprintf(stream, "%*s%-*s %" PRIuHADDR "\n", indent + 3, "", fwidth, "Address of index:", table->indexes[x].index_addr);
        fprintf(stream, "%*s%-*s %" PRIuHADDR "\n", indent + 3, "", fwidth, "Address of index's heap:", table->indexes[x].heap_addr);
        fprintf(stream, "%*s%-*s 0x%08x\n", indent + 3, "", fwidth, "Message type flags:", table->indexes[x].mesg_types);
        fprintf(stream, "%*s%-*s %zu\n", indent + 3, "", fwidth, "Minimum size of messages:", table->indexes[x].min_mesg_size);
        fprintf(stream, "%*s%-*s %zu\n", indent + 3, "", fwidth, "Number of messages:", table->indexes[x].num_messages);
        fprintf(stream, "%*s%-*s %zu\n", indent + 3, "", fwidth, "Maximum list size:", table->indexes[x].list_max);
        fprintf(stream, "%*s%-*s %zu\n", indent + 3, "", fwidth, "Minimum B-tree size:", table->indexes[x].btree_min);
    } /* end for */
...
herr_t H5SM_list_debug(H5F_t* f, haddr_t list_addr, FILE* stream, int indent, int fwidth, haddr_t table_addr)
{
    ...
    for (x = 0; x < table->indexes[index_num].num_messages; ++x) {
        ...
        fprintf(stream, "%*s%-*s %08lu\n", indent + 3, "", fwidth, "Hash value:", (unsigned long)list->messages[x].hash);
        if (list->messages[x].location == H5SM_IN_HEAP) {
            assert(fh);

            fprintf(stream, "%*s%-*s %s\n", indent + 3, "", fwidth, "Location:", "in heap");
            fprintf(stream, "%*s%-*s 0x%" PRIx64 "\n", indent + 3, "", fwidth, "Heap ID:", list->messages[x].u.heap_loc.fheap_id.val);
            fprintf(stream, "%*s%-*s %" PRIuHSIZE "\n", indent + 3, "", fwidth, "Reference count:", list->messages[x].u.heap_loc.ref_count);
        } /* end if */
        else if (list->messages[x].location == H5SM_IN_OH) {
            fprintf(stream, "%*s%-*s %s\n", indent + 3, "", fwidth, "Location:", "in object header");
            fprintf(stream, "%*s%-*s %" PRIuHADDR "\n", indent + 3, "", fwidth, "Object header address:", list->messages[x].u.mesg_loc.oh_addr);
            fprintf(stream, "%*s%-*s %" PRIuHADDR "\n", indent + 3, "", fwidth, "Message creation index:", list->messages[x].u.mesg_loc.oh_addr);
            fprintf(stream, "%*s%-*s %u\n", indent + 3, "", fwidth, "Message type ID:", list->messages[x].msg_type_id);
        } /* end if */
        else {
