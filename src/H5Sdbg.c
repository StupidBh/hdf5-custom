...
herr_t H5S_debug(H5F_t* f, const void* _mesg, FILE* stream, int indent, int fwidth)
{
    ...
    case H5S_NULL: fprintf(stream, "%*s%-*s H5S_NULL\n", indent, "", fwidth, "Space class:"); break;

    case H5S_SCALAR: fprintf(stream, "%*s%-*s H5S_SCALAR\n", indent, "", fwidth, "Space class:"); break;
    ...
    case H5S_NO_CLASS:
    default          : fprintf(stream, "%*s%-*s **UNKNOWN-%ld**\n", indent, "", fwidth, "Space class:", (long)(H5S_GET_EXTENT_TYPE(mesg))); break;
    } /* end switch */
