...
herr_t H5Soffset_simple(hid_t space_id, const hssize_t* offset)
{
    ...
    if (space->extent.rank == 0 || (H5S_GET_EXTENT_TYPE(space) == H5S_SCALAR || H5S_GET_EXTENT_TYPE(space) == H5S_NULL)) {
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5S_select_copy(H5S_t* dst, const H5S_t* src, bool share_selection)
{
    H5S_t tmp_space;
 ...
 *-------------------------------------------------------------------------
 */
hssize_t H5S_select_serial_size(H5S_t* space)
{
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5S_select_serialize(H5S_t* space, uint8_t** p)
{
    herr_t ret_value = SUCCEED; /* Return value */
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5S_select_deserialize(H5S_t** space, const uint8_t** p, const size_t p_size)
{
    ...
    switch (sel_type) {
    case H5S_SEL_POINTS: /* Sequence of points selected */ ret_value = (*H5S_sel_point->deserialize)(space, p, sel_info_size, skip); break;

    case H5S_SEL_HYPERSLABS: /* Hyperslab selection defined */ ret_value = (*H5S_sel_hyper->deserialize)(space, p, sel_info_size, skip); break;

    case H5S_SEL_ALL: /* Entire extent selected */ ret_value = (*H5S_sel_all->deserialize)(space, p, sel_info_size, skip); break;

    case H5S_SEL_NONE: /* Nothing selected */ ret_value = (*H5S_sel_none->deserialize)(space, p, sel_info_size, skip); break;
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5Sget_select_bounds(hid_t spaceid, hsize_t start[] /*out*/, hsize_t end[] /*out*/)
{
    H5S_t* space;     /* Dataspace to modify selection of */
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
htri_t H5S_select_is_regular(H5S_t* space)
{
    herr_t ret_value = FAIL; /* Return value */
...
--------------------------------------------------------------------------*/
herr_t H5S_select_iter_init(H5S_sel_iter_t* sel_iter, H5S_t* space, size_t elmt_size, unsigned flags)
{
    herr_t ret_value = FAIL; /* Return value */
...
--------------------------------------------------------------------------*/
herr_t H5S_select_iterate(void* buf, const H5T_t* type, H5S_t* space, const H5S_sel_iter_op_t* op, void* op_data)
{
    ...
    while (max_elem > 0 && user_ret == 0) {
        ...
        for (curr_seq = 0; curr_seq < nseq && user_ret == 0; curr_seq++) {
            ...
            while (curr_len > 0 && user_ret == 0) {
                ...
                default: HGOTO_ERROR(H5E_DATASPACE, H5E_UNSUPPORTED, FAIL, "unsupported op type");
                } /* end switch */
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
htri_t H5S_select_shape_same(H5S_t* space1, H5S_t* space2)
{
        ...
        /* Otherwise, iterate through all the blocks in the selection */
            ...
            while (1) {
                ...
                if (first_block) {
                     ...
                     */
                    while (space_b_dim >= 0) {
                        if ((end_a[space_a_dim] - start_a[space_a_dim]) != (end_b[space_b_dim] - start_b[space_b_dim])) {
                            HGOTO_DONE(false);
                        }
...
/*--------------------------------------------------------------------------
 ...
 COMMENTS, BUGS, ASSUMPTIONS
    ...
    don't call it directly, use the appropriate macro defined in H5Sprivate.h.
--------------------------------------------------------------------------*/
htri_t H5S_select_intersect_block(H5S_t* space, const hsize_t* start, const hsize_t* end)
{
    htri_t ret_value = true; /* Return value */
...
htri_t H5Sselect_intersect_block(hid_t space_id, const hsize_t* start, const hsize_t* end)
{
    ...
    for (u = 0; u < space->extent.rank; u++) {
        if (start[u] > end[u]) {
            HGOTO_ERROR(H5E_DATASPACE,
                        H5E_BADRANGE,
                        FAIL,
                        "block start[%u] (%llu) > end[%u] (%llu)",
                        u,
                        (unsigned long long)start[u],
                        u,
                        (unsigned long long)end[u]);
        }
    }
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5S_select_construct_projection(H5S_t* base_space, H5S_t** new_space_ptr, unsigned new_space_rank, hsize_t element_size, ptrdiff_t* buf_adj)
{
    ...
    assert((H5S_GET_EXTENT_TYPE(base_space) == H5S_SCALAR) || (H5S_GET_EXTENT_TYPE(base_space) == H5S_SIMPLE));
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5S_select_fill(const void* fill, size_t fill_size, H5S_t* space, void* _buf)
{
    H5S_sel_iter_t* iter = NULL; /* Selection iteration info */
    bool iter_init = false;      /* Selection iteration info has been initialized */
    hsize_t* off = NULL;         /* Array to store sequence offsets */
...
to share structures inside dst_space with proj_space
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5S_select_project_intersection(H5S_t* src_space, H5S_t* dst_space, H5S_t* src_intersect_space, H5S_t** new_space_ptr, bool share_selection)
{
    H5S_t* new_space = NULL;               /* New dataspace constructed */
     ...
     * "none" */
    else if ((H5S_GET_SELECT_NPOINTS(src_intersect_space) == 0) || (H5S_GET_SELECT_NPOINTS(src_space) == 0) || (H5S_GET_SELECT_NPOINTS(dst_space) == 0)) {
        ...
        if (H5S_GET_EXTENT_TYPE(dst_space) == H5S_SCALAR) {
            ...
            assert(0 == memcmp(coords_start, coords_end, H5S_GET_EXTENT_NDIMS(src_space) * sizeof(coords_start[0])));
            ...
            if ((H5S_GET_SELECT_TYPE(src_space) == H5S_SEL_POINTS) || (H5S_GET_SELECT_TYPE(dst_space) == H5S_SEL_POINTS)) {
                ...
                do {
                    ...
                    if (H5S_SELECT_ITER_NEXT(ss_iter, 1) < 0) {
                        HGOTO_ERROR(H5E_DATASPACE, H5E_CANTNEXT, FAIL, "can't advance source selection iterator");
                    }
                    ss_iter->elmt_left--;
                    if (H5S_SELECT_ITER_NEXT(ds_iter, 1) < 0) {
                        HGOTO_ERROR(H5E_DATASPACE, H5E_CANTNEXT, FAIL, "can't advance destination selection iterator");
                    }
                    ds_iter->elmt_left--;
...
hid_t H5Sselect_project_intersection(hid_t src_space_id, hid_t dst_space_id, hid_t src_intersect_space_id)
{
    ...
    if (H5S_GET_SELECT_NPOINTS(src_space) != H5S_GET_SELECT_NPOINTS(dst_space)) {
        HGOTO_ERROR(H5E_DATASPACE, H5E_BADVALUE, H5I_INVALID_HID, "number of points selected in source space does not match that in destination space");
    }
    ...
    if (H5S_GET_EXTENT_NDIMS(src_space) != H5S_GET_EXTENT_NDIMS(src_intersect_space)) {
        HGOTO_ERROR(H5E_DATASPACE, H5E_BADVALUE, H5I_INVALID_HID, "rank of source space does not match rank of source intersect space");
    }
...
herr_t H5S_select_subtract(H5S_t* space, H5S_t* subtract_space)
{
    ...
    if ((space->select.type->type != H5S_SEL_NONE) && (subtract_space->select.type->type != H5S_SEL_NONE)) {
        ...
        else if ((subtract_space->select.type->type == H5S_SEL_POINTS) || (space->select.type->type == H5S_SEL_POINTS)) {
...
hid_t H5Ssel_iter_create(hid_t space_id, size_t elmt_size, unsigned flags)
{
    ...
    if ((ret_value = H5I_register(H5I_SPACE_SEL_ITER, sel_iter, true)) < 0) {
        HGOTO_ERROR(H5E_DATASPACE, H5E_CANTREGISTER, H5I_INVALID_HID, "unable to register dataspace selection iterator ID");
    }
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5S_select_contig_block(H5S_t* space, bool* is_contig, hsize_t* off, size_t* len)
{
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5S__sel_iter_close_cb(void* _sel_iter, void H5_ATTR_UNUSED** request)
{
