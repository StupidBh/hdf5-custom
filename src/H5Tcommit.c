...
/* Local Prototypes */
/********************/
static herr_t H5T__commit_api_common(hid_t loc_id,
                                     const char* name,
                                     hid_t type_id,
                                     hid_t lcpl_id,
                                     hid_t tcpl_id,
                                     ...
                                     H5VL_object_t** _vol_obj_ptr)
{
    ...
    /* Commit the type */
    if (NULL == (data = H5VL_datatype_commit(*vol_obj_ptr, &loc_params, name, type_id, lcpl_id, tcpl_id, tapl_id, H5P_DATASET_XFER_DEFAULT, token_ptr))) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Tcommit_async(const char* app_file,
                       const char* app_func,
                       unsigned app_line,
                       hid_t loc_id,
                       const char* name,
...
herr_t H5Tcommit_anon(hid_t loc_id, hid_t type_id, hid_t tcpl_id, hid_t tapl_id)
{
    ...
    /* Commit the datatype */
    if (NULL ==
        (dt = H5VL_datatype_commit(vol_obj, &loc_params, NULL, type_id, H5P_LINK_CREATE_DEFAULT, tcpl_id, tapl_id, H5P_DATASET_XFER_DEFAULT, H5_REQUEST_NULL))) {
        HGOTO_ERROR(H5E_DATATYPE, H5E_CANTINIT, FAIL, "unable to commit datatype");
    }
...
herr_t H5T__commit(H5F_t* file, H5T_t* type, hid_t tcpl_id)
{
    ...
    if (H5O_msg_append_oh(file, oh, H5O_DTYPE_ID, H5O_MSG_FLAG_CONSTANT | H5O_MSG_FLAG_DONTSHARE, H5O_UPDATE_TIME, type) < 0) {
 ...
 *-------------------------------------------------------------------------
 */
hid_t H5Topen_async(const char* app_file, const char* app_func, unsigned app_line, hid_t loc_id, const char* name, hid_t tapl_id, hid_t es_id)
{
    H5VL_object_t* vol_obj = NULL;      /* Object for loc_id */
    void* token = NULL;                 /* Request token for async operation */
    void** token_ptr = H5_REQUEST_NULL; /* Pointer to request token for async operation */
 ...
 *-------------------------------------------------------------------------
 */
bool H5T_already_vol_managed(const H5T_t* dt)
{
    FUNC_ENTER_NOAPI_NOINIT_NOERR
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T_invoke_vol_optional(H5T_t* dt, H5VL_optional_args_t* args, hid_t dxpl_id, void** req, H5VL_object_t** vol_obj_ptr)
{
    herr_t ret_value = SUCCEED; /* Return value */
