 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T_reclaim(const H5T_t* type, H5S_t* space, void* buf)
{
 ...
 *-------------------------------------------------------------------------
 */
bool H5T_get_force_conv(const H5T_t* dt)
{
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__reverse_order(uint8_t* rev, uint8_t* s, const H5T_t* dtype)
{
    H5T_order_t order;
    size_t size;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_noop(const H5T_t H5_ATTR_UNUSED* src,
                      const H5T_t H5_ATTR_UNUSED* dst,
                      H5T_cdata_t* cdata,
                      ...
                      void H5_ATTR_UNUSED* background)
{
    ...
    case H5T_CONV_INIT: cdata->need_bkg = H5T_BKG_NO; break;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_order(const H5T_t* src,
                       const H5T_t* dst,
                       H5T_cdata_t* cdata,
                       const H5T_conv_ctx_t H5_ATTR_UNUSED* conv_ctx,
                       size_t nelmts,
                       ...
                       void H5_ATTR_UNUSED* background)
{
    ...
    case H5T_CONV_INIT:
        ...
        if (!((H5T_ORDER_BE == src_order && H5T_ORDER_LE == dst_order) || (H5T_ORDER_LE == src_order && H5T_ORDER_BE == dst_order))) {
            HGOTO_ERROR(H5E_DATATYPE, H5E_UNSUPPORTED, FAIL, "conversion not supported");
        }
        ...
        case H5T_FLOAT:
            if (src->shared->u.atomic.u.f.sign != dst->shared->u.atomic.u.f.sign || src->shared->u.atomic.u.f.epos != dst->shared->u.atomic.u.f.epos ||
                src->shared->u.atomic.u.f.esize != dst->shared->u.atomic.u.f.esize || src->shared->u.atomic.u.f.ebias != dst->shared->u.atomic.u.f.ebias ||
                src->shared->u.atomic.u.f.mpos != dst->shared->u.atomic.u.f.mpos || src->shared->u.atomic.u.f.msize != dst->shared->u.atomic.u.f.msize ||
                src->shared->u.atomic.u.f.norm != dst->shared->u.atomic.u.f.norm || src->shared->u.atomic.u.f.pad != dst->shared->u.atomic.u.f.pad) {
                HGOTO_ERROR(H5E_DATATYPE, H5E_UNSUPPORTED, FAIL, "conversion not supported");
            } /* end if */
            break;
        ...
        case H5T_COMPLEX:
            {
                ...
                if (src_base_sh->u.atomic.u.f.sign != dst_base_sh->u.atomic.u.f.sign || src_base_sh->u.atomic.u.f.epos != dst_base_sh->u.atomic.u.f.epos ||
                    src_base_sh->u.atomic.u.f.esize != dst_base_sh->u.atomic.u.f.esize || src_base_sh->u.atomic.u.f.ebias != dst_base_sh->u.atomic.u.f.ebias ||
                    src_base_sh->u.atomic.u.f.mpos != dst_base_sh->u.atomic.u.f.mpos || src_base_sh->u.atomic.u.f.msize != dst_base_sh->u.atomic.u.f.msize ||
                    src_base_sh->u.atomic.u.f.norm != dst_base_sh->u.atomic.u.f.norm || src_base_sh->u.atomic.u.f.pad != dst_base_sh->u.atomic.u.f.pad) {
                    HGOTO_ERROR(H5E_DATATYPE, H5E_UNSUPPORTED, FAIL, "conversion not supported");
                }
        ...
        case H5T_ARRAY:
        case H5T_NCLASSES:
        default           : HGOTO_ERROR(H5E_DATATYPE, H5E_UNSUPPORTED, FAIL, "conversion not supported");
        } /* end switch */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_order_opt(const H5T_t* src,
                           const H5T_t* dst,
                           H5T_cdata_t* cdata,
                           const H5T_conv_ctx_t H5_ATTR_UNUSED* conv_ctx,
                           size_t nelmts,
                           ...
                           void H5_ATTR_UNUSED* background)
{
    ...
    case H5T_CONV_INIT:
        ...
        if (src->shared->type != H5T_REFERENCE &&
            !((H5T_ORDER_BE == src_order && H5T_ORDER_LE == dst_order) || (H5T_ORDER_LE == src_order && H5T_ORDER_BE == dst_order))) {
            HGOTO_ERROR(H5E_DATATYPE, H5E_UNSUPPORTED, FAIL, "conversion not supported");
        }
    ...
    case H5T_CONV_CONV:
        ...
        default: HGOTO_ERROR(H5E_DATATYPE, H5E_UNSUPPORTED, FAIL, "invalid conversion size");
        } /* end switch */
        break;
