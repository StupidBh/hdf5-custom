...
static herr_t H5T__conv_struct_init(const H5T_t* src, const H5T_t* dst, H5T_cdata_t* cdata, const H5T_conv_ctx_t* conv_ctx);
...
static herr_t H5T__conv_struct_init(const H5T_t* src, const H5T_t* dst, H5T_cdata_t* cdata, const H5T_conv_ctx_t* conv_ctx)
{
    ...
    if (!priv) {
        ...
        if (NULL == (priv->src2dst = (int*)H5MM_malloc(src_nmembs * sizeof(int)))) {
            HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL, "couldn't allocate source to destination member mapping array");
        }
        if (NULL == (priv->src_memb = (H5T_t**)H5MM_malloc(src_nmembs * sizeof(H5T_t*)))) {
            HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL, "couldn't allocate source compound member datatype array");
        }
        if (NULL == (priv->dst_memb = (H5T_t**)H5MM_malloc(dst_nmembs * sizeof(H5T_t*)))) {
            HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL, "couldn't allocate destination compound member datatype array");
        }

        /* Allocate and initialize arrays for datatype IDs */
        if (NULL == (priv->src_memb_id = (hid_t*)H5MM_malloc(src_nmembs * sizeof(hid_t)))) {
            HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL, "couldn't allocate source compound member datatype ID array");
        }
        ...
        if (NULL == (priv->dst_memb_id = (hid_t*)H5MM_malloc(dst_nmembs * sizeof(hid_t)))) {
            HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL, "couldn't allocate destination compound member datatype ID array");
        }
        ...
        for (i = 0; i < src_nmembs; i++) {
            ...
            if (src2dst[i] >= 0) {
                ...
                if (NULL == (type = H5T_copy(src->shared->u.compnd.memb[i].type, H5T_COPY_ALL))) {
                    HGOTO_ERROR(H5E_DATATYPE, H5E_CANTCOPY, FAIL, "can't copy source compound member datatype");
                }
                priv->src_memb[i] = type;

                if (NULL == (type = H5T_copy(dst->shared->u.compnd.memb[src2dst[i]].type, H5T_COPY_ALL))) {
                    HGOTO_ERROR(H5E_DATATYPE, H5E_CANTCOPY, FAIL, "can't copy destination compound member datatype");
                }
    ...
    for (i = 0; i < src_nmembs; i++) {
        if (src2dst[i] >= 0) {
            ...
            tpath = H5T_path_find(src->shared->u.compnd.memb[i].type, dst->shared->u.compnd.memb[src2dst[i]].type);
             ...
             * was provided.
             */
            need_ids = tpath->conv.is_app || (cdata->command == H5T_CONV_INIT && conv_ctx->u.init.cb_struct.func) ||
            ...
            if (need_ids) {
                ...
                if (priv->src_memb_id[i] == H5I_INVALID_HID) {
                    if ((tid = H5I_register(H5I_DATATYPE, priv->src_memb[i], false)) < 0) {
                        H5T__conv_struct_free(priv);
                        cdata->priv = NULL;
                        HGOTO_ERROR(H5E_DATATYPE, H5E_CANTREGISTER, FAIL, "can't register ID for source compound member datatype");
                    }
                ...
                if (priv->dst_memb_id[src2dst[i]] == H5I_INVALID_HID) {
                    if ((tid = H5I_register(H5I_DATATYPE, priv->dst_memb[src2dst[i]], false)) < 0) {
                        H5T__conv_struct_free(priv);
                        cdata->priv = NULL;
                        HGOTO_ERROR(H5E_DATATYPE, H5E_CANTREGISTER, FAIL, "can't register ID for destination compound member datatype");
                    }
    ...
    if (src_nmembs < dst_nmembs) {
        ...
        if (priv->subset_info.subset == H5T_SUBSET_SRC) {
            priv->subset_info.copy_size = src->shared->u.compnd.memb[src_nmembs - 1].offset + src->shared->u.compnd.memb[src_nmembs - 1].size;
        }
    }
        ...
        if (priv->subset_info.subset == H5T_SUBSET_DST) {
            priv->subset_info.copy_size = dst->shared->u.compnd.memb[dst_nmembs - 1].offset + dst->shared->u.compnd.memb[dst_nmembs - 1].size;
        }
    }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_struct(const H5T_t* src,
                        const H5T_t* dst,
                        H5T_cdata_t* cdata,
        ...
        for (elmtno = 0; elmtno < nelmts; elmtno++) {
            ...
            for (u = 0, offset = 0; u < src->shared->u.compnd.nmembs; u++) {
                ...
                if (dst_memb->size <= src_memb->size) {
                    ...
                    if (H5T_convert_with_ctx(priv->memb_path[u],
                                             priv->src_memb[u],
                                             priv->dst_memb[src2dst[u]],
                                             &tmp_conv_ctx,
                                             (size_t)1,
                                             (size_t)0,
                                             (size_t)0, /*no striding (packed array)*/
                                             xbuf + src_memb->offset,
                                             xbkg + dst_memb->offset) < 0) {
                        HGOTO_ERROR(H5E_DATATYPE, H5E_CANTCONVERT, FAIL, "unable to convert compound datatype member");
                    }
            ...
            for (int i = (int)src->shared->u.compnd.nmembs - 1; i >= 0; --i) {
                ...
                if (dst_memb->size > src_memb->size) {
                    ...
                    offset -= src_memb->size;
                    if (H5T_convert_with_ctx(priv->memb_path[i],
                                             priv->src_memb[i],
                                             priv->dst_memb[src2dst[i]],
                                             &tmp_conv_ctx,
                                             (size_t)1,
                                             (size_t)0,
                                             (size_t)0, /*no striding (packed array)*/
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_struct_opt(const H5T_t* src,
                            const H5T_t* dst,
                            H5T_cdata_t* cdata,
                            const H5T_conv_ctx_t* conv_ctx,
                            size_t nelmts,
