...
static herr_t H5T__conv_enum_init(const H5T_t* src, const H5T_t* dst, H5T_cdata_t* cdata, const H5T_conv_ctx_t* conv_ctx);
...
static herr_t H5T__conv_enum_init(const H5T_t* src, const H5T_t* dst, H5T_cdata_t* cdata, const H5T_conv_ctx_t* conv_ctx)
{
    ...
    if (rebuild_cache) {
        ...
        if (NULL == (tmp_realloc = realloc(priv->src2dst, src_nmembs * sizeof(int)))) {
            free(priv->src2dst);
            HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL, "unable to allocate space for source to destination enum mapping");
        }
        ...
        for (size_t i = 0, j = 0; i < src_nmembs && j < dst_nmembs; i++, j++) {
            ...
            if (j >= dst_nmembs) {
                HGOTO_ERROR(H5E_DATATYPE, H5E_UNSUPPORTED, FAIL, "source enum type is not a subset of destination enum type");
            }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_enum(const H5T_t* src,
                      const H5T_t* dst,
                      H5T_cdata_t* cdata,
        ...
        if (priv->length) {
            for (i = 0; i < nelmts; i++, s += src_delta, d += dst_delta) {
                ...
                if (n < 0 || (unsigned)n >= priv->length || priv->src2dst[n] < 0) {
                    ...
                    H5MM_memcpy(d, (uint8_t*)dst_sh->u.enumer.value + ((unsigned)priv->src2dst[n] * dst_sh->size), dst_sh->size);
                }
            }
        }
        else {
            for (i = 0; i < nelmts; i++, s += src_delta, d += dst_delta) {
                ...
                while (lt < rt) {
                    md = (lt + rt) / 2;
                    cmp = memcmp(s, (uint8_t*)src_sh->u.enumer.value + (md * src_sh->size), src_sh->size);
                    if (cmp < 0) {
                        rt = md;
                    }
                    else if (cmp > 0) {
                ...
                if (lt >= rt) {
                    ...
                    H5MM_memcpy(d, (uint8_t*)dst_sh->u.enumer.value + ((unsigned)priv->src2dst[md] * dst_sh->size), dst_sh->size);
                } /* end else */
            }
        }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_enum_numeric(const H5T_t* src,
                              const H5T_t* dst,
                              H5T_cdata_t* cdata,
