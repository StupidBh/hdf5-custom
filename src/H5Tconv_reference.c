 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ref(const H5T_t* src,
                     const H5T_t* dst,
                     H5T_cdata_t* cdata,
            ...
            while (nelmts > 0) {
                ...
                for (elmtno = 0; elmtno < safe; elmtno++) {
                    ...
                    if (is_nil) {
                        ...
                        if (0 == (buf_size = src->shared->u.atomic.u.r.cls
                                                 ->getsize(src->shared->u.atomic.u.r.file, s, src->shared->size, dst->shared->u.atomic.u.r.file, &dst_copy))) {
                        ...
                        if (dst_copy && (src->shared->u.atomic.u.r.loc == H5T_LOC_DISK)) {
                            ...
                            /* Read reference */
                            if (src->shared->u.atomic.u.r.cls
                                    ->read(src->shared->u.atomic.u.r.file, s, src->shared->size, dst->shared->u.atomic.u.r.file, conv_buf, buf_size) < 0) {
                        ...
                        if (dst_copy && (dst->shared->u.atomic.u.r.loc == H5T_LOC_DISK)) {
                            ...
                            if (dst->shared->u.atomic.u.r.cls->write(src->shared->u.atomic.u.r.file,
                                                                     conv_buf,
                                                                     buf_size,
                                                                     src->shared->u.atomic.u.r.rtype,
                                                                     dst->shared->u.atomic.u.r.file,
                                                                     d,
                                                                     dst->shared->size,
                                                                     b) < 0) {
