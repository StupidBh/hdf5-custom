 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_s_s(const H5T_t* src,
                     const H5T_t* dst,
                     H5T_cdata_t* cdata,
                     ...
                     void H5_ATTR_UNUSED* bkg)
{
    ...
    case H5T_CONV_INIT:
        ...
        if (src->shared->u.atomic.u.s.pad < 0 || src->shared->u.atomic.u.s.pad >= H5T_NSTR || dst->shared->u.atomic.u.s.pad < 0 ||
            dst->shared->u.atomic.u.s.pad >= H5T_NSTR) {
    ...
    case H5T_CONV_CONV:
        ...
        if (src->shared->size == dst->shared->size || buf_stride) {
            ...
            double olapd = ceil((double)(dst->shared->size) / (double)(src->shared->size - dst->shared->size));
            olap = (size_t)olapd;
            sp = dp = (uint8_t*)buf;
            direction = 1;
        }
        else {
            double olapd = ceil((double)(src->shared->size) / (double)(dst->shared->size - src->shared->size));
            olap = (size_t)olapd;
        ...
        for (elmtno = 0; elmtno < nelmts; elmtno++) {
            ...
            case H5T_STR_NULLTERM:
                for (nchars = 0; nchars < dst->shared->size && nchars < src->shared->size && s[nchars]; nchars++) {
                    d[nchars] = s[nchars];
                }
                break;
            ...
            case H5T_STR_ERROR:
            default                 : HGOTO_ERROR(H5E_DATATYPE, H5E_UNSUPPORTED, FAIL, "source string padding method not supported");
            } /* end switch */
            ...
            case H5T_STR_RESERVED_15:
            case H5T_STR_ERROR:
            default                 : HGOTO_ERROR(H5E_DATATYPE, H5E_UNSUPPORTED, FAIL, "destination string padding method not supported");
            } /* end switch */
