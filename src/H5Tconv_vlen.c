...
static herr_t H5T__conv_vlen_nested_free(uint8_t* buf, H5T_t* dt)
{
    ...
    case H5T_NO_CLASS:
    case H5T_NCLASSES:
    default          : HGOTO_ERROR(H5E_DATATYPE, H5E_BADTYPE, FAIL, "invalid datatype class");
    }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_vlen(const H5T_t* src,
                      const H5T_t* dst,
                      H5T_cdata_t* cdata,
                      ...
                      void* bkg)
{
    ...
    bool convert_forward = true;         /* Current direction of conversion (forward or backward, used for error handling) */
    bool conversions_made = false;       /* Flag to indicate conversions have been performed, used for error handling */
    herr_t ret_value = SUCCEED;          /* Return value */
    ...
    case H5T_CONV_CONV:
        ...
        if (NULL == (tpath = H5T_path_find(src->shared->parent, dst->shared->parent))) {
            ...
            if (NULL == (tsrc_cpy = H5T_copy(src->shared->parent, H5T_COPY_ALL))) {
                HGOTO_ERROR(H5E_DATATYPE, H5E_CANTCOPY, FAIL, "unable to copy src base type for conversion");
            }
            ...
            if (NULL == (tdst_cpy = H5T_copy(dst->shared->parent, H5T_COPY_ALL))) {
                HGOTO_ERROR(H5E_DATATYPE, H5E_CANTCOPY, FAIL, "unable to copy dst base type for conversion");
            }
            ...
            if (tpath->conv.is_app || conv_ctx->u.conv.cb_struct.func) {
                if ((tsrc_id = H5I_register(H5I_DATATYPE, tsrc_cpy, false)) < 0) {
                    HGOTO_ERROR(H5E_DATATYPE, H5E_CANTREGISTER, FAIL, "unable to register ID for source base datatype");
                }
                if ((tdst_id = H5I_register(H5I_DATATYPE, tdst_cpy, false)) < 0) {
                    HGOTO_ERROR(H5E_DATATYPE, H5E_CANTREGISTER, FAIL, "unable to register ID for destination base datatype");
                }
            }
                    ...
                    if (!noop_conv) {
                        ...
                        if (nested) {
                            ...
                            if (bg_seq_len < seq_len) {
                                memset((uint8_t*)tmp_buf + dst_base_size * bg_seq_len, 0, (seq_len - bg_seq_len) * dst_base_size);
                            }
                        } /* end if */

                        /* Convert VL sequence */
                        tmp_conv_ctx.u.conv.recursive = true;
                        if (H5T_convert_with_ctx(tpath, tsrc_cpy, tdst_cpy, &tmp_conv_ctx, seq_len, (size_t)0, (size_t)0, conv_buf, tmp_buf) < 0) {
    ...
    default: /* Some other command we don't know about yet.*/ HGOTO_ERROR(H5E_DATATYPE, H5E_UNSUPPORTED, FAIL, "unknown conversion command");
    } /* end switch */
