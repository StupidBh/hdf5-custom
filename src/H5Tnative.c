...
static H5T_t* H5T__get_native_type(H5T_t* dt, H5T_direction_t direction, size_t* struct_align, size_t* offset, size_t* comp_size);
static H5T_t* H5T__get_native_integer(size_t prec, H5T_sign_t sign, H5T_direction_t direction, size_t* struct_align, size_t* offset, size_t* comp_size);
static H5T_t* H5T__get_native_float(const H5T_t* dtype, H5T_direction_t direction, size_t* struct_align, size_t* offset, size_t* comp_size);
static H5T_t* H5T__get_native_bitfield(size_t prec, H5T_direction_t direction, size_t* struct_align, size_t* offset, size_t* comp_size);
static herr_t H5T__cmp_offset(size_t* comp_size, size_t* offset, size_t elem_size, size_t nelems, size_t align, size_t* struct_align);
 ...
 *-------------------------------------------------------------------------
 */
static H5T_t* H5T__get_native_type(H5T_t* dtype, H5T_direction_t direction, size_t* struct_align, size_t* offset, size_t* comp_size)
{
    ...
    case H5T_FLOAT:
        if (NULL == (ret_value = H5T__get_native_float(dtype, direction, struct_align, offset, comp_size))) {
    ...
    case H5T_STRING:
        ...
        if (H5T_IS_VL_STRING(dtype->shared)) {
            ...
            if (H5T__cmp_offset(comp_size, offset, sizeof(char*), (size_t)1, H5T_POINTER_ALIGN_g, struct_align) < 0) {
                HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL, "cannot compute compound offset");
            }
        } /* end if */
        else {
            /* Update size, offset and compound alignment for parent. */
            if (H5T__cmp_offset(comp_size, offset, sizeof(char), size, H5T_NATIVE_SCHAR_ALIGN_g, struct_align) < 0) {
                HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL, "cannot compute compound offset");
            }
        } /* end else */
        break;
     ...
     * message for now.*/
    case H5T_TIME: HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL, "time type is not supported yet");
    ...
    case H5T_ENUM:
        {
            ...
            for (u = 0; u < nmemb; u++) {
                ...
                if (H5T_convert(tpath, super_type, nat_super_type, (size_t)1, (size_t)0, (size_t)0, memb_value, NULL) < 0) {
                    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL, "cannot get member value");
                }
    ...
    case H5T_ARRAY:
        {
            ...
            if (NULL == (nat_super_type = H5T__get_native_type(super_type, direction, &super_align, &super_offset, &super_size))) {
    ...
    case H5T_NO_CLASS:
    case H5T_NCLASSES:
    default          : HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL, "data type doesn't match any native type");
    } /* end switch */
...
static H5T_t* H5T__get_native_integer(size_t prec, H5T_sign_t sign, H5T_direction_t direction, size_t* struct_align, size_t* offset, size_t* comp_size)
{
    ...
    enum match_type
    { /* The different kinds of integers we can match */
    ...
    default                          : HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL, "Unknown native integer match");
    } /* end switch */
...
static H5T_t* H5T__get_native_float(const H5T_t* dtype, H5T_direction_t direction, size_t* struct_align, size_t* offset, size_t* comp_size)
{
    ...
    enum match_type
    { /* The different kinds of floating point types we can match */
    ...
    if (direction == H5T_DIR_DEFAULT || direction == H5T_DIR_ASCEND) {
        if (size == 1) {
            ...
            if (0 == H5T_cmp(dtype, f8_e4m3_dt, false) || 0 == H5T_cmp(dtype, f8_e5m2_dt, false) || 0 == H5T_cmp(dtype, f6_e2m3_dt, false) ||
                0 == H5T_cmp(dtype, f6_e3m2_dt, false) || 0 == H5T_cmp(dtype, f4_e2m1_dt, false)) {
    ...
    default                            : HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL, "Unknown native floating-point match");
    } /* end switch */
...
static H5T_order_t get_host_byte_order(void)
{
    static const union
    {
        uint64_t u64;
 ...
 */
herr_t H5T__init_native_internal(void)
{
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__init_native_complex_types(void)
{
    ...
    if ((H5T_NATIVE_LDOUBLE_COMPLEX_g = H5I_register(H5I_DATATYPE, dt, false)) < 0) {
        HGOTO_ERROR(H5E_DATATYPE, H5E_CANTINIT, FAIL, "can't register ID for native long double complex datatype");
    }
