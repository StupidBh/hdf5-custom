                             ...
                             void H5_ATTR_UNUSED** req)
{
    ...
    if (loc_params->type == H5VL_OBJECT_BY_SELF) {
        ...
        if (NULL == (attr = H5A__open_by_idx(&loc,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_attr_read(void* attr, hid_t dtype_id, void* buf, hid_t dxpl_id, void H5_ATTR_UNUSED** req)
{
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_attr_write(void* attr, hid_t dtype_id, const void* buf, hid_t dxpl_id, void H5_ATTR_UNUSED** req)
{
    H5T_t* mem_type;  /* Memory datatype */
    herr_t ret_value; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_attr_get(void* obj, H5VL_attr_get_args_t* args, hid_t H5_ATTR_UNUSED dxpl_id, void H5_ATTR_UNUSED** req)
{
    ...
    case H5VL_ATTR_GET_NAME:
        {
            ...
            if (H5VL_OBJECT_BY_SELF == get_name_args->loc_params.type) {
                if (H5A__get_name((H5A_t*)obj, get_name_args->buf_size, get_name_args->buf, get_name_args->attr_name_len) < 0) {
                ...
                /* Open the attribute on the object header */
                if (NULL == (attr = H5A__open_by_idx(&loc,
                                                     get_name_args->loc_params.loc_data.loc_by_idx.name,
                ...
                *get_name_args->attr_name_len = strlen(attr->shared->name);
                ...
                if (get_name_args->buf) {
                    strncpy(get_name_args->buf, attr->shared->name, MIN((*get_name_args->attr_name_len + 1), get_name_args->buf_size));
    ...
    case H5VL_ATTR_GET_INFO:
        {
            ...
            if (H5VL_OBJECT_BY_SELF == get_info_args->loc_params.type) {
                ...
                /* Open the attribute on the object header */
                if (NULL == (attr = H5A__open_by_name(&loc, get_info_args->loc_params.loc_data.loc_by_name.name, get_info_args->attr_name))) {
                    HGOTO_ERROR(H5E_ATTR, H5E_CANTOPENOBJ, FAIL, "can't open attribute");
                }
                ...
                /* Open the attribute on the object header */
                if (NULL == (attr = H5A__open_by_idx(&loc,
                                                     get_info_args->loc_params.loc_data.loc_by_idx.name,
    ...
    default: HGOTO_ERROR(H5E_VOL, H5E_CANTGET, FAIL, "can't get this type of information from attr");
    } /* end switch */
                                  ...
                                  void H5_ATTR_UNUSED** req)
{
    ...
    case H5VL_ATTR_DELETE_BY_IDX:
        {
            H5VL_attr_delete_by_idx_args_t* del_by_idx_args = &args->args.delete_by_idx; /* Arguments to delete_by_idx operation */

            if (H5VL_OBJECT_BY_NAME == loc_params->type) {
                /* Delete the attribute from the location */
                if (H5A__delete_by_idx(&loc, loc_params->loc_data.loc_by_name.name, del_by_idx_args->idx_type, del_by_idx_args->order, del_by_idx_args->n) < 0) {
                    HGOTO_ERROR(H5E_ATTR, H5E_CANTDELETE, FAIL, "unable to delete attribute");
                }
            } /* end if */
            else {
    ...
    case H5VL_ATTR_EXISTS:
        {
            if (loc_params->type == H5VL_OBJECT_BY_SELF) {
                ...
                /* Check if the attribute exists */
                if (H5A__exists_by_name(loc, loc_params->loc_data.loc_by_name.name, args->args.exists.name, args->args.exists.exists) < 0) {
                    HGOTO_ERROR(H5E_ATTR, H5E_CANTGET, FAIL, "unable to determine if attribute exists");
                }
            } /* end else-if */
            else {
    ...
    case H5VL_ATTR_ITER:
        {
            ...
            if ((ret_value = H5A__iterate(&loc, loc_name, iter_args->idx_type, iter_args->order, iter_args->idx, iter_args->op, iter_args->op_data)) < 0) {
    ...
    case H5VL_ATTR_RENAME:
        {
            if (loc_params->type == H5VL_OBJECT_BY_SELF) { /* H5Arename */
                ...
                /* Call attribute rename routine */
                if (H5A__rename_by_name(loc, loc_params->loc_data.loc_by_name.name, args->args.rename.old_name, args->args.rename.new_name) < 0) {
                    HGOTO_ERROR(H5E_ATTR, H5E_CANTRENAME, FAIL, "can't rename attribute");
                }
            } /* end else-if */
            else {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_attr_optional(void H5_ATTR_UNUSED* obj, H5VL_optional_args_t* args, hid_t H5_ATTR_UNUSED dxpl_id, void H5_ATTR_UNUSED** req)
{
    ...
    default: HGOTO_ERROR(H5E_VOL, H5E_UNSUPPORTED, FAIL, "invalid optional operation");
    } /* end switch */
