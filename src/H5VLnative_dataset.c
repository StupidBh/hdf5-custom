...
static herr_t H5VL__native_dataset_io_setup(size_t count,
                                            void* obj[],
                                            hid_t mem_type_id[],
                                            hid_t mem_space_id[],
                                            hid_t file_space_id[],
    ...
    for (i = 0; i < count; i++) {
        ...
        if (H5S_PLIST == file_space_id[i] && dinfo[i].file_space) {
            if (H5S_select_all(dinfo[i].file_space, true) < 0) {
                HDONE_ERROR(H5E_DATASET, H5E_CANTRELEASE, FAIL, "unable to release file dataspace selection for H5S_PLIST");
            }
        }
    }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_dataset_read(size_t count,
                                 void* obj[],
                                 hid_t mem_type_id[],
                                 hid_t mem_space_id[],
                                 hid_t file_space_id[],
                                 ...
                                 void H5_ATTR_UNUSED** req)
{
    ...
    if (H5VL__native_dataset_io_setup(count, obj, mem_type_id, mem_space_id, file_space_id, dxpl_id, (H5_flexible_const_ptr_t*)buf, dinfo) < 0) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_dataset_write(size_t count,
                                  void* obj[],
                                  hid_t mem_type_id[],
                                  hid_t mem_space_id[],
                                  hid_t file_space_id[],
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_dataset_get(void* obj, H5VL_dataset_get_args_t* args, hid_t H5_ATTR_UNUSED dxpl_id, void H5_ATTR_UNUSED** req)
{
    ...
    default: HGOTO_ERROR(H5E_VOL, H5E_CANTGET, FAIL, "can't get this type of information from dataset");
    } /* end switch */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_dataset_specific(void* obj, H5VL_dataset_specific_args_t* args, hid_t H5_ATTR_UNUSED dxpl_id, void H5_ATTR_UNUSED** req)
{
    H5D_t* dset = (H5D_t*)obj;
    herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_dataset_optional(void* obj, H5VL_optional_args_t* args, hid_t dxpl_id, void H5_ATTR_UNUSED** req)
{
    ...
    case H5VL_NATIVE_DATASET_CHUNK_READ:
        {
            ...
            /* Read the raw chunk */
            if (H5D__chunk_direct_read(dset, offset_copy, &chunk_read_args->filters, chunk_read_args->buf, chunk_read_args->buf_size) < 0) {
