...
static size_t H5Z__filter_nbit(unsigned flags, size_t cd_nelmts, const unsigned cd_values[], size_t nbytes, size_t* buf_size, void** buf);
...
static herr_t H5Z__set_parms_atomic(const H5T_t* type, unsigned* cd_values_index, unsigned cd_values[], bool* need_not_compress);
static herr_t H5Z__set_parms_array(const H5T_t* type, unsigned* cd_values_index, unsigned cd_values[], bool* need_not_compress);
static herr_t H5Z__set_parms_compound(const H5T_t* type, unsigned* cd_values_index, unsigned cd_values[], bool* need_not_compress);

static void H5Z__nbit_next_byte(size_t* j, size_t* buf_len);
static void H5Z__nbit_decompress_one_byte(unsigned char* data,
                                          size_t data_offset,
                                          unsigned k,
                                          unsigned begin_i,
                                          unsigned end_i,
                                          ...
                                          size_t datatype_len);
static void H5Z__nbit_compress_one_byte(const unsigned char* data,
                                        size_t data_offset,
                                        unsigned k,
                                        unsigned begin_i,
                                        unsigned end_i,
                                        ...
                                        const parms_atomic* p,
                                        size_t datatype_len);
static void H5Z__nbit_decompress_one_nooptype(unsigned char* data, size_t data_offset, const unsigned char* buffer, size_t* j, size_t* buf_len, unsigned size);
static void H5Z__nbit_decompress_one_atomic(unsigned char* data, size_t data_offset, unsigned char* buffer, size_t* j, size_t* buf_len, const parms_atomic* p);
static herr_t H5Z__nbit_decompress_one_array(unsigned char* data,
                                             size_t data_offset,
                                             unsigned char* buffer,
                                             size_t* j,
                                             size_t* buf_len,
...
static herr_t H5Z__nbit_decompress_one_compound(unsigned char* data,
                                                size_t data_offset,
                                                unsigned char* buffer,
                                                size_t* j,
                                                size_t* buf_len,
                                                const unsigned parms[],
                                                unsigned* parms_index);
static herr_t H5Z__nbit_decompress(unsigned char* data, unsigned d_nelmts, unsigned char* buffer, const unsigned parms[]);
static void H5Z__nbit_compress_one_nooptype(const unsigned char* data, size_t data_offset, unsigned char* buffer, size_t* j, size_t* buf_len, unsigned size);
static void H5Z__nbit_compress_one_array(unsigned char* data,
                                         size_t data_offset,
                                         unsigned char* buffer,
                                         size_t* j,
                                         size_t* buf_len,
                                         const unsigned parms[],
                                         unsigned* parms_index);
static void H5Z__nbit_compress_one_compound(unsigned char* data,
                                            size_t data_offset,
                                            unsigned char* buffer,
                                            size_t* j,
                                            size_t* buf_len,
                                            const unsigned parms[],
                                            unsigned* parms_index);
static void H5Z__nbit_compress(unsigned char* data, unsigned d_nelmts, unsigned char* buffer, size_t* buffer_size, const unsigned parms[]);
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5Z__can_apply_nbit(hid_t H5_ATTR_UNUSED dcpl_id, hid_t type_id, hid_t H5_ATTR_UNUSED space_id)
{
...
static herr_t H5Z__calc_parms_array(const H5T_t* type, size_t* cd_values_actual_nparms)
{
    ...
    *cd_values_actual_nparms += 1;
    ...
    case H5T_INTEGER:
    case H5T_FLOAT  : H5Z__calc_parms_atomic(cd_values_actual_nparms); break;
...
static herr_t H5Z__set_parms_atomic(const H5T_t* type, unsigned* cd_values_index, unsigned cd_values[], bool* need_not_compress)
{
    ...
    switch (dtype_order) {
    case H5T_ORDER_LE: /* Little-endian byte order */ cd_values[(*cd_values_index)++] = H5Z_NBIT_ORDER_LE; break;

    case H5T_ORDER_BE: /* Big-endian byte order */ cd_values[(*cd_values_index)++] = H5Z_NBIT_ORDER_BE; break;
                                          ...
                                          size_t datatype_len)
{
    ...
    if (*buf_len > dat_len) {
        data[data_offset + k] = (unsigned char)(((unsigned)(val >> (*buf_len - dat_len)) & (unsigned)(~((unsigned)(~0) << dat_len))) << dat_offset);
        *buf_len -= dat_len;
    }
    else {
        data[data_offset + k] = (unsigned char)(((val & ~((unsigned)(~0) << *buf_len)) << (dat_len - *buf_len)) << dat_offset);
        dat_len -= *buf_len;
...
static void H5Z__nbit_decompress_one_atomic(unsigned char* data, size_t data_offset, unsigned char* buffer, size_t* j, size_t* buf_len, const parms_atomic* p)
{
    ...
    if (p->order == H5Z_NBIT_ORDER_LE) { /* little endian */
        ...
        for (k = (int)begin_i; k >= (int)end_i; k--) {
            H5Z__nbit_decompress_one_byte(data, data_offset, (unsigned)k, begin_i, end_i, buffer, j, buf_len, p, datatype_len);
        }
    }
    else { /* big endian */
        /* Sanity check */
                                             ...
                                             unsigned* parms_index)
{
    ...
    case H5Z_NBIT_ATOMIC:
        ...
        n = total_size / p.size;
        for (i = 0; i < n; i++) {
            H5Z__nbit_decompress_one_atomic(data, data_offset + i * (size_t)p.size, buffer, j, buf_len, &p);
        }
        break;
    ...
    case H5Z_NBIT_ARRAY:
        ...
        for (i = 0; i < n; i++) {
            if (H5Z__nbit_decompress_one_array(data, data_offset + i * (size_t)base_size, buffer, j, buf_len, parms, parms_index) < 0) {
    ...
    case H5Z_NBIT_COMPOUND:
        ...
        begin_index = *parms_index;
        for (i = 0; i < n; i++) {
            if (H5Z__nbit_decompress_one_compound(data, data_offset + i * (size_t)base_size, buffer, j, buf_len, parms, parms_index) < 0) {
                                                ...
                                                unsigned* parms_index)
{
    ...
    for (i = 0; i < nmembers; i++) {
        ...
        case H5Z_NBIT_ARRAY:
            if (H5Z__nbit_decompress_one_array(data, data_offset + member_offset, buffer, j, buf_len, parms, parms_index) < 0) {
                HGOTO_ERROR(H5E_PLINE, H5E_CANTFILTER, FAIL, "can't decompress array");
            }
            break;

        case H5Z_NBIT_COMPOUND:
            if (H5Z__nbit_decompress_one_compound(data, data_offset + member_offset, buffer, j, buf_len, parms, parms_index) < 0) {
                HGOTO_ERROR(H5E_PLINE, H5E_CANTFILTER, FAIL, "can't decompress compound");
            }
            break;
        ...
        case H5Z_NBIT_NOOPTYPE:
            /* Advance past member size */
            (*parms_index)++;
            H5Z__nbit_decompress_one_nooptype(data, data_offset + member_offset, buffer, j, buf_len, member_size);
            break;
...
static void H5Z__nbit_compress_one_atomic(unsigned char* data, size_t data_offset, unsigned char* buffer, size_t* j, size_t* buf_len, const parms_atomic* p)
{
    /* begin_i: the index of byte having first significant bit
       end_i: the index of byte having last significant bit */
    int k;
    ...
    if (p->order == H5Z_NBIT_ORDER_LE) { /* little endian */
        ...
        for (k = (int)begin_i; k >= (int)end_i; k--) {
            H5Z__nbit_compress_one_byte(data, data_offset, (unsigned)k, begin_i, end_i, buffer, j, buf_len, p, datatype_len);
        }
    }
    else { /* big endian */
        /* Sanity check */
                                         ...
                                         unsigned* parms_index)
{
    ...
    case H5Z_NBIT_ARRAY:
        ...
        begin_index = *parms_index;
        for (i = 0; i < n; i++) {
            H5Z__nbit_compress_one_array(data, data_offset + i * (size_t)base_size, buffer, j, buf_len, parms, parms_index);
            *parms_index = begin_index;
        }
        break;
    ...
    case H5Z_NBIT_COMPOUND:
        ...
        begin_index = *parms_index;
        for (i = 0; i < n; i++) {
            H5Z__nbit_compress_one_compound(data, data_offset + i * (size_t)base_size, buffer, j, buf_len, parms, parms_index);
            *parms_index = begin_index;
        }
        break;
                                            ...
                                            unsigned* parms_index)
{
    ...
    for (i = 0; i < nmembers; i++) {
        ...
        case H5Z_NBIT_ARRAY: H5Z__nbit_compress_one_array(data, data_offset + member_offset, buffer, j, buf_len, parms, parms_index); break;

        case H5Z_NBIT_COMPOUND: H5Z__nbit_compress_one_compound(data, data_offset + member_offset, buffer, j, buf_len, parms, parms_index); break;
