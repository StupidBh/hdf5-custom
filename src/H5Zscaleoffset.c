...
static enum H5Z_scaleoffset_t H5Z__scaleoffset_get_type(unsigned dtype_class, unsigned dtype_size, unsigned dtype_sign);
static herr_t H5Z__scaleoffset_set_parms_fillval(H5P_genplist_t* dcpl_plist, H5T_t* type, enum H5Z_scaleoffset_t scale_type, unsigned cd_values[], int need_convert);
static herr_t H5Z__set_local_scaleoffset(hid_t dcpl_id, hid_t type_id, hid_t space_id);
static size_t H5Z__filter_scaleoffset(unsigned flags, size_t cd_nelmts, const unsigned cd_values[], size_t nbytes, size_t* buf_size, void** buf);
...
static void H5Z__scaleoffset_precompress_i(void* data,
                                           unsigned d_nelmts,
                                           enum H5Z_scaleoffset_t type,
                                           unsigned filavail,
                                           const unsigned cd_values[],
                                           uint32_t* minbits,
                                           unsigned long long* minval);
static void H5Z__scaleoffset_postdecompress_i(void* data,
                                              unsigned d_nelmts,
                                              enum H5Z_scaleoffset_t type,
                                              unsigned filavail,
                                              const unsigned cd_values[],
                                              uint32_t minbits,
                                              unsigned long long minval);
static herr_t H5Z__scaleoffset_precompress_fd(void* data,
                                              unsigned d_nelmts,
                                              enum H5Z_scaleoffset_t type,
                                              unsigned filavail,
                                              const unsigned cd_values[],
                                              ...
                                              unsigned long long* minval,
                                              double D_val);
static herr_t H5Z__scaleoffset_postdecompress_fd(void* data,
                                                 unsigned d_nelmts,
                                                 enum H5Z_scaleoffset_t type,
                                                 unsigned filavail,
                                                 const unsigned cd_values[],
...
static herr_t H5Z__scaleoffset_decompress_one_byte(unsigned char* data,
                                                   size_t data_offset,
                                                   unsigned k,
                                                   unsigned begin_i,
                                                   ...
                                                   parms_atomic p,
                                                   unsigned dtype_len);
static void H5Z__scaleoffset_compress_one_byte(const unsigned char* data,
                                               size_t data_offset,
                                               unsigned k,
                                               unsigned begin_i,
                                               ...
                                               parms_atomic p,
                                               unsigned dtype_len);
static herr_t H5Z__scaleoffset_decompress_one_atomic(unsigned char* data,
                                                     size_t data_offset,
                                                     unsigned char* buffer,
                                                     size_t buf_size,
                                                     size_t* j,
                                                     unsigned* bits_to_fill,
                                                     parms_atomic p);
static void H5Z__scaleoffset_compress_one_atomic(unsigned char* data, size_t data_offset, unsigned char* buffer, size_t* j, unsigned* bits_to_fill, parms_atomic p);
static herr_t H5Z__scaleoffset_decompress(unsigned char* data, unsigned d_nelmts, unsigned char* buffer, size_t buf_size, parms_atomic p);
static void H5Z__scaleoffset_compress(unsigned char* data, unsigned d_nelmts, unsigned char* buffer, size_t buffer_size, parms_atomic p);
...
#define H5Z_scaleoffset_check_3(i, type, pow_fun, round_fun, max, min, minbits, D_val)                                                                          \
    {                                                                                                                                                           \
        if (sizeof(type) == sizeof(int)) {                                                                                                                      \
            if (round_fun(max * pow_fun((type)10, (type)D_val) - min * pow_fun((type)10, (type)D_val)) > pow_fun((type)2, (type)(sizeof(int) * 8 - 1))) {       \
                *minbits = sizeof(int) * 8;                                                                                                                     \
                goto done;                                                                                                                                      \
            }                                                                                                                                                   \
        }                                                                                                                                                       \
        else if (sizeof(type) == sizeof(long)) {                                                                                                                \
            if (round_fun(max * pow_fun((type)10, (type)D_val) - min * pow_fun((type)10, (type)D_val)) > pow_fun((type)2, (type)(sizeof(long) * 8 - 1))) {      \
                *minbits = sizeof(long) * 8;                                                                                                                    \
                goto done;                                                                                                                                      \
            }                                                                                                                                                   \
        }                                                                                                                                                       \
        else if (sizeof(type) == sizeof(long long)) {                                                                                                           \
            if (round_fun(max * pow_fun((type)10, (type)D_val) - min * pow_fun((type)10, (type)D_val)) > pow_fun((type)2, (type)(sizeof(long long) * 8 - 1))) { \
    ...
    do {                                                                                                                                                     \
        ...
        if (filavail == H5Z_SCALEOFFSET_FILL_DEFINED) {   /* fill value defined */                                                                           \
            ...
            if (*minbits == H5Z_SO_INT_MINBITS_DEFAULT) { /* minbits not set yet, calculate max, min, and minbits */                                         \
                H5Z_scaleoffset_max_min_1(i, d_nelmts, buf, filval, max, min) H5Z_scaleoffset_check_1(type, max, min, minbits) span = (type)(max - min + 1); \
...
#define H5Z_scaleoffset_precompress_2(type, data, d_nelmts, filavail, cd_values, minbits, minval)                                                             \
    do {                                                                                                                                                      \
        ...
        if (filavail == H5Z_SCALEOFFSET_FILL_DEFINED) {   /* fill value defined */                                                                            \
            H5Z_scaleoffset_get_filval_1(type, cd_values, filval);                                                                                            \
            if (*minbits == H5Z_SO_INT_MINBITS_DEFAULT) { /* minbits not set yet, calculate max, min, and minbits */                                          \
                H5Z_scaleoffset_max_min_1(i, d_nelmts, buf, filval, max, min) H5Z_scaleoffset_check_2(type, max, min, minbits) span =                         \
                    (unsigned type)(max - min + 1);                                                                                                           \
                *minbits = H5Z__scaleoffset_log2((unsigned long long)(span + 1));                                                                             \
            }                                                                                                                                                 \
            else                              /* minbits already set, only calculate min */                                                                   \
                H5Z_scaleoffset_min_1(i, d_nelmts, buf, filval, min);                                                                                         \
            ...
            if (*minbits == H5Z_SO_INT_MINBITS_DEFAULT) { /* minbits not set yet, calculate max, min, and minbits */                                          \
                H5Z_scaleoffset_max_min_2(i, d_nelmts, buf, max, min) H5Z_scaleoffset_check_2(type, max, min, minbits) span = (unsigned type)(max - min + 1); \
                *minbits = H5Z__scaleoffset_log2((unsigned long long)span);                                                                                   \
            }                                                                                                                                                 \
            else                                                                               /* minbits already set, only calculate min */                  \
                H5Z_scaleoffset_min_2(i, d_nelmts, buf, min) if (*minbits != sizeof(type) * 8) /* change values if minbits != full precision */               \
        ...
        if (sizeof(type) == sizeof(int))                                                                                                         \
            for (i = 0; i < d_nelmts; i++) {                                                                                                     \
                if (abs_fun(buf[i] - filval) < pow_fun((type)10, (type) - D_val))                                                                \
                    ...
                    *(int*)((void*)&buf[i]) = (int)lround_fun(buf[i] * pow_fun((type)10, (type)D_val) - min * pow_fun((type)10, (type)D_val));   \
            }                                                                                                                                    \
        else if (sizeof(type) == sizeof(long))                                                                                                   \
            for (i = 0; i < d_nelmts; i++) {                                                                                                     \
                if (abs_fun(buf[i] - filval) < pow_fun((type)10, (type) - D_val))                                                                \
                    ...
                    *(long*)((void*)&buf[i]) = lround_fun(buf[i] * pow_fun((type)10, (type)D_val) - min * pow_fun((type)10, (type)D_val));       \
            }                                                                                                                                    \
        else if (sizeof(type) == sizeof(long long))                                                                                              \
            for (i = 0; i < d_nelmts; i++) {                                                                                                     \
                if (abs_fun(buf[i] - filval) < pow_fun((type)10, (type) - D_val))                                                                \
                    ...
                    *(long long*)((void*)&buf[i]) = llround_fun(buf[i] * pow_fun((type)10, (type)D_val) - min * pow_fun((type)10, (type)D_val)); \
            }                                                                                                                                    \
        else                                                                                                                                     \
            HGOTO_ERROR(H5E_PLINE, H5E_BADTYPE, FAIL, "cannot find matched integer datatype");                                                   \
    }
    ...
    do {                                                                                                                                                       \
        ...
        *minval = 0;                                                                                                                                           \
        if (filavail == H5Z_SCALEOFFSET_FILL_DEFINED) { /* fill value defined */                                                                               \
            ...
            span = (unsigned long long)(llround_fun(max * pow_fun((type)10, (type)D_val) - min * pow_fun((type)10, (type)D_val)) + 1);                         \
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5Z__can_apply_scaleoffset(hid_t H5_ATTR_UNUSED dcpl_id, hid_t type_id, hid_t H5_ATTR_UNUSED space_id)
{
...
static herr_t H5Z__set_local_scaleoffset(hid_t dcpl_id, hid_t type_id, hid_t space_id)
{
    ...
    switch (dtype_class) {
    case H5T_INTEGER: cd_values[H5Z_SCALEOFFSET_PARM_CLASS] = H5Z_SCALEOFFSET_CLS_INTEGER; break;

    case H5T_FLOAT: cd_values[H5Z_SCALEOFFSET_PARM_CLASS] = H5Z_SCALEOFFSET_CLS_FLOAT; break;
    ...
    if (dtype_class == H5T_INTEGER) {
        ...
        switch (dtype_sign) {
        case H5T_SGN_NONE: cd_values[H5Z_SCALEOFFSET_PARM_SIGN] = H5Z_SCALEOFFSET_SGN_NONE; break;

        case H5T_SGN_2: cd_values[H5Z_SCALEOFFSET_PARM_SIGN] = H5Z_SCALEOFFSET_SGN_2; break;

        case H5T_SGN_ERROR:
        case H5T_NSGN:
        default           : HGOTO_ERROR(H5E_PLINE, H5E_BADTYPE, FAIL, "bad integer sign");
        } /* end switch */
    } /* end if */
    ...
    switch (dtype_order) {
    case H5T_ORDER_LE: /* Little-endian byte order */ cd_values[H5Z_SCALEOFFSET_PARM_ORDER] = H5Z_SCALEOFFSET_ORDER_LE; break;

    case H5T_ORDER_BE: /* Big-endian byte order */ cd_values[H5Z_SCALEOFFSET_PARM_ORDER] = H5Z_SCALEOFFSET_ORDER_BE; break;

    case H5T_ORDER_ERROR:
    case H5T_ORDER_VAX:
    case H5T_ORDER_MIXED:
    case H5T_ORDER_NONE:
    default             : HGOTO_ERROR(H5E_PLINE, H5E_BADTYPE, FAIL, "bad datatype endianness order");
    } /* end switch */
    ...
    if (status == H5D_FILL_VALUE_UNDEFINED) {
        ...
        if ((scale_type =
                 H5Z__scaleoffset_get_type(cd_values[H5Z_SCALEOFFSET_PARM_CLASS], cd_values[H5Z_SCALEOFFSET_PARM_SIZE], cd_values[H5Z_SCALEOFFSET_PARM_SIGN])) ==
            0) {
...
static size_t H5Z__filter_scaleoffset(unsigned flags, size_t cd_nelmts, const unsigned cd_values[], size_t nbytes, size_t* buf_size, void** buf)
{
    ...
    case H5T_ORDER_MIXED:
    case H5T_ORDER_NONE:
    default             : HGOTO_ERROR(H5E_PLINE, H5E_BADTYPE, 0, "bad H5T_NATIVE_INT endianness order");
    } /* end switch */
    ...
    if (flags & H5Z_FLAG_REVERSE) {
         ...
         */
        minval_size = sizeof(unsigned long long) <= ((unsigned char*)*buf)[4] ? sizeof(unsigned long long) : ((unsigned char*)*buf)[4];
        minval = 0;
        ...
        if (minbits != 0) {
            if (H5Z__scaleoffset_decompress(outbuf, d_nelmts, (unsigned char*)(*buf) + buf_offset, *buf_size - buf_offset, p)) {
        ...
        if (dtype_class == H5Z_SCALEOFFSET_CLS_FLOAT) {
            if (scale_type == 0) { /* variable-minimum-bits method */
                if (H5Z__scaleoffset_postdecompress_fd(outbuf, d_nelmts, type, filavail, cd_values, minbits, minval, D_val) == FAIL) {
    ...
    /* output; compress */
        ...
        if (dtype_class == H5Z_SCALEOFFSET_CLS_FLOAT) {
            if (scale_type == 0) { /* variable-minimum-bits method */
                if (H5Z__scaleoffset_precompress_fd(*buf, d_nelmts, type, filavail, cd_values, &minbits, &minval, D_val) == FAIL) {
 ...
 */
static unsigned H5Z__scaleoffset_log2(unsigned long long num)
{
    unsigned v = 0;
                                           ...
                                           unsigned long long* minval)
{
    if (type == t_uchar) {
        ...
        if (filavail == H5Z_SCALEOFFSET_FILL_DEFINED) {   /* fill value defined */
            ...
            if (*minbits == H5Z_SO_INT_MINBITS_DEFAULT) { /* minbits not set yet, calculate max, min, and minbits */
                H5Z_scaleoffset_max_min_1(i, d_nelmts, buf, filval, max, min) if ((unsigned char)(max - min) > (unsigned char)(~(unsigned char)0 - 2))
                {
                ...
                *minbits = H5Z__scaleoffset_log2((unsigned long long)(span + 1));
                ...
                H5Z_scaleoffset_min_1(i, d_nelmts, buf, filval, min) if (*minbits != sizeof(signed char) * 8) /* change values if minbits != full precision */
                    for (i = 0; i < d_nelmts; i++) buf[i] = (signed char)((buf[i] == filval) ? (((unsigned char)1 << *minbits) - 1) : (buf[i] - min));
            }
        }
        else {                                            /* fill value undefined */
            if (*minbits == H5Z_SO_INT_MINBITS_DEFAULT) { /* minbits not set yet, calculate max, min, and minbits */
                H5Z_scaleoffset_max_min_2(i, d_nelmts, buf, max, min) if ((unsigned char)(max - min) > (unsigned char)(~(unsigned char)0 - 2))
                {
                    *minbits = sizeof(signed char) * 8;
                    *minval = (unsigned long long)min;
                    return;
                ...
                *minbits = H5Z__scaleoffset_log2((unsigned long long)span);
                ...
                H5Z_scaleoffset_min_2(i, d_nelmts, buf, min) if (*minbits != sizeof(signed char) * 8) /* change values if minbits != full precision */
                                              ...
                                              double D_val)
{
    ...
    if (type == t_float) {
        H5Z_scaleoffset_precompress_3(float, powf, fabsf, roundf, lroundf, llroundf, data, d_nelmts, filavail, cd_values, minbits, minval, D_val);
    }
    else if (type == t_double) {
        H5Z_scaleoffset_precompress_3(double, pow, fabs, round, lround, llround, data, d_nelmts, filavail, cd_values, minbits, minval, D_val);
    }
                                                 ...
                                                 double D_val)
{
    ...
    if (type == t_float) {
        H5Z_scaleoffset_postdecompress_3(float, powf, data, d_nelmts, filavail, cd_values, minbits, sminval, D_val);
    }
    else if (type == t_double) {
        H5Z_scaleoffset_postdecompress_3(double, pow, data, d_nelmts, filavail, cd_values, minbits, sminval, D_val);
    }
                                                   ...
                                                   unsigned dtype_len)
{
    ...
    if (*bits_to_fill > bits_to_copy) {
        data[data_offset + k] = (unsigned char)((unsigned)(val >> (*bits_to_fill - bits_to_copy)) & (unsigned)(~((unsigned)~0 << bits_to_copy)));
        *bits_to_fill -= bits_to_copy;
    } /* end if */
    else {
        data[data_offset + k] = (unsigned char)((val & ~((unsigned)(~0) << *bits_to_fill)) << (bits_to_copy - *bits_to_fill));
        ...
        val = buffer[*j];
        data[data_offset + k] |= (unsigned char)((unsigned)(val >> (*bits_to_fill - bits_to_copy)) & ~((unsigned)(~0) << bits_to_copy));
        *bits_to_fill -= bits_to_copy;
    } /* end else */
                                                     ...
                                                     parms_atomic p)
{
    ...
    if (p.mem_order == H5Z_SCALEOFFSET_ORDER_LE) { /* little endian */
        ...
        for (k = (int)begin_i; k >= 0; k--) {
            if (H5Z__scaleoffset_decompress_one_byte(data, data_offset, (unsigned)k, begin_i, buffer, buf_size, j, bits_to_fill, p, dtype_len)) {
                                               ...
                                               unsigned dtype_len)
{
    ...
    if (*bits_to_fill > bits_to_copy) {
        buffer[*j] |= (unsigned char)((val & ~((unsigned)(~0) << bits_to_copy)) << (*bits_to_fill - bits_to_copy));
        *bits_to_fill -= bits_to_copy;
    }
    else {
        buffer[*j] |= (unsigned char)((unsigned)(val >> (bits_to_copy - *bits_to_fill)) & ~((unsigned)(~0) << *bits_to_fill));
        bits_to_copy -= *bits_to_fill;
        H5Z__scaleoffset_next_byte(j, bits_to_fill);
        if (bits_to_copy == 0) {
            return;
...
static void H5Z__scaleoffset_compress_one_atomic(unsigned char* data, size_t data_offset, unsigned char* buffer, size_t* j, unsigned* bits_to_fill, parms_atomic p)
{
    ...
    if (p.mem_order == H5Z_SCALEOFFSET_ORDER_LE) { /* little endian */
        ...
        for (k = (int)begin_i; k >= 0; k--) {
            H5Z__scaleoffset_compress_one_byte(data, data_offset, (unsigned)k, begin_i, buffer, j, bits_to_fill, p, dtype_len);
        }
    }
    else { /* big endian */
        assert(p.mem_order == H5Z_SCALEOFFSET_ORDER_BE);
