    ...
    {                                                                                                                                               \
        if ((tree->lchild->type == H5Z_XFORM_INTEGER) && (tree->rchild->type == H5Z_XFORM_INTEGER)) {                                               \
            ...
            tree->value.float_val = ((tree->lchild->type == H5Z_XFORM_FLOAT) ? tree->lchild->value.float_val : (double)tree->lchild->value.int_val) \
                OP((tree->rchild->type == H5Z_XFORM_FLOAT) ? tree->rchild->value.float_val : (double)tree->rchild->value.int_val);                  \
            H5MM_xfree(tree->lchild);                                                                                                               \
            H5MM_xfree(tree->rchild);                                                                                                               \
            tree->lchild = NULL;                                                                                                                    \
            tree->rchild = NULL;                                                                                                                    \
 ...
 *-------------------------------------------------------------------------
 */
static void H5Z__unget_token(H5Z_token* current)
{
...
static H5Z_token* H5Z__get_token(H5Z_token* current)
{
    ...
    while (current->tok_begin[0] != '\0') {
        if (isspace(current->tok_begin[0])) {
            ...
            default : current->tok_type = H5Z_XFORM_ERROR; HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, current, "Unknown H5Z_token in data transform expression ");
            }
...
static H5Z_node* H5Z__parse_expression(H5Z_token* current, H5Z_datval_ptrs* dat_val_pointers)
{
    ...
    for (;;) {
        ...
        case H5Z_XFORM_RPAREN: H5Z__unget_token(current); HGOTO_DONE(expr);

        case H5Z_XFORM_END: HGOTO_DONE(expr);
        ...
        default               : H5Z__xform_destroy_parse_tree(expr); HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, NULL, "Error parsing data transform expression");
        }
    }
...
static H5Z_node* H5Z__parse_term(H5Z_token* current, H5Z_datval_ptrs* dat_val_pointers)
{
    ...
    for (;;) {
        ...
        case H5Z_XFORM_RPAREN: H5Z__unget_token(current); HGOTO_DONE(term);

        case H5Z_XFORM_END: HGOTO_DONE(term);
        ...
        case H5Z_XFORM_PLUS:
        case H5Z_XFORM_MINUS:
        case H5Z_XFORM_LPAREN : H5Z__unget_token(current); HGOTO_DONE(term);

        case H5Z_XFORM_ERROR:
        default             : H5Z__xform_destroy_parse_tree(term); HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, NULL, "bad transform type passed to data transform expression");
        } /* end switch */
    } /* end for */
...
static H5Z_node* H5Z__parse_factor(H5Z_token* current, H5Z_datval_ptrs* dat_val_pointers)
{
    ...
    case H5Z_XFORM_PLUS:
        ...
        if (new_node) {
            if (new_node->type != H5Z_XFORM_INTEGER && new_node->type != H5Z_XFORM_FLOAT && new_node->type != H5Z_XFORM_SYMBOL) {
                H5Z__xform_destroy_parse_tree(new_node);
                H5Z__xform_destroy_parse_tree(factor);
                HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, NULL, "Error parsing data transform expression");
            }
    ...
    case H5Z_XFORM_END: break;

    case H5Z_XFORM_MULT:
    case H5Z_XFORM_DIVIDE:
    case H5Z_XFORM_ERROR:
    default              : HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, NULL, "Invalid token while parsing data transform expression");
    }
...
static herr_t H5Z__xform_eval_full(H5Z_node* tree, const size_t array_size, const hid_t array_type, H5Z_result* res)
{
    ...
    if (tree->type == H5Z_XFORM_INTEGER) {
        ...
        switch (tree->type) {
        case H5Z_XFORM_PLUS: H5Z_XFORM_TYPE_OP(resl, resr, array_type, +, array_size) break;

        case H5Z_XFORM_MINUS: H5Z_XFORM_TYPE_OP(resl, resr, array_type, -, array_size) break;

        case H5Z_XFORM_MULT: H5Z_XFORM_TYPE_OP(resl, resr, array_type, *, array_size) break;

        case H5Z_XFORM_DIVIDE: H5Z_XFORM_TYPE_OP(resl, resr, array_type, /, array_size) break;
        ...
        case H5Z_XFORM_RPAREN:
        case H5Z_XFORM_END:
        default               : HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL, "Invalid expression tree");
        } /* end switch */
...
static bool H5Z__op_is_numbs2(H5Z_node* _tree)
{
    ...
    if ((!_tree->lchild && ((_tree->rchild->type == H5Z_XFORM_INTEGER) || (_tree->rchild->type == H5Z_XFORM_FLOAT))) ||
        ((_tree->lchild && ((_tree->lchild->type == H5Z_XFORM_INTEGER) || (_tree->lchild->type == H5Z_XFORM_FLOAT))) &&
         (_tree->rchild && ((_tree->rchild->type == H5Z_XFORM_INTEGER) || (_tree->rchild->type == H5Z_XFORM_FLOAT))))) {
        ret_value = true;
    }
    ...
    for (i = 0; i < strlen(expr); i++) {
        if (isalpha(expr[i])) {
            if ((i > 0) && (i < (strlen(expr) - 1))) {
                if (((expr[i] == 'E') || (expr[i] == 'e')) && (isdigit(expr[i - 1]) || (expr[i - 1] == '.')) &&
...
herr_t H5Z_xform_copy(H5Z_data_xform_t** data_xform_prop)
{
    ...
    if (*data_xform_prop) {
        ...
        if ((new_data_xform_prop->parse_root =
                 (H5Z_node*)H5Z__xform_copy_tree((*data_xform_prop)->parse_root, (*data_xform_prop)->dat_val_pointers, new_data_xform_prop->dat_val_pointers)) ==
            NULL) {
 ...
 *-------------------------------------------------------------------------
 */
bool H5Z_xform_noop(const H5Z_data_xform_t* data_xform_prop)
{
    ...
    if (data_xform_prop) {
        ...
        if ((strlen(data_xform_prop->xform_exp) == 1) && data_xform_prop->dat_val_pointers && (data_xform_prop->dat_val_pointers->num_ptrs == 1)) {
            ret_value = true;
        } /* end if */
    } /* end if */
