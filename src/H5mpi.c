 ...
 *-------------------------------------------------------------------------
 */
H5_ATTR_PURE hsize_t H5_mpi_get_bigio_count(void)
{
...
herr_t H5_mpio_create_large_type(hsize_t num_elements, MPI_Aint stride_bytes, MPI_Datatype old_type, MPI_Datatype* new_type)
{
    ...
    else if (MPI_SUCCESS != (mpi_code = MPI_Type_create_hvector((int)bigio_count_g, 1, stride_bytes, old_type, &inner_type))) {
    ...
    else if (MPI_SUCCESS != (mpi_code = MPI_Type_create_hvector(num_big_types, 1, stride_bytes, inner_type, &outer_type))) {
        HMPI_GOTO_ERROR(FAIL, "MPI_Type_create_hvector failed", mpi_code)
    }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5_mpio_gatherv_alloc(void* send_buf,
                             int send_count,
                             ...
                             size_t* out_buf_num_entries)
{
    ...
    if (allgather) {
        if (MPI_SUCCESS != (mpi_code = MPI_Allgatherv(send_buf, send_count, send_type, recv_buf, recv_counts, displacements, recv_type, comm))) {
            HMPI_GOTO_ERROR(FAIL, "MPI_Allgatherv failed", mpi_code)
        }
    }
    else {
        if (MPI_SUCCESS != (mpi_code = MPI_Gatherv(send_buf, send_count, send_type, recv_buf, recv_counts, displacements, recv_type, root, comm))) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5_mpio_gatherv_alloc_simple(void* send_buf,
                                    int send_count,
                                    MPI_Datatype send_type,
                                    MPI_Datatype recv_type,
                                    bool allgather,
                                    ...
                                    size_t* out_buf_num_entries)
{
    ...
    if (allgather) {
        if (MPI_SUCCESS != (mpi_code = MPI_Allgather(&send_count, 1, MPI_INT, recv_counts_disps_array, 1, MPI_INT, comm))) {
            HMPI_GOTO_ERROR(FAIL, "MPI_Allgather failed", mpi_code)
        }
    }
    else {
        if (MPI_SUCCESS != (mpi_code = MPI_Gather(&send_count, 1, MPI_INT, recv_counts_disps_array, 1, MPI_INT, root, comm))) {
    ...
    /* Perform gather operation */
    if (H5_mpio_gatherv_alloc(send_buf,
                              send_count,
                              send_type,
                              recv_counts_disps_array,
                              &recv_counts_disps_array[mpi_size],
                              recv_type,
                              allgather,
                              root,
                              comm,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5_mpio_get_file_sync_required(MPI_File fh, bool* file_sync_required)
{
    MPI_Info info_used;
    int flag;
    char* sync_env_var;
    ...
    *file_sync_required = false;
    ...
    if (MPI_SUCCESS != MPI_Info_get(info_used, "romio_visibility_immediate", MPI_MAX_INFO_VAL - 1, value, &flag)) {
        HGOTO_ERROR(H5E_LIB, H5E_CANTGET, FAIL, "can't get MPI info");
    }
