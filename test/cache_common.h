...
typedef struct test_entry_t
{
    ...
    unsigned flush_dep_ndirty_chd;              /* Number of dirty flush dependency children (including grandchildren, etc.) */
...
#define CACHE_CONFIGS_EQUAL(a, b, cmp_set_init, cmp_init_size)                                                                                                     \
    (((a).version == (b).version) && ((a).rpt_fcn_enabled == (b).rpt_fcn_enabled) && ((a).open_trace_file == (b).open_trace_file) &&                               \
     ((a).close_trace_file == (b).close_trace_file) && (((a).open_trace_file == false) || (strcmp((a).trace_file_name, (b).trace_file_name) == 0)) &&              \
     ((a).evictions_enabled == (b).evictions_enabled) && ((!cmp_set_init) || ((a).set_initial_size == (b).set_initial_size)) &&                                    \
     ((!cmp_init_size) || ((a).initial_size == (b).initial_size)) && (H5_DBL_ABS_EQUAL((a).min_clean_fraction, (b).min_clean_fraction)) &&                         \
     ((a).max_size == (b).max_size) && ((a).min_size == (b).min_size) && ((a).epoch_length == (b).epoch_length) && ((a).incr_mode == (b).incr_mode) &&             \
     (H5_DBL_ABS_EQUAL((a).lower_hr_threshold, (b).lower_hr_threshold)) && (H5_DBL_ABS_EQUAL((a).increment, (b).increment)) &&                                     \
     ((a).apply_max_increment == (b).apply_max_increment) && ((a).max_increment == (b).max_increment) && ((a).flash_incr_mode == (b).flash_incr_mode) &&           \
     (H5_DBL_ABS_EQUAL((a).flash_multiple, (b).flash_multiple)) && (H5_DBL_ABS_EQUAL((a).flash_threshold, (b).flash_threshold)) &&                                 \
     ((a).decr_mode == (b).decr_mode) && (H5_DBL_ABS_EQUAL((a).upper_hr_threshold, (b).upper_hr_threshold)) && (H5_DBL_ABS_EQUAL((a).decrement, (b).decrement)) && \
     ((a).apply_max_decrement == (b).apply_max_decrement) && ((a).max_decrement == (b).max_decrement) &&                                                           \
     ((a).epochs_before_eviction == (b).epochs_before_eviction) && ((a).apply_empty_reserve == (b).apply_empty_reserve) &&                                         \
     (H5_DBL_ABS_EQUAL((a).empty_reserve, (b).empty_reserve)) && ((a).dirty_bytes_threshold == (b).dirty_bytes_threshold) &&                                       \
...
extern "C"
{
    ...
    H5TEST_DLL void add_flush_op(int target_type, int target_idx, int op_code, int type, int idx, bool flag, size_t size, unsigned* order);
    ...
    H5TEST_DLL void create_pinned_entry_dependency(H5F_t* file_ptr, int pinning_type, int pinning_idx, int pinned_type, int pinned_idx);
    ...
    H5TEST_DLL void verify_entry_status(H5C_t* cache_ptr, int tag, int num_entries, struct expected_entry_status expected[]);

    H5TEST_DLL void verify_unprotected(void);

    H5TEST_DLL void create_flush_dependency(int32_t parent_type, int32_t parent_idx, int32_t child_type, int32_t child_idx);

    H5TEST_DLL void destroy_flush_dependency(int32_t parent_type, int32_t parent_idx, int32_t child_type, int32_t child_idx);

    /*** H5AC level utility functions ***/

    H5TEST_DLL bool resize_configs_are_equal(const H5C_auto_size_ctl_t* a, const H5C_auto_size_ctl_t* b, bool compare_init);

    H5TEST_DLL void check_and_validate_cache_hit_rate(hid_t file_id, double* hit_rate_ptr, bool dump_data, int64_t min_accesses, double min_hit_rate);
    ...
    H5TEST_DLL void validate_mdc_config(hid_t file_id, H5AC_cache_config_t* ext_config_ptr, bool compare_init, int test_num);
