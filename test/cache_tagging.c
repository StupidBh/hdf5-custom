...
static unsigned check_link_iteration_tags(void)
{
    ...
    for (i = 0; i < 500; i++) {
        ...
        if ((did = H5Dcreate2(fid, dsetname, H5T_NATIVE_UCHAR, sid, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
            TEST_ERROR;
        }
        if (H5Dclose(did) < 0) {
            TEST_ERROR;
...
static unsigned check_dense_attribute_tags(void)
{
    ...
    if ((aid = H5Aopen_by_idx(did, ".", H5_INDEX_NAME, H5_ITER_INC, (hsize_t)4, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        TEST_ERROR;
    }
