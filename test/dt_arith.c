 ...
 */
#define INIT_FP_NORM(TYPE, SRC_MAX, SRC_MIN, SRC_MAX_10_EXP, SRC_MIN_10_EXP, SRC_SIZE, DST_SIZE, BUF, SAVED, NELMTS) \
    do {                                                                                                             \
        unsigned char *buf_p, *saved_p;                                                                              \
...
static int my_isinf(int endian, const unsigned char* val, size_t size, size_t mpos, size_t msize, size_t epos, size_t esize);
 ...
 *-------------------------------------------------------------------------
 */
static void fpe_handler(int H5_ATTR_UNUSED signo)
{
    SKIPPED();
...
static H5T_conv_ret_t except_func(H5T_conv_except_t except_type,
                                  hid_t H5_ATTR_UNUSED src_id,
                                  hid_t H5_ATTR_UNUSED dst_id,
                                  void H5_ATTR_UNUSED* src_buf,
                                  void* dst_buf,
                                  void* user_data)
{
...
static herr_t my_conv_int_float_func(hid_t H5_ATTR_UNUSED src_id,
                                     hid_t H5_ATTR_UNUSED dst_id,
                                     H5T_cdata_t H5_ATTR_UNUSED* cdata,
 ...
 *-------------------------------------------------------------------------
 */
static int test_hard_query(void)
{
    ...
    H5Tregister(H5T_PERS_HARD, "int_flt", H5T_NATIVE_INT, H5T_NATIVE_FLOAT, (H5T_conv_t)((void (*)(void))my_conv_int_float_func));
 ...
 *-------------------------------------------------------------------------
 */
static H5T_conv_ret_t expt_handle(H5T_conv_except_t except_type,
                                  hid_t H5_ATTR_UNUSED src_id,
                                  hid_t H5_ATTR_UNUSED dst_id,
                                  void H5_ATTR_UNUSED* src_buf,
                                  void* dst_buf,
                                  void* user_data)
{
    ...
    if (except_type == H5T_CONV_EXCEPT_RANGE_HI || except_type == H5T_CONV_EXCEPT_RANGE_LOW || except_type == H5T_CONV_EXCEPT_TRUNCATE) {
        if (*(int*)user_data == 0) {
 ...
 *-------------------------------------------------------------------------
 */
static int test_derived_flt(void)
{
    ...
    for (i = 0; i < nelmts; i++) {
         ...
         */
        if ((buf[i * src_size + 2] == 0x7f && saved_buf[i * src_size + 2] == 0x7f) || (buf[i * src_size + 2] == 0xff && saved_buf[i * src_size + 2] == 0xff)) {
            continue;
        }
 ...
 *-------------------------------------------------------------------------
 */
static int test_derived_integer(void)
{
    hid_t file = H5I_INVALID_HID, tid1 = H5I_INVALID_HID, tid2 = H5I_INVALID_HID;
    hid_t dxpl_id = H5I_INVALID_HID;
    char filename[1024];
 ...
 *-------------------------------------------------------------------------
 */
static int test_derived_complex(void)
{
...
static int test_bfloat16(void)
{
    ...
    if (H5Tget_fields(H5T_NATIVE_FLOAT, &float_spos, &float_epos, &float_esize, &float_mpos, &float_msize) < 0) {
        H5_FAILED();
    ...
    if (0 == my_isinf((int)native_type_order, buf_ptr, sizeof(float), float_mpos, float_msize, float_epos, float_esize)) {
        H5_FAILED();
    ...
    if (1 == my_isinf((int)native_type_order, buf_ptr, sizeof(float), float_mpos, float_msize, float_epos, float_esize)) {
        H5_FAILED();
 ...
 *-------------------------------------------------------------------------
 */
static int test_conv_int_1(const char* name, hid_t src, hid_t dst)
{
    ...
    for (j = 0; j < nelmts; j++) {
        if (INT_SCHAR == dst_type) {
            ...
            case LDBL_COMPLEX:
            case OTHER:
            default          : assert(0 && "Unknown type"); break;
            }
        }
        ...
        if (H5T_SGN_2 == src_sign && H5T_SGN_2 == dst_sign) {
            if (src_nbits > dst_nbits) {
                if (0 == H5T__bit_get_d(src_bits, src_nbits - 1, (size_t)1) &&
                     ...
                     */
                    if (0 == H5T__bit_get_d(dst_bits, dst_nbits - 1, (size_t)1) && H5T__bit_find(dst_bits, (size_t)0, dst_nbits - 1, H5T_BIT_LSB, 0) < 0) {
                        continue; /*no error*/
                    }
                }
                else if (1 == H5T__bit_get_d(src_bits, src_nbits - 1, (size_t)1) &&
                         H5T__bit_find(src_bits, (size_t)0, src_nbits - 1, H5T_BIT_MSB, 0) + 1 >= (ssize_t)dst_nbits) {
                    /*
                     * Source is negative but the magnitude is too large for
                     * the destination. The destination should be set to the
                     * smallest possible value: 0x80...0
                     */
                    if (1 == H5T__bit_get_d(dst_bits, dst_nbits - 1, (size_t)1) && H5T__bit_find(dst_bits, (size_t)0, dst_nbits - 1, H5T_BIT_LSB, 1) < 0) {
                        continue; /*no error*/
                    }
                }
            }
                ...
                if (0 == H5T__bit_get_d(src_bits, src_nbits - 1, (size_t)1)) {
                     ...
                     */
                    if (0 == H5T__bit_get_d(dst_bits, src_nbits - 1, (size_t)1) && H5T__bit_find(dst_bits, src_nbits, dst_nbits - src_nbits, H5T_BIT_LSB, 1) < 0) {
                        continue; /*no error*/
                    }
                }
                else {
                     ...
                     */
                    if (1 == H5T__bit_get_d(dst_bits, src_nbits - 1, (size_t)1) && H5T__bit_find(dst_bits, src_nbits, dst_nbits - src_nbits, H5T_BIT_LSB, 0) < 0) {
                        continue; /*no error*/
                    }
                }
            }
            ...
            else if (src_nbits > dst_nbits && H5T__bit_find(src_bits, dst_nbits - 1, src_nbits - dst_nbits, H5T_BIT_LSB, 1) >= 0) {
                /*
            ...
            if (src_nbits >= dst_nbits && H5T__bit_find(src_bits, dst_nbits - 1, (src_nbits - dst_nbits) + 1, H5T_BIT_LSB, 1) >= 0) {
                /*
            ...
            if (src_nbits > dst_nbits && H5T__bit_find(src_bits, dst_nbits, src_nbits - dst_nbits, H5T_BIT_LSB, 1) >= 0) {
                /*
        ...
        printf("%*s", (int)(3 * MAX(0, (ssize_t)src_size - (ssize_t)dst_size)), "");
        switch (dst_type) {
        case INT_SCHAR   : printf(" %29d\n", (int)*((signed char*)((void*)hw))); break;
        case INT_UCHAR   : printf(" %29u\n", (unsigned)*((unsigned char*)((void*)hw))); break;
        case INT_SHORT   : printf(" %29hd\n", *((short*)((void*)hw))); break;
        case INT_USHORT  : printf(" %29hu\n", *((unsigned short*)((void*)hw))); break;
        case INT_INT     : printf(" %29d\n", *((int*)((void*)hw))); break;
        case INT_UINT    : printf(" %29u\n", *((unsigned*)((void*)hw))); break;
        case INT_LONG    : printf(" %29ld\n", *((long*)((void*)hw))); break;
        case INT_ULONG   : printf(" %29lu\n", *((unsigned long*)((void*)hw))); break;
        case INT_LLONG   : fprintf(stdout, " %29lld\n", *((long long*)((void*)hw))); break;
        case INT_ULLONG  : fprintf(stdout, " %29llu\n", *((unsigned long long*)((void*)hw))); break;
        case FLT_FLOAT16 :
        case FLT_FLOAT   :
        case FLT_DOUBLE  :
        case FLT_LDOUBLE :
 ...
 *-------------------------------------------------------------------------
 */
static int test_conv_int_2(void)
{
    int i, j;
...
static int my_isinf(int endian, const unsigned char* val, size_t size, size_t mpos, size_t msize, size_t epos, size_t esize)
{
    ...
    if (H5T__bit_find(bits, mpos, msize, H5T_BIT_LSB, 1) < 0 && H5T__bit_find(bits, epos, esize, H5T_BIT_LSB, 0) < 0) {
        retval = 1;
    }
 ...
 */
#ifdef H5_HAVE__FLOAT16
static conv_func_ret_t test_conv_flt_1_hw_conv_from_flt16(void* hw_dst, unsigned char* src_buf, size_t idx, dtype_t dst_type)
{
    H5__Float16 aligned;
    ...
    switch (dst_type) {
    case FLT_FLOAT16: *((H5__Float16*)hw_dst) = aligned; break;
    case FLT_FLOAT  : *((float*)hw_dst) = (float)aligned; break;
    case FLT_DOUBLE : *((double*)hw_dst) = (double)aligned; break;
    #if H5_SIZEOF_LONG_DOUBLE != H5_SIZEOF_DOUBLE
    case FLT_LDOUBLE: *((long double*)hw_dst) = (long double)aligned; break;
    #endif

    #ifdef H5_HAVE_COMPLEX_NUMBERS
        #ifdef H5_HAVE_C99_COMPLEX_NUMBERS
    case FLT_COMPLEX : *((H5_float_complex*)hw_dst) = (H5_float_complex)aligned; break;
    case DBL_COMPLEX : *((H5_double_complex*)hw_dst) = (H5_double_complex)aligned; break;
    case LDBL_COMPLEX: *((H5_ldouble_complex*)hw_dst) = (H5_ldouble_complex)aligned; break;
        #else  /* H5_HAVE_C99_COMPLEX_NUMBERS */
    case FLT_COMPLEX : *((H5_float_complex*)hw_dst) = H5_CMPLXF(aligned, 0.0F); break;
    case DBL_COMPLEX : *((H5_double_complex*)hw_dst) = H5_CMPLX(aligned, 0.0); break;
    case LDBL_COMPLEX: *((H5_ldouble_complex*)hw_dst) = H5_CMPLXL(aligned, 0.0L); break;
 ...
 *-------------------------------------------------------------------------
 */
static conv_func_ret_t test_conv_flt_1_hw_conv_from_flt(void* hw_dst, unsigned char* src_buf, size_t idx, dtype_t dst_type)
{
    float aligned;
    conv_func_ret_t ret = CONV_SUCCESS;
    ...
    switch (dst_type) {
    case FLT_FLOAT : *((float*)hw_dst) = aligned; break;
    case FLT_DOUBLE: *((double*)hw_dst) = (double)aligned; break;
#if H5_SIZEOF_LONG_DOUBLE != H5_SIZEOF_DOUBLE
    case FLT_LDOUBLE: *((long double*)hw_dst) = (long double)aligned; break;
#endif
 ...
 *-------------------------------------------------------------------------
 */
static conv_func_ret_t test_conv_flt_1_hw_conv_from_double(void* hw_dst, unsigned char* src_buf, size_t idx, dtype_t dst_type)
{
    double aligned;
    conv_func_ret_t ret = CONV_SUCCESS;
    ...
    case FLT_FLOAT:
        *((float*)hw_dst) = (float)aligned;
        ...
        break;
    case FLT_DOUBLE: *((double*)hw_dst) = aligned; break;
#if H5_SIZEOF_LONG_DOUBLE != H5_SIZEOF_DOUBLE
    case FLT_LDOUBLE: *((long double*)hw_dst) = (long double)aligned; break;
#endif
    case FLT_FLOAT16:
 ...
 */
#if H5_SIZEOF_LONG_DOUBLE != H5_SIZEOF_DOUBLE
static conv_func_ret_t test_conv_flt_1_hw_conv_from_ldouble(void* hw_dst, unsigned char* src_buf, size_t idx, dtype_t dst_type)
{
    long double aligned;
    conv_func_ret_t ret = CONV_SUCCESS;
    ...
    case FLT_DOUBLE:
        *((double*)hw_dst) = (double)aligned;
        ...
        break;
    case FLT_LDOUBLE: *((long double*)hw_dst) = aligned; break;
    case FLT_FLOAT16:
    #ifdef H5_HAVE__FLOAT16
        /* Suppress warning about non-standard floating-point literal suffix */
        H5_WARN_NONSTD_SUFFIX_OFF
 ...
 *-------------------------------------------------------------------------
 */
static conv_func_ret_t test_conv_flt_1_hw_conv_from_fcomplex(void* hw_dst, unsigned char* src_buf, size_t idx, dtype_t dst_type)
{
    ...
    case FLT_FLOAT16:
        ...
        goto done;
    #endif
    case FLT_COMPLEX: *((H5_float_complex*)hw_dst) = aligned; break;
    case DBL_COMPLEX:
    #ifdef H5_HAVE_C99_COMPLEX_NUMBERS
        *((H5_double_complex*)hw_dst) = (H5_double_complex)aligned;
    #else
    ...
    case LDBL_COMPLEX:
        ...
        *((H5_ldouble_complex*)hw_dst) = (H5_ldouble_complex)aligned;
    #else
        *((H5_ldouble_complex*)hw_dst) = H5_CMPLXL((long double)crealf(aligned), (long double)crealf(aligned));
    #endif
        break;
    case INT_SCHAR:
    case INT_UCHAR:
 ...
 *-------------------------------------------------------------------------
 */
static conv_func_ret_t test_conv_flt_1_hw_conv_from_dcomplex(void* hw_dst, unsigned char* src_buf, size_t idx, dtype_t dst_type)
{
    ...
    switch (dst_type) {
    case FLT_FLOAT: real_val = creal(aligned);
    ...
    case DBL_COMPLEX: *((H5_double_complex*)hw_dst) = aligned; break;
    case LDBL_COMPLEX:
    #ifdef H5_HAVE_C99_COMPLEX_NUMBERS
        *((H5_ldouble_complex*)hw_dst) = (H5_ldouble_complex)aligned;
    #else
        *((H5_ldouble_complex*)hw_dst) = H5_CMPLXL((long double)creal(aligned), (long double)cimag(aligned));
    #endif
        break;
    case INT_SCHAR:
    case INT_UCHAR:
 ...
 *-------------------------------------------------------------------------
 */
static conv_func_ret_t test_conv_flt_1_hw_conv_from_lcomplex(void* hw_dst, unsigned char* src_buf, size_t idx, dtype_t dst_type)
{
    ...
    switch (dst_type) {
    case FLT_FLOAT: real_val = creall(aligned);
        ...
        *((float*)hw_dst) = (float)real_val;
        ...
        break;
    case FLT_DOUBLE: real_val = creall(aligned);
    ...
    case LDBL_COMPLEX: *((H5_ldouble_complex*)hw_dst) = aligned; break;
    case INT_SCHAR:
    case INT_UCHAR:
    case INT_SHORT:
    case INT_USHORT:
 ...
 *-------------------------------------------------------------------------
 */
static int test_conv_flt_1(const char* name, int run_test, hid_t src, hid_t dst)
{
    dtype_t src_type, dst_type;       /*data types        */
    size_t nelmts = 0;                /*num values per test    */
    const size_t max_fails = 8;       /*max number of failures*/
    ...
    if (OTHER == src_type || OTHER == dst_type) {
        if (!strcmp(name, "noop")) {
            ...
            snprintf(str, sizeof(str), "Testing %s special %s -> %s conversions", name, src_type_name, dst_type_name);
        }
        else if (run_test == TEST_NORMAL) {
            snprintf(str, sizeof(str), "Testing %s normalized %s -> %s conversions", name, src_type_name, dst_type_name);
        }
        else if (run_test == TEST_DENORM) {
            snprintf(str, sizeof(str), "Testing %s denormalized %s -> %s conversions", name, src_type_name, dst_type_name);
        }
    ...
    case TEST_NORMAL:
        if (src_type == FLT_FLOAT) {
            INIT_FP_NORM(float, FLT_MAX, FLT_MIN, FLT_MAX_10_EXP, FLT_MIN_10_EXP, src_size, dst_size, buf, saved, nelmts);
        }
        else if (src_type == FLT_DOUBLE) {
            INIT_FP_NORM(double, DBL_MAX, DBL_MIN, DBL_MAX_10_EXP, DBL_MIN_10_EXP, src_size, dst_size, buf, saved, nelmts);
#if H5_SIZEOF_LONG_DOUBLE != H5_SIZEOF_DOUBLE
        }
        else if (src_type == FLT_LDOUBLE) {
            INIT_FP_NORM(long double, LDBL_MAX, LDBL_MIN, LDBL_MAX_10_EXP, LDBL_MIN_10_EXP, src_size, dst_size, buf, saved, nelmts);
#endif
        }
            ...
            INIT_FP_NORM(H5__Float16, FLT16_MAX, FLT16_MIN, FLT16_MAX_10_EXP, FLT16_MIN_10_EXP, src_size, dst_size, buf, saved, nelmts);
            ...
            INIT_FP_NORM(float, FLT_MAX, FLT_MIN, FLT_MAX_10_EXP, FLT_MIN_10_EXP, part_size, dst_size, buf, saved, nelmts);
            ...
            size_t part_size = src_size / 2;
            INIT_FP_NORM(double, DBL_MAX, DBL_MIN, DBL_MAX_10_EXP, DBL_MIN_10_EXP, part_size, dst_size, buf, saved, nelmts);
            ...
            size_t part_size = src_size / 2;
            INIT_FP_NORM(long double, LDBL_MAX, LDBL_MIN, LDBL_MAX_10_EXP, LDBL_MIN_10_EXP, part_size, dst_size, buf, saved, nelmts);
    ...
    case TEST_DENORM:
        if (src_type == FLT_FLOAT) {
            ...
            INIT_FP_DENORM(long double, LDBL_MANT_DIG, src_size, src_nbits, sendian, dst_size, buf, saved, nelmts);
#endif
        }
        else if (src_type == FLT_FLOAT16) {
#ifdef H5_HAVE__FLOAT16
            INIT_FP_DENORM(H5__Float16, FLT16_MANT_DIG, src_size, src_nbits, sendian, dst_size, buf, saved, nelmts);
#else
            assert(0 && "Should not reach this point!");
#endif
        }
            ...
            size_t part_size = src_size / 2;
            INIT_FP_DENORM(long double, LDBL_MANT_DIG, part_size, src_nbits, sendian, dst_size, buf, saved, nelmts);
            /* Treat long double buffer as long double _Complex buffer of nelmts / 2 elements */
            assert(nelmts % 2 == 0);
            nelmts /= 2;
        }
    ...
    for (j = 0; j < nelmts; j++) {
        ...
        overflow = (conv_ret == CONV_OVERFLOW || conv_ret == CONV_OVERFLOW_REAL || conv_ret == CONV_OVERFLOW_IMAG || conv_ret == CONV_OVERFLOW_BOTH ||
                    conv_ret == CONV_OVERUNDER || conv_ret == CONV_UNDEROVER);
        underflow = (conv_ret == CONV_UNDERFLOW || conv_ret == CONV_UNDERFLOW_REAL || conv_ret == CONV_UNDERFLOW_IMAG || conv_ret == CONV_UNDERFLOW_BOTH ||
                     conv_ret == CONV_OVERUNDER || conv_ret == CONV_UNDEROVER);
        ...
        else if (FLT_DOUBLE == dst_type && my_isnan(dst_type, buf + j * sizeof(double)) && my_isnan(dst_type, hw)) {
            continue;
#if H5_SIZEOF_LONG_DOUBLE != H5_SIZEOF_DOUBLE
        }
        else if (FLT_LDOUBLE == dst_type && my_isnan(dst_type, buf + j * sizeof(long double)) && my_isnan(dst_type, hw)) {
            continue;
#endif
        }
#ifdef H5_HAVE__FLOAT16
        else if (FLT_FLOAT16 == dst_type && my_isnan(dst_type, buf + j * sizeof(H5__Float16)) && my_isnan(dst_type, hw)) {
            continue;
        }
#endif
#ifdef H5_HAVE_COMPLEX_NUMBERS
        else if (FLT_COMPLEX == dst_type && my_isnan(dst_type, buf + j * sizeof(H5_float_complex)) && my_isnan(dst_type, hw)) {
            continue;
        }
        else if (DBL_COMPLEX == dst_type && my_isnan(dst_type, buf + j * sizeof(H5_double_complex)) && my_isnan(dst_type, hw)) {
            continue;
        }
        else if (LDBL_COMPLEX == dst_type && my_isnan(dst_type, buf + j * sizeof(H5_ldouble_complex)) && my_isnan(dst_type, hw)) {
            continue;
        }
#endif
         ...
         */
        {
            ...
            if (FLT_FLOAT == dst_type) {
                ...
                if (overflow && my_isinf(dendian, buf + j * sizeof(float), dst_size, dst_mpos, dst_msize, dst_epos, dst_esize)) {
                ...
                if (overflow && my_isinf(dendian, buf + j * sizeof(double), dst_size, dst_mpos, dst_msize, dst_epos, dst_esize)) {
                    continue; /* all overflowed, no error */
                }
    ...
    #else
                if (underflow && fabsf((float)x) <= (float)FLT16_MIN && fabsf((float)hw_half) <= (float)FLT16_MIN) {
                    continue; /* all underflowed, no error */
                }
    #endif
                H5_WARN_NONSTD_SUFFIX_ON

                if (overflow && my_isinf(dendian, buf + j * sizeof(H5__Float16), dst_size, dst_mpos, dst_msize, dst_epos, dst_esize)) {
                    continue; /* all overflowed, no error */
                }
                ...
                if (overflow) {
                    ...
                    if (conv_ret == CONV_OVERFLOW_REAL || conv_ret == CONV_OVERFLOW_BOTH || conv_ret == CONV_OVERUNDER) {
                        if (!my_isinf(dendian, buf + j * sizeof(H5_float_complex), dst_size / 2, dst_mpos, dst_msize, dst_epos, dst_esize)) {
                            val_passes = false;
                        }
                    }
                    ...
                    if (conv_ret == CONV_OVERFLOW_IMAG || conv_ret == CONV_OVERFLOW_BOTH || conv_ret == CONV_UNDEROVER) {
                        if (!my_isinf(dendian, buf + (j * sizeof(H5_float_complex)) + (dst_size / 2), dst_size / 2, dst_mpos, dst_msize, dst_epos, dst_esize)) {
                            val_passes = false;
                        }
                    }
                }
                ...
                if (overflow) {
                    /* Check real part against hardware */
                    if (conv_ret == CONV_OVERFLOW_REAL || conv_ret == CONV_OVERFLOW_BOTH || conv_ret == CONV_OVERUNDER) {
                        if (!my_isinf(dendian, buf + j * sizeof(H5_double_complex), dst_size / 2, dst_mpos, dst_msize, dst_epos, dst_esize)) {
                            val_passes = false;
                        }
                    }
                    ...
                    /* Check imaginary part against hardware */
                    if (conv_ret == CONV_OVERFLOW_IMAG || conv_ret == CONV_OVERFLOW_BOTH || conv_ret == CONV_UNDEROVER) {
                        if (!my_isinf(dendian, buf + (j * sizeof(H5_double_complex)) + (dst_size / 2), dst_size / 2, dst_mpos, dst_msize, dst_epos, dst_esize)) {
                            val_passes = false;
                        }
                    }
                }
            ...
            if (FLT_COMPLEX == dst_type || DBL_COMPLEX == dst_type || LDBL_COMPLEX == dst_type) {
                if (check_expo[0] < (-(int)dst_ebias) || check_expo[1] < (-(int)dst_ebias) || check_expo[2] < (-(int)dst_ebias) ||
                    check_expo[3] < (-(int)dst_ebias)) {
                    ...
                    if (fabs(check_mant[0] - check_mant[1]) <= epsilon_real && fabs(check_mant[2] - check_mant[3]) <= epsilon_imag) {
                        continue;
                    }
                }
                else {
 ...
 *-------------------------------------------------------------------------
 */
static int test_conv_int_fp_conv_to_schar(void* hw_p, unsigned char* src_buf, size_t idx, dtype_t src_type)
{
    signed char aligned;
    int ret = 0;
 ...
 *-------------------------------------------------------------------------
 */
static int test_conv_int_fp_conv_to_uchar(void* hw_p, unsigned char* src_buf, size_t idx, dtype_t src_type)
{
 ...
 *-------------------------------------------------------------------------
 */
static int test_conv_int_fp_conv_to_short(void* hw_p, unsigned char* src_buf, size_t idx, dtype_t src_type)
{
    short aligned;
    int ret = 0;
 ...
 *-------------------------------------------------------------------------
 */
static int test_conv_int_fp_conv_to_ushort(void* hw_p, unsigned char* src_buf, size_t idx, dtype_t src_type)
{
    unsigned short aligned;
    int ret = 0;
 ...
 *-------------------------------------------------------------------------
 */
static int test_conv_int_fp_conv_to_int(void* hw_p, unsigned char* src_buf, size_t idx, dtype_t src_type)
{
    int aligned;
    int ret = 0;
 ...
 *-------------------------------------------------------------------------
 */
static int test_conv_int_fp_conv_to_uint(void* hw_p, unsigned char* src_buf, size_t idx, dtype_t src_type)
{
 ...
 *-------------------------------------------------------------------------
 */
static int test_conv_int_fp_conv_to_long(void* hw_p, unsigned char* src_buf, size_t idx, dtype_t src_type)
{
    long aligned;
    int ret = 0;
 ...
 *-------------------------------------------------------------------------
 */
static int test_conv_int_fp_conv_to_ulong(void* hw_p, unsigned char* src_buf, size_t idx, dtype_t src_type)
{
    unsigned long aligned;
    int ret = 0;
 ...
 *-------------------------------------------------------------------------
 */
static int test_conv_int_fp_conv_to_llong(void* hw_p, unsigned char* src_buf, size_t idx, dtype_t src_type)
{
    long long aligned;
    int ret = 0;
 ...
 *-------------------------------------------------------------------------
 */
static int test_conv_int_fp_conv_to_ullong(void* hw_p, unsigned char* src_buf, size_t idx, dtype_t src_type)
{
    unsigned long long aligned;
    int ret = 0;
 ...
 */
#ifdef H5_HAVE__FLOAT16
static int test_conv_int_fp_conv_to_flt16(void* hw_p, unsigned char* src_buf, size_t idx, dtype_t src_type)
{
    H5__Float16 aligned;
    int ret = 0;
 ...
 *-------------------------------------------------------------------------
 */
static int test_conv_int_fp_conv_to_flt(void* hw_p, unsigned char* src_buf, size_t idx, dtype_t src_type)
{
    float aligned;
    int ret = 0;
 ...
 *-------------------------------------------------------------------------
 */
static int test_conv_int_fp_conv_to_double(void* hw_p, unsigned char* src_buf, size_t idx, dtype_t src_type)
{
    double aligned;
    int ret = 0;
 ...
 *-------------------------------------------------------------------------
 */
static int test_conv_int_fp_conv_to_ldouble(void* hw_p, unsigned char* src_buf, size_t idx, dtype_t src_type)
{
    long double aligned;
    int ret = 0;
 ...
 *-------------------------------------------------------------------------
 */
static int test_conv_int_fp_conv_to_fcomplex(void* hw_p, unsigned char* src_buf, size_t idx, dtype_t src_type)
{
    H5_float_complex aligned;
    int ret = 0;
 ...
 *-------------------------------------------------------------------------
 */
static int test_conv_int_fp_conv_to_dcomplex(void* hw_p, unsigned char* src_buf, size_t idx, dtype_t src_type)
{
    H5_double_complex aligned;
    int ret = 0;
 ...
 *-------------------------------------------------------------------------
 */
static int test_conv_int_fp_conv_to_lcomplex(void* hw_p, unsigned char* src_buf, size_t idx, dtype_t src_type)
{
    H5_ldouble_complex aligned;
    int ret = 0;
 ...
 *-------------------------------------------------------------------------
 */
static int test_conv_int_fp(const char* name, int run_test, hid_t src, hid_t dst)
{
    ...
    if ((INT_SCHAR == src_type || INT_UCHAR == src_type || INT_SHORT == src_type || INT_USHORT == src_type || INT_INT == src_type || INT_UINT == src_type ||
         INT_LONG == src_type || INT_ULONG == src_type || INT_LLONG == src_type || INT_ULLONG == src_type) &&
        (FLT_FLOAT != dst_type && FLT_DOUBLE != dst_type && FLT_LDOUBLE != dst_type && FLT_FLOAT16 != dst_type && FLT_COMPLEX != dst_type &&
         DBL_COMPLEX != dst_type && LDBL_COMPLEX != dst_type)) {
        snprintf(str, sizeof(str), "Testing %s %s -> %s conversions", name, src_type_name, dst_type_name);
        printf("%-70s", str);
        H5_FAILED();
    ...
    if ((FLT_FLOAT == src_type || FLT_DOUBLE == src_type || FLT_LDOUBLE == src_type || FLT_FLOAT16 == src_type || FLT_COMPLEX == src_type ||
         DBL_COMPLEX == src_type || LDBL_COMPLEX == src_type) &&
        (INT_SCHAR != dst_type && INT_UCHAR != dst_type && INT_SHORT != dst_type && INT_USHORT != dst_type && INT_INT != dst_type && INT_UINT != dst_type &&
         INT_LONG != dst_type && INT_ULONG != dst_type && INT_LLONG != dst_type && INT_ULLONG != dst_type)) {
        snprintf(str, sizeof(str), "Testing %s %s -> %s conversions", name, src_type_name, dst_type_name);
        printf("%-70s", str);
        H5_FAILED();
    ...
    if ((src_type == INT_INT && dst_type == FLT_FLOAT) || (src_type == INT_UINT && dst_type == FLT_FLOAT) || (src_type == FLT_FLOAT && dst_type == INT_UINT) ||
    ...
    /* Set pointer to matching type for hardware conversion */
    switch (dst_type) {
    case INT_SCHAR : hw_p = &hw_schar; break;
    case INT_UCHAR : hw_p = &hw_uchar; break;
    case INT_SHORT : hw_p = &hw_short; break;
    case INT_USHORT: hw_p = &hw_ushort; break;
    case INT_INT   : hw_p = &hw_int; break;
    case INT_UINT  : hw_p = &hw_uint; break;
    case INT_LONG  : hw_p = &hw_long; break;
    case INT_ULONG : hw_p = &hw_ulong; break;
    case INT_LLONG : hw_p = &hw_llong; break;
    case INT_ULLONG: hw_p = &hw_ullong; break;
    case FLT_FLOAT16:
#ifdef H5_HAVE__FLOAT16
        hw_p = &hw_half;
        break;
        ...
        goto error;
#endif
    case FLT_FLOAT  : hw_p = &hw_float; break;
    case FLT_DOUBLE : hw_p = &hw_double; break;
    case FLT_LDOUBLE: hw_p = &hw_ldouble; break;
#ifdef H5_HAVE_COMPLEX_NUMBERS
    case FLT_COMPLEX : hw_p = &hw_float_complex; break;
    case DBL_COMPLEX : hw_p = &hw_double_complex; break;
    case LDBL_COMPLEX: hw_p = &hw_ldouble_complex; break;
#else
    case FLT_COMPLEX:
    case DBL_COMPLEX:
    case LDBL_COMPLEX:
    ...
    for (j = 0; j < nelmts; j++) {
        ...
        switch (dst_type) {
        case INT_SCHAR : conv_ret = test_conv_int_fp_conv_to_schar(hw_p, saved, j, src_type); break;
        case INT_UCHAR : conv_ret = test_conv_int_fp_conv_to_uchar(hw_p, saved, j, src_type); break;
        case INT_SHORT : conv_ret = test_conv_int_fp_conv_to_short(hw_p, saved, j, src_type); break;
        case INT_USHORT: conv_ret = test_conv_int_fp_conv_to_ushort(hw_p, saved, j, src_type); break;
        case INT_INT   : conv_ret = test_conv_int_fp_conv_to_int(hw_p, saved, j, src_type); break;
        case INT_UINT  : conv_ret = test_conv_int_fp_conv_to_uint(hw_p, saved, j, src_type); break;
        case INT_LONG  : conv_ret = test_conv_int_fp_conv_to_long(hw_p, saved, j, src_type); break;
        case INT_ULONG : conv_ret = test_conv_int_fp_conv_to_ulong(hw_p, saved, j, src_type); break;
        case INT_LLONG : conv_ret = test_conv_int_fp_conv_to_llong(hw_p, saved, j, src_type); break;
        case INT_ULLONG: conv_ret = test_conv_int_fp_conv_to_ullong(hw_p, saved, j, src_type); break;
        case FLT_FLOAT16:
#ifdef H5_HAVE__FLOAT16
            ...
            goto error;
#endif
        case FLT_FLOAT  : conv_ret = test_conv_int_fp_conv_to_flt(hw_p, saved, j, src_type); break;
        case FLT_DOUBLE : conv_ret = test_conv_int_fp_conv_to_double(hw_p, saved, j, src_type); break;
        case FLT_LDOUBLE: conv_ret = test_conv_int_fp_conv_to_ldouble(hw_p, saved, j, src_type); break;
#ifdef H5_HAVE_COMPLEX_NUMBERS
        case FLT_COMPLEX : conv_ret = test_conv_int_fp_conv_to_fcomplex(hw_p, saved, j, src_type); break;
        case DBL_COMPLEX : conv_ret = test_conv_int_fp_conv_to_dcomplex(hw_p, saved, j, src_type); break;
        case LDBL_COMPLEX: conv_ret = test_conv_int_fp_conv_to_lcomplex(hw_p, saved, j, src_type); break;
#else
        case FLT_COMPLEX:
        case DBL_COMPLEX:
        case LDBL_COMPLEX:
        ...
        if ((FLT_FLOAT == src_type || FLT_DOUBLE == src_type || FLT_LDOUBLE == src_type || FLT_FLOAT16 == src_type || FLT_COMPLEX == src_type ||
             DBL_COMPLEX == src_type || LDBL_COMPLEX == src_type) &&
            (INT_SCHAR == dst_type || INT_SHORT == dst_type || INT_INT == dst_type || INT_LONG == dst_type || INT_LLONG == dst_type)) {
            if (0 == H5T__bit_get_d(src_bits, src_nbits - 1, (size_t)1) && overflows(src_bits, src, dst_nbits - 1)) {
                /*
                 * Source is positive and the magnitude is too large for
                 * the destination.  The destination should be set to the
                 * maximum possible value: 0x7f...f
            ...
            else if (1 == H5T__bit_get_d(src_bits, src_nbits - 1, (size_t)1) && overflows(src_bits, src, dst_nbits - 1)) {
                /*
                 * Source is negative but the magnitude is too large for
                 * the destination. The destination should be set to the
                 * smallest possible value: 0x80...0
        ...
        if ((FLT_FLOAT == src_type || FLT_DOUBLE == src_type || FLT_LDOUBLE == src_type || FLT_FLOAT16 == src_type || FLT_COMPLEX == src_type ||
             DBL_COMPLEX == src_type || LDBL_COMPLEX == src_type) &&
            (INT_UCHAR == dst_type || INT_USHORT == dst_type || INT_UINT == dst_type || INT_ULONG == dst_type || INT_ULLONG == dst_type)) {
            if (H5T__bit_get_d(src_bits, src_nbits - 1, (size_t)1)) {
                /*
                 * The source is negative so the result should be zero.
                 * The source is negative if the most significant bit is
        ...
        case FLT_COMPLEX:
        case DBL_COMPLEX:
        case LDBL_COMPLEX: assert(0 && "Should not reach this point!"); break;
#endif
        case OTHER:
        default   : assert(0 && "Unknown type"); break;
        }
        ...
        case INT_USHORT : printf(" %29hu\n", *((unsigned short*)((void*)hw))); break;
        case INT_INT    : printf(" %29d\n", *((int*)((void*)hw))); break;
        case INT_UINT   : printf(" %29u\n", *((unsigned int*)((void*)hw))); break;
        case INT_LONG   : printf(" %29ld\n", *((long*)((void*)hw))); break;
        case INT_ULONG  : printf(" %29lu\n", *((unsigned long*)((void*)hw))); break;
        case INT_LLONG  : fprintf(stdout, " %29lld\n", *((long long*)((void*)hw))); break;
        case INT_ULLONG : fprintf(stdout, " %29llu\n", *((unsigned long long*)((void*)hw))); break;
        case FLT_FLOAT  : printf(" %29f\n", (double)*((float*)((void*)hw))); break;
        case FLT_DOUBLE : printf(" %29f\n", *((double*)((void*)hw))); break;
        case FLT_LDOUBLE: printf(" %29Lf\n", *((long double*)((void*)hw))); break;
        case FLT_FLOAT16:
#ifdef H5_HAVE__FLOAT16
...
#endif
#ifdef H5_HAVE_COMPLEX_NUMBERS
        case FLT_COMPLEX : printf(" %29f%+fi\n", (double)crealf(*((H5_float_complex*)((void*)hw))), (double)cimagf(*((H5_float_complex*)((void*)hw)))); break;
        case DBL_COMPLEX : printf(" %29f%+fi\n", creal(*((H5_double_complex*)((void*)hw))), cimag(*((H5_double_complex*)((void*)hw)))); break;
        case LDBL_COMPLEX: printf(" %29Lf%+Lfi\n", creall(*((H5_ldouble_complex*)((void*)hw))), cimagl(*((H5_ldouble_complex*)((void*)hw)))); break;
#else
        case FLT_COMPLEX:
        case DBL_COMPLEX:
        case LDBL_COMPLEX: assert(0 && "Should not reach this point!"); break;
 ...
 *-------------------------------------------------------------------------
 */
static int run_integer_tests(const char* name)
{
    int nerrors = 0;
 ...
 *-------------------------------------------------------------------------
 */
static int run_fp_tests(const char* name)
{
    int nerrors = 0;
        ...
        #else
    {
        ...
        snprintf(str, sizeof(str), "Testing %s normalized %s -> %s conversions", name, "long double", "_Float16");
        printf("%-70s", str);
        SKIPPED();
        ...
        #else
    {
        ...
        snprintf(str, sizeof(str), "Testing %s normalized %s -> %s conversions", name, "long double _Complex", "_Float16");
        printf("%-70s", str);
        SKIPPED();
        puts("    Test skipped due to compiler error in handling conversion.");
    }
    ...
    #else
    {
        ...
        snprintf(str, sizeof(str), "Testing %s denormalized %s -> %s conversions", name, "long double", "float");
        printf("%-70s", str);
        SKIPPED();
        ...
        #else
    {
        ...
        snprintf(str, sizeof(str), "Testing %s denormalized %s -> %s conversions", name, "long double", "_Float16");
        printf("%-70s", str);
        SKIPPED();
        puts("    Test skipped due to compiler error in handling conversion.");
    }
    ...
    #else
    {
        ...
        snprintf(str, sizeof(str), "Testing %s denormalized %s -> %s conversions", name, "long double _Complex", "float");
        printf("%-70s", str);
        SKIPPED();
        puts("    Test skipped due to the conversion problem on IBM ppc64le cpu.");
    }
        ...
        #else
    {
        ...
        snprintf(str, sizeof(str), "Testing %s denormalized %s -> %s conversions", name, "long double _Complex", "_Float16");
        printf("%-70s", str);
        SKIPPED();
        puts("    Test skipped due to compiler error in handling conversion.");
    }
    ...
    #else
    {
        ...
        snprintf(str, sizeof(str), "Testing %s denormalized %s -> %s conversions", name, "long double _Complex", "float _Complex");
        printf("%-70s", str);
        SKIPPED();
        puts("    Test skipped due to the conversion problem on IBM ppc64le cpu.");
    }
    ...
    #else
    {
        ...
        snprintf(str, sizeof(str), "Testing %s special %s -> %s conversions", name, "long double", "float or double");
        printf("%-70s", str);
        SKIPPED();
        puts("    Test skipped due to the conversion problem on IBM ppc64le cpu.");
    }
        ...
        #else
    {
        ...
        snprintf(str, sizeof(str), "Testing %s special %s -> %s conversions", name, "long double", "_Float16");
        printf("%-70s", str);
        SKIPPED();
        puts("    Test skipped due to compiler error in handling conversion.");
    }
    ...
    #else
    {
        ...
        snprintf(str, sizeof(str), "Testing %s special %s -> %s conversions", name, "long double _Complex", "float");
        printf("%-70s", str);
        SKIPPED();
        puts("    Test skipped due to the conversion problem on IBM ppc64le cpu.");
    }
    ...
    #else
    {
        ...
        snprintf(str, sizeof(str), "Testing %s special %s -> %s conversions", name, "long double _Complex", "double");
        printf("%-70s", str);
        SKIPPED();
        puts("    Test skipped due to the conversion problem on IBM ppc64le cpu.");
    }
    ...
    #else
    {
        ...
        snprintf(str, sizeof(str), "Testing %s special %s -> %s conversions", name, "long double", "float _Complex");
        printf("%-70s", str);
        SKIPPED();
        puts("    Test skipped due to the conversion problem on IBM ppc64le cpu.");
    }
    ...
    #else
    {
        ...
        snprintf(str, sizeof(str), "Testing %s special %s -> %s conversions", name, "long double", "double _Complex");
        printf("%-70s", str);
        SKIPPED();
        puts("    Test skipped due to the conversion problem on IBM ppc64le cpu.");
    }
    ...
    #else
    {
        ...
        snprintf(str, sizeof(str), "Testing %s special %s -> %s conversions", name, "long double", "long double _Complex");
        printf("%-70s", str);
        SKIPPED();
        puts("    Test skipped due to the conversion problem on IBM ppc64le cpu.");
    }
    ...
    #else
    {
        ...
        snprintf(str, sizeof(str), "Testing %s special %s -> %s conversions", name, "long double _Complex", "long double");
        printf("%-70s", str);
        SKIPPED();
        puts("    Test skipped due to the conversion problem on IBM ppc64le cpu.");
    }
        ...
        #else
    {
        ...
        snprintf(str, sizeof(str), "Testing %s special %s -> %s conversions", name, "long double _Complex", "_Float16");
        printf("%-70s", str);
        SKIPPED();
        puts("    Test skipped due to compiler error in handling conversion.");
    }
    ...
    #else
    {
        ...
        snprintf(str, sizeof(str), "Testing %s special %s -> %s conversions", name, "long double _Complex", "float _Complex");
        printf("%-70s", str);
        SKIPPED();
        puts("    Test skipped due to the conversion problem on IBM ppc64le cpu.");
    }
    ...
    #else
    {
        ...
        snprintf(str, sizeof(str), "Testing %s special %s -> %s conversions", name, "long double _Complex", "double _Complex");
        printf("%-70s", str);
        SKIPPED();
        puts("    Test skipped due to the conversion problem on IBM ppc64le cpu.");
    }
 ...
 *-------------------------------------------------------------------------
 */
static int run_int_fp_conv(const char* name)
{
    int nerrors = 0;
        ...
        #else
    {
        ...
        snprintf(str, sizeof(str), "Testing %s %s -> %s conversions", name, "unsigned long", "long double _Complex");
        printf("%-70s", str);
        SKIPPED();
        puts("    Test skipped due to the special algorithm of hardware conversion.");
    }
        ...
        #else
    {
        ...
        snprintf(str, sizeof(str), "Testing %s %s -> %s conversions", name, "long long", "long double _Complex");
        printf("%-70s", str);
        SKIPPED();
        puts("    Test skipped due to compiler error in handling conversion.");
    }
        ...
        #else
    {
        ...
        snprintf(str, sizeof(str), "Testing %s %s -> %s conversions", name, "unsigned long long", "long double _Complex");
        printf("%-70s", str);
        SKIPPED();
        puts("    Test skipped due to compiler error in handling conversion.");
    }
        ...
        #else  /* H5_LLONG_TO_LDOUBLE_CORRECT */
    {
        ...
        snprintf(str, sizeof(str), "Testing %s %s -> %s conversions", name, "unsigned long long", "long double");
        printf("%-70s", str);
        SKIPPED();
 ...
 *-------------------------------------------------------------------------
 */
static int run_fp_int_conv(const char* name)
{
    int nerrors = 0;
    int test_values;
    ...
    for (test_values = TEST_NORMAL; test_values <= TEST_SPECIAL; test_values++) {
        ...
        if (test_values != TEST_SPECIAL)
            ...
            snprintf(str, sizeof(str), "Testing %s special %s -> %s conversions", name, "long double _Complex", "signed char");
            printf("%-70s", str);
            SKIPPED();
            puts("    Test skipped due to the conversion problem on IBM ppc64le cpu.");
        }
        ...
        if (test_values != TEST_SPECIAL)
            ...
            snprintf(str, sizeof(str), "Testing %s special %s -> %s conversions", name, "long double _Complex", "unsigned char");
            printf("%-70s", str);
            SKIPPED();
            puts("    Test skipped due to the conversion problem on IBM ppc64le cpu.");
        }
        ...
        if (test_values != TEST_SPECIAL)
            ...
            snprintf(str, sizeof(str), "Testing %s special %s -> %s conversions", name, "long double _Complex", "short");
            printf("%-70s", str);
            SKIPPED();
            puts("    Test skipped due to the conversion problem on IBM ppc64le cpu.");
        }
        ...
        if (test_values != TEST_SPECIAL)
            ...
            snprintf(str, sizeof(str), "Testing %s special %s -> %s conversions", name, "long double _Complex", "unsigned short");
            printf("%-70s", str);
            SKIPPED();
            puts("    Test skipped due to the conversion problem on IBM ppc64le cpu.");
        }
        ...
        if (test_values != TEST_SPECIAL)
            ...
            snprintf(str, sizeof(str), "Testing %s special %s -> %s conversions", name, "long double _Complex", "int");
            printf("%-70s", str);
            SKIPPED();
            puts("    Test skipped due to the conversion problem on IBM ppc64le cpu.");
        }
        ...
        if (test_values != TEST_SPECIAL)
            ...
            snprintf(str, sizeof(str), "Testing %s special %s -> %s conversions", name, "long double _Complex", "unsigned int");
            printf("%-70s", str);
            SKIPPED();
            puts("    Test skipped due to the conversion problem on IBM ppc64le cpu.");
        }
        ...
        #else
        {
            ...
            snprintf(str, sizeof(str), "Testing %s %s -> %s conversions", name, "long double _Complex", "long");
            printf("%-70s", str);
            SKIPPED();
            puts("    Test skipped due to the special algorithm of hardware conversion.");
        }
        ...
        #else
        {
            ...
            snprintf(str, sizeof(str), "Testing %s %s -> %s conversions", name, "long double _Complex", "unsigned long");
            printf("%-70s", str);
            SKIPPED();
            puts("    Test skipped due to the special algorithm of hardware conversion.");
        }
        ...
        if (!strcmp(name, "hw")) { /* Hardware conversion */
        ...
        #else
            {
                ...
                snprintf(str, sizeof(str), "Testing %s %s -> %s conversions", name, "long double _Complex", "long long");
                printf("%-70s", str);
                SKIPPED();
        ...
        #else
        {
            ...
            snprintf(str, sizeof(str), "Testing %s %s -> %s conversions", name, "long double _Complex", "unsigned long long");
            printf("%-70s", str);
            SKIPPED();
            puts("    Test skipped due to hardware conversion error.");
        }
        ...
        if (test_values != TEST_SPECIAL) {
            ...
            snprintf(str, sizeof(str), "Testing %s special %s -> %s conversions", name, "long double", "signed and unsigned char, short, int, long");
            printf("%-70s", str);
            SKIPPED();
            puts("    Test skipped due to the conversion problem on IBM ppc64le cpu.");
    #endif
        ...
        #else
        {
            ...
            snprintf(str, sizeof(str), "Testing %s %s -> %s conversions", name, "long double", "(unsigned) long");
            printf("%-70s", str);
            SKIPPED();
            puts("    Test skipped due to the special algorithm of hardware conversion.");
        }
        ...
        #else  /*H5_LDOUBLE_TO_LLONG_ACCURATE*/
        {
            ...
            snprintf(str, sizeof(str), "Testing %s %s -> %s conversions", name, "long double", "unsigned long long");
            printf("%-70s", str);
            SKIPPED();
            puts("    Test skipped due to hardware conversion error.");
        }
