...
static const char* FILENAME[] = { "dtypes0",  "dtypes1",  "dtypes2",  "dtypes3",  "dtypes4",  "dtypes5",  "dtypes6",  "dtypes7",  "dtypes8",  "dtypes9", "dtypes10",
                                  "dtypes11", "dtypes12", "dtypes13", "dtypes14", "dtypes15", "dtypes16", "dtypes17", "dtypes18", "dtypes19", NULL };
...
static herr_t
    convert_opaque(hid_t st, hid_t dt, H5T_cdata_t* cdata, size_t nelmts, size_t buf_stride, size_t bkg_stride, void* _buf, void* bkg, hid_t dset_xfer_plid);
...
static herr_t user_compound_convert(hid_t H5_ATTR_UNUSED src_id,
                                    ...
                                    hid_t H5_ATTR_UNUSED dxpl)
{
    herr_t retval = EXIT_SUCCESS;
    switch (cdata->command) {
    case H5T_CONV_INIT: break;
    case H5T_CONV_CONV:
        for (size_t i = 0; i < nelmts; ++i) {
            ((dst_cmpd_t*)buf)[i].b = ((src_cmpd_t*)buf)[i].b;
        }
        break;
    case H5T_CONV_FREE: break;
    default           : break;
    }
    return retval;
}
 ...
 *-------------------------------------------------------------------------
 */
static int test_compound_1(void)
{
    hid_t complex_id;
    hid_t super;
    size_t size;
 ...
 *-------------------------------------------------------------------------
 */
static int test_compound_2(void)
{
    struct st
    {
        int a, b, c[4], d, e;
    ...
    if ((st = H5Tcreate(H5T_COMPOUND, sizeof(struct st))) < 0 || H5Tinsert(st, "a", HOFFSET(struct st, a), H5T_NATIVE_INT) < 0 ||
        H5Tinsert(st, "b", HOFFSET(struct st, b), H5T_NATIVE_INT) < 0 || H5Tinsert(st, "c", HOFFSET(struct st, c), array_dt) < 0 ||
        H5Tinsert(st, "d", HOFFSET(struct st, d), H5T_NATIVE_INT) < 0 || H5Tinsert(st, "e", HOFFSET(struct st, e), H5T_NATIVE_INT) < 0) {
        goto error;
    }
    H5Tclose(array_dt);

    array_dt = H5Tarray_create2(H5T_NATIVE_INT, 1, &four);
    if ((dt = H5Tcreate(H5T_COMPOUND, sizeof(struct dt))) < 0 || H5Tinsert(dt, "a", HOFFSET(struct dt, a), H5T_NATIVE_INT) < 0 ||
        H5Tinsert(dt, "b", HOFFSET(struct dt, b), H5T_NATIVE_INT) < 0 || H5Tinsert(dt, "c", HOFFSET(struct dt, c), array_dt) < 0 ||
        H5Tinsert(dt, "d", HOFFSET(struct dt, d), H5T_NATIVE_INT) < 0 || H5Tinsert(dt, "e", HOFFSET(struct dt, e), H5T_NATIVE_INT) < 0) {
        goto error;
    }
    H5Tclose(array_dt);
    ...
    for (i = 0; i < (int)nelmts; i++) {
        ...
        if (s_ptr->a != d_ptr->a || s_ptr->b != d_ptr->b || s_ptr->c[0] != d_ptr->c[0] || s_ptr->c[1] != d_ptr->c[1] || s_ptr->c[2] != d_ptr->c[2] ||
            ...
            H5_FAILED();
            printf("    i=%d\n", i);
            printf("    src={a=%d, b=%d, c=[%d,%d,%d,%d], d=%d, e=%d\n", s_ptr->a, s_ptr->b, s_ptr->c[0], s_ptr->c[1], s_ptr->c[2], s_ptr->c[3], s_ptr->d, s_ptr->e);
            printf("    dst={a=%d, b=%d, c=[%d,%d,%d,%d], d=%d, e=%d\n", d_ptr->a, d_ptr->b, d_ptr->c[0], d_ptr->c[1], d_ptr->c[2], d_ptr->c[3], d_ptr->d, d_ptr->e);
            goto error;
        }
    }
 ...
 *-------------------------------------------------------------------------
 */
static int test_compound_3(void)
{
    struct st
    {
        int a, b, c[4], d, e;
    ...
    array_dt = H5Tarray_create2(H5T_NATIVE_INT, 1, &four);
    if ((dt = H5Tcreate(H5T_COMPOUND, sizeof(struct dt))) < 0 || H5Tinsert(dt, "a", HOFFSET(struct dt, a), H5T_NATIVE_INT) < 0 ||
        H5Tinsert(dt, "c", HOFFSET(struct dt, c), array_dt) < 0 || H5Tinsert(dt, "e", HOFFSET(struct dt, e), H5T_NATIVE_INT) < 0) {
        goto error;
    }
    H5Tclose(array_dt);
    ...
    for (i = 0; i < (int)nelmts; i++) {
        ...
        if (s_ptr->a != d_ptr->a || s_ptr->c[0] != d_ptr->c[0] || s_ptr->c[1] != d_ptr->c[1] || s_ptr->c[2] != d_ptr->c[2] || s_ptr->c[3] != d_ptr->c[3] ||
            ...
            printf("    i=%d\n", i);
            printf("    src={a=%d, b=%d, c=[%d,%d,%d,%d], d=%d, e=%d\n", s_ptr->a, s_ptr->b, s_ptr->c[0], s_ptr->c[1], s_ptr->c[2], s_ptr->c[3], s_ptr->d, s_ptr->e);
            printf("    dst={a=%d, c=[%d,%d,%d,%d], e=%d\n", d_ptr->a, d_ptr->c[0], d_ptr->c[1], d_ptr->c[2], d_ptr->c[3], d_ptr->e);
            goto error;
        }
    }
 ...
 *-------------------------------------------------------------------------
 */
static int test_compound_4(void)
{
    struct st
    {
        int a, b, c[4], d, e;
    ...
    array_dt = H5Tarray_create2(H5T_NATIVE_INT, 1, &four);
    if ((dt = H5Tcreate(H5T_COMPOUND, sizeof(struct dt))) < 0 || H5Tinsert(dt, "a", HOFFSET(struct dt, a), H5T_NATIVE_INT) < 0 ||
        H5Tinsert(dt, "b", HOFFSET(struct dt, b), H5T_NATIVE_SHORT) < 0 || H5Tinsert(dt, "c", HOFFSET(struct dt, c), array_dt) < 0 ||
        H5Tinsert(dt, "d", HOFFSET(struct dt, d), H5T_NATIVE_SHORT) < 0 || H5Tinsert(dt, "e", HOFFSET(struct dt, e), H5T_NATIVE_INT) < 0) {
        goto error;
    }
    H5Tclose(array_dt);
 ...
 *-------------------------------------------------------------------------
 */
static int test_compound_5(void)
{
    typedef struct
    {
        char name[16];
 ...
 *-------------------------------------------------------------------------
 */
static int test_compound_6(void)
{
    struct st
    {
        short b;
    ...
    /* Build hdf5 datatypes */
    if ((st = H5Tcreate(H5T_COMPOUND, sizeof(struct st))) < 0 || H5Tinsert(st, "b", HOFFSET(struct st, b), H5T_NATIVE_SHORT) < 0 ||
        H5Tinsert(st, "d", HOFFSET(struct st, d), H5T_NATIVE_SHORT) < 0) {
        H5_FAILED();
        goto error;
    }

    if ((dt = H5Tcreate(H5T_COMPOUND, sizeof(struct dt))) < 0 || H5Tinsert(dt, "b", HOFFSET(struct dt, b), H5T_NATIVE_LONG) < 0 ||
 ...
 *-------------------------------------------------------------------------
 */
static int test_compound_7(void)
{
    struct s1
    {
        int a;
 ...
 *-------------------------------------------------------------------------
 */
static int test_compound_8(void)
{
    typedef struct s1
    {
        char a;
 ...
 *-------------------------------------------------------------------------
 */
static int test_compound_9(void)
{
 ...
 *-------------------------------------------------------------------------
 */
static int test_compound_10(void)
{
    ...
    for (i = 0; i < ARRAY_DIM; i++) {
        if (rdata[i].i1 != wdata[i].i1 || rdata[i].i2 != wdata[i].i2 || strcmp(rdata[i].str, wdata[i].str) != 0) {
 ...
 *-------------------------------------------------------------------------
 */
static int test_compound_11(void)
{
    typedef struct
    {
        double d1;
 ...
 *-------------------------------------------------------------------------
 */
static int test_compound_12(void)
{
    hid_t complex_id;
    size_t size = 0;
 ...
 *-------------------------------------------------------------------------
 */
static int test_compound_13(void)
{
    struct s1
    {
 ...
 *-------------------------------------------------------------------------
 */
static int test_compound_14(void)
{
    ...
    if (rdata2.c1 != wdata2.c1 || rdata2.c2 != wdata2.c2 || strcmp(rdata2.str, wdata2.str) != 0 || rdata2.l1 != wdata2.l1 || rdata2.l2 != wdata2.l2 ||
        rdata2.l3 != wdata2.l3 || rdata2.l4 != wdata2.l4) {
        H5_FAILED();
        AT();
        printf("incorrect read data\n");
        goto error;
 ...
 *-------------------------------------------------------------------------
 */
static int test_compound_15(void)
{
    typedef struct cmpd_struct
    {
        int i1;
 ...
 *-------------------------------------------------------------------------
 */
static int test_compound_15_attr(void)
{
    typedef struct cmpd_struct
    {
        int i1;
 ...
 *-------------------------------------------------------------------------
 */
static int test_compound_16(void)
{
    typedef struct cmpd_struct
    {
        int i1;
 ...
 *-------------------------------------------------------------------------
 */
static int test_compound_17(void)
{
    hid_t file;
 ...
 *-------------------------------------------------------------------------
 */
static int test_compound_18(void)
{
    hid_t file = H5I_INVALID_HID;
 ...
 *-------------------------------------------------------------------------
 */
static int test_user_compound_conversion(void)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_compound_member_convert_id_leak_func1(hid_t src_id,
                                                         hid_t dst_id,
                                                         H5T_cdata_t* cdata,
                                                         size_t nelmts,
                                                         size_t H5_ATTR_UNUSED buf_stride,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_compound_member_convert_id_leak_func2(hid_t src_id,
                                                         hid_t dst_id,
                                                         H5T_cdata_t* cdata,
                                                         size_t nelmts,
                                                         size_t H5_ATTR_UNUSED buf_stride,
 ...
 *-------------------------------------------------------------------------
 */
static int test_compound_member_convert_id_leak(void)
{
     ...
     */
    if (H5Tregister(H5T_PERS_HARD, "myflttodbl", H5T_NATIVE_FLOAT, H5T_NATIVE_DOUBLE, test_compound_member_convert_id_leak_func1) < 0) {
        TEST_ERROR;
    }
    ...
    if (H5Tunregister(H5T_PERS_HARD, "myflttodbl", H5T_NATIVE_FLOAT, H5T_NATIVE_DOUBLE, test_compound_member_convert_id_leak_func1) < 0) {
        TEST_ERROR;
    }
     ...
     * and convert the value again
     */
    if (H5Tregister(H5T_PERS_HARD, "myflttodbl", H5T_NATIVE_FLOAT, H5T_NATIVE_DOUBLE, test_compound_member_convert_id_leak_func2) < 0) {
        TEST_ERROR;
    }
    ...
    if (H5Tunregister(H5T_PERS_HARD, "myflttodbl", H5T_NATIVE_FLOAT, H5T_NATIVE_DOUBLE, test_compound_member_convert_id_leak_func2) < 0) {
        TEST_ERROR;
    }
...
error:
    H5E_BEGIN_TRY
    {
        H5Tunregister(H5T_PERS_HARD, "myflttodbl", H5T_NATIVE_FLOAT, H5T_NATIVE_DOUBLE, test_compound_member_convert_id_leak_func1);
        H5Tunregister(H5T_PERS_HARD, "myflttodbl", H5T_NATIVE_FLOAT, H5T_NATIVE_DOUBLE, test_compound_member_convert_id_leak_func2);
        H5Tclose(tid1);
        H5Tclose(tid2);
    }
    H5E_END_TRY;
 ...
 *-------------------------------------------------------------------------
 */
static int test_set_fields_offset(void)
{
    hid_t tid = H5I_INVALID_HID;
 ...
 *-------------------------------------------------------------------------
 */
static int test_transient(hid_t fapl)
{
    static hsize_t ds_size[2] = { 10, 20 };
    hid_t file = H5I_INVALID_HID, type = H5I_INVALID_HID, space = H5I_INVALID_HID, dset = H5I_INVALID_HID, t2 = H5I_INVALID_HID;
    char filename[1024];
 ...
 *-------------------------------------------------------------------------
 */
static int test_named(hid_t fapl)
{
    hid_t file = H5I_INVALID_HID, type = H5I_INVALID_HID, space = H5I_INVALID_HID, dset = H5I_INVALID_HID, t2 = H5I_INVALID_HID, t3 = H5I_INVALID_HID,
          attr1 = H5I_INVALID_HID;
    herr_t status;
    static hsize_t ds_size[2] = { 10, 20 };
    size_t i, j;
 ...
 *-------------------------------------------------------------------------
 */
static int test_str_create(void)
{
 ...
 *-------------------------------------------------------------------------
 */
static int test_conv_str_1(void)
{
    char* buf = NULL;
 ...
 *-------------------------------------------------------------------------
 */
static int test_conv_str_2(void)
{
    char *buf = NULL, s[80];
 ...
 *-------------------------------------------------------------------------
 */
static int test_conv_str_3(void)
{
    char* buf = NULL;
 ...
 *-------------------------------------------------------------------------
 */
static int test_conv_enum_1(void)
{
    const size_t nelmts = NTESTELEM;
    int i, val, *buf = NULL;
 ...
 *-------------------------------------------------------------------------
 */
static int test_conv_enum_2(void)
{
 ...
 *-------------------------------------------------------------------------
 */
static int test_conv_bitfield(void)
{
    unsigned char buf[4];
    hid_t st = H5I_INVALID_HID, dt = H5I_INVALID_HID;
 ...
 *-------------------------------------------------------------------------
 */
static int test_bitfield_funcs(void)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t convert_opaque(hid_t H5_ATTR_UNUSED st,
                             hid_t H5_ATTR_UNUSED dt,
                             H5T_cdata_t* cdata,
                             size_t H5_ATTR_UNUSED nelmts,
                             size_t H5_ATTR_UNUSED buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride,
                             void H5_ATTR_UNUSED* _buf,
                             void H5_ATTR_UNUSED* bkg,
...
static int test__Float16(void)
{
    ...
    if (is_little_endian) {
        ...
        if (strcmp(path->name, "fbo(opt)") != 0) {
            H5_FAILED();
            printf("Conversion path for H5T_NATIVE_FLOAT16 -> H5T_IEEE_F16LE was not a byte-swap conversion\n");
            goto error;
        }
    }
    ...
    if (is_little_endian) {
        if (strcmp(path->name, "fbo(opt)") != 0) {
            H5_FAILED();
            printf("Conversion path for H5T_NATIVE_FLOAT16 -> H5T_IEEE_F16BE was not a byte-swap conversion\n");
            goto error;
        }
    }
    else {
    ...
    if (is_little_endian) {
         ...
         */
        if (H5Tequal(H5T_NATIVE_SHORT, H5T_STD_I32LE) == true || H5Tequal(H5T_NATIVE_INT, H5T_STD_I32LE) == true ||
         ...
         * checking for a hard conversion path
         */
        if (H5Tequal(H5T_NATIVE_SHORT, H5T_STD_I32BE) == true || H5Tequal(H5T_NATIVE_INT, H5T_STD_I32BE) == true ||
    ...
    if (is_little_endian) {
         ...
         * checking for a hard conversion path
         */
        if (H5Tequal(H5T_NATIVE_FLOAT, H5T_IEEE_F32LE) == true || H5Tequal(H5T_NATIVE_DOUBLE, H5T_IEEE_F32LE) == true ||
    ...
    if ((dset_id = H5Dcreate2(fid, "Dataset", H5T_NATIVE_FLOAT16, space_id, H5P_DEFAULT, dcpl_id, H5P_DEFAULT)) < 0) {
        H5_FAILED();
        AT();
        printf("Can't create dataset\n");
        goto error;
...
static int test_bfloat16(void)
{
    ...
    if (lsb_pad != H5T_PAD_ZERO || msb_pad != H5T_PAD_ZERO) {
        H5_FAILED();
        printf(
            "Datatype padding type for H5T_FLOAT_BFLOAT16LE was incorrect (expected H5T_PAD_ZERO, got lsb "
            "pad "
    ...
    if (lsb_pad != H5T_PAD_ZERO || msb_pad != H5T_PAD_ZERO) {
        H5_FAILED();
        printf(
            "Datatype padding type for H5T_FLOAT_BFLOAT16BE was incorrect (expected H5T_PAD_ZERO, got lsb "
            "pad "
            "type %d, msb pad type %d)\n",
            lsb_pad,
            msb_pad);
    ...
    if (sign_pos != 15 || expo_pos != 7 || mant_pos != 0 || expo_size != 8 || mant_size != 7) {
        H5_FAILED();
        printf(
            "H5T_FLOAT_BFLOAT16LE didn't match bfloat16 specification "
    ...
    if (sign_pos != 15 || expo_pos != 7 || mant_pos != 0 || expo_size != 8 || mant_size != 7) {
        H5_FAILED();
        printf(
            "H5T_FLOAT_BFLOAT16BE didn't match bfloat16 specification "
            "(expected spos=15, epos=7, esize=8, mpos=0, msize=7, "
            "got spos=%zu, epos=%zu, esize=%zu, mpos=%zu, msize=%zu",
            sign_pos,
            expo_pos,
    ...
    if (inpad != H5T_PAD_ZERO) {
        H5_FAILED();
        printf(
            "Datatype unused bits padding type for H5T_FLOAT_BFLOAT16LE was incorrect (expected "
            "H5T_PAD_ZERO, "
            "got pad type %d)\n",
            inpad);
        goto error;
    ...
    if (inpad != H5T_PAD_ZERO) {
        H5_FAILED();
        printf(
            "Datatype unused bits padding type for H5T_FLOAT_BFLOAT16BE was incorrect (expected "
            "H5T_PAD_ZERO, "
            "got pad type %d)\n",
            inpad);
        goto error;
    ...
    if ((dset_id = H5Dcreate2(fid, "Dataset", H5T_FLOAT_BFLOAT16LE, space_id, H5P_DEFAULT, dcpl_id, H5P_DEFAULT)) < 0) {
        H5_FAILED();
        AT();
        printf("Can't create dataset\n");
        goto error;
...
static int test_fp8(void)
{
    ...
    if (lsb_pad != H5T_PAD_ZERO || msb_pad != H5T_PAD_ZERO) {
        H5_FAILED();
        printf(
            "Datatype padding type for H5T_FLOAT_F8E4M3 was incorrect (expected H5T_PAD_ZERO, got lsb pad "
            "type %d, msb pad type %d)\n",
            lsb_pad,
            msb_pad);
        goto error;
    ...
    if (lsb_pad != H5T_PAD_ZERO || msb_pad != H5T_PAD_ZERO) {
        H5_FAILED();
        printf(
            "Datatype padding type for H5T_FLOAT_F8E5M2 was incorrect (expected H5T_PAD_ZERO, got lsb pad "
            "type %d, msb pad type %d)\n",
            lsb_pad,
            msb_pad);
        goto error;
    ...
    if (sign_pos != 7 || expo_pos != 3 || mant_pos != 0 || expo_size != 4 || mant_size != 3) {
        H5_FAILED();
        printf(
            "H5T_FLOAT_F8E4M3 didn't match FP8 E4M3 specification "
    ...
    if (sign_pos != 7 || expo_pos != 2 || mant_pos != 0 || expo_size != 5 || mant_size != 2) {
        H5_FAILED();
        printf(
            "H5T_FLOAT_F8E5M2 didn't match FP8 E5M2 specification "
    ...
    if (inpad != H5T_PAD_ZERO) {
        H5_FAILED();
        printf(
            "Datatype unused bits padding type for H5T_FLOAT_F8E4M3 was incorrect (expected H5T_PAD_ZERO, "
            "got pad type %d)\n",
            inpad);
        goto error;
    }
    ...
    if (inpad != H5T_PAD_ZERO) {
        H5_FAILED();
        printf(
            "Datatype unused bits padding type for H5T_FLOAT_F8E5M2 was incorrect (expected H5T_PAD_ZERO, "
            "got pad type %d)\n",
            inpad);
        goto error;
    }
    ...
    if ((dset_id = H5Dcreate2(fid, "DatasetE4M3", H5T_FLOAT_F8E4M3, space_id, H5P_DEFAULT, dcpl_id, H5P_DEFAULT)) < 0) {
        H5_FAILED();
        AT();
        printf("Can't create dataset\n");
        goto error;
    ...
    if ((dset_id = H5Dcreate2(fid, "DatasetE5M2", H5T_FLOAT_F8E5M2, space_id, H5P_DEFAULT, dcpl_id, H5P_DEFAULT)) < 0) {
        H5_FAILED();
        AT();
        printf("Can't create dataset\n");
        goto error;
...
static int test_fp6(void)
{
    ...
    if (lsb_pad != H5T_PAD_ZERO || msb_pad != H5T_PAD_ZERO) {
        H5_FAILED();
        printf(
            "Datatype padding type for H5T_FLOAT_F6E2M3 was incorrect (expected H5T_PAD_ZERO, got lsb pad "
            "type %d, msb pad type %d)\n",
            lsb_pad,
            msb_pad);
        goto error;
    ...
    if (lsb_pad != H5T_PAD_ZERO || msb_pad != H5T_PAD_ZERO) {
        H5_FAILED();
        printf(
            "Datatype padding type for H5T_FLOAT_F6E3M2 was incorrect (expected H5T_PAD_ZERO, got lsb pad "
            "type %d, msb pad type %d)\n",
            lsb_pad,
            msb_pad);
        goto error;
    ...
    if (sign_pos != 5 || expo_pos != 3 || mant_pos != 0 || expo_size != 2 || mant_size != 3) {
        H5_FAILED();
        printf(
            "H5T_FLOAT_F6E2M3 didn't match FP6 E2M3 specification "
    ...
    if (sign_pos != 5 || expo_pos != 2 || mant_pos != 0 || expo_size != 3 || mant_size != 2) {
        H5_FAILED();
        printf(
            "H5T_FLOAT_F6E3M2 didn't match FP6 E3M2 specification "
    ...
    if (inpad != H5T_PAD_ZERO) {
        H5_FAILED();
        printf(
            "Datatype unused bits padding type for H5T_FLOAT_F6E2M3 was incorrect (expected H5T_PAD_ZERO, "
            "got pad type %d)\n",
            inpad);
        goto error;
    }
    ...
    if (inpad != H5T_PAD_ZERO) {
        H5_FAILED();
        printf(
            "Datatype unused bits padding type for H5T_FLOAT_F6E3M2 was incorrect (expected H5T_PAD_ZERO, "
            "got pad type %d)\n",
            inpad);
        goto error;
    }
    ...
    if ((dset_id = H5Dcreate2(fid, "DatasetE2M3", H5T_FLOAT_F6E2M3, space_id, H5P_DEFAULT, dcpl_id, H5P_DEFAULT)) < 0) {
        H5_FAILED();
        AT();
        printf("Can't create dataset\n");
        goto error;
    ...
    if ((dset_id = H5Dcreate2(fid, "DatasetE3M2", H5T_FLOAT_F6E3M2, space_id, H5P_DEFAULT, dcpl_id, H5P_DEFAULT)) < 0) {
        H5_FAILED();
        AT();
        printf("Can't create dataset\n");
        goto error;
...
static int test_fp4(void)
{
    ...
    if (lsb_pad != H5T_PAD_ZERO || msb_pad != H5T_PAD_ZERO) {
        H5_FAILED();
        printf(
            "Datatype padding type for H5T_FLOAT_F4E2M1 was incorrect (expected H5T_PAD_ZERO, got lsb pad "
            "type %d, msb pad type %d)\n",
            lsb_pad,
            msb_pad);
        goto error;
    ...
    if (sign_pos != 3 || expo_pos != 1 || mant_pos != 0 || expo_size != 2 || mant_size != 1) {
        H5_FAILED();
        printf(
            "H5T_FLOAT_F4E2M1 didn't match FP4 E2M1 specification "
    ...
    if (inpad != H5T_PAD_ZERO) {
        H5_FAILED();
        printf(
            "Datatype unused bits padding type for H5T_FLOAT_F4E2M1 was incorrect (expected H5T_PAD_ZERO, "
            "got pad type %d)\n",
            inpad);
        goto error;
    }
    ...
    if ((dset_id = H5Dcreate2(fid, "DatasetE2M1", H5T_FLOAT_F4E2M1, space_id, H5P_DEFAULT, dcpl_id, H5P_DEFAULT)) < 0) {
        H5_FAILED();
        AT();
        printf("Can't create dataset\n");
        goto error;
 ...
 *-------------------------------------------------------------------------
 */
static int test_complex_type(void)
{
#ifdef H5_HAVE_COMPLEX_NUMBERS
    const char* driver_name;
    char filename[256];
    ...
    if ((dset_id = H5Dcreate2(fid, "Dataset", H5T_NATIVE_FLOAT_COMPLEX, space_id, H5P_DEFAULT, dcpl_id, H5P_DEFAULT)) < 0) {
        H5_FAILED();
        AT();
        printf("Can't create dataset\n");
        goto error;
 ...
 *-------------------------------------------------------------------------
 */
static int test_complex_type_conv_funcs(void)
{
    ...
    if (strcmp(conv_path->name, "no-op") != 0) {
        H5_FAILED();
        printf("Conversion path for H5T_NATIVE_DOUBLE_COMPLEX -> H5T_NATIVE_DOUBLE_COMPLEX was not a no-op\n");
        goto error;
    }
    ...
    if (strcmp(conv_path->name, "no-op") != 0) {
        H5_FAILED();
        printf("Conversion path for H5T_NATIVE_LDOUBLE_COMPLEX -> H5T_NATIVE_LDOUBLE_COMPLEX was not a no-op\n");
        goto error;
    }
    ...
    if (strcmp(conv_path->name, "complexbo") != 0) {
        H5_FAILED();
        printf(
            "Conversion path for H5T_COMPLEX_IEEE_F32LE -> H5T_COMPLEX_IEEE_F32BE was not a byte-swap "
            "conversion\n");
        goto error;
    }
    ...
    if (strcmp(conv_path->name, "complexbo") != 0) {
        H5_FAILED();
        printf(
            "Conversion path for H5T_COMPLEX_IEEE_F32BE -> H5T_COMPLEX_IEEE_F32LE was not a byte-swap "
            "conversion\n");
        goto error;
    }
    ...
    if (!H5Tequal(conv_type, H5T_NATIVE_FLOAT_COMPLEX) && !H5Tequal(conv_type, H5T_NATIVE_DOUBLE_COMPLEX) && !H5Tequal(conv_type, H5T_NATIVE_LDOUBLE_COMPLEX)) {
 ...
 *-------------------------------------------------------------------------
 */
static int test_complex_array_compat_conv(void)
{
    const H5T_path_t* conv_path;
     ...
     */
    if ((dset_id = H5Dcreate2(file_id, "DatasetArray", array_type_id, space_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        H5_FAILED();
        printf("Can't create dataset\n");
        goto error;
    }
     ...
     */
    if ((dset_id = H5Dcreate2(file_id, "DatasetFloatComplex", complex_type_id, space_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        H5_FAILED();
        printf("Can't create dataset\n");
        goto error;
    }
 ...
 *-------------------------------------------------------------------------
 */
static int test_complex_compound_compat_conv(void)
{
    const H5T_path_t* conv_path;
    const H5T_t* complex_type = NULL;
    ...
    cc_compat fake_data[4] = { { .real = 1.0, .imag = 2.0 }, { .real = 3.0, .imag = 4.0 }, { .real = 5.0, .imag = 6.0 }, { .real = 7.0, .imag = 8.0 } };
     ...
     * to it using a matching float complex type.
     */
    if ((dset_id = H5Dcreate2(file_id, "DatasetCompound", compound_type_id, space_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        H5_FAILED();
        printf("Can't create dataset\n");
        goto error;
    }
 ...
 *-------------------------------------------------------------------------
 */
static int test_array_cmpd_vl(void)
{
    typedef struct cmpd_struct
    {
        hvl_t vl;
    ...
    if ((dset_id = H5Dcreate2(file, "Dataset", outer_array_tid, space_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        H5_FAILED();
        AT();
        printf("Can't create dataset\n");
        goto error;
 ...
 *-------------------------------------------------------------------------
 */
static int test_set_size_invalid(void)
{
 ...
 *-------------------------------------------------------------------------
 */
static int test_encode(bool H5_ATTR_DEPRECATED_USED use_old_decode_api)
{
    struct cmpd
    {
        int a;
    ...
    snprintf(test_msg, sizeof(test_msg), "%s functions of encoding and decoding datatypes", use_old_decode_api ? "old" : "new");
    TESTING(test_msg);
...
static H5T_conv_ret_t conv_except(H5T_conv_except_t except_type,
                                  hid_t H5_ATTR_UNUSED src_id,
                                  hid_t H5_ATTR_UNUSED dst_id,
                                  void H5_ATTR_UNUSED* src_buf,
                                  void H5_ATTR_UNUSED* dst_buf,
                                  void* _user_data)
{
 ...
 *-------------------------------------------------------------------------
 */
static int test_int_float_except(void)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_app_conv_ids_func(hid_t src_id,
                                     hid_t dst_id,
                                     H5T_cdata_t* cdata,
                                     size_t H5_ATTR_UNUSED nelmts,
                                     size_t H5_ATTR_UNUSED buf_stride,
 ...
 *-------------------------------------------------------------------------
 */
static int test_app_conv_ids(void)
{
    ...
    for (int src_type = H5T_INTEGER; src_type < H5T_NCLASSES; src_type++) {
        ...
        for (int dst_type = H5T_INTEGER; dst_type < H5T_NCLASSES; dst_type++) {
            ...
            if (H5Tregister(H5T_PERS_SOFT, "app_conv_ids_func", src_type_id, dst_type_id, &test_app_conv_ids_func) < 0) {
                TEST_ERROR;
            }
            ...
            if (H5Tunregister(H5T_PERS_SOFT, "app_conv_ids_func", src_type_id, dst_type_id, &test_app_conv_ids_func) < 0) {
                TEST_ERROR;
            }
    ...
    if (H5Tregister(H5T_PERS_HARD, "app_conv_ids_func", H5T_NATIVE_INT, H5T_NATIVE_LONG, &test_app_conv_ids_func) < 0) {
        TEST_ERROR;
    }
    ...
    if (H5Tunregister(H5T_PERS_HARD, "app_conv_ids_func", H5T_NATIVE_INT, H5T_NATIVE_LONG, &test_app_conv_ids_func) < 0) {
        TEST_ERROR;
    }
 ...
 *-------------------------------------------------------------------------
 */
static int test_set_order(void)
{
 ...
 *-------------------------------------------------------------------------
 */
static int test_set_order_compound(hid_t fapl)
{
    typedef struct
    { /* Struct with atomic fields */
        int i;
    ...
    hid_t file = H5I_INVALID_HID;
    hid_t cmpd = H5I_INVALID_HID, memb_cmpd = H5I_INVALID_HID, memb_array1 = H5I_INVALID_HID, memb_array2 = H5I_INVALID_HID, cmpd_array = H5I_INVALID_HID;
...
static int test_enum_member_order(void)
{
    typedef enum
    {
        SOLID,
 ...
 *-------------------------------------------------------------------------
 */
static int test_named_indirect_reopen(hid_t fapl)
{
    hid_t file = H5I_INVALID_HID, type = H5I_INVALID_HID, reopened_type = H5I_INVALID_HID, strtype = H5I_INVALID_HID, dset = H5I_INVALID_HID,
 ...
 *-------------------------------------------------------------------------
 */
static int test_named_indirect_reopen_file(hid_t fapl)
{
    hid_t file = H5I_INVALID_HID;
...
static void create_del_obj_named_test_file(const char* filename, hid_t fapl, H5F_libver_t low, H5F_libver_t high)
{
    hid_t file;                          /* File ID */
 ...
 *-------------------------------------------------------------------------
 */
static int test_delete_obj_named(hid_t fapl)
{
 ...
 *-------------------------------------------------------------------------
 */
static int test_delete_obj_named_fileid(hid_t fapl)
{
    hid_t filea1 = H5I_INVALID_HID, filea2 = H5I_INVALID_HID, fileb = H5I_INVALID_HID; /* File IDs */
 ...
 */
#ifndef H5_NO_DEPRECATED_SYMBOLS
static int test_deprec(hid_t fapl)
{
    hid_t file = H5I_INVALID_HID; /* File ID */
    hid_t type = H5I_INVALID_HID; /* Datatype ID */
    unsigned rank = 2;            /* Rank for array datatype */
 ...
 *-------------------------------------------------------------------------
 */
static int test_utf_ascii_conv(void)
{
    hid_t fid = H5I_INVALID_HID;
    hid_t did = H5I_INVALID_HID;
 ...
 *
 *************************************************************************/
static herr_t verify_version(hid_t dtype, H5F_libver_t low, H5F_libver_t high, unsigned* highest_version)
{
    ...
    case H5T_OPAQUE:
    case H5T_REFERENCE:
    default           : PUTS_ERROR("invalid datatype class for test"); TEST_ERROR;
    } /* end switch */
...
static herr_t test_versionbounds_create_datatype(H5F_libver_t low, H5F_libver_t high, hid_t* dtype_out)
{
    ...
    typedef struct
    { /* Struct for the simple compound type */
        int single_int;
    ...
    typedef struct
    { /* Struct for the inner compound type */
        simple_cmp_t inner_arr[ARRAY_LEN];
        hvl_t vlen_float;
    } inner_cmp_t;

    typedef struct
    { /* Struct for the outer compound type */
    ...
    if (high >= H5F_LIBVER_V200) {
        ...
        ret = H5Tinsert(simple_cmp_type, "float_complex", HOFFSET(simple_cmp_t, float_complex_sim), float_cpxtype);
        if (ret < 0) {
            PUTS_ERROR("couldn't insert compound datatype member");
        }
        ...
        ret = H5Tinsert(simple_cmp_type, "native_float_complex", HOFFSET(simple_cmp_t, float_complex_mem), H5T_NATIVE_FLOAT_COMPLEX);
        if (ret < 0) {
            PUTS_ERROR("couldn't insert compound datatype member");
        }
#endif
...
static int test_versionbounds(void)
{
    hid_t file = H5I_INVALID_HID;       /* File ID */
    ...
    H5F_libver_t versions[] = { H5F_LIBVER_EARLIEST, H5F_LIBVER_V18, H5F_LIBVER_V110, H5F_LIBVER_V112, H5F_LIBVER_V114, H5F_LIBVER_V200 };
...
static int test_complex_type_versionbounds(void)
{
    H5F_libver_t versions[] = { H5F_LIBVER_EARLIEST, H5F_LIBVER_V18, H5F_LIBVER_V110, H5F_LIBVER_V112, H5F_LIBVER_V114, H5F_LIBVER_V200 };
    ...
    for (low = 0; low < versions_count; low++) {
        for (high = 0; high < versions_count; high++) {
            ...
            if (high < H5F_LIBVER_V200) {
                ...
                H5E_BEGIN_TRY
                {
                    dset = H5Dcreate2(file, VERDSNAME, complex_dtype, space, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
                }
                H5E_END_TRY;
                if (dset >= 0) {
                    TEST_ERROR;
                ...
                H5E_BEGIN_TRY
                {
                    dset = H5Dcreate2(file, VERDSNAME, array_dtype, space, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
                }
                H5E_END_TRY;
                if (dset >= 0) {
                    TEST_ERROR;
                ...
                H5E_BEGIN_TRY
                {
                    dset = H5Dcreate2(file, VERDSNAME, vlen_dtype, space, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
                }
                H5E_END_TRY;
                if (dset >= 0) {
                    TEST_ERROR;
                ...
                H5E_BEGIN_TRY
                {
                    dset = H5Dcreate2(file, VERDSNAME, compound_dtype, space, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
                }
                H5E_END_TRY;
                if (dset >= 0) {
                    TEST_ERROR;
