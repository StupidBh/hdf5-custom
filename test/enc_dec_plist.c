...
static int test_encode_decode(hid_t orig_pl, H5F_libver_t low, H5F_libver_t high)
{
 ...
 * correctly */
static int test_encode_decode_page_buf_size(H5F_libver_t low, H5F_libver_t high)
{
...
int main(void)
{
    ...
    H5AC_cache_image_config_t my_cache_image_config = { H5AC__CURR_CACHE_IMAGE_CONFIG_VERSION, true, false, -1 };
    ...
    for (low = H5F_LIBVER_EARLIEST; low < H5F_LIBVER_NBOUNDS; low++) {
        for (high = H5F_LIBVER_EARLIEST; high < H5F_LIBVER_NBOUNDS; high++) {
            ...
            snprintf(msg, sizeof(msg), "Testing ENCODE/DECODE with file version bounds: (%s, %s):", low_string, high_string);
            puts(msg);
            ...
            if ((H5Pset_meta_block_size(fapl, 2'098'452)) < 0) {
                FAIL_STACK_ERROR;
            }
            if ((H5Pset_sieve_buf_size(fapl, 1'048'576)) < 0) {
                FAIL_STACK_ERROR;
            }
            if ((H5Pset_alignment(fapl, 2, 1024)) < 0) {
                FAIL_STACK_ERROR;
            }
            if ((H5Pset_cache(fapl, 1024, 128, 10'485'760, 0.3)) < 0) {
                FAIL_STACK_ERROR;
            }
            if ((H5Pset_elink_file_cache_size(fapl, 10'485'760)) < 0) {
                FAIL_STACK_ERROR;
            }
