...
static int test_plists(const char* filename_prefix)
{
    ...
    for (config_1 = 0; config_1 < (NCONFIG - 1); config_1++) {
        for (config_2 = config_1 + 1; config_2 < NCONFIG; config_2++) {
            ...
            if (snprintf(filename, sizeof(filename), "%s%s%s", filename_prefix, config_1 & CONFIG_64 ? "64" : "32", config_1 & CONFIG_LE ? "le" : "be") < 0) {
                TEST_ERROR;
            }
            ...
            if (snprintf(filename, sizeof(filename), "%s%s%s", filename_prefix, config_2 & CONFIG_64 ? "64" : "32", config_2 & CONFIG_LE ? "le" : "be") < 0) {
                TEST_ERROR;
            }
