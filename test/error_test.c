 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_error(hid_t file)
{
    ...
    if ((dataset = H5Dcreate2(file, DSET_NAME, H5T_STD_I32BE, space, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        H5Epush(H5E_DEFAULT, __FILE__, FUNC_test_error, __LINE__, ERR_CLS, ERR_MAJ_IO, ERR_MIN_CREATE, "H5Dcreate2 failed");
        goto error;
    }
    ...
    if (H5Dwrite(FAKE_ID, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, ipoints2) >= 0) {
        H5Epush(H5E_DEFAULT, __FILE__, FUNC_test_error, __LINE__, ERR_CLS, ERR_MAJ_IO, ERR_MIN_WRITE, "H5Dwrite shouldn't succeed");
        goto error;
    }
    ...
    if (H5Dwrite(FAKE_ID, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, ipoints2) < 0) {
        H5Epush(H5E_DEFAULT, __FILE__, FUNC_test_error, __LINE__, ERR_CLS, ERR_MAJ_IO, ERR_MIN_WRITE, "H5Dwrite failed as supposed to");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t error_stack(void)
{
    ssize_t err_num;
    ...
    if ((err_num = H5Eget_num(ERR_STACK)) == 0) {
        H5Epush(ERR_STACK, __FILE__, FUNC_error_stack, __LINE__, ERR_CLS, ERR_MAJ_API, ERR_MIN_GETNUM, "Get number test failed, returned %d", (int)err_num);
        goto error;
    }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t long_desc_cb(unsigned H5_ATTR_UNUSED n, const H5E_error2_t* err_desc, void* client_data)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_long_desc(void)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t dump_error(hid_t estack)
{
...
static herr_t custom_print_cb(unsigned n, const H5E_error2_t* err_desc, void* client_data)
{
    ...
    fprintf(stream, "%*serror #%03d: %s in %s(): line %u\n", indent, "", n, err_desc->file_name, err_desc->func_name, err_desc->line);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_create(void)
{
    ...
    /* Push an error with a long description */
    if (H5Epush(estack_id, __FILE__, __func__, __LINE__, ERR_CLS, ERR_MAJ_TEST, ERR_MIN_SUBROUTINE, "%s", err_msg) < 0) {
        TEST_ERROR;
    }
...
static herr_t test_copy(void)
{
    ...
    /* Push an error with a long description */
    if (H5Epush(H5E_DEFAULT, __FILE__, __func__, __LINE__, ERR_CLS, ERR_MAJ_TEST, ERR_MIN_SUBROUTINE, "%s", err_msg) < 0) {
        TEST_ERROR;
    }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_append(void)
{
    ...
    if (H5Epush(estack_id2, __FILE__, __func__, __LINE__, ERR_CLS, ERR_MAJ_IO, ERR_MIN_CREATE, "%s", err_msg2) < 0) {
        TEST_ERROR;
    }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_overflow_stack(void)
{
    ssize_t err_num;
    char err_buf[32];
    ...
    for (int i = 0; i < H5E_MAX_ENTRIES; i++) {
        ...
        if (H5Epush(H5E_DEFAULT, __FILE__, __func__, __LINE__, ERR_CLS, ERR_MAJ_TEST, ERR_MIN_SUBROUTINE, "%s", err_buf) < 0) {
            TEST_ERROR;
        }
    }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t close_error(void)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_filter_error(const char* fname, hid_t fapl)
{
...
int main(void)
{
    ...
    if (error_stack() < 0) {
        ...
        if (H5Epush(ERR_STACK, __FILE__, FUNC_main, __LINE__, ERR_CLS, ERR_MAJ_TEST, ERR_MIN_ERRSTACK, "Error stack test failed") < 0) {
            TEST_ERROR;
        }
        ...
        H5Epush(ERR_STACK, __FILE__, FUNC_main, __LINE__, ERR_CLS2, ERR_MAJ_TEST, ERR_MIN_ERRSTACK, "Error stack test failed");
    ...
    if (test_error(file) < 0) {
        H5Epush(H5E_DEFAULT, __FILE__, FUNC_main, __LINE__, ERR_CLS, ERR_MAJ_TEST, ERR_MIN_SUBROUTINE, "Error test failed, %s", "it's wrong");
        estack_id = H5Eget_current_stack();
