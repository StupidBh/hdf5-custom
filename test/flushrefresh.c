...
herr_t refresh_verification(const char* obj_pathname)
{
    ...
    do {
        ...
        else if ((strcmp(obj_pathname, D3) == 0) || (strcmp(obj_pathname, G3) == 0) || (strcmp(obj_pathname, T3) == 0)) {
            if (H5Orefresh(oid) < 0) {
                PROCESS_ERROR;
            }
        } /* end if */
        ...
        if ((!token_cmp) && (flushed_oinfo.type == refreshed_oinfo.type) && (flushed_oinfo.num_attrs != refreshed_oinfo.num_attrs) &&
            (flushed_ninfo.hdr.version == refreshed_ninfo.hdr.version) && (flushed_ninfo.hdr.flags == refreshed_ninfo.hdr.flags) &&
            (flushed_ninfo.hdr.nmesgs != refreshed_ninfo.hdr.nmesgs) && (flushed_ninfo.hdr.nchunks != refreshed_ninfo.hdr.nchunks) &&
    ...
    if (!ok) {
        printf("FLUSHED: num_attrs=%d, nmesgs=%d, nchunks=%d, total=%d\n",
               ...
               (int)flushed_ninfo.hdr.nmesgs,
               (int)flushed_ninfo.hdr.nchunks,
               (int)flushed_ninfo.hdr.space.total);
        printf("REFRESHED: num_attrs=%d, nmesgs=%d, nchunks=%d, total=%d\n",
               (int)refreshed_oinfo.num_attrs,
               (int)refreshed_ninfo.hdr.nmesgs,
               (int)refreshed_ninfo.hdr.nchunks,
