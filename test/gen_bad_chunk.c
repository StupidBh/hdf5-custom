...
static unsigned char* slurp(const char* name, size_t* len_out)
{
...
static int spew(const char* name, const unsigned char* buf, size_t len)
{
...
static size_t find_once(const unsigned char* buf, size_t len, const unsigned char* pat, size_t patlen)
{
...
int main(void)
{
    ...
    if (H5Dclose(dset) < 0 || H5Pclose(dcpl) < 0 || H5Sclose(sid) < 0 || H5Fclose(file) < 0 || H5Pclose(fapl) < 0) {
        TEST_ERROR;
    }
    ...
    if (off < 11 || buf[off - 11] != 3 /* version */ || buf[off - 10] != 2 /* chunked */ || buf[off - 9] != 4 /* ndims */) {
        TEST_ERROR;
    }
    buf[off - 9] = 3;
