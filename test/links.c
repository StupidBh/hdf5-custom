...
static herr_t UD_hard_create_deprec(const char* link_name, hid_t loc_group, const void* udata, size_t udata_size, hid_t lcpl_id);
static hid_t UD_hard_traverse_deprec(const char* link_name, hid_t cur_group, const void* udata, size_t udata_size, hid_t lapl_id, hid_t dxpl_id);
...
static hid_t UD_rereg_traverse(const char* link_name, hid_t cur_group, const void* udata, size_t udata_size, hid_t lapl_id, hid_t dxpl_id);
...
static herr_t UD_cb_create(const char* link_name, hid_t loc_group, const void* udata, size_t udata_size, hid_t lcpl_id);
static herr_t UD_cb_move(const char* new_name, hid_t new_loc, const void* udata, size_t udata_size);
static hid_t UD_cb_traverse(const char* link_name, hid_t cur_group, const void* udata, size_t udata_size, hid_t lapl_id, hid_t dxpl_id);
static herr_t UD_cb_delete(const char* link_name, hid_t file, const void* udata, size_t udata_size);
static ssize_t UD_cb_query(const char* link_name, const void* udata, size_t udata_size, void* buf, size_t buf_size);
 ...
 *-------------------------------------------------------------------------
 */
static void fix_ext_filename(char* path_name, char* cwd, const char* file_name)
{
 ...
 *-------------------------------------------------------------------------
 */
static int mklinks(hid_t fapl, bool new_format)
{
 ...
 *-------------------------------------------------------------------------
 */
static int new_links(hid_t fapl, bool new_format)
{
    hid_t file_a, file_b = (H5I_INVALID_HID);
    hid_t grp1_a = (H5I_INVALID_HID), grp1_b = (H5I_INVALID_HID), grp2_a = (H5I_INVALID_HID), grp2_b = (H5I_INVALID_HID);
    ...
    /* Create datasets */
    if ((dset1 = H5Dcreate2(file_a, "dataset1", H5T_NATIVE_INT, scalar, H5P_DEFAULT, dcpl_g, H5P_DEFAULT)) < 0) {
        TEST_ERROR;
    }
    if ((dset2 = H5Dcreate2(grp1_a, "dataset2", H5T_NATIVE_INT, scalar, H5P_DEFAULT, dcpl_g, H5P_DEFAULT)) < 0) {
        TEST_ERROR;
    }
 ...
 *-------------------------------------------------------------------------
 */
static int cklinks(hid_t fapl, bool new_format)
{
    hid_t file;
 ...
 *-------------------------------------------------------------------------
 */
static int ck_new_links(hid_t fapl, bool new_format)
{
    hid_t file;
 ...
 *-------------------------------------------------------------------------
 */
static int long_links(hid_t fapl, bool new_format)
{
 ...
 *-------------------------------------------------------------------------
 */
static int toomany(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID;                         /* File ID */
 ...
 *-------------------------------------------------------------------------
 */
static int test_lcpl(hid_t fapl, bool new_format)
{
    ...
    if ((dset_id = H5Dcreate2(file_id, "/dataset", H5T_NATIVE_INT, space_id, H5P_DEFAULT, dcpl_g, H5P_DEFAULT)) < 0) {
        TEST_ERROR;
    }
    ...
    if ((dset_id = H5Dcreate2(file_id, "/dataset2", H5T_NATIVE_INT, space_id, lcpl_id, dcpl_g, H5P_DEFAULT)) < 0) {
        TEST_ERROR;
    }
    if (H5Dclose(dset_id) < 0) {
        TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_move(hid_t fapl, bool new_format)
{
    hid_t file_a = H5I_INVALID_HID, file_b = H5I_INVALID_HID;
    hid_t grp_1 = H5I_INVALID_HID, grp_2 = H5I_INVALID_HID, grp_move = H5I_INVALID_HID, moved_grp = H5I_INVALID_HID;
    char filename[1024];
    herr_t status; /* Generic return value */
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy(hid_t fapl, bool new_format)
{
    hid_t file_a = H5I_INVALID_HID, file_b = H5I_INVALID_HID;
    hid_t grp_1 = H5I_INVALID_HID, grp_2 = H5I_INVALID_HID, grp_move = H5I_INVALID_HID, moved_grp = H5I_INVALID_HID;
    char filename[1024];
 ...
 *-------------------------------------------------------------------------
 */
static int test_move_preserves(hid_t fapl_id, bool new_format)
{
    hid_t file_id = H5I_INVALID_HID;
    hid_t group_id = H5I_INVALID_HID;
...
static int test_deprec(hid_t fapl, bool new_format)
{
    hid_t file_id = H5I_INVALID_HID;
 ...
 *-------------------------------------------------------------------------
 */
static int cklinks_deprec(hid_t fapl, bool new_format)
{
    hid_t file;
 ...
 *-------------------------------------------------------------------------
 */
static int test_lcpl_deprec(hid_t fapl, bool new_format)
{
    hid_t file_id = H5I_INVALID_HID;
    hid_t group_id = H5I_INVALID_HID;
    hid_t space_id = H5I_INVALID_HID;
 ...
 *-------------------------------------------------------------------------
 */
static int test_move_preserves_deprec(hid_t fapl_id, bool new_format)
{
    hid_t file_id = H5I_INVALID_HID;
    hid_t group_id = H5I_INVALID_HID;
    hid_t fcpl_id = H5I_INVALID_HID; /* Group creation property list ID */
    ...
    if (new_format) {
        TESTING(
            "moving and copying links using deprecated routines preserves their properties (w/new group "
            "format)");
    }
    else {
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_root_deprec(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID;                         /* File ID */
    hid_t gid = H5I_INVALID_HID, gid2 = H5I_INVALID_HID; /* Group IDs */
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_query_deprec(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID;                             /* File ID */
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_closing_deprec(hid_t fapl, bool new_format)
{
    hid_t fid1 = (H5I_INVALID_HID), fid2 = (H5I_INVALID_HID), fid3 = (H5I_INVALID_HID), fid4 = (H5I_INVALID_HID);
    hid_t gid = (H5I_INVALID_HID), tid = (H5I_INVALID_HID), tid2 = (H5I_INVALID_HID), sid = (H5I_INVALID_HID), did = (H5I_INVALID_HID);
    hid_t lcpl_id = (H5I_INVALID_HID);
    hsize_t dims[2];
    char filename1[NAME_BUF_SIZE], filename2[NAME_BUF_SIZE], filename3[NAME_BUF_SIZE], filename4[NAME_BUF_SIZE], /* Names of files to externally link across */
    ...
    if ((did = H5Dcreate2(fid1, "elink/elink/elink/dataset1", tid2, sid, H5P_DEFAULT, dcpl_g, H5P_DEFAULT)) < 0) {
        TEST_ERROR;
    }
    ...
    /* Test move */
    if (H5Lmove(fid1, "elink/elink/elink/group1", fid1, "elink/elink/elink/group1_moved", H5P_DEFAULT, H5P_DEFAULT) < 0) {
        TEST_ERROR;
    }
    ...
    if (H5Lcopy(H5L_SAME_LOC, "elink/elink/elink", fid1, "elink/elink/elink/elink_copied2", H5P_DEFAULT, H5P_DEFAULT) < 0) {
        FAIL_STACK_ERROR;
    }
    ...
    if ((gid = H5Gcreate2(fid1, "elink/elink2/group2/group3/group4", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        TEST_ERROR;
    }
    if (H5Gclose(gid) < 0) {
        TEST_ERROR;
     ...
     * points to file 4.
     */
    if ((gid = H5Gcreate2(fid1, "elink/file2group1/file2group2/slink/group3", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        TEST_ERROR;
    }
    if (H5Gclose(gid) < 0) {
        TEST_ERROR;
...
static herr_t UD_hard_create_deprec(const char H5_ATTR_UNUSED* link_name, hid_t loc_group, const void* udata, size_t udata_size, hid_t H5_ATTR_UNUSED lcpl_id)
{
    haddr_t addr;
...
static hid_t UD_hard_traverse_deprec(const char H5_ATTR_UNUSED* link_name,
                                     hid_t cur_group,
                                     const void* udata,
                                     size_t udata_size,
...
static herr_t UD_hard_delete_deprec(const char H5_ATTR_UNUSED* link_name, hid_t file, const void* udata, size_t udata_size)
{
    haddr_t addr;
    hid_t target_obj = H5I_INVALID_HID;
    herr_t ret_value = 0;
...
static int ud_hard_links_deprec(hid_t fapl)
{
    hid_t fid = H5I_INVALID_HID;                         /* File ID */
    hid_t gid = H5I_INVALID_HID, gid2 = H5I_INVALID_HID; /* Group IDs */
    H5L_info1_t li;                                      /* Link information */
     ...
     * from H5Lget_info1 */
    if (H5Lcreate_ud(fid, "ud_link", (H5L_type_t)UD_HARD_TYPE, &(li.u.address), (size_t)sizeof(haddr_t), H5P_DEFAULT, H5P_DEFAULT) < 0) {
        TEST_ERROR;
    }
...
static int ud_link_reregister_deprec(hid_t fapl)
{
    hid_t fid = H5I_INVALID_HID;                         /* File ID */
    hid_t gid = H5I_INVALID_HID, gid2 = H5I_INVALID_HID; /* Group IDs */
    H5L_info1_t li;                                      /* Link information */
    ...
    if (H5Lcreate_ud(fid, "ud_link", (H5L_type_t)UD_HARD_TYPE, &(li.u.address), sizeof(li.u.address), H5P_DEFAULT, H5P_DEFAULT) < 0) {
        TEST_ERROR;
    }
...
static int ud_callbacks_deprec(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID;          /* File ID */
    hid_t gid = H5I_INVALID_HID;          /* Group ID */
    ...
    if (H5Lcreate_ud(fid, UD_CB_LINK_NAME, (H5L_type_t)UD_CB_TYPE, ud_target_name, (size_t)UD_CB_TARGET_LEN, H5P_DEFAULT, H5P_DEFAULT) < 0) {
        TEST_ERROR;
    }
    if (H5Gclose(gid) < 0) {
        TEST_ERROR;
    ...
    if (H5Lcreate_ud(fid, UD_CB_LINK_NAME, (H5L_type_t)UD_CB_TYPE, ud_target_name, (size_t)UD_CB_TARGET_LEN, lcpl, H5P_DEFAULT) < 0) {
        FAIL_STACK_ERROR;
    }
    if (H5Pclose(lcpl) < 0) {
        FAIL_STACK_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int lapl_nlinks_deprec(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID;                                                  /* File ID */
    hid_t gid = H5I_INVALID_HID, gid2 = H5I_INVALID_HID;                          /* Group IDs */
 ...
 *-------------------------------------------------------------------------
 */
static int linkinfo_deprec(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID;                        /* File ID */
    hid_t gid = H5I_INVALID_HID;                        /* Group ID */
    ...
    if (H5Lcreate_ud(fid, "ud_link", (H5L_type_t)UD_PLIST_TYPE, NULL, (size_t)0, H5P_DEFAULT, H5P_DEFAULT) < 0) {
        TEST_ERROR;
    }
 ...
 *-------------------------------------------------------------------------
 */
static int corder_create_compact_deprec(hid_t fapl)
{
 ...
 *-------------------------------------------------------------------------
 */
static int corder_create_dense_deprec(hid_t fapl)
{
    hid_t file_id = H5I_INVALID_HID;                               /* File ID */
    hid_t group_id = H5I_INVALID_HID, group_id2 = H5I_INVALID_HID; /* Group IDs */
    hid_t gcpl_id = H5I_INVALID_HID;                               /* Group creation property list ID */
 ...
 *-------------------------------------------------------------------------
 */
static int link_info_by_idx_check_deprec(hid_t group_id, const char* linkname, hsize_t n, bool hard_link, bool use_index)
{
    ...
    if (H5Lget_info_by_idx1(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC, (hsize_t)0, &linfo, H5P_DEFAULT) < 0) {
        TEST_ERROR;
    }
    if (linfo.corder != 0) {
        TEST_ERROR;
    ...
    if (!hard_link) {
        ...
        if (H5Lget_val_by_idx(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC, n, tmpval, (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0) {
            TEST_ERROR;
        }
    ...
    if (H5Lget_name_by_idx(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC, n, tmpname, (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0) {
        TEST_ERROR;
    }
    ...
    if (use_index) {
        ...
        memset(&linfo, 0, sizeof(linfo));
        if (H5Lget_info_by_idx1(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_NATIVE, (hsize_t)0, &linfo, H5P_DEFAULT) < 0) {
            TEST_ERROR;
        }
        if (linfo.corder != 0) {
            TEST_ERROR;
        ...
        memset(&linfo, 0, sizeof(linfo));
        if (H5Lget_info_by_idx1(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_NATIVE, n, &linfo, H5P_DEFAULT) < 0) {
            TEST_ERROR;
        }
        if (linfo.corder != (int64_t)n) {
            TEST_ERROR;
        ...
        if (!hard_link) {
            memset(tmpval, 0, (size_t)NAME_BUF_SIZE);
            if (H5Lget_val_by_idx(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_NATIVE, n, tmpval, (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0) {
                TEST_ERROR;
            }
            if (strcmp(valname, tmpval) != 0) {
                TEST_ERROR;
        ...
        memset(tmpname, 0, (size_t)NAME_BUF_SIZE);
        if (H5Lget_name_by_idx(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_NATIVE, n, tmpname, (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0) {
            TEST_ERROR;
        }
        if (strcmp(linkname, tmpname) != 0) {
            TEST_ERROR;
    ...
    memset(&linfo, 0, sizeof(linfo));
    if (H5Lget_info_by_idx1(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_DEC, (hsize_t)0, &linfo, H5P_DEFAULT) < 0) {
        TEST_ERROR;
    }
    if (linfo.corder != (int64_t)n) {
        TEST_ERROR;
    ...
    if (!hard_link) {
        memset(tmpval, 0, (size_t)NAME_BUF_SIZE);
        if (H5Lget_val_by_idx(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_DEC, (hsize_t)0, tmpval, (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0) {
            TEST_ERROR;
        }
        if (strcmp(valname, tmpval) != 0) {
            TEST_ERROR;
    ...
    memset(tmpname, 0, (size_t)NAME_BUF_SIZE);
    if (H5Lget_name_by_idx(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_DEC, (hsize_t)0, tmpname, (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0) {
        TEST_ERROR;
    }
    if (strcmp(linkname, tmpname) != 0) {
        TEST_ERROR;
    ...
    if (!hard_link) {
        memset(tmpval, 0, (size_t)NAME_BUF_SIZE);
        if (H5Lget_val_by_idx(group_id, ".", H5_INDEX_NAME, H5_ITER_INC, n, tmpval, (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0) {
            TEST_ERROR;
        }
        if (strcmp(valname, tmpval) != 0) {
            TEST_ERROR;
    ...
    memset(tmpname, 0, (size_t)NAME_BUF_SIZE);
    if (H5Lget_name_by_idx(group_id, ".", H5_INDEX_NAME, H5_ITER_INC, n, tmpname, (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0) {
        TEST_ERROR;
    }
    if (strcmp(linkname, tmpname) != 0) {
        TEST_ERROR;
    ...
    if (!hard_link) {
        memset(tmpval, 0, (size_t)NAME_BUF_SIZE);
        if (H5Lget_val_by_idx(group_id, ".", H5_INDEX_NAME, H5_ITER_DEC, (hsize_t)0, tmpval, (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0) {
            TEST_ERROR;
        }
        if (strcmp(valname, tmpval) != 0) {
            TEST_ERROR;
    ...
    memset(tmpname, 0, (size_t)NAME_BUF_SIZE);
    if (H5Lget_name_by_idx(group_id, ".", H5_INDEX_NAME, H5_ITER_DEC, (hsize_t)0, tmpname, (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0) {
        TEST_ERROR;
    }
    if (strcmp(linkname, tmpname) != 0) {
        TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int link_info_by_idx_deprec(hid_t fapl)
{
    hid_t file_id = H5I_INVALID_HID;  /* File ID */
    ...
    for (hard_link = false; hard_link <= true; hard_link++) {
        ...
        for (use_index = false; use_index <= true; use_index++) {
            if (hard_link) {
                if (use_index) {
                    TESTING(
                        "querying info by index w/creation order index, using hard links and deprecated "
                        "routines");
                }
                else {
                    TESTING(
                        "querying info by index w/o creation order index, using hard links and "
                        "deprecated routines");
                }
            } /* end if */
            else {
                if (use_index) {
                    TESTING(
                        "querying info by index w/creation order index, using soft links and deprecated "
                        "routines");
                }
                else {
                    TESTING(
                        "querying info by index w/o creation order index, using soft links and "
                        "deprecated routines");
                }
            } /* end else */
 ...
 *-------------------------------------------------------------------------
 */
static int link_info_by_idx_old_deprec(hid_t fapl)
{
    hid_t file_id = H5I_INVALID_HID;                               /* File ID */
    hid_t group_id = H5I_INVALID_HID, group_id2 = H5I_INVALID_HID; /* Group IDs */
    unsigned hard_link;                                            /* Create hard or soft link? */
    ...
    for (hard_link = false; hard_link <= true; hard_link++) {
        ...
        for (u = 0; u < CORDER_NLINKS; u++) {
            ...
            if (hard_link) {
                if (H5Lget_info_by_idx1(group_id, ".", H5_INDEX_NAME, H5_ITER_INC, (hsize_t)u, &linfo, H5P_DEFAULT) < 0) {
                    TEST_ERROR;
                }
                ...
                if (H5Lget_val_by_idx(group_id, ".", H5_INDEX_NAME, H5_ITER_INC, (hsize_t)u, tmpval, (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0) {
                    TEST_ERROR;
                }
                if (strcmp(valname, tmpval) != 0) {
                    TEST_ERROR;
            ...
            if (H5Lget_name_by_idx(group_id, ".", H5_INDEX_NAME, H5_ITER_INC, (hsize_t)u, tmpname, (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0) {
                TEST_ERROR;
            }
            ...
            if (hard_link) {
                if (H5Lget_info_by_idx1(group_id, ".", H5_INDEX_NAME, H5_ITER_NATIVE, (hsize_t)u, &linfo, H5P_DEFAULT) < 0) {
                    TEST_ERROR;
                }
                if (H5_addr_ne(linfo.u.address, objno[u])) {
                    TEST_ERROR;
                ...
                if (H5Lget_val_by_idx(group_id, ".", H5_INDEX_NAME, H5_ITER_NATIVE, (hsize_t)u, tmpval, (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0) {
                    TEST_ERROR;
                }
                if (strcmp(valname, tmpval) != 0) {
                    TEST_ERROR;
            ...
            if (H5Lget_name_by_idx(group_id, ".", H5_INDEX_NAME, H5_ITER_NATIVE, (hsize_t)u, tmpname, (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0) {
                TEST_ERROR;
            }
            if (strcmp(objname, tmpname) != 0) {
                TEST_ERROR;
            ...
            if (hard_link) {
                if (H5Lget_info_by_idx1(group_id, ".", H5_INDEX_NAME, H5_ITER_DEC, (hsize_t)u, &linfo, H5P_DEFAULT) < 0) {
                    TEST_ERROR;
                }
                ...
                if (H5Lget_val_by_idx(group_id, ".", H5_INDEX_NAME, H5_ITER_DEC, (hsize_t)u, tmpval, (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0) {
                    TEST_ERROR;
                }
                if (strcmp(valname, tmpval) != 0) {
                    TEST_ERROR;
            ...
            if (H5Lget_name_by_idx(group_id, ".", H5_INDEX_NAME, H5_ITER_DEC, (hsize_t)u, tmpname, (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0) {
                TEST_ERROR;
            }
            if (strcmp(objname, tmpname) != 0) {
                TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int delete_by_idx_deprec(hid_t fapl)
{
    hid_t file_id = H5I_INVALID_HID;  /* File ID */
    hid_t group_id = H5I_INVALID_HID; /* Group ID */
    hid_t gcpl_id = H5I_INVALID_HID;  /* Group creation property list ID */
            ...
            for (use_index = false; use_index <= true; use_index++) {
                ...
                if (idx_type == H5_INDEX_CRT_ORDER) {
                    if (order == H5_ITER_INC) {
                        if (use_index) {
                            TESTING(
                                "deleting links by creation order index in increasing order w/creation "
                                "order index using deprecated routines");
                        }
                        else {
                            TESTING(
                                "deleting links by creation order index in increasing order w/o creation "
                                "order index using deprecated routines");
                        }
                    } /* end if */
                    else {
                        if (use_index) {
                            TESTING(
                                "deleting links by creation order index in decreasing order w/creation "
                                "order index using deprecated routines");
                        }
                        else {
                            TESTING(
                                "deleting links by creation order index in decreasing order w/o creation "
                                "order index using deprecated routines");
                        }
                    } /* end else */
                } /* end if */
                    ...
                    if (order == H5_ITER_INC) {
                        if (use_index) {
                            ...
                            TESTING(
                                "deleting links by name index in increasing order w/o creation order "
                        ...
                        if (use_index) {
                            ...
                            TESTING(
                                "deleting links by name index in decreasing order w/o creation order "
                                "index using deprecated routines");
                        }
                    } /* end else */
                } /* end else */
                ...
                for (u = 0; u < (max_compact - 1); u++) {
                    ...
                    memset(&linfo, 0, sizeof(linfo));
                    if (H5Lget_info_by_idx1(group_id, ".", idx_type, order, (hsize_t)0, &linfo, H5P_DEFAULT) < 0) {
                        TEST_ERROR;
                    }
                    if (order == H5_ITER_INC) {
                        if (linfo.corder != (u + 1)) {
                    ...
                    memset(tmpname, 0, (size_t)NAME_BUF_SIZE);
                    if (H5Lget_name_by_idx(group_id, ".", idx_type, order, (hsize_t)0, tmpname, (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0) {
                        TEST_ERROR;
                    }
                    if (order == H5_ITER_INC) {
                ...
                for (u = 0; u < max_compact; u++) {
                    ...
                    memset(&linfo, 0, sizeof(linfo));
                    if (H5Lget_info_by_idx1(group_id, ".", idx_type, order, (hsize_t)u, &linfo, H5P_DEFAULT) < 0) {
                        TEST_ERROR;
                    }
                    if (order == H5_ITER_INC) {
                    ...
                    memset(tmpname, 0, (size_t)NAME_BUF_SIZE);
                    if (H5Lget_name_by_idx(group_id, ".", idx_type, order, (hsize_t)u, tmpname, (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0) {
                        TEST_ERROR;
                    }
                    if (order == H5_ITER_INC) {
                        ...
                        snprintf(objname, sizeof(objname), "filler %02u", ((max_compact * 2) - ((u * 2) + 2)));
                    }
                    if (strcmp(objname, tmpname) != 0) {
                        TEST_ERROR;
                    }
                ...
                for (u = 0; u < (max_compact - 1); u++) {
                    ...
                    if (order == H5_ITER_INC) {
                        ...
                        snprintf(objname, sizeof(objname), "filler %02u", ((max_compact * 2) - ((u * 2) + 4)));
                    }
                    if (strcmp(objname, tmpname) != 0) {
                        TEST_ERROR;
                    }
 ...
 *-------------------------------------------------------------------------
 */
static int delete_by_idx_old_deprec(hid_t fapl)
{
    hid_t file_id = H5I_INVALID_HID;                               /* File ID */
    hid_t group_id = H5I_INVALID_HID, group_id2 = H5I_INVALID_HID; /* Group IDs */
    H5L_info1_t linfo;                                             /* Link info struct */
    ...
    for (order = H5_ITER_INC; order <= H5_ITER_DEC; order++) {
        ...
        for (u = 0; u < (CORDER_NLINKS - 1); u++) {
            ...
            /* Verify the name for first link in appropriate order */
            memset(tmpname, 0, (size_t)NAME_BUF_SIZE);
            if (H5Lget_name_by_idx(group_id, ".", H5_INDEX_NAME, order, (hsize_t)0, tmpname, (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0) {
                TEST_ERROR;
            }
            if (order == H5_ITER_INC) {
                snprintf(objname, sizeof(objname), "filler %02u", (u + 1));
        ...
        for (u = 0; u < (CORDER_NLINKS / 2); u++) {
            ...
            /* Verify the name for current link in appropriate order */
            memset(tmpname, 0, (size_t)NAME_BUF_SIZE);
            if (H5Lget_name_by_idx(group_id, ".", H5_INDEX_NAME, order, (hsize_t)u, tmpname, (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0) {
                TEST_ERROR;
            }
            if (order == H5_ITER_INC) {
                snprintf(objname, sizeof(objname), "filler %02u", ((u * 2) + 1));
 ...
 *-------------------------------------------------------------------------
 */
static int link_iterate_deprec_cb(hid_t group_id, const char* link_name, const H5L_info1_t* info, void* _op_data)
{
 ...
 *-------------------------------------------------------------------------
 */
static int group_iterate_deprec_cb(hid_t group_id, const char* link_name, void* _op_data)
{
 ...
 *-------------------------------------------------------------------------
 */
static int link_iterate_fail_deprec_cb(hid_t H5_ATTR_UNUSED group_id,
                                       const char H5_ATTR_UNUSED* link_name,
                                       const H5L_info1_t H5_ATTR_UNUSED* info,
 ...
 *-------------------------------------------------------------------------
 */
static int link_iterate_check_deprec(hid_t group_id, H5_index_t idx_type, H5_iter_order_t order, unsigned max_links, link_iter_info_t* iter_info)
{
 ...
 *-------------------------------------------------------------------------
 */
static int link_iterate_deprec(hid_t fapl)
{
    hid_t file_id = (H5I_INVALID_HID);  /* File ID */
    hid_t group_id = (H5I_INVALID_HID); /* Group ID */
    hid_t gcpl_id = (H5I_INVALID_HID);  /* Group creation property list ID */
            ...
            for (use_index = false; use_index <= true; use_index++) {
                ...
                if (idx_type == H5_INDEX_CRT_ORDER) {
                    if (order == H5_ITER_INC) {
                        if (use_index) {
                            TESTING(
                                "iterating over links by creation order index in increasing order "
                                "w/creation order index using deprecated routines");
                        }
                        else {
                            TESTING(
                                "iterating over links by creation order index in increasing order w/o "
                        ...
                        if (use_index) {
                            TESTING(
                                "iterating over links by creation order index in decreasing order "
                                "w/creation order index using deprecated routines");
                        }
                        else {
                            TESTING(
                                "iterating over links by creation order index in decreasing order w/o "
                                "creation order index using deprecated routines");
                        }
                    } /* end else */
                    else {
                        ...
                        if (use_index) {
                            TESTING(
                                "iterating over links by creation order index in native order w/creation "
                                "order index using deprecated routines");
                        }
                        else {
                            TESTING(
                                "iterating over links by creation order index in native order w/o "
                                "creation order index using deprecated routines");
                        }
                    } /* end else */
                } /* end if */
                    ...
                    if (order == H5_ITER_INC) {
                        if (use_index) {
                            TESTING(
                                "iterating over links by name index in increasing order w/creation order "
                                "index using deprecated routines");
                        }
                        else {
                            TESTING(
                                "iterating over links by name index in increasing order w/o creation "
                                "order index using deprecated routines");
                        }
                    } /* end if */
                    else if (order == H5_ITER_DEC) {
                        if (use_index) {
                            TESTING(
                                "iterating over links by name index in decreasing order w/creation order "
                                "index using deprecated routines");
                        }
                        else {
                            TESTING(
                                "iterating over links by name index in decreasing order w/o creation "
                                "order index using deprecated routines");
                        }
                    } /* end else */
                    else {
                        ...
                        if (use_index) {
                            TESTING(
                                "iterating over links by name index in native order w/creation order "
                                "index using deprecated routines");
                        }
                        else {
                            TESTING(
                                "iterating over links by name index in native order w/o creation order "
                                "index using deprecated routines");
                        }
                    } /* end else */
                } /* end else */
 ...
 *-------------------------------------------------------------------------
 */
static int link_iterate_old_deprec_cb(hid_t group_id, const char* link_name, const H5L_info1_t* info, void* _op_data)
{
    link_iter_info_t* op_data = (link_iter_info_t*)_op_data; /* User data */
    char objname[NAME_BUF_SIZE];                             /* Object name */
    H5L_info1_t my_info;                                     /* Local link info */
    ...
    /* Verify name of link */
    snprintf(objname, sizeof(objname), "filler %02u", (info ? (unsigned)op_data->curr : (unsigned)((op_data->ncalled - 1) + op_data->nskipped)));
    if (strcmp(link_name, objname) != 0) {
        return H5_ITER_ERROR;
    }
 ...
 *-------------------------------------------------------------------------
 */
static int group_iterate_old_deprec_cb(hid_t group_id, const char* link_name, void* _op_data)
{
 ...
 *-------------------------------------------------------------------------
 */
static int link_iterate_old_check_deprec(hid_t group_id, H5_iter_order_t order, unsigned max_links, link_iter_info_t* iter_info)
{
    unsigned v;   /* Local index variable */
    hsize_t skip; /* # of links to skip in group */
    int gskip;    /* # of links to skip in group, with H5Giterate */
 ...
 *-------------------------------------------------------------------------
 */
static int link_iterate_old_deprec(hid_t fapl)
{
    hid_t file_id = H5I_INVALID_HID;  /* File ID */
    hid_t group_id = H5I_INVALID_HID; /* Group ID */
    H5_iter_order_t order;            /* Order within in the index */
    ...
    for (order = H5_ITER_INC; order <= H5_ITER_NATIVE; order++) {
        ...
        if (order == H5_ITER_INC) {
            TESTING(
                "iterating over links by name index in increasing order in old-style group using "
                "deprecated routines");
        } /* end if */
        else if (order == H5_ITER_DEC) {
            TESTING(
                "iterating over links by name index in decreasing order in old-style group using "
                "deprecated routines");
        } /* end else */
        else {
            assert(order == H5_ITER_NATIVE);
            TESTING(
                "iterating over links by name index in native order in old-style group using deprecated "
                "routines");
        } /* end else */
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_root(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID;                         /* File ID */
    hid_t gid = H5I_INVALID_HID, gid2 = H5I_INVALID_HID; /* Group IDs */
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_path(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID;                         /* File ID */
    hid_t gid = H5I_INVALID_HID, gid2 = H5I_INVALID_HID; /* Group IDs */
    char objname[NAME_BUF_SIZE];                         /* Object name */
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_mult(hid_t fapl, bool new_format)
{
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_self(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID;                         /* File ID */
    hid_t gid = H5I_INVALID_HID, gid2 = H5I_INVALID_HID; /* Group IDs */
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_pingpong(hid_t fapl, bool new_format)
{
    hid_t fid = (H5I_INVALID_HID);                           /* File ID */
    hid_t gid = (H5I_INVALID_HID), gid2 = (H5I_INVALID_HID); /* Group IDs */
    char objname[NAME_BUF_SIZE];                             /* Object name */
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_toomany(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID;                             /* File ID */
    hid_t gid = H5I_INVALID_HID, gid2 = H5I_INVALID_HID;     /* Group IDs */
    char objname[NAME_BUF_SIZE];                             /* Object name */
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_dangling(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID;                             /* File ID */
    hid_t gid = H5I_INVALID_HID;                             /* Group IDs */
    ...
    if (status >= 0) {
        H5_FAILED();
        puts("    Retrieving name of object by index through dangling file external link should have failed.");
    } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_prefix(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID; /* File ID */
    hid_t gid = H5I_INVALID_HID; /* Group IDs */
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_abs_mainpath(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID; /* File ID */
    hid_t gid = H5I_INVALID_HID; /* Group IDs */
    char filename1[NAME_BUF_SIZE], filename2[NAME_BUF_SIZE], filename3[NAME_BUF_SIZE], tmpname[NAME_BUF_SIZE], cwdpath[NAME_BUF_SIZE];
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_rel_mainpath(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID; /* File ID */
    hid_t gid = H5I_INVALID_HID; /* Group IDs */
    char filename1[NAME_BUF_SIZE], filename2[NAME_BUF_SIZE], filename3[NAME_BUF_SIZE];
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_cwd(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID; /* File ID */
    hid_t gid = H5I_INVALID_HID; /* Group IDs */
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_abstar(hid_t fapl, bool new_format)
{
    hid_t fid = (H5I_INVALID_HID); /* File ID */
    hid_t gid = (H5I_INVALID_HID); /* Group IDs */
    char filename1[NAME_BUF_SIZE], filename2[NAME_BUF_SIZE], filename3[NAME_BUF_SIZE], tmpname[NAME_BUF_SIZE], cwdpath[NAME_BUF_SIZE];
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_abstar_cur(hid_t fapl, bool new_format)
{
    hid_t fid = (H5I_INVALID_HID); /* File ID */
    hid_t gid = (H5I_INVALID_HID); /* Group IDs */
    char filename1[NAME_BUF_SIZE], filename2[NAME_BUF_SIZE], filename3[NAME_BUF_SIZE], tmpname[NAME_BUF_SIZE], cwdpath[NAME_BUF_SIZE];
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_reltar(hid_t fapl, bool new_format)
{
    hid_t fid = (H5I_INVALID_HID); /* File ID */
    hid_t gid = (H5I_INVALID_HID); /* Group IDs */
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_chdir(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID; /* File ID */
    hid_t gid = H5I_INVALID_HID; /* Group IDs */
    char filename1[NAME_BUF_SIZE], filename2[NAME_BUF_SIZE], filename3[NAME_BUF_SIZE];
 ...
 *-------------------------------------------------------------------------
 */
static int external_set_elink_fapl1(hid_t fapl, bool new_format)
{
    ...
    char filename1[NAME_BUF_SIZE], filename2A[NAME_BUF_SIZE], filename2B[NAME_BUF_SIZE], tmpname[NAME_BUF_SIZE], cwdpath[NAME_BUF_SIZE];
 ...
 *-------------------------------------------------------------------------
 */
static int external_set_elink_fapl2(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID;
 ...
 *-------------------------------------------------------------------------
 */
static int external_set_elink_fapl3(bool new_format)
{
 ...
 *-------------------------------------------------------------------------
 */
static int external_set_elink_acc_flags(const char* driver_name, hid_t fapl, bool new_format)
{
    hid_t file1 = H5I_INVALID_HID, file2 = H5I_INVALID_HID, group = H5I_INVALID_HID, subgroup = H5I_INVALID_HID, gapl = H5I_INVALID_HID;
    char filename1[NAME_BUF_SIZE], filename2[NAME_BUF_SIZE];
    herr_t ret;
    unsigned flags;
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_inherit_locking(hid_t fapl_id, bool new_format)
{
...
/* Callback function */
static herr_t external_set_elink_cb_cb(const char* parent_file,
...
/* Main test function */
static int external_set_elink_cb(hid_t fapl, bool new_format)
{
    hid_t file1 = H5I_INVALID_HID, file2 = H5I_INVALID_HID, group = H5I_INVALID_HID, gapl = H5I_INVALID_HID, fam_fapl = H5I_INVALID_HID, ret_fapl = H5I_INVALID_HID,
          base_driver;
    ...
    if (base_driver == H5FD_CORE || base_driver == H5FD_FAMILY || base_driver == H5FD_MULTI || base_driver == H5FD_DIRECT || driver_is_parallel) {
 ...
 *-------------------------------------------------------------------------
 */
static int external_reset_register(void)
{
    hid_t file;
    char filename[NAME_BUF_SIZE];
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_win1(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID; /* File ID */
    hid_t gid = H5I_INVALID_HID; /* Group IDs */
    char filename1[NAME_BUF_SIZE], filename2[NAME_BUF_SIZE], filename3[NAME_BUF_SIZE], tmpname[NAME_BUF_SIZE], cwdpath[NAME_BUF_SIZE];
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_win2(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID; /* File ID */
    hid_t gid = H5I_INVALID_HID; /* Group IDs */
    char filename1[NAME_BUF_SIZE], filename2[NAME_BUF_SIZE], filename3[NAME_BUF_SIZE], tmpname[NAME_BUF_SIZE], cwdpath[NAME_BUF_SIZE];
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_win3(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID; /* File ID */
    hid_t gid = H5I_INVALID_HID; /* Group IDs */
    int drive = 0;
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_win4(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID; /* File ID */
    hid_t gid = H5I_INVALID_HID; /* Group IDs */
    int drive = 0;
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_win5(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID; /* File ID */
    hid_t gid = H5I_INVALID_HID; /* Group IDs */
    int drive = 0;
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_win6(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID; /* File ID */
    hid_t gid = H5I_INVALID_HID; /* Group IDs */
    int drive = 0;
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_win7(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID; /* File ID */
    hid_t gid = H5I_INVALID_HID; /* Group IDs */
    char filename1[NAME_BUF_SIZE], filename2[NAME_BUF_SIZE], filename3[NAME_BUF_SIZE], tmpname[NAME_BUF_SIZE], cwdpath[NAME_BUF_SIZE];
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_win8(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID; /* File ID */
    hid_t gid = H5I_INVALID_HID; /* Group IDs */
    int drive = 0;
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_win9(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID; /* File ID */
    hid_t gid = H5I_INVALID_HID; /* Group IDs */
    char filename1[NAME_BUF_SIZE], filename2[NAME_BUF_SIZE], filename3[NAME_BUF_SIZE], tmpname[NAME_BUF_SIZE], cwdpath[NAME_BUF_SIZE];
    ...
    if (new_format) {
        TESTING(
            "external links via main file's Long UNC local host/main drive/rel path(windows)(w/new group "
            "format)");
    }
    else {
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_recursive(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID;   /* File ID */
    hid_t gid = H5I_INVALID_HID;   /* Group IDs */
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_query(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID;                             /* File ID */
    hid_t gid = H5I_INVALID_HID;                             /* Group IDs */
    const char* file_name;                                   /* Name of the file the external link points to */
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_unlink_compact(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID;                             /* File ID */
    hid_t gid = H5I_INVALID_HID, gid2 = H5I_INVALID_HID;     /* Group IDs */
    char filename1[NAME_BUF_SIZE], filename2[NAME_BUF_SIZE]; /* Names of files to externally link across */
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_unlink_dense(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID;                             /* File ID */
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_move(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID;                             /* File ID */
    hid_t gid = H5I_INVALID_HID, gid2 = H5I_INVALID_HID;     /* Group IDs */
    char objname[NAME_BUF_SIZE];                             /* Object name */
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_ride(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID;                             /* File ID */
    hid_t gcpl = H5I_INVALID_HID;                            /* Group creation property list ID */
    hid_t gid = H5I_INVALID_HID, gid2 = H5I_INVALID_HID;     /* Group IDs */
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_closing(hid_t fapl, bool new_format)
{
    hid_t fid1 = (H5I_INVALID_HID), fid2 = (H5I_INVALID_HID), fid3 = (H5I_INVALID_HID), fid4 = (H5I_INVALID_HID);
    hid_t gid = (H5I_INVALID_HID), tid = (H5I_INVALID_HID), tid2 = (H5I_INVALID_HID), sid = (H5I_INVALID_HID), did = (H5I_INVALID_HID);
    hid_t lcpl_id = (H5I_INVALID_HID);
    ...
    if (H5Lcopy(fid1, "elink/elink/elink", fid1, "elink/elink/elink/elink_copied2", H5P_DEFAULT, H5P_DEFAULT) < 0) {
        FAIL_STACK_ERROR;
    }
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_endian(bool new_format)
{
    hid_t fid = H5I_INVALID_HID;                         /* File ID */
    hid_t gid = H5I_INVALID_HID, gid2 = H5I_INVALID_HID; /* Group IDs */
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_strong(hid_t fapl, bool new_format)
{
 ...
 *-------------------------------------------------------------------------
 */
static int external_symlink(const char* driver_name, hid_t fapl, bool new_format)
{
 ...
 *-------------------------------------------------------------------------
 */
static int external_copy_invalid_object(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID;      /* File ID */
    hid_t gid = H5I_INVALID_HID;      /* Group ID */
 ...
 *-------------------------------------------------------------------------
 */
static int external_dont_fail_to_source(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID; /* File ID */
    hid_t gid = H5I_INVALID_HID; /* Group ID */
 ...
 *-------------------------------------------------------------------------
 */
static int external_file_cache(hid_t fapl, bool new_format)
{
 ...
 *-------------------------------------------------------------------------
 */
static int external_open_twice(hid_t fapl, bool new_format)
{
    hid_t fid1 = H5I_INVALID_HID;  /* File ID */
    hid_t fid2 = H5I_INVALID_HID;  /* File ID */
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_with_committed_datatype(hid_t fapl, bool new_format)
{
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_public_macros(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID; /* File ID */
    H5L_info2_t linfo;           /* Link information */
    char objname[NAME_BUF_SIZE]; /* Object name */
...
/* Callback functions for UD hard links. */
/* UD_hard_create increments the object's reference count */
static herr_t UD_hard_create(const char H5_ATTR_UNUSED* link_name, hid_t loc_group, const void* udata, size_t udata_size, hid_t H5_ATTR_UNUSED lcpl_id)
{
    H5O_token_t token;
    hid_t target_obj = H5I_INVALID_HID;
    herr_t ret_value = 0;
...
/* Traverse a hard link by opening the object */
static hid_t UD_hard_traverse(const char H5_ATTR_UNUSED* link_name,
                              hid_t cur_group,
                              const void* udata,
                              size_t udata_size,
                              hid_t H5_ATTR_UNUSED lapl_id,
...
/* UD_hard_delete decrements the object's reference count */
static herr_t UD_hard_delete(const char H5_ATTR_UNUSED* link_name, hid_t file, const void* udata, size_t udata_size)
{
    H5O_token_t token;
    hid_t target_obj = H5I_INVALID_HID;
    herr_t ret_value = 0;
...
static int ud_hard_links(hid_t fapl)
{
    hid_t fid = H5I_INVALID_HID;                         /* File ID */
    hid_t gid = H5I_INVALID_HID, gid2 = H5I_INVALID_HID; /* Group IDs */
    H5L_info2_t li;                                      /* Link information */
    ...
    /* Create a user-defined "hard link" to the group using the address we got
     * from H5Lget_info2 */
    if (H5Lcreate_ud(fid, "ud_link", (H5L_type_t)UD_HARD_TYPE, &(li.u.token), sizeof(H5O_token_t), H5P_DEFAULT, H5P_DEFAULT) < 0) {
        TEST_ERROR;
    }
 ...
 */
static hid_t UD_rereg_traverse(const char H5_ATTR_UNUSED* link_name,
                               hid_t cur_group,
                               const void H5_ATTR_UNUSED* udata,
                               size_t H5_ATTR_UNUSED udata_size,
                               hid_t lapl_id,
                               hid_t H5_ATTR_UNUSED dxpl_id)
{
    hid_t ret_value;
...
static int ud_link_reregister(hid_t fapl)
{
    hid_t fid = H5I_INVALID_HID;                         /* File ID */
    hid_t gid = H5I_INVALID_HID, gid2 = H5I_INVALID_HID; /* Group IDs */
    H5L_info2_t li;                                      /* Link information */
...
static hid_t UD_cb_traverse(const char* link_name, hid_t cur_group, const void* udata, size_t udata_size, hid_t lapl_id, hid_t H5_ATTR_UNUSED dxpl_id)
{
...
static int ud_callbacks(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID;          /* File ID */
    hid_t gid = H5I_INVALID_HID;          /* Group ID */
    hid_t lcpl = H5I_INVALID_HID;         /* Link Creation PL */
 ...
 *-------------------------------------------------------------------------
 */
static hid_t UD_plist_traverse(const char H5_ATTR_UNUSED* link_name,
                               hid_t cur_group,
                               const void H5_ATTR_UNUSED* udata,
                               size_t udata_size,
                               hid_t lapl_id,
...
static int lapl_udata(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID;                         /* File ID */
    hid_t gid = H5I_INVALID_HID, gid2 = H5I_INVALID_HID; /* Group IDs */
    ...
    if (H5Pinsert2(plist_id, DEST_PROP_NAME, (size_t)NAME_BUF_SIZE, group_a_name, NULL, NULL, NULL, NULL, NULL, NULL) < 0) {
        TEST_ERROR;
    }
 ...
 */
static herr_t
    UD_cbsucc_create(const char H5_ATTR_UNUSED* link_name, hid_t H5_ATTR_UNUSED loc_group, const void* udata, size_t udata_size, hid_t H5_ATTR_UNUSED lcpl_id)
{
...
static hid_t UD_cbsucc_traverse(const char H5_ATTR_UNUSED* link_name,
                                hid_t cur_group,
                                const void* udata,
                                size_t H5_ATTR_UNUSED udata_size,
                                hid_t lapl_id,
...
static herr_t UD_cbfail_move(const char H5_ATTR_UNUSED* new_name, hid_t H5_ATTR_UNUSED new_loc, const void H5_ATTR_UNUSED* udata, size_t H5_ATTR_UNUSED udata_size)
{
...
static herr_t UD_cbsucc_move(const char H5_ATTR_UNUSED* new_name, hid_t H5_ATTR_UNUSED new_loc, const void H5_ATTR_UNUSED* udata, size_t H5_ATTR_UNUSED udata_size)
{
...
/* Callback for when the link is deleted.  Also called during move */
static herr_t UD_cbsucc_delete(const char H5_ATTR_UNUSED* link_name, hid_t H5_ATTR_UNUSED file, const void H5_ATTR_UNUSED* udata, size_t H5_ATTR_UNUSED udata_size)
{
...
/* Callback for when the link is deleted.  Also called during move */
static herr_t UD_cbfail_delete(const char H5_ATTR_UNUSED* link_name, hid_t H5_ATTR_UNUSED file, const void H5_ATTR_UNUSED* udata, size_t H5_ATTR_UNUSED udata_size)
{
    /* This traversal function will always fail. */
...
/* Callback for when the link is queried */
static ssize_t UD_cbfail_query(const char H5_ATTR_UNUSED* link_name,
                               const void H5_ATTR_UNUSED* udata,
                               size_t H5_ATTR_UNUSED udata_size,
...
/* Callback for when the link is queried */
static ssize_t UD_cbfail_on_write_query(const char H5_ATTR_UNUSED* link_name,
                                        const void H5_ATTR_UNUSED* udata,
                                        size_t H5_ATTR_UNUSED udata_size,
                                        void* buf,
                                        size_t H5_ATTR_UNUSED buf_size)
...
/* Callback for when the link is queried */
static ssize_t UD_cbsucc_query(const char H5_ATTR_UNUSED* link_name, const void H5_ATTR_UNUSED* udata, size_t H5_ATTR_UNUSED udata_size, void* buf, size_t buf_size)
{
    /* This traversal function will return a buffer size,
     * but will fail when a buffer is passed in ("writing to the buffer"
     * fails
...
static int ud_link_errors(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID; /* File ID */
    hid_t gid = H5I_INVALID_HID; /* Group IDs */
    ...
    if (H5Lcreate_ud(fid, "/ud_link", (H5L_type_t)UD_CBFAIL_TYPE, &group_name, strlen(group_name) + 1, H5P_DEFAULT, H5P_DEFAULT) < 0) {
        FAIL_STACK_ERROR;
    }
 ...
 *-------------------------------------------------------------------------
 */
static int lapl_nlinks(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID;                                                  /* File ID */
    hid_t gid = H5I_INVALID_HID, gid2 = H5I_INVALID_HID;                          /* Group IDs */
    hid_t plist = H5I_INVALID_HID;                                                /* lapl ID */
 ...
 *-------------------------------------------------------------------------
 */
static int linkinfo(hid_t fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID;                        /* File ID */
    hid_t gid = H5I_INVALID_HID;                        /* Group ID */
    hid_t tid = H5I_INVALID_HID;                        /* Type ID */
 ...
 *-------------------------------------------------------------------------
 */
static int check_all_closed(hid_t fapl, bool new_format, int stopat)
{
    hid_t fid = H5I_INVALID_HID;
    char filename[NAME_BUF_SIZE];
    int x;
 ...
 *-------------------------------------------------------------------------
 */
static hid_t build_visit_file(hid_t fapl)
{
    hid_t fid = H5I_INVALID_HID;                                /* File ID */
    hid_t gid = H5I_INVALID_HID, gid2 = H5I_INVALID_HID;        /* Group IDs */
    hid_t sid = H5I_INVALID_HID;                                /* Dataspace ID */
    ...
    if ((did = H5Dcreate2(fid, "/Group1/Dataset_one", H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl_g, H5P_DEFAULT)) < 0) {
        TEST_ERROR;
    }
    if (H5Dclose(did) < 0) {
        TEST_ERROR;
    ...
    if ((did = H5Dcreate2(fid, "/Group1/Group2/Dataset_two", H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl_g, H5P_DEFAULT)) < 0) {
        TEST_ERROR;
    }
    if (H5Dclose(did) < 0) {
        TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int visit_link_cb(hid_t H5_ATTR_UNUSED group_id, const char* name, const H5L_info2_t* linfo, void* _op_data)
{
 ...
 *-------------------------------------------------------------------------
 */
static int link_visit(hid_t fapl, bool new_format)
{
 ...
 *-------------------------------------------------------------------------
 */
static int link_visit_by_name(hid_t fapl, bool new_format)
{
    lvisit_ud_t udata;           /* User-data for visiting */
    hid_t fid = H5I_INVALID_HID;
    hid_t gid = H5I_INVALID_HID; /* Group ID */
    ...
    udata.idx = 0;
    udata.info = lvisit2;
    if (H5Lvisit_by_name2(fid, "/Group1/Group2", H5_INDEX_NAME, H5_ITER_INC, visit_link_cb, &udata, H5P_DEFAULT) < 0) {
        FAIL_STACK_ERROR;
    }
 ...
 *-------------------------------------------------------------------------
 */
static int visit_obj_cb(hid_t H5_ATTR_UNUSED group_id, const char* name, const H5O_info2_t* oinfo, void* _op_data)
{
 ...
 *-------------------------------------------------------------------------
 */
static int obj_visit(hid_t fapl, bool new_format)
{
 ...
 *-------------------------------------------------------------------------
 */
static int obj_visit_by_name(hid_t fapl, bool new_format)
{
    ovisit_ud_t udata;           /* User-data for visiting */
    hid_t fid = H5I_INVALID_HID;
    hid_t gid = H5I_INVALID_HID; /* Group ID */
    ...
    udata.idx = 0;
    udata.info = new_format ? ovisit0_new : ovisit0_old;
    if (H5Ovisit_by_name3(fid, "/", H5_INDEX_NAME, H5_ITER_INC, visit_obj_cb, &udata, H5O_INFO_BASIC, H5P_DEFAULT) < 0) {
        FAIL_STACK_ERROR;
    }
    ...
    udata.idx = 0;
    udata.info = new_format ? ovisit0_new : ovisit0_old;
    if (H5Ovisit_by_name3(gid, ".", H5_INDEX_NAME, H5_ITER_INC, visit_obj_cb, &udata, H5O_INFO_BASIC, H5P_DEFAULT) < 0) {
        FAIL_STACK_ERROR;
    }
    if (H5Gclose(gid) < 0) {
        FAIL_STACK_ERROR;
    ...
    udata.idx = 0;
    udata.info = new_format ? ovisit1_new : ovisit1_old;
    if (H5Ovisit_by_name3(fid, "/Group1", H5_INDEX_NAME, H5_ITER_INC, visit_obj_cb, &udata, H5O_INFO_BASIC, H5P_DEFAULT) < 0) {
        FAIL_STACK_ERROR;
    }
    ...
    udata.idx = 0;
    udata.info = new_format ? ovisit2_new : ovisit2_old;
    if (H5Ovisit_by_name3(fid, "/Group1/Group2", H5_INDEX_NAME, H5_ITER_INC, visit_obj_cb, &udata, H5O_INFO_BASIC, H5P_DEFAULT) < 0) {
        FAIL_STACK_ERROR;
    }
 ...
 *-------------------------------------------------------------------------
 */
static int visit_obj_stop_cb(hid_t H5_ATTR_UNUSED group_id, const char H5_ATTR_UNUSED* name, const H5O_info2_t H5_ATTR_UNUSED* oinfo, void* _op_data)
{
 ...
 *-------------------------------------------------------------------------
 */
static int obj_visit_stop(hid_t fapl, bool new_format)
{
    ...
    nvisited = 0;
    if ((ret = H5Ovisit_by_name3(fid, "/", H5_INDEX_NAME, H5_ITER_INC, visit_obj_stop_cb, &nvisited, H5O_INFO_BASIC, H5P_DEFAULT)) < 0) {
        FAIL_STACK_ERROR;
    }
    if (ret != H5_ITER_STOP) {
        TEST_ERROR;
...
static size_t link_filter_filter(unsigned int flags,
                                 size_t cd_nelmts,
...
static int link_filters(hid_t fapl, bool new_format)
{
    ...
    if (H5Pget_filter_by_id2(gcpl2, H5Z_FILTER_RESERVED + 42, &flags_out, &cd_nelmts, &cd_value_out, (size_t)24, name_out, &filter_config_out) < 0) {
        TEST_ERROR;
    }
    ...
    if (H5Pget_filter2(gcpl2, (unsigned)(nfilters - 1), &flags_out, &cd_nelmts, &cd_value_out, (size_t)24, name_out, &filter_config_out) < 0) {
        TEST_ERROR;
    }
    if (flags_out != 0 || cd_value_out != cd_value || strcmp(filter_class.name, name_out) != 0 ||
        filter_config_out != (H5Z_FILTER_CONFIG_ENCODE_ENABLED | H5Z_FILTER_CONFIG_DECODE_ENABLED)) {
    ...
    H5E_BEGIN_TRY
    {
        status = H5Pget_filter_by_id2(gcpl2, H5Z_FILTER_RESERVED + 42, &flags_out, &cd_nelmts, &cd_value_out, (size_t)24, name_out, &filter_config_out);
    }
    H5E_END_TRY
    if (status >= 0) {
        TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int obj_exists(hid_t fapl, bool new_format)
{
 ...
 *-------------------------------------------------------------------------
 */
static int delete_self_referential_link(hid_t fapl, bool new_format)
{
    char filename[NAME_BUF_SIZE]; /* Buffer for file name */
    hid_t fid = H5I_INVALID_HID;  /* File ID */
    hid_t gid = H5I_INVALID_HID;  /* Group ID */
 ...
 *-------------------------------------------------------------------------
 */
static int corder_create_empty(hid_t fapl)
{
    hid_t file_id = H5I_INVALID_HID;  /* File ID */
    hid_t group_id = H5I_INVALID_HID; /* Group ID */
    hid_t gcpl_id = H5I_INVALID_HID;  /* Group creation property list ID */
 ...
 *-------------------------------------------------------------------------
 */
static int corder_create_compact(hid_t fapl)
{
    hid_t file_id = H5I_INVALID_HID;                               /* File ID */
    hid_t group_id = H5I_INVALID_HID, group_id2 = H5I_INVALID_HID; /* Group IDs */
    hid_t gcpl_id = H5I_INVALID_HID;                               /* Group creation property list ID */
 ...
 *-------------------------------------------------------------------------
 */
static int corder_create_dense(hid_t fapl)
{
    hid_t file_id = H5I_INVALID_HID;                               /* File ID */
    hid_t group_id = H5I_INVALID_HID, group_id2 = H5I_INVALID_HID; /* Group IDs */
    hid_t gcpl_id = H5I_INVALID_HID;                               /* Group creation property list ID */
 ...
 *-------------------------------------------------------------------------
 */
static int corder_transition(hid_t fapl)
{
    hid_t file_id = H5I_INVALID_HID;                               /* File ID */
    hid_t group_id = H5I_INVALID_HID, group_id2 = H5I_INVALID_HID; /* Group IDs */
    hid_t gcpl_id = H5I_INVALID_HID;                               /* Group creation property list ID */
 ...
 *-------------------------------------------------------------------------
 */
static int corder_delete(hid_t fapl)
{
    hid_t file_id = H5I_INVALID_HID;                               /* File ID */
    hid_t group_id = H5I_INVALID_HID, group_id2 = H5I_INVALID_HID; /* Group IDs */
    hid_t gcpl_id = H5I_INVALID_HID;                               /* Group creation property list ID */
 ...
 *-------------------------------------------------------------------------
 */
static int link_info_by_idx_check(hid_t group_id, const char* linkname, hsize_t n, bool hard_link, bool use_index)
{
    char tmpname[NAME_BUF_SIZE]; /* Temporary link name */
    char valname[NAME_BUF_SIZE]; /* Link value name */
    char tmpval[NAME_BUF_SIZE];  /* Temporary link value */
    ...
    /* Verify the link information for first link, in increasing creation order */
    memset(&linfo, 0, sizeof(linfo));
    if (H5Lget_info_by_idx2(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC, (hsize_t)0, &linfo, H5P_DEFAULT) < 0) {
        TEST_ERROR;
    }
    if (linfo.corder != 0) {
        TEST_ERROR;
    ...
    if (use_index) {
        /* Verify the link information for first link, in native creation order (which is increasing) */
        memset(&linfo, 0, sizeof(linfo));
        if (H5Lget_info_by_idx2(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_NATIVE, (hsize_t)0, &linfo, H5P_DEFAULT) < 0) {
            TEST_ERROR;
        }
        if (linfo.corder != 0) {
            TEST_ERROR;
        ...
        /* Verify the link information for new link, in native creation order (which is increasing) */
        memset(&linfo, 0, sizeof(linfo));
        if (H5Lget_info_by_idx2(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_NATIVE, n, &linfo, H5P_DEFAULT) < 0) {
            TEST_ERROR;
        }
        if (linfo.corder != (int64_t)n) {
            TEST_ERROR;
    ...
    /* Verify the link information for new link, in decreasing creation order */
    memset(&linfo, 0, sizeof(linfo));
    if (H5Lget_info_by_idx2(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_DEC, (hsize_t)0, &linfo, H5P_DEFAULT) < 0) {
        TEST_ERROR;
    }
    if (linfo.corder != (int64_t)n) {
        TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int link_info_by_idx(hid_t fapl)
{
    hid_t file_id = H5I_INVALID_HID;  /* File ID */
    hid_t group_id = H5I_INVALID_HID; /* Group ID */
    hid_t gcpl_id = H5I_INVALID_HID;  /* Group creation property list ID */
 ...
 *-------------------------------------------------------------------------
 */
static int link_info_by_idx_old(hid_t fapl)
{
    hid_t file_id = H5I_INVALID_HID;                               /* File ID */
    hid_t group_id = H5I_INVALID_HID, group_id2 = H5I_INVALID_HID; /* Group IDs */
    unsigned hard_link;                                            /* Create hard or soft link? */
    ...
    for (hard_link = false; hard_link <= true; hard_link++) {
        ...
        for (u = 0; u < CORDER_NLINKS; u++) {
            ...
            /* Verify link information (in increasing order) */
            if (hard_link) {
                if (H5Lget_info_by_idx2(group_id, ".", H5_INDEX_NAME, H5_ITER_INC, (hsize_t)u, &linfo, H5P_DEFAULT) < 0) {
                    TEST_ERROR;
                }
            ...
            /* Verify link information (in native order - native is increasing) */
            if (hard_link) {
                if (H5Lget_info_by_idx2(group_id, ".", H5_INDEX_NAME, H5_ITER_NATIVE, (hsize_t)u, &linfo, H5P_DEFAULT) < 0) {
                    TEST_ERROR;
                }
                if (H5Otoken_cmp(group_id, &linfo.u.token, &objtoken[u], &token_cmp) < 0) {
                    TEST_ERROR;
            ...
            /* Verify link information (in decreasing order) */
            if (hard_link) {
                if (H5Lget_info_by_idx2(group_id, ".", H5_INDEX_NAME, H5_ITER_DEC, (hsize_t)u, &linfo, H5P_DEFAULT) < 0) {
                    TEST_ERROR;
                }
 ...
 *-------------------------------------------------------------------------
 */
static int delete_by_idx(hid_t fapl)
{
    hid_t file_id = H5I_INVALID_HID;  /* File ID */
    hid_t group_id = H5I_INVALID_HID; /* Group ID */
    hid_t gcpl_id = H5I_INVALID_HID;  /* Group creation property list ID */
        ...
        for (order = H5_ITER_INC; order <= H5_ITER_DEC; order++) {
            ...
            for (use_index = false; use_index <= true; use_index++) {
                ...
                for (u = 0; u < (max_compact - 1); u++) {
                    ...
                    /* Verify the link information for first link in appropriate order */
                    memset(&linfo, 0, sizeof(linfo));
                    if (H5Lget_info_by_idx2(group_id, ".", idx_type, order, (hsize_t)0, &linfo, H5P_DEFAULT) < 0) {
                        TEST_ERROR;
                    }
                    if (order == H5_ITER_INC) {
                        if (linfo.corder != (u + 1)) {
                ...
                for (u = 0; u < max_compact; u++) {
                    ...
                    /* Verify the link information for current link in appropriate order */
                    memset(&linfo, 0, sizeof(linfo));
                    if (H5Lget_info_by_idx2(group_id, ".", idx_type, order, (hsize_t)u, &linfo, H5P_DEFAULT) < 0) {
                        TEST_ERROR;
                    }
                    if (order == H5_ITER_INC) {
                        if (linfo.corder != ((u * 2) + 1)) {
 ...
 *-------------------------------------------------------------------------
 */
static int delete_by_idx_old(hid_t fapl)
{
    hid_t file_id = H5I_INVALID_HID;                               /* File ID */
    hid_t group_id = H5I_INVALID_HID, group_id2 = H5I_INVALID_HID; /* Group IDs */
    H5L_info2_t linfo;                                             /* Link info struct */
 ...
 *-------------------------------------------------------------------------
 */
static int link_iterate_cb(hid_t group_id, const char* link_name, const H5L_info2_t* info, void* _op_data)
{
    link_iter_info_t* op_data = (link_iter_info_t*)_op_data; /* User data */
    char objname[NAME_BUF_SIZE];                             /* Object name */
 ...
 *-------------------------------------------------------------------------
 */
static int group_iterate_cb(hid_t group_id, const char* link_name, void* _op_data)
{
 ...
 */
static int
    link_iterate_fail_cb(hid_t H5_ATTR_UNUSED group_id, const char H5_ATTR_UNUSED* link_name, const H5L_info2_t H5_ATTR_UNUSED* info, void H5_ATTR_UNUSED* _op_data)
{
    return H5_ITER_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int link_iterate_check(hid_t group_id, H5_index_t idx_type, H5_iter_order_t order, unsigned max_links, link_iter_info_t* iter_info)
{
    unsigned v;   /* Local index variable */
    hsize_t skip; /* # of links to skip in group */
#ifndef H5_NO_DEPRECATED_SYMBOLS
 ...
 *-------------------------------------------------------------------------
 */
static int link_iterate(hid_t fapl)
{
    hid_t file_id = (H5I_INVALID_HID);  /* File ID */
    hid_t group_id = (H5I_INVALID_HID); /* Group ID */
    hid_t gcpl_id = (H5I_INVALID_HID);  /* Group creation property list ID */
 ...
 *-------------------------------------------------------------------------
 */
static int link_iterate_old_cb(hid_t group_id, const char* link_name, const H5L_info2_t* info, void* _op_data)
{
    link_iter_info_t* op_data = (link_iter_info_t*)_op_data; /* User data */
    char objname[NAME_BUF_SIZE];                             /* Object name */
    H5L_info2_t my_info;                                     /* Local link info */
 ...
 *-------------------------------------------------------------------------
 */
static int group_iterate_old_cb(hid_t group_id, const char* link_name, void* _op_data)
{
 ...
 *-------------------------------------------------------------------------
 */
static int link_iterate_old_check(hid_t group_id, H5_iter_order_t order, unsigned max_links, link_iter_info_t* iter_info)
{
    unsigned v;   /* Local index variable */
    hsize_t skip; /* # of links to skip in group */
#ifndef H5_NO_DEPRECATED_SYMBOLS
 ...
 *-------------------------------------------------------------------------
 */
static int link_iterate_old(hid_t fapl)
{
    hid_t file_id = H5I_INVALID_HID;  /* File ID */
    hid_t group_id = H5I_INVALID_HID; /* Group ID */
    H5_iter_order_t order;            /* Order within in the index */
 ...
 *-------------------------------------------------------------------------
 */
static int open_by_idx_check(hid_t main_group_id,
                             hid_t soft_group_id,
                             hid_t mount_file_id,
                             H5_index_t idx_type,
                             ...
                             H5O_token_t* objno)
{
    ...
    if ((obj_id = H5Oopen_by_idx(group_id, ".", H5_INDEX_NAME, H5_ITER_INC, (hsize_t)mnt_idx, H5P_DEFAULT)) < 0) {
        TEST_ERROR;
    }
 ...
 *-------------------------------------------------------------------------
 */
static int open_by_idx(hid_t fapl)
{
    hid_t file_id = H5I_INVALID_HID;       /* File ID */
            ...
            for (use_index = false; use_index <= true; use_index++) {
                ...
                if (idx_type == H5_INDEX_CRT_ORDER) {
                    if (order == H5_ITER_INC) {
                        if (use_index) {
                            TESTING(
                                "open object by creation order index in increasing order w/creation "
                                "order index");
                        }
                        else {
                            TESTING(
                                "open object by creation order index in increasing order w/o creation "
                                "order index");
                        }
                    } /* end if */
                    else if (order == H5_ITER_DEC) {
                        if (use_index) {
                            TESTING(
                                "open object by creation order index in decreasing order w/creation "
                                "order index");
                        }
                        else {
                            TESTING(
                                "open object by creation order index in decreasing order w/o creation "
                                "order index");
                        }
                    } /* end else */
                    else {
                        ...
                        if (use_index) {
                            ...
                            TESTING(
                                "open object by creation order index in native order w/o creation order "
                                "index");
                        }
                    } /* end else */
                } /* end if */
                ...
                if ((soft_group_id = H5Gcreate2(file_id, CORDER_SOFT_GROUP_NAME, H5P_DEFAULT, gcpl_id, H5P_DEFAULT)) < 0) {
                    TEST_ERROR;
                }
 ...
 *-------------------------------------------------------------------------
 */
static int open_by_idx_check_old(hid_t main_group_id,
                                 hid_t soft_group_id,
                                 hid_t mount_file_id,
                                 H5_index_t idx_type,
                                 H5_iter_order_t order,
 ...
 *-------------------------------------------------------------------------
 */
static int open_by_idx_old(hid_t fapl)
{
    hid_t file_id = H5I_INVALID_HID;       /* File ID */
    hid_t mount_file_id = H5I_INVALID_HID; /* File ID for file to mount */
    hid_t group_id = H5I_INVALID_HID;      /* Group ID */
    ...
    for (order = H5_ITER_INC; order <= H5_ITER_NATIVE; order++) {
        ...
        if ((soft_group_id = H5Gcreate2(file_id, CORDER_SOFT_GROUP_NAME, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
            TEST_ERROR;
        }
 ...
 *-------------------------------------------------------------------------
 */
static int object_info_check(hid_t main_group_id, hid_t soft_group_id, H5_index_t idx_type, H5_iter_order_t order, unsigned max_links, H5O_token_t* objno)
{
    char objname[NAME_BUF_SIZE];      /* Object name */
    hid_t group_id = H5I_INVALID_HID; /* ID of group to test */
    ...
    for (v = 0; v < 2; v++) {
        ...
        for (u = 0; u < max_links; u++) {
            ...
            if (H5Oget_info_by_name3(group_id, objname, &oinfo, H5O_INFO_BASIC | H5O_INFO_NUM_ATTRS, H5P_DEFAULT) < 0) {
                TEST_ERROR;
            }
            ...
            if (H5Oget_info_by_idx3(group_id, ".", idx_type, order, (hsize_t)u, &oinfo, H5O_INFO_BASIC | H5O_INFO_NUM_ATTRS, H5P_DEFAULT) < 0) {
                TEST_ERROR;
            }
 ...
 *-------------------------------------------------------------------------
 */
static int object_info(hid_t fapl)
{
    hid_t file_id = H5I_INVALID_HID;       /* File ID */
    hid_t group_id = H5I_INVALID_HID;      /* Group ID */
    hid_t soft_group_id = H5I_INVALID_HID; /* Group ID for soft links */
            ...
            for (use_index = false; use_index <= true; use_index++) {
                ...
                if (idx_type == H5_INDEX_CRT_ORDER) {
                    if (order == H5_ITER_INC) {
                        if (use_index) {
                            TESTING(
                                "query object info by creation order index in increasing order "
                                "w/creation order index");
                        }
                        else {
                            TESTING(
                                "query object info by creation order index in increasing order w/o "
                                "creation order index");
                        }
                    } /* end if */
                    else if (order == H5_ITER_DEC) {
                        if (use_index) {
                            TESTING(
                                "query object info by creation order index in decreasing order "
                                "w/creation order index");
                        }
                        else {
                            TESTING(
                                "query object info by creation order index in decreasing order w/o "
                                "creation order index");
                        }
                    } /* end else */
                    else {
                        ...
                        if (use_index) {
                            TESTING(
                                "query object info by creation order index in native order w/creation "
                                "order index");
                        }
                        else {
                            TESTING(
                                "query object info by creation order index in native order w/o creation "
                                "order index");
                        }
                    } /* end else */
                } /* end if */
                    ...
                    if (order == H5_ITER_INC) {
                        if (use_index) {
                            ...
                            TESTING(
                                "query object info by name index in increasing order w/o creation order "
                                "index");
                        }
                    } /* end if */
                    else if (order == H5_ITER_DEC) {
                        if (use_index) {
                            ...
                            TESTING(
                                "query object info by name index in decreasing order w/o creation order "
                                "index");
                        }
                    } /* end else */
                    else {
 ...
 *-------------------------------------------------------------------------
 */
static int object_info_check_old(hid_t main_group_id, hid_t soft_group_id, H5_index_t idx_type, H5_iter_order_t order, unsigned max_links, H5O_token_t* objno)
{
    char objname[NAME_BUF_SIZE];      /* Object name */
    hid_t group_id = H5I_INVALID_HID; /* ID of group to test */
    H5O_info2_t oinfo;                /* Buffer for querying object's info */
 ...
 *-------------------------------------------------------------------------
 */
static int object_info_old(hid_t fapl)
{
    hid_t file_id = H5I_INVALID_HID;       /* File ID */
    hid_t group_id = H5I_INVALID_HID;      /* Group ID */
    hid_t soft_group_id = H5I_INVALID_HID; /* Group ID for soft links */
 ...
 *-------------------------------------------------------------------------
 */
static int group_info(hid_t fapl)
{
    hid_t file_id = H5I_INVALID_HID;       /* File ID */
    hid_t group_id = H5I_INVALID_HID;      /* Group ID */
    hid_t soft_group_id = H5I_INVALID_HID; /* Group ID for soft links */
            ...
            for (use_index = false; use_index <= true; use_index++) {
                ...
                if (idx_type == H5_INDEX_CRT_ORDER) {
                    if (order == H5_ITER_INC) {
                        if (use_index) {
                            TESTING(
                                "query group info by creation order index in increasing order w/creation "
                                "order index");
                        }
                        else {
                            TESTING(
                                "query group info by creation order index in increasing order w/o "
                                "creation order index");
                        }
                    } /* end if */
                    else if (order == H5_ITER_DEC) {
                        if (use_index) {
                            TESTING(
                                "query group info by creation order index in decreasing order w/creation "
                                "order index");
                        }
                        else {
                            TESTING(
                                "query group info by creation order index in decreasing order w/o "
                                "creation order index");
                        }
                    } /* end else */
                    else {
                        ...
                        if (use_index) {
                            TESTING(
                                "query group info by creation order index in native order w/creation "
                                "order index");
                        }
                        else {
                            TESTING(
                                "query group info by creation order index in native order w/o creation "
                                "order index");
                        }
                    } /* end else */
                } /* end if */
                    ...
                    if (order == H5_ITER_INC) {
                        if (use_index) {
                            ...
                            TESTING(
                                "query group info by name index in increasing order w/o creation order "
                                "index");
                        }
                    } /* end if */
                    else if (order == H5_ITER_DEC) {
                        if (use_index) {
                            ...
                            TESTING(
                                "query group info by name index in decreasing order w/o creation order "
                                "index");
                        }
                    } /* end else */
                    else {
                ...
                for (u = 0; u < max_compact; u++) {
                    ...
                    if (order != H5_ITER_NATIVE) {
                        if (order == H5_ITER_INC) {
                            if (H5Gget_info_by_idx(group_id, ".", idx_type, order, (hsize_t)u, &grp_info, H5P_DEFAULT) < 0) {
                                TEST_ERROR;
                            }
                        } /* end if */
                        else {
                            if (H5Gget_info_by_idx(group_id, ".", idx_type, order, (hsize_t)0, &grp_info, H5P_DEFAULT) < 0) {
                                TEST_ERROR;
                            }
                        } /* end else */
 ...
 *-------------------------------------------------------------------------
 */
static int group_info_old(hid_t fapl)
{
    hid_t file_id = H5I_INVALID_HID;       /* File ID */
    hid_t group_id = H5I_INVALID_HID;      /* Group ID */
    hid_t soft_group_id = H5I_INVALID_HID; /* Group ID for soft links */
    ...
    for (order = H5_ITER_INC; order <= H5_ITER_NATIVE; order++) {
        ...
        for (u = 0; u < CORDER_NLINKS; u++) {
            ...
            if (order != H5_ITER_NATIVE) {
                if (order == H5_ITER_INC) {
                    if (H5Gget_info_by_idx(group_id, ".", H5_INDEX_NAME, order, (hsize_t)u, &grp_info, H5P_DEFAULT) < 0) {
                        TEST_ERROR;
                    }
                } /* end if */
                else {
                    if (H5Gget_info_by_idx(group_id, ".", H5_INDEX_NAME, order, (hsize_t)0, &grp_info, H5P_DEFAULT) < 0) {
                        TEST_ERROR;
                    }
                } /* end else */
 ...
 *-------------------------------------------------------------------------
 */
static int timestamps(hid_t fapl)
{
    hid_t file_id = H5I_INVALID_HID;   /* File ID */
    hid_t group_id = H5I_INVALID_HID;  /* Group ID */
 ...
 *-------------------------------------------------------------------------
 */
static int link_path_handling(hid_t fapl, bool new_format)
{
    hid_t file_id = (H5I_INVALID_HID);
 ...
 *-------------------------------------------------------------------------
 */
static int ext_link_path_handling(hid_t fapl, bool new_format)
{
    hid_t file_id = (H5I_INVALID_HID);
    hid_t grp1 = (H5I_INVALID_HID), grp2 = (H5I_INVALID_HID);
    char filename[NAME_BUF_SIZE];
    ...
    if (H5Lcreate_external("nonexistent_file", "nonexistent_object", grp1, "grp2/relative_ext_link", H5P_DEFAULT, H5P_DEFAULT) < 0) {
        TEST_ERROR;
    }
    ...
    if (H5Lcreate_external("nonexistent_file", "nonexistent_object", grp1, "/grp1/grp2/relative_soft_link", H5P_DEFAULT, H5P_DEFAULT) < 0) {
        TEST_ERROR;
    }
 ...
 *-------------------------------------------------------------------------
 */
static int ud_link_path_handling(hid_t fapl, bool new_format)
{
    hid_t file_id = (H5I_INVALID_HID);
    hid_t grp1 = (H5I_INVALID_HID), grp2 = (H5I_INVALID_HID);
    char filename[NAME_BUF_SIZE];
    ...
    if (H5Lcreate_ud(grp1, "grp2/relative_ud_link", (H5L_type_t)UD_HARD_TYPE, &(li.u.token), sizeof(li.u.token), H5P_DEFAULT, H5P_DEFAULT) < 0) {
        TEST_ERROR;
    }
    ...
    if (H5Lcreate_ud(grp1, "/grp1/grp2/absolute_ud_link", (H5L_type_t)UD_HARD_TYPE, &(li.u.token), sizeof(li.u.token), H5P_DEFAULT, H5P_DEFAULT) < 0) {
        TEST_ERROR;
    }
