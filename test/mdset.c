...
static int test_mdset_location(hid_t fapl_id)
{
    ...
    if ((dset_ids[0] = H5Dcreate2(file_id1, dset_name[0], H5T_NATIVE_UINT, file_space_ids[0], H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        TEST_ERROR;
    }
    if ((dset_ids[1] = H5Dcreate2(file_id2, dset_name[1], H5T_NATIVE_UINT, file_space_ids[1], H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        TEST_ERROR;
    }
 ...
 *-------------------------------------------------------------------------
 */
static int test_mdset(size_t niter, unsigned flags, hid_t fapl_id)
{
    ...
    for (i = 0; i < niter; i++) {
        ...
        ndsets = (flags & MDSET_FLAG_MLAYOUT) ? 6 : (flags & MDSET_FLAG_MDSET) ? (size_t)((size_t)rand() % max_dsets) + 1 : 1;
        ...
        for (j = 0; j < ndsets; j++) {
            ...
            if ((flags & MDSET_FLAG_MLAYOUT) && (j == 6)) {
                if ((source_dset = H5Dcreate2(file_id,
                                              SOURCE_DS_NAME,
                                              (flags & MDSET_FLAG_TCONV && rand() % 2) ? H5T_NATIVE_LONG : H5T_NATIVE_UINT,
                                              file_space_ids[j],
                                              H5P_DEFAULT,
        ...
        for (j = 0; j < OPS_PER_FILE; j++) {
            ...
            for (k = 0; k < ndsets; k++) {
                ...
                if (sel_type == 0) {
                    ...
                    for (l = 0; l < nhs; l++) {
                        ...
                        start[0] = (count[0] == dset_dims[k][0]) ? 0 : (hsize_t)rand() % (dset_dims[k][0] - count[0] + 1);
                        start[1] = (count[1] == dset_dims[k][1]) ? 0 : (hsize_t)rand() % (dset_dims[k][1] - count[1] + 1);

                        /* Select hyperslab */
                        if (H5Sselect_hyperslab(mem_space_ids[k], H5S_SELECT_OR, start, NULL, count, NULL) < 0) {
                            TEST_ERROR;
                        }
                        if (H5Sselect_hyperslab(file_space_ids[k], H5S_SELECT_OR, start, NULL, count, NULL) < 0) {
                            TEST_ERROR;
                        }
            ...
            if (do_read) {
                if (flags & MDSET_FLAG_MDSET) {
                    ...
                    /* Read */
                    if (H5Dread(dset_ids[0], mem_type_ids[0], mem_space_ids[0], file_space_ids[0], H5P_DEFAULT, rbuf) < 0) {
                        TEST_ERROR;
                    }
                ...
                if (flags & MDSET_FLAG_MDSET) {
                    ...
                    /* Write */
                    if (H5Dwrite(dset_ids[0], mem_type_ids[0], mem_space_ids[0], file_space_ids[0], H5P_DEFAULT, wbuf) < 0) {
                        TEST_ERROR;
                    }
