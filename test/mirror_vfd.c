...
static herr_t _verify_datasets(unsigned min_dset, unsigned max_dset, hid_t* filespace_id, hid_t* dataset_id, hid_t memspace_id);

static herr_t _create_chunking_ids(hid_t file_id,
                                   unsigned min_dset,
                                   unsigned max_dset,
                                   hsize_t* chunk_dims,
                                   hsize_t* dset_dims,
...
static herr_t _close_chunking_ids(unsigned min_dset, unsigned max_dset, hid_t* dataspace_ids, hid_t* filespace_ids, hid_t* dataset_ids, hid_t* memspace_id);

static herr_t populate_filepath(const char* dirname, const char* _basename, hid_t fapl_id, char* path_out, bool h5suffix);
 ...
 * ----------------------------------------------------------------------------
 */
static herr_t populate_filepath(const char* dirname, const char* basename, hid_t fapl_id, char* path_out, bool h5suffix)
{
    char* path = NULL;
 ...
 * ---------------------------------------------------------------------------
 */
static herr_t build_paths(const char* basename, H5FD_splitter_vfd_config_t* splitter_config, struct mirrortest_filenames* names)
{
...
static int test_encdec_uint8_t(void)
{
    unsigned char buf[8];
...
static int test_encdec_uint16_t(void)
{
    unsigned char buf[8];
    unsigned char expected[8];
...
static int test_encdec_uint32_t(void)
{
    unsigned char buf[8];
    unsigned char expected[8];
...
static int test_encdec_uint64_t(void)
{
 ...
 */
static int test_encdec_header(H5FD_mirror_xmit_t xmit_mock)
{
 ...
 * Then repeat from a different offset in the buffer and compare.
 */
static int test_encdec_set_eoa(H5FD_mirror_xmit_t xmit_mock)
{
 ...
 * Then repeat from a different offset in the buffer and compare.
 */
static int test_encdec_lock(H5FD_mirror_xmit_t xmit_mock)
{
 ...
 */
static int test_encdec_open(H5FD_mirror_xmit_t xmit_mock)
{
 ...
 * string, with all following bytes in the encoded buffer being zeroed.
 */
static int test_encdec_reply(H5FD_mirror_xmit_t xmit_mock)
{
 ...
 * Then repeat from a different offset in the buffer and compare.
 */
static int test_encdec_write(H5FD_mirror_xmit_t xmit_mock)
{
 ...
 * ---------------------------------------------------------------------------
 */
static hid_t create_mirroring_split_fapl(const char* basename, struct mirrortest_filenames* names, const struct mt_opts* opts)
{
 ...
 * ---------------------------------------------------------------------------
 */
static int test_create_and_close(const struct mt_opts* opts)
{
 ...
 * ----------------------------------------------------------------------------
 */
static herr_t create_datasets(hid_t file_id, unsigned min_dset, unsigned max_dset)
{
    ...
    if (_create_chunking_ids(file_id, min_dset, max_dset, chunk_dims, dset_dims, dataspace_ids, filespace_ids, dataset_ids, &memspace_id) < 0) {
        TEST_ERROR;
    }
    ...
    for (i = 0; i < DSET_DIM; i += CHUNK_DIM) {
        ...
        for (j = 0; j < DSET_DIM; j += CHUNK_DIM) {
            ...
            for (m = min_dset; m <= max_dset; m++) {
                ...
                LOGPRINT(3, "    H5Dwrite()\n");
                if (H5Dwrite(dataset_ids[m], H5T_NATIVE_INT, memspace_id, filespace_ids[m], H5P_DEFAULT, data_chunk) < 0) {
                    TEST_ERROR;
                }
            }
        }
                                   ...
                                   hid_t* memspace_id)
{
    ...
    for (unsigned m = min_dset; m <= max_dset; m++) {
        ...
        if ((dataset_ids[m] = H5Dcreate2(file_id, dset_name, H5T_STD_I32BE, dataspace_ids[m], H5P_DEFAULT, dcpl_id, H5P_DEFAULT)) < 0) {
 ...
 */
static herr_t
    _open_chunking_ids(hid_t file_id, unsigned min_dset, unsigned max_dset, hsize_t* chunk_dims, hid_t* filespace_ids, hid_t* dataset_ids, hid_t* memspace_id)
{
    char dset_name[DSET_NAME_LEN + 1];
 ...
 * ---------------------------------------------------------------------------
 */
static herr_t _verify_datasets(unsigned min_dset, unsigned max_dset, hid_t* filespace_ids, hid_t* dataset_ids, hid_t memspace_id)
{
    unsigned i, j, k, l, m;
    int data_chunk[CHUNK_DIM][CHUNK_DIM];
    hsize_t offset[2];
    ...
    for (i = 0; i < DSET_DIM; i += CHUNK_DIM) {
        ...
        for (j = 0; j < DSET_DIM; j += CHUNK_DIM) {
            ...
            for (m = min_dset; m <= max_dset; m++) {
                ...
                if (H5Dread(dataset_ids[m], H5T_NATIVE_INT, memspace_id, filespace_ids[m], H5P_DEFAULT, data_chunk) < 0) {
                ...
                for (k = 0; k < CHUNK_DIM; k++) {
                    for (l = 0; l < CHUNK_DIM; l++) {
                        if ((unsigned)data_chunk[k][l] != ((DSET_DIM * DSET_DIM * m) + (DSET_DIM * (i + k)) + j + l)) {
 ...
 * ---------------------------------------------------------------------------
 */
static herr_t verify_datasets(hid_t file_id, unsigned min_dset, unsigned max_dset)
{
    hid_t dataset_ids[MAX_DSET_COUNT + 1];
    hid_t filespace_ids[MAX_DSET_COUNT + 1];
    hid_t memspace_id = H5I_INVALID_HID;
    ...
    /* Generate dataspace, dataset, and 'filespace' IDs */
    if (_open_chunking_ids(file_id, min_dset, max_dset, chunk_dims, filespace_ids, dataset_ids, &memspace_id) < 0) {
        TEST_ERROR;
    }
 ...
 * ---------------------------------------------------------------------------
 */
static int test_basic_dataset_write(const struct mt_opts* opts)
{
    struct mirrortest_filenames* names = NULL;
    hid_t file_id = H5I_INVALID_HID;
    hid_t fapl_id = H5I_INVALID_HID;
    ...
    if ((dset_id = H5Dcreate2(file_id, "dataset", dtype_id, dspace_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        TEST_ERROR;
    }
 ...
 * ---------------------------------------------------------------------------
 */
static int test_chunked_dataset_write(const struct mt_opts* opts)
{
    struct mirrortest_filenames* names = NULL;
 ...
 * ---------------------------------------------------------------------------
 */
static int test_on_disk_zoo(const struct mt_opts* opts)
{
 ...
 * ---------------------------------------------------------------------------
 */
static int test_vanishing_datasets(const struct mt_opts* opts)
{
    struct mirrortest_filenames* names = NULL;
    hid_t file_id = H5I_INVALID_HID;
    hid_t fapl_id = H5I_INVALID_HID;
    ...
    for (i = 0; i < max_loops; i++) {
        ...
        if (i < (max_loops - max_at_one_time)) {
            ...
            if ((dset_id = H5Dcreate2(file_id, namebuf, H5T_STD_U32LE, dspace_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
                TEST_ERROR;
            }
 ...
 * ---------------------------------------------------------------------------
 */
static int test_concurrent_access(const struct mt_opts* opts)
{
    struct file_bundle
    {
 ...
 * ----------------------------------------------------------------------------
 */
static int parse_args(int argc, char** argv, struct mt_opts* opts)
{
    int i = 0;
 ...
 * ----------------------------------------------------------------------------
 */
static int confirm_server(struct mt_opts* opts)
{
    char mybuf[16];
    int live_socket = -1;
