...
static const char* FILENAME[] = { "objcopy_ref_src", "objcopy_ref_dst", "objcopy_ref_ext", "objcopy_ref_src2", "verbound_ref_src", "verbound_ref_dst", NULL };
 ...
 *-------------------------------------------------------------------------
 */
static void token_insert(H5O_info2_t* oinfo)
{
    size_t n;
 ...
 *-------------------------------------------------------------------------
 */
static H5_ATTR_PURE bool token_lookup(hid_t loc_id, H5O_info2_t* oinfo)
{
    size_t n;
    int token_cmp;
...
static herr_t create_reg_ref_dataset(hid_t file_id, hid_t loc_id)
{
    ...
    if ((dsetr_id = H5Dcreate2(loc_id, dsetnamer, H5T_STD_REF, spacer_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        TEST_ERROR;
    }
    ...
    if ((dsetr_id = H5Dcreate2(loc_id, dsetnamer1, H5T_STD_REF, spacer_id, H5P_DEFAULT, pid, H5P_DEFAULT)) < 0) {
        TEST_ERROR;
    }
    if (H5Pclose(pid) < 0) {
        TEST_ERROR;
    ...
    if ((dsetr_id = H5Dcreate2(loc_id, dsetnamer2, H5T_STD_REF, spacer_id, H5P_DEFAULT, pid, H5P_DEFAULT)) < 0) {
        TEST_ERROR;
    }
    if (H5Pclose(pid) < 0) {
        TEST_ERROR;
...
static int compare_datasets(hid_t did, hid_t did2, hid_t pid, const void* wbuf)
{
     ...
     *  size will thus vary)
     */
    if (!(nfilters > 0 && (H5Tdetect_class(tid, H5T_VLEN) || (H5Tdetect_class(tid, H5T_REFERENCE) && H5Tequal(tid, H5T_STD_REF))))) {
