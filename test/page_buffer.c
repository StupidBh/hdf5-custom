...
static unsigned create_file(char* filename, hid_t fcpl, hid_t fapl)
{
    ...
    for (i = 0; i < NUM_DSETS; i++) {
        ...
        if ((dset_id = H5Dcreate2(grp_id, dset_name, H5T_NATIVE_INT, filespace, H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0) {
            FAIL_STACK_ERROR;
        }
 ...
 *
 */
static unsigned set_multi_split(const char* driver_name, hid_t fapl, hsize_t pagesize)
{
    bool split = false;
    bool multi = false;
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_args(hid_t orig_fapl, const char* driver_name)
{
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_pb_fapl_tolerance_at_open(void)
{
