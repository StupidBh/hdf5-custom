...
static herr_t check_actual_selection_io_mode(hid_t dxpl, uint32_t sel_io_mode_expected)
{
 ...
 */
static herr_t test_no_type_conv(hid_t fid, unsigned set_cache, unsigned chunked, unsigned dtrans, unsigned mwbuf)
{
    int i;
    ...
    snprintf(dset_name, sizeof(dset_name), "no_tconv_%s_%s_%s", chunked ? "chunked" : "contig", dtrans ? "xform" : "noxform", mwbuf ? "mwbuf" : "nomwbuf");
 ...
 */
static herr_t test_no_size_change_no_bkg(hid_t fid, unsigned set_cache, unsigned chunked, unsigned mwbuf)
{
    int i;
    hid_t did = H5I_INVALID_HID;
    hid_t sid = H5I_INVALID_HID;
    ...
    /* Generate dataset name */
    snprintf(dset_name, sizeof(dset_name), "no_size_change_%s_%s", chunked ? "chunked" : "contig", mwbuf ? "mwbuf" : "nomwbuf");
 ...
 *
 */
static herr_t test_larger_mem_type_no_bkg(hid_t fid, unsigned set_cache, unsigned chunked, unsigned dtrans, unsigned mwbuf)
{
    int i;
    hid_t did = H5I_INVALID_HID;
    hid_t sid = H5I_INVALID_HID;
    ...
    /* Generate dataset name */
    snprintf(dset_name, sizeof(dset_name), "larger_no_bkg_%s_%s_%s", chunked ? "chunked" : "contig", dtrans ? "xform" : "noxform", mwbuf ? "mwbuf" : "nomwbuf");
 ...
 */
static herr_t test_smaller_mem_type_no_bkg(hid_t fid, unsigned set_cache, unsigned chunked, unsigned dtrans, unsigned mwbuf)
{
    int i;
    hid_t did = H5I_INVALID_HID;
    hid_t sid = H5I_INVALID_HID;
    ...
    /* Generate dataset name */
    snprintf(dset_name, sizeof(dset_name), "smaller_no_bkg_%s_%s_%s", chunked ? "chunked" : "contig", dtrans ? "xform" : "noxform", mwbuf ? "mwbuf" : "nomwbuf");
 ...
 *
 */
static herr_t test_cmpd_with_bkg(hid_t fid, unsigned chunked, unsigned mwbuf)
{
    int i;
    hid_t did = H5I_INVALID_HID;
    hid_t sid = H5I_INVALID_HID;
    ...
    /* Generate dataset name */
    snprintf(dset_name, sizeof(dset_name), "cmpd_with_bkg_%s_%s", chunked ? "chunked" : "contig", mwbuf ? "mwbuf" : "nomwbuf");
    ...
    for (i = 0; i < DSET_SELECT_DIM; i++) {
        if (s1_rbuf[i].a != s1_wbuf[i].a || s1_rbuf[i].b != ((4 * i) + 1) || s1_rbuf[i].c != s1_wbuf[i].c || s1_rbuf[i].d != ((4 * i) + 3)) {
            H5_FAILED();
            printf("    Read different values than written.\n");
            printf("    At index %d\n", i);
            TEST_ERROR;
    ...
    /* Verify data read */
    for (i = 0; i < DSET_SELECT_DIM; i++) {
        if (s1_rbuf[i].a != ((4 * i) + (2 * DSET_SELECT_DIM)) || s1_rbuf[i].b != ((4 * i) + 1) || s1_rbuf[i].c != ((4 * i) + DSET_SELECT_DIM + 2) ||
            s1_rbuf[i].d != ((4 * i) + (2 * DSET_SELECT_DIM) + 3)) {
            H5_FAILED();
            printf("    Read different values than written.\n");
            printf("    At index %d\n", i);
    ...
    if (H5Tinsert(s2_tid, "a", HOFFSET(s2_t, a), H5T_NATIVE_INT) < 0 || H5Tinsert(s2_tid, "b", HOFFSET(s2_t, b), H5T_NATIVE_LONG) < 0 ||
 ...
 */
static herr_t test_multi_dsets_no_bkg(hid_t fid, unsigned set_cache, unsigned chunked, unsigned dtrans, unsigned mwbuf)
{
    size_t ndsets;
    int i, j;
    hid_t dcpl = H5I_INVALID_HID;
    ...
    for (i = 0; i < (int)ndsets; i++) {
        ...
        /* Create ith dataset */
        if ((dset_dids[i] = H5Dcreate2(fid, dset_names[i], ((rand() % 2) ? H5T_NATIVE_LONG : H5T_NATIVE_INT), file_sids[i], H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0) {
            TEST_ERROR;
        }
    }
 ...
 *      --Verify values read
 */
static herr_t test_multi_dsets_cmpd_with_bkg(hid_t fid, unsigned chunked, unsigned mwbuf)
{
    size_t ndsets;
    int i, j, mm;
    hid_t dcpl = H5I_INVALID_HID;
    ...
    for (i = 0; i < (int)ndsets; i++) {
        ...
        /* Generate dataset name */
        snprintf(dset_names[i], sizeof(dset_names[i]), "multi_cmpd_dset%d_%s_%s", i, chunked ? "chunked" : "contig", mwbuf ? "mwbuf" : "nomwbuf");
    ...
    for (i = 0; i < (int)ndsets; i++) {
        for (j = 0; j < DSET_SELECT_DIM; j++) {
            if (i == 0) { /* dset0 */
                if (wbufi[i][j].a != rbufi[i][j].a || ((4 * (int)j) + 1) != rbufi[i][j].b || wbufi[i][j].c != rbufi[i][j].c || ((4 * (int)j) + 3) != rbufi[i][j].d) {
                    H5_FAILED();
                    printf("    Read different values than written.\n");
                    printf("    For dset %d at index %d\n", i, j);
                    TEST_ERROR;
                ...
                for (j = 0; j < DSET_SELECT_DIM; j++) {
                    if ((4 * (int)j) != rbufi[i][j].a || ((4 * (int)j) + 1) != rbufi[i][j].b || ((4 * (int)j) + 2) != rbufi[i][j].c ||
    ...
    /* dset0 */
    for (j = 0; j < DSET_SELECT_DIM; j++) {
        if (wbufi[0][j].a != rbufi[0][j].a || ((4 * (int)j) + 1) != rbufi[0][j].b || wbufi[0][j].c != rbufi[0][j].c || ((4 * (int)j) + 3) != rbufi[0][j].d) {
            H5_FAILED();
            printf("    Read different values than written.\n");
    ...
    /* <mm> dset */
    for (j = 0; j < DSET_SELECT_DIM; j++) {
        if (rbufi[mm][j].a != ((4 * (int)j) + (2 * DSET_SELECT_DIM)) || rbufi[mm][j].b != ((4 * (int)j) + 1) || rbufi[mm][j].c != ((4 * (int)j) + 2) ||
    ...
    for (i = 0; i < (int)ndsets; i++) {
        ...
        for (j = 0; j < DSET_SELECT_DIM; j++) {
            if (rbufi[i][j].a != (4 * (int)j) || rbufi[i][j].b != ((4 * (int)j) + 1) || rbufi[i][j].c != ((4 * (int)j) + 2) || rbufi[i][j].d != ((4 * (int)j) + 3)) {
                H5_FAILED();
                printf("    Read different values than written.\n");
                printf("    For dset %d at index %d\n", i, j);
                TEST_ERROR;
    ...
    for (i = 0; i < (int)ndsets; i++) {
        for (j = 0; j < DSET_SELECT_DIM; j++) {
            if (s2_wbufi[i][j].a != s2_rbufi[i][j].a || s2_wbufi[i][j].b != s2_rbufi[i][j].b || s2_wbufi[i][j].c != s2_rbufi[i][j].c ||
 ...
 */
static herr_t test_multi_dsets_size_change_no_bkg(hid_t fid, unsigned chunked, unsigned mwbuf)
{
    size_t ndsets;
    int i, j;
    hid_t dcpl = H5I_INVALID_HID;
    ...
    for (i = 0; i < (int)ndsets; i++) {
        ...
        /* Generate dataset name */
        snprintf(dset_names[i], sizeof(dset_names[i]), "multi_size_dset%d_%s_%s", i, chunked ? "chunked" : "contig", mwbuf ? "mwbuf" : "nomwbuf");
 ...
 *
 */
static herr_t test_multi_dsets_all(int niter, hid_t fid, unsigned chunked, unsigned mwbuf)
{
    size_t ndsets;
    int i, j, mm;
    int s, n;
    ...
    for (n = 0; n < niter; n++) {
        ...
        for (i = 0; i < (int)ndsets; i++) {
            ...
            if (mm == 0) {
                ...
                snprintf(dset_names[i], sizeof(dset_names[i]), "multi_all_nconv_dset%d_%s_%s", i, chunked ? "chunked" : "contig", mwbuf ? "mwbuf" : "nomwbuf");
                ...
                snprintf(dset_names[i], sizeof(dset_names[i]), "multi_all_conv_nbkg_dset%d_%s_%s", i, chunked ? "chunked" : "contig", mwbuf ? "mwbuf" : "nomwbuf");
                ...
                snprintf(dset_names[i], sizeof(dset_names[i]), "multi_all_conv_bkg_dset%d_%s_%s", i, chunked ? "chunked" : "contig", mwbuf ? "mwbuf" : "nomwbuf");
                if ((dset_dids[i] = H5Dcreate2(fid, dset_names[i], s1_tid, file_sids[i], H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0) {
                    TEST_ERROR;
                }
            }
            ...
            for (i = 0; i < (int)ndsets; i++) {
                ...
                case DSET_WITH_CONV_AND_BKG:
                    if (s == SETTING_A) {
                        ...
                        for (j = 0; j < DSET_SELECT_DIM; j++) {
                            if (s1_rbufi3[i][j].a != s1_wbufi3[i][j].a || s1_rbufi3[i][j].b != (DSET_SELECT_DIM + j) || s1_rbufi3[i][j].c != s1_wbufi3[i][j].c ||
 ...
 */
static herr_t test_set_get_select_io_mode(const char* filename, hid_t fapl)
{
 ...
 */
static herr_t test_no_selection_io_cause_mode(const char* filename, hid_t fapl, uint32_t test_mode)
{
    hid_t dcpl = H5I_INVALID_HID;
    hid_t dxpl = H5I_INVALID_HID;
    hid_t fid = H5I_INVALID_HID;
 ...
 */
static herr_t test_get_no_selection_io_cause(const char* filename, hid_t fapl)
{
    hid_t dxpl = H5I_INVALID_HID;
    H5D_selection_io_mode_t selection_io_mode;
    int errs = 0;
                ...
                for (mwbuf = false; mwbuf <= true; mwbuf++) {
                    ...
                    for (test_select_config = (int)TEST_NO_TYPE_CONV; test_select_config < (int)TEST_SELECT_NTESTS; test_select_config++) {
                        ...
                        case TEST_LARGER_MEM_NO_BKG: /* case 3 */
                            ...
                            nerrors += (test_larger_mem_type_no_bkg(fid, set_cache, chunked, dtrans, mwbuf) < 0 ? 1 : 0);
                        ...
                        case TEST_SMALLER_MEM_NO_BKG: /* case 4 */
                            ...
                            nerrors += (test_smaller_mem_type_no_bkg(fid, set_cache, chunked, dtrans, mwbuf) < 0 ? 1 : 0);
