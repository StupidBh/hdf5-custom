...
static int test_start_swmr_write_concur(hid_t H5_ATTR_UNUSED in_fapl, bool new_format)
{
    if (new_format) {
 ...
 *
 */
static herr_t dummy_append_flush_cb(hid_t H5_ATTR_UNUSED dataset_id, hsize_t H5_ATTR_UNUSED* cur_dims, void H5_ATTR_UNUSED* user_data)
{
    return SUCCEED;
}

static herr_t tssw_persist_dapl_verify(hid_t did,
                                       hid_t vdsid1,
                                       hid_t vdsid2,
                                       hsize_t boundary,
                                       H5D_append_cb_t append_func,
                                       void* append_func_ud,
                                       size_t rdcc_nslots,
                                       size_t rdcc_nbytes,
                                       double rdcc_w0,
                                       const char* efile_prefix,
...
static int test_start_swmr_write_persist_dapl(hid_t in_fapl)
{
    ...
    if (tssw_persist_dapl_verify(did,
                                 vdsid1,
                                 vdsid2,
                                 boundary,
                                 append_func,
...
static herr_t flush_cb(hid_t H5_ATTR_UNUSED obj_id, void* _udata)
{
...
static herr_t append_cb(hid_t H5_ATTR_UNUSED dset_id, hsize_t H5_ATTR_UNUSED* cur_dims, void* _udata)
{
...
/* The callback function for append flush property */
static herr_t append_cb2(hid_t H5_ATTR_UNUSED dset_id, hsize_t H5_ATTR_UNUSED* cur_dims, void* _udata)
{
    unsigned* count = (unsigned*)_udata;
    ++(*count++);
    return 0;
...
**
*****************************************************************/
static int test_file_lock_same(hid_t in_fapl)
{
...
**
*****************************************************************/
static int test_file_lock_swmr_same(hid_t in_fapl)
{
    hid_t fid = H5I_INVALID_HID;  /* File IDs */
...
static int test_file_lock_concur(hid_t H5_ATTR_UNUSED in_fapl)
{
    /* Output message about test being performed */
...
static int test_file_lock_concur(hid_t in_fapl)
{
    hid_t fid = H5I_INVALID_HID;  /* File ID */
    hid_t fapl = H5I_INVALID_HID; /* File access property list */
    char filename[NAME_BUF_SIZE]; /* file name */
...
static int test_file_lock_swmr_concur(hid_t H5_ATTR_UNUSED in_fapl)
{
    /* Output message about test being performed */
...
static int test_file_lock_swmr_concur(hid_t in_fapl)
{
    hid_t fid = H5I_INVALID_HID;  /* File ID */
    hid_t fapl;                   /* File access property list */
    char filename[NAME_BUF_SIZE]; /* file name */
...
**
*****************************************************************/
static int test_different_lock_flags(hid_t in_fapl)
{
    hid_t fid1 = H5I_INVALID_HID;    /* File ID */
    hid_t fid2 = H5I_INVALID_HID;    /* File ID */
 ...
 */
static int test_bug_refresh(hid_t in_fapl)
{
...
static int test_refresh_concur(hid_t H5_ATTR_UNUSED in_fapl, bool new_format)
{
    if (new_format) {
...
static int test_multiple_same(hid_t in_fapl, bool new_format)
{
    hid_t fid = H5I_INVALID_HID, fid1 = H5I_INVALID_HID, fid2 = H5I_INVALID_HID, fid3 = H5I_INVALID_HID; /* File IDs */
    hid_t fapl;                                                                                          /* File access property list */
    char filename[NAME_BUF_SIZE];                                                                        /* File name */
    hid_t did = H5I_INVALID_HID, did1 = H5I_INVALID_HID, did2 = H5I_INVALID_HID, did3 = H5I_INVALID_HID;
    hid_t sid = H5I_INVALID_HID;
