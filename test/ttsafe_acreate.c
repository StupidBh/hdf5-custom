...
void tts_acreate(void H5_ATTR_UNUSED* params)
{
    /* Thread declarations */
...
tts_acreate_thread(void* client_data)
{
    ...
    attribute = H5Acreate2(attrib_data->dataset, attribute_name, attrib_data->datatype, attrib_data->dataspace, H5P_DEFAULT, H5P_DEFAULT);
...
void cleanup_acreate(void H5_ATTR_UNUSED* params)
{
    if (GetTestCleanup()) {
        HDunlink(FILENAME);
    }
