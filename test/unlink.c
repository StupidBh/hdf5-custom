...
static const char* FILENAME[] = { "unlink",        "new_move_a",     "new_move_b",      "lunlink",        "filespace",  "slashes",
                                  "resurrect_set", "resurrect_type", "resurrect_group", "unlink_chunked", "full_group", NULL };
 ...
 *-------------------------------------------------------------------------
 */
static int test_one(hid_t file)
{
 ...
 *-------------------------------------------------------------------------
 */
static int test_many(hid_t file)
{
    hid_t work = H5I_INVALID_HID, grp = H5I_INVALID_HID;
    int i;
 ...
 *-------------------------------------------------------------------------
 */
static int test_symlink(hid_t file)
{
 ...
 *-------------------------------------------------------------------------
 */
static int test_rename(hid_t file)
{
 ...
 *-------------------------------------------------------------------------
 */
static int test_new_move(hid_t fapl)
{
    ...
    H5E_BEGIN_TRY
    {
        if (H5Lmove(H5L_SAME_LOC, "group_move", H5L_SAME_LOC, "group_new_name", H5P_DEFAULT, H5P_DEFAULT) != FAIL) {
            TEST_ERROR;
        }
    }
    H5E_END_TRY
 ...
 *-------------------------------------------------------------------------
 */
static int check_new_move(hid_t fapl)
{
 ...
 *-------------------------------------------------------------------------
 */
static int test_filespace(hid_t fapl)
{
    ...
    if ((dataset = H5Dcreate2(file, DATASETNAME, H5T_NATIVE_INT, space, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        FAIL_STACK_ERROR;
    }
    ...
    /* Create a single dataset to remove */
    if ((dataset = H5Dcreate2(file, DATASETNAME, H5T_NATIVE_INT, space, H5P_DEFAULT, contig_dcpl, H5P_DEFAULT)) < 0) {
        FAIL_STACK_ERROR;
    }
    if (H5Dclose(dataset) < 0) {
        FAIL_STACK_ERROR;
    ...
    /* Create a single dataset to remove */
    if ((dataset = H5Dcreate2(file, DATASETNAME, H5T_NATIVE_INT, space, H5P_DEFAULT, late_chunk_dcpl, H5P_DEFAULT)) < 0) {
        FAIL_STACK_ERROR;
    }
    if (H5Dclose(dataset) < 0) {
        FAIL_STACK_ERROR;
    ...
    /* Create a single dataset to remove */
    if ((dataset = H5Dcreate2(file, DATASETNAME, H5T_NATIVE_INT, space, H5P_DEFAULT, early_chunk_dcpl, H5P_DEFAULT)) < 0) {
        FAIL_STACK_ERROR;
    }
    if (H5Dclose(dataset) < 0) {
        FAIL_STACK_ERROR;
    ...
    /* Create a single dataset to remove */
    if ((dataset = H5Dcreate2(file, DATASETNAME, H5T_NATIVE_INT, space, H5P_DEFAULT, comp_dcpl, H5P_DEFAULT)) < 0) {
        FAIL_STACK_ERROR;
    }
    if (H5Dclose(dataset) < 0) {
        FAIL_STACK_ERROR;
    ...
    /* Create a single dataset to remove */
    if ((dataset = H5Dcreate2(file, DATASETNAME, H5T_NATIVE_INT, space, H5P_DEFAULT, compact_dcpl, H5P_DEFAULT)) < 0) {
        FAIL_STACK_ERROR;
    }
    if (H5Dclose(dataset) < 0) {
        FAIL_STACK_ERROR;
    ...
    if ((dataset2 = H5Dcreate2(file, DATASET2NAME, H5T_NATIVE_INT, space, H5P_DEFAULT, contig_dcpl, H5P_DEFAULT)) < 0) {
        FAIL_STACK_ERROR;
    }
    ...
    for (u = 0; u < FILESPACE_TOP_GROUPS; u++) {
        ...
        for (v = 0; v < FILESPACE_NESTED_GROUPS; v++) {
            ...
            for (w = 0; w < FILESPACE_NDATASETS; w++) {
                ...
                if ((dataset = H5Dcreate2(group2, objname, H5T_NATIVE_INT, space, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
                    FAIL_STACK_ERROR;
                }
                if (H5Dclose(dataset) < 0) {
                    FAIL_STACK_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_link_slashes(hid_t fapl)
{
    hid_t fid;       /* File ID */
    ...
    if (H5Lcreate_hard(gid, SLASHES_GROUP_NAME, H5L_SAME_LOC, SLASHES_HARDLINK_NAME, H5P_DEFAULT, H5P_DEFAULT) < 0) {
        FAIL_STACK_ERROR;
    }
    ...
    if (H5Lcreate_soft("////" SLASHES_GROUP_NAME "" SLASHES_GROUP_NAME, gid, SLASHES_SOFTLINK2_NAME, H5P_DEFAULT, H5P_DEFAULT) < 0) {
        FAIL_STACK_ERROR;
    }
    ...
    if (H5Lcreate_hard(fid, SLASHES_GROUP_NAME, H5L_SAME_LOC, SLASHES_HARDLINK_NAME, H5P_DEFAULT, H5P_DEFAULT) < 0) {
        FAIL_STACK_ERROR;
    }
 ...
 *-------------------------------------------------------------------------
 */
static int test_unlink_slashes(hid_t fapl)
{
    hid_t fid; /* File ID */
    hid_t gid; /* Group ID */
    char filename[1024];
 ...
 *-------------------------------------------------------------------------
 */
static int test_unlink_rightleaf(hid_t fid)
{
 ...
 *-------------------------------------------------------------------------
 */
static int test_unlink_rightnode(hid_t fid)
{
    hid_t rootid = H5I_INVALID_HID, /* Group ID for root group */
        *gids = NULL;               /* Array of IDs for groups created */
    int n,                          /* Local index variable */
 ...
 *-------------------------------------------------------------------------
 */
static int test_unlink_middlenode(hid_t fid)
{
    hid_t rootid = H5I_INVALID_HID, /* Group ID for root group */
        *gids = NULL;               /* Array of IDs for groups created */
    int n,                          /* Local index variable */
 ...
 *-------------------------------------------------------------------------
 */
static int test_resurrect_dataset(hid_t fapl)
{
 ...
 *-------------------------------------------------------------------------
 */
static int test_resurrect_datatype(hid_t fapl)
{
 ...
 *-------------------------------------------------------------------------
 */
static int test_resurrect_group(hid_t fapl)
{
 ...
 *-------------------------------------------------------------------------
 */
static int test_unlink_chunked_dataset(hid_t fapl)
{
    ...
    /* Create the dataset */
    if ((dset_id = H5Dcreate2(file_id, DATASETNAME, H5T_NATIVE_INT, space_id, H5P_DEFAULT, dcpl_id, H5P_DEFAULT)) < 0) {
        FAIL_STACK_ERROR;
    }
 ...
 *-------------------------------------------------------------------------
 */
static int test_full_group_compact(hid_t fapl)
{
    hid_t file_id = H5I_INVALID_HID;
 ...
 *-------------------------------------------------------------------------
 */
static int test_full_group_dense(hid_t fapl)
{
    hid_t file_id = H5I_INVALID_HID;
