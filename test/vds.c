...
static const char* FILENAME[] = { "vds_virt_0", "vds_virt_1", "vds_src_0", "vds_src_1",  "vds%%_src", "vds_dapl", "vds_virt_2",
                                  "vds_virt_3", "vds_src_2",  "vds_src_3", "vds%%_src2", "vds_dapl2", NULL };
 ...
 *-------------------------------------------------------------------------
 */
static htri_t vds_select_equal(hid_t space1, hid_t space2)
{
    H5S_sel_type type1;
    H5S_sel_type type2;
    hsize_t* buf1 = NULL;
    ...
    case H5S_SEL_HYPERSLABS:
        {
             ...
             * allocating, to make sure buffers are freed. */
            if (NULL == (buf1 = (hsize_t*)malloc((size_t)2 * (size_t)rank1 * (size_t)nblocks1 * sizeof(*buf1)))) {
                TEST_ERROR;
            }
            if (NULL == (buf2 = (hsize_t*)malloc((size_t)2 * (size_t)rank1 * (size_t)nblocks1 * sizeof(*buf2)))) {
                TEST_ERROR;
            }
 ...
 *-------------------------------------------------------------------------
 */
static int vds_check_mapping(hid_t dcpl, size_t i, hid_t vspace, hid_t srcspace, const char* filename, const char* dsetname)
{
...
static int test_api_get_ex_dcpl(test_api_config_t config, hid_t fapl, hid_t dcpl, hid_t* ex_dcpl, hid_t vspace, char* filename, hsize_t exp_meta_size)
{
    ...
    if (config >= TEST_API_CREATE_DSET) {
        ...
        else if ((ninfo.meta_size.obj.heap_size != exp_meta_size) && (ninfo.meta_size.obj.heap_size != (hsize_t)0)) {
            TEST_ERROR;
        }
...
/* Main test function */
static int test_api(test_api_config_t config, hid_t fapl, H5F_libver_t low)
{
    char filename[FILENAME_BUF_SIZE];
    hid_t dcpl = H5I_INVALID_HID;                                                                           /* Dataset creation property list */
    hid_t ex_dcpl = H5I_INVALID_HID;                                                                        /* Temporary dcpl for examination */
    hid_t srcspace[4] = { H5I_INVALID_HID, H5I_INVALID_HID, H5I_INVALID_HID, H5I_INVALID_HID };             /* Source dataspaces */
    ...
    switch (config) {
    case TEST_API_BASIC       : TESTING_2("virtual dataset API functions"); break;
    case TEST_API_COPY_PLIST  : TESTING_2("virtual dataset API functions with copied plists"); break;
    case TEST_API_ENCDEC_PLIST: TESTING_2("virtual dataset API functions with encoded and decoded plists"); break;
    case TEST_API_CREATE_DSET : TESTING_2("virtual dataset create"); break;
    case TEST_API_REOPEN_DSET : TESTING_2("virtual dataset create with reopened dataset"); break;
    case TEST_API_REOPEN_FILE : TESTING_2("virtual dataset create with reopened file"); break;
    ...
    /* Get examination DCPL */
    if (test_api_get_ex_dcpl(config, fapl, dcpl, &ex_dcpl, vspace[0], filename, (low >= H5F_LIBVER_V112) ? (hsize_t)99 : (low >= H5F_LIBVER_V110 ? 174 : 213)) < 0) {
        TEST_ERROR;
    }
    ...
    /* Get examination DCPL */
    if (test_api_get_ex_dcpl(config, fapl, dcpl, &ex_dcpl, vspace[0], filename, (low >= H5F_LIBVER_V112) ? (hsize_t)607 : (hsize_t)697) < 0) {
        TEST_ERROR;
    }
 ...
 *-------------------------------------------------------------------------
 */
static int test_vds_prefix_first(unsigned config, hid_t vds_fapl, hid_t src_fapl)
{
    ...
    hid_t srcfile[4] = { H5I_INVALID_HID, H5I_INVALID_HID, H5I_INVALID_HID, H5I_INVALID_HID };  /* Files with source dsets */
    hid_t vfile = H5I_INVALID_HID;                                                              /* File with virtual dset */
    hid_t dcpl = H5I_INVALID_HID;                                                               /* Dataset creation property list */
    hid_t dapl = H5I_INVALID_HID;                                                               /* Dataset access property list */
    hid_t srcspace[4] = { H5I_INVALID_HID, H5I_INVALID_HID, H5I_INVALID_HID, H5I_INVALID_HID }; /* Source dataspaces */
    hid_t vspace[4] = { H5I_INVALID_HID, H5I_INVALID_HID, H5I_INVALID_HID, H5I_INVALID_HID };   /* Virtual dset dataspaces */
    hid_t memspace = H5I_INVALID_HID;                                                           /* Memory dataspace */
    hid_t srcdset[4] = { H5I_INVALID_HID, H5I_INVALID_HID, H5I_INVALID_HID, H5I_INVALID_HID };  /* Source datasets */
 ...
 *-------------------------------------------------------------------------
 */
static int test_basic_io(unsigned config, hid_t vds_fapl, hid_t src_fapl)
{
    char* srcfilename = NULL;
    char* srcfilename_map = NULL;
    char* vfilename = NULL;
    ...
    for (i = 0; i < (int)(sizeof(buf) / sizeof(buf[0])); i++) {
        for (j = 0; j < (int)(sizeof(buf[0]) / sizeof(buf[0][0])); j++) {
            if (rbuf[i][j] != (j < (int)(sizeof(buf[0]) / sizeof(buf[0][0]) / 2) ? (buf[i][j] - (int)(sizeof(buf) / sizeof(buf[0][0]))) : buf[i][j])) {
                TEST_ERROR_SUPPRESSED;
            }
        }
    }
 ...
 *-------------------------------------------------------------------------
 */
static int test_unlim(unsigned config, hid_t vds_fapl, hid_t src_fapl)
{
 ...
 *-------------------------------------------------------------------------
 */
static int test_printf(unsigned config, hid_t vds_fapl, hid_t src_fapl)
{
    char* srcfilename = NULL;
    char* srcfilename_map = NULL;
    ...
    hid_t memspace = H5I_INVALID_HID;                                                                                            /* Memory dataspace */
    hid_t filespace = H5I_INVALID_HID;                                                                                           /* File dataspace */
    hid_t srcdset[6] = { H5I_INVALID_HID, H5I_INVALID_HID, H5I_INVALID_HID, H5I_INVALID_HID, H5I_INVALID_HID, H5I_INVALID_HID }; /* Source datasets */
    hid_t vdset = H5I_INVALID_HID;                                                                                               /* Virtual dataset */
 ...
 *-------------------------------------------------------------------------
 */
static int test_all(unsigned config, hid_t vds_fapl, hid_t src_fapl)
{
    char vfilename[FILENAME_BUF_SIZE];
    char srcfilename[FILENAME_BUF_SIZE];
    char srcfilename_map[FILENAME_BUF_SIZE];
    ...
    hid_t memspace = H5I_INVALID_HID;                                                                           /* Memory dataspace */
    hid_t filespace = H5I_INVALID_HID;                                                                          /* File dataspace */
    hid_t srcdset[5] = { H5I_INVALID_HID, H5I_INVALID_HID, H5I_INVALID_HID, H5I_INVALID_HID, H5I_INVALID_HID }; /* Source datasets */
    hid_t vdset = H5I_INVALID_HID;                                                                              /* Virtual dataset */
 ...
 *-------------------------------------------------------------------------
 */
static int test_dapl_values(hid_t fapl_id)
{
...
int main(void)
{
    ...
    for (low = H5F_LIBVER_EARLIEST; low < H5F_LIBVER_NBOUNDS; low++) {
        for (high = H5F_LIBVER_EARLIEST; high < H5F_LIBVER_NBOUNDS; high++) {
            ...
            snprintf(msg, sizeof(msg), "Testing virtual dataset I/O with file version bounds: (%s, %s):", low_string, high_string);
            puts(msg);
            ...
            for (bit_config = 0; bit_config < TEST_IO_NTESTS; bit_config++) {
                ...
                /* Print message */
                PRINT_CONFIG("%s%s%s, %s file close degree",
                             ...
                             bit_config & TEST_IO_FCLOSE_SEMI ? "H5F_CLOSE_SEMI" : (bit_config & TEST_IO_FCLOSE_STRONG ? "H5F_CLOSE_STRONG" : "H5F_CLOSE_WEAK"));
