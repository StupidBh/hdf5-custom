 ...
 */
static hsize_t MAX_DIMS[N_SOURCES][RANK] = { { N_MAX_PLANES, SM_HEIGHT, WIDTH }, { N_MAX_PLANES, LG_HEIGHT, WIDTH }, { N_MAX_PLANES, SM_HEIGHT, WIDTH },
...
int main(void)
{
    ...
    for (i = 0; i < N_SOURCES; i++) {
        ...
        if ((did = H5Dcreate2(fid, SOURCE_DSET_NAME, SOURCE_DATATYPE, src_sid, H5P_DEFAULT, src_dcplid, H5P_DEFAULT)) < 0) {
            TEST_ERROR;
        }
    ...
    /* dataset */
    if ((did = H5Dcreate2(fid, VDS_DSET_NAME, VDS_DATATYPE, vds_sid, H5P_DEFAULT, vds_dcplid, H5P_DEFAULT)) < 0) {
        TEST_ERROR;
    }
