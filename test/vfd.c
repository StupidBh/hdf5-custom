...
static int splitter_create_single_file_at(const char* filename, hid_t fapl_id, const struct splitter_dataset_def* data);
...
static herr_t H5FD__ctl_test_vfd_read(H5FD_t* _file, H5FD_mem_t type, hid_t dxpl_id, haddr_t addr, size_t size, void* buf);
static herr_t H5FD__ctl_test_vfd_write(H5FD_t* _file, H5FD_mem_t type, hid_t dxpl_id, haddr_t addr, size_t size, const void* buf);
static herr_t H5FD__ctl_test_vfd_ctl(H5FD_t* _file, uint64_t op_code, uint64_t flags, const void* input, void** output);
...
static herr_t test_sec2(void)
{
    ...
    if (driver_flags != (H5FD_FEAT_AGGREGATE_METADATA | H5FD_FEAT_ACCUMULATE_METADATA | H5FD_FEAT_DATA_SIEVE | H5FD_FEAT_AGGREGATE_SMALLDATA |
                         H5FD_FEAT_POSIX_COMPAT_HANDLE | H5FD_FEAT_SUPPORTS_SWMR_IO | H5FD_FEAT_DEFAULT_VFD_COMPATIBLE)) {
        TEST_ERROR;
    }
...
static herr_t test_core(void)
{
    ...
    /* Check for extra flags not accounted for above */
    if (driver_flags != (H5FD_FEAT_AGGREGATE_METADATA | H5FD_FEAT_ACCUMULATE_METADATA | H5FD_FEAT_DATA_SIEVE | H5FD_FEAT_AGGREGATE_SMALLDATA |
                         H5FD_FEAT_ALLOW_FILE_IMAGE | H5FD_FEAT_CAN_USE_FILE_IMAGE_CALLBACKS)) {
        TEST_ERROR;
    }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_direct(void)
{
#ifdef H5_HAVE_DIRECT
    hid_t file = H5I_INVALID_HID, fapl = H5I_INVALID_HID, access_fapl = H5I_INVALID_HID;
    hid_t dset1 = H5I_INVALID_HID, dset2 = H5I_INVALID_HID, space1 = H5I_INVALID_HID, space2 = H5I_INVALID_HID;
    char filename[1024];
    int* fhandle = NULL;
    hsize_t file_size;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_family_opens(char* fname, hid_t fa_pl)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_family(void)
{
    hid_t file = H5I_INVALID_HID, fapl = H5I_INVALID_HID, fapl2 = H5I_INVALID_HID, space = H5I_INVALID_HID, dset = H5I_INVALID_HID;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_family_compat(void)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_family_member_fapl(void)
{
    hid_t file = H5I_INVALID_HID;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_multi_opens(char* fname)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_multi(void)
{
    hid_t file = H5I_INVALID_HID, fapl = H5I_INVALID_HID, fapl2 = H5I_INVALID_HID, dset = H5I_INVALID_HID, space = H5I_INVALID_HID;
    ...
    /* Check for extra flags not accounted for above */
    if (driver_flags != (H5FD_FEAT_DATA_SIEVE | H5FD_FEAT_AGGREGATE_SMALLDATA | H5FD_FEAT_USE_ALLOC_SIZE | H5FD_FEAT_PAGED_AGGR)) {
        TEST_ERROR;
    }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_multi_compat(void)
{
...
static herr_t test_stdio(void)
{
    ...
    /* Check for extra flags not accounted for above */
    if (driver_flags !=
        (H5FD_FEAT_AGGREGATE_METADATA | H5FD_FEAT_ACCUMULATE_METADATA | H5FD_FEAT_DATA_SIEVE | H5FD_FEAT_AGGREGATE_SMALLDATA | H5FD_FEAT_DEFAULT_VFD_COMPATIBLE)) {
        TEST_ERROR;
    }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_windows(void)
{
...
static herr_t test_ros3(void)
{
    ...
    if ((test_ros3_fa.version != ros3_fa_0.version) || (test_ros3_fa.authenticate != ros3_fa_0.authenticate) ||
        (strcmp(test_ros3_fa.aws_region, ros3_fa_0.aws_region) != 0) || (strcmp(test_ros3_fa.secret_id, ros3_fa_0.secret_id) != 0) ||
 ...
 *-------------------------------------------------------------------------
 */
static int compare_splitter_config_info(hid_t fapl_id, H5FD_splitter_vfd_config_t* info)
{
    int ret_value = 0;
    ...
    if ((strlen(info->wo_path) != strlen(fetched_info->wo_path)) || strncmp(info->wo_path, fetched_info->wo_path, H5FD_SPLITTER_PATH_MAX) != 0) {
...
static int run_splitter_test(const struct splitter_dataset_def* data, bool ignore_wo_errors, bool provide_logfile_path, const hid_t sub_fapl_ids[2])
{
    ...
    if ((true == provide_logfile_path && NULL == logfile) || (false == provide_logfile_path && NULL != logfile)) {
 ...
 *-------------------------------------------------------------------------
 */
static int driver_is_splitter_compatible(hid_t fapl_id)
{
    H5FD_splitter_vfd_config_t* vfd_config = NULL;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_splitter(void)
{
    int buf[SPLITTER_SIZE][SPLITTER_SIZE];
 ...
 */
static H5FD_t* H5FD__ctl_test_vfd_open(const char H5_ATTR_UNUSED* name, unsigned H5_ATTR_UNUSED flags, hid_t H5_ATTR_UNUSED fapl_id, haddr_t H5_ATTR_UNUSED maxaddr)
{
    return calloc(1, sizeof(H5FD_t));
}

static herr_t H5FD__ctl_test_vfd_close(H5FD_t H5_ATTR_UNUSED* _file)
{
    free(_file);
    return SUCCEED;
}

static haddr_t H5FD__ctl_test_vfd_get_eoa(const H5FD_t H5_ATTR_UNUSED* file, H5FD_mem_t H5_ATTR_UNUSED type)
{
    return HADDR_UNDEF;
}

static herr_t H5FD__ctl_test_vfd_set_eoa(H5FD_t H5_ATTR_UNUSED* _file, H5FD_mem_t H5_ATTR_UNUSED type, haddr_t H5_ATTR_UNUSED addr)
{
    return FAIL;
}

static haddr_t H5FD__ctl_test_vfd_get_eof(const H5FD_t H5_ATTR_UNUSED* file, H5FD_mem_t H5_ATTR_UNUSED type)
{
    return HADDR_UNDEF;
}

static herr_t H5FD__ctl_test_vfd_read(H5FD_t H5_ATTR_UNUSED* _file,
...
static herr_t H5FD__ctl_test_vfd_write(H5FD_t H5_ATTR_UNUSED* _file,
                                       H5FD_mem_t H5_ATTR_UNUSED type,
                                       hid_t H5_ATTR_UNUSED fapl_id,
                                       haddr_t H5_ATTR_UNUSED addr,
                                       size_t H5_ATTR_UNUSED size,
...
static herr_t H5FD__ctl_test_vfd_ctl(H5FD_t H5_ATTR_UNUSED* _file, uint64_t op_code, uint64_t flags, const void H5_ATTR_UNUSED* input, void H5_ATTR_UNUSED** output)
{
    herr_t ret_value = SUCCEED;

    switch (op_code) {
    /* Op code for testing purposes */
    case H5FD_CTL_TEST_OPCODE: break;
...
static herr_t run_ctl_test(uint64_t op_code, uint64_t flags, ctl_test_opc_type opc_type, hid_t fapl_id)
{
    ...
    if (is_passthrough_vfd) {
         ...
         */
        expect_fail = expect_fail || ((CTL_OPC_KNOWN_TERMINAL == opc_type) && !routing_flag_set && fail_if_unknown);
    }
...
static herr_t test_ctl(void)
{
    ...
    TESTING_2(
        "known op code through passthrough VFD to terminal VFD (without fail on unknown flag/no "
        "routing flag)");
    ...
    TESTING_2("known op code through passthrough VFD to terminal VFD (with fail on unknown flag/no routing flag)");
    ...
    TESTING_2(
        "known op code through passthrough VFD to terminal VFD (without fail on unknown flag/route to "
        "terminal VFD)");
    ...
    TESTING_2(
        "known op code through passthrough VFD to terminal VFD (with fail on unknown flag/route to "
        "terminal VFD)");
...
static bool
    test_vector_io__setup_v(uint32_t count, H5FD_mem_t types[], haddr_t addrs[], size_t sizes[], const void* write_bufs[], void* read_bufs[], char base_fill_char)
{
    ...
    uint32_t i;
    uint32_t j;
    H5FD_mem_t mem_types[6] = { H5FD_MEM_SUPER, H5FD_MEM_BTREE, H5FD_MEM_DRAW, H5FD_MEM_GHEAP, H5FD_MEM_LHEAP, H5FD_MEM_OHDR };
...
static bool test_vector_io__setup_fixed_size_v(uint32_t count,
                                               H5FD_mem_t types[],
                                               haddr_t addrs[],
                                               size_t sizes[],
...
static bool test_vector_io__read_v_indiv(H5FD_t* lf, uint32_t count, H5FD_mem_t types[], haddr_t addrs[], size_t sizes[], void* read_bufs[])
{
...
static bool test_vector_io__write_v_indiv(H5FD_t* lf, uint32_t count, H5FD_mem_t types[], haddr_t addrs[], size_t sizes[], const void* write_bufs[])
{
    bool size_fixed = false;
    bool type_fixed = false;
    bool result = true; /* will set to false on failure */
...
static bool test_vector_io__verify_v(uint32_t count, H5FD_mem_t types[], size_t sizes[], const void* write_bufs[], void* read_bufs[], const char* name)
{
    bool size_fixed = false;
    bool type_fixed = false;
    bool identical = true;
    ...
    const char* w_buf;
    char* r_buf;
    const char* mem_type_names[7] = { "H5FD_MEM_DEFAULT", "H5FD_MEM_SUPER", "H5FD_MEM_BTREE", "H5FD_MEM_DRAW", "H5FD_MEM_GHEAP", "H5FD_MEM_LHEAP", "H5FD_MEM_OHDR" };
    size_t size = SIZE_MAX;
    H5FD_mem_t type = H5FD_MEM_NTYPES;
        ...
        while ((j < size) && (identical)) {
            if (w_buf[j] != r_buf[j]) {
                ...
                if (verbose) {
                    ...
                    fprintf(stdout,
                            "\"%s\"/%u at offset %llu/%llu w/r = %c/%c type = %s\n\n",
                            name,
                            (unsigned)i,
...
static void test_vector_io__dump_test_vectors(uint32_t count,
                                              H5FD_mem_t types[],
                                              haddr_t addrs[],
                                              size_t sizes[],
                                              const void* write_bufs[],
                                              ...
                                              const char* name)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        fprintf(stdout,
                "%u: addr/len = %llu/%llu, type = %s, w_buf = \"%s\"\n",
                (unsigned)i,
                (long long unsigned)(addrs[i]),
                (long long unsigned)(size),
                mem_type_names[type],
                w_buf);
...
static herr_t test_vector_io(const char* vfd_name)
{
    char test_title[80];
    ...
    if (!(test_vector_io__setup_fixed_size_v(count, f_types_0, f_addrs_0, f_sizes_0, f_write_bufs_0, f_read_bufs_0, 'b') &&
          test_vector_io__setup_fixed_size_v(count, f_types_1, f_addrs_1, f_sizes_1, f_write_bufs_1, f_read_bufs_1, 'f') &&
          test_vector_io__setup_fixed_size_v(count, f_types_2, f_addrs_2, f_sizes_2, f_write_bufs_2, f_read_bufs_2, 'B'))) {
        TEST_ERROR;
    }
    ...
    if (verbose) {
        ...
        test_vector_io__dump_test_vectors(count, f_types_0, f_addrs_0, f_sizes_0, f_write_bufs_0, NULL, "fixed zero");

        test_vector_io__dump_test_vectors(count, f_types_1, f_addrs_1, f_sizes_1, f_write_bufs_1, NULL, "fixed one");

        test_vector_io__dump_test_vectors(count, f_types_2, f_addrs_2, f_sizes_2, f_write_bufs_2, NULL, "fixed two");
    }

    /* write and then read using vector I/O.  First, read/write vector
     * of length 1, then of length 2, then remainder of vector
     */
    if (H5FDwrite_vector(lf, H5P_DEFAULT, 1, &(types_0[0]), &(addrs_0[0]), &(sizes_0[0]), &(write_bufs_0[0])) < 0) {
        TEST_ERROR;
    }

    if (H5FDread_vector(lf, H5P_DEFAULT, 1, &(types_0[0]), &(addrs_0[0]), &(sizes_0[0]), &(read_bufs_0[0])) < 0) {
        TEST_ERROR;
    }

    if (H5FDwrite_vector(lf, H5P_DEFAULT, 2, &(types_0[1]), &(addrs_0[1]), &(sizes_0[1]), &(write_bufs_0[1])) < 0) {
        TEST_ERROR;
    }

    if (H5FDread_vector(lf, H5P_DEFAULT, 2, &(types_0[1]), &(addrs_0[1]), &(sizes_0[1]), &(read_bufs_0[1])) < 0) {
        TEST_ERROR;
    }

    if (H5FDwrite_vector(lf, H5P_DEFAULT, count - 3, &(types_0[3]), &(addrs_0[3]), &(sizes_0[3]), &(write_bufs_0[3])) < 0) {
        TEST_ERROR;
    }

    if (H5FDread_vector(lf, H5P_DEFAULT, count - 3, &(types_0[3]), &(addrs_0[3]), &(sizes_0[3]), &(read_bufs_0[3])) < 0) {
        TEST_ERROR;
    }
    ...
    if (H5FDread_vector(lf, H5P_DEFAULT, 1, &(types_1[0]), &(addrs_1[0]), &(sizes_1[0]), &(read_bufs_1[0])) < 0) {
        TEST_ERROR;
    }

    if (H5FDread_vector(lf, H5P_DEFAULT, 2, &(types_1[1]), &(addrs_1[1]), &(sizes_1[1]), &(read_bufs_1[1])) < 0) {
        TEST_ERROR;
    }

    if (H5FDread_vector(lf, H5P_DEFAULT, count - 3, &(types_1[3]), &(addrs_1[3]), &(sizes_1[3]), &(read_bufs_1[3])) < 0) {
        TEST_ERROR;
    }
     ...
     */
    if (H5FDwrite_vector(lf, H5P_DEFAULT, 1, &(types_2[0]), &(addrs_2[0]), &(sizes_2[0]), &(write_bufs_2[0])) < 0) {
        TEST_ERROR;
    }

    if (H5FDwrite_vector(lf, H5P_DEFAULT, 2, &(types_2[1]), &(addrs_2[1]), &(sizes_2[1]), &(write_bufs_2[1])) < 0) {
        TEST_ERROR;
    }

    if (H5FDwrite_vector(lf, H5P_DEFAULT, count - 3, &(types_2[3]), &(addrs_2[3]), &(sizes_2[3]), &(write_bufs_2[3])) < 0) {
        TEST_ERROR;
    }
...
static herr_t test_selection_io_write(H5FD_t* lf,
                                      H5FD_mem_t type,
                                      uint32_t count,
                                      hid_t mem_spaces[],
                                      hid_t file_spaces[],
                                      ...
                                      int* wbufs[])
{
    ...
    /* Issue write call */
    if (H5FDwrite_selection(lf, type, H5P_DEFAULT, count, mem_spaces, file_spaces, offsets, element_sizes, bufs) < 0) {
        TEST_ERROR;
    }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_selection_io_read_verify(H5FD_t* lf,
                                            H5FD_mem_t type,
                                            uint32_t count,
                                            hid_t mem_spaces[],
                                            hid_t file_spaces[],
                                            ...
                                            bool shorten_rbufs)
{
    ...
    /* Issue read call */
    if (H5FDread_selection(lf, type, H5P_DEFAULT, count, mem_spaces, file_spaces, offsets, element_sizes, (void**)rbufs) < 0) {
        TEST_ERROR;
    }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_selection_io(const char* vfd_name)
{
    char test_title[80];
    hid_t fapl_id = H5I_INVALID_HID;                             /* file access property list ID */
    char filename[1024];                                         /* filename                     */
    ...
    for (type = 1; type < H5FD_MEM_NTYPES; type++) {
         ...
         */
        /* Issue write call */
        if (test_selection_io_write(lf, type, 1, &mem_spaces[0], &file_spaces[0], &addrs[0], element_sizes, (int**)&wbufs[0]) < 0) {
            TEST_ERROR;
        }
        ...
        /* Read and verify */
        if (test_selection_io_read_verify(lf, type, 1, &mem_spaces[0], &file_spaces[0], &addrs[0], element_sizes, 1, (int**)&fbufs[0], false) < 0) {
            TEST_ERROR;
        }
         ...
         */
        /* Issue write call */
        if (test_selection_io_write(lf, type, 1, &mem_spaces[1], &file_spaces[1], &addrs[1], element_sizes, (int**)&wbufs[1]) < 0) {
            TEST_ERROR;
        }
        ...
        /* Read and verify */
        if (test_selection_io_read_verify(lf, type, 1, &mem_spaces[1], &file_spaces[1], &addrs[1], element_sizes, 1, (int**)&fbufs[1], false) < 0) {
            TEST_ERROR;
        }
        ...
        /* Read and verify */
        if (test_selection_io_read_verify(lf, type, 1, &mem_spaces[0], &file_spaces[0], &addrs[0], element_sizes, 1, (int**)&erbufs[0], false) < 0) {
            TEST_ERROR;
        }
        ...
        /* Read and verify */
        if (test_selection_io_read_verify(lf, type, 1, &mem_spaces[1], &file_spaces[1], &addrs[1], element_sizes, 1, (int**)&erbufs[1], false) < 0) {
            TEST_ERROR;
        }
        ...
        /* Issue write call */
        if (test_selection_io_write(lf, type, 1, &mem_spaces[0], &file_spaces[1], &addrs[1], element_sizes, (int**)&wbufs[0]) < 0) {
            TEST_ERROR;
        }
        ...
        /* Read and verify */
        if (test_selection_io_read_verify(lf, type, 1, &mem_spaces[0], &file_spaces[1], &addrs[1], element_sizes, 1, (int**)&erbufs[0], false) < 0) {
            TEST_ERROR;
        }
        ...
        /* Read entire file buffer and verify */
        if (test_selection_io_read_verify(lf, type, 1, &mem_spaces[0], &file_spaces[1], &addrs[1], element_sizes, 1, (int**)&fbufs[1], false) < 0) {
            TEST_ERROR;
        }
        ...
        /* Issue write call */
        if (test_selection_io_write(lf, type, 1, &mem_spaces[1], &file_spaces[0], &addrs[0], element_sizes, (int**)&wbufs[1]) < 0) {
            TEST_ERROR;
        }
        ...
        /* Read and verify */
        if (test_selection_io_read_verify(lf, type, 1, &mem_spaces[1], &file_spaces[0], &addrs[0], element_sizes, 1, (int**)&erbufs[1], false) < 0) {
            TEST_ERROR;
        }
        ...
        /* Read entire file buffer and verify */
        if (test_selection_io_read_verify(lf, type, 1, &mem_spaces[1], &file_spaces[0], &addrs[0], element_sizes, 1, (int**)&fbufs[0], false) < 0) {
            TEST_ERROR;
        }
        ...
        for (shorten_element_sizes = 0; shorten_element_sizes <= 1; shorten_element_sizes++) {
            ...
            /* Issue write call */
            if (test_selection_io_write(lf, type, 2, mem_spaces, file_spaces, addrs, element_sizes, (int**)wbufs) < 0) {
                TEST_ERROR;
            }
            ...
            /* Read and verify */
            if (test_selection_io_read_verify(lf, type, 2, mem_spaces, file_spaces, addrs, element_sizes, 2, (int**)erbufs, false) < 0) {
                TEST_ERROR;
            }
            ...
            /* Read entire file buffer and verify */
            if (test_selection_io_read_verify(lf, type, 2, mem_spaces, file_spaces, addrs, element_sizes, 2, (int**)fbufs, false) < 0) {
                TEST_ERROR;
            }
            ...
            /* Read and verify */
            if (test_selection_io_read_verify(lf,
                                              type,
                                              2,
                                              mem_spaces,
                                              file_spaces,
