...
static void test_async_one_dataset_io(void* params)
{
    ...
    if ((dset_id = H5Dcreate_async(file_id, "dset", H5T_NATIVE_INT, space_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT, es_id)) < 0) {
        TEST_ERROR;
    }
...
static void test_async_multi_dataset_io(void* params)
{
    ...
    hid_t file_id = H5I_INVALID_HID;
    hid_t fapl_id = H5I_INVALID_HID;
    hid_t dset_id[MULTI_DATASET_IO_TEST_NDSETS] = { H5I_INVALID_HID, H5I_INVALID_HID, H5I_INVALID_HID, H5I_INVALID_HID, H5I_INVALID_HID };
    hid_t space_id = H5I_INVALID_HID;
    hid_t mspace_id = H5I_INVALID_HID;
    hid_t es_id = H5I_INVALID_HID;
    char dset_name[32];
    ...
    BEGIN_MULTIPART
    {
        PART_BEGIN(multi_dset_open)
        {
            ...
            for (i = 0; i < MULTI_DATASET_IO_TEST_NDSETS; i++) {
                ...
                /* Create the dataset asynchronously */
                if ((dset_id[i] = H5Dcreate_async(file_id, dset_name, H5T_NATIVE_INT, space_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT, es_id)) < 0) {
                ...
                /* Write the dataset asynchronously */
                if (H5Dwrite_async(dset_id[i], H5T_NATIVE_INT, mspace_id, space_id, H5P_DEFAULT, &write_buf[buf_start_idx], es_id) < 0) {
                    PART_TEST_ERROR(multi_dset_open);
                }
            } /* end for */
            ...
            for (i = 0; i < MULTI_DATASET_IO_TEST_NDSETS; i++) {
                ...
                /* Read the dataset asynchronously */
                if (H5Dread_async(dset_id[i], H5T_NATIVE_INT, mspace_id, space_id, H5P_DEFAULT, &read_buf[buf_start_idx], es_id) < 0) {
                    PART_TEST_ERROR(multi_dset_open);
                }
            } /* end for */
        ...
        PART_BEGIN(multi_dset_close)
        {
            ...
            for (i = 0; i < MULTI_DATASET_IO_TEST_NDSETS; i++) {
                ...
                /* Write the dataset asynchronously */
                if (H5Dwrite_async(dset_id[0], H5T_NATIVE_INT, mspace_id, space_id, H5P_DEFAULT, &write_buf[buf_start_idx], es_id) < 0) {
                    PART_TEST_ERROR(multi_dset_close);
                }
            ...
            for (i = 0; i < MULTI_DATASET_IO_TEST_NDSETS; i++) {
                ...
                /* Read the dataset asynchronously */
                buf_start_idx = i * (data_size / MULTI_DATASET_IO_TEST_NDSETS / sizeof(int));
                if (H5Dread_async(dset_id[0], H5T_NATIVE_INT, mspace_id, space_id, H5P_DEFAULT, &read_buf[buf_start_idx], es_id) < 0) {
                    PART_TEST_ERROR(multi_dset_close);
                }
...
static void test_async_multi_file_dataset_io(void* params)
{
    ...
    size_t i, j, data_size, num_in_progress;
    hid_t fapl_id = H5I_INVALID_HID;
    hid_t file_id[MULTI_FILE_DATASET_IO_TEST_NFILES] = { H5I_INVALID_HID, H5I_INVALID_HID, H5I_INVALID_HID, H5I_INVALID_HID, H5I_INVALID_HID };
    hid_t dset_id[MULTI_FILE_DATASET_IO_TEST_NFILES] = { H5I_INVALID_HID, H5I_INVALID_HID, H5I_INVALID_HID, H5I_INVALID_HID, H5I_INVALID_HID };
    hid_t space_id = H5I_INVALID_HID;
    hid_t mspace_id = H5I_INVALID_HID;
    hid_t es_id = H5I_INVALID_HID;
    char file_name[32];
    ...
    BEGIN_MULTIPART
    {
        PART_BEGIN(multi_file_dset_open)
        {
            ...
            for (i = 0; i < MULTI_FILE_DATASET_IO_TEST_NFILES; i++) {
                ...
                /* Create the dataset asynchronously */
                if ((dset_id[i] = H5Dcreate_async(file_id[i], "dset", H5T_NATIVE_INT, space_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT, es_id)) < 0) {
                    PART_TEST_ERROR(multi_file_dset_open);
                }
    ...
    BEGIN_MULTIPART
    {
        PART_BEGIN(multi_file_grp_dset_no_kick)
        {
            ...
            for (i = 0; i < MULTI_FILE_GRP_DSET_IO_TEST_NFILES; i++) {
                ...
                /* Create the dataset asynchronously */
                if ((dset_id = H5Dcreate_async(grp_id, "dset", H5T_NATIVE_INT, space_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT, es_id)) < 0) {
                    PART_TEST_ERROR(multi_file_grp_dset_no_kick);
                }
                ...
                /* Write the dataset asynchronously */
                if (H5Dwrite_async(dset_id, H5T_NATIVE_INT, mspace_id, space_id, H5P_DEFAULT, &write_buf[buf_start_idx], es_id) < 0) {
                    PART_TEST_ERROR(multi_file_grp_dset_no_kick);
                }
            ...
            for (i = 0; i < MULTI_FILE_GRP_DSET_IO_TEST_NFILES; i++) {
                ...
                /* Read the dataset asynchronously */
                buf_start_idx = i * (data_size / MULTI_FILE_GRP_DSET_IO_TEST_NFILES / sizeof(int));
                if (H5Dread_async(dset_id, H5T_NATIVE_INT, mspace_id, space_id, H5P_DEFAULT, &read_buf[buf_start_idx], es_id) < 0) {
                    PART_TEST_ERROR(multi_file_grp_dset_no_kick);
                }
...
static void test_async_set_extent(void* params)
{
    ...
    if (!(vol_cap_flags_g & H5VL_CAP_FLAG_ASYNC) || !(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) || !(vol_cap_flags_g & H5VL_CAP_FLAG_FLUSH_REFRESH) ||
        ...
        if (MAINPROCESS) {
            SKIPPED();
            printf(
                "    API functions for basic file, dataset, dataset more, or flush aren't supported "
                "with this connector\n");
        }
    ...
    /* Create the dataset asynchronously */
    if ((dset_id = H5Dcreate_async(file_id, "dset", H5T_NATIVE_INT, space_id, H5P_DEFAULT, dcpl_id, H5P_DEFAULT, es_id)) < 0) {
        TEST_ERROR;
    }
...
static void test_async_attribute_exists(void* params)
{
    ...
    if (!(vol_cap_flags_g & H5VL_CAP_FLAG_ASYNC) || !(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) || !(vol_cap_flags_g & H5VL_CAP_FLAG_FLUSH_REFRESH) ||
        ...
        if (MAINPROCESS) {
            SKIPPED();
            printf(
                "    API functions for basic file, dataset, dataset more, attribute, or flush aren't "
    ...
    /* Create the dataset asynchronously */
    if ((dset_id = H5Dcreate_async(file_id, "attr_exists_dset", H5T_NATIVE_INT, space_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT, es_id)) < 0) {
        TEST_ERROR;
    }
    ...
    if ((attr_id = H5Acreate_async(dset_id, "attr", H5T_NATIVE_INT, space_id, H5P_DEFAULT, H5P_DEFAULT, es_id)) < 0) {
        TEST_ERROR;
    }
...
static void test_async_attribute_io(void* params)
{
    ...
    /* Create the dataset asynchronously */
    if ((dset_id = H5Dcreate_async(file_id, "attr_dset", H5T_NATIVE_INT, space_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT, es_id)) < 0) {
        TEST_ERROR;
    }
...
static void test_async_attribute_io_tconv(void* params)
{
    ...
    if (!(vol_cap_flags_g & H5VL_CAP_FLAG_ASYNC) || !(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) || !(vol_cap_flags_g & H5VL_CAP_FLAG_FLUSH_REFRESH) ||
        ...
        if (MAINPROCESS) {
            SKIPPED();
            printf(
                "    API functions for basic file, attribute, or flush aren't supported with this "
                "connector\n");
        }
    ...
    if ((attr_id = H5Acreate_by_name_async(file_id, "attr_dset", "attr_tconv", H5T_STD_U16BE, space_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT, es_id)) < 0) {
        TEST_ERROR;
    }
    ...
    /* Open the attribute asynchronously */
    if ((attr_id = H5Aopen_by_name_async(file_id, "attr_dset", "attr_tconv", H5P_DEFAULT, H5P_DEFAULT, es_id)) < 0) {
        TEST_ERROR;
    }
...
static void test_async_attribute_io_compound(void* params)
{
    ...
    /* Create the attribute asynchronously by name */
    if ((attr_id = H5Acreate_by_name_async(file_id, "attr_dset", "attr_cmpd", ftype_id, space_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT, es_id)) < 0) {
        TEST_ERROR;
    }
...
static void test_async_group(void* params)
{
    ...
    if (!(vol_cap_flags_g & H5VL_CAP_FLAG_ASYNC) || !(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) || !(vol_cap_flags_g & H5VL_CAP_FLAG_GROUP_BASIC) ||
        ...
        if (MAINPROCESS) {
            SKIPPED();
            printf(
                "    API functions for basic file, group, group more or flush aren't "
                "supported with this connector\n");
        }
    ...
    if ((parent_group_id = H5Gcreate_async(file_id, "group_parent", H5P_DEFAULT, gcpl_id, H5P_DEFAULT, es_id)) < 0) {
        TEST_ERROR;
    }

    /* Create 3 subgroups asynchronously, the first with no sub-subgroups, the
     * second with 1, and the third with 2 */
    if ((group_id = H5Gcreate_async(parent_group_id, "group1", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT, es_id)) < 0) {
        TEST_ERROR;
    }
    ...
    if ((group_id = H5Gcreate_async(parent_group_id, "group2", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT, es_id)) < 0) {
        TEST_ERROR;
    }
    if ((subgroup_id = H5Gcreate_async(group_id, "subgroup1", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT, es_id)) < 0) {
        TEST_ERROR;
    }
    ...
    if ((group_id = H5Gcreate_async(parent_group_id, "group3", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT, es_id)) < 0) {
        TEST_ERROR;
    }
    if ((subgroup_id = H5Gcreate_async(group_id, "subgroup1", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT, es_id)) < 0) {
        TEST_ERROR;
    ...
    if ((subgroup_id = H5Gcreate_async(group_id, "subgroup2", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT, es_id)) < 0) {
        TEST_ERROR;
    }
    if (H5Gclose_async(subgroup_id, es_id) < 0) {
        TEST_ERROR;
    ...
    if (vol_cap_flags_g & H5VL_CAP_FLAG_CREATION_ORDER) {
        ...
        if (H5Gget_info_by_idx_async(parent_group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC, 1, &info2, H5P_DEFAULT, es_id) < 0) {
            TEST_ERROR;
        }
    }
...
static void test_async_link(void* params)
{
    ...
    if (!(vol_cap_flags_g & H5VL_CAP_FLAG_ASYNC) || !(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) || !(vol_cap_flags_g & H5VL_CAP_FLAG_LINK_BASIC) ||
        !(vol_cap_flags_g & H5VL_CAP_FLAG_HARD_LINKS) || !(vol_cap_flags_g & H5VL_CAP_FLAG_SOFT_LINKS) || !(vol_cap_flags_g & H5VL_CAP_FLAG_FLUSH_REFRESH) ||
        !(vol_cap_flags_g & H5VL_CAP_FLAG_CREATION_ORDER)) {
        if (MAINPROCESS) {
            SKIPPED();
            printf(
                "    API functions for basic file, link, hard link, soft link, flush, or creation order "
    ...
    /* Create the parent group asynchronously */
    if ((parent_group_id = H5Gcreate_async(file_id, "link_parent", H5P_DEFAULT, gcpl_id, H5P_DEFAULT, es_id)) < 0) {
        TEST_ERROR;
    }

    /* Create subgroup asynchronously. */
    if ((group_id = H5Gcreate_async(parent_group_id, "group", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT, es_id)) < 0) {
        TEST_ERROR;
    }
    if (H5Gclose_async(group_id, es_id) < 0) {
        TEST_ERROR;
    ...
    if (H5Lcreate_hard_async(parent_group_id, "group", parent_group_id, "hard_link", H5P_DEFAULT, H5P_DEFAULT, es_id) < 0) {
        TEST_ERROR;
    }
    ...
    if (H5Lcreate_soft_async("/link_parent/group", parent_group_id, "soft_link", H5P_DEFAULT, H5P_DEFAULT, es_id) < 0) {
        TEST_ERROR;
    }
    ...
    if (H5Ldelete_by_idx_async(parent_group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC, 2, H5P_DEFAULT, es_id) < 0) {
        TEST_ERROR;
    }
...
static void test_async_ocopy_orefresh(void* params)
{
    ...
    /* Make sure the connector supports the API functions being tested */
    if (!(vol_cap_flags_g & H5VL_CAP_FLAG_ASYNC) || !(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) || !(vol_cap_flags_g & H5VL_CAP_FLAG_GROUP_BASIC) ||
        !(vol_cap_flags_g & H5VL_CAP_FLAG_DATASET_BASIC) || !(vol_cap_flags_g & H5VL_CAP_FLAG_OBJECT_MORE) || !(vol_cap_flags_g & H5VL_CAP_FLAG_FLUSH_REFRESH)) {
        if (MAINPROCESS) {
            SKIPPED();
            printf(
                "    API functions for basic file, group, dataset, object more, flush, or refresh "
                "aren't supported with this connector\n");
        }
    ...
    /* Create the parent group asynchronously */
    if ((parent_group_id = H5Gcreate_async(file_id, "ocopy_parent", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT, es_id)) < 0) {
        TEST_ERROR;
    }

    /* Create dataset asynchronously. */
    if ((dset_id = H5Dcreate_async(parent_group_id, "dset", H5T_NATIVE_INT, space_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT, es_id)) < 0) {
        TEST_ERROR;
    }
    if (H5Dclose_async(dset_id, es_id) < 0) {
        TEST_ERROR;
    ...
    /* Copy dataset */
    if (H5Ocopy_async(parent_group_id, "dset", parent_group_id, "copied_dset", H5P_DEFAULT, H5P_DEFAULT, es_id) < 0) {
        TEST_ERROR;
    }
 ...
 */
static void test_async_file_cleanup(void H5_ATTR_UNUSED* params)
{
    char file_name[64];
    int i;
...
void H5_api_async_test_parallel_add(void)
{
    ...
    AddTest("print_async_test_header (coll)", print_async_test_header, NULL, NULL, &coll_metadata_read, sizeof(coll_metadata_read), "Prints header for async tests");

    /* Add tests using collective metadata reads */
    AddTest("test_async_one_dataset_io (coll)",
            test_async_one_dataset_io,
            NULL,
            NULL,
            &coll_metadata_read,
            sizeof(coll_metadata_read),
            "async single dataset I/O (collective metadata reads)");
    AddTest("test_async_multi_dataset_io (coll)",
            test_async_multi_dataset_io,
            NULL,
            NULL,
            &coll_metadata_read,
            sizeof(coll_metadata_read),
            "async multi dataset I/O (collective metadata reads)");
    AddTest("test_async_multi_file_dataset_io (coll)",
            test_async_multi_file_dataset_io,
            NULL,
            NULL,
            &coll_metadata_read,
            sizeof(coll_metadata_read),
            "async multi file dataset I/O (collective metadata reads)");
    AddTest("test_async_multi_file_grp_dset_io (coll)",
            test_async_multi_file_grp_dset_io,
            NULL,
            NULL,
            &coll_metadata_read,
            sizeof(coll_metadata_read),
            "async multi file dataset I/O with groups (collective metadata reads)");
    AddTest("test_async_set_extent (coll)",
            test_async_set_extent,
            NULL,
            NULL,
            &coll_metadata_read,
            sizeof(coll_metadata_read),
            "async extending dataset (collective metadata reads)");
    AddTest("test_async_attribute_exists (coll)",
            test_async_attribute_exists,
            NULL,
            NULL,
            &coll_metadata_read,
            sizeof(coll_metadata_read),
            "async H5Aexists() (collective metadata reads)");
    AddTest("test_async_attribute_io (coll)",
            test_async_attribute_io,
            NULL,
            NULL,
            &coll_metadata_read,
            sizeof(coll_metadata_read),
            "async attribute I/O (collective metadata reads)");
    AddTest("test_async_attribute_io_tconv (coll)",
            test_async_attribute_io_tconv,
            NULL,
            NULL,
            &coll_metadata_read,
            sizeof(coll_metadata_read),
            "async attribute I/O with type conversion (collective metadata reads)");
    AddTest("test_async_attribute_io_compound (coll)",
            test_async_attribute_io_compound,
            NULL,
            NULL,
            &coll_metadata_read,
            sizeof(coll_metadata_read),
            "async attribute I/O with compound type conversion (collective metadata reads)");
    AddTest("test_async_group (coll)",
            test_async_group,
            NULL,
            NULL,
            &coll_metadata_read,
            sizeof(coll_metadata_read),
            "async group operations (collective metadata reads)");
    AddTest("test_async_link (coll)",
            test_async_link,
            NULL,
            NULL,
            &coll_metadata_read,
            sizeof(coll_metadata_read),
            "async link operations (collective metadata reads)");
    AddTest("test_async_ocopy_orefresh (coll)",
            test_async_ocopy_orefresh,
            NULL,
            NULL,
            &coll_metadata_read,
            sizeof(coll_metadata_read),
            "async H5Ocopy() and H5Orefresh() (collective metadata reads)");
    AddTest("test_async_file_reopen (coll)",
            test_async_file_reopen,
            NULL,
            NULL,
            &coll_metadata_read,
            sizeof(coll_metadata_read),
            "async H5Freopen() (collective metadata reads)");

    /* Add a fake test to cleanup test files due to current test interdependencies */
    AddTest("test_async_file_cleanup (coll)", test_async_file_cleanup, NULL, NULL, NULL, 0, "cleanup async test files");
    ...
    AddTest("print_async_test_header (ind)", print_async_test_header, NULL, NULL, &coll_metadata_read, sizeof(coll_metadata_read), "Prints header for async tests");

    /* Add tests using independent metadata reads */
    AddTest("test_async_one_dataset_io (ind)",
            test_async_one_dataset_io,
            NULL,
            NULL,
            &coll_metadata_read,
            sizeof(coll_metadata_read),
            "async single dataset I/O (independent metadata reads)");
    AddTest("test_async_multi_dataset_io (ind)",
            test_async_multi_dataset_io,
            NULL,
            NULL,
            &coll_metadata_read,
            sizeof(coll_metadata_read),
            "async multi dataset I/O (independent metadata reads)");
    AddTest("test_async_multi_file_dataset_io (ind)",
            test_async_multi_file_dataset_io,
            NULL,
            NULL,
            &coll_metadata_read,
    ...
    AddTest("test_async_multi_file_grp_dset_io (ind)",
            test_async_multi_file_grp_dset_io,
            NULL,
            NULL,
            &coll_metadata_read,
    ...
    AddTest("test_async_set_extent (ind)",
            test_async_set_extent,
            NULL,
            NULL,
            &coll_metadata_read,
            sizeof(coll_metadata_read),
            "async extending dataset (independent metadata reads)");
    AddTest("test_async_attribute_exists (ind)",
            test_async_attribute_exists,
            NULL,
            NULL,
            &coll_metadata_read,
            sizeof(coll_metadata_read),
            "async H5Aexists() (independent metadata reads)");
    AddTest("test_async_attribute_io (ind)",
            test_async_attribute_io,
            NULL,
            NULL,
            &coll_metadata_read,
            sizeof(coll_metadata_read),
            "async attribute I/O (independent metadata reads)");
    AddTest("test_async_attribute_io_tconv (ind)",
            test_async_attribute_io_tconv,
            NULL,
            NULL,
            &coll_metadata_read,
    ...
    AddTest("test_async_attribute_io_compound (ind)",
            test_async_attribute_io_compound,
            NULL,
            NULL,
            &coll_metadata_read,
    ...
    AddTest("test_async_group (ind)",
            test_async_group,
            NULL,
            NULL,
            &coll_metadata_read,
            sizeof(coll_metadata_read),
            "async group operations (independent metadata reads)");
    AddTest("test_async_link (ind)",
            test_async_link,
            NULL,
            NULL,
            &coll_metadata_read,
            sizeof(coll_metadata_read),
            "async link operations (independent metadata reads)");
    AddTest("test_async_ocopy_orefresh (ind)",
            test_async_ocopy_orefresh,
            NULL,
            NULL,
            &coll_metadata_read,
            sizeof(coll_metadata_read),
            "async H5Ocopy() and H5Orefresh() (independent metadata reads)");
    AddTest("test_async_file_reopen (ind)",
            test_async_file_reopen,
            NULL,
            NULL,
            &coll_metadata_read,
            sizeof(coll_metadata_read),
            "async H5Freopen() (independent metadata reads)");

    /* Add a fake test to cleanup test files due to current test interdependencies */
    AddTest("test_async_file_cleanup (ind)", test_async_file_cleanup, NULL, NULL, NULL, 0, "cleanup async test files");
}
...
{
    /* Add a fake test to print out a header to distinguish different test interfaces */
    AddTest("print_async_test_header", print_async_test_header, NULL, NULL, NULL, 0, "Prints header for async tests");
}
