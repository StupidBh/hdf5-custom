...
#define MDSET_ALL_FLAGS                                                                                                                                       \
    (MDSET_FLAG_CHUNK | MDSET_FLAG_MLAYOUT | MDSET_FLAG_SHAPESAME | MDSET_FLAG_MDSET | MDSET_FLAG_COLLECTIVE | MDSET_FLAG_COLLECTIVE_OPT | MDSET_FLAG_TCONV | \
     MDSET_FLAG_FILTER)
 ...
 *-------------------------------------------------------------------------
 */
static void test_pmdset(size_t niter, unsigned flags)
{
    ...
    for (i = 0; i < niter; i++) {
        ...
        ndsets = (flags & MDSET_FLAG_MLAYOUT) ? 3 : (flags & MDSET_FLAG_MDSET) ? (size_t)((size_t)h5_local_rand() % max_dsets) + 1 : 1;
        ...
        /* Create datasets */
        for (j = 0; j < ndsets; j++) {
            bool use_chunk = (flags & MDSET_FLAG_CHUNK) || ((flags & MDSET_FLAG_MLAYOUT) && (j == 1 || j == 2));
            ...
            if ((dset_ids[j] = H5Dcreate2(file_id,
                                          dset_name[j],
                                          (flags & MDSET_FLAG_TCONV && h5_local_rand() % 2) ? H5T_NATIVE_LONG : H5T_NATIVE_UINT,
                                          file_space_ids[j],
                                          H5P_DEFAULT,
                                          dcpl_id[j],
                                          H5P_DEFAULT)) < 0) {
        ...
        for (j = 0; j < OPS_PER_FILE; j++) {
                    ...
                    if (h5_local_rand() % 2) {
                        ...
                        for (m = 0; m < nhs; m++) {
                            ...
                            for (n = 0; overlap && (n < MAX_SEL_RETRIES); n++) {
                                ...
                                start[m][0] = (count[m][0] == dset_dims[k][0]) ? 0 : (hsize_t)h5_local_rand() % (dset_dims[k][0] - count[m][0] + 1);
                                start[m][1] = (count[m][1] == dset_dims[k][1]) ? 0 : (hsize_t)h5_local_rand() % (dset_dims[k][1] - count[m][1] + 1);

                                /* If writing, check for overlap with other processes */
                                overlap = false;
                                if (!do_read) {
                                    for (o = start[m][0]; (o < (start[m][0] + count[m][0])) && !overlap; o++) {
                                        for (p = start[m][1]; (p < (start[m][1] + count[m][1])) && !overlap; p++) {
                            ...
                            if (l == (size_t)mpi_rank) {
                                if (H5Sselect_hyperslab(mem_space_ids[k], H5S_SELECT_OR, start[m], NULL, count[m], NULL) < 0) {
                                    T_PMD_ERROR;
                                }
                                if (H5Sselect_hyperslab(file_space_ids[k], H5S_SELECT_OR, start[m], NULL, count[m], NULL) < 0) {
                                    T_PMD_ERROR;
                                }
                            } /* end if */
                        ...
                        /* Point selection */
                        size_t npoints = (size_t)(((size_t)h5_local_rand() % MAX_POINTS) + 1); /* Number of points */
            ...
            if (do_read) {
                if (flags & MDSET_FLAG_MDSET) {
                    ...
                    /* Read */
                    if (H5Dread(dset_ids[0], mem_type_ids[0], mem_space_ids[0], file_space_ids[0], dxpl_id, rbuf) < 0) {
                        T_PMD_ERROR;
                    }
                ...
                if (flags & MDSET_FLAG_MDSET) {
                    ...
                    /* Write */
                    if (H5Dwrite(dset_ids[0], mem_type_ids[0], mem_space_ids[0], file_space_ids[0], dxpl_id, wbuf) < 0) {
                        T_PMD_ERROR;
                    }
...
int main(int argc, char* argv[])
{
    ...
    for (i = 0; i <= MDSET_ALL_FLAGS; i++) {
        ...
        if (((i & MDSET_FLAG_MLAYOUT) && (i & MDSET_FLAG_CHUNK)) || ((i & MDSET_FLAG_MLAYOUT) && !(i & MDSET_FLAG_MDSET)) ||
            ((i & MDSET_FLAG_MLAYOUT) && !(i & MDSET_FLAG_COLLECTIVE)) || ((i & MDSET_FLAG_MLAYOUT) && (i & MDSET_FLAG_TCONV)) ||
            ((i & MDSET_FLAG_FILTER) && !(i & MDSET_FLAG_CHUNK)) || ((i & MDSET_FLAG_FILTER) && !(i & MDSET_FLAG_COLLECTIVE)) ||
            ((i & MDSET_FLAG_FILTER) && (i & MDSET_FLAG_TCONV)) || (!(i & MDSET_FLAG_COLLECTIVE_OPT) && !(i & MDSET_FLAG_COLLECTIVE))) {
            continue;
        }
