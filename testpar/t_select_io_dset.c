 ...
 */
static void set_dxpl(hid_t dxpl, H5D_selection_io_mode_t select_io_mode, H5FD_mpio_xfer_t mpio_type, H5FD_mpio_collective_opt_t mpio_coll_opt, unsigned mwbuf)
{
 ...
 */
static void check_io_mode(hid_t dxpl, unsigned chunked)
{
...
static void testing_check_io_mode(hid_t dxpl, H5D_mpio_actual_io_mode_t exp_io_mode)
{
    ...
    if (actual_io_mode != exp_io_mode) {
        nerrors++;
        if (MAINPROCESS) {
            printf("\n     Failed: Incorrect I/O mode (expected/actual) %u:%u", (unsigned)exp_io_mode, (unsigned)actual_io_mode);
        }
    }
 ...
 */
static void check_actual_selection_io_mode(hid_t dxpl, uint32_t sel_io_mode_expected)
{
    ...
    if (actual_sel_io_mode != sel_io_mode_expected) {
        if (MAINPROCESS) {
            printf("\n     Failed: Incorrect selection I/O mode (expected/actual) %u:%u", (unsigned)sel_io_mode_expected, (unsigned)actual_sel_io_mode);
        }
        P_TEST_ERROR;
    }
}
 ...
 * Helper routine to check actual selection I/O mode on a dxpl
 */
static void check_actual_selection_io_mode_either(hid_t dxpl, uint32_t sel_io_mode_expected1, uint32_t sel_io_mode_expected2)
{
    uint32_t actual_sel_io_mode;
    ...
    if (actual_sel_io_mode != sel_io_mode_expected1 && actual_sel_io_mode != sel_io_mode_expected2) {
        if (MAINPROCESS) {
            printf("\n     Failed: Incorrect selection I/O mode (expected/actual) %u or %u : %u",
                   (unsigned)sel_io_mode_expected1,
                   (unsigned)sel_io_mode_expected2,
 ...
 */
static void test_no_type_conv(hid_t fid, unsigned chunked, unsigned dtrans, unsigned select, unsigned mwbuf)
{
    int i;
    ...
    snprintf(dset_name,
             sizeof(dset_name),
             "no_tconv_%s_%s_%s_%s",
             chunked ? "chunked" : "contig",
             dtrans ? "xform" : "noxform",
             select ? "sel" : "nosel",
             mwbuf ? "mwbuf" : "nomwbuf");
 ...
 */
static void test_no_size_change_no_bkg(hid_t fid, unsigned chunked, unsigned select, unsigned mwbuf)
{
    int i;
    hid_t did = H5I_INVALID_HID;
    hid_t sid = H5I_INVALID_HID;
    ...
    /* Generate dataset name */
    snprintf(dset_name, sizeof(dset_name), "no_size_change_%s_%s_%s", chunked ? "chunked" : "contig", select ? "sel" : "nosel", mwbuf ? "mwbuf" : "nomwbuf");
    ...
    /* Verify data read */
    for (i = 0; i < (int)block[0]; i++) {
        if (rbuf[4 * i + 0] != wbuf[4 * i + 0] || rbuf[4 * i + 1] != wbuf[4 * i + 1] || rbuf[4 * i + 2] != wbuf[4 * i + 2] || rbuf[4 * i + 3] != wbuf[4 * i + 3]) {
            nerrors++;
    ...
    /* Verify data read */
    for (i = 0; i < (int)block[0]; i++) {
        if (rbuf[4 * i + 0] != wbuf[4 * i + 3] || rbuf[4 * i + 1] != wbuf[4 * i + 2] || rbuf[4 * i + 2] != wbuf[4 * i + 1] || rbuf[4 * i + 3] != wbuf[4 * i + 0]) {
            nerrors++;
            printf("\n     Error in data verification at index %d\n", i + (int)start[0]);
            break;
        }
 ...
 */
static void test_larger_mem_type_no_bkg(hid_t fid, unsigned chunked, unsigned dtrans, unsigned select, unsigned mwbuf)
{
    int i;
    hid_t did = H5I_INVALID_HID;
    hid_t sid = H5I_INVALID_HID;
    ...
    for (i = 0; i < (int)block[0]; i++) {
        if (rbuf[i] != (long long)(dtrans ? trans_wbuf[i] : wbuf[i])) {
            nerrors++;
            printf("\n     Error in first data verification:\n");
            printf("     At index %d: %lld, %lld\n", i + (int)start[0], (long long)(dtrans ? trans_wbuf[i] : wbuf[i]), rbuf[i]);
            break;
        }
    }
    ...
    if (dtrans) {
        ...
        for (i = 0; i < (int)block[0]; i++) {
            if (rbuf[i] != (long long)(100 - trans_wbuf[i])) {
                nerrors++;
                printf("\n     Error in second data verification:.\n");
                printf("     At index %d: %lld, %lld\n", i + (int)start[0], (long long)(100 - trans_wbuf[i]), rbuf[i]);
                break;
            }
        }
    }
 ...
 */
static void test_smaller_mem_type_no_bkg(hid_t fid, unsigned chunked, unsigned dtrans, unsigned select, unsigned mwbuf)
{
    int i;
    hid_t did = H5I_INVALID_HID;
    hid_t sid = H5I_INVALID_HID;
 ...
 */
static void test_cmpd_with_bkg(hid_t fid, unsigned chunked, unsigned select, unsigned mwbuf)
{
    int i;
    hid_t did = H5I_INVALID_HID;
    hid_t sid = H5I_INVALID_HID;
    ...
    if (H5Tinsert(s1_tid, "a", HOFFSET(s1_t, a), H5T_NATIVE_INT) < 0 || H5Tinsert(s1_tid, "b", HOFFSET(s1_t, b), H5T_NATIVE_INT) < 0 ||
        H5Tinsert(s1_tid, "c", HOFFSET(s1_t, c), H5T_NATIVE_INT) < 0 || H5Tinsert(s1_tid, "d", HOFFSET(s1_t, d), H5T_NATIVE_INT) < 0) {
        P_TEST_ERROR;
    }
    ...
    /* Generate dataset name */
    snprintf(dset_name, sizeof(dset_name), "cmpd_with_bkg_%s_%s_%s", chunked ? "chunked" : "contig", select ? "sel" : "nosel", mwbuf ? "mwbuf" : "nomwbuf");
    ...
    /* Verify data read */
    for (i = 0; i < (int)block[0]; i++) {
        if (s1_wbuf[i].a != s1_rbuf[i].a || s1_wbuf[i].b != s1_rbuf[i].b || s1_wbuf[i].c != s1_rbuf[i].c || s1_wbuf[i].d != s1_rbuf[i].d) {
            nerrors++;
    ...
    if (H5Tinsert(ss_ac_tid, "a", HOFFSET(s1_t, a), H5T_NATIVE_INT) < 0 || H5Tinsert(ss_ac_tid, "c", HOFFSET(s1_t, c), H5T_NATIVE_INT) < 0) {
        P_TEST_ERROR;
    }
    ...
    for (i = 0; i < (int)block[0]; i++) {
        if (s1_rbuf[i].a != s1_wbuf[i].a || s1_rbuf[i].b != (4 * (i + (int)start[0]) + 1) || s1_rbuf[i].c != s1_wbuf[i].c ||
            s1_rbuf[i].d != (4 * (i + (int)start[0]) + 3)) {
            nerrors++;
    ...
    /* but contains only subset members of s1_t */
    if (H5Tinsert(ss_bc_tid, "b", HOFFSET(s1_t, b), H5T_NATIVE_INT) < 0 || H5Tinsert(ss_bc_tid, "c", HOFFSET(s1_t, c), H5T_NATIVE_INT) < 0) {
        P_TEST_ERROR;
    }
    ...
    /* Verify data read */
    for (i = 0; i < (int)block[0]; i++) {
        if (s1_rbuf[i].a != ((4 * (i + (int)start[0])) + (2 * DSET_SELECT_DIM)) || s1_rbuf[i].b != (4 * (i + (int)start[0]) + 1) ||
            s1_rbuf[i].c != (4 * (i + (int)start[0]) + DSET_SELECT_DIM + 2) || s1_rbuf[i].d != ((4 * (i + (int)start[0])) + (2 * DSET_SELECT_DIM) + 3)) {
            nerrors++;
            printf("\n     Error in 3rd data verification:\n");
            printf("     At index %d: %d/%d, %d/%d, %d/%d, %d/%d\n",
                   i + (int)start[0],
                   ((4 * (i + (int)start[0])) + (2 * DSET_SELECT_DIM)),
                   s1_rbuf[i].a,
                   (4 * (i + (int)start[0]) + 1),
                   s1_rbuf[i].b,
                   (4 * (i + (int)start[0]) + DSET_SELECT_DIM + 2),
                   s1_rbuf[i].c,
                   ((4 * (i + (int)start[0])) + (2 * DSET_SELECT_DIM) + 3),
                   s1_rbuf[i].d);
            break;
        }
    }
    ...
    if (H5Tinsert(s2_tid, "a", HOFFSET(s2_t, a), H5T_NATIVE_INT) < 0 || H5Tinsert(s2_tid, "b", HOFFSET(s2_t, b), H5T_NATIVE_LLONG) < 0 ||
        H5Tinsert(s2_tid, "c", HOFFSET(s2_t, c), H5T_NATIVE_INT) < 0 || H5Tinsert(s2_tid, "d", HOFFSET(s2_t, d), H5T_NATIVE_SHORT) < 0) {
        P_TEST_ERROR;
    }
    ...
    /* Verify data read */
    for (i = 0; i < (int)block[0]; i++) {
        if (s2_wbuf[i].a != s2_rbuf[i].a || s2_wbuf[i].b != s2_rbuf[i].b || s2_wbuf[i].c != s2_rbuf[i].c || s2_wbuf[i].d != s2_rbuf[i].d) {
            nerrors++;
 ...
 */
static void test_type_conv_sel_empty(hid_t fid, unsigned chunked, unsigned dtrans, unsigned select, unsigned mwbuf)
{
    int i;
    hid_t did = H5I_INVALID_HID;
    hid_t sid = H5I_INVALID_HID;
    ...
    if (MAINPROCESS) {
        for (i = 0; i < (int)block[0]; i++) {
            if (lrbuf[i] != (dtrans ? trans_lwbuf[i] : lwbuf[i])) {
                nerrors++;
                printf("\n     Error in first data verification:\n");
                printf("     At index %d: %ld, %ld\n", i + (int)start[0], dtrans ? trans_lwbuf[i] : lwbuf[i], lrbuf[i]);
                break;
            }
        }
    }
    ...
    if (mpi_rank) {
        for (i = 0; i < (int)block[0]; i++) {
            if (srbuf[i] != (short)(dtrans ? trans_lwbuf[i] : lwbuf[i])) {
                printf("\n     Error in second data verification:\n");
                printf("     At index %d: %d, %d\n", i + (int)start[0], (short)(dtrans ? trans_lwbuf[i] : lwbuf[i]), srbuf[i]);
                break;
            }
        }
    }
    ...
    if (mpi_rank) {
        for (i = 0; i < (int)block[0]; i++) {
            if (llrbuf[i] != (long long)(dtrans ? trans_swbuf[i] : swbuf[i])) {
                ...
                printf("     At index %d: %lld, %lld\n", i + (int)start[0], (long long)(dtrans ? trans_swbuf[i] : swbuf[i]), llrbuf[i]);
                break;
            }
        }
    }
 ...
 */
static void test_multi_dsets_no_bkg(hid_t fid, unsigned chunked, unsigned dtrans, unsigned select, unsigned mwbuf)
{
    size_t ndsets;
    int i, j;
    hid_t dcpl = H5I_INVALID_HID;
    ...
    for (i = 0; i < (int)ndsets; i++) {
        ...
        /* Generate dataset name */
        snprintf(dset_names[i],
                 sizeof(dset_names[i]),
                 "multi_dset%d_%s_%s_%s_%s",
                 i,
                 chunked ? "chunked" : "contig",
                 dtrans ? "xform" : "noxform",
                 select ? "select" : "noselect",
                 mwbuf ? "mwbuf" : "nomwbuf");
        ...
        /* Create ith dataset */
        if ((dset_dids[i] = H5Dcreate2(fid, dset_names[i], (tconv ? H5T_NATIVE_LONG : H5T_NATIVE_INT), file_sids[i], H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0) {
            P_TEST_ERROR;
        }
    }
    ...
    for (i = 0; i < (int)ndsets; i++) {
        for (j = 0; j < (int)block[0]; j++) {
            if (rbufi[i][j] != (dtrans ? trans_wbufi[i][j] : wbufi[i][j])) {
                ...
                printf("     At index %d: %d, %d\n", j + (int)start[0], dtrans ? trans_wbufi[i][j] : wbufi[i][j], rbufi[i][j]);
                break;
            }
        }
    }
        ...
        for (i = 0; i < (int)ndsets; i++) {
            for (j = 0; j < (int)block[0]; j++) {
                if (rbufi[i][j] != (2 * trans_wbufi[i][j])) {
                    ...
                    printf("     At index %d: %d, %d\n", j + (int)start[0], 2 * trans_wbufi[i][j], rbufi[i][j]);
                    break;
                }
            }
        }
    ...
    for (i = 0; i < (int)ndsets; i++) {
        for (j = 0; j < (int)block[0]; j++) {
            if (lrbufi[i][j] != (dtrans ? (2 * trans_lwbufi[i][j]) : lwbufi[i][j])) {
                ...
                printf("     At index %d: %ld/%ld\n", j + (int)start[0], (dtrans ? (2 * trans_lwbufi[i][j]) : lwbufi[i][j]), lrbufi[i][j]);
                break;
            }
        }
    }
 ...
 *      --Verify values read
 */
static void test_multi_dsets_cmpd_with_bkg(hid_t fid, unsigned chunked, unsigned select, unsigned mwbuf)
{
    size_t ndsets;
    int i, j, mm;
    hid_t dcpl = H5I_INVALID_HID;
    ...
    for (i = 0; i < (int)ndsets; i++) {
        ...
        /* Create ith dataset */
        if ((dset_dids[i] = H5Dcreate2(fid, dset_names[i], s1_tid, file_sids[i], H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0) {
            P_TEST_ERROR;
        }
    ...
    for (i = 0; i < (int)ndsets; i++) {
        for (j = 0; j < (int)block[0]; j++) {
            if (wbufi[i][j].a != rbufi[i][j].a || wbufi[i][j].b != rbufi[i][j].b || wbufi[i][j].c != rbufi[i][j].c || wbufi[i][j].d != rbufi[i][j].d) {
                nerrors++;
                printf("\n     Error in 1st data verification for dset %d:\n", i);
                printf("     At index %d: %d/%d, %d/%d, %d/%d, %d/%d\n",
                       j + (int)start[0],
    ...
    for (i = 0; i < (int)ndsets; i++) {
        if (i == 0) { /* dset0 */
            for (j = 0; j < (int)block[0]; j++) {
                if (wbufi[i][j].a != rbufi[i][j].a || (4 * (j + (int)start[0]) + 1) != rbufi[i][j].b || wbufi[i][j].c != rbufi[i][j].c ||
            ...
            for (j = 0; j < (int)block[0]; j++) {
                if ((4 * (j + (int)start[0])) != rbufi[i][j].a || (4 * (j + (int)start[0]) + 1) != rbufi[i][j].b || (4 * (j + (int)start[0]) + 2) != rbufi[i][j].c ||
                    (4 * (j + (int)start[0]) + 3) != rbufi[i][j].d) {
                    nerrors++;
                    printf("\n     Error in 2nd data verification for dset %d:\n", i);
                    printf("     At index %d: %d/%d, %d/%d, %d/%d, %d/%d\n",
    ...
    /* dset0 */
    for (j = 0; j < (int)block[0]; j++) {
        if (wbufi[0][j].a != rbufi[0][j].a || ((4 * (j + (int)start[0])) + 1) != rbufi[0][j].b || wbufi[0][j].c != rbufi[0][j].c ||
    ...
    /* <mm> dset */
    for (j = 0; j < (int)block[0]; j++) {
        if (rbufi[mm][j].a != ((4 * (j + (int)start[0])) + (2 * DSET_SELECT_DIM)) || rbufi[mm][j].b != ((4 * (j + (int)start[0])) + 1) ||
            rbufi[mm][j].c != ((4 * (j + (int)start[0])) + 2) || rbufi[mm][j].d != ((4 * (j + (int)start[0])) + (2 * DSET_SELECT_DIM) + 3)) {
            nerrors++;
            printf("\n     Error in 3rd data verification for dset %d:\n", mm);
            printf("     At index %d: %d/%d, %d/%d, %d/%d, %d/%d\n",
                   j + (int)start[0],
                   ((4 * (j + (int)start[0])) + (2 * DSET_SELECT_DIM)),
                   rbufi[mm][j].a,
                   ((4 * (j + (int)start[0])) + 1),
                   rbufi[mm][j].b,
                   ((4 * (j + (int)start[0])) + 2),
                   rbufi[mm][j].c,
                   ((4 * (j + (int)start[0])) + (2 * DSET_SELECT_DIM) + 3),
                   rbufi[mm][j].d);
            break;
        }
    }
    ...
    for (i = 0; i < (int)ndsets; i++) {
        ...
        for (j = 0; j < (int)block[0]; j++) {
            if (rbufi[i][j].a != ((4 * (j + (int)start[0]))) || rbufi[i][j].b != ((4 * (j + (int)start[0])) + 1) ||
                rbufi[i][j].c != ((4 * (j + (int)start[0])) + 2) || rbufi[i][j].d != ((4 * (j + (int)start[0])) + 3)) {
                nerrors++;
                printf("\n     Error in 3rd data verification for dset %d:\n", i);
                printf("     At index %d: %d/%d, %d/%d, %d/%d, %d/%d\n",
                       j + (int)start[0],
    ...
    for (i = 0; i < (int)ndsets; i++) {
        for (j = 0; j < (int)block[0]; j++) {
            if (s2_rbufi[i][j].a != s2_wbufi[i][j].a || s2_rbufi[i][j].b != s2_wbufi[i][j].b || s2_rbufi[i][j].c != s2_wbufi[i][j].c ||
 ...
 */
static void test_multi_dsets_size_change_no_bkg(hid_t fid, unsigned chunked, unsigned select, unsigned mwbuf)
{
    size_t ndsets;
    int i, j;
    hid_t dcpl = H5I_INVALID_HID;
    ...
    for (i = 0; i < (int)ndsets; i++) {
        ...
        /* Create ith dataset */
        if ((dset_dids[i] = H5Dcreate2(fid, dset_names[i], H5T_STD_I32BE, file_sids[i], H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0) {
            P_TEST_ERROR;
        }
    }
    ...
    for (i = 0; i < (int)ndsets; i++) {
        ...
        if (H5Tget_size(H5Dget_type(dset_dids[i])) >= ss) {
            for (j = 0; j < (int)block[0]; j++) {
                if (rbufi[i][(int)ss * j + 0] != wbufi[i][(int)ss * j + 0] || rbufi[i][(int)ss * j + 1] != wbufi[i][(int)ss * j + 1] ||
                    rbufi[i][(int)ss * j + 2] != wbufi[i][(int)ss * j + 2] || rbufi[i][(int)ss * j + 3] != wbufi[i][(int)ss * j + 3]) {
                    H5_FAILED();
    ...
    for (i = 0; i < (int)ndsets; i++) {
        ...
        if (H5Tget_size(H5Dget_type(dset_dids[i])) >= ss) {
            for (j = 0; j < (int)block[0]; j++) {
                if (lrbufi[i][(int)ss * j + 0] != lwbufi[i][(int)ss * j + 0] || lrbufi[i][(int)ss * j + 1] != lwbufi[i][(int)ss * j + 1] ||
                    lrbufi[i][(int)ss * j + 2] != lwbufi[i][(int)ss * j + 2] || lrbufi[i][(int)ss * j + 3] != lwbufi[i][(int)ss * j + 3] ||
                    lrbufi[i][(int)ss * j + 4] != lwbufi[i][(int)ss * j + 4] || lrbufi[i][(int)ss * j + 5] != lwbufi[i][(int)ss * j + 5] ||
                    lrbufi[i][(int)ss * j + 6] != lwbufi[i][(int)ss * j + 6] || lrbufi[i][(int)ss * j + 7] != lwbufi[i][(int)ss * j + 7]) {
                    H5_FAILED();
                    printf("    Read different values than written.\n");
                    printf("    For dset %d at index %d\n", i, j + (int)start[0]);
                    P_TEST_ERROR;
    ...
    for (i = 0; i < (int)ndsets; i++) {
        ...
        for (j = 0; j < (int)block[0]; j++) {
            if (srbufi[i][(int)ss * j + 0] != swbufi[i][(int)ss * j + 0] || srbufi[i][(int)ss * j + 1] != swbufi[i][(int)ss * j + 1]) {
                H5_FAILED();
                printf("    Read different values than written.\n");
                printf("    For dset %d at index %d\n", i, j + (int)start[0]);
                P_TEST_ERROR;
 ...
 */
static void test_multi_dsets_conv_sel_empty(hid_t fid, unsigned chunked, unsigned dtrans, unsigned select, unsigned mwbuf)
{
    size_t ndsets;
    int i, j;
    hid_t dcpl = H5I_INVALID_HID;
    ...
    for (i = 0; i < (int)ndsets; i++) {
        ...
        if (i == 0) {
            if ((dset_dids[i] = H5Dcreate2(fid, dset_names[i], H5T_NATIVE_INT, file_sids[i], H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0) {
                P_TEST_ERROR;
            }
        }
        else {
            if ((dset_dids[i] =
                     H5Dcreate2(fid, dset_names[i], ((h5_local_rand() % 2) ? H5T_NATIVE_LLONG : H5T_NATIVE_SHORT), file_sids[i], H5P_DEFAULT, dcpl, H5P_DEFAULT)) <
                0) {
                P_TEST_ERROR;
            }
        }
    ...
    if (MAINPROCESS) {
        ...
        for (j = 0; j < save_block0; j++) {
            if (l_rbufi[0][j] != (dtrans ? (long)trans_wbufi[0][j] : (long)wbufi[0][j])) {
                nerrors++;
                printf("     Verify dset0 at index %d: %ld, %ld\n", j + (int)start[0], dtrans ? (long)trans_wbufi[0][j] : (long)wbufi[0][j], l_rbufi[0][j]);
                break;
            }
        }
    }
    ...
    if (mpi_rank) {
        ...
        for (j = 0; j < (int)block[0]; j++) {
            if (l_rbufi[mm][j] != (long)(dtrans ? trans_wbufi[mm][j] : wbufi[mm][j])) {
                nerrors++;
                printf("     Verify dset %d at index %d: %ld, %ld\n", mm, j + (int)start[0], (long)(dtrans ? trans_wbufi[mm][j] : wbufi[mm][j]), l_rbufi[mm][j]);
                break;
            }
        }
        ...
        for (j = 0; j < (int)dims[0]; j++) {
            if (l_rbufi[ll][j] != (long)(dtrans ? trans_wbufi[ll][j] : wbufi[ll][j])) {
                nerrors++;
                printf("     Verify dset %d at index %d: %ld, %ld\n", ll, j, (long)(dtrans ? trans_wbufi[ll][j] : wbufi[ll][j]), l_rbufi[ll][j]);
                break;
            }
        }
    }
 ...
 */
static void test_multi_dsets_all(int niter, hid_t fid, unsigned chunked, unsigned select, unsigned mwbuf)
{
    size_t ndsets;
    int i, j, mm;
    int s, n;
    ...
    for (n = 0; n < niter; n++) {
        ...
        if (H5Tinsert(s3_tid, "a", HOFFSET(s3_t, a), H5T_NATIVE_INT) < 0 || H5Tinsert(s3_tid, "b", HOFFSET(s3_t, b), H5T_NATIVE_INT) < 0 ||
            H5Tinsert(s3_tid, "c", HOFFSET(s3_t, c), H5T_NATIVE_INT) < 0 || H5Tinsert(s3_tid, "d", HOFFSET(s3_t, d), H5T_NATIVE_INT) < 0) {
            P_TEST_ERROR;
        }
        ...
        if (H5Tinsert(s4_tid, "b", HOFFSET(s4_t, b), H5T_NATIVE_UINT) < 0 || H5Tinsert(s4_tid, "d", HOFFSET(s4_t, d), H5T_NATIVE_UINT) < 0) {
            P_TEST_ERROR;
        }
        ...
        for (i = 0; i < (int)ndsets; i++) {
            ...
            if (mm == 0) {
                ...
                snprintf(dset_names[i],
                         sizeof(dset_names[i]),
                         "multi_all_nconv_dset%d_%s_%s_%s",
                         i,
                         chunked ? "chunked" : "contig",
                         select ? "select" : "noselect",
                         mwbuf ? "mwbuf" : "nomwbuf");
                ...
                snprintf(dset_names[i],
                         sizeof(dset_names[i]),
                         "multi_all_conv_nbkg_dset%d_%s_%s_%s",
                         i,
                         chunked ? "chunked" : "contig",
                         select ? "select" : "noselect",
                         mwbuf ? "mwbuf" : "nomwbuf");
                if ((dset_dids[i] = H5Dcreate2(fid, dset_names[i], H5T_NATIVE_LONG, file_sids[i], H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0) {
                    P_TEST_ERROR;
                }
            }
            else {
                ...
                snprintf(dset_names[i],
                         sizeof(dset_names[i]),
                         "multi_all_conv_bkg_dset%d_%s_%s_%s",
                         i,
                         chunked ? "chunked" : "contig",
                         select ? "select" : "noselect",
                         mwbuf ? "mwbuf" : "nomwbuf");
        ...
        for (s = SETTING_A; s <= SETTING_B; s++) {
            ...
            for (i = 0; i < (int)ndsets; i++) {
                ...
                case DSET_WITH_CONV_AND_BKG:
                    if (s == SETTING_A) {
                        for (j = 0; j < (int)block[0]; j++) {
                            if (s3_rbufi3[i][j].a != s1_wbufi3[i][j].a || s3_rbufi3[i][j].b != s1_wbufi3[i][j].b || s3_rbufi3[i][j].c != s1_wbufi3[i][j].c ||
                        ...
                        for (j = 0; j < (int)block[0]; j++) {
                            if (s1_rbufi3[i][j].a != s1_wbufi3[i][j].a || s1_rbufi3[i][j].b != (DSET_SELECT_DIM + j + (int)start[0]) ||
                                s1_rbufi3[i][j].c != s1_wbufi3[i][j].c || s1_rbufi3[i][j].d != (DSET_SELECT_DIM + j + (int)start[0])) {
                                nerrors++;
                                printf("    Read different values than written.\n");
                                printf("    For dset %d at index %d\n", i, j + (int)start[0]);
                                break;
 ...
 *        as needed.
 */
static void test_no_selection_io_cause_mode(const char* filename, hid_t fapl, uint32_t test_mode)
{
    hid_t dcpl = H5I_INVALID_HID;
    hid_t dxpl = H5I_INVALID_HID;
 ...
 */
static void test_get_no_selection_io_cause(const char* filename, hid_t fapl)
{
    if (MAINPROCESS) {
        printf("\n");
 ...
 */
static void test_bug_optimized_bufs(const char* filename, hid_t fapl)
{
    hid_t dxpl = H5I_INVALID_HID;
    hid_t dcpl = H5I_INVALID_HID;
    hid_t fid = H5I_INVALID_HID;
 ...
 */
static void test_bug_base_addr(const char* filename, hid_t fapl)
{
    hid_t dxpl = H5I_INVALID_HID;
    ...
    if ((did = H5Dcreate2(fid, "bug_base_addr", H5T_NATIVE_INT, sid, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        P_TEST_ERROR;
    }
 ...
 *
 */
static void test_bug_api_library(const char* filename, hid_t fapl)
{
    hid_t dxpl = H5I_INVALID_HID;
    hid_t fid = H5I_INVALID_HID;
    hid_t did = H5I_INVALID_HID;
 ...
 */
static void test_bugs_select_on(const char* filename, hid_t fapl)
{
    if (MAINPROCESS) {
        printf("\n");
        ...
        for (dtrans = false; dtrans <= true; dtrans++) {
            for (select = false; select <= true; select++) {
                ...
                for (mwbuf = false; mwbuf <= true; mwbuf++) {
                    ...
                    for (test_select_config = (int)TEST_NO_TYPE_CONV; test_select_config < (int)TEST_SELECT_NTESTS; test_select_config++) {
