...
static void setup_vfd_test_file(int file_name_id,
                                char* file_name,
                                int mpi_size,
                                ...
                                hid_t* fapl_id_ptr,
                                hid_t* dxpl_id_ptr);
static void takedown_vfd_test_file(int mpi_rank, char* filename, H5FD_t** lf_ptr, hid_t* fapl_id_ptr, hid_t* dxpl_id_ptr);

/* test functions */
static unsigned
    vector_read_test_1(int file_name_id, int mpi_rank, int mpi_size, H5FD_mpio_xfer_t xfer_mode, H5FD_mpio_collective_opt_t coll_opt_mode, const char* vfd_name);
static unsigned
    vector_read_test_2(int file_name_id, int mpi_rank, int mpi_size, H5FD_mpio_xfer_t xfer_mode, H5FD_mpio_collective_opt_t coll_opt_mode, const char* vfd_name);
static unsigned
    vector_read_test_3(int file_name_id, int mpi_rank, int mpi_size, H5FD_mpio_xfer_t xfer_mode, H5FD_mpio_collective_opt_t coll_opt_mode, const char* vfd_name);
static unsigned
    vector_read_test_4(int file_name_id, int mpi_rank, int mpi_size, H5FD_mpio_xfer_t xfer_mode, H5FD_mpio_collective_opt_t coll_opt_mode, const char* vfd_name);
static unsigned
    vector_read_test_5(int file_name_id, int mpi_rank, int mpi_size, H5FD_mpio_xfer_t xfer_mode, H5FD_mpio_collective_opt_t coll_opt_mode, const char* vfd_name);

static unsigned
    vector_write_test_1(int file_name_id, int mpi_rank, int mpi_size, H5FD_mpio_xfer_t xfer_mode, H5FD_mpio_collective_opt_t coll_opt_mode, const char* vfd_name);
static unsigned
    vector_write_test_2(int file_name_id, int mpi_rank, int mpi_size, H5FD_mpio_xfer_t xfer_mode, H5FD_mpio_collective_opt_t coll_opt_mode, const char* vfd_name);
static unsigned
    vector_write_test_3(int file_name_id, int mpi_rank, int mpi_size, H5FD_mpio_xfer_t xfer_mode, H5FD_mpio_collective_opt_t coll_opt_mode, const char* vfd_name);
static unsigned
    vector_write_test_4(int file_name_id, int mpi_rank, int mpi_size, H5FD_mpio_xfer_t xfer_mode, H5FD_mpio_collective_opt_t coll_opt_mode, const char* vfd_name);
static unsigned
    vector_write_test_5(int file_name_id, int mpi_rank, int mpi_size, H5FD_mpio_xfer_t xfer_mode, H5FD_mpio_collective_opt_t coll_opt_mode, const char* vfd_name);
static unsigned
    vector_write_test_6(int file_name_id, int mpi_rank, int mpi_size, H5FD_mpio_xfer_t xfer_mode, H5FD_mpio_collective_opt_t coll_opt_mode, const char* vfd_name);
static unsigned
    vector_write_test_7(int file_name_id, int mpi_rank, int mpi_size, H5FD_mpio_xfer_t xfer_mode, H5FD_mpio_collective_opt_t coll_opt_mode, const char* vfd_name);
static unsigned
    vector_write_test_8(int file_name_id, int mpi_rank, int mpi_size, H5FD_mpio_xfer_t xfer_mode, H5FD_mpio_collective_opt_t coll_opt_mode, const char* vfd_name);
/*
...
static herr_t test_selection_io_read_verify(hid_t dxpl,
                                            int mpi_rank,
                                            hsize_t start[],
                                            hsize_t block[],
                                            H5FD_t* lf,
...
static herr_t test_selection_io_write(hid_t dxpl,
                                      H5FD_t* lf,
                                      H5FD_mem_t type,
                                      uint32_t count,
                                      hid_t mem_spaces[],
...
static void test_selection_io_types_1d(int mpi_rank,
                                       int mpi_size,
                                       H5FD_t* lf,
                                       hid_t dxpl,
                                       H5FD_mem_t type,
                                       ...
                                       hid_t file_spaces[],
                                       hsize_t dims1[]);
static void test_selection_io_types_2d(int mpi_rank,
                                       int mpi_size,
                                       H5FD_t* lf,
                                       hid_t dxpl,
                                       H5FD_mem_t type,
                                       ...
                                       hid_t file_spaces[],
                                       hsize_t dims2[]);
static void test_selection_io_types_1d_2d(int mpi_rank,
                                          int mpi_size,
                                          H5FD_t* lf,
                                          hid_t dxpl,
                                          H5FD_mem_t type,
                                          ...
                                          hsize_t dims1[],
                                          hsize_t dims2[]);
static void test_selection_io_types_shorten(int mpi_rank,
                                            int mpi_size,
                                            H5FD_t* lf,
                                            hid_t dxpl,
                                            H5FD_mem_t type,
...
static unsigned alloc_and_init_file_images(int mpi_size)
{
    ...
    if (pass) {
        ...
        if ((!increasing_fi_buf) || (!decreasing_fi_buf) || (!negative_fi_buf) || (!zero_fi_buf) || (!read_fi_buf)) {
            pass = false;
    ...
    vector_read_test_1(int file_name_id, int mpi_rank, int mpi_size, H5FD_mpio_xfer_t xfer_mode, H5FD_mpio_collective_opt_t coll_opt_mode, const char* vfd_name)
{
    ...
    if (mpi_rank == 0) {
        if (xfer_mode == H5FD_MPIO_INDEPENDENT) {
            snprintf(test_title, sizeof(test_title), "parallel vector read test 1 -- %s / independent", vfd_name);
        }
        else if (coll_opt_mode == H5FD_MPIO_INDIVIDUAL_IO) {
            snprintf(test_title, sizeof(test_title), "parallel vector read test 1 -- %s / col op / ind I/O", vfd_name);
        }
        else {
            assert(coll_opt_mode == H5FD_MPIO_COLLECTIVE_IO);

            snprintf(test_title, sizeof(test_title), "parallel vector read test 1 -- %s / col op / col I/O", vfd_name);
        }
    ...
    if (pass) {
        ...
        setup_vfd_test_file(file_name_id, filename, mpi_size, xfer_mode, coll_opt_mode, vfd_name, eoa, &lf, &fapl_id, &dxpl_id);
    }
    ...
    if (pass) {
        ...
        if (mpi_rank == 0) {
            if (H5FDwrite(lf, H5FD_MEM_DRAW, H5P_DEFAULT, (haddr_t)0, image_size, (void*)increasing_fi_buf) < 0) {
                pass = false;
    ...
    vector_read_test_2(int file_name_id, int mpi_rank, int mpi_size, H5FD_mpio_xfer_t xfer_mode, H5FD_mpio_collective_opt_t coll_opt_mode, const char* vfd_name)
{
    ...
    if (mpi_rank == 0) {
        if (xfer_mode == H5FD_MPIO_INDEPENDENT) {
            snprintf(test_title, sizeof(test_title), "parallel vector read test 2 -- %s / independent", vfd_name);
        }
        else if (coll_opt_mode == H5FD_MPIO_INDIVIDUAL_IO) {
            snprintf(test_title, sizeof(test_title), "parallel vector read test 2 -- %s / col op / ind I/O", vfd_name);
        }
        else {
            assert(coll_opt_mode == H5FD_MPIO_COLLECTIVE_IO);

            snprintf(test_title, sizeof(test_title), "parallel vector read test 2 -- %s / col op / col I/O", vfd_name);
        }
    ...
    if (pass) {
        ...
        if (mpi_rank == 0) {
            if (H5FDwrite(lf, H5FD_MEM_DRAW, H5P_DEFAULT, (haddr_t)0, image_size, (void*)decreasing_fi_buf) < 0) {
                pass = false;
                failure_mssg = "H5FDwrite() on rank 0 failed.\n";
            }
        }
    ...
    vector_read_test_3(int file_name_id, int mpi_rank, int mpi_size, H5FD_mpio_xfer_t xfer_mode, H5FD_mpio_collective_opt_t coll_opt_mode, const char* vfd_name)
{
    ...
    if (mpi_rank == 0) {
        if (xfer_mode == H5FD_MPIO_INDEPENDENT) {
            snprintf(test_title, sizeof(test_title), "parallel vector read test 3 -- %s / independent", vfd_name);
        }
        else if (coll_opt_mode == H5FD_MPIO_INDIVIDUAL_IO) {
            snprintf(test_title, sizeof(test_title), "parallel vector read test 3 -- %s / col op / ind I/O", vfd_name);
        }
        else {
            assert(coll_opt_mode == H5FD_MPIO_COLLECTIVE_IO);

            snprintf(test_title, sizeof(test_title), "parallel vector read test 3 -- %s / col op / col I/O", vfd_name);
        }
    ...
    if (pass) {
        ...
        if (mpi_rank == 0) {
            if (H5FDwrite(lf, H5FD_MEM_DRAW, H5P_DEFAULT, (haddr_t)0, image_size, (void*)negative_fi_buf) < 0) {
                pass = false;
                failure_mssg = "H5FDwrite() on rank 0 failed.\n";
            }
        }
    ...
    if (pass) {
        ...
        for (i = 0; ((pass) && (i < INTS_PER_RANK / 4)); i++) {
            if (((mpi_rank % 2 == 0) && (buf_0[i] != negative_fi_buf[base_index + i])) || ((mpi_rank % 2 == 1) && (buf_3[i] != negative_fi_buf[base_index + i]))) {
                pass = false;
        ...
        for (i = 0; ((pass) && (i < INTS_PER_RANK / 4)); i++) {
            if (((mpi_rank % 2 == 0) && (buf_1[i] != negative_fi_buf[base_index + i])) || ((mpi_rank % 2 == 1) && (buf_2[i] != negative_fi_buf[base_index + i]))) {
                pass = false;
        ...
        for (i = 0; ((pass) && (i < INTS_PER_RANK / 4)); i++) {
            if (((mpi_rank % 2 == 0) && (buf_2[i] != negative_fi_buf[base_index + i])) || ((mpi_rank % 2 == 1) && (buf_1[i] != negative_fi_buf[base_index + i]))) {
                pass = false;
        ...
        for (i = 0; ((pass) && (i < INTS_PER_RANK / 4)); i++) {
            if (((mpi_rank % 2 == 0) && (buf_3[i] != negative_fi_buf[base_index + i])) || ((mpi_rank % 2 == 1) && (buf_0[i] != negative_fi_buf[base_index + i]))) {
                pass = false;
    ...
    vector_read_test_4(int file_name_id, int mpi_rank, int mpi_size, H5FD_mpio_xfer_t xfer_mode, H5FD_mpio_collective_opt_t coll_opt_mode, const char* vfd_name)
{
    ...
    if (mpi_rank == 0) {
        if (xfer_mode == H5FD_MPIO_INDEPENDENT) {
            snprintf(test_title, sizeof(test_title), "parallel vector read test 4 -- %s / independent", vfd_name);
        }
        else if (coll_opt_mode == H5FD_MPIO_INDIVIDUAL_IO) {
            snprintf(test_title, sizeof(test_title), "parallel vector read test 4 -- %s / col op / ind I/O", vfd_name);
        }
        else {
            assert(coll_opt_mode == H5FD_MPIO_COLLECTIVE_IO);

            snprintf(test_title, sizeof(test_title), "parallel vector read test 4 -- %s / col op / col I/O", vfd_name);
        }
    ...
    vector_read_test_5(int file_name_id, int mpi_rank, int mpi_size, H5FD_mpio_xfer_t xfer_mode, H5FD_mpio_collective_opt_t coll_opt_mode, const char* vfd_name)
{
    ...
    if (mpi_rank == 0) {
        if (xfer_mode == H5FD_MPIO_INDEPENDENT) {
            snprintf(test_title, sizeof(test_title), "parallel vector read test 5 -- %s / independent", vfd_name);
        }
        else if (coll_opt_mode == H5FD_MPIO_INDIVIDUAL_IO) {
            snprintf(test_title, sizeof(test_title), "parallel vector read test 5 -- %s / col op / ind I/O", vfd_name);
        }
        else {
            assert(coll_opt_mode == H5FD_MPIO_COLLECTIVE_IO);

            snprintf(test_title, sizeof(test_title), "parallel vector read test 5 -- %s / col op / col I/O", vfd_name);
        }
    ...
    vector_write_test_1(int file_name_id, int mpi_rank, int mpi_size, H5FD_mpio_xfer_t xfer_mode, H5FD_mpio_collective_opt_t coll_opt_mode, const char* vfd_name)
{
    ...
    if (mpi_rank == 0) {
        if (xfer_mode == H5FD_MPIO_INDEPENDENT) {
            snprintf(test_title, sizeof(test_title), "parallel vector write test 1 -- %s / independent", vfd_name);
        }
        else if (coll_opt_mode == H5FD_MPIO_INDIVIDUAL_IO) {
            snprintf(test_title, sizeof(test_title), "parallel vector write test 1 -- %s / col op / ind I/O", vfd_name);
        }
        else {
            assert(coll_opt_mode == H5FD_MPIO_COLLECTIVE_IO);

            snprintf(test_title, sizeof(test_title), "parallel vector write test 1 -- %s / col op / col I/O", vfd_name);
        }
    ...
    vector_write_test_2(int file_name_id, int mpi_rank, int mpi_size, H5FD_mpio_xfer_t xfer_mode, H5FD_mpio_collective_opt_t coll_opt_mode, const char* vfd_name)
{
    ...
    if (mpi_rank == 0) {
        if (xfer_mode == H5FD_MPIO_INDEPENDENT) {
            snprintf(test_title, sizeof(test_title), "parallel vector write test 2 -- %s / independent", vfd_name);
        }
        else if (coll_opt_mode == H5FD_MPIO_INDIVIDUAL_IO) {
            snprintf(test_title, sizeof(test_title), "parallel vector write test 2 -- %s / col op / ind I/O", vfd_name);
        }
        else {
            assert(coll_opt_mode == H5FD_MPIO_COLLECTIVE_IO);

            snprintf(test_title, sizeof(test_title), "parallel vector write test 2 -- %s / col op / col I/O", vfd_name);
        }
    ...
    vector_write_test_3(int file_name_id, int mpi_rank, int mpi_size, H5FD_mpio_xfer_t xfer_mode, H5FD_mpio_collective_opt_t coll_opt_mode, const char* vfd_name)
{
    ...
    if (mpi_rank == 0) {
        if (xfer_mode == H5FD_MPIO_INDEPENDENT) {
            snprintf(test_title, sizeof(test_title), "parallel vector write test 3 -- %s / independent", vfd_name);
        }
        else if (coll_opt_mode == H5FD_MPIO_INDIVIDUAL_IO) {
            snprintf(test_title, sizeof(test_title), "parallel vector write test 3 -- %s / col op / ind I/O", vfd_name);
        }
        else {
            assert(coll_opt_mode == H5FD_MPIO_COLLECTIVE_IO);

            snprintf(test_title, sizeof(test_title), "parallel vector write test 3 -- %s / col op / col I/O", vfd_name);
        }
    ...
    vector_write_test_4(int file_name_id, int mpi_rank, int mpi_size, H5FD_mpio_xfer_t xfer_mode, H5FD_mpio_collective_opt_t coll_opt_mode, const char* vfd_name)
{
    ...
    if (mpi_rank == 0) {
        if (xfer_mode == H5FD_MPIO_INDEPENDENT) {
            snprintf(test_title, sizeof(test_title), "parallel vector write test 4 -- %s / independent", vfd_name);
        }
        else if (coll_opt_mode == H5FD_MPIO_INDIVIDUAL_IO) {
            snprintf(test_title, sizeof(test_title), "parallel vector write test 4 -- %s / col op / ind I/O", vfd_name);
        }
        else {
            assert(coll_opt_mode == H5FD_MPIO_COLLECTIVE_IO);

            snprintf(test_title, sizeof(test_title), "parallel vector write test 4 -- %s / col op / col I/O", vfd_name);
        }
    ...
    vector_write_test_5(int file_name_id, int mpi_rank, int mpi_size, H5FD_mpio_xfer_t xfer_mode, H5FD_mpio_collective_opt_t coll_opt_mode, const char* vfd_name)
{
    ...
    if (mpi_rank == 0) {
        if (xfer_mode == H5FD_MPIO_INDEPENDENT) {
            snprintf(test_title, sizeof(test_title), "parallel vector write test 5 -- %s / independent", vfd_name);
        }
        else if (coll_opt_mode == H5FD_MPIO_INDIVIDUAL_IO) {
            snprintf(test_title, sizeof(test_title), "parallel vector write test 5 -- %s / col op / ind I/O", vfd_name);
        }
        else {
            assert(coll_opt_mode == H5FD_MPIO_COLLECTIVE_IO);

            snprintf(test_title, sizeof(test_title), "parallel vector write test 5 -- %s / col op / col I/O", vfd_name);
        }
                ...
                case 0:
                    if (((INTS_PER_RANK / 2) <= k) && (k < (3 * (INTS_PER_RANK / 4)))) {
                        if (read_fi_buf[j] != negative_fi_buf[j]) {
                            ...
                            printf("\nread_fi_buf[%d] = %d, %d expected.\n", j, read_fi_buf[j], negative_fi_buf[j]);
                        }
                    }
                    else if (((INTS_PER_RANK / 4) <= k) && (k < (3 * (INTS_PER_RANK / 8)))) {
                        if (read_fi_buf[j] != decreasing_fi_buf[j]) {
                            ...
                            printf("\nread_fi_buf[%d] = %d, %d expected.\n", j, read_fi_buf[j], decreasing_fi_buf[j]);
                        }
                    }
                    else if (((INTS_PER_RANK / 16) <= k) && (k < (INTS_PER_RANK / 8))) {
                        if (read_fi_buf[j] != increasing_fi_buf[j]) {
                            ...
                            printf("\nread_fi_buf[%d] = %d, %d expected.\n", j, read_fi_buf[j], increasing_fi_buf[j]);
                        }
                    }
                    else {
                        if (read_fi_buf[j] != 0) {
    ...
    vector_write_test_6(int file_name_id, int mpi_rank, int mpi_size, H5FD_mpio_xfer_t xfer_mode, H5FD_mpio_collective_opt_t coll_opt_mode, const char* vfd_name)
{
    ...
    if (mpi_rank == 0) {
        if (xfer_mode == H5FD_MPIO_INDEPENDENT) {
            snprintf(test_title, sizeof(test_title), "parallel vector write test 6 -- %s / independent", vfd_name);
        }
        else if (coll_opt_mode == H5FD_MPIO_INDIVIDUAL_IO) {
            snprintf(test_title, sizeof(test_title), "parallel vector write test 6 -- %s / col op / ind I/O", vfd_name);
        }
        else {
            assert(coll_opt_mode == H5FD_MPIO_COLLECTIVE_IO);

            snprintf(test_title, sizeof(test_title), "parallel vector write test 6 -- %s / col op / col I/O", vfd_name);
        }
    ...
    vector_write_test_8(int file_name_id, int mpi_rank, int mpi_size, H5FD_mpio_xfer_t xfer_mode, H5FD_mpio_collective_opt_t coll_opt_mode, const char* vfd_name)
{
    ...
    if (mpi_rank == 0) {
        if (xfer_mode == H5FD_MPIO_INDEPENDENT) {
            snprintf(test_title, sizeof(test_title), "parallel vector write test 8 -- %s / independent", vfd_name);
        }
        else if (coll_opt_mode == H5FD_MPIO_INDIVIDUAL_IO) {
            snprintf(test_title, sizeof(test_title), "parallel vector write test 8 -- %s / col op / ind I/O", vfd_name);
        }
        else {
            assert(coll_opt_mode == H5FD_MPIO_COLLECTIVE_IO);

            snprintf(test_title, sizeof(test_title), "parallel vector write test 8 -- %s / col op / col I/O", vfd_name);
        }
...
static void test_vector_io(int mpi_rank, int mpi_size)
{
    unsigned nerrs = 0;
    ...
    nerrs += vector_read_test_1(0, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT, H5FD_MPIO_INDIVIDUAL_IO, "mpio");
    ...
    nerrs += vector_read_test_2(1, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT, H5FD_MPIO_INDIVIDUAL_IO, "mpio");
    ...
    nerrs += vector_read_test_3(2, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT, H5FD_MPIO_INDIVIDUAL_IO, "mpio");
    ...
    nerrs += vector_read_test_4(3, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT, H5FD_MPIO_INDIVIDUAL_IO, "mpio");
    ...
    nerrs += vector_read_test_5(4, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT, H5FD_MPIO_INDIVIDUAL_IO, "mpio");
    ...
    nerrs += vector_write_test_1(0, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT, H5FD_MPIO_INDIVIDUAL_IO, "mpio");
    nerrs += vector_write_test_1(0, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE, H5FD_MPIO_INDIVIDUAL_IO, "mpio");
    nerrs += vector_write_test_1(0, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE, H5FD_MPIO_COLLECTIVE_IO, "mpio");

    nerrs += vector_write_test_2(1, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT, H5FD_MPIO_INDIVIDUAL_IO, "mpio");
    nerrs += vector_write_test_2(1, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE, H5FD_MPIO_INDIVIDUAL_IO, "mpio");
    nerrs += vector_write_test_2(1, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE, H5FD_MPIO_COLLECTIVE_IO, "mpio");

    nerrs += vector_write_test_3(2, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT, H5FD_MPIO_INDIVIDUAL_IO, "mpio");
    nerrs += vector_write_test_3(2, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE, H5FD_MPIO_INDIVIDUAL_IO, "mpio");
    nerrs += vector_write_test_3(2, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE, H5FD_MPIO_COLLECTIVE_IO, "mpio");

    nerrs += vector_write_test_4(3, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT, H5FD_MPIO_INDIVIDUAL_IO, "mpio");
    nerrs += vector_write_test_4(3, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE, H5FD_MPIO_INDIVIDUAL_IO, "mpio");
    nerrs += vector_write_test_4(3, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE, H5FD_MPIO_COLLECTIVE_IO, "mpio");

    nerrs += vector_write_test_5(4, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT, H5FD_MPIO_INDIVIDUAL_IO, "mpio");
    nerrs += vector_write_test_5(4, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE, H5FD_MPIO_INDIVIDUAL_IO, "mpio");
    nerrs += vector_write_test_5(4, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE, H5FD_MPIO_COLLECTIVE_IO, "mpio");

    nerrs += vector_write_test_6(5, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT, H5FD_MPIO_INDIVIDUAL_IO, "mpio");
    nerrs += vector_write_test_6(5, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE, H5FD_MPIO_INDIVIDUAL_IO, "mpio");
    nerrs += vector_write_test_6(5, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE, H5FD_MPIO_COLLECTIVE_IO, "mpio");

    nerrs += vector_write_test_7(6, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT, H5FD_MPIO_INDIVIDUAL_IO, "mpio");
    nerrs += vector_write_test_7(6, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE, H5FD_MPIO_INDIVIDUAL_IO, "mpio");
    nerrs += vector_write_test_7(6, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE, H5FD_MPIO_COLLECTIVE_IO, "mpio");

    nerrs += vector_write_test_8(7, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT, H5FD_MPIO_INDIVIDUAL_IO, "mpio");
    nerrs += vector_write_test_8(7, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE, H5FD_MPIO_INDIVIDUAL_IO, "mpio");
    nerrs += vector_write_test_8(7, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE, H5FD_MPIO_COLLECTIVE_IO, "mpio");
                                            ...
                                            bool shorten_rbufs)
{
    ...
    /* Issue read call */
    if (H5FDread_selection(lf, type, dxpl, count, mem_spaces, file_spaces, offsets, element_sizes, (void**)rbufs) < 0) {
        goto error;
    }
                                            ...
                                            hsize_t dims2[])
{
    ...
    for (shorten_element_sizes = 0; shorten_element_sizes <= 1; shorten_element_sizes++) {
        ...
        /* Issue write call */
        if (test_selection_io_write(dxpl, lf, type, 2, mem_spaces, file_spaces, addrs, element_sizes, (int**)wbufs) < 0) {
            P_TEST_ERROR;
        }
        ...
        /* Read and verify */
        if (test_selection_io_read_verify(dxpl,
                                          mpi_rank,
                                          verify_start,
                                          verify_block,
                                          lf,
                                          ...
                                          hsize_t dims2[])
{
    ...
    /* Issue write call */
    if (test_selection_io_write(dxpl, lf, type, 1, &mem_spaces[0], &file_spaces[1], &addrs[1], element_sizes, (int**)&wbufs[0]) < 0) {
        P_TEST_ERROR;
    }
    ...
    /* Issue write call */
    if (test_selection_io_write(dxpl, lf, type, 1, &mem_spaces[1], &file_spaces[0], &addrs[0], element_sizes, (int**)&wbufs[1]) < 0) {
        P_TEST_ERROR;
    }
                                       ...
                                       hsize_t dims2[])
{
    ...
    if (test_selection_io_write(dxpl, lf, type, 1, &mem_spaces[1], &file_spaces[1], &addrs[1], element_sizes, (int**)&wbufs[1]) < 0) {
        P_TEST_ERROR;
    }
                                       ...
                                       hsize_t dims1[])
{
    ...
    /* Issue write call */
    if (test_selection_io_write(dxpl, lf, type, 1, &mem_spaces[0], &file_spaces[0], &addrs[0], element_sizes, (int**)&wbufs[0]) < 0) {
        P_TEST_ERROR;
    }
...
static void test_selection_io_real(int mpi_rank, int mpi_size, H5FD_t* lf, hid_t dxpl)
{
    ...
    for (type = 1; type < H5FD_MEM_NTYPES; type++) {
        ...
        test_selection_io_types_1d(mpi_rank, mpi_size, lf, dxpl, type, addrs, element_sizes, mem_spaces, file_spaces, dims1);
        test_selection_io_types_2d(mpi_rank, mpi_size, lf, dxpl, type, addrs, element_sizes, mem_spaces, file_spaces, dims2);
        test_selection_io_types_1d_2d(mpi_rank, mpi_size, lf, dxpl, type, addrs, element_sizes, mem_spaces, file_spaces, dims1, dims2);
        test_selection_io_types_shorten(mpi_rank, mpi_size, lf, dxpl, type, addrs, element_sizes, mem_spaces, file_spaces, dims1, dims2);
