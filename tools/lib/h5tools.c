    ...
    case VFD_BY_VALUE:
        ...
        if (vfd_info->u.value == H5_VFD_SUBFILING) {
            ...
            if (H5Pset_driver_by_value(fapl_id, vfd_info->u.value, (const char*)vfd_info->info) < 0) {
                H5TOOLS_GOTO_ERROR(FAIL, "can't load VFD plugin by driver value '%ld'", (long int)vfd_info->u.value);
            }
        }
        break;

    default: H5TOOLS_GOTO_ERROR(FAIL, "invalid VFD retrieval type");
    }
...
herr_t h5tools_set_fapl_vol(hid_t fapl_id, h5tools_vol_info_t* vol_info)
{
    ...
    default: H5TOOLS_GOTO_ERROR(FAIL, "invalid VOL retrieval type");
    }
 ...
 *-------------------------------------------------------------------------
 */
H5_ATTR_PURE static size_t h5tools_count_ncols(const char* s)
{
    size_t i;
                            ...
                            hsize_t elmt_counter)
{
     ...
     * current location...
     */
    if (info->line_multi_new == 1 && (ctx->cur_column + h5tools_count_ncols(s) + strlen(OPT(info->elmt_suf2, " ")) + strlen(OPT(info->line_suf, ""))) > ncols) {
        ...
        else if ((ctx->prev_prefix_len + h5tools_count_ncols(s) + strlen(OPT(info->elmt_suf2, " ")) + strlen(OPT(info->line_suf, ""))) <= ncols) {
            /*
    ...
    if (info->line_multi_new == 1 && ctx->prev_multiline &&
        (ctx->cur_column + h5tools_count_ncols(s) + strlen(OPT(info->elmt_suf2, " ")) + strlen(OPT(info->line_suf, ""))) > ncols) {
        ctx->need_prefix = true;
    }
    H5TOOLS_DEBUG("ctx->need_prefix=%d", ctx->need_prefix);
    ...
    for (secnum = 0, multiline = 0; (section = strtok(secnum ? NULL : s, OPTIONAL_LINE_BREAK)); secnum++) {
         ...
         */
        if ((ctx->cur_column + strlen(section) + strlen(OPT(info->elmt_suf2, " ")) + strlen(OPT(info->line_suf, ""))) > ncols) {
            ctx->need_prefix = 1;
        }
                                   ...
                                   hsize_t elmt_counter)
{
    ...
    for (secnum = 0, multiline = 0; (section = strtok(secnum ? NULL : s, OPTIONAL_LINE_BREAK)); secnum++) {
        ...
        if ((!info->skip_first || local_elmt_counter) &&
            (ctx->cur_column + strlen(section) + strlen(OPT(info->elmt_suf2, " ")) + strlen(OPT(info->line_suf, ""))) > ncols) {
            ctx->need_prefix = 1;
        }
