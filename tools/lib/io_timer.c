...
{
    return (((double)a->tv_sec + ((double)a->tv_usec) / (double)MICROSECOND) - ((double)b->tv_sec + ((double)b->tv_usec) / (double)MICROSECOND));
}
 ...
 */
io_time_t* io_time_new(clock_type type)
{
...
io_time_t* io_time_set(io_time_t* pt, timer_type t, int start_stop)
{
    ...
    case MPI_CLOCK:
        ...
        break;
#else
    case MPI_CLOCK: fprintf(stderr, "MPI clock set in serial library\n"); return NULL;
    ...
    default: fprintf(stderr, "Unknown time clock type (%d)\n", pt->type); return NULL;
    } /* end switch */
 ...
 */
H5_ATTR_PURE double io_time_get(io_time_t* pt, timer_type t)
{
    /* sanity check */
    assert(pt);
