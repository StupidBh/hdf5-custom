...
int main(int argc, char* argv[])
{
    ...
    while ((opt = H5_get_option(argc, (const char* const*)argv, s_opts, l_opts)) != EOF) {
        switch ((char)opt) {
        case 'd': oname_dst = strdup(H5_optarg); break;
        ...
        case 'i': fname_src = strdup(H5_optarg); break;

        case 'o': fname_dst = strdup(H5_optarg); break;

        case 'p': parents = 1; break;

        case 's': oname_src = strdup(H5_optarg); break;
