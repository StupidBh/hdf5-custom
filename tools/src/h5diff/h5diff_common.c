...
static void check_options(diff_opt_t* opts)
{
    ...
    if ((opts->delta_bool + opts->percent_bool + opts->use_system_epsilon) > 1) {
        ...
        printf(
            "Try '-h' or '--help' option for more information or see the %s entry in the 'HDF5 "
            "Reference Manual'.\n",
            PROGRAMNAME);
...
void parse_command_line(int argc, const char* const* argv, const char** fname1, const char** fname2, const char** objname1, const char** objname2, diff_opt_t* opts)
{
    int i;
    int opt;
    ...
    while ((opt = H5_get_option(argc, argv, s_opts, l_opts)) != EOF) {
        ...
        case 'r': opts->mode_report = 1; break;

        case 'l': opts->follow_links = true; break;

        case 'x': opts->no_dangle_links = 1; break;
        ...
        case 'C': opts->disable_compact_subset = true; break;

        case 'A':
            opts->exclude_attr_path = 1;

            /* create linked list of excluding objects */
            if ((exclude_attr_node = (struct exclude_path_list*)malloc(sizeof(struct exclude_path_list))) == NULL) {
                printf("Error: lack of memory!\n");
                h5diff_exit(EXIT_FAILURE);
            }
        ...
        case 'N': opts->do_nans = 0; break;

        case 'c': opts->mode_list_not_cmp = 1; break;

        case 'e': opts->use_system_epsilon = 1; break;

        case 'K': opts->page_cache = strtoul(H5_optarg, NULL, 0); break;
        ...
        case '3': opts->vol_info[0].info_string = H5_optarg; break;
        ...
        case '6': opts->vol_info[1].info_string = H5_optarg; break;
        ...
        case '9': opts->vfd_info[0].info = (const void*)H5_optarg; break;
        ...
        case 'Z': opts->vfd_info[1].info = (const void*)H5_optarg; break;
        }
    }
 ...
 *-------------------------------------------------------------------------
 */
H5_ATTR_PURE static int check_n_input(const char* str)
{
    unsigned i;
    char c;
 ...
 *-------------------------------------------------------------------------
 */
static int check_p_input(const char* str)
{
    double x;
 ...
 *-------------------------------------------------------------------------
 */
static int check_d_input(const char* str)
{
    double x;
...
void usage(void)
{
    ...
    PRINTVALSTREAM(rawoutstream,
                   "                           If none of the above options are used to "
    ...
    PRINTVALSTREAM(rawoutstream, "  Default mode: print the number of differences found and where they occurred\n");
