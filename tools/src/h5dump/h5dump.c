...
#define DEFAULT_XSD "https://raw.githubusercontent.com/HDFGroup/hdf5/develop/tools/test/h5dump/testfiles/xml/HDF5-File.xsd"
#define DEFAULT_DTD "https://raw.githubusercontent.com/HDFGroup/hdf5/develop/tools/test/h5dump/testfiles/xml/HDF5-File.dtd"

/* Standard DDL output */
static const dump_functions ddl_function_table = { dump_group, dump_named_datatype, dump_dataset, dump_dataspace, dump_datatype, dump_attr_cb, dump_data };

/* XML output */
static const dump_functions xml_function_table = { xml_dump_group,    xml_dump_named_datatype, xml_dump_dataset, xml_dump_dataspace,
...
static void usage(const char* prog)
{
    ...
    PRINTVALSTREAM(rawoutstream, "  D - is the file driver to use in opening the file. Acceptable values are available from\n");
    PRINTVALSTREAM(rawoutstream,
                   "      "
...
ssize_t table_list_add(hid_t oid, unsigned long file_no)
{
    ...
    if (table_list.nused == table_list.nalloc) {
        ...
        if (NULL == (tmp_ptr = (h5dump_table_items_t*)realloc(table_list.tables, table_list.nalloc * sizeof(table_list.tables[0])))) {
            return -1;
        }
 ...
 *-------------------------------------------------------------------------
 */
H5_ATTR_PURE ssize_t table_list_visited(unsigned long file_no)
{
 ...
 *-------------------------------------------------------------------------
 */
static int set_binary_form(const char* form)
{
    int bform = -1;
 ...
 *-------------------------------------------------------------------------
 */
static H5_index_t set_sort_by(const char* form)
{
 ...
 *-------------------------------------------------------------------------
 */
static H5_iter_order_t set_sort_order(const char* form)
{
 ...
 *-------------------------------------------------------------------------
 */
static int parse_mask_list(const char* h_list)
{
    int soffset_value;
 ...
 *-------------------------------------------------------------------------
 */
static void free_handler(struct handler_t* hand, int len)
{
    int i;
 ...
 *-------------------------------------------------------------------------
 */
static struct handler_t* parse_command_line(int argc, const char* const* argv)
{
...
parse_start:
        ...
        case 'n':
            ...
            break;
        case 'p': dump_opts.display_dcpl = true; break;
        case 'y': dump_opts.display_ai = false; break;
        case 'e': dump_opts.display_escape = true; break;
        case 'H':
        ...
        case 'i':
            ...
            last_was_dset = false;
            break;
        case 'r': dump_opts.display_char = true; break;
        case 'V':
        ...
        case 'M':
            ...
            break;
        case 'v': dump_opts.display_vds_first = true; break;
        case 'G':
        ...
        case 'm':
            {
                ...
                if (plus_flag) {
                    snprintf(complex_num_fp_format, sizeof(complex_num_fp_format), "%s%si", fp_format, fp_format);
                }
                else {
                    snprintf(complex_num_fp_format, sizeof(complex_num_fp_format), "%s%%+%si", fp_format, format_specifier);
                }
...
end_collect:
        ...
        case 'E':
            ...
            break;
        case 'C': dump_opts.disable_compact_subset = true; break;
        case 'h':
            usage(h5tools_getprogname());
            free_handler(hand, argc);
            hand = NULL;
