 ...
 *-------------------------------------------------------------------------
 */
herr_t dump_attr_cb(hid_t oid, const char* attr_name, const H5A_info_t H5_ATTR_UNUSED* info, void H5_ATTR_UNUSED* _op_data)
{
    h5tools_context_t ctx; /* print context  */
    h5tool_format_t* outputformat = &h5tools_dataformat;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t dump_all_cb(hid_t group, const char* name, const H5L_info2_t* linfo, void H5_ATTR_UNUSED* op_data)
{
    hid_t obj;
    ...
    if (linfo->type == H5L_TYPE_HARD) {
        ...
        case H5L_TYPE_SOFT:
            if ((targbuf = (char*)malloc(linfo->u.val_size)) == NULL) {
                ...
                /* Render the element */
                h5tools_str_reset(&buffer);
                h5tools_str_append(&buffer, "%s \"%s\" %s", h5tools_dump_header_format->softlinkbegin, name, h5tools_dump_header_format->softlinkblockbegin);
                h5tools_render_element(rawoutstream, outputformat, &ctx, &buffer, &curr_pos, (size_t)outputformat->line_ncols, (hsize_t)0, (hsize_t)0);
        ...
        case H5L_TYPE_EXTERNAL:
            ...
            if ((targbuf = (char*)malloc(local_linfo.u.val_size)) == NULL) {
                ...
                /* Render the element */
                h5tools_str_reset(&buffer);
                h5tools_str_append(&buffer, "%s \"%s\" %s", h5tools_dump_header_format->extlinkbegin, name, h5tools_dump_header_format->extlinkblockbegin);
                h5tools_render_element(rawoutstream, outputformat, &ctx, &buffer, &curr_pos, (size_t)outputformat->line_ncols, (hsize_t)0, (hsize_t)0);
                ...
                if (H5Lget_val(group, name, targbuf, local_linfo.u.val_size, H5P_DEFAULT) < 0) {
                    ...
                    if (H5Lunpack_elink_val(targbuf, local_linfo.u.val_size, NULL, &filename, &targname) < 0) {
                        indentation(dump_indent);
        ...
        default:
            ...
            /* Render the element */
            h5tools_str_reset(&buffer);
            h5tools_str_append(&buffer, "%s \"%s\" %s", h5tools_dump_header_format->udlinkbegin, name, h5tools_dump_header_format->udlinkblockbegin);
            h5tools_render_element(rawoutstream, outputformat, &ctx, &buffer, &curr_pos, (size_t)outputformat->line_ncols, (hsize_t)0, (hsize_t)0);
 ...
 *-------------------------------------------------------------------------
 */
void dump_named_datatype(hid_t tid, const char* name)
{
    H5O_info2_t oinfo;
    ...
    /* Render the element */
    h5tools_str_reset(&buffer);
    h5tools_str_append(&buffer, "%s \"%s\" %s", h5tools_dump_header_format->datatypebegin, name, h5tools_dump_header_format->datatypeblockbegin);
    h5tools_render_element(rawoutstream, outputformat, &ctx, &buffer, &curr_pos, (size_t)outputformat->line_ncols, (hsize_t)0, (hsize_t)0);
 ...
 *-------------------------------------------------------------------------
 */
void dump_group(hid_t gid, const char* name)
{
    H5O_info2_t oinfo;
    hid_t dset;
    hid_t type;
    ...
    /* Render the element */
    h5tools_str_reset(&buffer);
    h5tools_str_append(&buffer, "%s \"%s\" %s", h5tools_dump_header_format->groupbegin, name, h5tools_dump_header_format->groupblockbegin);
    h5tools_render_element(rawoutstream, outputformat, &ctx, &buffer, &curr_pos, (size_t)outputformat->line_ncols, (hsize_t)0, (hsize_t)0);
 ...
 *-------------------------------------------------------------------------
 */
void dump_dataset(hid_t did, const char* name, struct subset_t* sset)
{
    h5tools_context_t ctx; /* print context  */
    h5tool_format_t* outputformat = &h5tools_dataformat;
    h5tool_format_t string_dataformat;
    ...
    if (dump_opts.display_data) {
        ...
        for (u = 0; u < data_loop; u++) {
            ...
            case H5T_NO_CLASS:
            case H5T_NCLASSES:
            default          : error_msg("invalid H5TCLASS type\n"); break;
            } /* end switch */
        } /* for(u=0; u<data_loop; u++) */
    }
    H5Tclose(type);
 ...
 *-------------------------------------------------------------------------
 */
void dump_data(hid_t obj_id, int obj_data, struct subset_t* sset, int display_index)
{
    h5tools_context_t ctx; /* print context  */
    h5tool_format_t* outputformat = &h5tools_dataformat;
    h5tool_format_t string_dataformat;
...
static herr_t attr_search(hid_t oid, const char* attr_name, const H5A_info_t H5_ATTR_UNUSED* ainfo, void* _op_data)
{
    herr_t ret = SUCCEED;
    int j;
    char* obj_op_name;
...
static herr_t
    obj_search(const char* path, const H5O_info2_t* oi, bool H5_ATTR_UNUSED already_visited, const trav_seen_t H5_ATTR_UNUSED* visited_obj_info, void* _op_data)
{
    ...
    if (strcmp(path, op_name) == 0) {
        switch (oi->type) {
        case H5O_TYPE_GROUP: handle_groups(handle_data->fid, path, NULL, 0, NULL); break;

        case H5O_TYPE_DATASET: handle_datasets(handle_data->fid, path, NULL, 0, NULL); break;

        case H5O_TYPE_NAMED_DATATYPE: handle_datatypes(handle_data->fid, path, NULL, 0, NULL); break;

        case H5O_TYPE_MAP:
        case H5O_TYPE_UNKNOWN:
        case H5O_TYPE_NTYPES:
        default              : error_msg("unknown object type value\n"); h5tools_setstatus(EXIT_FAILURE);
        } /* end switch */
    }
...
static herr_t lnk_search(const char* path, const H5L_info2_t* li, void* _op_data)
{
    size_t search_len;
    size_t k;
    char* search_name;
    ...
    if (search_name == NULL) {
        ...
        if (strcmp(path, search_name) == 0) {
            switch (li->type) {
            case H5L_TYPE_SOFT:
            case H5L_TYPE_EXTERNAL: handle_links(handle_data->fid, op_name, NULL, 0, NULL); break;
 ...
 *-------------------------------------------------------------------------
 */
void handle_paths(hid_t fid, const char* path_name, void H5_ATTR_UNUSED* data, int H5_ATTR_UNUSED pe, const char H5_ATTR_UNUSED* display_name)
{
...
void handle_attributes(hid_t fid, const char* attr, void H5_ATTR_UNUSED* data, int H5_ATTR_UNUSED pe, const char H5_ATTR_UNUSED* display_name)
{
    ...
    if ((oid = H5Oopen(fid, obj_name, H5P_DEFAULT)) < 0) {
        ...
        /* Render the element */
        h5tools_str_reset(&buffer);
        h5tools_str_append(&buffer, "%s \"%s\" %s", h5tools_dump_header_format->attributebegin, attr, h5tools_dump_header_format->attributeblockbegin);
        h5tools_render_element(rawoutstream, outputformat, &ctx, &buffer, &curr_pos, (size_t)outputformat->line_ncols, (hsize_t)0, (hsize_t)0);
...
static int dump_extlink(hid_t group, const char* linkname, const char* objname)
{
    ...
    if (idx) {
        ...
        switch (oi.type) {
        case H5O_TYPE_GROUP: handle_groups(group, linkname, NULL, 0, objname); break;

        case H5O_TYPE_DATASET: handle_datasets(group, linkname, NULL, 0, objname); break;

        case H5O_TYPE_NAMED_DATATYPE: handle_datatypes(group, linkname, NULL, 0, objname); break;

        case H5O_TYPE_MAP:
        case H5O_TYPE_UNKNOWN:
        case H5O_TYPE_NTYPES:
        default              : h5tools_setstatus(EXIT_FAILURE);
        }
