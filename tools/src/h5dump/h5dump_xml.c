 ...
 *-------------------------------------------------------------------------
 */
static herr_t xml_dump_all_cb(hid_t group, const char* name, const H5L_info2_t* linfo, void H5_ATTR_UNUSED* op_data)
{
    hid_t obj;
    herr_t ret = SUCCEED;
            ...
            if ((obj = H5Dopen2(group, name, H5P_DEFAULT)) >= 0) {
                if (oinfo.rc > 1 || hit_elink) {
                    ...
                    if (found_obj == NULL) {
                        ...
                        h5tools_str_append(&buffer, "%s \"%s\" %s", h5tools_dump_header_format->datasetbegin, name, h5tools_dump_header_format->datasetblockbegin);
                        ...
                        /* Render the element */
                        h5tools_str_reset(&buffer);
                        h5tools_str_append(&buffer, "<%sDatasetPtr OBJ-XID=\"%s\" H5Path=\"%s\"/>", xmlnsprefix, pointerxid, t_objname);
                        h5tools_render_element(rawoutstream, outputformat, &ctx, &buffer, &curr_pos, (size_t)outputformat->line_ncols, (hsize_t)0, (hsize_t)0);
 ...
 */
int xml_name_to_XID(hid_t loc_id, const char* str, char* outstr, int outlen, int gen)
{
 ...
 *-------------------------------------------------------------------------
 */
static char* xml_escape_the_name(const char* str)
{
    size_t extra;
    size_t len;
    size_t i;
 ...
 *-------------------------------------------------------------------------
 */
static char* xml_escape_the_string(const char* str, int slen)
{
    size_t extra;
    size_t len;
    size_t i;
 ...
 *-------------------------------------------------------------------------
 */
static void xml_print_datatype(hid_t type, unsigned in_group)
{
    char* mname;
    hid_t mtype;
    unsigned nmembers;
    ...
    if (!in_group && H5Tcommitted(type) > 0) {
        ...
        if (found_obj) {
            ...
            if (!found_obj->recorded) {
                ...
                /* Render the element */
                h5tools_str_reset(&buffer);
                h5tools_str_append(&buffer, "<%sNamedDataTypePtr OBJ-XID=\"%s\" H5Path=\"%s\"/>", xmlnsprefix, dtxid, t_objname);
                h5tools_render_element(rawoutstream, outputformat, &ctx, &buffer, &curr_pos, (size_t)outputformat->line_ncols, (hsize_t)0, (hsize_t)0);
                free(t_objname);
            }
            free(dtxid);
        ...
        case H5T_INTEGER:
            ...
            switch (ord) {
            case H5T_ORDER_LE   : h5tools_str_append(&buffer, "LE"); break;
            case H5T_ORDER_BE   : h5tools_str_append(&buffer, "BE"); break;
            case H5T_ORDER_VAX  :
            case H5T_ORDER_MIXED:
            case H5T_ORDER_NONE :
            case H5T_ORDER_ERROR:
            default             : h5tools_str_append(&buffer, "ERROR_UNKNOWN"); break;
            } /* end switch */

            h5tools_str_append(&buffer, "\" Sign=\"");

            switch (sgn) {
            case H5T_SGN_NONE : h5tools_str_append(&buffer, "false"); break;
            case H5T_SGN_2    : h5tools_str_append(&buffer, "true"); break;
            case H5T_SGN_ERROR:
            case H5T_NSGN     :
            default           : h5tools_str_append(&buffer, "ERROR_UNKNOWN"); break;
            } /* end switch */
        ...
        case H5T_FLOAT:
            ...
            case H5T_ORDER_LE   : h5tools_str_append(&buffer, "LE"); break;
            case H5T_ORDER_BE   : h5tools_str_append(&buffer, "BE"); break;
            case H5T_ORDER_VAX  : h5tools_str_append(&buffer, "VAX"); break;
            case H5T_ORDER_MIXED:
            case H5T_ORDER_NONE :
            case H5T_ORDER_ERROR:
            default             : h5tools_str_append(&buffer, "ERROR_UNKNOWN");
            } /* end switch */
    ...
    case H5S_SIMPLE:
        ...
        for (i = 0; i < ndims; i++) {
            if (maxsize[i] == H5S_UNLIMITED) {
                ...
                /* Render the element */
                h5tools_str_reset(&buffer);
                h5tools_str_append(&buffer, "<%sDimension  DimSize=\"%" PRIuHSIZE "\" MaxDimSize=\"UNLIMITED\"/>", xmlnsprefix, size[i]);
                h5tools_render_element(rawoutstream, outputformat, &ctx, &buffer, &curr_pos, (size_t)outputformat->line_ncols, (hsize_t)0, (hsize_t)0);
            }
                ...
                /* Render the element */
                h5tools_str_reset(&buffer);
                h5tools_str_append(&buffer, "<%sDimension  DimSize=\"%" PRIuHSIZE "\" MaxDimSize=\"%" PRIuHSIZE "\"/>", xmlnsprefix, size[i], size[i]);
                h5tools_render_element(rawoutstream, outputformat, &ctx, &buffer, &curr_pos, (size_t)outputformat->line_ncols, (hsize_t)0, (hsize_t)0);
            }
            else {
                ctx.need_prefix = true;
                ...
                /* Render the element */
                h5tools_str_reset(&buffer);
                h5tools_str_append(&buffer, "<%sDimension  DimSize=\"%" PRIuHSIZE "\" MaxDimSize=\"%" PRIuHSIZE "\"/>", xmlnsprefix, size[i], maxsize[i]);
                h5tools_render_element(rawoutstream, outputformat, &ctx, &buffer, &curr_pos, (size_t)outputformat->line_ncols, (hsize_t)0, (hsize_t)0);
            }
        }
        ctx.indent_level--;
 ...
 *-------------------------------------------------------------------------
 */
void xml_dump_data(hid_t obj_id, int obj_data, struct subset_t H5_ATTR_UNUSED* sset, int H5_ATTR_UNUSED pindex)
{
 ...
 *-------------------------------------------------------------------------
 */
herr_t xml_dump_attr(hid_t attr, const char* attr_name, const H5A_info_t H5_ATTR_UNUSED* info, void H5_ATTR_UNUSED* op_data)
{
    ...
    if ((attr_id = H5Aopen(attr, attr_name, H5P_DEFAULT)) >= 0) {
        ...
        if (dump_opts.display_attr_data && space_type != H5S_NULL) {
            ...
            case H5T_ENUM:
            case H5T_ARRAY:
            case H5T_COMPLEX : dump_function_table->dump_data_function(attr_id, ATTRIBUTE_DATA, NULL, 0); break;
 ...
 *-------------------------------------------------------------------------
 */
void xml_dump_named_datatype(hid_t type, const char* name)
{
    hsize_t curr_pos = 0;  /* total data element position   */
    h5tools_str_t buffer;  /* string into which to render   */
    h5tools_context_t ctx; /* print context  */
 ...
 *-------------------------------------------------------------------------
 */
void xml_dump_group(hid_t gid, const char* name)
{
    H5O_info2_t oinfo;
    hid_t gcpl_id;
    hid_t dset, type;
 ...
 *-------------------------------------------------------------------------
 */
static int xml_print_refs(hid_t did, int source)
{
    herr_t e;
    hid_t type = H5I_INVALID_HID;
    hid_t space = H5I_INVALID_HID;
 ...
 *-------------------------------------------------------------------------
 */
static int xml_print_strs(hid_t did, int source)
{
    herr_t e;
    hid_t type = H5I_INVALID_HID;
    hid_t space = H5I_INVALID_HID;
 ...
 *-------------------------------------------------------------------------
 */
static void check_filters(hid_t dcpl)
{
    int nfilt;
    int i;
    H5Z_filter_t filter;
 ...
 *-------------------------------------------------------------------------
 */
void xml_dump_dataset(hid_t did, const char* name, struct subset_t H5_ATTR_UNUSED* sset)
{
    hid_t type;
    hid_t space;
    hid_t dcpl;
    ...
    h5tools_str_reset(&buffer);
    h5tools_str_append(&buffer,
                       "<%sDataset Name=\"%s\" OBJ-XID=\"%s\" H5Path= \"%s\" Parents=\"%s\" H5ParentPaths=\"%s\">",
                       xmlnsprefix,
                       t_name,
                       rstr,
                       t_tmp,
    ...
    if (H5D_CHUNKED == H5Pget_layout(dcpl)) {
        if ((maxdims = H5Sget_simple_extent_ndims(space)) < 0) {
            ...
            for (i = 0; i < ndims; i++) {
                ...
                /* Render the element */
                h5tools_str_reset(&buffer);
                h5tools_str_append(&buffer, "<%sChunkDimension DimSize=\"%" PRIuHSIZE "\" />", xmlnsprefix, chsize[i]);
                h5tools_render_element(rawoutstream, outputformat, &ctx, &buffer, &curr_pos, (size_t)outputformat->line_ncols, (hsize_t)0, (hsize_t)0);
            }
    ...
    switch (ft) {
    case H5D_FILL_TIME_ALLOC: h5tools_str_append(&buffer, "FillOnAlloc"); break;
    case H5D_FILL_TIME_NEVER: h5tools_str_append(&buffer, "FillNever"); break;
    case H5D_FILL_TIME_IFSET: h5tools_str_append(&buffer, "FillIfSet"); break;
    case H5D_FILL_TIME_ERROR:
    default                 : h5tools_str_append(&buffer, "?"); break;
    } /* end switch */
    h5tools_str_append(&buffer, "\" ");
    H5Pget_alloc_time(dcpl, &at);
    h5tools_str_append(&buffer, "AllocationTime=\"");
    switch (at) {
    case H5D_ALLOC_TIME_EARLY  : h5tools_str_append(&buffer, "Early"); break;
    case H5D_ALLOC_TIME_INCR   : h5tools_str_append(&buffer, "Incremental"); break;
    case H5D_ALLOC_TIME_LATE   : h5tools_str_append(&buffer, "Late"); break;
 ...
 *-------------------------------------------------------------------------
 */
static void xml_print_enum(hid_t type)
{
