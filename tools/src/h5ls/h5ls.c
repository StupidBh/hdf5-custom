...
static void usage(void)
{
    ...
    PRINTVALSTREAM(rawoutstream, "                   Print messages from the HDF5 error stack as they occur.\n");
    ...
    PRINTVALSTREAM(rawoutstream, "                   Accepts tuple of \"(<aws-region>,<access-id>,<access-key>)\" or\n");
    PRINTVALSTREAM(rawoutstream, "                   \"(<aws-region>,<access-id>,<access-key>,<session-token>)\".\n");
    PRINTVALSTREAM(rawoutstream, "                   If absent or C->\"(,,)\" or C->\"(,,,)\", defaults to no-authentication.\n");
    PRINTVALSTREAM(rawoutstream, "                   Has no effect if vfd flag not set to \"ros3\".\n");
 ...
 *-------------------------------------------------------------------------
 */
static int print_string(h5tools_str_t* buffer, const char* s, bool escape_spaces)
{
    int nprint = 0;
 ...
 *-------------------------------------------------------------------------
 */
static int print_obj_name(h5tools_str_t* buffer, const iter_t* iter, const char* oname, const char* s)
{
 ...
 *-------------------------------------------------------------------------
 */
static bool print_native_type(h5tools_str_t* buffer, hid_t type, int ind)
{
    if (!simple_output_g) {
 ...
 *-------------------------------------------------------------------------
 */
static bool print_specific_float_type(h5tools_str_t* buffer, hid_t type, int ind)
{
 ...
 *-------------------------------------------------------------------------
 */
static void print_precision(h5tools_str_t* buffer, hid_t type, int ind)
{
    ...
    if (8 * H5Tget_size(type) != (prec = H5Tget_precision(type))) {
        ...
        if (H5Tget_offset(type) > 0) {
            switch (plsb) {
            case H5T_PAD_ZERO      : plsb_s = "zero"; break;
            case H5T_PAD_ONE       : plsb_s = "one"; break;
            case H5T_PAD_BACKGROUND: plsb_s = "bkg"; break;
            case H5T_PAD_ERROR     :
            case H5T_NPAD          : plsb_s = "unknown"; break;
            default                : break;
            }
        }
        if ((unsigned)H5Tget_offset(type) + prec < 8 * H5Tget_size(type)) {
            switch (pmsb) {
            case H5T_PAD_ZERO      : pmsb_s = "zero"; break;
            case H5T_PAD_ONE       : pmsb_s = "one"; break;
            case H5T_PAD_BACKGROUND: pmsb_s = "bkg"; break;
            case H5T_PAD_ERROR     :
            case H5T_NPAD          : pmsb_s = "unknown"; break;
            default                : break;
            }
        }
        if (plsb_s || pmsb_s) {
...
static bool print_float_type(h5tools_str_t* buffer, hid_t type, int ind)
{
    ...
    switch (norm) {
    case H5T_NORM_IMPLIED: norm_s = ", msb implied"; break;
    case H5T_NORM_MSBSET : norm_s = ", msb always set"; break;
    case H5T_NORM_NONE   : norm_s = ", no normalization"; break;
    case H5T_NORM_ERROR  : norm_s = ", unknown normalization"; break;
    default              :; break;
    }
    h5tools_str_append(buffer, "\n%*s(significant for %lu bit%s at bit %lu%s)", ind, "", (unsigned long)msize, 1 == msize ? "" : "s", (unsigned long)mpos, norm_s);
    h5tools_str_append(buffer,
    ...
    if ((1 + esize + msize) < H5Tget_precision(type)) {
        ...
        switch (pad) {
        case H5T_PAD_ZERO      : pad_s = "zero"; break;
        case H5T_PAD_ONE       : pad_s = "one"; break;
        case H5T_PAD_BACKGROUND: pad_s = "bkg"; break;
        case H5T_PAD_ERROR     :
        case H5T_NPAD          : pad_s = "unknown"; break;
        default                :; break;
        }
 ...
 *-------------------------------------------------------------------------
 */
static bool print_cmpd_type(h5tools_str_t* buffer, hid_t type, int ind)
{
 ...
 *-------------------------------------------------------------------------
 */
static bool print_enum_type(h5tools_str_t* buffer, hid_t type, int ind)
{
    int nmembs;  /* number of members */
 ...
 *-------------------------------------------------------------------------
 */
static bool print_string_type(h5tools_str_t* buffer, hid_t type, int H5_ATTR_UNUSED ind)
{
    H5T_str_t pad;
    ...
    switch (pad) {
    case H5T_STR_NULLTERM   : pad_s = "null-terminated"; break;
    case H5T_STR_NULLPAD    : pad_s = "null-padded"; break;
    case H5T_STR_SPACEPAD   : pad_s = "space-padded"; break;
    ...
    case H5T_STR_ERROR      : pad_s = "unknown-format"; break;
    default                 :; break;
    }
    ...
    switch (cset) {
    case H5T_CSET_ASCII      : cset_s = "ASCII"; break;
    case H5T_CSET_UTF8       : cset_s = "UTF-8"; break;
    ...
    case H5T_CSET_ERROR      : cset_s = "unknown-character-set"; break;
    default                  :; break;
    }
 ...
 *-------------------------------------------------------------------------
 */
static bool print_reference_type(h5tools_str_t* buffer, hid_t type, int H5_ATTR_UNUSED ind)
{
 ...
 *-------------------------------------------------------------------------
 */
static bool print_opaque_type(h5tools_str_t* buffer, hid_t type, int ind)
{
    char* tag;
    size_t size;
 ...
 *-------------------------------------------------------------------------
 */
static bool print_vlen_type(h5tools_str_t* buffer, hid_t type, int ind)
{
    hid_t super;
 ...
 *---------------------------------------------------------------------------
 */
static bool print_array_type(h5tools_str_t* buffer, hid_t type, int ind)
{
    hid_t super;
    int ndims, i;
    hsize_t* dims = NULL;
 ...
 *-------------------------------------------------------------------------
 */
static bool print_bitfield_type(h5tools_str_t* buffer, hid_t type, int ind)
{
    H5T_order_t order;          /* byte order value */
    const char* order_s = NULL; /* byte order string */
 ...
 *-------------------------------------------------------------------------
 */
static bool print_complex_type(h5tools_str_t* buffer, hid_t type, int ind)
{
    hid_t super;
...
static void print_type(h5tools_str_t* buffer, hid_t type, int ind)
{
    ...
    /* Print the type */
    if (print_native_type(buffer, type, ind) || print_specific_float_type(buffer, type, ind) || print_complex_type(buffer, type, ind) ||
        print_cmpd_type(buffer, type, ind) || print_enum_type(buffer, type, ind) || print_string_type(buffer, type, ind) ||
        print_reference_type(buffer, type, ind) || print_vlen_type(buffer, type, ind) || print_array_type(buffer, type, ind) ||
 ...
 *-------------------------------------------------------------------------
 */
static void dump_dataset_values(hid_t dset)
{
    ...
    if (H5Tget_class(f_type) == H5T_REFERENCE) {
        ...
        if (info->ascii) {
            ...
            h5tools_render_element(rawoutstream, info, &ctx, &buffer, &curr_pos, (size_t)ctx.cur_column + 1, (hsize_t)0, (hsize_t)0);
        }
    }
done:
    H5Sclose(space);
 ...
 *-------------------------------------------------------------------------
 */
static void dump_attribute_values(hid_t attr)
{
    hid_t f_type = H5I_INVALID_HID;
    hid_t space = H5I_INVALID_HID;
    hsize_t total_size[H5S_MAX_RANK];
 ...
 *-------------------------------------------------------------------------
 */
static herr_t list_attr(hid_t obj, const char* attr_name, const H5A_info_t H5_ATTR_UNUSED* ainfo, void H5_ATTR_UNUSED* op_data)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t dataset_list1(hid_t dset)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t dataset_list2(hid_t dset, const char H5_ATTR_UNUSED* name)
{
    ...
    if (verbose_g > 0) {
            ...
            if ((nf = H5Pget_external_count(dcpl)) > 0) {
                ...
                for (i = 0, total = 0; i < nf; i++) {
                    if (H5Pget_external(dcpl, (unsigned)i, sizeof(f_name), f_name, &f_offset, &f_size) < 0) {
                        ...
                        h5tools_str_append(&buffer, "        #%03d %10" PRIuHSIZE " %10" PRIuHSIZE " %10s ", i, total, (hsize_t)f_offset, "INF");
                        print_string(&buffer, f_name, true);
                    }
                    else {
                        h5tools_str_append(&buffer, "        #%03d %10" PRIuHSIZE " %10" PRIuHSIZE " %10" PRIuHSIZE " ", i, total, (hsize_t)f_offset, f_size);
                        print_string(&buffer, f_name, true);
                    }
                    h5tools_str_append(&buffer, "\n");
                    total += f_size;
        ...
        case H5D_NLAYOUTS:
        default              : h5tools_str_append(&buffer, "layout information not available"); break;
        }
        ...
        switch (tclass) {
        case H5T_VLEN: h5tools_str_append(&buffer, "information not available"); break;
        ...
        case H5T_NCLASSES:
        default:
            h5tools_str_append(&buffer, "%" PRIuHSIZE " logical byte%s, %" PRIuHSIZE " allocated byte%s", total, 1 == total ? "" : "s", used, 1 == used ? "" : "s");
            if (used > 0) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t datatype_list2(hid_t type, const char H5_ATTR_UNUSED* name)
{
    if (verbose_g > 0) {
        hsize_t curr_pos = 0;  /* total data element position   */
        h5tools_str_t buffer;  /* string into which to render   */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t list_obj(const char* name, const H5O_info2_t* oinfo, bool already_visited, const trav_seen_t* visited_obj_info, void* _iter)
{
 ...
 *-------------------------------------------------------------------------
 */
static herr_t list_lnk(const char* name, const H5L_info2_t* linfo, void* _iter)
{
    char* buf = NULL;
    iter_t* iter = (iter_t*)_iter;
    int ret;
...
static herr_t visit_obj(hid_t file, const char* oname, iter_t* iter)
{
    ...
    if (H5O_TYPE_GROUP == oi.type && !grp_literal_g) {
        ...
        h5trav_visit(file, oname, (bool)(display_root_g || iter->symlink_target), recursive_g, list_obj, list_lnk, iter, H5O_INFO_BASIC | H5O_INFO_TIME);
 ...
 *              Failure: false (0)
 *-------------------------------------------------------------------------*/
static bool is_valid_args(void)
{
    bool ret = true;
    ...
    for (argno = 1; argno < argc && '-' == argv[argno][0]; argno++) {
        if (!strcmp(argv[argno], "--")) {
...
#else
            fprintf(rawerrorstream,
                    "Error: Read-Only S3 VFD is not available unless enabled when HDF5 is "
            ...
            for (s = argv[argno] + 1; *s; s++) {
                ...
                case 'a': /* --address */ address_g = true; break;

                case 'd': /* --data */ data_g = true; break;

                /* deprecated -e */
                case 'e': /* --errors */ enable_error_stack = 1; break;

                case 'E': /* --external */ follow_elink_g = true; break;

                case 'f': /* --full */ fullname_g = true; break;

                case 'g': /* --group */ grp_literal_g = true; break;

                case 'l': /* --label */ label_g = true; break;
                ...
                case 'S': /* --simple */ simple_output_g = true; break;

                case 's': /* --string */ string_g = true; break;

                case 'v': /* --verbose */ verbose_g++; break;
                ...
                case 'x': /* --hexdump */ hexdump_g = true; break;

                default: usage(); leave(EXIT_FAILURE);
                } /* end switch */
            } /* end for */
        }
        else {
...
#else
            error_msg("Error: ROS3 VFD is not available unless enabled when HDF5 is configured and built.\n\n");
            leave(EXIT_FAILURE);
#endif
        }

        while (fname && *fname) {
            file_id = h5tools_fopen(fname, H5F_ACC_RDONLY, fapl_id, (custom_vol_fapl || custom_vfd_fapl), drivername, sizeof drivername);
