...
static herr_t do_write(results* res, file_descr* fd, parameters* parms, long ndsets, off_t nelmts, size_t buf_size, void* buffer);
static herr_t do_read(results* res, file_descr* fd, parameters* parms, long ndsets, off_t nelmts, size_t buf_size, void* buffer /*out*/);
 ...
 * Return:          results
 */
results do_pio(parameters param)
{
    /* return codes */
...
static char* pio_create_filename(iotype iot, const char* base_name, char* fullname, size_t size)
{
    ...
    switch (iot) {
    case POSIXIO: suffix = ".posix"; break;
    case MPIO   : suffix = ".mpio"; break;
    case PHDF5  : suffix = ".h5"; break;
    default     : break;
    }
 ...
 */
static herr_t do_write(results* res, file_descr* fd, parameters* parms, long ndsets, off_t nbytes, size_t buf_size, void* buffer)
{
    ...
    bytes_count = (off_t)(((double)nbytes * (pio_mpi_rank_g + 1)) / pio_mpi_nprocs_g) - (off_t)(((double)nbytes * pio_mpi_rank_g) / pio_mpi_nprocs_g);
    ...
    case MPIO: /* MPI-I/O setup */
        ...
        if (!parms->dim2d) {
            ...
            mrc = MPI_Type_vector((int)(buf_size / blk_size), (int)1, (int)pio_mpi_nprocs_g, mpi_blk_type, &mpi_file_type);
            VRFY((mrc == MPI_SUCCESS), "MPIO_TYPE_CREATE");
        ...
        /* 2D dataspace */
            ...
            mrc = MPI_Type_vector((int)blk_size, (int)1, (int)((size_t)snbytes / buf_size), mpi_partial_buffer_cont, &mpi_cont_type);
            VRFY((mrc == MPI_SUCCESS), "MPIO_TYPE_CREATE");
            ...
            mrc = MPI_Type_vector((int)buf_size, (int)1, (int)((size_t)snbytes / blk_size), mpi_partial_buffer_inter, &mpi_inter_type);
            VRFY((mrc == MPI_SUCCESS), "MPIO_TYPE_CREATE");
            ...
            mrc = MPI_Type_vector((int)(buf_size / blk_size), (int)1, (int)((size_t)snbytes / blk_size), mpi_full_chunk, &mpi_chunk_inter_type);
            VRFY((mrc == MPI_SUCCESS), "MPIO_TYPE_CREATE");
            ...
            case POSIXIO:
                ...
                /* 2D dataspace */
                    ...
                    if (!parms->h5_use_chunks) {
                        ...
                        if (!parms->interleaved) {
                            /* Compute file offset */
                            file_offset = posix_file_offset + (off_t)((((size_t)nbytes_xfer / blk_size) / (size_t)snbytes) * (blk_size * (size_t)snbytes) +
                        ...
                        /* Interleaved access pattern */
                            ...
                            file_offset = posix_file_offset +
                                          (off_t)(((((size_t)nbytes_xfer / buf_size) * (size_t)pio_mpi_nprocs_g) / (size_t)snbytes) * (buf_size * (size_t)snbytes) +
                    ...
                    /* Chunked storage */
                        ...
                        /*Interleaved access pattern */
                            ...
                            file_offset = posix_file_offset +
                                          (off_t)((((size_t)nbytes_xfer / (buf_size / blk_size) * (size_t)pio_mpi_nprocs_g) / ((size_t)snbytes * blk_size)) *
                                                      (buf_size * (size_t)snbytes) +
                                                  (((size_t)nbytes_xfer / (buf_size / blk_size)) * (size_t)pio_mpi_nprocs_g) % ((size_t)snbytes * blk_size));
                    ...
                    while (nbytes_toxfer > 0) {
                        ...
                        /* check if all bytes are written */
                        rc = ((ssize_t)nbytes_xfer_advance == POSIXWRITE(fd->posixfd, buf_p, nbytes_xfer_advance));
                        VRFY((rc != 0), "POSIXWRITE");
            ...
            case MPIO:
                ...
                if (!parms->dim2d) {
                    ...
                    if (!parms->collective) {
                        ...
                        if (!parms->interleaved) {
                            ...
                            mrc = MPI_File_write_at(fd->mpifd, mpi_offset, buffer, (int)(buf_size / blk_size), mpi_blk_type, &mpi_status);
                        ...
                        /* Interleaved access pattern */
                            ...
                            while (nbytes_toxfer > 0) {
                                ...
                                /* Perform independent write */
                                mrc = MPI_File_write_at(fd->mpifd, mpi_offset, buf_p, (int)1, mpi_blk_type, &mpi_status);
                                VRFY((mrc == MPI_SUCCESS), "MPIO_WRITE");
                    ...
                    /* Collective file access */
                        ...
                        if (!parms->interleaved) {
                            ...
                            /* Perform independent write */
                            mrc = MPI_File_write_at_all(fd->mpifd, mpi_offset, buffer, (int)(buf_size / blk_size), mpi_blk_type, &mpi_status);
                            VRFY((mrc == MPI_SUCCESS), "MPIO_WRITE");
                        ...
                        /* Interleaved access pattern */
                            ...
                            /* Set the file view */
                            mrc = MPI_File_set_view(fd->mpifd, mpi_offset, mpi_blk_type, mpi_file_type, "native", h5_io_info_g);
                            VRFY((mrc == MPI_SUCCESS), "MPIO_VIEW");

                            /* Perform write */
                            mrc = MPI_File_write_at_all(fd->mpifd, 0, buffer, (int)(buf_size / blk_size), mpi_blk_type, &mpi_status);
                            VRFY((mrc == MPI_SUCCESS), "MPIO_WRITE");
                ...
                /* 2D dataspace */
                    ...
                    if (!parms->h5_use_chunks) {
                        ...
                        if (!parms->interleaved) {
                            /* Compute offset in file */
                            mpi_offset = mpi_file_offset + (MPI_Offset)((((size_t)nbytes_xfer / blk_size) / (size_t)snbytes) * (blk_size * (size_t)snbytes)) +
                        ...
                        /* Interleaved access pattern */
                            ...
                            mpi_offset =
                                mpi_file_offset +
                                (MPI_Offset)(((((size_t)nbytes_xfer / buf_size) * (size_t)pio_mpi_nprocs_g) / (size_t)snbytes) * (buf_size * (size_t)snbytes)) +
                                (MPI_Offset)((((size_t)nbytes_xfer / buf_size) * (size_t)pio_mpi_nprocs_g) % (size_t)snbytes);
                    ...
                    if (!parms->collective) {
                        ...
                        while (nbytes_toxfer > 0) {
                            /* Perform independent write */
                            mrc = MPI_File_write_at(fd->mpifd, mpi_offset, buf_p, (int)nbytes_xfer_advance, MPI_BYTE, &mpi_status);
                            VRFY((mrc == MPI_SUCCESS), "MPIO_WRITE");
                    ...
                    /* Common code for collective file access */
                        ...
                        /* Set the file view */
                        mrc = MPI_File_set_view(fd->mpifd, mpi_offset, MPI_BYTE, mpi_collective_type, "native", h5_io_info_g);
                        VRFY((mrc == MPI_SUCCESS), "MPIO_VIEW");
 ...
 * Return:          SUCCESS or FAIL
 */
static herr_t do_read(results* res, file_descr* fd, parameters* parms, long ndsets, off_t nbytes, size_t buf_size, void* buffer /*out*/)
{
    int ret_code = SUCCESS;
            ...
            case POSIXIO:
                ...
                /* 2D dataspace */
                    ...
                    while (nbytes_toxfer > 0) {
                        ...
                        /* check if all bytes are read */
                        rc = ((ssize_t)nbytes_xfer_advance == POSIXREAD(fd->posixfd, buf_p, nbytes_xfer_advance));
                        VRFY((rc != 0), "POSIXREAD");
            ...
            case MPIO:
                ...
                if (!parms->dim2d) {
                    ...
                    if (!parms->collective) {
                        ...
                        if (!parms->interleaved) {
                            ...
                            mrc = MPI_File_read_at(fd->mpifd, mpi_offset, buffer, (int)(buf_size / blk_size), mpi_blk_type, &mpi_status);
                        ...
                        /* Interleaved access pattern */
                            ...
                            while (nbytes_toxfer > 0) {
                                ...
                                /* Perform independent read */
                                mrc = MPI_File_read_at(fd->mpifd, mpi_offset, buf_p, (int)1, mpi_blk_type, &mpi_status);
                                VRFY((mrc == MPI_SUCCESS), "MPIO_READ");
                    ...
                    /* Collective file access */
                        ...
                        if (!parms->interleaved) {
                            ...
                            mrc = MPI_File_read_at_all(fd->mpifd, mpi_offset, buffer, (int)(buf_size / blk_size), mpi_blk_type, &mpi_status);
                            VRFY((mrc == MPI_SUCCESS), "MPIO_READ");
                        ...
                        /* Interleaved access pattern */
                            ...
                            /* Perform collective read */
                            mrc = MPI_File_read_at_all(fd->mpifd, 0, buffer, (int)(buf_size / blk_size), mpi_blk_type, &mpi_status);
                            VRFY((mrc == MPI_SUCCESS), "MPIO_READ");
                ...
                /* 2D dataspace */
                    ...
                    if (!parms->collective) {
                        ...
                        while (nbytes_toxfer > 0) {
                            /* Perform independent read */
                            mrc = MPI_File_read_at(fd->mpifd, mpi_offset, buf_p, (int)nbytes_xfer_advance, MPI_BYTE, &mpi_status);
                            VRFY((mrc == MPI_SUCCESS), "MPIO_READ");
                    ...
                    /* Common code for collective file access */
                        ...
                        /* Perform read */
                        MPI_File_read_at_all(fd->mpifd, 0, buffer, (int)(buf_size * blk_size), MPI_BYTE, &mpi_status);
                        VRFY((mrc == MPI_SUCCESS), "MPIO_READ");
...
static herr_t do_fopen(parameters* param, char* fname, file_descr* fd /*out*/, int flags)
{
    ...
    case MPIO:
        if (flags & (PIO_CREATE | PIO_WRITE)) {
            ...
            mrc = MPI_File_open(pio_comm_g, fname, MPI_MODE_CREATE | MPI_MODE_RDWR, h5_io_info_g, &fd->mpifd);
 ...
 * Return:      SUCCESS or FAIL
 */
static herr_t do_fclose(iotype iot, file_descr* fd /*out*/)
{
...
static void do_cleanupfile(iotype iot, char* fname)
{
    ...
    if (clean_file_g) {
        switch (iot) {
        case POSIXIO: HDremove(fname); break;
        case MPIO   :
        case PHDF5  : MPI_File_delete(fname, h5_io_info_g); break;
        default     : break;
        }
    }
}
