...
static struct options* parse_command_line(int argc, const char* const* argv)
{
    ...
    while ((opt = H5_get_option(argc, argv, s_opts, l_opts)) != EOF) {
        ...
        case 'G': cl_opts->page_size = parse_size_directive(H5_optarg); break;
        case 'b': cl_opts->page_buffer_size = parse_size_directive(H5_optarg); break;
        case 'A':
            {
                ...
                while (end && *end != '\0') {
                    ...
                    if (strlen(buf) > 1 || isdigit(buf[0])) {
                        ...
                        case 'v':
                            ...
                            cl_opts->verify = true;
                            break;
                        default: fprintf(stderr, "sio_perf: invalid --debug option %s\n", buf); exit(EXIT_FAILURE);
                        }
                    }
        ...
        case 't': cl_opts->h5_extendable = true; break;
        case 'x':
            {
                const char* end = H5_optarg;
                int j = 0;
