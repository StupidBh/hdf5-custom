 ...
 *-------------------------------------------------------------------------
 */
static int get_hyperslab(hid_t dcpl_id, int rank_dset, const hsize_t dims_dset[], size_t size_datum, hsize_t dims_hslab[], hsize_t* hslab_nbytes_p);
static void print_dataset_info(hid_t dcpl_id, char* objname, double per, int pr, pack_opt_t* options, double read_time, double write_time);
...
int copy_objects(const char* fnamein, const char* fnameout, pack_opt_t* options)
{
     ...
     *-------------------------------------------------------------------------
     */
    if ((fidin = h5tools_fopen(fnamein, H5F_ACC_RDONLY, options->fin_fapl, (options->fin_vol || options->fin_vfd), NULL, (size_t)0)) < 0) {
        ...
        if (options->low_bound >= H5F_LIBVER_V18) {
            ...
            for (i = 0; i < 5; i++) {
                if (options->msg_size[i] > 0) {
                    switch (i) {
                    case 0: mesg_type_flags[nindex] = H5O_SHMESG_SDSPACE_FLAG; break;

                    case 1: mesg_type_flags[nindex] = H5O_SHMESG_DTYPE_FLAG; break;

                    case 2: mesg_type_flags[nindex] = H5O_SHMESG_FILL_FLAG; break;

                    case 3: mesg_type_flags[nindex] = H5O_SHMESG_PLINE_FLAG; break;

                    case 4: mesg_type_flags[nindex] = H5O_SHMESG_ATTR_FLAG; break;
            ...
            if (nindex > 0) {
                if (H5Pset_shared_mesg_nindexes(fcpl, nindex) < 0) {
                    H5TOOLS_GOTO_ERROR((-1),
                                       "H5Pset_shared_mesg_nindexes failed to set the number of shared "
                ...
                for (i = 0; i < (nindex - 1); i++) {
                    if (H5Pset_shared_mesg_index(fcpl, i, mesg_type_flags[i], min_mesg_sizes[i]) < 0) {
                        H5TOOLS_GOTO_ERROR((-1),
                                           "H5Pset_shared_mesg_index failed to configure the specified "
...
int get_hyperslab(hid_t dcpl_id, int rank_dset, const hsize_t dims_dset[], size_t size_datum, hsize_t dims_hslab[], hsize_t* hslab_nbytes_p)
{
    int k;
...
int do_copy_objects(hid_t fidin, hid_t fidout, trav_table_t* travt, pack_opt_t* options) /* repack options */
{
    hid_t grp_in = H5I_INVALID_HID;                                                      /* group ID */
    ...
    if (travt->objs) {
        for (i = 0; i < travt->nobjs; i++) {
            ...
            case H5TRAV_TYPE_DATASET:
                {
                     ...
                     *-------------------------------------------------------------------------
                     */
                    use_h5ocopy = !(options->op_tbl->nelems || options->all_filter == 1 || options->all_layout == 1 || is_ref || is_vlen || is_named);
                     ...
                     */
                    if (use_h5ocopy && (options->fin_vol || options->fout_vol || options->fin_vfd || options->fout_vfd)) {
                        hid_t in_vol_id;
                        hid_t out_vol_id;
                        ...
                        if (h5tools_canreadf((travt->objs[i].name), dcpl_in) == 1) {
                            ...
                            if (H5T_REFERENCE != H5Tget_class(wtype_id)) {
                                ...
                                if (dset_out == H5I_INVALID_HID) {
                                    ...
                                    if (options->verbose > 0) {
                                        printf(
                                            " warning: could not create dataset <%s>. Applying original "
                                            "settings\n",
                                            travt->objs[i].name);
                                    }
                                ...
                                if (nelmts > 0 && space_status != H5D_SPACE_STATUS_NOT_ALLOCATED) {
                                    ...
                                    if (buf != NULL) {
                                        ...
                                        for (elmtno = 0; elmtno < p_nelmts; elmtno += hs_select_nelmts) {
                                            ...
                                            if (H5Dwrite(dset_out, wtype_id, hslab_space, f_space_id, dxpl_id, hslab_buf) < 0) {
                                                H5TOOLS_GOTO_ERROR((-1), "H5Dwrite failed");
                                            }
                                            if (options->verbose == 2) {
                                                H5_timer_stop(&timer);
                                ...
                                if (options->verbose > 0) {
                                    ...
                                    if (apply_s && apply_f && req_filter) {
                                        ...
                                        print_dataset_info(dcpl_out, travt->objs[i].name, ratio, 1, options, read_time, write_time);
                                    }
                                    else {
                                        print_dataset_info(dcpl_in, travt->objs[i].name, ratio, 0, options, read_time, write_time);
                                    }
                                    ...
                                    if (has_filter && apply_s == 0) {
                                        printf(
                                            " <warning: filter not applied to %s. dataset smaller than "
                                            "%d bytes>\n",
                                            travt->objs[i].name,
            ...
            default: H5TOOLS_GOTO_ERROR((-1), "Object type not found");
            } /* switch */
 ...
 *-------------------------------------------------------------------------
 */
static void print_dataset_info(hid_t dcpl_id, char* objname, double ratio, int pr, pack_opt_t* options, double read_time, double write_time)
{
    char strfilter[255];
    ...
    for (i = 0; i < nfilters; i++) {
        ...
        if ((filtn = H5Pget_filter2(dcpl_id, (unsigned)i, &filt_flags, &cd_nelmts, cd_values, sizeof(f_objname), f_objname, NULL)) < 0) {
            strcat(strfilter, "ERROR ");
            continue;
        }

        switch (filtn) {
        case H5Z_FILTER_NONE: strcat(strfilter, "NONE "); break;

        case H5Z_FILTER_DEFLATE: strcat(strfilter, "GZIP ");
        ...
        case H5Z_FILTER_SZIP: strcat(strfilter, "SZIP ");
        ...
        case H5Z_FILTER_SHUFFLE: strcat(strfilter, "SHUF "); break;

        case H5Z_FILTER_FLETCHER32: strcat(strfilter, "FLET "); break;

        case H5Z_FILTER_NBIT: strcat(strfilter, "NBIT "); break;

        case H5Z_FILTER_SCALEOFFSET: strcat(strfilter, "SCALEOFFSET "); break;

        default: strcat(strfilter, "UD "); break;
        } /* end switch */
    } /* end for each filter */
