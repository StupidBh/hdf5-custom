...
static herr_t update_ref_value(hid_t obj_id, H5R_type_t ref_type, void* ref_in, hid_t fid_out, void* ref_out, trav_table_t* travt);
...
int do_copy_refobjs(hid_t fidin, hid_t fidout, trav_table_t* travt, pack_opt_t* options) /* repack options */
{
    ...
    for (i = 0; i < travt->nobjs; i++) {
        ...
        case H5TRAV_TYPE_UDLINK : H5TOOLS_GOTO_ERROR((-1), "H5TRAV invalid type"); break;
...
static int copy_refs_attr(hid_t loc_in, hid_t loc_out, trav_table_t* travt, hid_t fidout) /* for saving references */
{
                ...
                for (i = 0; i < (unsigned)nelmts; i++) {
                    for (j = 0; j < (unsigned)ref_comp_field_n; j++) {
                        if (ref_comp_size[j] == H5R_OBJ_REF_BUF_SIZE) {
                            ...
                            if (update_ref_value(attr_id, H5R_OBJECT, (hobj_ref_t*)((void*)(((char*)buf) + idx)), fidout, &ref_out, travt) <
                                0) { /* Extra (void *) cast to quiet "cast to create
...
static const char* MapIdToName(hid_t refobj_id, trav_table_t* travt)
{
    ...
    for (u = 0; u < travt->nobjs; u++) {
        if (travt->objs[u].type == (h5trav_type_t)H5O_TYPE_DATASET || travt->objs[u].type == (h5trav_type_t)H5O_TYPE_GROUP ||
