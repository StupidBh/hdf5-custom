 ...
 *-------------------------------------------------------------------------
 */
static const H5B2_class_t* get_H5B2_class(const uint8_t* sig)
{
    H5B2_subid_t subtype = (H5B2_subid_t)sig[H5_SIZEOF_MAGIC + 1];
    const H5B2_class_t* cls = NULL;

    switch (subtype) {
    case H5B2_TEST_ID: cls = H5B2_TEST; break;

    case H5B2_FHEAP_HUGE_INDIR_ID: cls = H5HF_HUGE_BT2_INDIR; break;

    case H5B2_FHEAP_HUGE_FILT_INDIR_ID: cls = H5HF_HUGE_BT2_FILT_INDIR; break;

    case H5B2_FHEAP_HUGE_DIR_ID: cls = H5HF_HUGE_BT2_DIR; break;

    case H5B2_FHEAP_HUGE_FILT_DIR_ID: cls = H5HF_HUGE_BT2_FILT_DIR; break;

    case H5B2_GRP_DENSE_NAME_ID: cls = H5G_BT2_NAME; break;

    case H5B2_GRP_DENSE_CORDER_ID: cls = H5G_BT2_CORDER; break;

    case H5B2_SOHM_INDEX_ID: cls = H5SM_INDEX; break;

    case H5B2_ATTR_DENSE_NAME_ID: cls = H5A_BT2_NAME; break;

    case H5B2_ATTR_DENSE_CORDER_ID: cls = H5A_BT2_CORDER; break;

    case H5B2_CDSET_ID: cls = H5D_BT2; break;

    case H5B2_CDSET_FILT_ID: cls = H5D_BT2_FILT; break;

    case H5B2_TEST2_ID: cls = H5B2_TEST2; break;

    case H5B2_NUM_BTREE_ID:
    default               : fprintf(stderr, "Unknown v2 B-tree subtype %u\n", (unsigned)(subtype));
    } /* end switch */
 ...
 *-------------------------------------------------------------------------
 */
static const H5EA_class_t* get_H5EA_class(const uint8_t* sig)
{
    H5EA_cls_id_t clsid = (H5EA_cls_id_t)sig[H5_SIZEOF_MAGIC + 1];
    const H5EA_class_t* cls = NULL;

    switch (clsid) {
    case H5EA_CLS_TEST_ID: cls = H5EA_CLS_TEST; break;

    case H5EA_CLS_CHUNK_ID: cls = H5EA_CLS_CHUNK; break;

    case H5EA_CLS_FILT_CHUNK_ID: cls = H5EA_CLS_FILT_CHUNK; break;

    case H5EA_NUM_CLS_ID:
    default             : fprintf(stderr, "Unknown extensible array class %u\n", (unsigned)(clsid));
    } /* end switch */
 ...
 *-------------------------------------------------------------------------
 */
static const H5FA_class_t* get_H5FA_class(const uint8_t* sig)
{
    H5FA_cls_id_t clsid = (H5FA_cls_id_t)sig[H5_SIZEOF_MAGIC + 1];
    const H5FA_class_t* cls = NULL;

    switch (clsid) {
    case H5FA_CLS_TEST_ID: cls = H5FA_CLS_TEST; break;

    case H5FA_CLS_CHUNK_ID: cls = H5FA_CLS_CHUNK; break;

    case H5FA_CLS_FILT_CHUNK_ID: cls = H5FA_CLS_FILT_CHUNK; break;

    case H5FA_NUM_CLS_ID:
    default             : fprintf(stderr, "Unknown fixed array class %u\n", (unsigned)(clsid));
    } /* end switch */
    ...
    if (!memcmp(sig, H5F_SIGNATURE, (size_t)H5F_SIGNATURE_LEN)) {
        ...
        case H5B_CHUNK_ID:
            ...
            if (extra_count == 0 || extra[0] == 0) {
                ...
                fprintf(stderr,
                        "\th5debug <filename> <B-tree node address> <# of dimensions> <slowest "
            ...
            for (u = 0; u < ndims; u++) {
                if (0 == dim[u]) {
                    ...
                    fprintf(stderr, "B-tree chunked storage node usage:\n");
                    fprintf(stderr,
                            "\th5debug <filename> <B-tree node address> <# of dimensions> "
        ...
        if ((cls == H5D_BT2 || cls == H5D_BT2_FILT) && (extra_count == 0 || extra[0] == 0)) {
            fprintf(stderr,
                    "ERROR: Need v2 B-tree header address and object header address containing the "
        ...
        if ((cls == H5D_BT2 || cls == H5D_BT2_FILT) && (extra_count == 0 || extra[0] == 0 || extra[1] == 0 || extra[2] == 0 || extra[3] == 0)) {
            ...
            fprintf(stderr,
                    "\th5debug <filename> <internal node address> <v2 B-tree header address> "
            ...
            fprintf(stderr,
                    "ERROR: Need v2 B-tree header address and the node's number of records and "
        ...
        if ((cls == H5D_BT2 || cls == H5D_BT2_FILT) && (extra_count == 0 || extra[0] == 0 || extra[1] == 0 || extra[2] == 0)) {
            fprintf(stderr,
                    "ERROR: Need v2 B-tree header address, number of records, and object header "
                    "address containing the layout message in order to dump leaf node\n");
            fprintf(stderr, "v2 B-tree leaf node usage:\n");
            fprintf(stderr,
                    "\th5debug <filename> <leaf node address> <v2 B-tree header address> <number "
        ...
        if (extra_count == 0 || extra[0] == 0 || extra[1] == 0) {
            fprintf(stderr,
                    "ERROR: Need fractal heap header address and size of direct block in order to "
                    "dump direct block\n");
        ...
        if (extra_count == 0 || extra[0] == 0 || extra[1] == 0) {
            fprintf(stderr,
                    "ERROR: Need fractal heap header address and number of rows in order to dump "
                    "indirect block\n");
        ...
        if (extra_count == 0 || extra[0] == 0 || extra[1] == 0) {
            fprintf(stderr,
                    "ERROR: Need free space header address and client address in order to dump "
                    "serialized sections\n");
            fprintf(stderr, "Free space serialized sections usage:\n");
            fprintf(stderr,
                    "\th5debug <filename> <serialized sections address> <free space header "
        ...
        if (extra_count == 0 || extra[0] == 0 || extra[1] == 0) {
            fprintf(stderr,
                    "ERROR: Need extensible array header address and object header address "
        ...
        if (extra_count == 0 || extra[0] == 0 || extra[1] == 0 || extra[2] == 0) {
            fprintf(stderr,
                    "ERROR: Need extensible array header address, super block index and object "
            ...
            fprintf(stderr,
                    "\th5debug <filename> <super block address> <array header address> <super "
        ...
        if (extra_count == 0 || extra[0] == 0 || extra[1] == 0 || extra[2] == 0) {
            ...
            fprintf(stderr,
                    "\th5debug <filename> <data block address> <array header address> <# of "
        ...
        if (extra_count == 0 || extra[0] == 0 || extra[1] == 0) {
            fprintf(stderr,
                    "ERROR: Need fixed array header address and object header address containing "
