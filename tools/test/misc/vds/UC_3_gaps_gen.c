...
static herr_t create_3_1_vds(void)
{
    ...
    /* dataset */
    if ((did = H5Dcreate2(fid, UC_3_VDS_DSET_NAME, UC_31_VDS_DATATYPE, vds_sid, H5P_DEFAULT, vds_dcplid, H5P_DEFAULT)) < 0) {
        UC_ERROR
    }
...
static herr_t create_3_2_vds(void)
{
    hid_t src_sid = H5I_INVALID_HID;    /* source dataset's dataspace ID            */
    hid_t vds_sid = H5I_INVALID_HID;    /* VDS dataspace ID                         */
    hid_t vds_dcplid = H5I_INVALID_HID; /* VDS dataset property list ID             */
    ...
    for (i = 0; i < UC_2_N_SOURCES; i++) {
        ...
        if (H5Sselect_hyperslab(vds_sid, H5S_SELECT_SET, UC_32_POSITIONS[i], NULL, UC_2_MAX_DIMS[i], NULL) < 0) {
            UC_ERROR
        }
    ...
    /* dataset */
    if ((did = H5Dcreate2(fid, UC_3_VDS_DSET_NAME, UC_32_VDS_DATATYPE, vds_sid, H5P_DEFAULT, vds_dcplid, H5P_DEFAULT)) < 0) {
        UC_ERROR
    }
