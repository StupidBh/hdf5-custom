...
int main(void)
{
    ...
    for (i = 0; i < UC_5_N_SOURCES; i++) {
        ...
        if ((did = H5Dcreate2(fid, UC_5_SOURCE_DSET_NAME, UC_5_SOURCE_DATATYPE, src_sid, H5P_DEFAULT, src_dcplid, H5P_DEFAULT)) < 0) {
            UC_ERROR
        }
    ...
    /* dataset */
    if ((did = H5Dcreate2(fid, UC_5_VDS_DSET_NAME, UC_5_VDS_DATATYPE, vds_sid, H5P_DEFAULT, vds_dcplid, H5P_DEFAULT)) < 0) {
        UC_ERROR
    }
