...
static size_t counter(unsigned flags, size_t cd_nelmts, const unsigned* cd_values, size_t nbytes, size_t* buf_size, void** buf);
...
static int create_dset1(hid_t file)
{
    ...
    if ((dataset = H5Dcreate2(file, DSET1_NAME, H5T_NATIVE_INT, dataspace, H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0) {
        goto error;
    }
...
static int create_dset2(hid_t file)
{
    ...
    /* Create a new dataset within the file using chunk creation properties.  */
    if ((dataset = H5Dcreate2(file, DSET2_NAME, H5T_NATIVE_INT, dataspace, H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0) {
        goto error;
    }
