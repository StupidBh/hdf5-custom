...
static void write_file(Bytef* source, uLongf sourceLen)
{
    Bytef *d_ptr, *dest;
    ...
    compression_time +=
        ((double)timer_stop.tv_sec + ((double)timer_stop.tv_usec) / MICROSECOND) - ((double)timer_start.tv_sec + ((double)timer_start.tv_usec) / MICROSECOND);
...
static void compress_buffer(Bytef* dest, uLongf* destLen, const Bytef* source, uLong sourceLen)
{
    ...
    *destLen = (uLongf)destLen_sz;
    ...
    if (rc != Z_OK) {
        ...
        case Z_BUF_ERROR   : error("not enough room in the output buffer"); break;
        case Z_STREAM_ERROR: error("level parameter (%d) is invalid", compress_level); break;
        default            : error("unknown compression error"); break;
        }
    }
}
...
static void get_unique_name(void)
{
 ...
 */
static unsigned long parse_size_directive(const char* size)
{
    unsigned long s;
    char* endptr;
    ...
    if (endptr && *endptr) {
        ...
        default : error("illegal size specifier '%c'", *endptr); break;
        }
    }
...
static void fill_with_random_data(Bytef* src, uLongf src_len)
{
    unsigned u;
    h5_stat_t stat_buf;
...
static void do_write_test(unsigned long file_size, unsigned long min_buf_size, unsigned long max_buf_size)
{
...
int main(int argc, char* argv[])
{
    ...
    while ((opt = H5_get_option(argc, (const char* const*)argv, s_opts, l_opts)) > 0) {
        ...
        case '7':
        case '8':
        case '9': compress_level = opt - '0'; break;
        case 'B': max_buf_size = parse_size_directive(H5_optarg); break;
        case 'b': min_buf_size = parse_size_directive(H5_optarg); break;
        case 'c':
            ...
            break;
        case 'p': option_prefix = H5_optarg; break;
        case 'r': random_test = true; break;
        case 's': file_size = parse_size_directive(H5_optarg); break;
        case '?':
            usage();
            exit(EXIT_FAILURE);
            break;
